...
static int docHtmlSaveParticules(const BufferItem *paraNode, int *pNoteRefCount,
				 ...
				 HtmlWritingContext *hwc)
{
	...
	while (part < partUpto) {
		...
		case DOCkindSPAN:
			...
			if (!hwc->hwcInHyperlink || !hwc->hwcInPageref ||
		...
		case DOCkindFIELDHEAD:
			...
			if (fki->fkiIsFieldInRtf &&
				...
				docHtmlStartField(df, hwc, paraNode,
						  textAttrNr);
			}
			...
			if (df->dfKind == DOCfkCHFTN) {
				...
				count = docCountParticulesInField(
					paraNode, &closed, part, partUpto);
				...
				if (paraNode->biTreeType == DOCinBODY) {
					sprintf(scratch, "%d",
						hwc->hwcNoteRefCount + 1);
					hwc->hwcNoteRefCount++;
					noteRefCount++;
				} else {
					sprintf(scratch, "%d",
						hwc->hwcNoteDefCount + 1);
					hwc->hwcNoteDefCount++;
				}
				...
				stroff = tp[done].tpStroff;
				s = docParaString(paraNode, stroff);

				if (paraNode->biTreeType == DOCinBODY &&
				    hwc->hwcInlineNotes) {
					if (utilIndexMappingAppend(
						    &(hwc->hwcDeferredNotes),
						    (int *)0,
						    tp->tpObjectNumber)) {
...
static void docHtmlStartParagraphBody(const BufferItem *paraNode,
				      int *pBulletTab, const char *tag,
				      int fontHeight, HtmlWritingContext *hwc)
{
	...
	if (hwc->hwcSupportsBullets && paraNode->biParaLeftIndentTwips >= 100 &&
		...
		for (part = 0; part < paraNode->biParaParticuleCount;
		     tp++, part++) {
			x += tp->tpTwipsWide;
	...
	if ((paraNode->biParaFirstIndentTwips <= -100 ||
		...
		sioOutPrintf(sos, "%dpt;",
			     paraNode->biParaFirstIndentTwips / 20);
		xw->xwColumn += 6;
	}
	...
	docCssEmitBorderStyleByNumber(&(xw->xwColumn), sos, bd, "border-top",
				      paraNode->biParaTopBorderNumber);
	docCssEmitBorderStyleByNumber(&(xw->xwColumn), sos, bd, "border-left",
				      paraNode->biParaLeftBorderNumber);
	docCssEmitBorderStyleByNumber(&(xw->xwColumn), sos, bd, "border-right",
				      paraNode->biParaRightBorderNumber);
	docCssEmitBorderStyleByNumber(&(xw->xwColumn), sos, bd, "border-bottom",
...
int docHtmlSaveParaNode(HtmlWritingContext *hwc, const BufferItem *paraNode,
			...
			const DocumentSelection *ds)
{
	...
	if (bulletTab >= 0) {
		...
		if (docHtmlSaveParticules(paraNode, &noteRefCount, 0, bulletTab,
					  hwc) < 0) {
			LDEB(bulletTab);
			return -1;
		}
	...
	if (hwc->hwcInlineNotes && noteRefCount > 0 &&
	...
	if (docUpdParaProperties((PropertyMask *)0,
				 &(hwc->hwcParagraphProperties), &ppSetMask,
