...
int docSvgDrawSpan(const DrawTextLine *dtl, int part, int count,
		   ...
		   const TextAttribute *ta, const char *printString, int nbLen)
{
	...
	if (nbLen > 0) {
		...
		if (ta->taSuperSub == TEXTvaSUPERSCRIPT) {
			psGetSuperBaseline(&y, spanBaseline, fontSizeTwips,
					   pd->pdAfi);
			textSizeTwips = SUPERSUB_SIZE(fontSizeTwips);
		}

		if (ta->taSuperSub == TEXTvaSUBSCRIPT) {
			psGetSubBaseline(&y, spanBaseline, fontSizeTwips,
					 pd->pdAfi);
			textSizeTwips = SUPERSUB_SIZE(fontSizeTwips);
		}
		...
		if (ta->taTextIsUnderlined) {
			...
			xmlWriteRgb8Attribute(xw, "stroke",
					      &(dc->dcCurrentColor));
			xmlWriteStringAttribute(xw, "text-decoration",
						"underline");
		}
#else
		{
...
int docSvgDrawFtnsep(const DrawTextLine *dtl, int part, int textAttrNr,
...
static int svgDrawTab(DrawingContext *dc, const DrawTextLine *dtl,
		      XmlWriter *xw, const TextAttribute *ta, int x0, int x1,
		      int baseLine, int step, const char *tabProc)
{
...
int docSvgDrawTab(const DrawTextLine *dtl, int part, int textAttrNr,
		  ...
		  const LayoutPosition *baseLine)
{
	...
	case DOCtlDOTS:
		...
		if (ta->taFontIsBold) {
			svgDrawTab(dc, dtl, xw, ta, x0, x1, spanBaseline, 60,
				   "dot-tab-bold");
		} else {
			svgDrawTab(dc, dtl, xw, ta, x0, x1, spanBaseline, 60,
				   "dot-tab");
	...
	case DOCtlUNDERLINE:
		...
		if (ta->taFontIsBold) {
			svgDrawTab(dc, dtl, xw, ta, x0, x1, spanBaseline, 20,
				   "ul-tab-bold");
		} else {
			svgDrawTab(dc, dtl, xw, ta, x0, x1, spanBaseline, 20,
				   "ul-tab");
	...
	case DOCtlHYPH:
		...
		if (ta->taFontIsBold) {
			svgDrawTab(dc, dtl, xw, ta, x0, x1, spanBaseline, 140,
				   "dash-tab-bold");
		} else {
			svgDrawTab(dc, dtl, xw, ta, x0, x1, spanBaseline, 140,
				   "dash-tab");
...
int docSvgDrawTextLine(BufferItem *paraBi, int line, const ParagraphFrame *pf,
		       const DocumentRectangle *drLine, void *vsw,
		       DrawingContext *dc, const BlockOrigin *bo)
{
	...
	dtl.dtlDrawParticulesSeparately = paraBi->biParaAlignment ==
					  DOCthaJUSTIFIED;
