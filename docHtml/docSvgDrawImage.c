...
static int docSvgDrawMetafile(SvgWriter *sw, const PictureProperties *pip,
...
static int docSvgDrawRasterImage(SvgWriter *sw, DrawingContext *dc,
				 ...
				 const DocumentRectangle *drDest)
{
	...
	if (io->ioRasterImage.riBytes) {
		...
		if (!drawRasterImageSvg(sw, &(io->ioRasterImage), &drSel,
					drDest)) {
			return 1;
		}
	}
...
static int docSvgDrawShapeRaster(SvgWriter *sw, DrawingContext *dc,
				 DrawingShape *ds,
				 const DocumentRectangle *drDest)
{
	...
	if (ds->dsRasterImage.riBytes) {
		...
		if (!drawRasterImageSvg(sw, &(ds->dsRasterImage), &drSel,
					drDest)) {
			return 1;
		}
	}
...
int docSvgDrawShapeImage(SvgWriter *sw, DrawingContext *dc, DrawingShape *ds,
...
int docSvgDrawObject(const DrawTextLine *dtl, int part, InsertedObject *io,
		     ...
		     const LayoutPosition *baseLine)
{
	...
	case DOCokPICTPNGBLIP:
		...
		done = docSvgDrawRasterImage(sw, dc, io, io->ioKind,
					     &(io->ioObjectData), &drDest);
		return done;
