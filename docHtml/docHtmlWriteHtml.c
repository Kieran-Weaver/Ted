...
docHtmlOpenImageStream(HtmlWritingContext *hwc, int n, const InsertedObject *io,
		       const char *mimeType, const char *ext);

static int docHtmlGetImageSrc(MemoryBuffer *target, HtmlWritingContext *hwc,
			      int n, const InsertedObject *io, const char *ext);

static int docHtmlGetCssName(MemoryBuffer *target, HtmlWritingContext *hwc);
...
int docHtmlSaveDocument(SimpleOutputStream *sos, BufferDocument *bd,
			const MemoryBuffer *filename, const LayoutContext *lc)
{
	int rval = 0;
	...
	hwc.hwcDocument = bd;
	hwc.hwcInlineCss =
		(bd->bdObjectList.iolPagedList.plItemCount == 0 || !filename);
	hwc.hwcInlineNotes = 0;

	hw.hwFilename = filename;

	if (hw.hwFilename) {
		if (appFileGetRelativeName(&(hw.hwRelativeName),
					   hw.hwFilename)) {
			LDEB(1);
			rval = -1;
			goto ready;
...
static int docHtmlGetNameX(MemoryBuffer *target, const HtmlWriter *hw,
			   int relative, const char *slashName)
{
	if (relative) {
...
static int docHtmlGetImageSrcX(MemoryBuffer *target, HtmlWriter *hw,
			       int relative, int n, const InsertedObject *io,
			       const char *ext)
{
	...
	if (1 || pip->pipBliptag == 0) {
		...
		sprintf(scratch, "/b%08lx.%s", pip->pipBliptag, ext);
	}
...
static int docHtmlGetCssNameX(MemoryBuffer *target, int relative,
			      HtmlWriter *hw)
{
