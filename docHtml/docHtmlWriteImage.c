...
int docHtmlObjectSaveHow(int *pType, const char **pMimeType, const char **pExt,
...
static void docHtmlSetShapeStyle(char *style, HtmlWritingContext *hwc,
...
static void docHtmlStartObject(HtmlWritingContext *hwc,
			       const struct BufferItem *paraNode,
			       const InsertedObject *io, int wide, int high,
			       const char *mimeType, const MemoryBuffer *src)
{
	...
	docHtmlWriteStringAttribute(hwc, "data",
...
static int docHtmlRasterImageSize(int *pWide, int *pHigh, InsertedObject *io)
{
...
int docHtmlSaveImgElement(int *pDone, HtmlWritingContext *hwc, int n,
			  const struct BufferItem *paraNode, InsertedObject *io)
{
	int rval = 0;
	int wide;
	int high;
	...
	case DOCokDRAWING_SHAPE: {
		...
		docHtmlStartObject(hwc, paraNode, io, wide, high, mimeType,
				   &src);
		asObject = 1;
	} break;
...
static int docHtmlSaveDrawingShape(HtmlWritingContext *hwc, int n,
				   InsertedObject *io, const char *mimeType,
				   const char *ext)
{
	int rval = 0;
	...
	if (docSvgSaveShapeObject(sosImage, page, pixelsWide, pixelsHigh, io,
				  bodySectBi, hwc->hwcLayoutContext)) {
		LDEB(1);
		return -1;
	}
...
static int docHtmlPlayMetafileToSvg(HtmlWritingContext *hwc, int n,
				    InsertedObject *io, const MemoryBuffer *mb,
				    ...
				    const char *mimeType, const char *ext)
{
	...
	docSetMetafilePlayer(&mp, sisHex, hwc->hwcLayoutContext, pip, sw.swWide,
			     sw.swHigh);
...
static int docHtmlSaveObjectData(HtmlWritingContext *hwc, int n,
				 InsertedObject *io, const MemoryBuffer *mb,
				 const char *mimeType, const char *ext)
{
	int rval = 0;
...
static int docHtmlSaveImageBytes(HtmlWritingContext *hwc,
				 bmWriteBitmap writeBitmap, int n,
				 InsertedObject *io, const char *mimeType,
				 const char *ext)
{
	int rval = 0;
...
static int docHtmlSaveImage(int n, void *vio, void *vhwc)
{
	...
	case DOCokPICTWMETAFILE:
		if (docHtmlPlayMetafileToSvg(hwc, n, io, mb, appMetaPlayWmfSvg,
					     mimeType, ext)) {
			SDEB(mimeType);
			return -1;
		}
	...
	case DOCokPICTEMFBLIP:
		if (docHtmlPlayMetafileToSvg(hwc, n, io, mb, appMetaPlayEmfSvg,
					     mimeType, ext)) {
			SDEB(mimeType);
			return -1;
		}
	...
	case DOCokMACPICT:
		if (docHtmlPlayMetafileToSvg(hwc, n, io, mb,
					     appMacPictPlayFileSvg, mimeType,
					     ext)) {
			SDEB(mimeType);
			return -1;
		}
	...
	case DOCokPICTJPEGBLIP:
		if (mb) {
			if (docHtmlSaveObjectData(hwc, n, io, mb, mimeType,
						  ext)) {
				SDEB(mimeType);
				return -1;
			}
	...
	switch (type) {
	case DOCokPICTPNGBLIP:
		if (docHtmlSaveImageBytes(hwc, bmPngWritePng, n, io, mimeType,
					  ext)) {
			SDEB(mimeType);
			return -1;
		}
	...
	case DOCokPICTJPEGBLIP:
		if (docHtmlSaveImageBytes(hwc, bmJpegWriteJfif, n, io, mimeType,
					  ext)) {
			SDEB(mimeType);
			return -1;
		}
	...
	case DOCokGIF_FILE:
		if (docHtmlSaveImageBytes(hwc, bmGifWriteGif, n, io, mimeType,
					  ext)) {
			SDEB(mimeType);
			return -1;
		}
...
int docHtmlSaveImages(HtmlWritingContext *hwc)
{
	...
	utilPagedListForAll(&(bd->bdObjectList.iolPagedList), docHtmlSaveImage,
			    (void *)hwc);
