...
void docHtmlPutString(const char *s, HtmlWritingContext *hwc)
{
...
void docHtmlWriteStringAttribute(HtmlWritingContext *hwc, const char *name,
				 const char *value)
{
	xmlWriteStringAttribute(&(hwc->hwcXmlWriter), name, value);
}

void docHtmlWriteIntAttribute(HtmlWritingContext *hwc, const char *name,
			      int value)
{
	xmlWriteIntAttribute(&(hwc->hwcXmlWriter), name, value);
}

void docHtmlEscapeCharacters(HtmlWritingContext *hwc, const char *ss, int len)
{
	xmlEscapeCharacters(&(hwc->hwcXmlWriter), ss, len);
}

static void docHtmlEscapeString(const char *s, HtmlWritingContext *hwc)
{
...
void docHtmlChangeAttributes(HtmlWritingContext *hwc, int taNr)
{
...
int docHtmlStartAnchor(HtmlWritingContext *hwc, int isNote,
		       const MemoryBuffer *fileName,
		       const MemoryBuffer *markName,
		       const MemoryBuffer *refName, const char *title,
		       int titleSize)
{
	...
	if (isNote) {
		docHtmlWriteStringAttribute(hwc, "style",
	...
	if (xw->xwColumn > 5 && xw->xwColumn + needed > 76) {
		docHtmlNewLine(hwc);
		afterSpace = 1;
	}
...
int docHtmlStartField(const DocumentField *df, HtmlWritingContext *hwc,
		      const BufferItem *bi, int attNr)
{
	int rval = 0;
	...
	case DOCfkHYPERLINK:
		...
		if (!hwc->hwcInBookmark && hwc->hwcInHyperlink == 1) {
			if (!docGetHyperlinkField(&hf, df)) {
				...
				docHtmlStartAnchor(hwc, isNote, &(hf.hfFile),
						   &(hf.hfBookmark),
	...
	case DOCfkBOOKMARK:
		...
		if (!docFieldGetBookmark(&mbBookmark, df)) {
			if (df->dfHeadPosition.epParaNr ==
			    ...
			    !hwc->hwcInHyperlink && hwc->hwcInBookmark == 1) {
				const int isNote = 0;

				docHtmlChangeAttributes(hwc, -1);

				docHtmlStartAnchor(hwc, isNote, fileName,
						   markName, mbBookmark, title,
						   titleSize);
...
int docHtmlFinishField(const DocumentField *df, HtmlWritingContext *hwc)
{
	switch (df->dfKind) {
	case DOCfkCHFTN:
	case DOCfkHYPERLINK:
...
void docHtmlEmitBackgroundProperty(const ItemShading *is,
				   HtmlWritingContext *hwc)
{
	...
	if (is->isPattern == DOCspSOLID) {
		...
		if (isFilled) {
			sprintf(scratch, "\"#%02x%02x%02x\"", rgb8.rgb8Red,
...
static void docHtmlSaveMetaElement(HtmlWritingContext *hwc, const char *name,
...
int docHtmlStartDocument(HtmlWritingContext *hwc)
{
	...
	docHtmlWriteStringAttribute(hwc, "xmlns",
	...
	if (!utilMemoryBufferIsEmpty(&(dp->dpTitle))) {
		docHtmlPutString("<title>", hwc);
		docHtmlEscapeString(utilMemoryBufferGetString(&(dp->dpTitle)),
				    hwc);
		docHtmlPutString("</title>", hwc);
		docHtmlNewLine(hwc);
	}
	...
	if (!hwc->hwcInlineCss) {
		...
		docHtmlWriteStringAttribute(hwc, "type",
					    "text/css; charset=UTF-8");
		docHtmlWriteStringAttribute(
			hwc, "href", utilMemoryBufferGetString(&stylesheet));
		docHtmlPutString("/>", hwc);
		docHtmlNewLine(hwc);
	}
	...
	if (!utilMemoryBufferIsEmpty(&(dp->dpHlinkbase))) {
		...
		docHtmlEscapeString(
			utilMemoryBufferGetString(&(dp->dpHlinkbase)), hwc);
		docHtmlPutString("\"/>", hwc);
		docHtmlNewLine(hwc);
	}
...
int docHtmlSaveNotes(HtmlWritingContext *hwc)
{
	...
	for (dfNote = docGetFirstNoteOfDocument(&dn, hwc->hwcDocument, -1);
	     dfNote; dfNote = docGetNextNoteInDocument(&dn, hwc->hwcDocument,
						       dfNote, -1)) {
		DocumentTree *ei;
		...
		if (docHtmlSaveSelection(hwc, ei,
