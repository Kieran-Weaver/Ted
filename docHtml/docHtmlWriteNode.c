...
static int docHtmlStartTable(HtmlWritingContext *hwc, const BufferItem *rowNode,
			     const BufferItem *bodySectNode)
{
	...
	if (rowNode->biChildCount > 1) {
		docCellFrameTwips(
			&(hwc->hwcParagraphFrame), &(hwc->hwcBlockFrame),
	...
	if (rowNode->biParent->biLevel == DOClevCELL) {
		docCellFrameTwips(&(hwc->hwcParagraphFrame),
				  &(hwc->hwcBlockFrame), rowNode->biParent);
	...
	sprintf(scratch, "<table style=\"width:%d%%\" cellspacing=\"0\"",
		width);
...
static int docHtmlFinishTable(HtmlWritingContext *hwc)
{
	...
	if (hwc->hwcInlineNotes && hwc->hwcTableNesting == 0) {
				...
				const BufferItem *cellNode)
{
	...
	docBlockFrameTwips(&bf, cellNode->biChildren[0], bd, page, column);
	...
	if (useStyle) {
		...
		if (is.isPattern == DOCspSOLID) {
			docCssEmitBackgroundStyle(&(xw->xwColumn), sos, bd,
						  &is);
		}
...
static int docHtmlEmitSectColumnStyleProps(const SectionProperties *sp,
					   int balanced, const char *prefix,
					   HtmlWritingContext *hwc)
{
	...
	sprintf(scratch, "%scolumn-count: %d;", prefix, sp->spColumnCount);
	docHtmlPutString(scratch, hwc);
	...
	if (balanced) {
		...
		sprintf(scratch, "%scolumn-fill: auto;", prefix);
	}
	docHtmlPutString(scratch, hwc);
...
static int dochtmlEnterSectNode(BufferItem *node, HtmlWritingContext *hwc)
{
	int rval = 0;
	...
	if (docSectionHasHeaderFooter(node, &applies,
		...
		headerTree = docSectionHeaderFooter(
			node, &applies, &(hwc->hwcDocument->bdProperties),
			DOCinFIRST_HEADER);
...
static int dochtmlLeaveSectNode(BufferItem *node, HtmlWritingContext *hwc)
{
...
static int docHtmlLeaveNode(BufferItem *node, const DocumentSelection *ds,
			    const BufferItem *bodySectNode, void *voidhwc)
{
...
static int docHtmlEnterNode(BufferItem *node, const DocumentSelection *ds,
			    const BufferItem *bodySectNode, void *voidhwc)
{
	HtmlWritingContext *hwc = (HtmlWritingContext *)voidhwc;
	...
	case DOClevCELL:
		if (docIsRowNode(node->biParent)) {
			if (docHtmlEnterCellNode(
				    hwc, bodySectNode, node->biParent,
				    node->biNumberInParent, node)) {
				LDEB(node->biNumberInParent);
				return -1;
			}
		}
		break;
...
int docHtmlSaveSelection(HtmlWritingContext *hwc, DocumentTree *dt,
			 const DocumentSelection *ds)
{
	const int flags = 0;

	if (ds) {
		if (docScanSelection(hwc->hwcDocument, ds, docHtmlEnterNode,
				     docHtmlLeaveNode, flags,
				     (void *)hwc) < 0) {
			LDEB(1);
			return -1;
		}
	} else {
		if (docScanTree(hwc->hwcDocument, dt, docHtmlEnterNode,
				docHtmlLeaveNode, flags, (void *)hwc) < 0) {
			LDEB(1);
			return -1;
		}
	}
