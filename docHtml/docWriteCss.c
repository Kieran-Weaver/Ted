...
int docCssUseBackgroundStyle(const ItemShading *is, const BufferDocument *bd)
{
	int isFilled = 0;
	RGB8Color rgb8;
...
void docCssEmitBackgroundStyle(int *pCol, SimpleOutputStream *sos,
...
static int docCssEmitBorderStyle(int *pCol, SimpleOutputStream *sos,
...
void docCssEmitBorderStyleByNumber(int *pCol, SimpleOutputStream *sos,
				   const BufferDocument *bd,
...
static int docCssSaveTextAttributeStyle(int n, const TextAttribute *ta,
					void *through)
{
	...
	if (docBorderNumberIsBorder(bd, ta->taBorderNumber)) {
		...
		docCssEmitBorderStyleByNumber(&col, at->atSos, bd, "border",
					      ta->taBorderNumber);
	}
	...
	if (docShadingNumberIsShading(bd, ta->taShadingNumber)) {
		...
		if (docCssUseBackgroundStyle(&is, at->atDocument)) {
			docCssEmitBackgroundStyle(&col, at->atSos,
						  at->atDocument, &is);
		}
	}
...
int docCssSaveTextAttributeStyles(SimpleOutputStream *sos, const IndexSet *used,
				  const BufferDocument *bd)
{
