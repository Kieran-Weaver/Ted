...
static int docHtmlSaveDeferredNote(int from, int to, void *vsn)
{
...
int docHtmlStartNote(const DocumentField *df, HtmlWritingContext *hwc,
		     const BufferItem *bi, int attNr)
{
	...
	if (bi->biTreeType == DOCinBODY) {
		...
		utilMemoryBufferPrintf(&mbRef, "_NREF_%d",
				       hwc->hwcNoteRefCount + 1);
		utilMemoryBufferPrintf(&mbDef, "_NDEF_%d",
				       hwc->hwcNoteRefCount + 1);
		...
		if (dn) {
			...
			if (!docTailPosition(&dpLast,
					     dn->dnDocumentTree.dtRoot)) {
				int paraNr =
					docNumberOfParagraph(dpLast.dpNode);

				if (paraNr == 1) {
					title = (char *)docParaString(
						dpLast.dpNode, 0);
					titleSize =
						docParaStrlen(dpLast.dpNode);
				}
			}
		}
	} else {
	...
	docHtmlStartAnchor(hwc, isNote, fileName, markName, refName, title,
			   titleSize);
