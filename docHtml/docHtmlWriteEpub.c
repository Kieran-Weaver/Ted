...
static int docEpubGetImageSrcX(MemoryBuffer *target, int relative, int n,
			       const InsertedObject *io, const char *ext);

static SimpleOutputStream *
docEpubOpenImageStream(HtmlWritingContext *hwc, int n, const InsertedObject *io,
		       const char *mimeType, const char *ext);

static int docEpubGetImageSrc(MemoryBuffer *target, HtmlWritingContext *hwc,
			      int n, const InsertedObject *io, const char *ext);

static int docEpubGetCssName(MemoryBuffer *target, HtmlWritingContext *hwc);
...
static const char DocEpubNameCssAbs[] = DIR_CONTENT "/" DIR_MEDIA "/" NAME_CSS;
...
static int docEpubEmitContainerXml(ZipOutput *zo)
{
	...
	if (!sosMember) {
		SXDEB(nameMember, sosMember);
		rval = -1;
		goto ready;
	}

	for (line = 0; line < sizeof(DocEpubContainerXml) / sizeof(char *);
	     line++) {
...
static void docEpubEmitOpfMetaElement(XmlWriter *xw, const char *id,
				      const char *name,
...
static void docEpubEmitOpfMetaElementIfSet(XmlWriter *xw, const char *id,
					   const char *name,
					   const MemoryBuffer *value)
{
...
static int docEpubEmitMetaData(XmlWriter *xw, const MemoryBuffer *title,
			       ...
			       const BufferDocument *bd)
{
	...
	docEpubEmitOpfMetaElement(xw, DocEpubIdentifierName, "dc:identifier",
				  identifier);

	docEpubEmitOpfMetaElementIfSet(xw, (char *)0, "dc:creator",
				       &(dp->dpAuthor));

	docEpubEmitOpfMetaElementIfSet(xw, (char *)0, "dc:subject",
				       &(dp->dpSubject));
	docEpubEmitOpfMetaElementIfSet(xw, (char *)0, "dc:subject",
				       &(dp->dpKeywords));

	docEpubEmitOpfMetaElementIfSet(xw, (char *)0, "dc:description",
				       &(dp->dpDoccomm));
	docEpubEmitOpfMetaElementIfSet(xw, (char *)0, "dc:publisher",
				       &(dp->dpCompany));
...
static int docEpubEmitManifestItem(XmlWriter *xw, const char *id,
				   const char *href, const char *mediaType)
{
...
static int docEpubEmitSpineItem(XmlWriter *xw, const char *id)
{
...
static int docEpubAddImageToOpf(int n, void *vio, void *vxw)
{
	int rval = 0;
	...
	if (1 || pip->pipBliptag == 0) {
		...
		sprintf(id, "b%08lx", pip->pipBliptag);
	}

	if (docEpubEmitManifestItem(xw, id, utilMemoryBufferGetString(&href),
				    mimeType)) {
		SDEB(id);
		rval = -1;
		goto ready;
...
static int docEpubAddImagesToOpf(const BufferDocument *bd, XmlWriter *xw)
{
...
static const char *DocEpubOpfHead[] = {
	...
	DocEpubIdentifierName,
	"\">\n",
};
static const char DocEpubOpfTail[] = "</package>\n";

static int docEpubStartOpf(ZipOutput *zo, XmlWriter *xw,
...
static int docEpubEmitSimpleOpf(ZipOutput *zo, const MemoryBuffer *title,
				const MemoryBuffer *identifier,
				const BufferDocument *bd)
{
	int rval = 0;
	...
	docEpubEmitManifestItem(&xw, DocEpubIdNcx, DocEpubNameNcx,
				DocEpubMediaNcx);
	docEpubEmitManifestItem(&xw, DocEpubIdCss, DocEpubNameCssAbs,
				DocEpubMediaCss);
	docEpubEmitManifestItem(&xw, DocEpubIdDoc, DocEpubNameDocAbs,
				DocEpubMediaDoc);
...
static void docEpubEmitNcxMetaElement(XmlWriter *xw, const char *name,
				      const char *value)
{
...
static int docEpubEmitNcxHead(XmlWriter *xw, const MemoryBuffer *title,
			      const MemoryBuffer *identifier, int depth)
{
	char scratch[10];
...
static int docEpubCloseNavPoint(XmlWriter *xw, int level)
{
	sioOutPrintf(xw->xwSos, "%*s</navPoint>\n", 4 + 4 * level, "");
	return 0;
}

static int docEpubStartNavPoint(XmlWriter *xw, int level, const char *id,
				int playOrder, const MemoryBuffer *label,
				const char *src)
{
...
static int docEpubStartNcx(ZipOutput *zo, XmlWriter *xw,
			   const MemoryBuffer *title,
			   const MemoryBuffer *identifier, int depth,
			   const BufferDocument *bd)
{
...
static int docEpubEmitSimpleNcx(ZipOutput *zo, const MemoryBuffer *title,
				const MemoryBuffer *identifier,
				const BufferDocument *bd)
{
	int rval = 0;
	...
	docEpubStartNavPoint(&xw, 0, DocEpubIdDoc, playOrder++, title,
			     DocEpubNameDocAbs);
...
static int docEpubEmitCompositeNcx(EpubWriter *ew, const MemoryBuffer *title,
				   const MemoryBuffer *identifier,
				   const BufferDocument *bd)
{
	int rval = 0;
	...
	if (docEpubStartNcx(&(ew->ewZipOutput), &xw, title, identifier,
			    ew->ewCalculateToc.ctDepth, bd)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
	...
	sioOutPrintf( xw.xwSos, "%*s<--  m[%d]=%d teLevel= %d -->\n",
		...
		sprintf(src, "%s#%.*s", DocEpubNameDocAbs,
			te->teMarkName->mbSize, te->teMarkName->mbBytes);
		...
		docEpubStartNavPoint(&xw, ncxLevel, id, playOrder++, &mbLabel,
				     src);
...
static int docEpubGetCssNameX(MemoryBuffer *target, int relative)
{
	if (relative) {
...
static int docEpubWriteCss(HtmlWritingContext *hwc)
{
	...
	sosCss = sioOutZipOpen(&(ew->ewZipOutput),
			       utilMemoryBufferGetString(&nameMember),
			       compressed);
	if (!sosCss) {
		XDEB(sosCss);
		rval = -1;
...
static int docEpubEmitDocument(HtmlWritingContext *hwc, BufferDocument *bd,
...
ready:
	...
	hwc->hwcXmlWriter.xwSos =
		(SimpleOutputStream *)0; /* Only for this doc! */
...
static DocumentField *docHtmlGetTocForEpub(TocField *tf, BufferDocument *bd)
{
...
int docEpubSaveDocument(SimpleOutputStream *sos, BufferDocument *bd,
			const LayoutContext *lc)
{
	...
	hwc.hwcXmlWriter.xwSos =
		(SimpleOutputStream *)0; /* sos is not for HTML */
	...
	if (dfToc) {
		...
		if (docEpubEmitSimpleOpf(&(ew.ewZipOutput), &title, &identifier,
					 bd)) {
			LDEB(1);
			rval = -1;
			goto ready;
		...
		if (docEpubEmitSimpleNcx(&(ew.ewZipOutput), &title, &identifier,
					 bd)) {
			LDEB(1);
			rval = -1;
			goto ready;
			       ...
			       const InsertedObject *io, const char *ext)
{
	...
	if (1 || pip->pipBliptag == 0) {
		...
		utilMemoryBufferPrintf(target, "%s/b%08lx.%s", dir,
				       pip->pipBliptag, ext);
	}
