...
static void docSvgDrawShapePathLow(SvgWriter *sw, const AffineTransform2D *at,
				   const ShapePath *sp)
{
	...
	for (i = 0; i < sp->spVertexCount; i++) {
		...
		sioOutPrintf(xw->xwSos, "%s%s %g,%g", sep, command, x1 - x0,
			     y1 - y0);
...
static int docSvgDrawShapePath(const DrawingShape *ds, const ShapePath *sp,
...
int docSvgDrawDrawDrawingShape(const DocumentRectangle *drTwips, int page,
			       DrawingShape *ds, DrawingContext *dc, void *vsw)
{
	...
	case SHPtyFLOW_CHART_EXTRACT:
		if (docSvgDrawShapePath(ds, &SP_ISOSCELES_TRIANGLE, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyFLOW_CHART_MERGE:
		if (docSvgDrawShapePath(ds, &SP_FLOW_CHART_MERGE, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyRIGHT_TRIANGLE:
		if (docSvgDrawShapePath(ds, &SP_RIGHT_TRIANGLE, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyFLOW_CHART_INPUT_OUTPUT:
		if (docSvgDrawShapePath(ds, &SP_PARALLELOGRAM, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyNOTCHED_RIGHT_ARROW:
		if (docSvgDrawShapePath(ds, &SP_NOTCHED_RIGHT_ARROW, drTwips,
					dc, sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyRIGHT_ARROW_CALLOUT:
		if (docSvgDrawShapePath(ds, &SP_RIGHT_ARROW_CALLOUT, drTwips,
					dc, sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyLEFT_ARROW_CALLOUT:
		if (docSvgDrawShapePath(ds, &SP_LEFT_ARROW_CALLOUT, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyUP_ARROW_CALLOUT:
		if (docSvgDrawShapePath(ds, &SP_UP_ARROW_CALLOUT, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyDOWN_ARROW_CALLOUT:
		if (docSvgDrawShapePath(ds, &SP_DOWN_ARROW_CALLOUT, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyUP_DOWN_ARROW_CALLOUT:
		if (docSvgDrawShapePath(ds, &SP_UP_DOWN_ARROW_CALLOUT, drTwips,
					dc, sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyQUAD_ARROW_CALLOUT:
		if (docSvgDrawShapePath(ds, &SP_QUAD_ARROW_CALLOUT, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyLEFT_RIGHT_ARROW:
		if (docSvgDrawShapePath(ds, &SP_LEFT_RIGHT_ARROW, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyUP_DOWN_ARROW:
		if (docSvgDrawShapePath(ds, &SP_UP_DOWN_ARROW, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyLEFT_RIGHT_UP_ARROW:
		if (docSvgDrawShapePath(ds, &SP_LEFT_RIGHT_UP_ARROW, drTwips,
					dc, sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyLEFT_UP_ARROW:
		if (docSvgDrawShapePath(ds, &SP_LEFT_UP_ARROW, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyBENT_UP_ARROW:
		if (docSvgDrawShapePath(ds, &SP_BENT_UP_ARROW, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyWEDGE_RECT_CALLOUT:
		if (docSvgDrawShapePath(ds, &SP_WEDGE_RECT_CALLOUT, drTwips, dc,
					sw)) {
			LDEB(1);
			rval = -1;
		}
