...
static int docSvgSetColorRgb(DrawingContext *dc, void *vsw,
			     const RGB8Color *rgb8)
{
	return 0;
}

static int docSvgSetFont(DrawingContext *dc, void *vsw, int textAttrNumber,
...
static void docSvgWriteBorderAttributes(SvgWriter *sw, const BufferDocument *bd,
					const BorderProperties *bp, int thick)
{
	RGB8Color rgb8;
...
static void docSvgDrawHorizontalBorder(SvgWriter *sw, const BufferDocument *bd,
...
static void docSvgDrawVerticalBorder(SvgWriter *sw, const BufferDocument *bd,
				     const BorderProperties *bp,
				     const DocumentRectangle *drBorder)
{
	XmlWriter *xw = &(sw->swXmlWriter);
...
static int docSvgDrawOrnaments(const BlockOrnaments *bo, int page,
			       ...
			       struct DrawingContext *dc)
{
	...
	if (PROPmaskISSET(&(bo->boPropMask), ORNdrawSHADE)) {
		...
		if (docGetSolidRgbShadeOfItem(&isFilled, &rgb8, bd,
					      &(bo->boShading))) {
			LDEB(1);
		}
		...
		if (isFilled) {
			...
			if (rectBorder) {
				docSvgWriteBorderAttributes(
					sw, bd, &(bo->boTopBorder),
	...
	if (!rectBorder &&
		...
		docSvgDrawHorizontalBorder(sw, bd, &(bo->boTopBorder),
					   &drBorder);
		xmlNewLine(xw);
		/*done= 1;*/
	}
	...
	if (!rectBorder &&
		...
		docSvgDrawVerticalBorder(sw, bd, &(bo->boLeftBorder),
					 &drBorder);
		/*done= 1;*/
	}
	...
	if (!rectBorder &&
		...
		docSvgDrawVerticalBorder(sw, bd, &(bo->boRightBorder),
					 &drBorder);
		/*done= 1;*/
	}
	...
	if (!rectBorder &&
		...
		docSvgDrawHorizontalBorder(sw, bd, &(bo->boBottomBorder),
					   &drBorder);
		/*done= 1;*/
	}
...
int docSvgEmitFill(SvgWriter *sw, const struct DrawingShape *ds)
{
	XmlWriter *xw = &(sw->swXmlWriter);
	RGB8Color rgb8Fill;
	int fill;
...
int docSvgEmitStroke(SvgWriter *sw, const DrawingShape *ds)
{
	...
	if (stroke) {
		...
		case DSdashDASHED_X:
		case DSdashDASHED_L:
			sprintf(dashArray, "%d %d", 6 * widthTwips,
				4 * widthTwips);
			break;

		case DSdashDOT:
		case DSdashDOT_X:
			sprintf(dashArray, "%d %d", 1 * widthTwips,
				2 * widthTwips);
			break;
		...
		case DSdashDASHDOT_L:
			sprintf(dashArray, "%d %d %d %d", 6 * widthTwips,
				2 * widthTwips, 1 * widthTwips, 2 * widthTwips);
			break;

		case DSdashDASHDOTDOT:
		case DSdashDASHDOTDOT_L:
			sprintf(dashArray, "%d %d %d %d %d %d", 6 * widthTwips,
				2 * widthTwips, 1 * widthTwips, 2 * widthTwips,
...
static int docSvgFinishPage(void *vsw, DrawingContext *dc,
			    BufferItem *bodySectNode, int page, int asLast)
{
...
static int docSvgDrawStartPage(void *vsw, const DocumentGeometry *dgPage,
			       DrawingContext *dc, int page)
{
	...
	if (sw->swMultiPage) {
		...
		xmlPutString("<path", xw);
		xmlWriteStringAttribute(
			xw, "d", "M625,225 l-225,-150 l0,300 l225,-150z");
...
static int docSvgDrawPageRange(SvgWriter *sw, DrawingContext *dc,
			       ...
			       int asLast)
{
	...
	if (!dc->dcPostponeHeadersFooters) {
		docDrawPageHeader(bodyBi->biChildren[i], (void *)sw, dc,
				  firstPage);
	}
	...
	if (!dc->dcPostponeHeadersFooters) {
		docDrawPageFooter(bodyBi->biChildren[i], (void *)sw, dc,
				  lastPage);
	}

	docSvgFinishPage((void *)sw, dc, bodyBi->biChildren[i], lastPage,
			 asLast);
...
static void docSvgSetDrawingContext(DrawingContext *dc, const LayoutContext *lc,
...
int docSvgDrawDocument(SimpleOutputStream *sos, const char *applicationName,
		       ...
		       const LayoutContext *lc)
{
	...
	xmlPutString( "<pageSet>", xw );
		...
		xmlNewLine(xw);
		xmlPutString(
			"    efr.setAttribute( \"visibility\", \"hidden\" );",
			xw);
		xmlNewLine(xw);
		xmlPutString("}", xw);
		xmlNewLine(xw);
	...
	if (docSvgDrawPageRange(&sw, &dc, bodyBi, dc.dcFirstPage, dc.dcLastPage,
				/* asLast */ 1)) {
		LDEB(dc.dcFirstPage);
		return -1;
	}
...
int docSvgSaveShapeObject(SimpleOutputStream *sos, int page, int pixelsWide,
			  int pixelsHigh, const InsertedObject *io,
			  BufferItem *bodySectNode, const LayoutContext *lc)
{
	int rval = 0;
	...
	xmlPutString("div { margin:-2px 0; }", xw);
	xmlNewLine(xw);

	xmlPutString("    ]]></style>", xw);
	xmlNewLine(xw);
	xmlPutString("</defs>", xw);
	xmlNewLine(xw);
