...
int psWriteAfmFile(SimpleOutputStream *sos, int omitKernPairs,
		   const AfmFontInfo *afi)
{
	...
	sioOutPrintf(sos, "FontBBox %d %d %d %d\n", afi->afiFontBBox.drX0,
		     afi->afiFontBBox.drY0, afi->afiFontBBox.drX1,
	...
	for (i = 0; i < afi->afiMetricCount; i++) {
		...
		acl = &(acm->acmDefaultCodeList);
		if (acl->aclCodeCount > 0 && acl->aclCodes[0] < 256) {
	...
	for (i = 0; i < 256; i++) {
		if (map[i] >= 0) {
			...
			for (code = 0; code < acl->aclCodeCount; code++) {
				psWriteCharMetric(sos, acl->aclCodes[code],
						  acm);
			}
		}
	}
			...
			for (k = 0; k < acm->acmKernPairCount; k++) {
				...
				if (acm->acmKernPairs[k].akpPosition < 0 ||
				    acm->acmKernPairs[k].akpPosition >=
					    afi->afiMetricCount) {
					LLDEB(acm->acmKernPairs[k].akpPosition,
					      afi->afiMetricCount);
					continue;
				}

				acmK = afi->afiMetrics[acm->acmKernPairs[k]
							       .akpPosition];
				if (!acmK || !acmK->acmN ||
				    !acmK->acmN[0]) { /*LXDEB(i,acmK->acmN);*/
					continue;
				}
