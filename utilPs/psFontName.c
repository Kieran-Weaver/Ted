			    ...
			    const char *name)
{
	...
	while (*s) {
		if (!strncmp(s, "Narrow", 6) && (s[6] == '\0' || s[6] == ' ')) {
			  ...
			  const char *name)
{
	...
	if (utilFontWidthFromString(&width, &widthStart, &widthLength,
				    target)) {
...
int psFontInfoMoveWidthToFamilyName(AfmFontInfo *afi)
{
	...
	familyHasWidth = utilFontWidthFromString(
		&width, &widthStart, &widthLength, afi->afiFamilyName);

	familyIsPrefix = strncmp(afi->afiFullName, afi->afiFamilyName,
				 familyLength) == 0;

	if (familyIsPrefix) {
		hasWidth = utilFontWidthFromString(
			&width, &widthStart, &widthLength,
		...
		hasWidth = utilFontWidthFromString(
			&width, &widthStart, &widthLength, afi->afiFullName);
	}
			...
			if (!full) {
				...
				if (from - afi->afiFullName < widthStart) {
					*(to++) = ' ';
					...
					while (from - afi->afiFullName <
					       widthStart) {
						*(to++) = *(from++);
					}
				}

				from = afi->afiFullName + widthStart +
				       widthLength;
				if (*from) {
					*(to++) = ' ';
					while (*from) {
...
int psFontGetWidth(int *pWidth, const char *swdth)
{
	...
	if (!strcmp(swdth, "normal")) {
		*pWidth = FONTwidthNORMAL;
		return 0;
	}

	if (!strcmp(swdth, "semi extended")) {
		*pWidth = FONTwidthSEMIEXPANDED;
		return 0;
	}
