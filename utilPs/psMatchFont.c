...
static int psFontFamilySupportsUnicodes(const PsFontFamily *aff,
					int *pMissingCount,
	...
	if (afi->afiMetricsDeferred) {
		if (afi->afiResolveMetrics) {
			if ((*afi->afiResolveMetrics)(afi)) {
				SLDEB(afi->afiFullName,
				      afi->afiMetricsDeferred);
			}
		} else {
...
static int psGetFamilyByName(const PsFontFamily **pPsf, int *pMissing,
			     const PsFontFamily **pPsfFallback,
			     const char *name, const IndexSet *unicodes,
			     const PostScriptFontList *psfl)
{
	...
	if (aff) {
		...
		if (psFontFamilySupportsUnicodes(aff, &missingCount,
						 unicodes)) {
			*pPsf = aff;
			return 0;
		}
...
static int psGetFamilyByModifiedName(const PsFontFamily **pPsf, int *pMissing,
				     const PsFontFamily **pPsfFallback,
				     const char *name, const IndexSet *unicodes,
				     const PostScriptFontList *psfl)
{
	...
	if (!psGetFamilyByName(pPsf, pMissing, pPsfFallback, name, unicodes,
			       psfl)) {
		return 0;
	}
...
static int psGetFamilyByStyle(const PsFontFamily **pPsf, int *pMissing,
			      const PsFontFamily **pPsfFallback, int styleInt,
			      ...
			      const PostScriptFontList *psfl)
{
	...
	if (styleFamilies[styleInt]) {
		if (!psGetFamilyByName(pPsf, pMissing, pPsfFallback,
				       styleFamilies[styleInt], unicodes,
				       psfl)) {
			return 0;
		}
	}
...
static int psGetFamilyByStyleFromName(const PsFontFamily **pPsf, int *pMissing,
				      const PsFontFamily **pPsfFallback,
				      const char *name,
				      const IndexSet *unicodes,
				      const PostScriptFontList *psfl)
{
	...
	if (psGetFamilyByStyle(pPsf, pMissing, pPsfFallback, styleInt, unicodes,
			       psfl)) { /*SSDEB(name,styleStr);*/
		return -1;
	}
					 ...
					 const PostScriptFontList *psfl)
{
	...
	/*  5  */
	if (psGetFamilyByName(&psf, &missing, &psfFallback, familyName,
			      unicodesUsed, psfl) &&
	    psGetFamilyByModifiedName(&psf, &missing, &psfFallback, familyName,
				      unicodesUsed, psfl) &&
	    psGetFamilyByStyle(&psf, &missing, &psfFallback, styleInt,
			       unicodesUsed, psfl) &&
	    psGetFamilyByStyleFromName(&psf, &missing, &psfFallback, familyName,
				       unicodesUsed, psfl)) {
		if (psfFallback) {
			psf = psfFallback;
		} else {
