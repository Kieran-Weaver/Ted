			   ...
			   const DocumentGeometry *dgPage, double fac)
{
	...
	/*  0  */
	if (rows == 1 && cols == 1 && fitsWithoutRot && !pg->pgRotatePage90) {
		/*  1  */
		if (dgPage->dgPageHighTwips > dgSheet->dgPageHighTwips) {
			ty = dgPage->dgPageHighTwips;
		} else {
			ty = dgSheet->dgPageHighTwips;
		}
			   ...
			   int hasPageFooter)
{
	...
	/**/
	utilDocumentGeometryGetPageBoundingBox(&drBBox, dgPage, hasPageHeader,
					       hasPageFooter);
	...
	for (i = 0; i < ns->nsNup; at++, i++) {
		...
		if (i == 0) {
			*sheetBBox = dr;
			...
			geoUnionRectangle(sheetBBox, sheetBBox, &dr);
		}
	}
...
static void utilNupSetSheetGrid(NupSchema *ns, const PrintGeometry *pg,
				int rotateGrid, double fac)
{
	const DocumentGeometry *dgSheet = &(pg->pgSheetGeometry);
	int cols = pg->pgGridCols;
	int rows = pg->pgGridRows;
		     ...
		     double fac, const DocumentGeometry *dgPage)
{
	...
	if (nup == 1) {
		if (pg->pgCenterPageHorizontally) {
			...
			if (rotateSheetGrid) {
				ns->nsXShift = (dgSheet->dgPageWideTwips -
		...
		at = (AffineTransform2D *)realloc(
			ns->nsNupTransforms, nup * sizeof(AffineTransform2D));
		if (!at) {
			XDEB(at);
			return -1;
		}
