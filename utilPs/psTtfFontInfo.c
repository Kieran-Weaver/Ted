...
static int utilTtfSetUnicodeCmapFormat0Names(const TrueTypeFont *ttf,
...
static int utilTtfSetUnicodeCmapFormat4Names(const TrueTypeFont *ttf,
					     const TrueTypeCmapRecord *ttcr,
					     AfmFontInfo *afi)
{
	...
	for (seg = 0; seg < segCount; seg++) {
		...
		startCode = utilEndianExtractBeUint16(s);
		s += 2;
		endCode = utilEndianExtractBeUint16(e);
		e += 2;
		idDelta = utilEndianExtractBeUint16(d);
		d += 2;
		idRangeOffset = utilEndianExtractBeUint16(r);
		r += 2;
		...
		for (v = startCode; v <= endCode; v++) {
			...
			if (g < 0 || g >= ttf->ttfGlyphCount) {
				LLLDEB(g, ttf->ttfGlyphCount, idRangeOffset);
				return -1;
			}
...
static int utilTtfSetWindowsCmapFormat4Names(const TrueTypeFont *ttf,
					     const TrueTypeCmapRecord *ttcr,
					     AfmFontInfo *afi)
{
	const unsigned char *p = ttcr->ttcr4Data;
	...
	for (seg = 0; seg < segCount; seg++) {
		...
		for (v = startCode; v <= endCode; v++) {
			...
			if (g < 0 || g >= ttf->ttfGlyphCount) {
				SLLDEB(afi->afiFontName, g, ttf->ttfGlyphCount);
				continue;
			}
...
static int utilTtfSetPostGlyphNames(const TrueTypeFont *ttf, AfmFontInfo *afi)
{
	...
	if (ttpt->ttptFormatUpper == 2 && ttpt->ttptFormatLower == 0) {
		...
		for (g = 0; g < ttf->ttfGlyphCount; ttg++, g++) {
			...
			if (g >= ttpt->ttptIndexCount) {
				LLDEB(g, ttpt->ttptIndexCount);
				break;
			}

			idx = ttpt->ttptIndices[g];
			if (idx < 258) {
				if (psFontInfoSetGlyphName(
					    afi, g,
					    psAppleTtfGlyphNames[idx])) {
					LSDEB(g, psAppleTtfGlyphNames[idx]);
					return -1;
				}
			...
			if (psFontInfoSetGlyphName(
				    afi, g, (char *)ttpt->ttptStrings[idx])) {
				LSDEB(g, (char *)ttpt->ttptStrings[idx]);
				return -1;
			}
		}
...
static int utilTtfSetCharNames(const TrueTypeFont *ttf, AfmFontInfo *afi)
{
	...
	for (i = 0; i < ttct->ttctEncodingRecordCount; ttcr++, i++) {
		if (ttcr->ttcrPlatformID == 0 && ttcr->ttcrFormat == 4) {
			break;
		}
		if (ttcr->ttcrPlatformID == 3 && ttcr->ttcrEncodingID == 1 &&
		    ttcr->ttcrFormat == 4) {
			break;
		}
	}
	...
	ttcr = ttct->ttctEncodingRecords;
	for (i = 0; i < ttct->ttctEncodingRecordCount; ttcr++, i++) {
		if (ttcr->ttcrPlatformID == 0 && ttcr->ttcrFormat == 0 &&
		    ttcr->ttcrLength > 6) {
			break;
		}
	}
	...
	ttcr = ttct->ttctEncodingRecords;
	for (i = 0; i < ttct->ttctEncodingRecordCount; ttcr++, i++) {
		if (ttcr->ttcrPlatformID == 3 && ttcr->ttcrFormat == 4) {
			break;
		}
	}
	...
	ttcr = ttct->ttctEncodingRecords;
	for (i = 0; i < ttct->ttctEncodingRecordCount; ttcr++, i++) {
		LLLDEB(ttcr->ttcrPlatformID, ttcr->ttcrEncodingID,
		       ttcr->ttcrFormat);
	}

	LDEB(ttct->ttctEncodingRecordCount);
	return -1;
}
...
static int utilTtfScaledInteger(const TrueTypeFont *ttf, int val)
{
...
static int utilTtfSetCharMetrics(const TrueTypeFont *ttf, AfmFontInfo *afi)
{
...
static int utilTtfGetKernPairs(AfmFontInfo *afi, const TrueTypeFont *ttf)
{
	int k0;
	int glyphno;
	...
	for (glyphno = 0; glyphno < afi->afiMetricCount; glyphno++) {
		...
		while (k0 < ttks->ttksPairCount && ttkp->ttkpLeft < glyphno) {
			ttkp++;
			k0++;
		}
		...
		acm->acmKernPairs =
			(AfmKerningPair *)malloc(n * sizeof(AfmKerningPair));
		if (!acm->acmKernPairs) {
			LXDEB(n, acm->acmKernPairs);
			return -1;
		}
...
static int utilTtfGetCharMetricsAndEncoding(AfmFontInfo *afi,
					    const TrueTypeFont *ttf)
{
	if (utilTtfSetCharMetrics(ttf, afi)) {
		LDEB(ttf->ttfGlyphCount);
		return -1;
	}
...
static int psTtfGetNames(AfmFontInfo *afi, char **pSavedVariant,
			 const TrueTypeFont *ttf)
{
...
static int psTtfGetWeight(AfmFontInfo *afi, const TrueTypeFont *ttf,
			  const char *savedVariant)
{
	...
	if (ttot->ttotWeightClass > 0) {
		...
		if (utilFontWeightFromString(&weight, &start, &length,
			...
			if (!afi->afiWeightStr) {
				XDEB(afi->afiWeightStr);
				return -1;
			}
			strncpy(afi->afiWeightStr, savedVariant + start,
				length)[length] = '\0';
	...
	if (!afi->afiWeightStr) {
		...
		if (utilFontWeightFromString(&weight, &start, &length, str)) {
			...
			strncpy(afi->afiWeightStr, str + start,
				length)[length] = '\0';
			afi->afiWeightInt = weight;
		}
	}
...
static int psTtfGetWidth(AfmFontInfo *afi, const TrueTypeFont *ttf,
			 const char *savedVariant)
{
	const TrueTypeOS_2Table *ttot = &(ttf->ttfOS_2Table);
	...
	if (ttot->ttotWidthClass > 0) {
		...
		if (utilFontWidthFromString(&width, &start, &length,
					    savedVariant)) {
			afi->afiWidthStr = (char *)malloc(length + 1);
			if (!afi->afiWidthStr) {
				XDEB(afi->afiWidthStr);
				return -1;
			}
			strncpy(afi->afiWidthStr, savedVariant + start,
				length)[length] = '\0';
	...
	if (!afi->afiWidthStr) {
		...
		if (utilFontWidthFromString(&width, &start, &length, str)) {
			...
			strncpy(afi->afiWidthStr, str + start, length)[length] =
				'\0';
			afi->afiWidthInt = width;
		}
	}
...
int psTtfFontInfo(AfmFontInfo *afi, const TrueTypeFont *ttf)
{
	...
	if (ttpt->ttptItalicAngleUpper >= 0) {
		afi->afiItalicAngle = ttpt->ttptItalicAngleUpper +
	...
	afi->afiAscender = utilTtfScaledInteger(ttf, hhea->hheaAscender);
	afi->afiDescender = utilTtfScaledInteger(ttf, hhea->hheaDescender);
...
int psTtfToAfi(AfmFontInfo *afi, const MemoryBuffer *fontFileName,
	       SimpleInputStream *sisTtf)
{
	...
	if (fontFileName) {
		if (utilCopyMemoryBuffer(&(afi->afiFontFileName),
					 fontFileName)) {
			LDEB(1);
			rval = -1;
			goto ready;
