...
int psPostScriptFontListInfosToFamilies(PostScriptFontList *psfl)
{
	...
	qsort(psfl->psflInfos, psfl->psflInfoCount, sizeof(AfmFontInfo *),
	      psFontCompareInfosIncludingFamily);
...
int psPostScriptFontListAddInfo(PostScriptFontList *psfl, AfmFontInfo *afi)
{
	...
	if (!utilMemoryBufferIsEmpty(&(afi->afiFontFileName))) {
		if (!psfl->psflFontFileToInfo) {
			...
			if (!psfl->psflFontFileToInfo) {
				XDEB(psfl->psflFontFileToInfo);
				return -1;
			}
		}

		if (utilTreeStoreValue(
			    psfl->psflFontFileToInfo, &vprev, (const char **)0,
