...
typedef struct AfmKeyword {
	const char *akString;
	int (*akConsumer)(AfmReader *ar, int valPos, char *input);
} AfmKeyword;

static int psAfmConsumeLines(AfmReader *ar, char *input, const AfmKeyword *ak,
			     int keywordCount);
...
static const AfmKeyword psAfmMetricsKeywords[] = {
	...
	{
		"UnderlinePosition",
		psAfmUnderlinePosition,
	},
	{
		"UnderlineThickness",
		psAfmUnderlineThickness,
	},
	{
		"ItalicAngle",
		psAfmItalicAngle,
...
static char *psGetInputLine(char *s, int *pLen, int size, AfmReader *ar)
{
	...
	if (s) {
		...
		if (lineLength > 0 && s[lineLength - 1] == '\n') {
			s[--lineLength] = '\0';
		}

		if (lineLength > 0 && s[lineLength - 1] == '\r') {
			s[--lineLength] = '\0';
		}
...
int psAfmReadAfm(SimpleInputStream *sisAfm, AfmFontInfo *afi, int afmFlags)
{
	int res;
...
static int psAfmConsumeLines(AfmReader *ar, char *input,
			     const AfmKeyword *keywords, int keywordCount)
{
	for (;;) {
		int k;
		int b;
...
static int psAfmIgnore(AfmReader *ar, int valPos, char *input)
{
	return 0;
}
...
static int psAfmStartFontMetrics(AfmReader *ar, int valPos, char *input)
{
	int res;

	res = psAfmConsumeLines(ar, input, psAfmMetricsKeywords,
				sizeof(psAfmMetricsKeywords) /
					sizeof(AfmKeyword));
	if (res < 0) {
		LDEB(res);
		return res;
...
static int psAfmEndFontMetrics(AfmReader *ar, int valPos, char *input)
{
	return 1;
}
...
static int psAfmSaveString(char **pTarget, char *source)
{
	if (*pTarget) {
		free(*pTarget);
	}
...
static int psAfmGetNumber(double *pNumber, char *input, int valPos)
{
	char *s = input + valPos;
	char *past;
	double d;
...
static int psAfmComment(AfmReader *ar, int valPos, char *input)
{
...
static int psAfmFontName(AfmReader *ar, int valPos, char *input)
{
	return psAfmSaveString(&(ar->arAfi->afiFontName), input + valPos);
}

static int psAfmFontFileName(AfmReader *ar, int valPos, char *input)
{
	return utilMemoryBufferSetString(&(ar->arAfi->afiFontFileName),
					 input + valPos);
}

static int psAfmX11Font(AfmReader *ar, int valPos, char *input)
{
	return psAddX11FontToInfo(ar->arAfi, input + valPos);
}

static int psAfmResourceName(AfmReader *ar, int valPos, char *input)
{
	return psAfmSaveString(&(ar->arAfi->afiResourceName), input + valPos);
}

static int psAfmFullName(AfmReader *ar, int valPos, char *input)
{
	return psAfmSaveString(&(ar->arAfi->afiFullName), input + valPos);
}

static int psAfmNotice(AfmReader *ar, int valPos, char *input)
{
	return psAfmSaveString(&(ar->arAfi->afiNotice), input + valPos);
}

static int psAfmVersion(AfmReader *ar, int valPos, char *input)
{
	return psAfmSaveString(&(ar->arAfi->afiVersion), input + valPos);
}

static int psAfmFamilyName(AfmReader *ar, int valPos, char *input)
{
	return psAfmSaveString(&(ar->arAfi->afiFamilyName), input + valPos);
}

static int psAfmWeight(AfmReader *ar, int valPos, char *input)
{
	int weight;
	int start;
	int length;

	if (utilFontWeightFromString(&weight, &start, &length,
				     input + valPos)) {
...
static int psAfmEncodingScheme(AfmReader *ar, int valPos, char *input)
{
...
static int psAfmBBox(int valPos, char *input, DocumentRectangle *dr)
{
	char *s = input + valPos;
	char *past;
...
static int psAfmFontBBox(AfmReader *ar, int valPos, char *input)
{
...
static int psAfmMappingScheme(AfmReader *ar, int valPos, char *input)
{
	LDEB(1);
	return -1;
}

static int psAfmCharacterSet(AfmReader *ar, int valPos, char *input)
{
	return psAfmSaveString(&(ar->arAfi->afiCharacterSet), input + valPos);
}

static int psAfmCharacters(AfmReader *ar, int valPos, char *input)
{
	long count;
	char *past;
	char *s = input + valPos;
...
static int psAfmIsFixedV(AfmReader *ar, int valPos, char *input)
{
	LDEB(1);
	return -1;
}

static int psAfmStartDirection(AfmReader *ar, int valPos, char *input)
{
	LDEB(1);
	return -1;
}

static int psAfmFontFileIndex(AfmReader *ar, int valPos, char *input)
{
	double d;
	...
	ar->arAfi->afiFontFileIndex = (int)d;
	return 0;
}

static int psAfmCapHeight(AfmReader *ar, int valPos, char *input)
{
	double d;
	...
	ar->arAfi->afiCapHeight = (int)d;
	return 0;
}

static int psAfmXHeight(AfmReader *ar, int valPos, char *input)
{
	double d;
	...
	ar->arAfi->afiXHeight = (int)d;
	return 0;
}

static int psAfmAscender(AfmReader *ar, int valPos, char *input)
{
	double d;
	...
	ar->arAfi->afiAscender = (int)d;
	return 0;
}

static int psAfmDescender(AfmReader *ar, int valPos, char *input)
{
	double d;
	...
	ar->arAfi->afiDescender = (int)d;
	return 0;
}

static int psAfmUnderlinePosition(AfmReader *ar, int valPos, char *input)
{
	double d;
	...
	ar->arAfi->afiUnderlinePosition = (int)d;
	return 0;
}

static int psAfmUnderlineThickness(AfmReader *ar, int valPos, char *input)
{
	double d;
	...
	ar->arAfi->afiUnderlineThickness = (int)d;
	return 0;
}

static int psAfmItalicAngle(AfmReader *ar, int valPos, char *input)
{
...
static int psAfmCharWidth(AfmReader *ar, int valPos, char *input)
{
	LDEB(1);
	return -1;
}

static int psAfmIsFixedPitch(AfmReader *ar, int valPos, char *input)
{
	if (!strcmp(input + valPos, "false")) {
		ar->arAfi->afiIsFixedPitch = 0;
		return 0;
	}
	if (!strcmp(input + valPos, "true")) {
		ar->arAfi->afiIsFixedPitch = 1;
		return 0;
	}
...
static int psAfmStartCharMetrics(AfmReader *ar, int valPos, char *input)
{
	int lineLength;
...
static int psAfmStartKernPairs(AfmReader *ar, int valPos, char *input)
{
	int lineLength;
...
nextToken:
			...
			akp = (AfmKerningPair *)realloc(
				acm->acmKernPairs,
				(acm->acmKernPairCount + 1) *
					sizeof(AfmKerningPair));
			if (!akp) {
				LXDEB(acm->acmKernPairCount, akp);
				return -1;
			}
...
static int psAfmStartKernData(AfmReader *ar, int valPos, char *input)
{
	return psAfmConsumeLines(ar, input, psAfmKernDataKeywords,
				 sizeof(psAfmKernDataKeywords) /
					 sizeof(AfmKeyword));
}

static int psAfmStartComposites(AfmReader *ar, int valPos, char *input)
{
	int lineLength;
...
static int psAfmStartTrackKern(AfmReader *ar, int valPos, char *input)
{
	int lineLength;
