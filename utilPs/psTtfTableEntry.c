...
static int utilTrueTypeFontCompareTables(const void *vttte1, const void *vttte2)
{
					  ...
					  int tableCount, int filePos)
{
	...
	for (tab = 0; tab < tableCount; ttte++, tab++) {
		...
		ttte->ttteChecksum = sioEndianGetBeUint32(sisTtf);
		filePos += 4;
		ttte->ttteOffset = sioEndianGetBeUint32(sisTtf);
		filePos += 4;
		ttte->ttteMemoryBuffer.mbSize = sioEndianGetBeUint32(sisTtf);
		filePos += 4;
	}
	...
	*/
		...
		while (filePos < ttte->ttteOffset) {
			...
			if (c == EOF) {
				LLLDEB(filePos, ttte->ttteOffset, c);
				goto ready;
			}
			filePos++;
		}
		...
		ttte->ttteMemoryBuffer.mbBytes =
			(unsigned char *)malloc(ttte->ttteMemoryBuffer.mbSize);
		if (!ttte->ttteMemoryBuffer.mbBytes) {
			LXDEB(ttte->ttteMemoryBuffer.mbSize,
			      ttte->ttteMemoryBuffer.mbBytes);
			goto ready;
		}
		if (sioInReadBytes(sisTtf, ttte->ttteMemoryBuffer.mbBytes,
		    ...
		    ttte->ttteMemoryBuffer.mbSize) {
			LDEB(ttte->ttteMemoryBuffer.mbSize);
			goto ready;
		}
	...
	rval = tables;
	tables = (TrueTypeTableEntry *)0; /* steal */
