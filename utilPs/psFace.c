...
static int psForOneUnicode(int code, int glyph, void *voidpsf)
{
...
static void psFindFontEncodingPages(PostScriptFace *psf)
{
	const AfmFontInfo *afi = psf->psfAfi;
...
int psRememberPostsciptFace(PostScriptTypeList *pstl, const AfmFontInfo *afi,
			    ...
			    int appearsInText)
{
	...
	if (!pstl->pstlFaceTree) {
		...
		if (!pstl->pstlFaceTree) {
			XDEB(pstl->pstlFaceTree);
			return -1;
		}
	}

	psf = (PostScriptFace *)utilTreeGetEQ(pstl->pstlFaceTree, &fontName,
					      afi->afiFontName);
	if (!psf) {
		int page;
		void *prevPsf = (void *)0;
		...
		if (utilTreeStoreValue(pstl->pstlFaceTree, &prevPsf, &fontName,
				       afi->afiFontName, psf)) {
			SDEB(afi->afiFontName);
			free(psf);
			return -1;
	...
	if (ref >= psf->psfReferenceCount) {
		fr = (FaceReference *)realloc(
			psf->psfReferences, (ref + 1) * sizeof(FaceReference));
		if (!fr) {
			LXDEB(psf->psfReferenceCount, fr);
			return -1;
		}
...
static int utilFreePostScriptFace(const char *key, void *vpsf, void *through)
{
...
void psCleanPostScriptFaceList(PostScriptTypeList *pstl)
{
	...
	if (pstl->pstlFaceTree) {
		utilTreeFreeTree(pstl->pstlFaceTree, utilFreePostScriptFace,
				 (void *)0);
	}
