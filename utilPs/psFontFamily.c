...
static int appPsSetFontInfo(PsFontFamily *aff, int n, AfmFontInfo *afi)
{
	...
	/*  2  */
	if (!strncmp(afi->afiFamilyName, afi->afiFullName, fl) &&
	    (afi->afiFullName[fl] == ' ' || afi->afiFullName[fl] == '\0')) {
...
static int appPsSetFontFaces(PsFontFamily *aff, AfmFontInfo **afis, int count)
{
	AfmFontInfo **saved;
	int face;
	...
	for (face = 1; face < count; face++) {
		if (afis[face]->afiIsFixedPitch) {
			if (aff->psfHasProportionalWidth) {
				SLDEB(afis[face]->afiFullName,
				      afis[face]->afiIsFixedPitch);
			}
		} else {
			/*
