			    ...
			    TrueTypeNameTable *ttnt)
{
	...
	ttnt->ttntRecords = (TrueTypeNameRecord *)malloc(
		recordCount * sizeof(TrueTypeNameRecord));
	if (!ttnt->ttntRecords) {
		LXDEB(recordCount, ttnt->ttntRecords);
		rval = -1;
		goto ready;
	}
	...
	for (rec = 0; rec < recordCount; ttnr++, rec++) {
		...
		from = ttte->ttteMemoryBuffer.mbBytes + ttnt->ttntOffset +
		       ttnr->ttnrOffset;

		ttnr->ttnrSavedBytes =
			(unsigned char *)malloc(ttnr->ttnrLength + 1);
		if (!ttnr->ttnrSavedBytes) {
...
static int utilTtfGetNameRecord(const TrueTypeNameTable *ttnt, int nameID)
{
	int found = -1;
	int rec;
...
int psTtfGetName(char **pName, const TrueTypeNameTable *ttnt, int which)
{
	...
	if (recFound < 0) { /*LLDEB(which,recFound);*/
		return -1;
	}
	...
	if (utilTtfStringBytes(ttnr->ttnrPlatformID, ttnr->ttnrEncodingID) ==
		...
		for (from = 0; from < ttnr->ttnrLength; from += 2) {
			...
			uni = utilEndianExtractBeUint16(ttnr->ttnrSavedBytes +
							from);
