		      ...
		      const DocumentRectangle *drBBox, const char *file)
{
	...
	if (geoAffineTransformForTriangles(
		    &at, drBBox->drX0, drBBox->drY0, drBBox->drX0, drBBox->drY1,
		    drBBox->drX1, drBBox->drY0, drTo->drX0, drTo->drY0,
		    drTo->drX0, drTo->drY1, drTo->drX1, drTo->drY0)) {
		LDEB(1);
		return;
	}
		  ...
		  const MemoryBuffer *filename)
{
	...
	while (sioInGetString(line, 512 + 1, sis)) {
		...
		if (!gotBBox) {
			n = sscanf(line, "%%%%BoundingBox: %d %d %d %d", &i1,
				   &i2, &i3, &i4);
			if (n == 4) {
				llx = i1;
				lly = i2;
