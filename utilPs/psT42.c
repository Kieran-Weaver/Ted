...
static long utilTtfWriteTableByteArray(SimpleOutputStream *sos, long filePos,
				       const char *tag,
...
static long utilTtfStepTableByteArray(long filePos, unsigned long count,
				      int pad)
{
	if (pad > 1) {
		while (count % pad) {
...
static long psTtfLocaChunk(int *pPad, int *pGlyphIdx, const TrueTypeFont *ttf,
			   const TrueTypeTableEntry *ttte, int upto,
			   const int glyphIdx)
{
...
static long psTtfHmtxChunk(int *pPad, int *pGlyphIdx, const TrueTypeFont *ttf,
			   const TrueTypeTableEntry *ttte, int upto,
			   const int glyphIdx)
{
...
static long psTtfVmtxChunk(int *pPad, int *pGlyphIdx, const TrueTypeFont *ttf,
			   const TrueTypeTableEntry *ttte, int upto,
			   const int glyphIdx)
{
	...
	partialCount =
		(ttte->ttteMemoryBuffer.mbSize - 4 * vhea->vheaMetricCount) / 2;
...
static long psTtfGlyphChunk(int *pPad, int *pGlyphIdx, const TrueTypeFont *ttf,
			    const TrueTypeTableEntry *ttte, int upto,
			    const int glyphIdx)
{
	int l, r, m;
	...
	if (m == ttf->ttfLocationCount - 1) {
		upto = ttte->ttteMemoryBuffer.mbSize;
		pad = 4;
	} else {
				...
				const TrueTypeFont *ttf)
{
	...
	static const char *const sfntfTags[] = {
		"head", "hhea", "hmtx", "loca", "prep", "fpgm",
		"vhea", "glyf", "cvt ", "maxp", "vmtx",
	};
			...
			if (ttte->ttteMemoryBuffer.mbSize > 65528) {
				...
				if (!strcmp(ttte->ttteTag, "glyf")) {
					while (pos <
						...
						upto = psTtfGlyphChunk(
							&pad, &glyphIdx, ttf,
							ttte, upto, glyphIdx);

						filePos =
							utilTtfStepTableByteArray(
								filePos,
								upto - pos,
								pad);
						pos = upto;
				...
				SSLDEB(debFontName, ttte->ttteTag,
				       ttte->ttteMemoryBuffer.mbSize);
				rval = -1;
				goto ready;
			} else {
				filePos = utilTtfStepTableByteArray(
	...
	sioEndianPutBeUint32(ttf->ttfVersion, sosHex);
	filePos += 4;
	sioEndianPutBeUint16(tableCount, sosHex);
	filePos += 2;
	sioEndianPutBeUint16(ttf->ttfSearchRange, sosHex);
	filePos += 2;
	sioEndianPutBeUint16(ttf->ttfEntrySelector, sosHex);
	filePos += 2;
	sioEndianPutBeUint16(ttf->ttfRangeShift, sosHex);
	filePos += 2;
	...
	for (tab = 0; tab < ttf->ttfTableCount; ttte++, tab++) {
		...
		sioEndianPutBeUint32(ttte->ttteChecksum, sosHex);
		filePos += 4;
		sioEndianPutBeUint32(ttte->ttteSaveOffset, sosHex);
		filePos += 4;
		sioEndianPutBeUint32(ttte->ttteMemoryBuffer.mbSize, sosHex);
		filePos += 4;
	}
		...
		if (ttte->ttteMemoryBuffer.mbSize > 65528) {
			...
			if (!strcmp(ttte->ttteTag, "glyf")) {
				while (pos < ttte->ttteMemoryBuffer.mbSize) {
					...
					filePos = utilTtfWriteTableByteArray(
						sos, filePos, ttte->ttteTag,
						ttte->ttteMemoryBuffer.mbBytes +
							pos,
						upto - pos, pad);
					if (filePos < 0) {
						SLLDEB(ttte->ttteTag,
						       ttte->ttteMemoryBuffer
							       .mbSize,
						       filePos);
						rval = -1;
						goto ready;
			...
			if (filePos < 0) {
				SLLDEB(ttte->ttteTag,
				       ttte->ttteMemoryBuffer.mbSize, filePos);
				rval = -1;
				goto ready;
			}
		}
...
static void psWriteFontEncoding(SimpleOutputStream *sos, const AfmFontInfo *afi)
{
	int glyph;
...
static int utilTtfWriteT42(SimpleOutputStream *sos, const AfmFontInfo *afi,
			   const TrueTypeFont *ttf)
{
	const TrueTypeHeadTable *ttht = &(ttf->ttfHeadTable);
	...
	sioOutPrintf(sos, "  /FontBBox [%d %d %d %d] def\n",
		     afi->afiFontBBox.drX0, afi->afiFontBBox.drY0,
		     afi->afiFontBBox.drX1, afi->afiFontBBox.drY1);
...
int psTtfToPt42(SimpleOutputStream *sosPf42, const MemoryBuffer *fontFileName,
...
int psTtcToPt42(SimpleOutputStream *sosPf42, const MemoryBuffer *fontFileName,
		int fontFileIndex, SimpleInputStream *sisTtf)
{
	int rval = 0;
	TrueTypeFont ttf;
