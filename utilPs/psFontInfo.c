...
static int psFontMetricAddCodeToList(AfmCodeList *acl, AfmFontInfo *afi, int C)
{
	int i;
...
int psGetUnicodesFromGlyphNames(AfmFontInfo *afi)
{
	...
	for (glyphIndex = 0; glyphIndex < afi->afiMetricCount; glyphIndex++) {
		...
		if (acm && acm->acmN) {
			...
			if (code < 0) { /*SDEB(acm->acmN);*/
				continue;
			}

			if (psFontMetricAddCodeToList(
				    &(acm->acmUnicodeCodeList), afi, code)) {
				LDEB(code);
				return -1;
			}
			...
			if (utilIndexMappingGet(tgm, code) < 0) {
				...
				if (utilIndexSetAdd(&(afi->afiUnicodesProvided),
						    code)) {
					LDEB(code);
				}
			}
...
void psRemoveMetricsFromInfo(AfmFontInfo *afi)
{
	...
	if (afi->afiNameToMetric) {
		utilTreeFreeTree(afi->afiNameToMetric, (UTIL_TREE_CALLBACK)0,
				 (void *)0);
		afi->afiNameToMetric = (void *)0;
	}
...
static int psFontMetricAddCode(AfmCharMetric *acm, AfmFontInfo *afi, int C)
{
			...
			const rect *abb)
{
	...
	fresh[afi->afiMetricCount] = acm =
		(AfmCharMetric *)malloc(sizeof(AfmCharMetric));
	if (!acm) {
		XDEB(acm);
		return -1;
	}
	...
	if (N && !duplicate) {
		...
		if (utilTreeStoreValue(afi->afiNameToMetric, (void **)0,
				       (const char **)0, acm->acmN, acm)) {
...
{
	if (glyphIndex < 0 || glyphIndex >= afi->afiMetricCount ||
	    !afi->afiMetrics[glyphIndex]) {
		LLDEB(glyphIndex, afi->afiMetricCount);
		return -1;
	}
...
int psFontInfoSetGlyphUnicode(AfmFontInfo *afi, int glyphIndex, int code)
{
	AfmCharMetric *acm;
	IndexMapping *tgm = &(afi->afiUnicodeToGlyphMapping);
			   ...
			   const char *glyphName)
{
	...
	if (!afi->afiMetrics[glyphIndex]->acmN) {
		XDEB(afi->afiMetrics[glyphIndex]->acmN);
		return -1;
	}
