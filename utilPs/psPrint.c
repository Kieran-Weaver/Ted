		   ...
		   int hasPageFooter)
{
	...
	if (utilNupSetSchema(&(ps->psNupSchema), ps->psRotateSheetGrid,
			     &at1Page, pg, fac, dgPage)) {
...
static int psPrintUnicodeStringValue(PrintingState *ps, const char *s, int len)
{
	SimpleOutputStream *sos = ps->psSos;
	int off;
			      ...
			      int len)
{
	...
	for (i = 0; i < len; i++) {
		if (s[i] >= 127) {
			return psPrintUnicodeStringValue(ps, (const char *)s,
							 len);
		}
	}
...
int psSetRgb8Color(PrintingState *ps, const RGB8Color *rgb8)
{
	if (rgb8->rgb8Red == rgb8->rgb8Green &&
		...
		if (sioOutPrintf(ps->psSos, "%g %g %g setrgbcolor\n",
				 rgb8->rgb8Red / 255.0, rgb8->rgb8Green / 255.0,
...
{
	sioOutPrintf(sos, "[ %g %g %g %g %g %g ]", at->at2Axx, at->at2Axy,
		     at->at2Ayx, at->at2Ayy, at->at2Tx, at->at2Ty);
	return;
}
		 ...
		 int lineHeight)
{
	if (ps->psLinkParticulesDone > 0) {
		...
		psSourcePdfmark(ps->psSos, &drLink, &(ps->psLinkFile),
				&(ps->psLinkMark));
