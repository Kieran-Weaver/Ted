...
utilTtfOpenTableStream(const TrueTypeTableEntry **pTtte, const char *tag,
		       const TrueTypeFont *ttf)
{
	...
	if (tab >= ttf->ttfTableCount) { /*SLDEB(tag,ttf->ttfTableCount);*/
...
static int utilTtfExtractPostTable(TrueTypeFont *ttf)
{
	...
	case 2:
		if (ttpt->ttptFormatLower != 0) {
			LDEB(ttpt->ttptFormatLower);
			goto ready;
		}

		idxCount = sioEndianGetBeUint16(sisPost);
		ttpt->ttptIndices = (unsigned short *)malloc(
			idxCount * sizeof(unsigned short));
		if (!ttpt->ttptIndices) {
			LXDEB(idxCount, ttpt->ttptIndices);
			rval = -1;
			goto ready;
		}
		...
		if (stringCount > 0) {
			ttpt->ttptStrings = (unsigned char **)malloc(
				stringCount * sizeof(unsigned char *));
			if (!ttpt->ttptStrings) {
...
static int utilTtfExtractHheaTable(TrueTypeFont *ttf)
{
	...
	for (i = 0; i < 5; i++) { /*reserved=*/
		(void)sioEndianGetBeInt16(sisHhea);
	}
...
static int utilTtfExtractVheaTable(TrueTypeFont *ttf)
{
	...
	if (!sisVhea) { /*XDEB(sisVhea); rval= -1; */
		goto ready;
	}
	...
	for (i = 0; i < 4; i++) { /*reserved=*/
		(void)sioEndianGetBeInt16(sisVhea);
	}
...
static int utilTtfExtractLocaTable(TrueTypeFont *ttf)
{
	...
	case 1:
		if (ttte->ttteMemoryBuffer.mbSize % 4) {
			LLDEB(ttte->ttteMemoryBuffer.mbSize, 4);
			rval = -1;
			goto ready;
		}
		...
		break;
	default:
		LDEB(ttht->tthtIndexToLocFormat);
		rval = -1;
		goto ready;
	}

	ttf->ttfLocations = (unsigned long *)malloc(ttf->ttfLocationCount *
	...
	case 0:
		for (i = 0; i < ttf->ttfLocationCount; i++) {
			ttf->ttfLocations[i] =
				2 * sioEndianGetBeUint16(sisLoca);
		}
		break;
...
static int utilTtfExtractHmtxTable(TrueTypeFont *ttf)
{
	...
	partialCount =
		(ttte->ttteMemoryBuffer.mbSize - 4 * hhea->hheaMetricCount) / 2;

	ttf->ttfHorizontalMetricCount = hhea->hheaMetricCount + partialCount;

	ttf->ttfHorizontalMetrics = (HorizontalMetric *)malloc(
		ttf->ttfHorizontalMetricCount * sizeof(HorizontalMetric));
	if (!ttf->ttfHorizontalMetrics) {
	...
	for (i = 0; i < hhea->hheaMetricCount; i++) {
		...
		ttf->ttfHorizontalMetrics[i].hmLsb =
			sioEndianGetBeInt16(sisHmtx);
	}

	for (i = hhea->hheaMetricCount; i < ttf->ttfHorizontalMetricCount;
	     i++) {
		ttf->ttfHorizontalMetrics[i].hmAdvanceWidth = advanceWidth;
		ttf->ttfHorizontalMetrics[i].hmLsb =
			sioEndianGetBeInt16(sisHmtx);
...
#define fg_rrcurveto(a, x1, y1, x2, y2, x3, y3) \
	appDebug("fg_rrcurveto(%g,%g,%g,%g,%g,%g)\n", x1, y1, x2, y2, x3, y3)
...
static int utilTtfExtractGlyfTable(TrueTypeFont *ttf)
{
	...
	ttf->ttfGlyphs = (TrueTypeGlyph *)malloc(ttf->ttfGlyphCount *
						 sizeof(TrueTypeGlyph));
	if (!ttf->ttfGlyphs) {
		LXDEB(ttf->ttfGlyphCount, ttf->ttfGlyphs);
		rval = -1;
		goto ready;
	}
	...
	for (glyphno = 0; glyphno < ttf->ttfGlyphCount; ttg++, glyphno++) {
		...
		/* signed ! */
		ttg->ttgContourCount = utilEndianExtractBeInt16(data);
		data += 2;

		ttg->ttgXMin = utilEndianExtractBeInt16(data);
		data += 2;
		ttg->ttgYMin = utilEndianExtractBeInt16(data);
		data += 2;
		ttg->ttgXMax = utilEndianExtractBeInt16(data);
		data += 2;
		ttg->ttgYMax = utilEndianExtractBeInt16(data);
		data += 2;
...
static int utilTtfExtractCmapTable(TrueTypeFont *ttf)
{
	...
	ttct->ttctEncodingRecords = (TrueTypeCmapRecord *)malloc(
		ttct->ttctEncodingRecordCount * sizeof(TrueTypeCmapRecord));
	if (!ttct->ttctEncodingRecords) {
	...
	for (i = 0; i < ttct->ttctEncodingRecordCount; ttcr++, i++) {
		...
		ttcr->ttcrFormat = utilEndianExtractBeUint16(p);
		p += 2;
		...
		case 0:
			ttcr->ttcrLength = utilEndianExtractBeUint16(p);
			p += 2;
			ttcr->ttcrLanguage = utilEndianExtractBeUint16(p);
			p += 2;

			if (ttcr->ttcrLength < 6) {
				LDEB(ttcr->ttcrLength);
				LLDEB(ttcr->ttcrPlatformID,
				      ttcr->ttcrEncodingID);
				break;
			}

			memcpy(ttcr->ttcr0GlyphIDArray, p,
			       ttcr->ttcrLength - 6);
			break;
		...
		case 4:
			...
			ttcr->ttcr4SegCountX2 = utilEndianExtractBeUint16(p);
			p += 2;
			ttcr->ttcr4SearchRange = utilEndianExtractBeUint16(p);
			p += 2;
			ttcr->ttcr4EntrySelector = utilEndianExtractBeUint16(p);
			p += 2;
			ttcr->ttcr4RangeShift = utilEndianExtractBeUint16(p);
			p += 2;
			ttcr->ttcr4Data = p;
			break;
		...
		case 6:
			...
			ttcr->ttcr6FirstCode = utilEndianExtractBeUint16(p);
			p += 2;
			ttcr->ttcr6_12CodeCount = utilEndianExtractBeUint16(p);
			p += 2;

			ttcr->ttcr6_12GlyphCodes = (unsigned int *)malloc(
				ttcr->ttcr6_12CodeCount * sizeof(unsigned int));
			if (!ttcr->ttcr6_12GlyphCodes) {
				LXDEB(ttcr->ttcr6_12CodeCount,
				      ttcr->ttcr6_12GlyphCodes);
				rval = -1;
				goto ready;
			}
			for (ent = 0; ent < ttcr->ttcr6_12CodeCount; ent++) {
				ttcr->ttcr6_12GlyphCodes[ent] =
					utilEndianExtractBeUint16(p);
				p += 2;
			}
			break;
		...
		case 12:
			p += 2;
			ttcr->ttcrLength = utilEndianExtractBeUint32(p);
			p += 4;
			ttcr->ttcrLanguage = utilEndianExtractBeUint32(p);
			p += 4;

			ttcr->ttcr12GroupCount = utilEndianExtractBeUint32(p);
			p += 4;
			pp = p;
			count = 0;
			for (ent = 0; ent < ttcr->ttcr12GroupCount; ent++) {
				...
				sc = utilEndianExtractBeUint32(pp);
				pp += 4;
				ec = utilEndianExtractBeUint32(pp);
				pp += 4;
				/*sg=*/(void)utilEndianExtractBeUint32(pp);
				pp += 4;
				count += ec - sc + 1;
			}
			break;
				      ...
				      const unsigned char *p)
{
	...
	for (pair = 0; pair < ttks->ttksPairCount; ttkp++, pair++) {
		ttkp->ttkpLeft = utilEndianExtractBeUint16(p);
		p += 2;
		ttkp->ttkpRight = utilEndianExtractBeUint16(p);
		p += 2;
		ttkp->ttkpValue = utilEndianExtractBeInt16(p);
		p += 2;
	}
...
static int utilTtfExtractKernTable(TrueTypeFont *ttf)
{
	...
	ttkt->ttktKernSubs =
		(TrueTypeKernSub *)malloc(subCount * sizeof(TrueTypeKernSub));
	if (!ttkt->ttktKernSubs) {
	...
	for (sub = 0; sub < subCount; ttks++, sub++) {
		...
		ttks->ttksVersion = utilEndianExtractBeUint16(p);
		p += 2;
		ttks->ttksLength = utilEndianExtractBeUint16(p);
		p += 2;
		ttks->ttksCoverage = utilEndianExtractBeUint16(p);
		p += 2;
		ttks->ttksPairCount = utilEndianExtractBeUint16(p);
		p += 2;

		if (ttks->ttksPairCount > 0) {
			ttks->ttksPairs = (TrueTypeKernPair *)malloc(
				ttks->ttksPairCount * sizeof(TrueTypeKernPair));
			if (!ttks->ttksPairs) {
...
int psTtfLoadFont(TrueTypeFont *ttf, SimpleInputStream *sisTtf, long filePos)
{
	int tableCount;

	ttf->ttfVersion = sioEndianGetBeUint32(sisTtf);
	filePos += 4;
	tableCount = sioEndianGetBeUint16(sisTtf);
	filePos += 2;
	ttf->ttfSearchRange = sioEndianGetBeUint16(sisTtf);
	filePos += 2;
	ttf->ttfEntrySelector = sioEndianGetBeUint16(sisTtf);
	filePos += 2;
	ttf->ttfRangeShift = sioEndianGetBeUint16(sisTtf);
	filePos += 2;

	ttf->ttfTables = psTtfReadTableEntries(sisTtf, tableCount, filePos);
	if (!ttf->ttfTables) {
		LXDEB(tableCount, ttf->ttfTables);
		return -1;
	}
...
int psTtcLoadFont(TrueTypeFont *ttf, SimpleInputStream *sisTtf, int fontIndex)
{
	long filePos = 0;
	...
	versionMA = sioEndianGetBeUint16(sisTtf);
	filePos += 2;
	/*versionMI=*/(void)sioEndianGetBeUint16(sisTtf);
	filePos += 2;
	...
	fontCount = sioEndianGetBeUint32(sisTtf);
	filePos += 4;
	if (fontCount <= fontIndex) {
		LLDEB(fontCount, fontIndex);
		return -1;
	}

	for (f = 0; f < fontIndex; f++) {
		sioEndianGetBeUint32(sisTtf);
		filePos += 4;
	}
	fontOffset = sioEndianGetBeUint32(sisTtf);
	filePos += 4;
