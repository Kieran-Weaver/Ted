			  ...
			  const MemoryBuffer *fontDirectory)
{
	...
	if (!utilMemoryBufferIsEmpty(&(psf->psfAfi->afiFontFileName))) {
		...
		res = utilPsGetEmbedFromFilename(
		...
		l = appAbsoluteName(&absolute, &relative, relativeIsFile,
				    fontDirectory);
		if (l <= 0) {
			res = PSembedNO;
			goto ready;
			...
			const PostScriptTypeList *pstl)
{
	...
	if (embedFonts) {
		...
		while (psf) {
			...
			psf = (PostScriptFace *)utilTreeGetNext(
...
static void psDefineEncodedFont(SimpleOutputStream *sos, const AfmFontInfo *afi,
				...
				const char *encodingArrayName)
{
	...
	sioOutPrintf(sos, "/%s %s /%s dcpf\n", encodedFontName,
		     encodingArrayName, afi->afiFontName);
#endif
	return;
}

int psIncludeFonts(SimpleOutputStream *sos, const PostScriptTypeList *pstl)
{
	int rval = 0;
	const char *fontName;
	PostScriptFace *psf;
	...
	while (psf) {
		...
		if (psf->psfEmbed > 0) {
			...
			case PSembedTTCTO42:
				if (psTtcToPt42(sos, &(psf->psfFontFileName),
...
static void psSetStdEncodingArrayName(char *encodingArrayName, int page)
{
	sprintf(encodingArrayName, "CP_%02X", page);
	return;
}

static void psSetFaceEncodingArrayName(char *encodingArrayName, int face,
				       int page)
{
...
static void psEmitStandardFontEncodingPage(SimpleOutputStream *sos, int page)
{
	int i;
	char name[20];
...
static void psListNonStdPage(SimpleOutputStream *sos, const AfmFontInfo *afi,
			     int page)
{
	int i;
	...
	for (i = 0; i < UPP; i++) {
		...
		if (!ucdIsCn(code)) {
			...
			glyph = utilIndexMappingGet(
				&(afi->afiUnicodeToGlyphMapping), code);
			if (glyph < 0) { /*LDEB(glyph);*/
			} else {
				const AfmCharMetric *acm =
					afi->afiMetrics[glyph];
				const char *stdName =
					psUnicodeToGlyphName(code);
...
static void psSetEncodedFontName(char *encodedFontName,
				 const PostScriptFace *psf, int page)
{
	const AfmFontInfo *afi = psf->psfAfi;
...
static void psListFontEncodingPages(SimpleOutputStream *sos,
				    const PostScriptFace *psf)
{
	...
	for (page = 0; page < UNPAGE; page++) {
		...
		if (psf->psfPageUsed[page]) {
			...
			psDefineEncodedFont(sos, afi, encodedFontName,
					    faceEncodingArrayName);
			done++;
		}
	}
...
void psSelectFontProcedures(SimpleOutputStream *sos,
			    const PostScriptTypeList *pstl, int allFonts)
{
	const char *fontName;
	PostScriptFace *psf;
	int page;
		...
		if (psf->psfAppearsInText || allFonts) {
			...
			for (page = 0; page < UNPAGE; page++) {
				if (psf->psfPageUsed[page]) {
					...
					psSetEncodedFontName(encodedFontName,
							     psf, page);
					sioOutPrintf(sos,
						     "    /%s findfont %% %d\n",
						     encodedFontName,
