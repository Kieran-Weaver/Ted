...
static int docCopyPropsCollectNode(BufferItem *node,
...
static int docCopyPropsCollectNodes(CollectNodes *cn, BufferDocument *bd,
				    DocumentTree *tree,
				    const DocumentSelection *ds)
{
	...
	if (ds) {
		if (docScanSelection(bd, ds, docCopyPropsCollectNode,
				     (NodeVisitor)0, flags, (void *)cn) < 0) {
			LDEB(1);
			return -1;
		}
	} else {
		if (docScanTree(bd, tree, docCopyPropsCollectNode,
				(NodeVisitor)0, flags, (void *)cn) < 0) {
			LDEB(1);
			return -1;
		}
	...
	if (cn->cnNodeCount > 0) {
		cn->cnNodes = (BufferItem **)malloc(cn->cnNodeCount *
						    sizeof(BufferItem *));
		if (!cn->cnNodes) {
			LXDEB(cn->cnNodeCount, cn->cnNodes);
			return -1;
		}
...
static int docCopyNodeProperties(SetProperties *setProps,
				 const DocumentSelection *dsTo,
				 BufferItem *nodeTo, const BufferItem *nodeFrom)
{
	switch (nodeTo->biLevel) {
	case DOClevPARA:
		setProps->spGotPara++;

		if (docEditChangeParaProperties(
			    setProps, dsTo, nodeTo, (const TextAttribute *)0,
	...
	case DOClevCELL:
		if (docIsRowNode(nodeTo->biParent)) {
			...
			if (!CELL_MERGED(cpFrom)) {
				if (docEditChangeCellProperties(
					    setProps, dsTo, nodeTo, cpFrom)) {
					LDEB(nodeTo->biLevel);
					return -1;
				}
			}
	...
	case DOClevROW:
		if (docIsRowNode(nodeTo)) {
			...
			if (docEditChangeRowProperties(
				    setProps, dsTo, nodeTo,
	...
	case DOClevSECT:
		if (setProps->sp_spSetMask) {
			...
			if (docEditUpdSectProperties(
				    eo, setProps->sp_spDoneMask, nodeTo,
				    setProps->sp_spSetMask,
				    &(nodeFrom->biSectProperties),
				    setProps->spAttributeMap)) {
				XDEB(setProps->sp_spSetMask);
				return -1;
			}
		}
		setProps->spGotSect++;
...

{
	...
	if (dpSetMask &&
	    docChangeDocumentProperties(eo, docDpDoneMask, dpSetMask,
					&(dcj->dcjSourceDocument->bdProperties),
					setProps.spAttributeMap)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (ppSetMask || cpSetMask || rpSetMask || spSetMask) {
		...
		if (docCopyPropsCollectNodes(&cnTo, eo->eoDocument, eo->eoTree,
					     dsTo)) {
			LDEB(1);
			rval = -1;
			goto ready;
		...
		for (n = 0; n < cnTo.cnNodeCount; n++) {
			if (cnTo.cnNodes[n]->biLevel !=
			    cnFrom.cnNodes[n]->biLevel) {
	...
	if (cpSetMask && !setProps.spGotCell) {
		...
		BufferItem *cellNodeFrom =
			docGetCellNode(dsFrom->dsHead.dpNode);

		if (!cellNodeTo || !cellNodeFrom) {
			XXDEB(cellNodeTo, cellNodeFrom);
			rval = -1;
			goto ready;
		}

		if (docCopyNodeProperties(&setProps, dsTo, cellNodeTo,
					  cellNodeFrom)) {
			XDEB(cpSetMask);
			rval = -1;
			goto ready;
	...
	if (rpSetMask && !setProps.spGotRow) {
		...
		if (!rowNodeTo || !rowNodeFrom) {
			XXDEB(rowNodeTo, rowNodeFrom);
			rval = -1;
			goto ready;
		}

		if (docCopyNodeProperties(&setProps, dsTo, rowNodeTo,
					  rowNodeFrom)) {
			XDEB(rpSetMask);
			rval = -1;
			goto ready;
	...
	if (spSetMask && !setProps.spGotSect) {
		...
		BufferItem *sectNodeFrom =
			docGetSectNode(dsFrom->dsHead.dpNode);

		if (!sectNodeTo || !sectNodeFrom) {
			XXDEB(sectNodeTo, sectNodeFrom);
			rval = -1;
			goto ready;
		}

		if (docCopyNodeProperties(&setProps, dsTo, sectNodeTo,
					  sectNodeFrom)) {
			XDEB(spSetMask);
			rval = -1;
			goto ready;
