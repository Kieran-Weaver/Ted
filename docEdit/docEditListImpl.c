...
static int docFindFreeLevelTemplateId(const DocumentListTable *dlt)
{
	int found = 1;
	...
	while (found) {
		...
		for (i = 0; i < dlt->dltListCount; dl++, i++) {
			...
			ll = dl->dlLevels;
			for (level = 0; level < dl->dlLevelCount;
			     ll++, level++) {
		...
		if (!found) {
			retLevelTemplateID = levelTemplateID;
			break;
		}
	}
...
static int docDefaultList(DocumentList *dlTo, const DocumentListTable *dlt,
			  int deftab, const PropertyMask *taSetMask,
			  const TextAttribute *taSet)
{
	...
	for (level = 0; level < DLmaxLEVELS; level++) {
		...
		if (docDefaultListLevel(dlTo->dlLevels + level, level, deftab,
					levelTemplateID, taSetMask, taSet)) {
			LDEB(level);
			return -1;
		}
	}
...
int docNewList(const DocumentList **pDl, const struct ListOverride **pLo,
	       ...
	       const TextAttribute *taSet)
{
	...
	listIndex = docDocumentListTableAddList(&dl, dlt, dlExample, copyIds,
	...
	if (!dlExample) {
		if (docDefaultList(dl, dlt, dp->dpTabIntervalTwips, taSetMask,
				   taSet)) {
			LDEB(1);
			return -1;
		}
	...
	ls = docListOverrideTableAddOverride(&lo, lot, -1, dl->dlListID,
					     listIndex);
	if (ls < 0) {
