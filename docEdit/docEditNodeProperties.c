...
static int docInsertListtextField(DocumentField **pField,
				  ...
				  BufferItem *paraNode, DocumentTree *dt,
				  BufferDocument *bd)
{
	int rval = 0;
	...
	if (docInsertParaHeadField(pField, dsInsideField, dsAroundField,
				   pPartBegin, pPartEnd, paraNode, bd, dt,
	...
	if (docListNumberTreesInsertParagraph(
		    &(dt->dtListNumberTrees), paraNode->biParaListOverride,
...
int docEditUpdParaProperties(EditOperation *eo, PropertyMask *pPpDoneMask,
			     BufferItem *paraNode,
			     ...
			     const DocumentAttributeMap *dam)
{
	...
	if (paraNode->biParaListOverride > 0) {
		...
		if (docDelimitParaHeadField(
			    &dfBullet, &dsInsideBullet, &dsAroundBullet,
			    &partBulletHead, &partBulletTail, paraNode,
			    bd)) { /* LDEB(paraNode->biParaListOverride); */
		}
	}
	...
	if (wasInList || isInList || olevel < PPoutlineBODYTEXT ||
	...
	if (wasInList && !isInList) {
		...
		if (docListNumberTreesDeleteParagraph(
			    &(eo->eoTree->dtListNumberTrees), ls,
			    paraNr)) { /*LLDEB(override,paraNr);*/
		}

		if (dfBullet) {
			if (docDeleteField(&dsInsideBullet, eo, paraNode,
					   paraNode, partBulletHead,
					   partBulletTail, dfBullet)) {
	...
	if (!wasInList && isInList) {
		if (docInsertListtextField(&dfBullet, &dsInsideBullet,
					   &dsAroundBullet, &partBulletHead,
					   &partBulletTail, paraNode,
					   eo->eoTree, bd)) {
			LDEB(1);
			return -1;
		}
	...
	if (wasInList && isInList &&
		...
		if (docListNumberTreesInsertParagraph(
			    &(eo->eoTree->dtListNumberTrees), ls, ilvl,
			    paraNr)) {
	...
	if (listChange) {
		...
		if (docAdaptParagraphToListLevel(&indentChanged, paraNode,
						 bd)) {
			LDEB(1);
		}
	}
...
static void docInvalidateNoteLayoutForSection(BufferDocument *bd, int sect)
{
...
int docEditUpdSectProperties(EditOperation *eo, PropertyMask *pSpDoneMask,
			     BufferItem *sectNode,
			     ...
			     const DocumentAttributeMap *dam)
{
	...
	if (spSetMask) {
		if (docUpdSectProperties(&spDoneMask,
	...
	*/
		...
		if (PROPmaskISSET(&spDoneMask, SPpropNUMBER_STYLE) ||
			...
			docInvalidateNoteLayoutForSection(
				eo->eoDocument, sectNode->biNumberInParent);
		}
	}
