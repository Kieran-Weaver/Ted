...
static int docCompareTocEntries(const void *voidte1, const void *voidte2)
{
...
static int docGetTocEntry(TocEntry *te, const CalculateToc *ct)
{
	char flag[2];
	...
	if (te->teField->dfKind == DOCfkBOOKMARK) {
		...
		if (!tf->tfUseStyleNames && !tf->tfUseStyleLevels &&
		...
		/*  Holds any text? */
		{
			...
			tp = te->teDsInside.dsHead.dpNode->biParaParticules +
			     te->tePart0 + 1;
			for (part = te->tePart0 + 1; part < te->tePart1;
			     tp++, part++) {
		...
		if (tf->tfType == TOCtypeTOC && tf->tfUseStyleLevels &&
		...
		if (tf->tfType == TOCtypeTOC && tf->tfUseStyleNames &&
		...
		if (tf->tfType == TOCtypeTOC && tf->tfUseOutlineLevels &&
	...
	if (tf->tfType == TOCtypeTOC && (te->teField->dfKind == DOCfkTC ||
					 te->teField->dfKind == DOCfkTCN)) {
		DocumentField *dfBookmark;

		if (!tf->tfUseTcEntries || te->teLevel > tf->tfEntryLevel1 ||
		    te->teLevel < tf->tfEntryLevel0) {
			return 1;
		}

		flag[1] = '\0';
		if (docFieldGetTc(te->teField, flag, &(te->teLevel),
				  &(te->teNumbered))) {
			LDEB(te->teField->dfFieldNumber);
			return -1;
		}

		dfBookmark = docFindTypedChildField(
			&(ct->ctBdDoc->bdBody.dtRootFields),
			&(te->teField->dfHeadPosition), DOCfkBOOKMARK);
		if (!dfBookmark) { /*XDEB(dfBookmark);*/
			return 1;
		}
	...
	if (te->teField->dfKind == DOCfkSEQ) {
		...
		paraNode = docGetParagraphByNumber(
			&(ct->ctBdDoc->bdBody),
		...
		dfBookmark = docGetParaTocBookmark(
			ct->ctBdDoc, &(ct->ctBdDoc->bdBody), paraNode);
		if (!dfBookmark) {
			XDEB(dfBookmark);
			return -1;
		}
...
static int docCollectTocEntries(CalculateToc *ct)
{
	...
	for (fieldNr = 0; fieldNr < fieldCount; fieldNr++) {
		...
		if (df->dfKind != DOCfkBOOKMARK && df->dfKind != DOCfkSEQ &&
		    df->dfKind != DOCfkTC && df->dfKind != DOCfkTCN) {
			continue;
		}
	...
	qsort(ct->ctEntries, ct->ctEntryCount, sizeof(TocEntry),
	      docCompareTocEntries);
...
static int docTocCollectTocStyles(CalculateToc *ct)
{
	...
	ct->ctStyleLevels =
		(unsigned char *)malloc(bd->bdStyleSheet.dssStyleCount);
	if (!ct->ctStyleLevels) {
	...
	if (tf->tfUseStyleNames) {
		...
		for (j = 0; j < tf->tfStyleNameLevelCount; snl++, j++) {
			...
			ds = docGetStyleByName(&(bd->bdStyleSheet),
					       snl->snlStyleName);
			if (ds) {
				ct->ctStyleLevels[ds->dsStyleNumber] =
					snl->snlLevel;
			}

			if (utilIndexSetAdd(&(ct->ctStyleNumbers),
					    ds->dsStyleNumber)) {
				LDEB(ds->dsStyleNumber);
				return -1;
			}
		}
	}
	...
	while (dp.dpNode) {
		if (dp.dpNode->biParaStyle > 0 &&
			...
			if (level <= tf->tfStyleLevel1 &&
				...
				docGetParaTocBookmark(bd, &(bd->bdBody),
						      dp.dpNode);
			}
		}
