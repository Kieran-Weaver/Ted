...
static int docSetTocBookmark(FieldInstructions *fiBookmark, BufferDocument *bd)
{
	int rval = 0;
	...
	for (fieldNr = 0; fieldNr < fieldCount; fieldNr++) {
		...
		if (df->dfKind != DOCfkBOOKMARK || !docIsTocBookmark(&id, df)) {
			continue;
		}
	...
	if (id0 > 1) {
		...
		sprintf(markName, "_Toc%ld", id1 + 1);
	}
	...
	for (fieldNr = 0; fieldNr < fieldCount; fieldNr++) {
		...
		if (df->dfKind == DOCfkBOOKMARK &&
			...
			if (docDeleteField(&dsExInside, &eo,
					   dsInside.dsHead.dpNode,
					   dsInside.dsTail.dpNode, headPart,
					   tailPart, df)) {
				LDEB(fieldNr);
			}
		}
				  ...
				  BufferItem *paraNode)
{
...

	{
		...
		docFindParticuleOfPosition(&part, (int *)0, &(ds.dsHead),
					   PARAfindLAST);
	...
	if (dfBookmark) {
		if (docSelectionForEditPositionsInTree(
			    dsBookmark, dt, &(dfBookmark->dfHeadPosition),
...
const DocumentField *docGetParaTocBookmark(BufferDocument *bd, DocumentTree *dt,
					   BufferItem *paraNode)
{
	int res;
	DocumentSelection dsBookmark;
	...
	res = docFindParaTocBookmark(&dfBookmark, &dsBookmark, bd, dt,
				     paraNode);
	if (res < 0) {
		LDEB(res);
		goto ready;
	...
	if (res > 0) {
		if (!docIsIBarSelection(&dsBookmark)) {
			...
			if (docSurroundTextSelectionByField(
				    bd, dt, &dsBookmark, &dfBookmark,
				    (DocumentSelection *)0,
				    (DocumentSelection *)0, (int *)0, (int *)0,
...
static int docSetTocBookmarkForLevel(int ilvl, void *vsbt)
{
	BufferItem *paraNode;
...
int docSetTocBookmarks(BufferDocument *bd)
{
	...
	for (fieldNr = 0; fieldNr < fieldCount; fieldNr++) {
		...
		if (docDelimitFieldInDoc(&dsInsideTc, &dsAroundTc, (int *)0,
					 (int *)0, bd, dfTc)) {
			LDEB(1);
			rval = -1;
			goto ready;
		...
		dfBookmark = docFindTypedFieldForPosition(
			bd, &(dsInsideTc.dsHead), DOCfkBOOKMARK, 0);
		if (!dfBookmark) {
			if (docSetTocBookmark(&fiBookmark, bd)) {
				LDEB(1);
				rval = -1;
