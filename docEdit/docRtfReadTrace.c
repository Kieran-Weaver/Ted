...
static int docRtfCommitNewProps(const RtfControlWord *rcw, RtfReader *rrc)
{
...
static int docRtfReadNewProps(const RtfControlWord *rcw, int arg,
			      RtfReader *rrc)
{
	...
	res = docRtfReadGroup(rcw, 0, 0, rrc, docRtfDocumentGroups,
			      docRtfRefuseText, docRtfCommitNewProps);
	if (res) {
		SLDEB(rcw->rcwWord, res);
	}
...
static int docRtfReadTraceStep(const RtfControlWord *rcw, int arg,
			       RtfReader *rrc)
{
	int res;
	...
	res = docRtfReadGroup(rcw, 0, 0, rrc, docRtfTraceDetailGroups,
...
#define RTF_TRACE_CMD(word, id) RTF_DEST_XX((word), (id), docRtfReadTraceStep)
...
const char *docRtfGetTraceTag(int command)
{
	...
	if (docRtfTraceGroupCount != EDITcmd_COUNT || command < 0 ||
	...
	if (rcw->rcwID != command) {
		LLSDEB(command, rcw->rcwID, rcw->rcwWord);
		return (const char *)0;
	}
...
static int docRtfReadEditStep(EditStep *es, SimpleInputStream *sis, int readOld,
			      int readNew, const BufferDocument *bdRef)
{
	int rval = 0;
	int res;
	...
	if (es->esCommand == EDITcmdUPD_SPAN_PROPS ||
		...
		if (PROPmaskISSET(&(es->esNewStyle.dsRowMask),
				  RPpropCELL_LAYOUT) ||
		    PROPmaskISSET(&(es->esNewStyle.dsRowMask),
				  RPpropCELL_PROPS)) {
			PROPmaskUNSET(&(es->esNewStyle.dsCellMask),
				      CLpropCELLX);
		}
	}

	if (docUpdDocumentProperties((PropertyMask *)0, &(es->esNewDocProps),
				     &(rr->rrcDocPropertyMask),
				     &(rr->rrcDocumentProperties), dam0)) {
		LDEB(es->esCommand);
		rval = -1;
		goto ready;
	}
...
int docRtfEditRangeProperty(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	switch (rcw->rcwID) {
...
static int docRtfReadTraceStepN(EditStep *es, int readOld, int readNew,
				const EditTrace *et, int n,
				const BufferDocument *bdRef)
{
	...
	if (!sis) {
		LPDEB(et->etTraceFileHandle, sis);
		rval = -1;
		goto ready;
	}
...
static int docRtfReadExtendedReplace(EditStep *es, int readOld, int readNew,
				     const EditTrace *et, int n,
				     const BufferDocument *bdRef)
{
	int rval = 0;
...
int docEditReadTraceStep(EditStep *es, int *pIsRepeat, int direction,
			 const EditTrace *et, const BufferDocument *bdRef)
{
	int rval = 0;
	int n;
	...
	case EDITcmdEXTEND_REPLACE:
		if (docRtfReadExtendedReplace(es, readOld, readNew, et, n - 1,
					      bdRef)) {
			LDEB(1);
			rval = -1;
			goto ready;
	...
	case EDITcmdREPLACE:
		...
		if (ts && ts->tsCommand == EDITcmdEXTEND_REPLACE) {
			if (docRtfReadExtendedReplace(es, readOld, readNew, et,
						      n, bdRef)) {
				LDEB(1);
				rval = -1;
				goto ready;
			...
			if (docRtfReadTraceStepN(es, readOld, readNew, et, n,
						 bdRef)) {
				LDEB(1);
				rval = -1;
				goto ready;
