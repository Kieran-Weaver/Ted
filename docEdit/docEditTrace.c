...
int docEditTraceSetTempName(EditTrace *et, const char *extension)
{
	...
	utilMemoryBufferPrintf(&(et->etTraceFileName), "/tmp/%s.%s", scratch,
			       extension);
	return 0;
}
...
int docEditTraceOpenTrace(EditTrace *et, int restart, int exclusive)
{
	...
	fdTrace = sioFdOpenFile(&(et->etTraceFileName), read, write, append,
				create, exclusive);
	if (fdTrace < 0) {
			...
			const EditTrace *et, int from)
{
	...
	if (n < 0 || n >= et->etCount) { /*LLDEB(n,et->etCount);*/
		return -1;
	}
