...
int docInsertParagraph(EditOperation *eo, DocumentPosition *dpNew,
		       BufferItem *paraNode, int after, int textAttributeNumber)
{
...

	{
		...
		docEditShiftReferences(eo, &(eo->eoSelectionScope), paraNr + 1,
	...
	if (docEditUpdParaProperties(eo, &ppChgMask, newParaBi, &ppUpdMask,
				     &(paraNode->biParaProperties),
				     (const DocumentAttributeMap *)0)) {
		LDEB(1);
	}

	if (!docInsertTextParticule(newParaBi, 0, 0, 0, DOCkindSPAN,
				    textAttributeNumber)) {
		LDEB(1);
		return -1;
	}
...
int docInsertSection(EditOperation *eo, DocumentPosition *dpBeforeSplit,
		     DocumentPosition *dpAfterSplit, BufferItem *paraNode,
		     int split, int after, int textAttributeNumber)
{
	BufferItem *sectNode;
	...
	if (split) {
		...
		if (docNodeAtExtremity(&atExtremity, sectNode, paraNode,
				       after)) {
			LDEB(after);
			return -1;
		}
	...
	if (split) {
		...
		newSectNode = docInsertNode(eo->eoDocument, parentNode, pos,
					    DOClevSECT);
		if (!newSectNode) {
			XDEB(newSectNode);
			return -1;
