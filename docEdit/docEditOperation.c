...
static void docSetInitialRange(EditOperation *eo, const DocumentSelection *ds)
{
...
void docEditIncludeNodeInReformatRange(EditOperation *eo, BufferItem *bi)
{
...
void docEditIncludeRowsInReformatRange(EditOperation *eo,
				       BufferItem *parentNode, int row0,
				       int row1)
{
...
void docSetParagraphAdjust(EditOperation *eo, BufferItem *paraNode,
			   int stroffShift, int stroffUpto)
{
	...
	if (er->erHead.epParaNr == paraNr && er->erTail.epParaNr == paraNr &&
...
void docExtendParagraphAdjust(EditOperation *eo, BufferItem *paraNode,
			      int stroffShift)
{
	int paraNr;
	...
	if (eo->eoReformatNeeded != REFORMAT_ADJUST_PARAGRAPH) {
		LLDEB(eo->eoReformatNeeded, REFORMAT_ADJUST_PARAGRAPH);
		return;
	}
	if (eo->eoParaAdjustParagraphNumber != paraNr) {
		LLDEB(eo->eoParaAdjustParagraphNumber, paraNr);
		return;
	}
				  ...
				  const EditOperation *eo)
{
	...
	docSetRangeSelection(dsNew, &(eo->eoHeadDp), &(eo->eoTailDp),
			     direction);
}
			  ...
			  BufferDocument *bd)
{
	...
	/*  1  */
	if (docFindParticuleOfPosition(&headPart, &headFlags, &(dsBal.dsHead),
				       PARAfindPAST)) {
		LDEB(1);
		return -1;
	}
	if (docFindParticuleOfPosition(&tailPart, &tailFlags, &(dsBal.dsTail),
				       PARAfindPAST)) {
		LDEB(1);
		return -1;
	}
	...
	while ((multiPara || tailPart > headPart) && tailPart > 0 &&
	...
	if (docGetRootOfSelectionScope(&(eo->eoTree), (BufferItem **)0, bd,
				       &(eo->eoSelectionScope))) {
		LDEB(1);
		return -1;
	}
	...
	/*  2  */
	if (docBalanceFieldSelection(&dfLeft, &dfRight, &beginMoved, &endMoved,
				     &headPart, &tailPart, &(dsBal.dsHead),
				     &(dsBal.dsTail), eo->eoTree, bd)) {
		LDEB(1);
		return -1;
	}
...
int docMoveEditOperationToBodySect(EditOperation *eo, BufferItem *bodySectNode)
{
