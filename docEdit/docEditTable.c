				      ...
				      const BufferItem *refRowBi)
{
	...
	if (rowBi->biChildCount < refRowBi->biChildCount) {
		...
		colCount = refRowBi->biChildCount;
	}
	...
	for (col = 0; col < colCount; col++) {
		...
		if (docUpdParaProperties(
			    (PropertyMask *)0, &(dp.dpNode->biParaProperties),
...
int docInsertTableRows(DocumentSelection *dsRows, EditOperation *eo,
		       BufferItem *parentBi, const BufferItem *refRowBi,
		       ...
		       int paraNr, int rows)
{
	...
	for (row = 0; row < rows; n++, row++) {
		...
		if (paraNr == 1) {
			docEditShiftReferences(eo, &(eo->eoSelectionScope),
					       paraNr, 0, sectShift, paraShift,
					       stroffShift);
		} else {
			docEditShiftReferences(eo, &(eo->eoSelectionScope),
					       paraNr - 1, INT_MAX, sectShift,
					       paraShift, stroffShift);
		}

		paraNr += rp->rpCellCount;

		if (refRowBi && docCopyRowColumnAttributes(rowBi, refRowBi)) {
			LDEB(1);
			return -1;
		}
...

	{
		...
		if (docHeadPosition(&(dsNew.dsHead),
				    parentBi->biChildren[row0])) {
			LDEB(row0);
		}
		if (docTailPosition(&(dsNew.dsTail),
				    parentBi->biChildren[n - 1])) {
			LDEB(n - 1);
		}

		docSetRangeSelection(dsRows, &(dsNew.dsHead), &(dsNew.dsTail),
				     direction);
...
static BufferItem *docInsertCellInRow(BufferDocument *bd, BufferItem *rowBi,
				      int col, const CellProperties *cp,
...
int docSplitColumnInRows(BufferItem **pNewParaBi, EditOperation *eo,
			 BufferItem *parentBi, int row0, int row, int row1,
			 int col, int after)
{
	...
	while (row0 <= row1) {
		...
		if (after) {
			...
			newCp.cpRightBoundaryTwips = middle;
		}
...
int docDeleteColumnsFromRows(EditOperation *eo, BufferItem *parentBi,
			     int delRow0, int delRow1, int delCol0, int delCol1)
{
	int rval = 0;
	...
	for (row = delRow0; row <= delRow1; row++) {
		...
		if (docHeadPosition(&dsCols.dsHead,
				    rowBi->biChildren[delCol0])) {
			LDEB(1);
			rval = -1;
			goto ready;
		}
		if (docTailPosition(&dsCols.dsTail,
				    rowBi->biChildren[delCol1])) {
			LDEB(1);
			rval = -1;
			goto ready;
		}
	...
	for (row = delRow0; row <= delRow1; row++) {
		...
		docEditDeleteNodes(eo, &sectionsDeleted, &firstParaDeleted,
				   &paragraphsDeleted, rowBi, delCol0, count);
...
int docFillTableDocument(BufferDocument *bdTable, BufferItem *parentNode,
