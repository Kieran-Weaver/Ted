				   ...
				   int paraUpto)
{
	...
	if (paraFrom <= 1) {
		...
		dpBefore.dpNode =
			docGetParagraphByNumber(eo->eoTree, paraFrom - 1);
		if (!dpBefore.dpNode ||
		    docTailPosition(&dpBefore, dpBefore.dpNode)) {
			LDEB(paraFrom);
			return;
		}
			    ...
			    int paraShift, int stroffShift)
{
	...
	if (docGetRootOfSelectionScope(&dt, (BufferItem **)0, eo->eoDocument,
				       ssRoot)) {
		SDEB(docTreeTypeStr(ssRoot->ssTreeType));
		return;
	}

#if SHOW_SELECTION_RANGE
	appDebug("REFS FROM %3d:%-3d BY %2d:%d:%-2d <%s>\n", paraFrom,
		 stroffFrom, sectShift, paraShift, stroffShift,
	...
	if (docSelectionSameScope(&(eo->eoSelectionScope), ssRoot)) {
		...
		docShiftEditRange(&(eo->eoReformatRange), paraFrom, stroffFrom,
				  paraShift, stroffShift);
		docShiftEditRange(&(eo->eoSelectedRange), paraFrom, stroffFrom,
				  paraShift, stroffShift);

		if (eo->eoSplitAtHead) {
			docAdjustEditPositionE(&(eo->eoAffectedRange.erTail),
					       paraFrom, stroffFrom, paraShift,
					       stroffShift);
		} else {
			docShiftEditRange(&(eo->eoAffectedRange), paraFrom,
					  stroffFrom, paraShift, stroffShift);
		}
...
int docEditShiftParticuleOffsets(EditOperation *eo, BufferItem *paraBi,
				 ...
				 int stroffFrom, int stroffShift)
{
	...
	docAdjustEditPositionOffsetB(&(eo->eoSelectedRange.erHead), paraNr,
				     stroffFrom, stroffShift);
	docAdjustEditPositionOffsetE(&(eo->eoSelectedRange.erTail), paraNr,
				     stroffFrom, stroffShift);

	docAdjustEditPositionOffsetB(&(eo->eoAffectedRange.erHead), paraNr,
				     stroffFrom, stroffShift);
	docAdjustEditPositionOffsetE(&(eo->eoAffectedRange.erTail), paraNr,
				     stroffFrom, stroffShift);

	rval = docShiftParticuleOffsets(bd, paraBi, partFrom, partUpto,
					stroffShift);
