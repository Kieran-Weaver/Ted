...
static BufferItem *docCopyXNode(DocumentCopyJob *dcj,
				...
				BufferItem *parentNodeTo, int n,
				const BufferItem *biFrom);

/************************************************************************/

static BufferItem *docCopyCellNode(DocumentCopyJob *dcj,
				   const SelectionScope *ssRoot,
				   BufferItem *rowNodeTo, int colTo,
				   ...
				   int copyProperties)
{
	...
	if (copyProperties) {
		...
		if (rpTo->rpCellCount > 0 && colTo == rpTo->rpCellCount) {
			...
			left = docColumnLeft(rpTo, colTo);
		}

		cpScratch.cpRightBoundaryTwips =
			left + docColumnWidth(rpFrom, colFrom);

		if (docInsertRowColumn(rpTo, colTo, shiftTail, &cpScratch,
				       &(dcj->dcjAttributeMap))) {
			LDEB(1);
			return (BufferItem *)0;
		}
...
int docCopySectChildren(DocumentCopyJob *dcj, BufferItem *sectNodeTo,
			const BufferItem *sectBiFrom)
{
	...
	for (row = 0; row < sectBiFrom->biChildCount; row++) {
		...
		rowNodeTo = docCopyXNode(dcj,
					 &(sectNodeTo->biSectSelectionScope),
				   ...
				   const BufferItem *sectBiFrom)
{
	...
	if (parentNodeTo) {
		...
		docInitNode(sectNodeTo, (BufferItem *)0, bdTo, n, DOClevSECT,
			    ssRoot->ssTreeType);
	}
...
BufferItem *docCopyNode(DocumentCopyJob *dcj, BufferItem *parentNodeTo, int n,
			const BufferItem *biFrom)
{
	EditOperation *eo = dcj->dcjEditOperation;
