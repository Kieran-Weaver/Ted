...
void docDescribeSelection(SelectionDescription *sd, const DocumentSelection *ds,
			  BufferDocument *bd, int headPage, int tailPage,
			  unsigned int documentId, int documentRo)
{
	...
	docGetFramePropertiesByNumber(&fp, bd,
	...
	if (sd->sdHeadInFrame && sd->sdInContiguousParagraphs) {
		...
		for (para = dpHead->dpNode->biNumberInParent + 1;
		     para < dpTail->dpNode->biNumberInParent; para++) {
	...
	if (sd->sdHeadInTable) {
		...
		if (rowNode && rowNode->biRowIsTableHeader) {
	...
	sd->sdHeaderTypeForSelection = docWhatPageHeader(
		(DocumentTree **)0, (int *)0, bodySectNode, sd->sdHeadPage, bd);
	sd->sdFooterTypeForSelection = docWhatPageFooter(
		(DocumentTree **)0, (int *)0, bodySectNode, sd->sdTailPage, bd);
	...
	if (documentRo) {
		...
		sd->sdCanReplace = (!sd->sdHeadInTable && !sd->sdTailInTable) ||
	...
	sd->sdTextAttributeNumber = docGetSelectionAttributes(
		bd, ds, &(sd->sdTextAttributeMask), &(sd->sdTextAttribute));
	...
	sd->sdHasLists = !docFindListOfSelection(
		&(sd->sdListOverride), &(sd->sdListLevel), &(sd->sdMultiList),
		&(sd->sdMultiLevel), &(sd->sdFirstListParaNr), ds, selTree, bd);

	sd->sdIsListBullet = 0;
	if (sd->sdHasLists && sd->sdIsSingleParagraph &&
		...
		if (docDelimitParaHeadField(&dfHead, &dsInsideHead,
			...
			if (dsAroundHead.dsHead.dpStroff == dpHead->dpStroff &&
				...
				const ParagraphProperties *pp =
					&(dpHead->dpNode->biParaProperties);

				sd->sdIsListBullet = 1;

				if (docGetListLevelOfParagraph(
					    startPath, formatPath, &lo, &dl,
					    &ll, pp, bd)) {
					LLDEB(pp->ppListOverride,
					      pp->ppListLevel);
				} else {
					sd->sdTextAttributeMask =
						ll->llTextAttributeMask;
					sd->sdTextAttribute =
						ll->llTextAttribute;
			   ...
			   const SelectionDescription *sd)
{
	...
	if (!sd->sdDocumentReadonly) {
		cmdEnabled[EDITcmdSET_HYPERLINK] =
			...
			(sd->sdHeadInHyperlink || !sd->sdIsIBarSelection);

		cmdEnabled[EDITcmdSET_BOOKMARK] =
			sd->sdIsSingleParagraph &&
			(sd->sdHeadInBookmark || !sd->sdHeadInField);

		cmdEnabled[EDITcmdDEL_FIELD] =
			sd->sdHeadInField; /* Note: kind */
		cmdEnabled[EDITcmdUPD_FIELD] =
			sd->sdHeadInField; /* Note: kind */

		cmdEnabled[EDITcmdUPD_LIST] =
			sd->sdHasLists && sd->sdListOverride >= 1 &&
			!sd->sdMultiList && !sd->sdMultiLevel;

		cmdEnabled[EDITcmdSET_NEW_LIST] = sd->sdInContiguousParagraphs;

		cmdEnabled[EDITcmdINS_NOTE] =
			sd->sdCanReplace && sd->sdIsSingleParagraph &&
			sd->sdInDocumentBody && !sd->sdHeadInTableHeader &&
			!sd->sdHeadInField;
		...
		cmdEnabled[EDITcmdUPD_SECT_PROPS] = sd->sdInTreeType ==
						    DOCinBODY;
		cmdEnabled[EDITcmdUPD_SECTDOC_PROPS] = sd->sdInTreeType ==
						       DOCinBODY;
		cmdEnabled[EDITcmdUPD_DOC_PROPS] = sd->sdInTreeType ==
						   DOCinBODY;

		cmdEnabled[EDITcmdREPLACE] = sd->sdCanReplace;

		cmdEnabled[EDITcmdREPLACE_FTNSEP] =
			sd->sdCanReplace && sd->sdInTreeType == DOCinFTNSEP;

		cmdEnabled[EDITcmdREPLACE_BODY_LEVEL] =
			sd->sdCanReplace && !sd->sdHeadInTable &&
			!sd->sdTailInTable && sd->sdInTreeType == DOCinBODY;

		cmdEnabled[EDITcmdDELETE_SELECTION] =
			!sd->sdIsIBarSelection &&
			(sd->sdCanReplace || sd->sdIsTableSlice);
		...
		cmdEnabled[EDITcmdMERGE_PARAS] = !sd->sdIsSingleParagraph &&
						 sd->sdInContiguousParagraphs;

		cmdEnabled[EDITcmdDELETE_PARA] = sd->sdInContiguousParagraphs;

		cmdEnabled[EDITcmdDELETE_SECT] = sd->sdInTreeType ==
							 DOCinBODY &&
						 sd->sdDocumentSections > 1;

		cmdEnabled[EDITcmdDELETE_TABLE] = sd->sdInOneTable;
		cmdEnabled[EDITcmdDELETE_ROW] =
			sd->sdInOneTable && !sd->sdTableRectangle.trIsColSlice;
		cmdEnabled[EDITcmdDELETE_COLUMN] =
			sd->sdInOneTable && !sd->sdTableRectangle.trIsRowSlice;
		...
		cmdEnabled[EDITcmdINSERT_TABLE] =
			sd->sdCanReplace && sd->sdIsSingleParagraph &&
			!sd->sdHeadInTable && !sd->sdTailInTable;
		...
		cmdEnabled[EDITcmdINSERT_HEADER] = sd->sdInTreeType ==
						   DOCinBODY;
		cmdEnabled[EDITcmdINSERT_FOOTER] = sd->sdInTreeType ==
						   DOCinBODY;

		cmdEnabled[EDITcmdDELETE_HEADER] =
			sd->sdInTreeType == DOCinBODY ||
			docIsHeaderType(sd->sdInTreeType);
		cmdEnabled[EDITcmdDELETE_FOOTER] =
			sd->sdInTreeType == DOCinBODY ||
		...
		cmdEnabled[EDITcmdSHIFT_RIGHT] =
			cmdEnabled[EDITcmdUPD_PARA_PROPS];
		cmdEnabled[EDITcmdSHIFT_LEFT] =
			cmdEnabled[EDITcmdUPD_PARA_PROPS];
