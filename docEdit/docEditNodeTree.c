			   ...
			   BufferItem *node)
{
	...
	while (node && node->biChildCount == 0) {
		...
		if (!parent) {
			LXDEB(node->biNumberInParent, node->biParent);
			break;
		}
...
int docRemoveSelectionTail(EditOperation *eo)
{
	...
	for (;;) {
		...
		while (prevParaBi && prevParaBi != eo->eoTailDp.dpNode &&
		...
		if (prevParaBi->biParent == parent) {
			...
			if (parentCount == 0 && lastParaBi->biNumberInParent <
							parent->biChildCount) {
				mergeTail = 1;
			}
		}
		...
		docEditDeleteNodes(eo, &sectionsDeleted, &firstParaDeleted,
				   &paragraphsDeleted, parent, from, count);
	...
	if (sectShift != 0) {
		...
		docEditShiftReferences(eo, &(eo->eoSelectionScope), paraNr,
				       stroffFrom, sectShift, paraShiftX,
				       stroffShift);
	}
...
void docEditDeleteNodes(EditOperation *eo, int *pSectionsDeleted,
			...
			BufferItem *parentNode, int first, int count)
{
	int i;
	...
	if (count > 0) {
		...
		if (docHeadPosition(&dp, parentNode->biChildren[first])) {
			...
			firstParaDeleted = docNumberOfParagraph(dp.dpNode);
		}

		/*  1  */
		if (docTailPosition(
			    &dp, parentNode->biChildren[first + count - 1])) {
			LDEB(1);
		} else {
			lastStroff = docParaStrlen(dp.dpNode);
		}
	}
	...
	if (count > 0) {
		...
		if (firstParaDeleted < 0) {
			...
			docEditAvoidDeletedParagraphs(
				eo, &(eo->eoSelectionScope), firstParaDeleted,
				firstParaDeleted + paragraphsDeleted);

			docEditShiftReferences(eo, &(eo->eoSelectionScope),
					       firstParaDeleted +
						       paragraphsDeleted - 1,
					       lastStroff, sectShift,
					       -paragraphsDeleted, stroffShift);
		}
	...
	*/
	...
	if (first == 0 && (parentNode->biLevel == DOClevCELL ||
...
int docSectionParagraph(EditOperation *eo, BufferItem **pParaBi,
			BufferItem *sectBi, int sectShift,
			const ParagraphProperties *pp, int textAttributeNumber)
{
	...
	if (pp) {
		...
		if (docEditUpdParaProperties(eo, &ppChgMask, paraBi, &ppUpdMask,
					     pp,
...

	{
		...
		docEditShiftReferences(eo, &(sectBi->biSectSelectionScope),
				       paraNr, stroffFrom, sectShift, paraShift,
				       stroffShift);
	}
...
int docRollNodeChildren(EditOperation *eo, BufferItem *parentNode, int from,
			int upto, int by)
{
	int rval = 0;
	int del0 = upto;
...

	{
		...
		docEditDeleteNodes(eo, &sectionsDeleted, &firstParaDeleted,
				   &paragraphsDeleted, parentNode, del0,
				   del1 - del0 + 1);
	}
