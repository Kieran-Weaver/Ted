...
static int docSetPropsEnterNode(BufferItem *node, const DocumentSelection *ds,
	...
	case DOClevSECT:
		...
		if (setProps->sp_spSetMask && node->biTreeType == DOCinBODY) {
			if (docEditUpdSectProperties(
				    eo, setProps->sp_spDoneMask, node,
				    setProps->sp_spSetMask, setProps->sp_spSet,
...
int docChangeSelectionProperties(
	...
	PropertyMask *selTaDoneMask, const PropertyMask *taSetMask,
...

{
	...
	if (dpSetMask &&
	    docChangeDocumentProperties(eo, docDpDoneMask, dpSetMask, dpSet,
					setProps.spAttributeMap)) {
		LDEB(1);
		return -1;
	}
	...
	if (!ds) {
		if (docScanTree(eo->eoDocument, eo->eoTree,
				docSetPropsEnterNode, (NodeVisitor)0, flags,
				(void *)&setProps) < 0) {
			LDEB(1);
			return -1;
		}
		...
		if (docSelectionInsideParagraph(ds) && !ppSetMask &&
		    !cpSetMask && !rpSetMask && !spSetMask && !dpSetMask) {
		...
		if (docScanSelection(eo->eoDocument, ds, docSetPropsEnterNode,
				     (NodeVisitor)0, flags,
				     (void *)&setProps) < 0) {
			LDEB(1);
			return -1;
		...
		if (cpSetMask && !setProps.spGotCell) {
			BufferItem *cellNode =
				docGetCellNode(ds->dsHead.dpNode);
		...
		if (spSetMask && !setProps.spGotSect) {
			BufferItem *sectNode =
				docGetSectNode(ds->dsHead.dpNode);
		...
		if (spSetMask && ds->dsHead.dpNode->biTreeType != DOCinBODY) {
			...
			if (docSetPropsEnterNode(bodySectNode, ds, bodySectNode,
						 (void *)&setProps) < 0) {
				XDEB(spSetMask);
				return -1;
			}
