...
static int docFieldInsertEndParticule(BufferDocument *bd,
				      const DocumentPosition *dpEnd, int part1,
				      int *pTailPart, int fieldNumber,
				      ...
				      const TextAttribute *taSet)
{
	...
	/*  3  */
	tp = docMakeSpecialParticule(paraBi, part1, dpEnd->dpStroff,
...
static int docFieldInsertStartParticule(
	BufferDocument *bd, const DocumentPosition *dpStart, int *pStroffShift,
	int part0, int *pHeadPart, int *pTailPart, int fieldNumber,
	const PropertyMask *taSetMask, const TextAttribute *taSet)
{
	...
	/*  3  */
	tp = docMakeSpecialParticule(paraBi, part0, dpStart->dpStroff,
	...
	int fieldKind, const FieldInstructions *fi)
{
	...
	if (singlePara &&
	    dsBalanced.dsHead.dpNode != dsBalanced.dsTail.dpNode) {
		LXXDEB(singlePara, dsBalanced.dsHead.dpNode,
	...
	if (docFieldInsertEndParticule(bd, &(dsBalanced.dsTail), tailPart,
				       &tailPart, df->dfFieldNumber, taSetMask,
				       taSet)) {
		LDEB(1);
		return -1;
	}
	...
	if (taSetMask && !utilPropMaskIsEmpty(taSetMask)) {
		...
		if (docChangeParticuleAttributes(
			    (int *)0, &taDoneMask, bd, dsBalanced.dsHead.dpNode,
			    headPart, tailPart, taSet, taSetMask)) {
			LDEB(1);
		}
	}
	...
	if (docDelimitFieldInDoc(dsInside, dsAround, pHeadPart, pTailPart, bd,
				 df)) {
...
static int docDeleteFieldParticule(EditOperation *eo, BufferItem *paraBi,
				   int paraNr, int part)
{
	int textAttrNr;
	int stroffShift = 0;
	...
	docParaStringReplace(&stroffShift, paraBi, partStroff,
			     partStroff + partStrlen, (const char *)0, 0);
	docDeleteParticules(paraBi, part, 1);

	docEditShiftParticuleOffsets(eo, paraBi, paraNr, part,
				     paraBi->biParaParticuleCount, partStroff,
				     -partStrlen);
...
int docDeleteField(DocumentSelection *dsExInside, EditOperation *eo,
		   BufferItem *paraNodeHead, BufferItem *paraNodeTail,
		   int partHead, int partTail, DocumentField *df)
{
	int rval = 0;
	...
	if (partTail < 0 || partTail >= paraNodeTail->biParaParticuleCount ||
	    partHead < 0 || partHead >= paraNodeHead->biParaParticuleCount) {
	...
	if (docDeleteFieldParticule(eo, paraNodeTail,
				    df->dfTailPosition.epParaNr, partTail)) {
		LLDEB(partTail, paraNodeTail->biParaParticuleCount);
		return -1;
	}

	if (docDeleteFieldParticule(eo, paraNodeHead,
				    df->dfHeadPosition.epParaNr, partHead)) {
		LLDEB(partHead, paraNodeHead->biParaParticuleCount);
		return -1;
	}
