	...
	int fieldKind, const FieldInstructions *fi)
{
	...
	if (docSurroundTextSelectionByField(bd, dt, &ds, &df, dsInside,
					    dsAround, pHeadPart, pTailPart,
...
static int docEditForgetFields(BufferItem *node, const DocumentSelection *ds,
			       ...
			       void *voideo)
{
	if (node->biLevel == DOClevPARA) {
		...
		if (docIntersectSelectionWithParagraph(
			    &dsPara, &partFrom, &partUpto, &headFlags,
			    &tailFlags, node, ds) < 0) {
			LDEB(1);
			return -1;
		}
...
int docDeleteFieldsInRange(EditOperation *eo, DocumentSelection *dsBalanced)
{
	...
	if (docBalanceFieldSelection(
		    &dfLeft, &dfRight, &headMoved, &tailMoved, pHeadPart,
		    pTailPart, &(dsBalanced->dsHead), &(dsBalanced->dsTail),
	...
	if (docDeleteFieldRange(&updateFlags, eo->eoDocument, &erBalanced,
				&(eo->eoTree->dtRootFields), dfLeft, dfRight)) {
		LDEB(1);
	}

	eo->eoFieldUpdate |= updateFlags;

	docScanSelection(eo->eoDocument, dsBalanced, docEditForgetFields,
			 (NodeVisitor)0, flags, (void *)eo);
