...
int docReplaceSelection(EditOperation *eo, const char *addedString,
			int addedStrlen, int addedAttrNr)
{
	int rval = 0;
	...
	if (eo->eoHeadDp.dpStroff == 0 &&
		...
		if (docRemoveParagraphFromList(eo->eoHeadDp.dpNode, eo->eoTree,
					       eo->eoDocument)) {
			LDEB(1);
		}
	}
	...
	if (!docPositionsInsideParagraph(&(eo->eoHeadDp), &(eo->eoTailDp))) {
		...
		/*  3  */
		if (eo->eoTailDp.dpStroff <
		    docParaStrlen(eo->eoTailDp.dpNode)) {
...
int docEditCleanParticules(EditOperation *eo, BufferItem *paraBi, int partFrom,
			   int partUpto)
{
	int part;
	TextParticule *tp;
	...
	for (part = partFrom; part < partUpto; tp++, part++) {
		...
		if (tp->tpKind == DOCkindFIELDHEAD ||
			...
			if (tp->tpObjectNumber >= 0) {
				...
				df = docGetFieldByNumber(
					&(eo->eoDocument->bdFieldList),
					tp->tpObjectNumber);
				if (df && df->dfFieldNumber >= 0) {
					LSLDEB(part, docKindStr(tp->tpKind),
					       df->dfFieldNumber);
					docListFieldParticule(0, "FLD", part,
							      paraBi, tp, df);
					return -1;
				}
			}
...
int docMergeParagraphsInSelection(EditOperation *eo)
{
	...
	for (;;) {
		...
		while (partFrom < biFrom->biParaParticuleCount - 1 &&
		       docIsIndentationParticule(
			       biFrom, biFrom->biParaParticules + partFrom)) {
			partFrom++;
		}
		...
		if (partFrom < biFrom->biParaParticuleCount) {
			...
			if (docCopyParticules(
				...
				LDEB(biFrom->biParaParticuleCount);
				rval = -1;
				goto ready;
			}
		}
