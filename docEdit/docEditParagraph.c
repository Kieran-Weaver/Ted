...
static int docEditParaFindHeadPositions(int *pPartHead, int *pStroff0,
					int *pSplitHead, int *pMergeHead,
					...
					int addedAttrNr)
{
	...
	if (docFindParticuleOfPosition(&partHead, &headFlags, dpHead,
				       PARAfindPAST)) {
		LDEB(1);
		return -1;
	}
	...
	if (partHead < paraNode->biParaParticuleCount) {
		...
		if (tp->tpKind == DOCkindSPAN &&
		    (addedAttrNr < 0 || addedAttrNr == tp->tpTextAttrNr)) {
			stroff0 = tp->tpStroff;
		}

		if (partHead > 0 && tp[-1].tpKind == DOCkindSPAN &&
		    dpHead->dpStroff == tp->tpStroff &&
		    (addedAttrNr < 0 || addedAttrNr == tp[-1].tpTextAttrNr)) {
			mergeHead = 1;
			partHead--;
		...
		if (paraNode->biParaParticuleCount > 0 &&
		    ...
		    (addedAttrNr < 0 || addedAttrNr == tp->tpTextAttrNr) &&
		    docParaPastLastNonBlank(paraNode, tp->tpStroff, stroff0) ==
			    stroff0) {
			mergeHead = 1;
			partHead--;
			stroff0 = tp->tpStroff;
...
static int docEditParaFindTailPositions(int *pPartUpto, int *pStroff1,
					int *pSplitTail, int *pMergeTail,
					...
					int addedAttrNr)
{
	...
	if (docFindParticuleOfPosition(&partUpto, &tailFlags, dpTail,
				       PARAfindPAST)) {
		LDEB(1);
		return -1;
	}
	...
	if (partUpto < dpTail->dpNode->biParaParticuleCount) {
		...
		if (partUpto < dpTail->dpNode->biParaParticuleCount - 1 &&
			...
			if (tp->tpKind == DOCkindSPAN) {
				...
				if (dpTail->dpStroff != tp->tpStroff) {
					LLLDEB(dpTail->dpStroff, tp->tpStroff,
					       tp->tpStrlen);
					return -1;
				}
			}
		       ...
		       int addedAttributeNr)
{
	...
	/*  B,C  */
	if (docEditParaFindHeadPositions(&partHead, &stroff0, &splitHead,
					 &mergeHead, dpHead,
					 addedAttributeNr)) {
	...
	/*  D,E  */
	if (docEditParaFindTailPositions(&partUpto, &stroff1, &splitTail,
					 &mergeTail, &dpTail,
					 addedAttributeNr)) {
	...
	/*  G  */
	stroffShift = 0;
	if (docParaStringReplace(&stroffShift, dpHead->dpNode, dpHead->dpStroff,
				 stroffTail, addedText, addedLength)) {
		LDEB(addedLength);
		rval = -1;
		goto ready;
	}
	...
	stroff1 += stroffShift;
	partsMade = docRedivideStringInParticules(dpHead->dpNode, stroff0,
						  stroff1 - stroff0, partHead,
						  partsFree, addedAttributeNr);
	if (partsMade < 0) {
		LDEB(partsMade);
		rval = -1;
		goto ready;
	...
	if (partsMade < partsFree) {
		docDeleteParticules(dpHead->dpNode, partHead + partsMade,
				    partsFree - partsMade);
		...
		if (dpHead->dpNode->biParaParticuleCount == 0) {
			...
			tp = docInsertTextParticule(
				dpHead->dpNode, partHead,
				docParaStrlen(dpHead->dpNode), 0, DOCkindSPAN,
				addedAttributeNr);
			if (!tp) {
				XDEB(tp);
				rval = -1;
	...
	if (docEditShiftParticuleOffsets(eo, dpHead->dpNode, paraNr,
					 partHead + partsMade,
					 dpHead->dpNode->biParaParticuleCount,
	...
	/*  K  */
	docSetDocumentPosition(dpNewTail, dpHead->dpNode,
			       dpHead->dpStroff + addedLength);
...
int docParaDeleteText(EditOperation *eo, int paraNr, DocumentPosition *dpTail,
		      const DocumentPosition *dpHead, unsigned int stroffTail)
{
	return docParaReplaceText(eo, paraNr, dpTail, dpHead, stroffTail,
				  (const char *)0, 0, -1);
}
...
static int docParaMergeParticule(EditOperation *eo, const int paraNrTo,
				 const int textAttrTo, DocumentPosition *dpTo,
				 int *pPartTo, DocumentPosition *dpFrom,
				 int *pPartFrom)
{
	...
	const TextParticule *const tpFrom =
		nodeFrom->biParaParticules + *pPartFrom;
	...
	if (docFindParticuleOfPosition(pPartTo, (int *)0, dpTo, PARAfindPAST)) {
		LDEB(dpTo->dpStroff);
		return -1;
	}
	...
	if (*pPartTo < dpTo->dpNode->biParaParticuleCount &&
		...
		if (docSplitTextParticule((TextParticule **)0,
					  (TextParticule **)0, dpTo->dpNode,
					  *pPartTo, dpTo->dpStroff)) {
			LDEB(*pPartTo);
			return -1;
		}
	}
...
int docParaInsertTail(DocumentCopyJob *dcj, int paraNrTo,
		      DocumentPosition *dpTail, const DocumentPosition *dp_To,
		      const DocumentPosition *dp_From)
{
	...
	if (docFindParticuleOfPosition(&partFrom, &fromFlags, &dpFrom,
				       PARAfindPAST)) {
		LDEB(1);
		return -1;
	}
	...
	if (nodeFrom->biParaListOverride > 0) {
		...
		if (docDelimitParaHeadField(
			    &dfHead, &dsInsideHead, &dsAroundHead, &partHead,
			    &partTail, nodeFrom,
			    dcj->dcjSourceDocument)) { /*LDEB(nodeFrom->biParaListOverride);*/
		} else {
...

	{
		...
		if (tpFrom->tpKind == DOCkindSPAN) {
			textAttrTo = docMapTextAttributeNumber(
				dcj, tpFrom->tpTextAttrNr);
		} else {
			textAttrTo = INT_MAX; /* not an existing value */
		}
	}

	/*  B,C  */
	if (docEditParaFindHeadPositions(&partTo, &stroff0, &splitHead,
					 &mergeHead, &dpTo, textAttrTo)) {
		LDEB(dpTo.dpStroff);
		rval = -1;
		goto ready;
	}
	...
	if (mergeHead) {
		if (docParaMergeParticule(eo, paraNrTo, textAttrTo, &dpTo,
					  &partTo, &dpFrom, &partFrom)) {
			LDEB(1);
			rval = -1;
			goto ready;
		}
	...
	if (dpTo.dpStroff < docParaStrlen(dpTo.dpNode)) {
		const TextParticule *const tpFrom =
			nodeFrom->biParaParticules +
			nodeFrom->biParaParticuleCount - 1;

		const TextParticule *const tpTo =
			nodeTo->biParaParticules + partTo;

		textAttrTo =
			docMapTextAttributeNumber(dcj, tpFrom->tpTextAttrNr);
	...
	/*  E  */
	particulesCopied =
		nodeFrom->biParaParticuleCount - partFrom - copyLastAsText;
	...
	if (copyLastAsText > 0) {
		...
		if (docParaReplaceText(eo, paraNrTo, &dpTo, &dpTo,
				       dpTo.dpStroff, addedString, addedStrlen,
				       textAttrTo)) {
			LDEB(addedStrlen);
			rval = -1;
			goto ready;
...
static void docEditShiftSplitReferences(EditOperation *eo, int splitLevel)
{
	...
	docEditShiftReferences(eo, &(eo->eoSelectionScope), paraNr, stroffFrom,
...
static int docMoveTailToNew(EditOperation *eo, DocumentCopyJob *dcj,
			    BufferItem *newNode, BufferItem *oldNode, int part)
{
	TextParticule *newTp;
	...
	/*  2  */
	if (docParaStringReplace(&stroffShift, newNode, docParaStrlen(newNode),
				 docParaStrlen(newNode),
				 (char *)docParaString(oldNode, stroff),
				 docParaStrlen(oldNode) - stroff)) {
		LLDEB(docParaStrlen(oldNode), stroff);
		return -1;
	}
	...
	if (newNode->biParaParticuleCount == 0) {
		tp = oldNode->biParaParticules + oldNode->biParaParticuleCount -
		     1;

		newTp = docInsertTextParticule(newNode, -1, 0, 0, DOCkindSPAN,
					       tp->tpTextAttrNr);
		if (!newTp) {
			LDEB(newNode->biParaParticuleCount);
			return -1;
		}
	}
	...
	if (oldNode->biParaParticuleCount == 0) {
		...
		newTp = docInsertTextParticule(oldNode, -1, 0, 0, DOCkindSPAN,
					       tp->tpTextAttrNr);
		if (!newTp) {
			LDEB(oldNode->biParaParticuleCount);
			return -1;
		}
	} else {
		docCheckNoBreakAsLast(eo, oldNode);
	}
...
int docEditSplitParaParent(EditOperation *eo, BufferItem **pBeforeNode,
			   BufferItem **pAfterNode, BufferItem *paraNode,
			   int after, int splitLevel)
{
	...
	if (docSplitGroupNodeAtLevel(
		    eo->eoDocument, &beforeNode, &afterNode, paraNode->biParent,
		    paraNode->biNumberInParent + after, splitLevel)) {
		LDEB(splitLevel);
		return -1;
	}
...
int docSplitParaNode(BufferItem **pNewParaNode, EditOperation *eo,
		     int splitLevel)
{
	int rval = 0;
	...
	if (docFindParticuleOfPosition(&part, &headFlags, &(eo->eoTailDp),
				       PARAfindPAST)) {
		LDEB(eo->eoTailDp.dpStroff);
		rval = -1;
		goto ready;
	}
	...
	if (!(headFlags & (POSflagPART_HEAD | POSflagPARA_TAIL))) {
		if (docSplitTextParticule(
			    ...
			    eo->eoTailDp.dpNode, part, eo->eoTailDp.dpStroff)) {
			LXDEB(part, headFlags);
			rval = -1;
			goto ready;
		}
	...
	if (docMoveTailToNew(eo, &dcj, newParaNode, oldParaNode, part)) {
		...
		/* Try to undo! */
		docDeleteNodes(eo->eoDocument, eo->eoTree,
			       oldParaNode->biParent,
...
static TextParticule *docParaSpecialParticule(EditOperation *eo, int kind,
					      int *pPartShift,
					      int *pStroffShift)
{
	TextParticule *tp;
	...
	if (docFindParticuleOfPosition(&tailPart, &tailFlags, dp,
				       PARAfindPAST)) {
		LDEB(1);
	...
	/*  1  */
	if (docParaStringReplace(&stroffShift, dp->dpNode, dp->dpStroff,
				 dp->dpStroff, " ", 1)) {
		LDEB(dp->dpStroff);
		return (TextParticule *)0;
	}
	...
	if (dp->dpNode->biParaParticuleCount == 1 &&
		...
		*pStroffShift = stroffShift;
		...
		if (docEditShiftParticuleOffsets(
			    eo, dp->dpNode, paraNr, tailPart,
			    dp->dpNode->biParaParticuleCount, dp->dpStroff,
			    1)) {
			LDEB(1);
		}
	...
	if (tailFlags & POSflagPART_HEAD) {
		...
		*pPartShift = 1;
		...
		if (tailFlags & POSflagPART_TAIL) {
			...
			tpRet->tpObjectNumber =
				-1; /* set by caller if needed */

			*pPartShift = 2;
			tailPart += 2;
		} else {
			/*  5  */
			if (docSplitTextParticule(&tp, (TextParticule **)0,
						  dp->dpNode, tailPart,
						  dp->dpStroff)) {
				LLDEB(tailPart, dp->dpStroff);
				return (TextParticule *)0;
			}
	...
	if (docEditShiftParticuleOffsets(eo, dp->dpNode, paraNr, tailPart,
					 dp->dpNode->biParaParticuleCount,
					 dp->dpStroff, 1)) {
		LDEB(1);
	}
...
TextParticule *docEditParaSpecialParticule(EditOperation *eo, int kind)
{
	TextParticule *tp;
	int partShift = 0;
	int stroffShift = 0;
			    ...
			    BufferDocument *bd, int paraNr)
{
	...
	if (paraNode->biParaListOverride > 0) {
		...
		if (docDelimitParaHeadField(&dfHead, &dsInsideHead,
					    &dsAroundHead, &partBegin, &partEnd,
					    paraNode, bd)) {
	...
	tp = paraNode->biParaParticules + partHead;
	for (part = partHead; part < paraNode->biParaParticuleCount;
	     tp++, part++) {
		const int paraShift = 0;

		if (tp->tpStroff < stroffHead) {
			LLDEB(tp->tpStroff, stroffHead);
			continue;
		}
		if (tp->tpStroff > stroffHead) {
			break;
		...
		docParaStringReplace(&stroffShift, paraNode, tp->tpStroff,
				     tp->tpStroff + tp->tpStrlen, (char *)0, 0);

		docShiftEditRange(&(eo->eoSelectedRange), paraNr, tp->tpStroff,
				  paraShift, stroffShift);
		docShiftEditRange(&(eo->eoAffectedRange), paraNr, tp->tpStroff,
				  paraShift, stroffShift);
