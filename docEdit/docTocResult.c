...
static int docParaInsertTocEntry(CalculateToc *ct, int makeHyperlinks,
				 const TocEntry *te, const SelectionScope *ss,
				 BufferItem *paraBi)
{
	int rval = 0;
	...
	tp = docMakeSpecialParticule(paraBi, paraBi->biParaParticuleCount,
				     ep.epStroff, DOCkindFIELDHEAD, textAttrNr);
	if (!tp) {
		XDEB(tp);
		rval = -1;
		goto ready;
	...
	tp = docInsertTextParticule(paraBi, paraBi->biParaParticuleCount,
				    ep.epStroff, stroffShift, DOCkindSPAN,
				    textAttrNr);
	if (!tp) {
		XDEB(tp);
		rval = -1;
	...
	tp = docMakeSpecialParticule(paraBi, paraBi->biParaParticuleCount,
				     ep.epStroff, DOCkindFIELDTAIL, textAttrNr);
	if (!tp) {
		XDEB(tp);
		rval = -1;
		goto ready;
	...
	if (te->teNumbered) {
		/* SEPARATOR */
		if (docParaStringReplace(&stroffShift, paraBi, ep.epStroff,
					 ep.epStroff, " ", 1)) {
			LDEB(1);
			rval = -1;
			goto ready;
...
static BufferItem *docMakeTocParagraph(CalculateToc *ct, BufferItem *refBi,
				       int level, int numbered)

{
	...
	if (ds && !utilPropMaskIsEmpty(&(ds->dsParaMask))) {
		...
		if (ct->ctTocField.tfType == TOCtypeTOC && level >= 0 &&
		    level <= PPoutlineDEEPEST) {
			paraNodeToc->biParaLeftIndentTwips =
				level * dp->dpTabIntervalTwips;
		}
	}
	...
	if (numbered &&
		...
		ts.tsTwips =
			pf.pfCellContentRect.drX1 - pf.pfCellContentRect.drX0;
		...
		if (n < 0) {
			...
			paraNodeToc->biParaProperties.ppTabStopListNumber = n;
		}
	}
...
static int docStartTocField(CalculateToc *ct, const DocumentField *dfToc,
			    BufferItem *paraNodeToc)
{
	...
	ct->ctDfTocTo = docClaimFieldCopy(
		&(ct->ctBdToc->bdFieldList), dfToc,
		&(ct->ctSectNode->biSectSelectionScope), &epStart);
	if (!ct->ctDfTocTo) {
		XDEB(ct->ctDfTocTo);
		return -1;
	}
...
static int docSetupTocDocument(CalculateToc *ct)
{
	...
	ct->ctDefaultTextAttributeNumber =
		docTextAttributeNumber(ct->ctBdToc, &(ct->ctTextAttribute));
	...
	for (level = 0; level <= PPoutlineDEEPEST; level++) {
		...
		ct->ctLevelAttributeNumbers[level] =
			ct->ctDefaultTextAttributeNumber;
		if (ds && !utilPropMaskIsEmpty(&(ds->dsTextMask))) {
			...
			utilUpdateTextAttribute(&doneMask, &ta,
						...
						&(ds->dsTextAttribute));
			ct->ctLevelAttributeNumbers[level] =
				docTextAttributeNumber(ct->ctBdToc, &ta);
		}
	}
...
static int docFinishTocField(CalculateToc *ct, BufferItem *paraNodeToc)
{
	TextParticule *tp;
	int textAttrNr = ct->ctDefaultTextAttributeNumber;
	...
	docSetDocumentPosition(&dpTail, paraNodeToc,
			       docParaStrlen(paraNodeToc));
	docSetEditPosition(&epTail, &dpTail);

	tp = docMakeSpecialParticule(paraNodeToc,
				     paraNodeToc->biParaParticuleCount,
				     epTail.epStroff, DOCkindFIELDTAIL,
				     textAttrNr);
	if (!tp) {
		XDEB(tp);
		return -1;
...
static int docGetTocParaProperties(CalculateToc *ct,
				   const DocumentSelection *dsToc, int part0,
				   int part1)
{
	int rval = 0;

	docBlockFrameTwips(&(ct->ctBlockFrame), dsToc->dsHead.dpNode,
			   ct->ctBdDoc,
...

	{
		...
		docGetTextAttributeByNumber(&(ct->ctTextAttribute), ct->ctBdDoc,
					    tp->tpTextAttrNr);

		part = part0;
		while (part < dsToc->dsHead.dpNode->biParaParticuleCount) {
			if (tp->tpKind == DOCkindSPAN) {
				docGetTextAttributeByNumber(
					&(ct->ctTextAttribute), ct->ctBdDoc,
					tp->tpTextAttrNr);
...
static int docCalculateTocField(CalculateToc *ct,
				const DocumentSelection *dsToc, int part0,
				int part1, const DocumentField *dfToc)
{
	BufferItem *paraNodeToc = (BufferItem *)0;
	...
	for (entryNr = 0; entryNr < ct->ctEntryCount; te++, entryNr++) {
		paraNodeToc = docMakeTocParagraph(ct, refBi, te->teLevel,
						  te->teNumbered);
		if (!paraNodeToc) {
			XDEB(paraNodeToc);
			return -1;
	...
	if (!paraNodeToc) {
		...
		if (docParaStringReplace(&stroffShift, paraNodeToc, stroff,
					 stroff, text, len)) {
			LDEB(len);
			return -1;
		}

		if (docRedivideStringInParticules(
			    paraNodeToc, stroff, len,
...
int docRecalculateOneTocField(BufferDocument *bdDoc, const DocumentField *dfToc)
{
	int rval = 0;
	...
	if (docDelimitFieldInDoc(&dsInsideToc, &dsAroundToc, &part0, &part1,
				 bdDoc, dfToc)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	eo.eoBodySectNode =
		docGetBodySectNodeOfScope(&(dfToc->dfSelectionScope), bdDoc);
