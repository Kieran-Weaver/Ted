	...
	while (paraNode) {
		...
		if (paraNode->biParaListOverride >= 1) {
			...
			if (ppSet.ppLeftIndentTwips >= 0) {
				...
				if (ppSet.ppLeftIndentTwips +
					    ppSet.ppFirstIndentTwips <
				    0) {
					ppSet.ppFirstIndentTwips =
						-ppSet.ppLeftIndentTwips;
					PROPmaskADD(&ppSetMask,
		...
		if (apply) {
			if (docEditUpdParaProperties(
				    eo, &ppDoneMask, paraNode, &ppSetMask,
				    &ppSet, (const DocumentAttributeMap *)0)) {
				LDEB(1);
				rval = -1;
				goto ready;
			}
