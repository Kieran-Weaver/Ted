...
static int fontReportProvideData(int *pCalculated, MemoryBuffer *mbResult,
				 const char *value)
{
	utilMemoryBufferAppendBytes(mbResult, (unsigned char *)value,
				    strlen(value));
	*pCalculated = 1;
	return 0;
}

static int fontReportProvideOptional(int *pCalculated, MemoryBuffer *mbResult,
				     const char *value)
{
	if (value) {
...
static int fontReportProvideMb(int *pCalculated, MemoryBuffer *mbResult,
...
static int docFontReportGetUnicode(const FontReport *fr, const char *fieldName,
				   int off)
{
...
static int fontReportFieldDataProvider(int *pCalculated, MemoryBuffer *mbResult,
				       const char *fieldName, void *through)
{
	...
	if (!strcmp(fieldName, "Slant")) {
		return fontReportProvideData(
			pCalculated, mbResult,
			afi->afiItalicAngle < -1 ? "Italic" : "Roman");
	}

	if (!strcmp(fieldName, "IsFixed")) {
		return fontReportProvideData(
			pCalculated, mbResult,
			afi->afiIsFixedPitch ? "Fixed" : "Proportional");
	}
	...
	if (!strcmp(fieldName, "BlockName")) {
		return fontReportProvideOptional(
			pCalculated, mbResult,
	...
	if (!strncmp(fieldName, "Nr", 2) && isdigit(fieldName[2])) {
		...
		if (uni < past && !ucdIsCn(uni)) {
			if (uni <= 0xff) {
				...
				sprintf(scratch, "0x%04x", (unsigned)uni);
			}
	...
	if (!strncmp(fieldName, "N", 1) && isdigit(fieldName[1])) {
		...
		if (uni > 0 && uni < past &&
		    utilIndexMappingGet(&(afi->afiUnicodeToGlyphMapping),
					uni) >= 0) {
	...
	if (!strncmp(fieldName, "G", 1) && isdigit(fieldName[1])) {
		...
		if (uni > 0 && uni < past &&
			...
			if (step < 1) {
				...
				return fontReportProvideData(pCalculated,
							     mbResult, scratch);
			}
		}
...
static int docSubstituteFontPage(int *pUsed, RecalculateFields *rf,
				 const FontReport *fr)
{
	int uni;
	int rval = 0;
	...
	for (uni = fr->frPageFirst; uni <= fr->frPageLast; uni++) {
		if (utilIndexMappingGet(&(afi->afiUnicodeToGlyphMapping),
					uni) >= 0) {
			used = 1;
			break;
		}
	...
	if (used) {
		...
		if (fr->frCurrentSection < bd->bdBody.dtRoot->biChildCount) {
			...
			bodySectBi = bd->bdBody.dtRoot
					     ->biChildren[fr->frCurrentSection];
			      ...
			      const MemoryBuffer *outputDir)
{
	...
	if (appAbsoluteName(&absolute, &relative, relativeIsFile, outputDir) <
	    0) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	/*  HACK internals of document font list. */
	{
		...
		dfTo = docFontListGetFontByNumber(bd->bdProperties.dpFontList,
						  n);
		if (!dfTo) {
			LXDEB(n, dfTo);
			goto ready;
		...
		for (face = 0; face < aff->psfFaceCount; face++) {
			...
			if (afi->afiWeightInt >= FONTweightDEMIBOLD) {
				if (afi->afiItalicAngle < -1.0) {
					...
					relativeName = "font-b.rtf";
				}
			} else {
				if (afi->afiItalicAngle < -1.0) {
					relativeName = "font-i.rtf";
				} else {
					relativeName = "font.rtf";
				}
			}

			if (utilMemoryBufferSetString(&relative,
						      relativeName)) {
				LDEB(1);
				rval = -1;
				goto ready;
			...
			if (docMakeFontExample(afi, psfl, &dfl, &absolute,
					       outputDir)) {
				SDEB(afi->afiFontName);
				rval = -1;
				goto ready;
