...
int docReinsertNodes(EditOperation *eo, BufferItem *parentTo,
		     const BufferItem *parentFrom, int to, const EditStep *es)
{
	int rval = 0;
	DocumentCopyJob dcj;
	int from;
	...
	if (es->esSelectionPosition == SELposTAIL) {
		...
		docEditDeleteNodes(eo, &sectionsDeleted, &firstParaDeleted,
				   &paragraphsDeleted, parentTo, to, 1);
	}
...
const PictureProperties *docTraceGetFromPictureProps(const EditStep *es)
{
	...
	if (docHeadPosition(&(dsObjFrom.dsHead),
		...
		LDEB(1);
		return (const PictureProperties *)0;
	}
