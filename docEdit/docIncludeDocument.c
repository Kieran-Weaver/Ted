...
static int docIncludeTransferParaProperties(BufferItem *paraNodeTo,
					    ...
					    DocumentCopyJob *dcjInsert)
{
	...
	if (docEditUpdParaProperties(dcjInsert->dcjEditOperation,
				     (PropertyMask *)0, paraNodeTo, &ppSetMask,
...
static int
docReplaceMultipleParagraphsWithSingle(DocumentCopyJob *dcjInsert,
				       const DocumentPosition *dpFrom,
				       int partFrom, const int paraNrBegin)
{
	...
	/*  2  */
	if (docParaDeleteText(eo, paraNrBegin, &tailDp, &(eo->eoHeadDp),
			      docParaStrlen(eo->eoHeadDp.dpNode))) {
		LDEB(1);
		rval = -1;
		goto ready;
	}

	if (docParaInsertTail(dcjInsert, paraNrBegin, &tailDp, &tailDp,
			      dpFrom)) {
		LDEB(partFrom);
		rval = -1;
		goto ready;
	...
	if (eo->eoTailDp.dpStroff < docParaStrlen(eo->eoTailDp.dpNode)) {
		...
		if (docParaInsertTail(&dcjTail, paraNrBegin, &afterDp, &tailDp,
				      &(eo->eoTailDp))) {
...
static int docInsertSingleParagraph(DocumentCopyJob *dcjInsert,
				    ...
				    int paraNrBegin)
{
	...
	if (eo->eoHeadDp.dpNode != eo->eoTailDp.dpNode) {
		if (docReplaceMultipleParagraphsWithSingle(
			    dcjInsert, &(dsFrom->dsHead), partFrom,
			    paraNrBegin)) {
			LDEB(partFrom);
			rval = -1;
			goto ready;
		...
		if (docParaInsertTail(dcjInsert, paraNrBegin, &(eo->eoTailDp),
				      &(eo->eoTailDp), &(dsFrom->dsHead))) {
			LDEB(partFrom);
			rval = -1;
			goto ready;
		}
	...
	if ((dcjInsert->dcjCopyHeadParaProperties ||
	    ...
	    docIncludeTransferParaProperties(
		    eo->eoTailDp.dpNode, dsFrom->dsHead.dpNode, dcjInsert)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
	...
	while (fromNode) {
		...
		if (fromNode->biParent &&
			...
			if (tcj->tcjToIndex < tcj->tcjToNode->biChildCount) {
				...
				if (docSplitGroupNode(
					    tcj->tcjDocument, &insertedNode,
				...
				tcj->tcjToIndex =
					insertedNode->biNumberInParent + 1;
				tcj->tcjToNode = insertedNode->biParent;
				tcj->tcjTailNode = insertedNode;
			} else {
				tcj->tcjToIndex =
					tcj->tcjToNode->biNumberInParent + 1;
...
static int docInsertMultipleParagraphs(DocumentCopyJob *dcjInsert,
				       const DocumentSelection *dsFrom,
				       int paraNrBegin)
{
	int rval = 0;
	...
	if (docFindParticuleOfPosition(&part, &partFlags, &(eo->eoTailDp),
				       PARAfindPAST)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (headSplitLevel == DOClevPARA ||
		...
		if (docNodeAtExtremity(&atExtremity, rowBi, eo->eoTailDp.dpNode,
				       after)) {
			LDEB(1);
			return -1;
		}
		...
		if (atExtremity) {
			...
			tcj.tcjFromNode = docGetRowNode(dsFrom->dsHead.dpNode);
			tcj.tcjTailNode =
				rowBi; /* Wrong for ! after, but irrelevant */
	...
	if (headSplitLevel == DOClevPARA) {
		...
		if (dcjInsert->dcjCopyHeadParaProperties &&
		    docIncludeTransferParaProperties(
			    tailDp.dpNode, dsFrom->dsHead.dpNode, dcjInsert)) {
			LDEB(1);
			rval = -1;
			goto ready;
		}
	...
	while (tcj.tcjFromNode) {
		...
		insertedNode = docCopyNode(dcjInsert, tcj.tcjToNode,
					   tcj.tcjToIndex, tcj.tcjFromNode);
		if (!insertedNode) {
			XDEB(insertedNode);
			rval = -1;
			goto ready;
	...
	if (sectShift != 0) {
		...
		docEditShiftReferences(eo, &(eo->eoSelectionScope), paraNr,
				       stroffFrom, sectShift, paraShift,
				       stroffShift);
	}

	if (bdFrom->bdProperties.dpHasOpenEnd && tailSplitLevel == DOClevPARA &&
		...
		if (docParaInsertTail(&dcjTail, paraNrTail, &dpTailTo,
				      &(eo->eoTailDp), &dpTailFrom)) {
			LDEB(partTailFrom);
			rval = -1;
			goto ready;
		}
...
int docIncludeDocument(DocumentCopyJob *dcjInsert)
{
	...
	if (bdFrom->bdProperties.dpHasOpenEnd &&
		...
		if (docInsertMultipleParagraphs(dcjInsert, &dsFrom,
						paraNrBegin)) {
			LDEB(1);
			return -1;
		}
