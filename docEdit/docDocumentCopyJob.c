...
static int docSetAttributeMap(DocumentAttributeMap *dam, BufferDocument *bdTo,
			      BufferDocument *bdFrom)
{
	...
	if (docMergeTabstopListLists(&rulerMap, &(dplTo->dplTabStopListList),
	...
	if (docMergeDocumentLists(&fontMap, &lsmap, bdTo, bdFrom, colorMap,
				  rulerMap)) {
		LDEB(1);
		rval = -1;
		goto ready;
