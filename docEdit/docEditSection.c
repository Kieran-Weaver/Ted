...
static int docCopySectHdFt(DocumentTree *eiTo, BufferDocument *bdTo,
			   const BufferItem *bodySectBi, BufferDocument *bdFrom,
			   DocumentTree *dtFrom, int treeType)
{
	int rval = 0;
	...
	if (dtFrom->dtRoot) {
		...
		sectBi = docMakeTreeRoot(bdTo, eiTo, bodySectBi, ownerNumber,
					 treeType);
		if (!sectBi) {
			XDEB(sectBi);
			rval = -1;
...
int docCopySectHeadersFooters(BufferItem *sectBiTo, BufferDocument *bdTo,
			      BufferItem *sectBiFrom, BufferDocument *bdFrom)
{
	...
	if (!shfFrom) {
		if (sectBiTo->biSectHeadersFooters) {
			docCleanSectHeadersFooters(
				bdTo, sectBiTo->biSectHeadersFooters);
			free(sectBiTo->biSectHeadersFooters);
			sectBiTo->biSectHeadersFooters =
				(SectHeadersFooters *)0;
		}
	...
	if (!sectBiTo->biSectHeadersFooters) {
		...
		if (!sectBiTo->biSectHeadersFooters) {
			XDEB(sectBiTo->biSectHeadersFooters);
			return -1;
		}
	...
	if (docCopySectHdFt(&(shfTo->shfFirstPageHeader), bdTo, sectBiTo,
			    bdFrom, &(shfFrom->shfFirstPageHeader),
			    DOCinFIRST_HEADER)) {
		LDEB(1);
		return -1;
	}
	if (docCopySectHdFt(&(shfTo->shfLeftPageHeader), bdTo, sectBiTo, bdFrom,
			    &(shfFrom->shfLeftPageHeader), DOCinLEFT_HEADER)) {
		LDEB(1);
		return -1;
	}
	if (docCopySectHdFt(&(shfTo->shfRightPageHeader), bdTo, sectBiTo,
			    bdFrom, &(shfFrom->shfRightPageHeader),
			    DOCinRIGHT_HEADER)) {
		LDEB(1);
		return -1;
	}

	if (docCopySectHdFt(&(shfTo->shfFirstPageFooter), bdTo, sectBiTo,
			    bdFrom, &(shfFrom->shfFirstPageFooter),
			    DOCinFIRST_FOOTER)) {
		LDEB(1);
		return -1;
	}
	if (docCopySectHdFt(&(shfTo->shfLeftPageFooter), bdTo, sectBiTo, bdFrom,
			    &(shfFrom->shfLeftPageFooter), DOCinLEFT_FOOTER)) {
		LDEB(1);
		return -1;
	}
	if (docCopySectHdFt(&(shfTo->shfRightPageFooter), bdTo, sectBiTo,
			    bdFrom, &(shfFrom->shfRightPageFooter),
			    DOCinRIGHT_FOOTER)) {
		LDEB(1);
		return -1;
	}
...
int docCopySectDescription(BufferItem *sectNodeTo, BufferDocument *bdTo,
			   BufferItem *sectNodeFrom, BufferDocument *bdFrom)
{
	if (sectNodeFrom->biTreeType == DOCinBODY &&
	    docCopySectHeadersFooters(sectNodeTo, bdTo, sectNodeFrom, bdFrom)) {
		LDEB(sectNodeFrom->biTreeType);
		return -1;
	}
