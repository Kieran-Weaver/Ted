...
static DocumentField *docStartFieldCopy(DocumentCopyJob *dcj,
					const EditPosition *epStart,
					int obnrFrom)
{
	EditOperation *eo = dcj->dcjEditOperation;
	BufferDocument *bdTo = eo->eoDocument;
	const DocumentFieldList *dflFrom =
		&(dcj->dcjSourceDocument->bdFieldList);
	...
	if (dfTo->dfKind == DOCfkBOOKMARK) {
		...
		if (docFieldGetBookmark(&mbFrom, dfFrom)) {
			...
			utilCopyMemoryBuffer(&mbTo, mbFrom);
		}
	...
	if (docFieldHasNote(dfFrom->dfKind) && dfFrom->dfNoteIndex >= 0) {
		...
		utilIndexSetAdd(&(dcj->dcjNoteFieldsCopied),
				dfTo->dfFieldNumber);
	}
...
static int docFinishFieldCopy(DocumentCopyJob *dcj, const EditPosition *epTail,
			      int obnrFrom)
{
	EditOperation *eo = dcj->dcjEditOperation;
	BufferDocument *bdTo = eo->eoDocument;
	...
	if (dcj->dcjFieldMap[obnrFrom] < 0) {
		LLDEB(obnrFrom, fieldCount);
		SDEB(docFieldKindStr(
			docGetFieldKindByNumber(dflFrom, obnrFrom)));
		return -1;
	}
...
static int docCopyParticuleData(DocumentCopyJob *dcj, BufferItem *biTo,
				const BufferItem *biFrom, TextParticule *tpTo,
				const TextParticule *tpFrom)
{
	...
	case DOCkindOBJECT: {
		...
		if (!ioFrom) {
			LPDEB(tpFrom->tpObjectNumber, ioFrom);
			return -1;
		}
		...
		if (ioFrom->ioKind == DOCokDRAWING_SHAPE) {
			if (!ioFrom->ioDrawingShape) {
				...
				ioTo->ioDrawingShape = docCopyDrawingShape(
					dcj, ioFrom->ioDrawingShape);
	...
	case DOCkindFIELDTAIL: {
		...
		if (dcj->dcjCopyFields) {
			tpTo->tpObjectNumber = docFinishFieldCopy(
				dcj, &ep, tpFrom->tpObjectNumber);
		} else {
...
int docCopyParticules(DocumentCopyJob *dcj, BufferItem *biTo,
		      const BufferItem *biFrom, int partTo, int partFrom,
		      ...
		      int *pCharactersCopied)
{
	...
	if (partTo == biTo->biParaParticuleCount) {
		...
		stroffTo = biTo->biParaParticules[partTo].tpStroff;
	}

	/*  2  */
	tpFrom = biFrom->biParaParticules + partFrom;
	stroffFrom = tpFrom->tpStroff;
	strlenFrom = tpFrom[countFrom - 1].tpStroff +
		     tpFrom[countFrom - 1].tpStrlen - tpFrom->tpStroff;
	...
	if (docParaStringReplace(&stroffShift, biTo, stroffTo, stroffTo,
				 (char *)docParaString(biFrom, stroffFrom),
				 strlenFrom)) {
		LDEB(strlenFrom);
	...
	/*  6  */
	if (docEditShiftParticuleOffsets(eo, biTo, paraNrTo, partTo + countFrom,
					 biTo->biParaParticuleCount, stroffTo,
					 stroffShift)) {
		LDEB(stroffShift);
	}
...
int docCopyParticuleAndData(TextParticule **pTpTo, DocumentCopyJob *dcj,
			    BufferItem *paraBiTo, int partTo, int stroffTo,
			    int strlenTo, const BufferItem *paraBiFrom,
			    const TextParticule *tpFrom)
{
	...
	textAttributeNumberTo =
		docMapTextAttributeNumber(dcj, tpSaved.tpTextAttrNr);
	...
	tpTo = docInsertTextParticule(paraBiTo, partTo, stroffTo, strlenTo,
				      tpSaved.tpKind, textAttributeNumberTo);
	if (!tpTo) {
		LXDEB(partTo, tpTo);
		return -1;
	}
...
int docCheckNoBreakAsLast(EditOperation *eo, BufferItem *paraBi)
{
	...
	if (tp->tpKind != DOCkindLINEBREAK && tp->tpKind != DOCkindPAGEBREAK &&
	    tp->tpKind != DOCkindCOLUMNBREAK) {
		return 0;
	}

	tp = docInsertTextParticule(paraBi, part, stroff, len, DOCkindSPAN,
				    tp->tpTextAttrNr);
	if (!tp) {
		XDEB(tp);
		return -1;
...
BufferItem *docCopyParaNode(DocumentCopyJob *dcj, const SelectionScope *ssRoot,
			    BufferItem *biCellTo, int n,
			    const BufferItem *biParaFrom)
{
	...
	/*  2  */
	{
		...
		docEditShiftReferences(eo, ssRoot, paraNr, stroff, sectShift,
				       paraShift, -stroff);
	}
	...
	if (biParaFrom->biParaListOverride > 0) {
		...
		if (docDelimitParaHeadField(&dfHead, &dsInsideHead,
					    &dsAroundHead, &bulletPartBegin,
					    &bulletPartEnd, biParaFrom,
					    dcj->dcjSourceDocument)) {
			LDEB(1);
		}
	...
	if (partFrom < biParaFrom->biParaParticuleCount) {
		...
		if (docCopyParticules(
			    dcj, biParaTo, biParaFrom, partTo, partFrom,
			    biParaFrom->biParaParticuleCount - partFrom,
		...
		if (!docInsertTextParticule(biParaTo, 0, 0, 0, DOCkindSPAN,
					    textAttributeNumberTo)) {
			LDEB(1);
			return (BufferItem *)0;
		}
