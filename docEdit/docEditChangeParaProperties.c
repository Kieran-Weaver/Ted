...
static int docEditChangeParaAndTextProperties(
	EditOperation *eo, PropertyMask *pPpDoneMask, PropertyMask *pTaDoneMask,
	BufferItem *paraNode, int partFrom, int partUpto,
...
static int docFormatHeadParticule(BufferItem *paraNode, int *pSplit,
...
static int docFormatTailParticule(BufferItem *paraNode, int *pSplit,
				  const PropertyMask *taSetMask,
				  const TextAttribute *taSet,
				  BufferDocument *bd,
				  const DocumentSelection *ds)
{
	...
	if (docFindParticuleOfPosition(&part1, (int *)0, &(ds->dsTail),
				       PARAfindFIRST)) {
		LDEB(ds->dsTail.dpStroff);
		return -1;
	}
				...
				const ParagraphProperties *ppSet)
{
	...
	if (paraNode == ds->dsHead.dpNode) {
		part = docFormatHeadParticule(paraNode, &split,
					      setProps->sp_taSetMask, taSet, bd,
					      ds);
		if (part < 0) {
			LDEB(part);
			return -1;
	...
	/*  4  */
	if (docEditChangeParaAndTextProperties(
		    eo, &paraPpDoneMask, &paraTaDoneMask, paraNode, part, part1,
		    setProps->sp_taSetMask, taSet, setProps->sp_ppSetMask,
		    ppSet)) {
		LDEB(1);
	}
