...
extern int docRollNodeChildren(EditOperation *eo, struct BufferItem *parentBi,
			       int from, int to, int by);

extern int docEditDeleteReplacedFields(EditOperation *eo);

extern int docParaReplaceText(EditOperation *eo, int paraNr,
...
extern int docParaDeleteText(EditOperation *eo, int paraNr,
			     DocumentPosition *dpTail,
			     const DocumentPosition *dpHead,
			     unsigned int stroffTail);

extern int docParaInsertTail(DocumentCopyJob *dcj, int paraNrTo,
			     DocumentPosition *dpTail,
			     const DocumentPosition *dpTo,
			     const DocumentPosition *dpFrom);

extern int docSplitParaNode(struct BufferItem **pNewParaBi, EditOperation *eo,
			    int splitLevel);
...
extern void docDeleteEmptyParents(EditOperation *eo, int *pSectsDeleted,
				  struct BufferItem *bi);

extern void docEditDeleteNodes(EditOperation *eo, int *pSectionsDeleted,
			       int *pFirstParaDeleted, int *pParagraphsDeleted,
			       struct BufferItem *bi, int first, int count);

extern struct TextParticule *docEditParaSpecialParticule(EditOperation *eo,
							 int kind);

extern int docReplaceSelection(EditOperation *eo, const char *addedText,
			       int addedLength, int addedAttributeNumber);

extern int docDeleteSelection(EditOperation *eo);

extern int docMergeParagraphsInSelection(EditOperation *eo);

extern int docInsertTableRows(DocumentSelection *dsRows, EditOperation *eo,
...
extern void docEditAvoidDeletedParagraphs(EditOperation *eo,
					  const SelectionScope *ssRoot,
					  int paraFrom, int paraUpto);

extern int docEditUpdParaProperties(EditOperation *eo,
...
extern int docEditUpdSectProperties(EditOperation *eo,
...
extern int docEditCleanParticules(EditOperation *eo, struct BufferItem *paraBi,
				  int partFrom, int partUpto);

extern int docCheckNoBreakAsLast(EditOperation *eo, struct BufferItem *paraBi);

extern void
docEditAdaptRowPropertiesToFirstChild(EditOperation *eo,
				      const struct BufferItem *paraBi);

extern int docEditFixParaBreakKind(EditOperation *eo, DocumentTree *dt,
				   BufferDocument *bd, int paraNr);

extern int docIncludeDocument(DocumentCopyJob *dcj);

extern int docSectionParagraph(EditOperation *eo, struct BufferItem **pParaBi,
			       struct BufferItem *sectBi, int sectShift,
			       const ParagraphProperties *pp,
			       int textAttributeNumber);

extern int docEditDeleteFieldsForBlockDelete(EditOperation *eo,
...
extern int docInsertParagraph(EditOperation *eo, DocumentPosition *dpNew,
			      struct BufferItem *paraNode, int after,
			      int textAttributeNumber);

extern int docInsertSection(EditOperation *eo, DocumentPosition *dpBeforeSplit,
			    DocumentPosition *dpAfterSplit,
			    struct BufferItem *paraNode, int split, int after,
			    int textAttributeNumber);

extern int docEditChangeList(EditOperation *eo, struct DocumentList *dl,
...
extern int docEditShiftIndent(EditOperation *eo, int step);

extern int docDeleteField(DocumentSelection *dsExInside, EditOperation *eo,
