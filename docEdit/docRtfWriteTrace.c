...
static int docRtfTraceContents(RtfWriter *rw, const char *tag,
...
int docRtfTraceNewProperties(
	EditOperation *eo, const PropertyMask *taSetMask,
...
static int docRtfTraceImageProperties(EditOperation *eo, const char *tag,
...
static int docRtfTraceList(EditOperation *eo, const char *tag,
...
static int docRtfTraceNoteProperties(EditOperation *eo, const char *textTag,
...
int docRtfTraceOldNoteProperties(EditOperation *eo,
				 ...
				 const NoteProperties *npSet)
{
	return docRtfTraceNoteProperties(eo, RTFtag_OTX, npSetMask, npSet);
}
...
int docRtfTraceNewNoteProperties(EditOperation *eo,
				 ...
				 const NoteProperties *npSet)
{
	return docRtfTraceNoteProperties(eo, RTFtag_NTX, npSetMask, npSet);
}
...
int docRtfTraceOldContentsLow(const EditOperation *eo,
			      const DocumentSelection *ds, unsigned int flags)
{
	int rval = 0;
...
int docRtfTraceOldContents(DocumentSelection *ds, const EditOperation *eo,
...
int docRtfTraceNewContents(EditOperation *eo, int posWhere)
{
	...
	docSelectionForEditPositionsInTree(&ds, eo->eoTree, &(er->erHead),
					   &(er->erTail));
...
static const char *const DOCrtfSEL_TAGS[] = { RTFtag_SEH, RTFtag_SEN,
					      RTFtag_SAL, RTFtag_SET,
					      RTFtag_SEB, RTFtag_SEA };
...
int docRtfTraceNewPosition(EditOperation *eo, const SelectionScope *ssNew,
			   int posWhere)
{
	EditRange *er = &(eo->eoAffectedRange);
	...
	if (posWhere >= 0 && posWhere < DOCrtfSEL_TAG_COUNT) {
		...
		LLDEB(posWhere, DOCrtfSEL_TAG_COUNT);
	}
...
static int docTraceStartStepLow(EditOperation *eo, EditTrace *et, int command,
				int fieldKind)
{
	...
	if (et->etIndex > 0) {
		...
		et->etThisStep.tsTraceOffset =
			ts->tsTraceOffset + ts->tsByteCount;
	}

	eo->eoTraceStream = sioOutFdOpenAt(et->etTraceFileHandle,
					   et->etThisStep.tsTraceOffset);
	if (!eo->eoTraceStream) {
		XDEB(eo->eoTraceStream);
		return -1;
	}
	eo->eoTraceWriter =
		docRtfOpenWriter(eo->eoTraceStream, eo->eoDocument, flags);
	if (!eo->eoTraceWriter) {
		XDEB(eo->eoTraceWriter);
		return -1;
	}
...
static int docRtfTraceFieldImpl(EditOperation *eo, const DocumentField *df,
				const char *tag)
{
	RtfWriter *rw = eo->eoTraceWriter;
		       ...
		       EditTrace *et)
{
	...
	if (filename && !utilMemoryBufferIsEmpty(filename)) {
		...
		docRtfWriteDocEncodedStringDestination(
			rw, RTFtag_hlinkbase, fn, strlen(fn), addSemicolon);
	}
