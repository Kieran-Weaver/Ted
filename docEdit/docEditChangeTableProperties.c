...
static int docChangeCellMerge(SetProperties *setProps, BufferItem *cellNode,
				...
				const CellProperties *cpSet)
{
	...
	if (setProps->sp_cpSetMask) {
		...
		if (!utilPropMaskIsEmpty(&cpDoneMask)) {
			...
			docEditIncludeNodeInReformatRange(
				setProps->spEditOperation, rowNode);

			if (setProps->sp_cpDoneMask) {
				utilPropMaskOr(setProps->sp_cpDoneMask,
					       setProps->sp_cpDoneMask,
					       &cpDoneMask);
			}
		}
	}
...
int docEditChangeRowProperties(SetProperties *setProps,
			       const DocumentSelection *ds, BufferItem *rowNode,
