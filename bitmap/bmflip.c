...
static void bmFlip1Init(int n)
{
	...
	for (i = 0; i < 256; i++) {
		unsigned char value = (i & 0x01) << 7 | (i & 0x02) << 5 |
				      (i & 0x04) << 3 | (i & 0x08) << 1 |
				      (i & 0x10) >> 1 | (i & 0x20) >> 3 |
				      (i & 0x40) >> 5 | (i & 0x80) >> 7;
...
static void bmFlip2Init(int n)
{
	...
	for (i = 0; i < 256; i++) {
		unsigned char value = (i & 0x03) << 6 | (i & 0x0c) << 2 |
				      (i & 0x30) >> 2 | (i & 0xc0) >> 6;
...
static void bmFlip4Init(int n)
{
	...
	for (i = 0; i < 256; i++) {
		unsigned char value = (i & 0x0f) << 4 | (i & 0xf0) >> 4;
...
int bmFlipBytes(unsigned char *buffer, int byteCount, int bitsPerPixel)
{
	switch (bitsPerPixel) {
	case 1:
	case 2:
...
static void bmFlipBits(unsigned char *to, const unsigned char *from,
		       int pixelsWide, int pixelsPerByte)
{
	int i;
	int count;
...
static void bmFlip8Bits(unsigned char *to, const unsigned char *from,
			int pixelsWide, int ignored)
{
	int i;
...
static void bmFlip24Bits(unsigned char *to, const unsigned char *from,
			 int pixelsWide, int ignored)
{
	int i;
...
int bmVerticalFlip(RasterImage *riOut, const RasterImage *riIn, int ignoredInt)
{
	int rval = 0;
	...
	for (row = 0; row < bdIn->bdPixelsHigh; row++) {
		...
		to = ri.riBytes +
		     (bdIn->bdPixelsHigh - row - 1) * bdIn->bdBytesPerRow;
...
int bmRotate180(RasterImage *riOut, const RasterImage *riIn, int ignoredInt)
{
	int rval = 0;
	const BitmapDescription *bdIn = &(riIn->riDescription);
	...
	void (*flip)(unsigned char *, const unsigned char *, int, int);
	...
	case 1:
		flip = bmFlipBits;
		bmFlip1Init(bdIn->bdPixelsWide);
		break;
	case 2:
		flip = bmFlipBits;
		bmFlip2Init(bdIn->bdPixelsWide);
		break;
	case 4:
		flip = bmFlipBits;
		bmFlip4Init(bdIn->bdPixelsWide);
		break;
	case 8:
		flip = bmFlip8Bits;
		break;
...
int bmHorizontalFlip(RasterImage *riOut, const RasterImage *riIn,
		     int ignoredInt)
{
	int rval = 0;
	const BitmapDescription *bdIn = &(riIn->riDescription);
...
int bmRotate90(RasterImage *riOut, const RasterImage *riIn, int angle)
{
	int rval = 0;
	const BitmapDescription *bdIn = &(riIn->riDescription);
	...
	case 1:
		...
		if (angle == 90) {
			for (row = 0; row < ri.riDescription.bdPixelsHigh;
			     row++) {
				int toRow =
					ri.riDescription.bdPixelsHigh - row - 1;

				mr = m >> bdIn->bdBitsPerPixel * (row % step);

				for (s = 0; s < row % step; s++) {
					to = ri.riBytes +
					     toRow * ri.riDescription
							     .bdBytesPerRow;
					from = riIn->riBytes + row / step +
					       s * bdIn->bdBytesPerRow;
	...
	case 8:
		if (angle == 90) {
			for (row = 0; row < ri.riDescription.bdPixelsHigh;
				...
				for (col = 0;
				     col < ri.riDescription.bdPixelsWide;
				     col++) {
					*(to++) = *from;
...
typedef void (*ShearRow)(unsigned char *rowBufOut,
			 ...
			 const BitmapDescription *bdOut, int colShift, int fl);

typedef void (*ShearCol)(unsigned char *imgBufOut,
			 const unsigned char *imgBufIn,
			 const BitmapDescription *bdIn,
			 const BitmapDescription *bdOut, int col, int rowShift,
			 int fl);
...
static void bmShearCol8Bit(unsigned char *imgBufOut,
			   const unsigned char *imgBufIn,
			   const BitmapDescription *bdIn,
			   const BitmapDescription *bdOut, int col,
			   int rowShift, int fl)
{
	int fr = 255 - fl;
	int spp = bdIn->bdSamplesPerPixel;
		       ...
		       const RasterImage *riIn, ShearRow shearRow)
{
	...
	if (bdOut->bdPixelsHigh != bdIn->bdPixelsHigh) {
		LLDEB(bdOut->bdPixelsHigh, bdIn->bdPixelsHigh);
		return -1;
	}
		       ...
		       const RasterImage *riIn, ShearCol shearCol)
{
	...
	if (bdOut->bdPixelsWide != bdIn->bdPixelsWide) {
		LLDEB(bdOut->bdPixelsWide, bdIn->bdPixelsWide);
		return -1;
	}
	...
	for (col = 0; col < bdIn->bdPixelsWide; col++) {
		...
		(*shearCol)(bufOut, riIn->riBytes, bdIn, bdOut, col, rowShift,
			    fl);
	}
...
int bmRotate(RasterImage *riOut, const RasterImage *riIn, double theta)
{
	const BitmapDescription *bdIn = &(riIn->riDescription);
	int rval = 0;
	...
	/*********/
	drIn.drX0 = 0;
	drIn.drX1 = bdIn->bdPixelsWide - 1;
	drIn.drY0 = 0;
	drIn.drY1 = bdIn->bdPixelsHigh - 1;

	rectIn[0].x = 0;
	rectIn[0].y = 0;
	rectIn[1].x = drIn.drX1;
	rectIn[1].y = 0;
	rectIn[2].x = drIn.drX1;
	rectIn[2].y = drIn.drY1;
	rectIn[3].x = 0;
	rectIn[3].y = drIn.drY1;
	...
	bmTransformRectangle(&(riC.riDescription), rectC, &drC, &C, rectIn,
			     &drIn);
	...
	if (!riC.riBytes) {
		LLDEB(riC.riDescription.bdBufferLength, riC.riBytes);
		rval = -1;
		goto ready;
	}
	memset(riC.riBytes, 0, riC.riDescription.bdBufferLength);

	if (bmShearRows(riC.riBytes, &(riC.riDescription), shearC, &drC, riIn,
			shearRow)) {
		LDEB(1);
		rval = -1;
		goto ready;
...
#define NO_C 0
	...
	bmTransformRectangle(&(riB.riDescription), rectB, &drB, &B, rectC,
			     &drC);
	...
	if (!riB.riBytes) {
		LLDEB(riB.riDescription.bdBufferLength, riB.riBytes);
		rval = -1;
		goto ready;
	}
	memset(riB.riBytes, 0, riB.riDescription.bdBufferLength);

	if (bmShearCols(riB.riBytes, &(riB.riDescription), shearB, &drB, &riC,
			shearCol)) {
		LDEB(1);
		rval = -1;
		goto ready;
...
#define NO_B 0
	...
	bmTransformRectangle(&(riA.riDescription), rectA, &drA, &A, rectB,
			     &drB);
	...
	if (!riA.riBytes) {
		LLDEB(riA.riDescription.bdBufferLength, riA.riBytes);
		rval = -1;
		goto ready;
	}
	memset(riA.riBytes, 0, riA.riDescription.bdBufferLength);

	if (bmShearRows(riA.riBytes, &(riA.riDescription), shearA, &drA, &riB,
			shearRow)) {
		LDEB(1);
		rval = -1;
		goto ready;
