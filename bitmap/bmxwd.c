...
/************************************************************************/
static int bmReadX11wd(SimpleInputStream *sis, XWDFileHeader *xh,
		       unsigned char **pBuf, BitmapDescription *bd);

/************************************************************************/
/*  Read an XWD file.							*/
/************************************************************************/

int bmReadXwdFile(const MemoryBuffer *filename, unsigned char **pBuf,
		  BitmapDescription *bd, int *pPrivateFormat)
{
	...
	/***************/
	x11Header.header_size = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.file_version = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.pixmap_format = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.pixmap_depth = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.pixmap_width = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.pixmap_height = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.xoffset = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.byte_order = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.bitmap_unit = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.bitmap_bit_order = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.bitmap_pad = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.bits_per_pixel = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.bytes_per_line = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.visual_class = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.red_mask = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.green_mask = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.blue_mask = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.bits_per_rgb = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.colormap_entries = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.ncolors = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.window_width = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.window_height = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.window_x = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.window_y = sioEndianGetBeInt32(sis);
	done += 4;
	x11Header.window_bdrwidth = sioEndianGetBeInt32(sis);
	done += 4;
	/***************/
		       ...
		       unsigned char **pBuf, BitmapDescription *bd)
{
	...
	case PseudoColor:
		...
		if (colors) {
			for (cp = 0; cp < xh->ncolors; cp++) {
				bd->bdPalette.cpColors[cp].rgb8Red =
					colors[cp].red / 256;
				bd->bdPalette.cpColors[cp].rgb8Green =
					colors[cp].green / 256;
				bd->bdPalette.cpColors[cp].rgb8Blue =
					colors[cp].blue / 256;
	...
	case DirectColor:
		...
		if (xh->ncolors > 0 &&
		    (xh->ncolors < rm + 1 || xh->ncolors < gm + 1 ||
		     xh->ncolors < bm + 1)) {
			LLLLDEB(xh->ncolors, rm, gm, bm);
			rval = -1;
			goto ready;
		}
		break;
...
int bmWriteXwdFile(const MemoryBuffer *filename, const unsigned char *buffer,
