	...
	case BMcoRGB:
		...
		if (bd->bdBitsPerSample == 16) {
			...
			if (bd->bdColorEncoding == BMcoRGB && bd->bdHasAlpha &&
				...
				return (pixelsWide * bd->bdBitsPerPixel + 7) /
				       8;
			}
		}
		    ...
		    const unsigned char *buffer)
{
	...
	return bmPsPrintBitmapImage(sos, level, xscale, yscale, ox, oy, drSel,
				    onWhite, useFilters, indexedImages, bd,
				    buffer);
}
...
static void bmPsWriteMonoImageInstructions(SimpleOutputStream *sos,
					   const BitmapDescription *bd,
					   int selWidePix, int selHighPix,
					   const char *source)
{
...
static void bmPsWriteGrayImageInstructions(SimpleOutputStream *sos,
					   const BitmapDescription *bd,
					   int selWidePix, int selHighPix,
					   const char *source)
{
...
static void bmPsWriteRgbImageInstructions(SimpleOutputStream *sos,
					  const BitmapDescription *bd,
					  int selWidePix, int selHighPix,
					  const char *source)
{
...
static void bmPsWriteIndexedImageInstructions(SimpleOutputStream *sos,
					      const BitmapDescription *bd,
					      int selWidePix, int selHighPix,
					      const char *source)
{
...
void bmPsWriteImageInstructions(SimpleOutputStream *sos,
				...
				int selWidePix, int selHighPix,
				const char *source, int indexedImages)
{
	int bitsPerSample = bd->bdBitsPerSample;
	...
	case BMcoBLACKWHITE:
		...
		if (bd->bdBitsPerSample == 1) {
			bmPsWriteMonoImageInstructions(sos, bd, selWidePix,
						       selHighPix, source);
		} else {
			bmPsWriteGrayImageInstructions(sos, bd, selWidePix,
						       selHighPix, source);
		}
	...
	case BMcoRGB:
		...
		bmPsWriteRgbImageInstructions(sos, bd, selWidePix, selHighPix,
					      source);
		break;

	case BMcoRGB8PALETTE:

		if (onWhite && bd->bdBitsPerPixel == 1 &&
		    bd->bdPalette.cpColors[0].rgb8Red == 255 &&
		    bd->bdPalette.cpColors[0].rgb8Green == 255 &&
		    bd->bdPalette.cpColors[0].rgb8Blue == 255) {
			bmPsWriteMonoImageInstructions(sos, bd, selWidePix,
		...
		if (indexedImages) {
			bmPsWriteIndexedImageInstructions(sos, bd, selWidePix,
							  selHighPix, source);
			break;
		}
			 ...
			 const unsigned char *buffer)
{
	...
	if (useFilters) {
#ifdef USE_LZW
		bmPsWriteImageInstructions(
			sos, bd, onWhite, drSel->drX1 - drSel->drX0 + 1,
			drSel->drY1 - drSel->drY0 + 1,
		...
		int bytesPerRow = bmPsRowStringSize(
			bd, drSel->drX1 - drSel->drX0 + 1, indexedImages);
		if (bytesPerRow < 0) {
			LDEB(bytesPerRow);
			return -1;
		}
