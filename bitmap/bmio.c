	    ...
	    const BitmapDescription *bd, int fileFormat)
{
	if (!bmFileFormats[fileFormat].bffFileType->bftWrite) {
		LLDEB(fileFormat,
		      bmFileFormats[fileFormat].bffFileType->bftWrite);
		return -1;
	}

	return (*bmFileFormats[fileFormat].bffFileType->bftWrite)(
		filename, buffer, bd, bmFileFormats[fileFormat].bffPrivate);
}

int bmCanWrite(const BitmapDescription *bd, int fileFormat)
{
	if (!bmFileFormats[fileFormat].bffFileType->bftWrite) {
		return -1;
	}
...
int bmRead(const MemoryBuffer *filename, unsigned char **pBuffer,
	   BitmapDescription *bd, int *pFileFormat)
{
	...
	for (fileType = 0; fileType < bmNumberOfFileTypes; fileType++) {
		...
		if (utilMemoryBufferEqualsString(
			...
			if ((*bmFileTypes[fileType]->bftRead)(
				    filename, &buffer, bd, &privateType)) {
				LDEB(fileType);
				break;
			}

			for (fileFormat = 0; fileFormat < bmNumberOfFileFormats;
			     fileFormat++) {
				if (bmFileFormats[fileFormat]
						    .bffFileType->bftRead ==
			...
			SLDEB(utilMemoryBufferGetString(filename), privateType);
			break;
		}
	}
