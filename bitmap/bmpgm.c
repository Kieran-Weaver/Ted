...
static int bmPbmGetNumber(int *pNum, SimpleInputStream *sis)
{
	int num = 0;
...
int bmReadPbmFile(const MemoryBuffer *filename, unsigned char **pBuf,
...
int bmCanWritePbmFile(const BitmapDescription *bd, int privateFormat)
{
	...
	if (privateFormat == 1 && bd->bdColorEncoding == BMcoWHITEBLACK &&
	    bd->bdBitsPerPixel == 1) {
		return 0;
	}
	if (privateFormat == 1 && bd->bdColorEncoding == BMcoBLACKWHITE &&
	    bd->bdBitsPerPixel == 1) {
		return 0;
	}

	if (privateFormat == 2 && bd->bdColorEncoding == BMcoWHITEBLACK &&
	    bd->bdBitsPerPixel == 1) {
		return 0;
	}
	if (privateFormat == 2 && bd->bdColorEncoding == BMcoBLACKWHITE &&
	    bd->bdBitsPerPixel == 1) {
		return 0;
	}

	if (privateFormat == 4 && bd->bdColorEncoding == BMcoBLACKWHITE &&
	    bd->bdBitsPerPixel == 1) {
		return 0;
	}

	if (privateFormat == 5 && bd->bdColorEncoding == BMcoWHITEBLACK &&
	    bd->bdBitsPerPixel == 8) {
		return 0;
	}
...
static void bmWriteBitmapBitsAscii(SimpleOutputStream *sos,
				   const unsigned char *buffer,
				   const BitmapDescription *bd, const char *b1,
				   const char *b0)
{
	int row;
...
static void bmStartPbm(SimpleOutputStream *sos, int privateFormat,
...
static void bmStartPnm(SimpleOutputStream *sos, int privateFormat,
		       const BitmapDescription *bd)
{
...
static void bmPbmRuler(SimpleOutputStream *sos, const BitmapDescription *bd)
{
	int col;
...
int bmWritePbmFile(const MemoryBuffer *filename, const unsigned char *buffer,
	...
	case 1:
		...
		case BMcoWHITEBLACK:
			...
			case 1:
				...
				bmWriteBitmapBitsAscii(sos, buffer, bd, " 0",
						       " 1");
				break;
		...
		case BMcoBLACKWHITE:
			...
			case 1:
				...
				bmWriteBitmapBitsAscii(sos, buffer, bd, " 1",
						       " 0");
				break;
