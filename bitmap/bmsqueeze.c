...
static void bmSqueeze_8_4(const unsigned char *fr, unsigned char *to, int wide)
{
	int col;
	unsigned char s[8];

	for (col = 0; col + 1 < wide; fr += 2, col += 2, to++) {
		*to = ((fr[0] & 0x0f) << 4) | ((fr[1] & 0x0f));
	}

	if (col < wide) {
		memset(s, 0, 8);
		memcpy(s, fr, wide - col);
		fr = s;
...
static void bmSqueeze_8_2(const unsigned char *fr, unsigned char *to, int wide)
{
	int col;
	unsigned char s[8];

	for (col = 0; col + 3 < wide; fr += 4, col += 4, to++) {
		*to = ((fr[0] & 0x03) << 6) | ((fr[1] & 0x03) << 4) |
		      ((fr[2] & 0x03) << 2) | ((fr[3] & 0x03));
	}
...
static void bmSqueeze_8_1(const unsigned char *fr, unsigned char *to, int wide)
{
	int col;
	unsigned char s[8];

	for (col = 0; col + 7 < wide; fr += 8, col += 8, to++) {
		*to = ((fr[0] & 0x01) << 7) | ((fr[1] & 0x01) << 6) |
		      ((fr[2] & 0x01) << 5) | ((fr[3] & 0x01) << 4) |
		      ((fr[4] & 0x01) << 3) | ((fr[5] & 0x01) << 2) |
		      ((fr[6] & 0x01) << 1) | ((fr[7] & 0x01));
	}
...
static void bmSqueeze_4_2(const unsigned char *fr, unsigned char *to, int wide)
{
	int col;
	unsigned char s[8];

	for (col = 0; col + 3 < wide; fr += 2, col += 4, to++) {
		*to = ((fr[0] & 0x30) << 2) | ((fr[0] & 0x03) << 6) |
		      ((fr[1] & 0x30) << 2) | ((fr[1] & 0x03));
	}
...
static void bmSqueeze_4_1(const unsigned char *fr, unsigned char *to, int wide)
{
	int col;
	unsigned char s[8];

	for (col = 0; col + 7 < wide; fr += 4, col += 8, to++) {
		*to = ((fr[0] & 0x10) << 3) | ((fr[0] & 0x01) << 6) |
		      ((fr[1] & 0x10) << 1) | ((fr[1] & 0x01) << 4) |
		      ((fr[2] & 0x10) >> 1) | ((fr[2] & 0x01) << 2) |
		      ((fr[3] & 0x10) >> 3) | ((fr[3] & 0x01));
	}
...
static void bmSqueeze_2_1(const unsigned char *fr, unsigned char *to, int wide)
{
	int col;
	unsigned char s[8];

	for (col = 0; col + 7 < wide; fr += 2, col += 8, to++) {
		*to = ((fr[0] & 0x40) << 1) | ((fr[0] & 0x10) << 2) |
		      ((fr[0] & 0x04) << 3) | ((fr[0] & 0x01) << 4) |
		      ((fr[1] & 0x40) << 3) | ((fr[1] & 0x10) << 2) |
		      ((fr[1] & 0x04) << 1) | ((fr[1] & 0x01));
	}
...
static int bmSqueezeBuffer(const BitmapDescription *bd, int bitsPerPixel,
			   int bytesPerRow, unsigned char *buffer)
{
	int row;
	...
	case 8:
		...
		case 1:
			...
			return 0;
		default:
			LLDEB(bd->bdBitsPerPixel, bitsPerPixel);
			return -1;
		}
	case 4:
		switch (bitsPerPixel) {
...
int bmMakeMonochrome(BitmapDescription *bd, unsigned char *buffer)
{
	...
	if (cp->cpColorCount == 2) {
		...
		if (cp->cpColors[0].rgb8Red == 0 &&
			...
			if (bmSqueezeBuffer(bd, bitsPerPixel, bytesPerRow,
					    buffer)) {
				LDEB(1);
				return -1;
			}
