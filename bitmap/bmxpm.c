...
static int bmXpmPaletteColor(const char *c_color, int *pTransparent,
			     RGB8Color *rgb8)
{
	char ch;
	unsigned r, g, b;
	...
	/*  1  */
	if (!c_color || !strcmp(c_color, "None") || !strcmp(c_color, "none") ||
	...
	if (f) {
		...
		while (fgets(line, 100, f)) {
			if (sscanf(line, "%u %u %u %s", &r, &g, &b, color) ==
			    4) {
				const char *s1 = color;
		  ...
		  BitmapDescription *bd, int *pPrivateFormat)
{
	...
	if (ret != XpmSuccess) {
		SSDEB(fn, XpmGetErrorString(ret));
		rval = -1;
		goto ready;
	}
	...
	case 16:
		...
		bd->bdBytesPerRow =
			(bd->bdBitsPerPixel + 7) / 8 * bd->bdPixelsWide;
	...
	default:
		LLDEB(bd->bdBitsPerPixel, image.cpp);
		rval = -1;
		goto ready;
	}
...
static int bmXpmAllocColors(XpmImage *xpmi, int ncolors)
{
	int col;

	xpmi->colorTable = (XpmColor *)malloc(xpmi->ncolors * sizeof(XpmColor));
	if (!xpmi->colorTable) {
		LXDEB(ncolors, xpmi->colorTable);
		return -1;
	}
...
static int bmXpmSetColor(XpmColor *xpmc, int charactersPerPixel, char *bytes,
			 const RGB8Color *rgb8)
{
	if (rgb8->rgb8Alpha >= 128) {
		...
		sprintf(xpmc->c_color, "#%04x%04x%04x", 257 * rgb8->rgb8Red,
			257 * rgb8->rgb8Green, 257 * rgb8->rgb8Blue);
	} else {
		xpmc->c_color = (char *)0;
...
static int bmXpmFillPalette(XpmImage *image, const BitmapDescription *bd,
			    const RGB8Color *palette)
{
	...
	for (col = 0; col < image->ncolors; col++) {
		...
		if (bmXpmSetColor(image->colorTable + col, charactersPerPixel,
				  scratch, palette + col)) {
			XDEB(image->colorTable[col].c_color);
			return -1;
		}
	}
...
int bmWriteXpmFile(const MemoryBuffer *filename, const unsigned char *buffer,
		   const BitmapDescription *bd, int privateFormat)
{
	...
	{
		int size =
			(image.width * image.height + 7) * sizeof(unsigned int);
		image.data = (unsigned int *)malloc(size);
		if (!image.data) {
			LXDEB(size, image.data);
			rval = -1;
			goto ready;
		}
	}
	...
	for (row = 0; row < bd->bdPixelsHigh; row++) {
		...
		if (bmInflateToInt(to, from, bd, transparentColor,
				   bd->bdHasAlpha)) {
			LDEB(bd->bdHasAlpha);
			rval = -1;
			goto ready;
	...
	{
		int ret = XpmWriteFileFromXpmImage((char *)fn, &image,
						   (XpmInfo *)0);
		if (ret != XpmSuccess) {
			SSDEB(fn, XpmGetErrorString(ret));
			rval = -1;
