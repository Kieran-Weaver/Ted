...
static int bmSetAlphaBytes(unsigned char *b1, unsigned char *b2,
			   int bitsPerPixel, unsigned int t)
{
	unsigned int i;
	unsigned int m;
	...
	case 2:
		...
		for (i = 0; i < 256; i++) {
			...
			b1[i] |= (i & 0xc0) | (i & 0x30) >> 2;

			b1[i] |= (m * (((i & 0xc0) >> 6) != t)) << 4 |
				 (m * (((i & 0x60) >> 4) != t)) << 0;

			b2[i] |= (i & 0x0c) << 4 | (i & 0x03) << 2;
...
int bmWhiteToTransparent(RasterImage *riOut, const RasterImage *riIn,
			 int ignoredInt)
{
	int rval = 0;
	...
	case BMcoRGB8PALETTE:
		...
		for (row = 0; row < ri.riDescription.bdPalette.cpColorCount;
		     row++) {
			if (ri.riDescription.bdPalette.cpColors[row].rgb8Red ==
				    255 &&
			    ri.riDescription.bdPalette.cpColors[row].rgb8Green ==
				    255 &&
			    ri.riDescription.bdPalette.cpColors[row].rgb8Blue ==
				    255) {
				ri.riDescription.bdPalette.cpColors[row]
					.rgb8Alpha = 0;
				break;
			}
		}
	...
	case BMcoRGB8PALETTE:
		...
		case 8:
			for (row = 0; row < bdIn->bdPixelsHigh; row++) {
				from = riIn->riBytes +
				       row * bdIn->bdBytesPerRow;
				to = ri.riBytes +
				     row * ri.riDescription.bdBytesPerRow;
...
static int bmGetRowAlpha(unsigned char *to, const unsigned char *from,
			 int pixelsWide, int bpp, int spp)
{
...
int bmGetAlphaMask(RasterImage *riOut, const RasterImage *riIn, int ignoredInt)
{
	const BitmapDescription *bdIn = &(riIn->riDescription);
	int rval = 0;
	int row;
...
static int bmRemoveRowAlpha(unsigned char *to, const unsigned char *from,
			    int pixelsWide, int bpp, int spp, int empty)
{
	int col;
	int step;
...
int bmRemoveAlpha(RasterImage *riOut, const RasterImage *riIn, int ignoredInt)
{
	const BitmapDescription *bdIn = &(riIn->riDescription);
	int rval = 0;
	int row;
	...
	case BMcoRGB8PALETTE:
		...
		for (row = 0; row < ri.riDescription.bdPalette.cpColorCount;
		     row++) {
			if (ri.riDescription.bdPalette.cpColors[row].rgb8Alpha ==
			    0) {
				ri.riDescription.bdPalette.cpColors[row]
					.rgb8Red = 255;
				ri.riDescription.bdPalette.cpColors[row]
...
int bmSetAlphaMask(BitmapDescription *bdOut, const BitmapDescription *bdImage,
		   ...
		   const unsigned char *bufImage, const unsigned char *bufAlpha)
{
	...
	if (bdImage->bdHasAlpha) {
		LDEB(bdImage->bdHasAlpha);
		rval = -1;
		goto ready;
	}
	if (bdAlpha->bdHasAlpha) {
		LDEB(bdAlpha->bdHasAlpha);
		rval = -1;
		goto ready;
	}
	if (bdAlpha->bdColorEncoding != BMcoBLACKWHITE) {
		LDEB(bdAlpha->bdColorEncoding);
		rval = -1;
		goto ready;
	}
	if (bdAlpha->bdBitsPerPixel != 1) {
		LDEB(bdAlpha->bdBitsPerPixel);
		rval = -1;
		goto ready;
	}
	...
	if (!bufOut) {
		LXDEB(bd.bdBufferLength, bufOut);
		rval = -1;
		goto ready;
	}
