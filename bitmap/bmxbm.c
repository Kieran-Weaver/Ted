...
static int XReadBitmapFileData(const char *filename,
...
int bmWriteXbmFile(const MemoryBuffer *filename, const unsigned char *buffer,
		  ...
		  BitmapDescription *bd, int *pPrivateFormat)
{
	...
	if (XReadBitmapFileData(utilMemoryBufferGetString(filename), &width,
				&height, pBuffer, &x_hot, &y_hot)) {
		LDEB(1);
		return -1;
	}
