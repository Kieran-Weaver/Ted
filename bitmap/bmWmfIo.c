...
int bmMetaReadWmfHeader(WmfHeader *wh, SimpleInputStream *sis)
{
...
int bmMetaReadWmfFileHeader(unsigned long *pKey, WmfFileHeader *wfh,
...
int bmWmfReadRectangle(DocumentRectangle *dr, SimpleInputStream *sis)
{
	int done = 0;

	dr->drY1 = sioEndianGetLeInt16(sis);
	done += 2;
	dr->drX1 = sioEndianGetLeInt16(sis);
	done += 2;
	dr->drY0 = sioEndianGetLeInt16(sis);
	done += 2;
	dr->drX0 = sioEndianGetLeInt16(sis);
	done += 2;

	return done;
}

int bmWmfReadHWYXRectangle(DocumentRectangle *dr, SimpleInputStream *sis)
{
	int done = 0;
	int h;
	int w;
...
int bmWmfGetColor(SimpleInputStream *sis, RGB8Color *rgb8)
{
	int done = 0;
