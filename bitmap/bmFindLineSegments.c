...
static int bmCollectLineSegment(void *voidflsj, int row, int col)

{
	...
	if (flsj->flsjSegmentCount >= flsj->flsjSegmentsAllocated) {
		...
		fresh = (LineSegment2DI *)realloc(flsj->flsjSegments,
...
static int bmCompareLineSegments(const void *vls1, const void *vls2)
{
...
static int bmStartFindLineSegmentsJob(FindLineSegmentsJob *flsj, void *through,
				      BM_SEG_FUN segFun, int size, int x0,
				      int y0, int x1, int y1)
{
	geoLineConstants(&(flsj->flsjA), &(flsj->flsjB), &(flsj->flsjC), x0, y0,
			 x1, y1);
	geoLineAffineTransform2D(&(flsj->flsjTransform), flsj->flsjA,
				 flsj->flsjB, flsj->flsjC);

	if (bmMorphoLineElement(&(flsj->flsjRiSel), size, size, x0, y0, x1,
				y1)) {
		LDEB(size);
		return -1;
	}
...
static void bmChainLineSegments(FindLineSegmentsJob *flsj)
{
	...
	for (to = 0; to < flsj->flsjSegmentCount; lsTo++, to++) {
		...
		if (c1 < c0 + 0.05) {
			...
			c = lsTo->ls2diX0;
			lsTo->ls2diX0 = lsTo->ls2diX1;
			lsTo->ls2diX1 = c;
			c = lsTo->ls2diY0;
			lsTo->ls2diY0 = lsTo->ls2diY1;
			lsTo->ls2diY1 = c;
			continue;
		}
	...
	qsort(flsj->flsjSegments, flsj->flsjSegmentCount,
	      sizeof(LineSegment2DI), bmCompareLineSegments);
...
static int bmRunFindLineSegmentsJob(FindLineSegmentsJob *flsj,
				    const RasterImage *riIn, void *through,
				    BM_SEG_FUN segFun, int size, int x0, int y0,
				    int x1, int y1)
{
	int fr;
	const LineSegment2DI *lsFr;

	if (bmStartFindLineSegmentsJob(flsj, through, segFun, size, x0, y0, x1,
				       y1)) {
		LDEB(size);
		return -1;
	}
	...
	if (bmMorphoErode(&(flsj->flsjRiSegs), riIn, &(flsj->flsjRiSel),
			  size / 2, size / 2)) {
		LDEB(1);
		return -1;
	}
	...
	if (flsj->flsjRiSegs.riDescription.bdColorEncoding == BMcoWHITEBLACK) {
		bmForAll1Pixels(&(flsj->flsjRiSegs), 0xff, (void *)flsj,
				bmCollectLineSegment);
	} else {
		bmForAll1Pixels(&(flsj->flsjRiSegs), 0x00, (void *)flsj,
				bmCollectLineSegment);
	}
...
*/
		...
		(*flsj->flsjSegFun)(flsj->flsjThrough, lsFr->ls2diX0,
				    lsFr->ls2diY0, lsFr->ls2diX1,
				    lsFr->ls2diY1);
	}
...
int bmFindLineSegments(const RasterImage *riIn, int size, void *through,
		       BM_SEG_FUN segFun)
{
	...
	/*  2a  */
	for (i = 0; i < size; i++) {
		if (bmRunFindLineSegmentsJob(&flsj, riIn, through, segFun, size,
					     i, 0, size - i - 1, size - 1)) {
			LDEB(i);
			rval = -1;
			goto ready;
