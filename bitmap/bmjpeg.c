...
static boolean bmJpegFillInputBuffer(j_decompress_ptr cinfo)
{
	...
	nbytes = sioInReadBytes(bjis->bjisSis, bjis->bjisReadBuffer,
				INPUT_BUF_SIZE);
...
static void bmJpegSkipInputData(j_decompress_ptr cinfo, long num_bytes)
{
	BmJpegInputSource *bjis = (BmJpegInputSource *)cinfo->src;
...
pixel_density:
	...
	case 1:
		if (cinfo->X_density < 5 || cinfo->Y_density < 5) {
			LLLDEB(cinfo->density_unit, cinfo->X_density,
			       cinfo->Y_density);
			goto pixel_density;
		}
	...
	case JCS_RGB:
		if (cinfo->jpeg_color_space == JCS_GRAYSCALE) {
			...
			bjis->bjisBd.bdColorEncoding = BMcoWHITEBLACK;
			bjis->bjisBd.bdSamplesPerPixel = 1;
			bjis->bjisBd.bdBitsPerPixel =
				bjis->bjisBd.bdBitsPerSample;
		} else {
			bjis->bjisBd.bdColorEncoding = BMcoRGB;
			bjis->bjisBd.bdSamplesPerPixel = 3;
			bjis->bjisBd.bdBitsPerPixel =
				3 * bjis->bjisBd.bdBitsPerSample;
		}
		break;

	case JCS_UNKNOWN:
		LLDEB(JCS_UNKNOWN, cinfo->out_color_space);
		return -1;
	case JCS_YCbCr:
		LLDEB(JCS_YCbCr, cinfo->out_color_space);
		return -1;
	case JCS_YCCK:
		LLDEB(JCS_YCCK, cinfo->out_color_space);
		return -1;
	case JCS_CMYK:
	...
	bjis->bjisBd.bdBytesPerRow =
		(bjis->bjisBd.bdPixelsWide * bjis->bjisBd.bdBitsPerPixel + 7) /
		8;
	bjis->bjisBd.bdBufferLength =
		bjis->bjisBd.bdPixelsHigh * bjis->bjisBd.bdBytesPerRow;

	bjis->bjisBitmapBuffer =
		(unsigned char *)malloc(bjis->bjisBd.bdBufferLength);
...
static int read_JPEG_file(BmJpegInputSource *bjis)
{
	...
	bjis->bjisReadBuffer = (JOCTET *)(*cinfo.mem->alloc_small)(
...
int bmCanWriteJpegFile(const BitmapDescription *bd, int privateFormat)
{
	if (bd->bdBitsPerSample != 8) { /* LDEB(bd->bdBitsPerSample); */
		return -1;
	}
...
static void bmJpegInitDestination(j_compress_ptr cinfo)
{
	...
	bjod->bjodWriteBuffer = (JOCTET *)(*cinfo->mem->alloc_small)(
...
int bmJpegWriteJfif(const BitmapDescription *bd, const unsigned char *buffer,
		    SimpleOutputStream *sos)
{
	...
	case BMcoBLACKWHITE:
		bjod.bjodScratchBuffer =
			(unsigned char *)malloc(bd->bdBytesPerRow);
...
int bmWriteJpegFile(const MemoryBuffer *filename, const unsigned char *buffer,
