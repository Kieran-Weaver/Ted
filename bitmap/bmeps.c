...
static int bmWriteEps(SimpleOutputStream *sos, const char *filename,
		      ...
		      int privateFormat)
{
	...
	if (bmPsPrintBitmap(sos, privateFormat, 1.0, 1.0, 0, 0,
			    (const DocumentRectangle *)0, useFilters,
			    indexedImages, bd, buffer)) {
		LDEB(1);
		return -1;
	}
...
int bmWriteEpsFile(const MemoryBuffer *filename, const unsigned char *buffer,
		   const BitmapDescription *bd, int privateFormat)
{
	...
	if (bmWriteEps(sos, utilMemoryBufferGetString(filename), buffer, bd,
		       privateFormat)) {
		LDEB(1);
		rval = -1;
		goto ready;
		    ...
		    const BitmapDescription *bd)
{
	...
	/*  4  */
	psStartEpsFile(sos, "Scan, Mark de Does, mark@mdedoes.com", filename,
...
int bmWritePdfFile(const MemoryBuffer *filename, const unsigned char *buffer,
		   const BitmapDescription *bd, int privateFormat)
{
	int rval = 0;
	...
	if (strchr((char *)filename->mbBytes, '\'') || filename->mbSize < 1 ||
	...
	utilMemoryBufferPrintf(
		...
		(rectangleWideTwips + 19) / 20, (rectangleHighTwips + 19) / 20,
