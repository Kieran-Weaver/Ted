...
static void bmAverageMono8Row(const BitmapDescription *bdOut,
			      const BitmapDescription *bdIn, unsigned char *to,
			      const unsigned char *rf, int xpixels, int ypixels)
{
	int col;
	...
	for (col = 0; col < bdIn->bdPixelsWide / xpixels;
	     rf += xpixels, col++) {
		int v = 0;
...
static void bmAverageMono8Image(const BitmapDescription *bdOut,
				...
				const unsigned char *bufIn, int xpixels,
				int ypixels)
{
	int row;
	...
	for (row = 0; row < bdIn->bdPixelsHigh / ypixels; row++) {
		const unsigned char *rf =
			bufIn + ypixels * row * bdIn->bdBytesPerRow;
...
int bmAverage(RasterImage *riOut, const RasterImage *riIn, int xpixels,
	      int ypixels)
{
	...
	ri.riDescription.bdPixelsWide =
		(bdIn->bdPixelsWide + xpixels - 1) / xpixels;
	ri.riDescription.bdPixelsHigh =
		(bdIn->bdPixelsHigh + ypixels - 1) / ypixels;
	...
	case BMcoBLACKWHITE:
	case BMcoWHITEBLACK:
		bmAverageMono8Image(&(ri.riDescription), bdIn, ri.riBytes,
				    riIn->riBytes, xpixels, ypixels);
		break;

	case BMcoRGB:
		bmAverageRgb24Image(&(ri.riDescription), bdIn, ri.riBytes,
				    riIn->riBytes, xpixels, ypixels);
		break;
