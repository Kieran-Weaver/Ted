	     ...
	     const DocumentRectangle *drSel)
{
	...
	if (drSel->drX0 < 0 || drSel->drY0 < 0 ||
	    drSel->drX1 >= bdIn->bdPixelsWide ||
	    drSel->drY1 >= bdIn->bdPixelsHigh || drSel->drX0 >= drSel->drX1 ||
	...
	if (originBit == 0) {
		...
		for (row = 0; row < ri.riDescription.bdPixelsHigh; row++) {
			...
			from = riIn->riBytes +
			       (row + drSel->drY0) * bdIn->bdBytesPerRow +
			       originByte;

			*to = *from << originBit;
			from++;

			for (byte = 1; byte < ri.riDescription.bdBytesPerRow;
			     byte++) {
				*to |= *from >> rightShift;
				to++;
				*to = *from << originBit;
...
int bmCopyArea(int x0, int y0, RasterImage *riTo, const RasterImage *riFrom)
{
	int rval = 0;
	int *colorMap = (int *)0;
	...
	if (bdTo->bdColorEncoding == BMcoRGB8PALETTE) {
		...
		if (utilMergeColorPalettes(&colorMap, &(bdTo->bdPalette),
					   &(bdFrom->bdPalette), avoidZero,
					   maxColors)) {
			LDEB(maxColors);
			rval = -1;
			goto ready;
...
static int bmPaintArea8(int x0, int x1, int y0, int y1, unsigned char *buffer1,
			...
			const BitmapDescription *bdFrom)
{
	...
	for (yt = y0; yt < y1; yf++, yt++) {
		int x;
		unsigned char *to =
			buffer1 + yt * bdTo->bdBytesPerRow + x0 * spp1;
...
int bmPaintArea(int x0, int y0, RasterImage *riTo, const RasterImage *riFrom)
{
	int x1;
	int y1;
	...
	if (bdTo->bdSamplesPerPixel + !bdTo->bdHasAlpha !=
	    bdFrom->bdSamplesPerPixel) {
		LLLDEB(bdTo->bdSamplesPerPixel, bdTo->bdHasAlpha,
		       bdFrom->bdSamplesPerPixel);
		return -1;
	}
	...
	case 8:
		return bmPaintArea8(x0, x1, y0, y1, riTo->riBytes,
				    riFrom->riBytes, bdTo, bdFrom);

	default:
		LDEB(bdTo->bdBitsPerSample);
		return -1;
	}
}
