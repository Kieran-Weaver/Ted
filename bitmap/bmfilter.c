...
typedef void (*BM_FILTER_OP)(ColorValue *cvTo, const FillJob *fj);
...
static const int *const BM_laplace_[3] = { BM_laplace_P + 1, BM_laplace_T + 1,
					   BM_laplace_N + 1 };
...
static const int *const BM_sobelx_[3] = { BM_sobelx_P + 1, BM_sobelx_T + 1,
					  BM_sobelx_N + 1 };
...
static const int *const BM_sobely_[3] = { BM_sobely_P + 1, BM_sobely_T + 1,
					  BM_sobely_N + 1 };
...
static const int *const BM_smoothe_[3] = { BM_smoothe_P + 1, BM_smoothe_T + 1,
					   BM_smoothe_N + 1 };
...
static void bmFilterRow3x3(ColorValue *cvTo, const FillJob *fj,
			   const int *const *const f, int absoluteValues,
			   int maxValue)
{
	int i;
	...
	for (i = 0; i < fj->fjFrWide; cvTo++, p++, t++, n++, i++) {
		...
		/* R */
		val = 0;
		val += f[-1][-1] * p[-1].cvR + f[-1][0] * p[0].cvR +
		       f[-1][1] * p[1].cvR;
		val += f[0][-1] * t[-1].cvR + f[0][0] * t[0].cvR +
		       f[0][1] * t[1].cvR;
		val += f[1][-1] * n[-1].cvR + f[1][0] * n[0].cvR +
		       f[1][1] * n[1].cvR;
		...
		/* G */
		val = 0;
		val += f[-1][-1] * p[-1].cvG + f[-1][0] * p[0].cvG +
		       f[-1][1] * p[1].cvG;
		val += f[0][-1] * t[-1].cvG + f[0][0] * t[0].cvG +
		       f[0][1] * t[1].cvG;
		val += f[1][-1] * n[-1].cvG + f[1][0] * n[0].cvG +
		       f[1][1] * n[1].cvG;
		...
		/* B */
		val = 0;
		val += f[-1][-1] * p[-1].cvB + f[-1][0] * p[0].cvB +
		       f[-1][1] * p[1].cvB;
		val += f[0][-1] * t[-1].cvB + f[0][0] * t[0].cvB +
		       f[0][1] * t[1].cvB;
		val += f[1][-1] * n[-1].cvB + f[1][0] * n[0].cvB +
		       f[1][1] * n[1].cvB;
...
static void bmSobel(ColorValue *cvTo, const FillJob *fj)
{
...
static void bmLaplace(ColorValue *cvTo, const FillJob *fj)
{
	const int absoluteValues = 1;
...
static void bmSmoothe(ColorValue *cvTo, const FillJob *fj)
{
...
static void bmBlur(ColorValue *cvTo, const FillJob *fj)
{
	int i;
	int r, g, b;
...
static int bmFilter3Row(unsigned char *bufOut, ColorValue *cvOut,
			const BitmapDescription *bdOut, const RasterImage *riIn,
			FillJob *fj, int flip, GetSourceRow getRow,
			PutScreenRow putRow, BM_FILTER_OP how)
{
	...
	fj->fjPrevRow[bdOut->bdPixelsWide] =
		fj->fjPrevRow[bdOut->bdPixelsWide - 1];
	fj->fjThisRow[0] = fj->fjThisRow[1];
	fj->fjThisRow[bdOut->bdPixelsWide] =
		fj->fjThisRow[bdOut->bdPixelsWide - 1];
	...
	for (rowTo = 0; rowTo < bdOut->bdPixelsHigh - 1; rowTo++) {
		...
		swap = fj->fjPrevRow;
		fj->fjPrevRow = fj->fjThisRow;
		fj->fjThisRow = fj->fjNextRow;
		fj->fjNextRow = swap;

		bmInitColorRow(fj->fjNextRow + 1, fj->fjFrWide);
		(*getRow)(fj->fjNextRow + 1, 0, from, 0, bdIn->bdPixelsWide,
			  bdIn);
...
static int bmStartFilterOperation(RasterImage *riOut, ColorValue **pCvOut,
				  ColorAllocator *ca, PutScreenRow *pPutRow,
				  GetSourceRow *pGetRow, FillJob *fj,
				  const BitmapDescription *bdIn)
{
	...
	/*  4  */
	*pCvOut = (ColorValue *)malloc((riOut->riDescription.bdPixelsWide + 2) *
				       sizeof(ColorValue));
	if (!*pCvOut) {
		LXDEB(riOut->riDescription.bdPixelsWide, *pCvOut);
		return -1;
	}
	...
	if (bmGetPutRow(pPutRow, &scratchSize, ca, swapBitmapUnit,
			swapBitmapBytes, swapBitmapBits,
			&(riOut->riDescription))) {
		LDEB(1);
		return -1;
	}
	...
	if (bmSetFillJob(fj, ca, bdIn->bdPixelsWide,
			 riOut->riDescription.bdPixelsWide, scratchSize,
			 dither)) {
		LDEB(scratchSize);
		return -1;
	}
...
static int bmFilterOperation(RasterImage *riOut, const RasterImage *riIn,
			     int flip, BM_FILTER_OP how)
{
	const BitmapDescription *bdIn = &(riIn->riDescription);
	int rval = 0;
	...
	if (bmStartFilterOperation(&ri, &cvOut, &ca, &putRow, &getRow, &fj,
				   bdIn)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (bmFilter3Row(ri.riBytes, cvOut, &(ri.riDescription), riIn, &fj,
			 flip, getRow, putRow, how)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
...
int bmFilterSobel(RasterImage *riOut, const RasterImage *riIn, int ignoredInt)
{
	const int flip = 0;
...
int bmFilterLaplace(RasterImage *riOut, const RasterImage *riIn, int ignoredInt)
{
	const int flip = 0;
...
int bmFilterSmoothe(RasterImage *riOut, const RasterImage *riIn, int ignoredInt)
{
	const int flip = 0;
...
static int bmStartBlurOperation(RasterImage *riDown, RasterImage *riUp,
				ColorValue **pCvDown, ColorValue **pCvUp,
				ColorAllocator *ca, PutScreenRow *pPutRowBlur,
				GetSourceRow *pGetRowIn,
				GetSourceRow *pGetRowBlur, FillJob *fj,
				const BitmapDescription *bdIn)
{
	if (bmStartFilterOperation(riDown, pCvDown, ca, pPutRowBlur, pGetRowIn,
				   fj, bdIn)) {
		LDEB(1);
		return -1;
	}
	...
	*pCvUp = (ColorValue *)malloc((riUp->riDescription.bdPixelsWide + 2) *
				      sizeof(ColorValue));
	if (!*pCvUp) {
		LXDEB(riUp->riDescription.bdPixelsWide, *pCvUp);
		return -1;
	}
...
static int bmBlurAverage(FillJob *fj, const BitmapDescription *bdBlur,
			 ColorValue *cvDown, ColorValue *cvUp,
			 unsigned char *bufDown, const unsigned char *bufUp,
			 GetSourceRow getRow, PutScreenRow putRow)
{
	int row;
	...
	for (row = 0; row < bdBlur->bdPixelsHigh; row++) {
		...
		from = bufUp +
		       (bdBlur->bdPixelsHigh - row - 1) * bdBlur->bdBytesPerRow;
...
int bmFilterBlur(RasterImage *riOut, const RasterImage *riIn, int ignoredInt)
{
	int rval = 0;
	const BitmapDescription *bdIn = &(riIn->riDescription);
	...
	if (bmStartBlurOperation(&riDown, &riUp, &cvDown, &cvUp, &ca, &putRow,
				 &getRowIn, &getRowBlur, &fj, bdIn)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
	...
	if (bmFilter3Row(riUp.riBytes, cvUp, &(riUp.riDescription), riIn, &fj,
			 1, getRowIn, putRow, bmBlur)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}

	if (bmBlurAverage(&fj, &(riDown.riDescription), cvDown, cvUp,
			  riDown.riBytes, riUp.riBytes, getRowBlur, putRow)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
...
static int bmBlurRelative(FillJob *fj, const RasterImage *riIn,
			  const BitmapDescription *bdBlur, ColorValue *cvDown,
			  ColorValue *cvUp, unsigned char *bufDown,
			  const unsigned char *bufUp, GetSourceRow getRowIn,
			  GetSourceRow getRowBlur, PutScreenRow putRowBlur)
{
	const BitmapDescription *bdIn = &(riIn->riDescription);
	int row;
	...
	for (row = 0; row < bdBlur->bdPixelsHigh; row++) {
		...
		for (col = 0; col < bdBlur->bdPixelsWide;
		     cvd++, cvu++, cvi++, col++) {
			int blr;
			int ref;
...
int bmFilterRelative(RasterImage *riOut, const RasterImage *riIn,
		     int ignoredInt)
{
	int rval = 0;
	const BitmapDescription *bdIn = &(riIn->riDescription);
	...
	if (bmBlurRelative(&fj, riIn, &(riDown.riDescription), cvDown, cvUp,
			   riDown.riBytes, riUp.riBytes, getRowIn, getRowBlur,
			   putRow)) {
		LDEB(1);
		rval = -1;
		goto ready;
