			...
			PutScreenRow putRow)
{
	...
	if (!drSel) {
		...
		if (drSel->drX0 != drAll.drX0 || drSel->drY0 != drAll.drY0 ||
		    drSel->drX1 != drAll.drX1 || drSel->drY1 != drAll.drY1) {
	...
	if (toHigh <= frHigh) {
		...
		for (rowOut = 0; rowOut < toHigh; rowOut++) {
			...
			while (e >= 0) {
				from = riIn->riBytes +
				       rowIn * riIn->riDescription.bdBytesPerRow;
		...
		const DocumentRectangle *drSel)
{
	...
	if (bmSetFillJob(&fj, ca, pixelsWideIn, bdOut->bdPixelsWide,
			 scratchSize, dither)) {
		LDEB(scratchSize);
		rval = -1;
		goto ready;
...
int bmSetColorAllocatorForImage(ColorAllocator *ca, const BitmapDescription *bd)
{
