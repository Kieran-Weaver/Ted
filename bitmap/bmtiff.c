		   ...
		   BitmapDescription *bd, int *pPrivateFormat)
{
	...
	if (TIFFGetField(t, TIFFTAG_SAMPLESPERPIXEL, &unsignedShort) != 1) {
		LDEB(TIFFTAG_SAMPLESPERPIXEL);
		rval = -1;
		goto ready;
	}
	bd->bdSamplesPerPixel = unsignedShort;

	if (TIFFGetField(t, TIFFTAG_EXTRASAMPLES, &unsignedShort, &whatKind) !=
	    1) {
		bd->bdHasAlpha = 0;
	} else {
		...
		case 1:
			if (whatKind[0] != EXTRASAMPLE_ASSOCALPHA &&
				...
				LLDEB(TIFFTAG_EXTRASAMPLES, whatKind[0]);
				return -1;
			}
			bd->bdHasAlpha = 1;
			break;
	...
	case PHOTOMETRIC_MINISWHITE:
		...
		bd->bdBitsPerPixel =
			bd->bdBitsPerSample * bd->bdSamplesPerPixel;
		break;
	...
	case PHOTOMETRIC_PALETTE:
		if (TIFFGetField(t, TIFFTAG_COLORMAP, &redMap, &greenMap,
				 &blueMap) != 1) {
			LDEB(TIFFTAG_COLORMAP);
			rval = -1;
			goto ready;
		}
	...
	if (TIFFGetField(t, TIFFTAG_RESOLUTIONUNIT, &unsignedShort) != 1) {
		...
		case RESUNIT_INCH:
			bd->bdUnit = BMunINCH;
			if (TIFFGetField(t, TIFFTAG_XRESOLUTION, &resolution) !=
			    1) {
				LDEB(TIFFTAG_XRESOLUTION);
				rval = -1;
				goto ready;
			}
			bd->bdXResolution = (int)resolution;
			if (TIFFGetField(t, TIFFTAG_YRESOLUTION, &resolution) !=
			    1) {
				LDEB(TIFFTAG_YRESOLUTION);
				rval = -1;
				goto ready;
			}
		...
		if (bd->bdXResolution == 0 || bd->bdYResolution == 0) {
	...
	if (bd->bdBitsPerPixel % 8 == 0) {
		...
		for (row = 0; row < bd->bdPixelsHigh; row++) {
			if (TIFFReadScanline(t,
					     buffer + row * bd->bdBytesPerRow,
					     row, 0) < 0) {
				LDEB(row);
				free(buffer);
				TIFFClose(t);
...
int bmWriteTiffFile(const MemoryBuffer *filename, const unsigned char *buffer,
		    const BitmapDescription *bd, int privateFormat)
{
	...
	if (!TIFFSetField(t, TIFFTAG_COMPRESSION, privateFormat)) {
		LLDEB(TIFFTAG_COMPRESSION, privateFormat);
		return -1;
	}

	if (!TIFFSetField(t, TIFFTAG_IMAGELENGTH, (long)bd->bdPixelsHigh)) {
		LLDEB(TIFFTAG_IMAGELENGTH, (long)bd->bdPixelsHigh);
		return -1;
	}

	if (!TIFFSetField(t, TIFFTAG_IMAGEWIDTH, (long)bd->bdPixelsWide)) {
		LLDEB(TIFFTAG_IMAGEWIDTH, (long)bd->bdPixelsWide);
		return -1;
	}
	...
	case BMcoRGB:
		...
		break;
	case BMcoRGB8PALETTE:
		colorSpace = PHOTOMETRIC_PALETTE;
		break;
	default:
	...
	if (!TIFFSetField(t, TIFFTAG_PHOTOMETRIC, colorSpace)) {
		LLDEB(TIFFTAG_PHOTOMETRIC, colorSpace);
		return -1;
	}
	...
	if (colorSpace == PHOTOMETRIC_PALETTE) {
		...
		case 16:
			if (bd->bdHasAlpha) {
				...
				LDEB(bd->bdHasAlpha);
				LLDEB(bd->bdBitsPerPixel, bd->bdBitsPerSample);
				return -1;
			}
			break;
		...
		if (bd->bdPalette.cpColorCount > colorCountIn) {
			LLDEB(bd->bdPalette.cpColorCount, colorCountIn);
			return -1;
		}
	...
	*/
		...
		redMap = (unsigned short *)malloc(colorCountOut *
						  sizeof(unsigned short));
		greenMap = (unsigned short *)malloc(colorCountOut *
						    sizeof(unsigned short));
		blueMap = (unsigned short *)malloc(colorCountOut *
						   sizeof(unsigned short));
		...
		if (bd->bdHasAlpha) {
			if (!TIFFSetField(t, TIFFTAG_BITSPERSAMPLE,
					  bd->bdBitsPerPixel / 2)) {
				LLDEB(TIFFTAG_BITSPERSAMPLE,
				      bd->bdBitsPerSample);
				return -1;
			}
			if (!TIFFSetField(t, TIFFTAG_SAMPLESPERPIXEL, 2)) {
				LLDEB(TIFFTAG_SAMPLESPERPIXEL, 1);
				TIFFClose(t);
				return -1;
			}
		} else {
		...
		if (TIFFSetField(t, TIFFTAG_COLORMAP, redMap, greenMap,
				 blueMap) != 1) {
			LDEB(TIFFTAG_COLORMAP);
			TIFFClose(t);
			return -1;
		...
		if (!TIFFSetField(t, TIFFTAG_BITSPERSAMPLE,
				  bd->bdBitsPerSample)) {
			LLDEB(TIFFTAG_BITSPERSAMPLE, bd->bdBitsPerSample);
			return -1;
		}

		if (!TIFFSetField(t, TIFFTAG_SAMPLESPERPIXEL,
				  bd->bdSamplesPerPixel)) {
			LLDEB(TIFFTAG_SAMPLESPERPIXEL, bd->bdSamplesPerPixel);
			return -1;
		}
	}

	if (!TIFFSetField(t, TIFFTAG_PLANARCONFIG, 1)) {
		LLDEB(TIFFTAG_PLANARCONFIG, 1);
		TIFFClose(t);
		return -1;
	}
	...
	if (!TIFFSetField(t, TIFFTAG_ROWSPERSTRIP, rowsPerStrip)) {
		LLDEB(TIFFTAG_ROWSPERSTRIP, rowsPerStrip);
		TIFFClose(t);
		return -1;
	}
	...
	if (!TIFFSetField(t, TIFFTAG_RESOLUTIONUNIT, unit)) {
		LLDEB(TIFFTAG_RESOLUTIONUNIT, unit);
		return -1;
	}

	if (!TIFFSetField(t, TIFFTAG_XRESOLUTION, (float)xResolution)) {
		LLDEB(TIFFTAG_XRESOLUTION, xResolution);
		return -1;
	}

	if (!TIFFSetField(t, TIFFTAG_YRESOLUTION, (float)yResolution)) {
		LLDEB(TIFFTAG_YRESOLUTION, yResolution);
		return -1;
	}

	if (!TIFFSetField(t, TIFFTAG_SOFTWARE,
			  "appFrame:"
			  " Mark de Does,"
			  " mark@mdedoes.com")) {
		LDEB(TIFFTAG_SOFTWARE);
		return -1;
	}

	for (row = 0; row < bd->bdPixelsHigh; row++) {
		if (TIFFWriteScanline(t,
				      (char *)buffer + row * bd->bdBytesPerRow,
				      row, 0) < 0) {
			LDEB(row);
			return -1;
		}
