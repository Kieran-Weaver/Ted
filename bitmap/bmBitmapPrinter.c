			     ...
			     const uint16_t *from)
{
	...
	if (invert) {
		...
		for (col = 0; col + 1 < count; col += 2) {
			...
			if (sioOutPutByte((val0 & 0xf0) | ((val1 & 0xf00) >> 8),
					  sos) < 0) {
				return -1;
			}
...
static int bmPsWriteByteRow0(SimpleOutputStream *sos, int invert,
			     int bytesPerRow, const unsigned char *from)
{
	int col;
...
static int bmPsWriteByteRowX(SimpleOutputStream *sos, int invert, int originBit,
			     int rightShift, int bytesPerRow,
			     const unsigned char *from)
{
	...
	if (invert) {
		...
		for (col = 1; col < bytesPerRow; col++) {
			val = (from[0] << originBit) | (from[1] >> rightShift);
...
static int bmPsWritePal8Row(SimpleOutputStream *sos,
			    const BitmapDescription *bd, int bytesPerRow,
			    const unsigned char *from)
{
	int col;
...
static int bmPsWritePal4Row0(SimpleOutputStream *sos,
			     const BitmapDescription *bd, int selWidePix,
			     const unsigned char *from)
{
	int col;
	int val;
			     ...
			     const unsigned char *from)
{
	...
	for (col = 0; col + 3 < selWidePix; col += 4) {
		for (shift = 6; shift >= 0; shift -= 2) {
			...
			if (sioOutPutByte(cp->cpColors[val].rgb8Green, sos) <
			    0) {
				return -1;
			}
			if (sioOutPutByte(cp->cpColors[val].rgb8Blue, sos) <
			    0) {
				return -1;
			}
		}
			...
			const unsigned char *inputBuffer)
{
	...
	if (bd->bdBitsPerSample == 16 && directPixels) {
		...
		for (row = 0; row < selHighPix; row++) {
			from = inputBuffer +
			       (row + drSel->drY0) * bd->bdBytesPerRow +
			       originByte;
	...
	if (bd->bdColorEncoding == BMcoRGB && bd->bdHasAlpha) {
		...
		if (bd->bdBitsPerSample == 8) {
			for (row = 0; row < selHighPix; row++) {
				...
				bmPsWriteByteRow0(sos, invert, 3 * selWidePix,
						  scratch);
			}
		} else {
	...
	if (bd->bdColorEncoding == BMcoWHITEBLACK ||
		...
		for (row = 0; row < selHighPix; row++) {
			...
			bmPsWriteByteRowX(sos, invert, originBit, rightShift,
					  bytesPerRow, from);
		}
	...
	if (bd->bdColorEncoding == BMcoRGB8PALETTE && !directPixels &&
	    bd->bdBitsPerPixel == 8) {
		for (row = 0; row < selHighPix; row++) {
			from = inputBuffer +
			       (row + drSel->drY0) * bd->bdBytesPerRow +
	...
	if (bd->bdColorEncoding == BMcoRGB8PALETTE && !directPixels &&
	    bd->bdBitsPerPixel == 4 && originBit == 0) {
		for (row = 0; row < selHighPix; row++) {
			from = inputBuffer +
			       (row + drSel->drY0) * bd->bdBytesPerRow +
			       originByte;
	...
	if (bd->bdColorEncoding == BMcoRGB8PALETTE && !directPixels &&
	    bd->bdBitsPerPixel == 4 && originBit == 4) {
		for (row = 0; row < selHighPix; row++) {
			from = inputBuffer +
			       (row + drSel->drY0) * bd->bdBytesPerRow +
			       originByte;
	...
	if (bd->bdColorEncoding == BMcoRGB8PALETTE && !directPixels &&
	    bd->bdBitsPerPixel == 2 && originBit == 0) {
		for (row = 0; row < selHighPix; row++) {
			from = inputBuffer +
			       (row + drSel->drY0) * bd->bdBytesPerRow +
			       originByte;
	...
	LLDEB(originBit, bd->bdColorEncoding);
	return -1;
}
