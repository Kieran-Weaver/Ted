...
static int bmGifReadImageBuffer(BitmapDescription *bd, GifFileType *gft,
				unsigned char *buffer, int tc, int interlaced)
{
	int row;
	...
	if (interlaced) {
		for (row = 0; row < bd->bdPixelsHigh; row += 8) {
			if (bmGifGetPixels(gft, &ft, buffer + row * bpr, wide,
					   tc) != GIF_OK) {
				LDEB(row);
				return -1;
			}
		 ...
		 SimpleInputStream *sis)
{
	...
	for (;;) {
		...
		case EXTENSION_RECORD_TYPE: {
			...
			while (got) {
				if (DGifGetExtensionNext(gft, &got,
							 extension) != GIF_OK) {
					LDEB(1);
					rval = -1;
					goto ready;
...
int bmGifWriteGif(const BitmapDescription *bd, const unsigned char *buffer,
		  SimpleOutputStream *sos)
{
	...
	if (EGifPutImageDesc(gft, left, top, bd->bdPixelsWide, bd->bdPixelsHigh,
...
int bmWriteGifFile(const MemoryBuffer *filename, const unsigned char *buffer,
...
int bmCanWriteGifFile(const BitmapDescription *bd, int privateFormat)
{
