...
static void bmDraw1Run1Bit(unsigned char *buffer, int col0, int col1)
{
	int d;
	unsigned char c1, c2;
...
static void bmDraw0Run1Bit(unsigned char *buffer, int col0, int col1)
{
	int d;
	unsigned char c1, c2;
	unsigned char *b1 = (unsigned char *)0;
...
int bmDrawBox(unsigned char *buffer, const BitmapDescription *bd, int x0,
	      int y0, int x1, int y1, int wide)
{
	if (bmDrawLine(buffer, bd, x0, y0 + wide / 2, x1, y0 + wide / 2,
		       wide)) {
		LDEB(1);
		return -1;
	}

	if (bmDrawLine(buffer, bd, x0, y1 - wide / 2, x1, y1 - wide / 2,
		       wide)) {
		LDEB(1);
		return -1;
	}

	if (bmDrawLine(buffer, bd, x0 + wide / 2, y0, x0 + wide / 2, y1,
		       wide)) {
		LDEB(1);
		return -1;
	}

	if (bmDrawLine(buffer, bd, x1 - wide / 2, y0, x1 - wide / 2, y1,
		       wide)) {
		LDEB(1);
		return -1;
	}
...
static void bmDrawBres1Line1Bit(unsigned char *buffer, int x0, int y0, int x1,
				int y1, int bytesPerRow)
{
	int dx;
	int dy;
	int i;
...
static void bmDrawBres0Line1Bit(unsigned char *buffer, int x0, int y0, int x1,
				int y1, int bytesPerRow)
{
	int dx;
	int dy;
...
static void bmDrawRun24Bit(unsigned char *buffer, int col0, int col1)
{
	/*  1  */
	memset(buffer + 3 * col0, 0x00, 3 * (col1 - col0));
}

static void bmDrawBresLine24Bit(unsigned char *buffer, int x0, int y0, int x1,
				int y1, int bytesPerRow)
{
	int dx;
	int dy;
...
int bmDrawLine(unsigned char *buffer, const BitmapDescription *bd, int x0,
	       int y0, int x1, int y1, int wide)
{
	int i;
	...
	void (*drawRun)(unsigned char *_buffer, int _x0, int _y0);

	void (*drawLine)(unsigned char *_buffer, int _x0, int _y0, int _x1,
			 int _y1, int _bytesPerRow);
