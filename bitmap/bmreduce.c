...
#define ppm_hash(r, g, b)                                                    \
	((((int)(r)*33023 + (int)(g)*30013 + (int)(b)*27011) & 0x7fffffff) % \
	 HASH_SIZE)
...
static int bmInsertHash(HashBucket **hashTable, int hash, int number, int r,
			int g, int b)
{
...
static int bmAllocateCutNodes(ColorReducer *cr, int colorCount)
{
	CutNode *fresh;

	fresh = (CutNode *)realloc(cr->crCutNodes,
				   2 * colorCount * sizeof(CutNode));
	if (!fresh) {
		LLDEB(colorCount, fresh);
		return -1;
	}
...
static int bmAllocateHistogram(ColorReducer *cr, int colorCount)
{
...
static int bmHistRedMedian(int *pPixelCount, int *pColorCount, int pixelCount,
			   HistogramEntry *histogram, int colorCount,
			   unsigned int cMin, unsigned int cMax)
{
	int cMed;
	...
	*pPixelCount = pixelCount;
	*pColorCount = lt;
	return cMed;
}

static int bmHistGreenMedian(int *pPixelCount, int *pColorCount, int pixelCount,
			     HistogramEntry *histogram, int colorCount,
			     unsigned int cMin, unsigned int cMax)
{
	int cMed;
...
static int bmHistBlueMedian(int *pPixelCount, int *pColorCount, int pixelCount,
			    HistogramEntry *histogram, int colorCount,
			    unsigned int cMin, unsigned int cMax)
{
	int cMed;
...
static void bmDelimitColorBox(ColorBox *cb, const HistogramEntry *histogram)
{
	const HistogramEntry *he;
	int i;

	if (cb->cbColorCount == 0) {
		LDEB(cb->cbColorCount);
		return;
	}
...
static void bmSplitBox(HistogramEntry *histogram, const ColorBox *cbParent,
		       ColorBox *cbLow, ColorBox *cbHigh, int *pLeftMaximum,
		       int *pComponent)
{
	...
	*cbHigh = *cbParent;
	...
	if (77 * rDif > 150 * gDif && 77 * rDif > 29 * bDif) {
		cbLow->cbRMax = bmHistRedMedian(
			&(cbLow->cbPixelCount), &(cbLow->cbColorCount),
			cbParent->cbPixelCount, histogram,
			cbParent->cbColorCount, cbParent->cbRMin,
			cbParent->cbRMax);

		cbHigh->cbRMin = cbLow->cbRMax + 1;
		cbHigh->cbColorCount =
			cbParent->cbColorCount - cbLow->cbColorCount;
		cbHigh->cbPixelCount =
			cbParent->cbPixelCount - cbLow->cbPixelCount;
		cbHigh->cbHistogramOffset =
			cbParent->cbHistogramOffset + cbLow->cbColorCount;
		...
		*pComponent = CN_RED;
		...
		appDebug("CUT RED   at %3d..%3d..%3d "
			 ...
			 cbParent->cbColorCount, cbLow->cbPixelCount,
			 cbHigh->cbPixelCount, cbParent->cbPixelCount);
#endif
	} else {
		if (150 * gDif > 29 * bDif) {
			cbLow->cbGMax = bmHistGreenMedian(
				&(cbLow->cbPixelCount), &(cbLow->cbColorCount),
				cbParent->cbPixelCount, histogram,
				cbParent->cbColorCount, cbParent->cbGMin,
				cbParent->cbGMax);
			...
			*pComponent = CN_GREEN;
			...
			appDebug("CUT GREEN at %3d..%3d..%3d "
				 "%4d+ %4d= %4dC %6d+ %6d= %6dP\n",
				 cbParent->cbGMin, cbLow->cbGMax,
				 cbParent->cbGMax, cbLow->cbColorCount,
				 cbHigh->cbColorCount, cbParent->cbColorCount,
				 cbLow->cbPixelCount, cbHigh->cbPixelCount,
				 cbParent->cbPixelCount);
#endif
		} else {
			cbLow->cbBMax = bmHistBlueMedian(
				&(cbLow->cbPixelCount), &(cbLow->cbColorCount),
				cbParent->cbPixelCount, histogram,
				cbParent->cbColorCount, cbParent->cbBMin,
				cbParent->cbBMax);
			...
			*pComponent = CN_BLUE;
			...
			appDebug("CUT BLUE  at %3d..%3d..%3d "
				 "%4d+ %4d= %4dC %6d+ %6d= %6dP\n",
				 cbParent->cbBMin, cbLow->cbBMax,
				 cbParent->cbBMax, cbLow->cbColorCount,
				 cbHigh->cbColorCount, cbParent->cbColorCount,
				 cbLow->cbPixelCount, cbHigh->cbPixelCount,
				 cbParent->cbPixelCount);
#endif
...
static int bmFindCuts(ColorBox *boxes, CutNode *nodes, HistogramEntry *histo,
		      int colorCount, int pixelCount, int maxcolors)
{
	int i;
	int boxCount;
	int nodeCount = 0;
	...
	while (boxCount < maxcolors) {
		...
		if (i >= boxCount) { /* LLDEB(i,boxCount); */
			break;
		}
		biggestBox = i;
		for (i = biggestBox + 1; i < boxCount; i++) {
			if (boxes[i].cbColorCount > 1 &&
			    boxes[i].cbPixelCount >
		...
		bmSplitBox(histo, &cbSplit, boxes + biggestBox,
			   boxes + boxCount, &leftMaximum, &cutComponent);
...
static int bmFindColors(ColorReducer *cr, int colorCount, int pixelCount,
			int maxcolors)
{
	ColorBox *boxes;
	const ColorBox *cb;
	...
	for (i = 0; i < boxCount; cb++, i++) {
		...
		appDebug("BOX %3d: "
			 ...
			 i, cb->cbRMin, cr->crPalette[i].rgb8Red, cb->cbRMax,
...
static int bmReduceAllocateColorCut(AllocatorColor *ac, ColorAllocator *ca,
				    ...
				    unsigned int b)
{
	...
	for (;;) {
		...
		case CN_LEAF:
			...
			ac->acGreen =
				257 * cr->crPalette->cpColors[i].rgb8Green;
			ac->acBlue = 257 * cr->crPalette->cpColors[i].rgb8Blue;
			ac->acColorNumber = i;
			return 0;

		default:
			LDEB(cn[i].cnComponent);
			return -1;
		}
	}
}

static int bmReduceAllocateColorHash(AllocatorColor *ac, ColorAllocator *ca,
				     unsigned int r, unsigned int g,
				     unsigned int b)
{
	ColorReducer *cr = (ColorReducer *)ca->caSystemPrivate;
	...
	while (hashBucket) {
		if (hashBucket->hbRed == r && hashBucket->hbGreen == g &&
		    hashBucket->hbBlue == b) {
			break;
		}
...
static int bmHashColorsRGB8(HashBucket ***pHashTable, int *pColorCount,
			    int pixelsHigh, int pixelsWide, int bytesPerRow,
			    int hasAlpha, const unsigned char *bufIn,
			    int maxcolors, unsigned int mask)
{
	unsigned int r;
	unsigned int g;
	unsigned int b;
	...
	for (row = 0; row < pixelsHigh; row++) {
		...
		for (col = 0; col < pixelsWide; col++) {
			...
			if (bmInsertHash(hashTable, hash, colorCount++, r, g,
					 b)) {
				LDEB(1);
...
static int bmHashToHistogram(ColorReducer *cr, int colorCount)
{
	HashBucket *hashBucket;
...
int bmColorReduce(RasterImage *riOut, const RasterImage *riIn, int maxcolors)
{
	...
	if (mask == 0xff && colorCount <= maxcolors) {
		...
		/*  5  */
		colorsFound = bmFindColors(
			cr, colorCount, bdIn->bdPixelsHigh * bdIn->bdPixelsWide,
			maxcolors);
	...
	if (cr->crCutNodes) {
		...
		ca.caSystemAllocator = bmReduceAllocateColorHash;
	}
				       ...
				       const BitmapDescription *bd)
{
	...
	if (bd->bdColorEncoding != BMcoRGB8PALETTE) {
		LLDEB(bd->bdColorEncoding, BMcoRGB8PALETTE);
		rval = -1;
		goto ready;
	}
	...
	if (!boxes) {
		LLDEB(cp->cpColorCount, boxes);
		return -1;
	}

	map = (int *)malloc(cp->cpColorCount * sizeof(int));
	if (!map) {
		LLDEB(cp->cpColorCount, map);
		return -1;
	}
	...
	boxCount = bmFindCuts(boxes, cr->crCutNodes, cr->crHistogram,
			      cp->cpColorCount, cp->cpColorCount,
			      cp->cpColorCount);

	if (boxCount != cp->cpColorCount) {
		LLDEB(boxCount, cp->cpColorCount);
		rval = -1;
		goto ready;
	}
	...
	for (i = 0; i < cp->cpColorCount; i++) {
		...
		bmReduceAllocateColorCut(&ac, ca, cp->cpColors[i].rgb8Red,
