...
int bmTransparentImage(BitmapDescription *bdOut, unsigned char **pBufOut,
...
int bmRGBImage(BitmapDescription *bdOut, unsigned char **pBufOut,
	       int colorEncoding, int r, int g, int b, int wide, int high)
{
	int rval = 0;
	BitmapDescription bd;
	unsigned char *buffer = (unsigned char *)0;
...
int bmSetSolidWhite(RasterImage *ri)
{
	...
	case BMcoRGB8PALETTE:
		...
		if (col < 0) {
			LLDEB(bd->bdPalette.cpColorCount, col);
			return -1;
		}
