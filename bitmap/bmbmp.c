...
static int bmBmpRead3ColorTable(int colorCount, SimpleInputStream *sis,
				RGB8Color *palette)
{
	int i;
...
static int bmBmpWrite3ColorTable(int colorCount, SimpleOutputStream *sos,
...
static int bmBmpRead4ColorTable(int colorCount, SimpleInputStream *sis,
				RGB8Color *palette)
{
	int i;
...
static int bmBmpWrite4ColorTable(int colorCount, SimpleOutputStream *sos,
				 const RGB8Color *palette)
{
	int i;
		       ...
		       RGB8Color **pPalette)
{
	...
	case 12: /*  OS/2 1.*	*/
		...
		/*  3  */
		if (colorCount > 0) {
			palette = (RGB8Color *)malloc(colorCount *
						      sizeof(RGB8Color));
...
palette40:
		...
		case 1:
			if (bih->bihColorCount == 0) {
				colorCount = (1U << (bih->bihPlanes *
						     bih->bihBitsPerPixel));
			} else {
				colorCount = bih->bihColorCount;
			}
...
static int bmpReadFileHeader(BmpImageHeader *bih, SimpleInputStream *sis,
			     RGB8Color **pPalette)
{
	int done;
...
static int bmBmpReadUncompressed(SimpleInputStream *sis, unsigned char *buffer,
				 int pixelsHigh, int bytesPerRowB,
				 int bytesPerRowF, int bitsPerPixelF)
{
	int row;
	int col;
	int done = 0;
	...
	case 8:
		if (bytesPerRowB != bytesPerRowF) {
			LLDEB(bytesPerRowB, bytesPerRowF);
			return -1;
		}
...
palette:
	...
	*pBytesPerRow = (bih->bihBitsPerPixel * bd->bdPixelsWide + 7) / 8;
	...
	if (bd->bdXResolution < 500 || bd->bdXResolution < 500) {
		bd->bdYResolution = bd->bdXResolution = 1;
		bd->bdUnit = BMunPIXEL;
	}
			     ...
			     const BitmapDescription *bd)
{
	...
	case BMcoBLACKWHITE:
	case BMcoWHITEBLACK:
		if (bmMakeGrayPalette(bd, &colorCount, &transparentColor,
				      BWPalette, 257)) {
...
int bmBmpReadImageBytes(unsigned char **pBuffer, SimpleInputStream *sis,
			int compression, int bytesPerRowF, int bitsPerPixelF,
			const BitmapDescription *bd)
{
	int rval = -1;
	...
	switch (compression) {
	case COMPRESS_NONE:
		done = bmBmpReadUncompressed(sis, buffer, high, bytesPerRowB,
					     bytesPerRowF, bitsPerPixelF);
		  ...
		  SimpleInputStream *sis)
{
	...
	done = bmBmpReadImageBytes(&buffer, sis, bih->bihCompression,
				   bytesPerRowF, bih->bihBitsPerPixel, &bd);
	if (done < 0) {
			...
			const RGB8Color *palette)
{
	...
	case 12: /*  OS/2 1.*	*/
		...
		/*  2  */
		if (bih->bihColorCount > 0 && palette) {
			if (bmBmpWrite3ColorTable(bih->bihColorCount, sos,
						  palette)) {
				XDEB(sos);
				return -1;
			}
	...
	case 40: /*  Windows 3.*	*/
		...
		/*  2  */
		if (bih->bihColorCount > 0 && palette) {
			if (bmBmpWrite4ColorTable(bih->bihColorCount, sos,
						  palette)) {
				XDEB(sos);
				return -1;
			}
...
static int bmpWriteFileHeader(const BmpImageHeader *bih,
			      SimpleOutputStream *sos, const RGB8Color *palette)
{
	int bytesWritten;
...
static int bmpSizeFileHeader(const BmpImageHeader *bih, RGB8Color *palette)
{
	int bytesNeeded;
...
static int bmBmpWriteScanLine(SimpleOutputStream *sos, int pixelsPerByte,
			      int maxByteCount, const unsigned char *buffer,
			      int npixels, int nbytes)
{
	int done = 0;
	int byteCount;
	int pixelCount;
	...
	while (npixels > 0) {
		...
		/*  1  */
		while (byteCount < nbytes && byteCount < maxByteCount &&
		       buffer[byteCount] == byteValue) {
			byteCount++;
			pixelCount += pixelsPerByte;
		}

		if (pixelsPerByte == 2 && pixelCount == npixels - 1 &&
		...
		/*  3  */
		while (byteCount + 1 < nbytes && byteCount + 1 < maxByteCount &&
		       buffer[byteCount + 1] != buffer[byteCount]) {
			byteCount++;
			pixelCount += pixelsPerByte;
		}

		if (byteCount + 1 == nbytes && byteCount + 1 < maxByteCount) {
			byteCount++;
			pixelCount += pixelsPerByte;
		}
		...
		case 2:
			...
			LLDEB(pixelCount, pixelsPerByte);
			return -1;
		case 0:
			LLDEB(pixelCount, pixelsPerByte);
			return -1;
			...
			const unsigned char *buffer, int compression)
{
	...
	case COMPRESS_NONE:
		for (row = 0; row < bd->bdPixelsHigh; row++) {
			from = buffer +
			       (bd->bdPixelsHigh - row - 1) * bd->bdBytesPerRow;
			...
			if (bd->bdBitsPerPixel == 24) {
				...
				if (sioOutWriteBytes(sos, from,
						     bd->bdBytesPerRow) !=
				    bd->bdBytesPerRow) {
					LDEB(bd->bdBytesPerRow);
					bytesWritten = -1;
					goto ready;
				}
			}
	...
	case COMPRESS_RLE4:
		...
		if (compression == 2 && bd->bdBitsPerPixel == 2) {
			inflated =
				(unsigned char *)malloc(2 * bd->bdBytesPerRow);
			if (!inflated) {
				LXDEB(bd->bdBytesPerRow, inflated);
				bytesWritten = -1;
				goto ready;
			}
		...
		if (compression == 1 && bd->bdHasAlpha &&
		...
		for (row = 0; row < bd->bdPixelsHigh; row++) {
			...
			done = bmBmpWriteScanLine(sos, compression,
						  maxByteCount, from,
						  bd->bdPixelsWide,
						  bytesPerRow);
		 ...
		 RGB8Color **pPalette, int privateFormat, int fileHeader)
{
	...
	if (bmbmpDescriptionToHeader(bih, &palette, privateFormat,
				     bd->bdHasAlpha, bd)) {
		LDEB(privateFormat);
		return -1;
	}
...
int bmWriteBmpFile(const MemoryBuffer *filename, const unsigned char *buffer,
