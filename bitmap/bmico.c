		  ...
		  BitmapDescription *bd, int *pPrivateFormat)
{
	...
	if (bmSetAlphaMask(bd, &bdImage, &bdAlpha, &buffer, bufImage,
			   bufAlpha)) {
		LDEB(1);
		rval = -1;
		goto ready;
...
static int bmIcoCollectResources(IcoFileHeader *ifh, IcoDirectoryEntry *ide,
				 BmpImageHeader *bih, RGB8Color **pPalette,
				 MemoryBuffer *mbImage, MemoryBuffer *mbAlpha,
				 ...
				 const unsigned char *buffer)
{
	...
	if (bd->bdHasAlpha) {
		...
		imageBytes =
			bmBmpSaveImageBytes(mbImage, &(riImage.riDescription),
		...
		alphaBytes =
			bmBmpSaveImageBytes(mbAlpha, &(riAlpha.riDescription),
		...
		if (bmAllocateBuffer(&riAlpha)) {
			LXDEB(riAlpha.riDescription.bdBufferLength,
			      riAlpha.riBytes);
			rval = -1;
			goto ready;
		}
		...
		imageBytes =
			bmBmpSaveImageBytes(mbImage, bd, buffer, COMPRESS_NONE);
		if (imageBytes < 0) {
			LDEB(imageBytes);
			rval = -1;
			goto ready;
...
static int bmIcoWriteResources(SimpleOutputStream *sos,
			       ...
			       const RGB8Color *palette, MemoryBuffer *mbImage,
			       MemoryBuffer *mbAlpha)
{
	...
	if (bih->bihBitmapDataOffset != bytesWritten) {
		LLDEB(bih->bihBitmapDataOffset, bytesWritten);
		return -1;
	}
...
int bmWriteIcoFile(const MemoryBuffer *filename, const unsigned char *buffer,
		   const BitmapDescription *bd, int privateFormat)
{
	...
	if (bytesWritten != bytesNeeded) {
		LLDEB(bytesNeeded, bytesWritten);
		rval = -1;
		goto ready;
	}
