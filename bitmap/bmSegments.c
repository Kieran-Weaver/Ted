...
static int bmRememberSegmentNode(BitmapSegment *bs, SegmentNode *sn)
{
	SegmentNode **nw;
	int row = sn->snY0;

	nw = (SegmentNode **)realloc(
		bs->bsNodes, (bs->bsNodeCount + 1) * sizeof(SegmentNode *));
...
static int bmRememberSegmentEdge(BitmapSegment *bs, SegmentEdge *se)
{
	SegmentEdge **nw;
	int i;
	DataRun *dr;

	nw = (SegmentEdge **)realloc(
		bs->bsEdges, (bs->bsEdgeCount + 1) * sizeof(SegmentEdge *));
...
static SegmentNode *bmSegmentNode(BitmapSegment *bs, int row)
{
	SegmentNode *sn;
...
static int bmConnectSegmentEdgeToNode(SegmentNode *sn, SegmentEdge *se, int up)
{
	short int *pcount;
	SegmentEdge ***pold;
	...
	if (up) {
		pcount = &sn->snUpEdgeCount;
		pold = &sn->snUpEdges;
	} else {
		pcount = &sn->snDownEdgeCount;
		pold = &sn->snDownEdges;
	}
...
static SegmentEdge *bmSegmentEdge(BitmapSegment *bs, SegmentNode *from, int x0,
				  int xp)
{
	SegmentEdge *se;
...
static int bmAddRunToSegmentEdge(BitmapSegment *bs, SegmentEdge *se, int x0,
				 int xp)
{
	DataRun *dr;
	int nrun;
...
static BitmapSegment *bmNewSegment(void)
{
	...
	if (!bs->bsNodes || !bs->bsEdges) {
		XXDEB(bs->bsEdges, bs->bsNodes);
		bmFreeSegment(bs);
		return (BitmapSegment *)0;
	}

	bs->bsRect.drX0 = bs->bsRect.drX1 = bs->bsRect.drY0 = bs->bsRect.drY1 =
		-1;
...
static int bmMoveComponentContents(BitmapSegment *to, BitmapSegment *from)
{
	int i;
...
/*  1  */
static int bmc0Run(const unsigned char *buf, int bytesPerRow, int xsz,
		   const int col, int nfret)
{
	int colbyte = col / 8;
...
/*  2  */
static int bmc1Run(const unsigned char *buf, int bytesPerRow, int xsz,
		   const int col, int nfret)
{
	int colbyte = col / 8;
	int bit = col - 8 * colbyte;
...
static int bcBranch(CompIndex *ci, LookUp *lu, SegmentNode *sn)
{
	SegmentEdge *se;
	DataRun *dr;
...
static int bmStartNewSegment(int *pnco, int *pmco, CompIndex **pcomps,
			     SegmentEdge **pse, int x0, int xp, int row)
{
	int nco = *pnco;
	int mco = *pmco;
...
static int bcMergeComp(CompIndex *comps, int *pnco, int newcomp, int oldcomp,
		       LookUp *olu, int oLuCount, LookUp *nlu, int nLuCount)
{
	int nco = *pnco;

	if (oldcomp == newcomp) {
		comps[oldcomp].ci_count--;
		return 0;
	}
...
static void bcCountComp(CompIndex *comps, int newcomp, LookUp *olu,
			int oLuCount, LookUp *nlu, int nLuCount)
{
	comps[newcomp].ci_count = 0;

	while (--oLuCount >= 0) {
		if (olu->lu_use != LU_ENDED && olu->lu_compid == newcomp) {
...
static int bcTerminate(CompIndex *ci, LookUp *plu, int row)
{
	SegmentNode *sn;
...
static int bcContinueSegments(int *pNLuCount, int *pNCo, LookUp *olu,
			      const int oLuCount, LookUp *nlu, int nLuCount,
			      int nco, CompIndex *comps, const int row,
			      const int x0, const int xp, int beg, int end)
{
	int compid;
	...
	if (beg < end - 1) {
		...
		while (beg < end - 1) {
			...
			if (bcMergeComp(comps, &nco, compid,
					olu[beg + 1].lu_compid, olu + beg,
					oLuCount - beg, nlu, nLuCount)) {
				LDEB(1);
				return -1;
			}

			if (bmConnectSegmentEdgeToNode(sn, olu[beg].lu_edge,
						       1)) {
				LDEB(1);
				return -1;
			}
		...
		bcCountComp(comps, compid, olu + beg, oLuCount - beg, nlu,
			    nLuCount);
		 ...
		 const unsigned char *buffer, const BitmapDescription *bd)
{
	...
	int (*whiteRun)(const unsigned char *_buf, int bytesPerRow, int xsz,
			const int col, int nfret);
	int (*blackRun)(const unsigned char *_buf, int bytesPerRow, int xsz,
			const int col, int nfret);
	...
	for (row = 0; row < bd->bdPixelsHigh; row++) {
		...
		for (;;) {
			...
			/*  7  */
			if (end > beg) {
				if (bcContinueSegments(&nLuCount, &nco, olu,
						       oLuCount, nlu, nLuCount,
						       nco, comps, row, x0, xp,
						       beg, end)) {
					LDEB(1);
					goto failure;
				...
				compid = bmStartNewSegment(&nco, &mco, &comps,
							   &se, x0, xp, row);
				if (compid < 0) {
					LDEB(compid);
					goto failure;
		...
		while (beg < oLuCount) {
			...
			if (olu[beg].lu_use == LU_UNSPEC) {
				if (bcTerminate(comps + compid, olu + beg,
						row - 1)) {
					goto failure;
				}
    ...
    */
	...
	for (beg = 0; beg < end; beg++) {
		...
		wide = comps[beg].ci_compo->bsRect.drX1 -
		       comps[beg].ci_compo->bsRect.drX0 + 1;
		high = comps[beg].ci_compo->bsRect.drY1 -
		       comps[beg].ci_compo->bsRect.drY0 + 1;
		     ...
		     int row0, int bytesPerRow, int colorEncoding)
{
	...
	for (i = 0; i < bs->bsEdgeCount; i++, se++) {
		...
		for (j = 0; j < se[0]->seRunCount; j++, dr++) {
			bmFillBlock(buffer, dr->drX0 - col0, row,
		      ...
		      const void *voidbs)
{
	...
	if (pBuffer) {
		buffer =
			bmBackgroundBuffer(resbd.bdBufferLength, colorEncoding);
		if (!buffer) {
			LLDEB(resbd.bdBufferLength, buffer);
			return -1;
		}

		if (bmcDrawComponent(bs, buffer, bs->bsRect.drX0, 0,
				     resbd.bdBytesPerRow, colorEncoding)) {
			free(buffer);
			return -1;
		}
