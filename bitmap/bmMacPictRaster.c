...
static int bmMacPictGetRect(DocumentRectangle *dr, SimpleInputStream *sis)
{
	int done = 0;
...
static int bmMacPictReadPaletteColors(BitmapDescription *bd, int colorCount,
				      int flags, SimpleInputStream *sis)
{
	int col;
	...
	for (col = 0; col < colorCount; rgb8++, col++) {
		...
		rgb8->rgb8Red = sioEndianGetBeUint16(sis) / 256;
		bytesRead += 2;
		rgb8->rgb8Green = sioEndianGetBeUint16(sis) / 256;
		bytesRead += 2;
		rgb8->rgb8Blue = sioEndianGetBeUint16(sis) / 256;
		bytesRead += 2;
	}
...
static int bmMacPictReadPacBitsRow(unsigned char *to, int bytesPerRow,
				   ...
				   SimpleInputStream *sis)
{
	...
	if (bytesPerRow > 250) {
		byteCount = sioEndianGetBeInt16(sis);
		bytesRead += 2;
	} else {
		byteCount = sioInGetByte(sis);
		bytesRead++;
	}
...
static int bmMacPictReadPacBitsImage(int bytesPerRow, RasterImage *abi,
				     SimpleInputStream *sis)
{
	int rval = -1;
	int bytesRead = 0;
...
static int bmMacPictReadPlanarPacBitsImage(int bytesPerRow, RasterImage *abi,
					   SimpleInputStream *sis)
{
	int rval = -1;
	int bytesRead = 0;
	...
	if (!scratch) {
		LPDEB(bd->bdBytesPerRow, scratch);
		goto ready;
	}
...
int bmMacPictGetImage(RasterImage *abi, DocumentRectangle *drSource,
		      ...
		      int packed, int direct, int clipRegion)
{
	...
	if (direct) {
		...
		bytesPerRow = sioEndianGetBeUint16(sis);
		bytesRead += 2;
	...
	if (direct || hasPalette) {
		...
		version = sioEndianGetBeInt16(sis);
		bytesRead += 2;
		packType = sioEndianGetBeInt16(sis);
		bytesRead += 2;
		if (packType == 4) {
			packed = 1;
		}
		packSize = sioEndianGetBeInt32(sis);
		bytesRead += 4;
		xRes = sioEndianGetBeInt32(sis);
		bytesRead += 4;
		yRes = sioEndianGetBeInt32(sis);
		bytesRead += 4;
		pixelType = sioEndianGetBeInt16(sis);
		bytesRead += 2;
		bd->bdBitsPerPixel = sioEndianGetBeInt16(sis);
		bytesRead += 2;
		bd->bdSamplesPerPixel = sioEndianGetBeInt16(sis);
		bytesRead += 2;
		bd->bdBitsPerSample = sioEndianGetBeInt16(sis);
		bytesRead += 2;
		planeBytes = sioEndianGetBeInt32(sis);
		bytesRead += 4;
		table = sioEndianGetBeInt32(sis);
		bytesRead += 4;
		reserved = sioEndianGetBeInt32(sis);
		bytesRead += 4;
	...
	if (hasPalette) {
		...
		/*seed=*/(void)sioEndianGetBeInt32(sis);
		bytesRead += 4;
		flags = sioEndianGetBeUint16(sis);
		bytesRead += 2;
		colorCount = sioEndianGetBeInt16(sis) + 1;
		bytesRead += 2;
	...
	if (!abi->riBytes) {
		LXDEB(bd->bdBufferLength, abi->riBytes);
		return -1;
	}
	...
	drImage.drX1 -= drImage.drX0 + 1;
	drImage.drX0 = 0;
	drImage.drY1 -= drImage.drY0 + 1;
	drImage.drY0 = 0;
	...
	if (clipRegion) {
		int i;
		int count = sioEndianGetBeInt16(sis);
		bytesRead += 2;
	...
	if (!packed || bytesPerRow < 8) {
		...
		if (bd->bdColorEncoding == BMcoRGB) {
			...
			for (row = 0; row < bd->bdPixelsHigh; row++) {
				...
				for (i = 0;
				     i < bd->bdBytesPerRow && i < bytesPerRow;
				     i++) {
					*(to++) = sioInGetByte(sis);
					bytesRead++;
				}
		...
		if (rowsArePlanar) {
			step = bmMacPictReadPlanarPacBitsImage(bytesPerRow, abi,
							       sis);
			if (step < 0) {
				LDEB(step);
				return -1;
