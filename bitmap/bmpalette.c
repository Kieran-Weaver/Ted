...
int bmExpandRGB8Palette(unsigned char *to, const unsigned char *from,
			int pixelsWide, int bitsPerPixel,
			const ColorPalette *cp, int hasAlpha)
{
	unsigned char f1, f2;
...
int bmExpandPaletteImage(RasterImage *riOut, const RasterImage *riIn,
			 int ignoredInt)
{
    ...
    */
	...
	for (row = 0; row < bdIn->bdPixelsHigh; row++) {
		if (bmExpandRGB8Palette(to, from, bdIn->bdPixelsWide,
					bdIn->bdBitsPerPixel,
...
int bmMakeGrayPalette(const BitmapDescription *bd, int *pColorCount,
		      int *pTransparentColor, RGB8Color *palette, int maxColors)
{
	int i;
	int colorCount;
...
int bmInflateTo8bit(unsigned char *to, const unsigned char *from,
		    const BitmapDescription *bd, const int transpColor,
		    int removeAlpha)
{
	int col;
	unsigned char b;
	...
	case 8:
		if (bd->bdHasAlpha && removeAlpha) {
			...
			memcpy(to, from, bd->bdBytesPerRow);
		}
