...
static int scanFillRow_8_FromRGB(unsigned char *to, const FillJob *fj,
				 ColorValue *val)
{
	...
	if (fj->fjToWide <= fj->fjFrWide) {
		...
		for (col = 0; col < fj->fjToWide; cor++, col++) {
			...
			case CA_INDEX_ALLOC:
				...
				c = ac.acColorNumber;
				if (ca->caColors[c].acAllocated ==
				    AC_UNALLOCATED) {
					if ((*ca->caSystemAllocator)(&ac, ca, r,
								     g, b)) {
						LDEB(col);
						return -1;
					}
			...
			case CA_ALLOCATOR:
				if ((*ca->caSystemAllocator)(&ac, ca, r, g,
							     b)) {
					LDEB(col);
					return -1;
				}
...
static int scanFillRow_4_FromRGB(unsigned char *to, const FillJob *fj,
				 ColorValue *val)
{
	...
	for (col = 0; col < fj->fjToWide; to++, from += 2, col += 2) {
		to[0] = ((from[0] & 0x0f) << 4) | ((from[1] & 0x0f) << 0);
	}
...
static int scanFillRow_2_FromRGB(unsigned char *to, const FillJob *fj,
				 ColorValue *val)
{
	const unsigned char *from;
	int col;
	...
	for (col = 0; col < fj->fjToWide; to++, from += 4, col += 4) {
		to[0] = ((from[0] & 0x03) << 6) | ((from[1] & 0x03) << 4) |
			((from[2] & 0x03) << 2) | ((from[3] & 0x03) << 0);
	}
...
static int scanFillD1RgbRow_32SX(unsigned char *to, const FillJob *fj,
				 ColorValue *val)
{
	const unsigned char *from;
	int col;
	...
	for (col = 0; col < fj->fjToWide; to += 4, from += 32, col += 32) {
		to[3] = (from[0] << 7) | (from[1] << 6) | (from[2] << 5) |
			(from[3] << 4) | (from[4] << 3) | (from[5] << 2) |
			(from[6] << 1) | (from[7] << 0);

		to[2] = (from[8] << 7) | (from[9] << 6) | (from[10] << 5) |
			(from[11] << 4) | (from[12] << 3) | (from[13] << 2) |
			(from[14] << 1) | (from[15] << 0);

		to[1] = (from[16] << 7) | (from[17] << 6) | (from[18] << 5) |
			(from[19] << 4) | (from[20] << 3) | (from[21] << 2) |
			(from[22] << 1) | (from[23] << 0);

		to[0] = (from[24] << 7) | (from[25] << 6) | (from[26] << 5) |
			(from[27] << 4) | (from[28] << 3) | (from[29] << 2) |
			(from[30] << 1) | (from[31] << 0);
	}
...
static int scanFillD1RgbRow_32SS(unsigned char *to, const FillJob *fj,
				 ColorValue *val)
{
	const unsigned char *from;
	int col;
	...
	from = fj->fjScratch;
	for (col = 0; col < fj->fjToWide; to += 4, from += 32, col += 32) {
		to[3] = (from[7] << 7) | (from[6] << 6) | (from[5] << 5) |
			(from[4] << 4) | (from[3] << 3) | (from[2] << 2) |
			(from[1] << 1) | (from[0] << 0);

		to[2] = (from[15] << 7) | (from[14] << 6) | (from[13] << 5) |
			(from[12] << 4) | (from[11] << 3) | (from[10] << 2) |
			(from[9] << 1) | (from[8] << 0);

		to[1] = (from[23] << 7) | (from[22] << 6) | (from[21] << 5) |
			(from[20] << 4) | (from[19] << 3) | (from[18] << 2) |
			(from[17] << 1) | (from[16] << 0);

		to[0] = (from[31] << 7) | (from[30] << 6) | (from[29] << 5) |
			(from[28] << 4) | (from[27] << 3) | (from[26] << 2) |
			(from[25] << 1) | (from[24] << 0);
	}
...
static int scanFillD1RgbRow_16SX(unsigned char *to, const FillJob *fj,
				 ColorValue *val)
{
	const unsigned char *from;
	int col;
	...
	for (col = 0; col < fj->fjToWide; to += 2, from += 16, col += 16) {
		to[1] = (from[0] << 7) | (from[1] << 6) | (from[2] << 5) |
			(from[3] << 4) | (from[4] << 3) | (from[5] << 2) |
			(from[6] << 1) | (from[7] << 0);

		to[0] = (from[8] << 7) | (from[9] << 6) | (from[10] << 5) |
			(from[11] << 4) | (from[12] << 3) | (from[13] << 2) |
			(from[14] << 1) | (from[15] << 0);
	}
...
static int scanFillD1RgbRow_16SS(unsigned char *to, const FillJob *fj,
				 ColorValue *val)
{
	const unsigned char *from;
	int col;
	...
	from = fj->fjScratch;
	for (col = 0; col < fj->fjToWide; to += 2, from += 16, col += 16) {
		to[1] = (from[7] << 7) | (from[6] << 6) | (from[5] << 5) |
			(from[4] << 4) | (from[3] << 3) | (from[2] << 2) |
			(from[1] << 1) | (from[0] << 0);

		to[0] = (from[15] << 7) | (from[14] << 6) | (from[13] << 5) |
			(from[12] << 4) | (from[11] << 3) | (from[10] << 2) |
			(from[9] << 1) | (from[8] << 0);
	}
...
static int scanFillD1RgbRow_8X(unsigned char *to, const FillJob *fj,
			       ColorValue *val)
{
	const unsigned char *from;
	int col;
	...
	for (col = 0; col < fj->fjToWide; to++, from += 8, col += 8) {
		to[0] = (from[0] << 7) | (from[1] << 6) | (from[2] << 5) |
			(from[3] << 4) | (from[4] << 3) | (from[5] << 2) |
			(from[6] << 1) | (from[7] << 0);
	}
...
static int scanFillD1RgbRow_8S(unsigned char *to, const FillJob *fj,
			       ColorValue *val)
{
	const unsigned char *from;
	int col;
	...
	from = fj->fjScratch;
	for (col = 0; col < fj->fjToWide; to++, from += 8, col += 8) {
		to[0] = (from[7] << 7) | (from[6] << 6) | (from[5] << 5) |
			(from[4] << 4) | (from[3] << 3) | (from[2] << 2) |
			(from[1] << 1) | (from[0] << 0);
	}
...
static int scanFillRow_16_FromRGB(unsigned char *cto, const FillJob *fj,
				  ColorValue *val)
{
	ColorAllocator *ca = fj->fjColorAllocator;
	ColorValue *cor = fj->fjNextCor + 1;
...
static int scanFillD24RgbRow(unsigned char *to, const FillJob *fj,
			     ColorValue *val)
{
	/*
    ColorAllocator *	ca= fj->fjColorAllocator;
...
static int scanFillRow_32_FromRGB(unsigned char *cto, const FillJob *fj,
				  ColorValue *val)
{
	ColorAllocator *ca = fj->fjColorAllocator;
	ColorValue *cor = fj->fjNextCor + 1;
