...
static OfficeCharset PS_OfficeCharsets[] = {
	{
		FONTcharsetANSI,
		/* 0 */ FONTcodepageANSI, /* MUST BE FIRST!! */
		0,
		" Western",
		"CP1252",
	...
	{
		FONTcharsetVIETNAMESE,
		/* 163 */ FONTcodepageVIETNAMESE,
		1,
		" (Vietnamese)",
		"CP1258",
	...
	/* conversion */
	{ FONTcharsetMAC_ROMAN, FONTcodepageMAC_ROMAN, 0, (const char *)0,
	  "MACINTOSH" },

	{ FONTcharsetMAC_256, FONTcodepageMAC_ROMAN, 0, (const char *)0,
	  "MACINTOSH" },

	{
		FONTcharsetGB2312,
		/* 0 */ FONTcodepageGB2312,
		0,
		(const char *)0,
		"GB2312",
		(const int *)0,
...
const OfficeCharset *utilGetOfficeCharsetByCharset(int *pIdx, int charset)
{
	int i;
...
const OfficeCharset *utilGetOfficeCharsetByIndex(int charsetIdx)
{
	int n;
...
const OfficeCharset *utilGetOfficeCharsetBySuffix(int *pIdx,
						  const char *fontname)
{
	int i;
	const OfficeCharset *oc;
	...
	oc = PS_OfficeCharsets;
	for (i = 0; i < sizeof(PS_OfficeCharsets) / sizeof(OfficeCharset);
	     oc++, i++) {
		int lenS;
		...
		if (lenN > lenS && !strcmp(fontname + lenN - lenS,
...
int utilOfficeCharsetCodeForUnicode(int charsetIdx, int unicode)
{
