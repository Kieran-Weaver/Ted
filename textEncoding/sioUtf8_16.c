...
static int sioInUtf16ReadBytes(void *voiduis, unsigned char *buffer,
			       unsigned int count)
{
	...
	if (uis->uisLeftCount > 0) {
		if (count < uis->uisLeftCount) {
			...
			memmove(uis->uisLeft, uis->uisLeft + count,
				uis->uisLeftCount);
			return count;
		} else {
	...
	while (done < count) {
		...
		if (uis->uisSwab) {
			...
			step = uniPutUtf8(uis->uisLeft, 256 * b0 + b1);
		}
		if (step < 1) {
			LDEB(step);
			return -1;
		...
		if (done + step <= count) {
			...
			memmove(uis->uisLeft, uis->uisLeft + count - done,
				left);
...
SimpleInputStream *sioInUtf8_16Open(SimpleInputStream *sisUtf16, int endian)
{
...
static int sioOutUtf16Emit(SimpleOutputStream *sosUtf16, int swab,
...
static int sioOutUtf16WriteBytes(void *voiduos, const unsigned char *buffer,
				 int count)
{
	...
	if (uos->uosLeftCount > 0) {
		...
		step = sioOutUtf16Emit(sosUtf16, uos->uosSwab, uos->uosLeft,
				       got);
		if (step < 0) {
			LDEB(step);
			return -1;
	...
	step = sioOutUtf16Emit(sosUtf16, uos->uosSwab, buffer + done,
			       count - done);
	if (step < 0) {
		LDEB(step);
		return -1;
...
SimpleOutputStream *sioOutUtf8_16Open(SimpleOutputStream *sosUtf16, int endian,
				      int bom)
{
	UtfOutputStream *uos =
		(UtfOutputStream *)malloc(sizeof(UtfOutputStream));
