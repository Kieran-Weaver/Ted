...
static int textConverterConvertBytesFromUtf8(IndexMapping *im,
					     const int unicodes[256],
					     void *through,
					     ...
					     const char *text, int len)
{
	...
	if (utilIndexMappingIsEmpty(im) && unicodes &&
	...
	while (len > 0) {
		...
		if (done >= scratchLen) {
			step = (*produce)(through, produced, (char *)scratch,
					  done);
			if (step < 0) {
				LLLDEB(produced, done, step);
				return -1;
			}
			produced += step;
			done = 0;
...
int textConverterConvertFromUtf8(TextConverter *tc, void *through,
				 int *pConsumed, int produced, const char *text,
				 int len)
{
	if (tc && tc->tcNativeEncodingName && tc->tcNativeEncodingName[0]) {
		if ((iconv_t)tc->tcIconvFrUtf8 == (iconv_t)-1) {
			tc->tcIconvFrUtf8 =
				(struct TextConverterImpl *)iconv_open(
					tc->tcNativeEncodingName, "UTF-8");

			if ((iconv_t)tc->tcIconvFrUtf8 == (iconv_t)-1 &&
			    !strcmp(tc->tcNativeEncodingName, "SYMBOL")) {
				return textConverterConvertBytesFromUtf8(
					&UNI_SymbolToGlyphMapping,
					uniSymbolGlyphUnicodes, through,
					tc->tcProduce, pConsumed, produced,
					text, len);
			}
			...
			if ((iconv_t)tc->tcIconvFrUtf8 == (iconv_t)-1 &&
				...
				return textConverterConvertBytesFromUtf8(
					...
					uniDingbatsGlyphUnicodes, through,
					tc->tcProduce, pConsumed, produced,
					text, len);
#endif
			}
