...
extern BidiNode *bidiNodeAddChild(BidiNode *parent, int embedding, int level,
				  int from, int upto);

extern int bidiTraverseNode(const BidiNode *bn, int from, int upto,
			    BidiGotLevel gotLevel, void *through);

extern void bidiListNode(const BidiNode *bn, const char *symbols);

extern int bidiStretchNode(BidiNode *bn, int upto);
