...
BidiNode *bidiNodeAddChild(BidiNode *parent, int embedding, int level, int from,
			   int upto)
{
	...
	if (level <= parent->bnLevel || level > parent->bnLevel + 2) {
		LLDEB(level, parent->bnLevel);
		return (BidiNode *)0;
	}

	children = (BidiNode **)realloc(parent->bnChildren,
					(parent->bnChildCount + 1) *
						sizeof(BidiNode *));
	if (!children) {
		LXDEB(parent->bnChildCount, children);
		return (BidiNode *)0;
	}
	...
	if (parent->bnChildCount > 0) {
		...
		if (nip > 0 && parent->bnChildren[nip - 1]->bnUpto > from) {
...
int bidiStretchNode(BidiNode *bn, int upto)
{
	while (bn) {
		if (bn->bnUpto < upto) {
			bn->bnUpto = upto;
...
static int bidiTraverseNodeImpl(const BidiNode *bn, const TraverseNode *tn);

static int bidiTraverseChildrenBwd(const BidiNode *bn, const TraverseNode *tn,
				   int fr, int to)
{
	to--;
	...
	while (to >= fr) {
		...
		if (bn->bnChildren[to]->bnLevel == bn->bnLevel + 2) {
			while (tt >= fr &&
			       bn->bnChildren[tt]->bnLevel == bn->bnLevel + 2) {
				tt--;
			}
		}
...
static int bidiTraverseChildrenFwd(const BidiNode *bn, const TraverseNode *tn,
				   int fr, int to)
{
	fr++;
...
static int bidiTraverseNodeImpl(const BidiNode *bn, const TraverseNode *tn)
{
	...
	if (bn->bnParent && (bn->bnEmbedding == BIDIembedLRE ||
	...
	if (forward) {
		...
		for (to = 0; to < bn->bnChildCount; to++) {
			...
			if (child->bnLevel <= bn->bnLevel ||
				...
				LLDEB(bn->bnLevel, child->bnLevel);
				return -1;
			}
			...
			if (child->bnFrom > from) {
				...
				if ((*tn->tnGotLevel)(
					    tn->tnThrough, bn->bnEmbedding,
					    bn->bnLevel, from, child->bnFrom)) {
					LDEB(1);
					return -1;
				}
			}
		...
		if (from < upto) {
			if ((*tn->tnGotLevel)(tn->tnThrough, bn->bnEmbedding,
					      bn->bnLevel, from, upto)) {
				LDEB(1);
				return -1;
			}
		}
...
int bidiTraverseNode(const BidiNode *bn, int from, int upto,
		     BidiGotLevel gotLevel, void *through)
{
	TraverseNode tn;
...
void bidiListNode(const BidiNode *bn, const char *symbols)
{
	int i;
	int from = bn->bnFrom;
	...
	for (i = 0; i < bn->bnChildCount; i++) {
		...
		if (i == 0 || ch->bnFrom > from) {
			appDebug("%3d..%3d/%3d..%3d (%3d@%2d): %*s%s %d", from,
				 ch->bnFrom, bn->bnFrom, bn->bnUpto,
				 (ch->bnFrom - from), bn->bnNumberInParent,
				 3 * bn->bnLevel, "",
				 bidiEmbeddingStr(bn->bnEmbedding),
				 bn->bnLevel);

			if (symbols) {
				appDebug(" \"%.*s\"", ch->bnFrom - from,
					 symbols + from);
			}
	...
	if (i == 0 || from < bn->bnUpto) {
		appDebug("%3d..%3d/%3d..%3d (%3d@%2d): %*s%s %d", from,
			 bn->bnUpto, bn->bnFrom, bn->bnUpto,
			 (bn->bnUpto - from), bn->bnNumberInParent,
			 3 * bn->bnLevel, "", bidiEmbeddingStr(bn->bnEmbedding),
			 bn->bnLevel);

		if (symbols) {
			appDebug(" \"%.*s\"", bn->bnUpto - from,
				 symbols + from);
		}
