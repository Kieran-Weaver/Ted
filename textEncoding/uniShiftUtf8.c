...
int uniShiftUtf8String(char **pShifted, int **pSegments, int *pSegmentCount,
		       int how, const char *sourceString, int len)
{
	int rval = 0;
	...
	case SHIFT_UPPER:
		...
		segmentCount = uniShiftUtf8StringUp(shiftedString, segments,
						    len + 2, sourceString, len);
		if (segmentCount < 1) {
			LDEB(segmentCount);
			rval = -1;
			goto ready;
		...
		*pShifted = shiftedString;
		shiftedString = (char *)0; /* steal */
		if (pSegments) {
			*pSegments = segments;
