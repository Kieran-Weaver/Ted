...
static int sioInIconvReadBytes(void *voidiis, unsigned char *buffer,
			       unsigned int count)
{
	IconvInStream *iis = (IconvInStream *)voidiis;
	...
	while (!tooBig) {
		...
		while (outbytesleft > 0 &&
		       iconv(iis->iisIconv, &inbuf, &(iis->iisInLeft), &outbuf,
			     &outbytesleft) > 0) {
			;
		}
...
SimpleInputStream *sioInIconvOpen(SimpleInputStream *sisIn, const char *tocode,
				  const char *frcode)
{
...
static int sioOutIconvWriteBytes(void *voidios, const unsigned char *buffer,
				 int count)
{
	IconvOutStream *ios = (IconvOutStream *)voidios;
	...
	if (ios->iosInLeft > 0) {
		...
		while (got < count &&
		       iconv(ios->iosIconv, &ib, &ibl, &outbuf,
			     &outbytesleft) == -1 &&
		       errno == EINVAL) {
	...
	while (!incomplete) {
		...
		/*  3  */
		while (iconv(ios->iosIconv, &inbuf, &inbytesleft, &outbuf,
			     &outbytesleft) > 0) {
			;
		}
		...
		if (done > 0) {
			if (sioOutWriteBytes(ios->iosSosOut, out, done) !=
			    done) {
				LDEB(done);
				return -1;
			}
...
SimpleOutputStream *sioOutIconvOpen(SimpleOutputStream *sosOut,
				    const char *tocode, const char *frcode)
{
