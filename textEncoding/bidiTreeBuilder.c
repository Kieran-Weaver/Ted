...
void bidiStartTreeBuilder(BidiTreeBuilder *btb, BidiNode *bottom, int embedding,
			  int level)
{
...
int bidiTreeBuilderAddRun(BidiTreeBuilder *btb, int embedding, int level,
			  int from, int upto)
{
	if (from > upto) {
		LLDEB(from, upto);
	...
	case BIDIembedRLO:
		...
		fresh = bidiNodeAddChild(btb->btbExplicitNode, embedding, level,
					 from, upto);
		if (!fresh) {
