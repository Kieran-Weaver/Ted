...
extern int drawSetForegroundColor(DrawingSurface ds, const RGB8Color *rgb8);
...
extern int drawOpenScreenFont(DrawingSurface ds, const AfmFontInfo *afi,
			      int pixelSize, const IndexSet *unicodesWanted);

extern DrawingSurface drawMakeDrawingSurfaceForParent(DrawingSurface parent,
						      int wide, int high);
...
extern int drawRasterImage(DrawingSurface ds, const DocumentRectangle *drDest,
			   const RasterImage *abi,
			   const DocumentRectangle *drSrc);

extern int drawString(DrawingSurface ds, int x0, int y0, int screenFont,
		      const char *s, int len);

extern void drawSymbol(DrawingSurface ds, int x, int y, int screenFont,
		       int symbol);

extern int drawSetLineAttributes(DrawingSurface ds, int lineWidth,
				 int lineStyle, int capStyle, int joinStyle,
				 const unsigned char *dashList, int dashCount);

extern int drawLine(DrawingSurface ds, int x0, int y0, int x1, int y1);

extern int drawLines(DrawingSurface ds, const Point2DI *points, int count,
		     int close);

extern int drawFillPolygon(DrawingSurface ds, const Point2DI *points,
			   int count);

extern void drawArc(DrawingSurface ds, const Arc2DI *arc);

extern void drawFillArc(DrawingSurface ds, const Arc2DI *arc);

extern int drawGetTextExtents(DocumentRectangle *drText,
			      const DrawingSurface ds, int x0, int y0,
			      int screenFont, const char *s, int len);

extern int drawGetSymbolExtents(DocumentRectangle *drText, DrawingSurface ds,
...
extern int drawGetSuperBaseline(int *pSuperBaseline, const DrawingSurface ds,
				int screenFont, int baselinePixels);

extern void drawSetClipRect(DrawingSurface ds, const DocumentRectangle *drClip);

extern void drawNoClipping(DrawingSurface ds);

extern void drawChildSurface(DrawingSurface ds, const DrawingSurface child,
			     int xDest, int yDest,
