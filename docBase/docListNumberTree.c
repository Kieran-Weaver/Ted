...
static int docClaimListNumberTreeNodes(ListNumberTreeNode **pLntn, int *pCount,
				       int count)
{
	...
	fresh = (ListNumberTreeNode *)realloc(
		*pLntn, count * sizeof(ListNumberTreeNode));
	if (!fresh) {
		LXDEB(count, fresh);
		return -1;
	}
...
{
	if (ls >= lnt->lntTreeCount && docClaimListNumberTrees(lnt, ls + 1)) {
		LDEB(ls);
...
int docListNumberTreeFindParagraph(int *pLevel, ListNumberTreeNode **path,
				   int *nums, ListNumberTreeNode *root,
				   int paraNr)
{
	ListNumberTreeNode *node = root;
	int level = 0;

	if (paraNr < 0) { /*LDEB(paraNr);*/
		return 1;
	}
	...
	while (node->lntnChildCount > 0) {
		...
		/*  3  */
		if (m >= 0 && node->lntnChildren[m]->lntnParaNr == paraNr) {
			nums[level] = m;
			if (path) {
...
int docListNumberTreeGetNumberPath(int *nums, ListNumberTreeNode *root,
				   int ilvl, int paraNr)
{
	int level = 0;
    ...
    */
	...
	if (docListNumberTreeFindParagraph(&level, (ListNumberTreeNode **)0,
					   nums, root, paraNr)) {
...
static int docListNumberTreePrevPath(int *nums, int *pLevel,
				     ListNumberTreeNode **path, int level)
{
	int ilvl = level;
...
/*  2  */
int docListNumberTreeGetPrevPath(int *nums, int *pLevel,
...
static int docListNumberTreeMoveChildren(ListNumberTreeNode *to, int topos,
					 ListNumberTreeNode *fr, int frpos,
					 int cnt)
{
	int i;
	ListNumberTreeNode **fresh;
	...
	appDebug("MOVE %d CHILDREN FROM %d[%d/%d] TO %d[%d/%d]\n", cnt,
		 fr->lntnParaNr, frpos, fr->lntnChildCount, to->lntnParaNr,
		 topos, to->lntnChildCount);
#endif

	/*  1  */
	fresh = realloc(to->lntnChildren, (to->lntnChildCount + cnt) *
	...
	if (fr->lntnChildCount > frpos + cnt) {
		for (i = 0; i < cnt; i++) {
			fr->lntnChildren[frpos + i] =
				fr->lntnChildren[frpos + i + cnt];
		}
	}
...
static ListNumberTreeNode *
docListNumberTreeInsertChild(ListNumberTreeNode *node, int pos, int paraNr)
{
	int i;
	...
	/*  1  */
	fresh = realloc(node->lntnChildren, (node->lntnChildCount +
...
static void docListNumberTreeDeleteChild(ListNumberTreeNode *parent, int pos)
{
	int child;
...
static int docListNumberTreeExtendPath(ListNumberTreeNode **path, int *nums,
				       int paraNr, int level, int ilvl)
{
	ListNumberTreeNode *fresh;
				     ...
				     int paraNr)
{
...
#if LOG_TRANSACTIONS
	SLLDEB("ADD", ilvl, paraNr);
	docListListNumberNode(root);
#endif

	for (res = 0; res <= DLmaxLEVELS; res++) {
		path[res] = (ListNumberTreeNode *)0;
		nums[res] = -2;
	}

	if (ilvl < 0 || ilvl >= DLmaxLEVELS) {
		LLDEB(ilvl, DLmaxLEVELS);
		return -1;
	}
...
#if LOG_TRANSACTIONS
	SLLDEB("PATH", level, ilvl);
	docListNumberTreeLogPath(path, nums, level);
#endif

	/*  2  */
	if (level < ilvl) {
		if (docListNumberTreeExtendPath(path, nums, paraNr, level,
						ilvl)) {
			LLDEB(level, ilvl);
		}
...

	{
		...
		if (pos == 0 && tgt->lntnChildren[pos]->lntnParaNr == -1) {
			...
			if (splitLevel > ilvl) {
				...
				if (docListNumberTreeMoveChildren(
					    fresh, 0, ifr, ifrpos + 1,
					    ifr->lntnChildCount - ifrpos - 1)) {
					LDEB(ifr->lntnChildCount);
					return -1;
				}
		    ...
		    appDebug( "L=%d@%d/%d:P=%d BRANCH\n", lev, lfrpos,
					...
					if (lev < splitLevel ||
					    cfch->lntnChildCount > 0) {
						ListNumberTreeNode *f;

						f = docListNumberTreeInsertChild(
							fresh, 0, -1);
						if (!f) {
							XDEB(f);
							return -1;
					...
					if (docListNumberTreeMoveChildren(
						    ...
						    lfr->lntnChildCount -
							    lfrpos - 1)) {
						LDEB(lfr->lntnChildCount);
						return -1;
					}

					if (fresh->lntnChildCount == 0) {
						LDEB(fresh->lntnChildCount);
						return -1;
					}
				...
				if (cfch->lntnChildCount > 0) {
					if (docListNumberTreeMoveChildren(
						    fresh, 0, cfch, 0,
						    cfch->lntnChildCount)) {
						LDEB(cfch->lntnChildCount);
						return -1;
					}

					if (cfch->lntnParaNr < 0) {
						docListNumberTreeDeleteChild(
							cfr, 0);
					}
				}
				      ...
				      int paraNr)
{
	...
	return docListNumberTreeInsertParagraph(lnt->lntTrees + ls, ilvl,
						paraNr);
}
					     ...
					     ListNumberTreeNode *node)
{
	while (node->lntnChildCount > 0) {
		...
		if (prev->lntnChildCount == 0) {
			if (docListNumberTreeMoveChildren(
				    prev, prev->lntnChildCount, node, 0,
				    node->lntnChildCount)) {
				LDEB(node->lntnChildCount);
				return -1;
			}
...
int docListNumberTreeDeleteParagraph(ListNumberTreeNode *root, int paraNr)
{
	...
	/*  1  */
	if (docListNumberTreeFindParagraph(&level, path, nums, root,
					   paraNr)) { /*LDEB(paraNr);*/
		return -1;
	}
	...
	if (cfch->lntnParaNr != paraNr) {
		LLLDEB(level, cfch->lntnParaNr, paraNr);
		return -1;
	}
	...
	for (lev = level; lev >= 0; lev--) {
		...
		if (lfch->lntnParaNr < 0) {
			if (lfrpos > 0 && lfch->lntnChildCount > 0) {
				ListNumberTreeNode *pfch =
					lfr->lntnChildren[lfrpos - 1];
				if (lfch->lntnChildCount > 0 &&
					...
					if (pfch->lntnChildCount > 0) {
						...
						pfchz = pfch->lntnChildren
								[pfch->lntnChildCount -
								 1];

						if (docListNumberTreeMoveChildren(
							    pfchz,
							    pfchz->lntnChildCount,
							    lfch0, 0,
							    lfch0->lntnChildCount)) {
							LDEB(lfch->lntnChildCount);
							return -1;
						}
				...
				if (docListNumberTreeMoveChildren(
					    pfch, pfch->lntnChildCount, lfch, 0,
					    lfch->lntnChildCount)) {
					LDEB(lfch->lntnChildCount);
					return -1;
				}
	...
	for (;;) {
		if (node->lntnParaNr >= paraFrom) {
			...
			appDebug("SHIFT %2d+ %d -> %2d\n", node->lntnParaNr,
				 paraShift, node->lntnParaNr + paraShift);
#endif
		...
		if (node->lntnChildCount == 0) {
			...
			for (pos = 0; pos < node->lntnChildCount - 1; pos++) {
				/*  3  */
				if (node->lntnChildren[pos + 1]->lntnParaNr <=
				    paraFrom) {
					continue;
				}

				docShiftListNodeReferences(
					node->lntnChildren[pos], paraFrom,
					paraShift);
			}
...
int docListNumberTreeForAll(ListNumberTreeNode *node, LIST_TREE_FUNC forOne,
			    void *through)
{
	int i;
	...
	if (node->lntnParaNr >= 0) {
		if ((*forOne)(node->lntnParaNr, through)) {
			LDEB(node->lntnParaNr);
			return -1;
		}
	}

	for (i = 0; i < node->lntnChildCount; i++) {
		if (docListNumberTreeForAll(node->lntnChildren[i], forOne,
					    through)) {
			LDEB(i);
			return -1;
		}
