				   ...
				   const TextAttribute *taSet)
{
	utilUpdateTextAttribute((PropertyMask *)0, &(ll->llTextAttribute),
				taSetMask, taSet);
	utilPropMaskOr(&(ll->llTextAttributeMask), &(ll->llTextAttributeMask),
		       taSetMask);
	return 0;
}
			     ...
			     const int *rulerMap)
{
	...
	if (from->llLevelNumbers) {
		...
		levelNumbers =
			(LevelNumber *)malloc(count * sizeof(LevelNumber));
		if (!levelNumbers) {
			LPDEB(count, levelNumbers);
			rval = -1;
			goto ready;
		}
	...
	/* steal */
	to->llLevelNumbers = levelNumbers;
	levelNumbers = (LevelNumber *)0;
			 ...
			 int *pLevel, const ListLevel *ll, int field)
{
	...
	if (field < 0 || field > ll->llLevelNumberCount) {
		LLDEB(field, ll->llLevelNumberCount);
		return -1;
	}
...
int docListLevelGetText(char *to, int maxsize, const ListLevel *ll, int field)
{
	int constOff;
	int constLen;
	int constChars;
	...
	if (docListLevelGetField(&constOff, &constLen, &constChars, &level, ll,
				 field)) {
		LDEB(field);
		return -1;
	}
	if (constLen > maxsize) {
		LLDEB(constLen, maxsize);
		return -1;
	}
			...
			int field)
{
	...
	/*  1  */
	if (docListLevelGetField(&constOff, &obytes, &ochars, &level, ll,
				 field)) {
		LDEB(field);
		return -1;
	}
	...
	/*  2,3  */
	if (utilMemoryBufferReplaceBytes(&(ll->llFormatString), constOff,
					 constOff + obytes,
...
int docListLevelSetNumber(int *pChanged, ListLevel *ll, int level, int field)
{
	if (field < 0 || field > ll->llLevelNumberCount) {
		LLDEB(field, ll->llLevelNumberCount);
		return -1;
...
int docListLevelInsertNumber(ListLevel *ll, int level, int field)
{
	LevelNumber *fresh;
	...
	/*  2  */
	fresh = (LevelNumber *)realloc(ll->llLevelNumbers,
				       count * sizeof(LevelNumber));
	if (!fresh) {
		LPDEB(count, fresh);
		return -1;
	}
...
int docListLevelDeleteNumber(ListLevel *ll, int field)
{
	int i;
...
static int docListLevelFormatConst(int *pConstLen, int *pLevel, char *to,
				   int maxbytes, const ListLevel *ll, int field)
{
	int constOff;
	int constLen;
	int constChars;
...
int docListLevelFormatParagraphNumber(char *to, int maxbytes, int *byteOffsets,
				      ...
				      const ListLevel *ll)
{
	...
	for (field = 0; field < ll->llLevelNumberCount; field++) {
		...
		level = -1;
		if (docListLevelFormatConst(&constLen, &level, to,
					    maxbytes - bytesDone, ll, field)) {
			LDEB(field);
			return -1;
		}
		...
		varLen = docListLevelFormatLevelNumber(
			to, maxbytes - bytesDone,
	...
	level = -1;
	if (docListLevelFormatConst(&constLen, &level, to, maxbytes - bytesDone,
				    ll, field)) {
		LDEB(field);
		return -1;
	}
			...
			const TextAttribute *taSet)
{
	...
	if (utilMemoryBufferSetBytes(&(llTo->llFormatString), formatString,
				     level)) {
		LDEB(level);
		rval = -1;
		goto ready;
			     ...
			     const ListLevel *ll)
{
	...
	for (field = 0; field < ll->llLevelNumberCount; field++) {
		...
		if (utilMemoryBufferAppendBytes(mbtext, formatString + constOff,
						constLen)) {
			LDEB(constLen);
			return -1;
		}
...
static int docListLevelConstFromRtfStrings(const char *from, int constChars)
{
	int c;
	int constLen = 0;
			       ...
			       const MemoryBuffer *mbnumbers)
{
	...
	if (utilMemoryBufferSetBytes(&(ll->llFormatString), formatString,
				     constOff)) {
		LDEB(constOff);
		rval = -1;
		goto ready;
