...
int docGetDateField(DateField *dtf, const DocumentField *df)
{
	const FieldInstructions *fi = &(df->dfInstructions);
	const InstructionsComponent *ic;
	int comp;

	if (df->dfKind != DOCfkCREATEDATE && df->dfKind != DOCfkPRINTDATE &&
	    df->dfKind != DOCfkSAVEDATE && df->dfKind != DOCfkDATE &&
	    df->dfKind != DOCfkTIME) {
		return 1;
	}
	...
	for (comp = 1; comp < fi->fiComponentCount; ic++, comp++) {
		if (docComponentIsFlag(fi, comp, 'h')) {
			dtf->dtfCalendar = FIELDcalendarLUNAR;
			continue;
		}
		if (docComponentIsFlag(fi, comp, 's')) {
			dtf->dtfCalendar = FIELDcalendarSAKA;
			continue;
		}
		...
		if (docComponentIsArgFlag(fi, comp, '@')) {
			...
			if (utilCopyMemoryBuffer(&(dtf->dtfPicture),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
