...
int docGetRefField(RefField *rf, const DocumentField *df)
{
	...
	for (comp = 1; comp < fi->fiComponentCount; ic++, comp++) {
		...
		if (docComponentIsArgFlag(fi, comp, 'd')) {
			...
			if (utilCopyMemoryBuffer(&(rf->rfSeparators),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
		...
		if (!ic->icIsFlag) {
			if (utilCopyMemoryBuffer(&(rf->rfBookmark),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
...
int docSetRefField(DocumentField *df, const RefField *rf)
{
