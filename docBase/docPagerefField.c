...
int docGetPagerefField(PagerefField *pf, const DocumentField *df)
{
	...
	for (comp = 1; comp < fi->fiComponentCount; ic++, comp++) {
		...
		if (docComponentIsArgFlag(fi, comp, '*')) {
			...
			if (docFieldComponentNumberFormat(&(pf->pfNumberFormat),
							  ic)) {
				LDEB(comp);
			}
		...
		if (!ic->icIsFlag) {
			if (utilCopyMemoryBuffer(&(pf->pfBookmark),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
...
int docSetPagerefField(DocumentField *df, const PagerefField *pf)
{
