...
int docGetHyperlinkField(HyperlinkField *hf, const DocumentField *df)
{
	...
	for (comp = 1; comp < fi->fiComponentCount; ic++, comp++) {
		if (comp == 1 && !ic->icIsFlag) {
			if (utilCopyMemoryBuffer(&(hf->hfFile),
						 &(ic->icBuffer))) {
				CDEB('o');
				return -1;
			}
		...
		if (docComponentIsArgFlag(fi, comp, 'o')) {
			...
			if (utilCopyMemoryBuffer(&(hf->hfFont),
						 &(ic->icBuffer))) {
				CDEB('o');
				return -1;
			}
		...
		if (docComponentIsArgFlag(fi, comp, 'l')) {
			...
			if (utilCopyMemoryBuffer(&(hf->hfBookmark),
						 &(ic->icBuffer))) {
				CDEB('l');
				return -1;
			}
