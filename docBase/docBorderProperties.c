...
int docBorderThick(int *pSpace, const BorderProperties *bp)
{
	int space;
	int thick;
...
static void docBorderPropertyDifference(PropertyMask *pBpDifMask,
...
int docBordersDiffer(const BorderProperties *bpTo,
		     const BorderProperties *bpFrom, const int *colorMap)
{
	...
	docBorderPropertyDifference(&bpDifMask, bpTo, &bpCmpMask, bpFrom,
				    colorMap);
				...
				ColorPalette *cp)
{
	...
	if (PROPmaskISSET(bpSetMask, BRDRpropCOLOR)) {
		...
		if (ebpFrom->ebpColorExplicit) {
			color = utilPaletteInsertColor(cp, avoidZero, maxColors,
						       &(ebpFrom->ebpColor));
			if (color < 0) {
				LDEB(color);
				return -1;
