...
static const int DocParaBorderProps[] = {
	PPpropTOP_BORDER,   PPpropBOTTOM_BORDER,  PPpropLEFT_BORDER,
	PPpropRIGHT_BORDER, PPpropBETWEEN_BORDER, PPpropBAR_BORDER,
};

static const int DocParaBorderPropCount =
	sizeof(DocParaBorderProps) / sizeof(int);
...
int docUpdParaProperties(PropertyMask *pDoneMask, ParagraphProperties *ppTo,
...
int docGetParaProperty(const ParagraphProperties *pp, int prop)
{
	switch (prop) {
	case PPpropSTYLE:
		return pp->ppStyle;
