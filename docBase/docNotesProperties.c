...
static int docUpdNotesProperties(PropertyMask *pDoneMask, NotesProperties *np,
...
{
	docUpdNotesProperties(pDoneMask, &(fep->fepFootnotesProps), setMask,
			      &(fepSet->fepFootnotesProps),
			      fepPropMap + FEPpropFOOT_STARTNR);

	docUpdNotesProperties(pDoneMask, &(fep->fepEndnotesProps), setMask,
			      &(fepSet->fepEndnotesProps),
...
static int docNotesPropertyDifference(PropertyMask *pDifMask,
				      ...
				      const int fepPropMap[FEPprop_COUNT])
{
	docNotesPropertyDifference(pDifMask, &(fep1->fepFootnotesProps),
				   ...
				   fepPropMap + FEPpropFOOT_STARTNR);
	docNotesPropertyDifference(pDifMask, &(fep1->fepEndnotesProps), cmpMask,
				   &(fep2->fepEndnotesProps),
				   fepPropMap + FEPpropEND_STARTNR);
	return 0;
}
...
const char *docNotesJustificationStr(int pos)
{
	...
	switch (pos) {
	case FTNjustifyBELOW_TEXT:
		return "BELOW_TEXT";
	case FTNjustifyPAGE_BOTTOM:
		return "PAGE_BOTTOM";
...
const char *docNotesPlacementStr(int pos)
{
	...
	switch (pos) {
	case FTNplaceSECT_END:
		return "SECT_END";
	case FTNplaceDOC_END:
		return "DOC_END";
