...
DocumentField *docGetNextField(const ChildFields *rootFields, DocumentField *df)
{
	...
	if (!df) {
		if (rootFields->cfChildCount == 0) {
			...
			return rootFields->cfChildren[0];
		}
	}
	...
	while (df->dfParent) {
		...
		if (df->dfNumberInParent < parentFields->cfChildCount - 1) {
			return parentFields
				->cfChildren[df->dfNumberInParent + 1];
		}
...
DocumentField *docGetPrevField(const ChildFields *rootFields, DocumentField *df)
{
	const ChildFields *parentFields;
	...
	if (!df) {
		if (rootFields->cfChildCount == 0) {
			...
			df = rootFields
				     ->cfChildren[rootFields->cfChildCount - 1];
		}
	} else {
	...
	while (df->dfChildFields.cfChildCount > 0) {
		df = df->dfChildFields
			     .cfChildren[df->dfChildFields.cfChildCount - 1];
	}
