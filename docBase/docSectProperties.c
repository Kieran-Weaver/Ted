...
const int *const DOCsectENDNOTE_PROP_MAP =
	DOCsectNOTE_PROP_MAP + NOTESprop_COUNT;
			 ...
			 const SectionProperties *spSet)
{
	...
	if (PROPmaskISSET(spSetMask, SPpropCOLUMN_COUNT)) {
		if (spTo->spColumnCount != spSet->spColumnCount) {
			if (docSectionPropertiesSetColumnCount(
				    spTo, spSet->spColumnCount)) {
				LDEB(spSet->spColumnCount);
				return -1;
			}
	...
	if (PROPmaskISSET(spSetMask, SPpropCOLUMNS)) {
		...
		if (count > 1) {
			...
			for (col = 0; col < count - 1;
			     scTo++, scFrom++, col++) {
				if (scTo->scColumnWidthTwips !=
				    scFrom->scColumnWidthTwips) {
					scTo->scColumnWidthTwips =
						scFrom->scColumnWidthTwips;
					PROPmaskADD(&doneMask, SPpropCOLUMNS);
				}
				if (scTo->scSpaceToRightTwips !=
				    scFrom->scSpaceToRightTwips) {
					scTo->scSpaceToRightTwips =
						scFrom->scSpaceToRightTwips;
					PROPmaskADD(&doneMask, SPpropCOLUMNS);
				}
			}
	...
	if (PROPmaskISSET(cmpMask, SPpropCOLUMNS)) {
		...
		if (count > 1) {
			...
			for (col = 0; col < count - 1; sc1++, sc2++, col++) {
				if (sc1->scColumnWidthTwips !=
				    sc2->scColumnWidthTwips) {
					PROPmaskADD(&diffMask, SPpropCOLUMNS);
				}
				if (sc1->scSpaceToRightTwips !=
				    sc2->scSpaceToRightTwips) {
					PROPmaskADD(&diffMask, SPpropCOLUMNS);
				}
			}
			if (sc1->scColumnWidthTwips !=
...
int docSectionPropertiesSetColumnCount(SectionProperties *sp, int n)
{
	if (n > 1 && sp->spColumnCount < n) {
		SectionColumn *sc = (SectionColumn *)realloc(
			sp->spColumns, n * sizeof(SectionColumn));
		if (!sc) {
			LXDEB(n, sc);
			return -1;
		}
...
int docSectSetEqualColumnWidth(SectionProperties *sp)
{
	...
	pageWide = dg->dgPageWideTwips - dg->dgLeftMarginTwips -
		   dg->dgRightMarginTwips;
	...
	colWide = (pageWide -
		   (sp->spColumnCount - 1) * sp->spColumnSpacingTwips) /
		  sp->spColumnCount;
		       ...
		       const DocumentGeometry *dgPage, int column)
{
	...
	if (col < column || sp->spColumns[column].scColumnWidthTwips == 0) {
		...
		pageWide = dgPage->dgPageWideTwips - dgPage->dgLeftMarginTwips -
		...
		*pX1 = x0 + colWide;
		...
		if (column == 0) {
			...
			*pXLine =
				x0 -
				sp->spColumns[column - 1].scSpaceToRightTwips /
					2;
		}
...
static int docSectColsSumOthers(int *pVictim, int *pMaxValue,
...
static int docSectReserveWidth(SectionProperties *sp, int victim, int sumValues)
{
	const DocumentGeometry *dg = &(sp->spDocumentGeometry);
	int pageWide;
	...
	if (sumValues > pageWide) {
		...
		narrowed = pageWide - sumValues +
			   sp->spColumns[victim].scColumnWidthTwips;
...
static void docSectGetEqualWidths(int *pColWide, int *pGapWide,
				  int *pMaxColWide, int *pMaxGapWide,
				  const SectionProperties *sp)
{
	const DocumentGeometry *dg = &(sp->spDocumentGeometry);
	int pageWide;
			    ...
			    const SectionProperties *sp, int col)
{
	...
	if (haveFixedWidth) {
		docSectGetEqualWidths((int *)0, &value, (int *)0, &maxValue,
				      sp);
	} else {
		docSectColsSumOthers((int *)0, &maxValue, sp, col,
...
int docSectSetColumnSpacing(SectionProperties *sp, int col, int newValue)
{
	int maxValue;
	int haveFixedWidth;
	...
	if (haveFixedWidth) {
		if (col > 0) {
			LLLDEB(haveFixedWidth, col, newValue);
			return -1;
		}

		docSectGetEqualWidths((int *)0, (int *)0, (int *)0, &maxValue,
				      sp);
		if (newValue < MIN_GAP_WIDE || newValue > maxValue) {
			LLDEB(newValue, maxValue);
			return -1;
		}
		...
		sumOthers = docSectColsSumOthers(
			&victim, &maxValue, sp, col,
			sp->spColumns[col].scColumnWidthTwips);
			  ...
			  const SectionProperties *sp, int col)
{
	...
	if (haveFixedWidth) {
		docSectGetEqualWidths(&value, (int *)0, &maxValue, (int *)0,
				      sp);
	} else {
		docSectColsSumOthers((int *)0, &maxValue, sp, col,
...
int docSectSetColumnWidth(SectionProperties *sp, int col, int newValue)
{
	int maxValue;
	int haveFixedWidth;
	...
	if (haveFixedWidth) {
		...
		docSectGetEqualWidths((int *)0, &gapWide, &maxValue, (int *)0,
				      sp);
		...
		if (gapWide < MIN_GAP_WIDE) {
			LLDEB(gapWide, MIN_GAP_WIDE);
			gapWide = MIN_GAP_WIDE;
		}
