...
int docFieldGetSymbol(SymbolField *sf, const DocumentField *df)
{
	...
	for (comp = 1; comp < fi->fiComponentCount; ic++, comp++) {
		...
		if (docComponentIsArgFlag(fi, comp, 'f')) {
			...
			if (utilCopyMemoryBuffer(&(sf->sfFontName),
						 &(ic->icBuffer))) {
				LDEB(1);
				return -1;
			}
		...
		if (docComponentIsFlag(fi, comp, 'j')) {
			sf->sfEncoding = SYMBOLencSHIFT_JIS;
			continue;
		}
		...
		if (docComponentIsFlag(fi, comp, 'u')) {
			sf->sfEncoding = SYMBOLencUNICODE;
			continue;
		}
