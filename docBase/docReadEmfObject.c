...
int docReadEmfObject(InsertedObject *io, const MemoryBuffer *filename)
{
	EmfHeader eh;
	int rval = 0;
	...
	if (dr->drX1 > dr->drX0) {
		pip->pipTwipsWide =
			(TWIPS_PER_M * (dr->drX1 - dr->drX0)) / 100000;
	} else {
		pip->pipTwipsWide =
			(TWIPS_PER_M * (dr->drX0 - dr->drX1)) / 100000;
	}

	if (dr->drY1 > dr->drY0) {
		pip->pipTwipsHigh =
			(TWIPS_PER_M * (dr->drY1 - dr->drY0)) / 100000;
	} else {
		pip->pipTwipsHigh =
			(TWIPS_PER_M * (dr->drY0 - dr->drY1)) / 100000;
	}
	...
	if (dr->drX1 > dr->drX0) {
		...
		pip->pip_xWinExt = dr->drX0 - dr->drX1;
	}
	...
	sioOutClose(sosMeta);
	sosMeta = (SimpleOutputStream *)0; /*flush,steal*/
	sioOutClose(sosMem);
	sosMem = (SimpleOutputStream *)0; /*flush,steal*/

	io->ioObjectData = mb;
	utilInitMemoryBuffer(&mb); /* steal */
