			     ...
			     const ListOverrideTable *from)
{
	...
	if (to->lotOverrideCount > from->lotOverrideCount) {
		for (i = from->lotOverrideCount; i < to->lotOverrideCount;
		     i++) {
	...
	if (to->lotOverrideCount < from->lotOverrideCount) {
		...
		fresh = (ListOverride *)realloc(to->lotOverrides,
						from->lotOverrideCount *
							sizeof(ListOverride));
		if (!fresh) {
			LXDEB(from->lotOverrideCount, fresh);
			return -1;
		}
		...
		while (to->lotOverrideCount < from->lotOverrideCount) {
			docInitListOverride(
				&(to->lotOverrides[to->lotOverrideCount++]));
		}
	}
...
int docListOverrideTableAddOverride(ListOverride **pLo, ListOverrideTable *lot,
				    int ls, int listId, int listIndex)
{
	if (ls < 0) {
	...
	if (ls >= lot->lotOverrideCount) {
		...
		fresh = (ListOverride *)realloc(
			lot->lotOverrides, (ls + 1) * sizeof(ListOverride));
		if (!fresh) {
			LXDEB(ls, fresh);
			return -1;
		}
...
int docMergeListOverrideIntoTable(ListOverrideTable *lot,
				  const ListOverride *loSet, const int *fontMap,
				  const int *colorMap, const int *rulerMap)
{
	int ls;
	ListOverride *lo;

	if (loSet->loIndex > 0 && loSet->loIndex < lot->lotOverrideCount &&
	    lot->lotOverrides[loSet->loIndex].loIndex == loSet->loIndex &&
	    lot->lotOverrides[loSet->loIndex].loListID == loSet->loListID) {
		return loSet->loIndex;
	}

	ls = docListOverrideTableAddOverride(&lo, lot, -1, loSet->loListID,
					     loSet->loListIndex);
	if (ls < 0) {
		LLDEB(ls, lot->lotOverrideCount);
		return -1;
	}
				    ...
				    const int *rulerMap)
{
	...
	if (ls >= lot->lotOverrideCount) {
		ls = docListOverrideTableAddOverride(
			&lo, lot, ls, loSet->loListID, loSet->loListIndex);
		if (ls < 0) {
			LLDEB(ls, lot->lotOverrideCount);
			return -1;
		}
