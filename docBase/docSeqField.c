...
int docGetSeqField(SeqField *sf, const DocumentField *df)
{
	...
	for (comp = 1; comp < fi->fiComponentCount; ic++, comp++) {
		if (docComponentIsArgFlag(fi, comp, '*')) {
			...
			if (docFieldComponentNumberFormat(&(sf->sfNumberFormat),
							  ic)) {
				LDEB(comp);
			}
		...
		if (!ic->icIsFlag && str == SEQ_Identifier) {
			if (utilCopyMemoryBuffer(&(sf->sfIdentifier),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
			str++;
		...
		if (!ic->icIsFlag && str == SEQ_Bookmark) {
			if (utilCopyMemoryBuffer(&(sf->sfBookmark),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
			str++;
