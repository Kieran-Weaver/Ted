...
static int docInsertFieldAt(DocumentField *parent, ChildFields *cf,
			    DocumentField *df, int pos)
{
	DocumentField **fresh;
	int i;
	...
	fresh = (DocumentField **)realloc(cf->cfChildren,
					  (cf->cfChildCount + 1) *
						  sizeof(DocumentField *));
	if (!fresh) {
		LXDEB(cf->cfChildCount, fresh);
		return -1;
	}
	cf->cfChildren = fresh;
			...
			DocumentField *df)
{
	...
	while (pos >= 0 &&
	       docCompareEditPositions(&(df->dfHeadPosition),
				       &(cf->cfChildren[pos]->dfTailPosition)) <
		       0) {
		pos--;
	}
	pos++;
...
int docDeleteChildField(ChildFields *cf, DocumentField *df)
{
	...
	if (df->dfNumberInParent < 0 ||
		...
		LLLDEB(df->dfFieldNumber, df->dfNumberInParent,
		       cf->cfChildCount);
		return -1;
	}
	...
	if (df->dfChildFields.cfChildCount >= 1) {
		...
		if (step > 0) {
			...
			fresh = (DocumentField **)realloc(
				cf->cfChildren,
				newCount * sizeof(DocumentField *));
			if (!fresh) {
				LXDEB(newCount, fresh);
				return -1;
			}
			cf->cfChildren = fresh;

			for (i = cf->cfChildCount + step - 1; i > pos + step;
			     i--) {
...
DocumentField *docFindTypedChildField(const ChildFields *cf,
				      const EditPosition *ep, int kind)
{
	const int lastOne = 1;
				   ...
				   int lastOne, int kind)
{
	...
	if (lastOne) {
		for (pos = cf->cfChildCount - 1; pos >= 0; pos--) {
			...
			if (docCompareEditPositions(epHead, &(er->erTail)) >
			    0) {
				continue;
			}
			if (docCompareEditPositions(epTail, &(er->erHead)) <
			    0) {
				break;
			}

			if (docCompareEditPositions(epHead, &(er->erHead)) >=
				    0 &&
			    docCompareEditPositions(epTail, &(er->erTail)) <=
				    0 &&
			    (dfC->dfKind == kind || kind < 0)) {
				return dfC;
			}

			df = docFindFieldInRange(er, &(dfC->dfChildFields),
						 lastOne, kind);
			if (df) {
				return df;
			}
...
void docSetFieldTail(DocumentField *dfPa, const EditPosition *epTail)
{
	...
	if (dfPa->dfChildFields.cfChildCount > 0) {
		...
		dfCh = dfPa->dfChildFields
			       .cfChildren[dfPa->dfChildFields.cfChildCount - 1];
...
int docInsertFieldInTree(ChildFields *cf, DocumentField *df)
{
	...
	if (df->dfChildFields.cfChildCount > 0) {
		LDEB(df->dfChildFields.cfChildCount);
		return -1;
	}
...
static int docFieldPath(DocumentField ***pPath, DocumentField *dfTo)
{
	DocumentField *df;
	int deep = 0;
