...
int docCopyListTable(DocumentListTable *to, const DocumentListTable *from)
{
	...
	if (to->dltListCount < from->dltListCount) {
		...
		while (to->dltListCount < from->dltListCount) {
			docInitDocumentList(
				&(to->dltLists[to->dltListCount++]));
		}
	}

	for (i = 0; i < from->dltListCount; i++) {
		if (docCopyDocumentListSameDocument(&(to->dltLists[i]),
						    &(from->dltLists[i]))) {
			LDEB(i);
			return -1;
		}
...
int docDocumentListTableDeleteList(DocumentListTable *dlt, int listIndex)
{
	if (listIndex < 0 || listIndex >= dlt->dltListCount) {
		LLDEB(listIndex, dlt->dltListCount);
		return -1;
	}
...
int docDocumentListTableAddList(DocumentList **pDl, DocumentListTable *dlt,
				const DocumentList *dl, int copyIds,
				const int *fontMap, const int *colorMap,
				const int *rulerMap)
{
	DocumentList *fresh;

	fresh = (DocumentList *)realloc(
		dlt->dltLists, (dlt->dltListCount + 1) * sizeof(DocumentList));
	if (!fresh) {
		LXDEB(dlt->dltListCount, fresh);
		return -1;
	}
	dlt->dltLists = fresh;
	...
	if (dl && docCopyDocumentList(fresh, dl, copyIds, fontMap, colorMap,
				      rulerMap)) {
		LDEB(dlt->dltListCount);
		return -1;
	}
...
int docMergeListIntoTable(DocumentListTable *dlt, const DocumentList *dlNew,
			  const int *fontMap, const int *colorMap,
			  const int *rulerMap)
{
	DocumentList *dl;
	...
	for (i = 0; i < dlt->dltListCount; dl++, i++) {
		...
		for (level = dl->dlLevelCount; level < dlNew->dlLevelCount;
		     level++) {
			if (docDocumentListAddLevel(dl, dlNew->dlLevels + level,
						    copyIds, fontMap, colorMap,
						    rulerMap)) {
				LDEB(level);
				return -1;
			}
	...
	i = docDocumentListTableAddList((DocumentList **)0, dlt, dlNew, copyIds,
					fontMap, colorMap, rulerMap);
	if (i < 0) {
		LLDEB(i, dlt->dltListCount);
		return -1;
	}
