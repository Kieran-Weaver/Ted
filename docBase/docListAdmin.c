	...
	for (li = 0; li < dlt->dltListCount; dl++, li++) {
		...
		if (ov >= lot->lotOverrideCount) {
			...
			ls = docListOverrideTableAddOverride(
				(ListOverride **)0, lot, -1, dl->dlListID, li);
			if (ls < 0) {
				LDEB(ls);
				return -1;
			}
		       ...
		       const int *rulerMap)
{
	...
	for (from = 0; from < lotFrom->lotOverrideCount; lo++, from++) {
		...
		listStyle = docMergeListOverrideIntoTable(lotTo, lo, fontMap,
							  colorMap, rulerMap);
		if (listStyle < 0) {
			LDEB(listStyle);
			return -1;
	...
	for (from = 0; from < dltFrom->dltListCount; dl++, from++) {
		...
		listIndex = docMergeListIntoTable(dltTo,
						  dltFrom->dltLists + from,
						  fontMap, colorMap, rulerMap);
		if (listIndex < 0) {
			LLDEB(from, listIndex);
			return -1;
		}
	}
...
int docGetListOfOverride(ListOverride *lo, const DocumentListTable *dlt)
{
	int li;
	const DocumentList *dl;
...
int docGetListForStyle(ListOverride **pLo, DocumentList **pDl, int ls,
		       const ListAdmin *la)
{
	...
	if (lo->loListIndex < 0 && docGetListOfOverride(lo, dlt) < 0) {
		LLDEB(lo->loListIndex, dlt->dltListCount);
		return -1;
	}
...
int docListGetFormatPath(int *startPath, int *formatPath, const ListLevel **pLl,
			 int ilvl, const DocumentList *dl,
			 const ListOverride *lo)
{
	int level = 0;
	const ListLevel *dllFound = (const ListLevel *)0;

	if (dl->dlLevelCount < 1 || dl->dlLevelCount > DLmaxLEVELS) {
