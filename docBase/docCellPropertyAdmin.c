...
void docInitCellPropertyList(NumberedPropertiesList *cpl)
{
	...
	utilStartNumberedPropertyList(
		...
		CLprop_COUNT, (NumberedPropertiesGetProperty)docGetCellProperty,
...
void docGetCellPropertiesByNumber(CellProperties *cp,
				  const NumberedPropertiesList *cpl, int n)
{
				...
				const NumberedPropertiesList *cplFrom)
{
	...
	if (fromCount > 0) {
		...
		for (n = 0; n < fromCount; n++) {
			...
			vcp = utilPagedListGetItemByNumber(
				&(cplFrom->nplPagedList), n);
			if (!vcp) {
				continue;
			}
