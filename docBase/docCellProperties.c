...
int docUpdCellProperties(PropertyMask *pCpDonePask, CellProperties *cpTo,
			 ...
			 const DocumentAttributeMap *dam)
{
	...
	if (PROPmaskISSET(cpSetMask, CLpropCELLX)) {
		if (cpTo->cpRightBoundaryTwips !=
		    cpFrom->cpRightBoundaryTwips) {
			cpTo->cpRightBoundaryTwips =
				cpFrom->cpRightBoundaryTwips;
	...
	if (PROPmaskISSET(cpSetMask, CLpropCLFTS_WIDTH)) {
		if (cpTo->cpPreferredWidthUnit !=
		    cpFrom->cpPreferredWidthUnit) {
			cpTo->cpPreferredWidthUnit =
				cpFrom->cpPreferredWidthUnit;
...
int docCopyCellProperties(CellProperties *cpTo, const CellProperties *cpFrom,
			  const DocumentAttributeMap *dam)
{
	PropertyMask cpDoneMask;
