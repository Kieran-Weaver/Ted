...
static const int DocRowIntProperties[] = {
	RPpropGAP_WIDTH,    RPpropLEFT_INDENT,	   RPpropHEIGHT,
	RPpropALIGNMENT,    RPpropIS_TABLE_HEADER, RPpropKEEP_ON_ONE_PAGE,
	RPprop_KEEPFOLLOW,  RPpropAUTOFIT,	   RPpropRTOL,
	RPpropTRW_WIDTH,    RPpropTRFTS_WIDTH,	   RPpropTRSPDL,
	RPpropTRSPDR,	    RPpropTRSPDT,	   RPpropTRSPDB,
	RPpropTRSPDFL,	    RPpropTRSPDFR,	   RPpropTRSPDFT,
	RPpropTRSPDFB,	    RPpropTRPADDL,	   RPpropTRPADDR,
	RPpropTRPADDT,	    RPpropTRPADDB,	   RPpropTRPADDFL,
	RPpropTRPADDFR,	    RPpropTRPADDFT,	   RPpropTRPADDFB,
	RPpropTRW_WIDTHB,   RPpropTRW_WIDTHA,	   RPpropTRFTS_WIDTHB,
	RPpropTRFTS_WIDTHA,
};
...
static const int DocRowBorderProperties[] = {
	RPpropTOP_BORDER,   RPpropBOTTOM_BORDER, RPpropLEFT_BORDER,
	RPpropRIGHT_BORDER, RPpropHORIZ_BORDER,	 RPpropVERT_BORDER,
};
...
int docInsertRowColumn(RowProperties *rp, int col, int shiftTail,
		       ...
		       const DocumentAttributeMap *dam)
{
	...
	fresh = (CellProperties *)realloc(
		rp->rpCells, (rp->rpCellCount + 1) * sizeof(CellProperties));
	if (!fresh) {
		LXDEB(rp->rpCellCount, fresh);
		return -1;
	}
	rp->rpCells = fresh;
...
int docColumnWidth(const RowProperties *rp, int col)
{
	...
	if (col == 0) {
		...
		left = rp->rpCells[col - 1].cpRightBoundaryTwips;
	}
...
int docCellRight(int *pColspan, const RowProperties *rp, int col)
{
	...
	if (cp->cpHorizontalMerge == CELLmergeHEAD) {
		...
		for (c = col + 1; c < rp->rpCellCount; c++) {
			if (rp->rpCells[c].cpHorizontalMerge !=
			    CELLmergeFOLLOW) {
				break;
			}
			    ...
			    int shiftTail)
{
	...
	if (col0 + count > rp->rpCellCount) {
		LLLDEB(col0, count, rp->rpCellCount);
		return -1;
	}
...
void docRowMaskToCellMask(PropertyMask *cellMask, const PropertyMask *rowMask)
{
	...
	if (PROPmaskISSET(rowMask, RPpropCELL_LAYOUT)) {
		...
		PROPmaskUNSET(cellMask, CLpropCELLX);
	}
			      ...
			      const DocumentAttributeMap *dam)
{
	...
	if (PROPmaskISSET(rpCmpMask, RPpropCELL_LAYOUT) ||
		...
		cp1 = rp1->rpCells;
		cp2 = rp2->rpCells;
		for (col = 0; col < rp1->rpCellCount && col < rp2->rpCellCount;
		     cp1++, cp2++, col++) {
			...
			docCellPropertyDifference(&cpDifMask, cp1, &cpCmpMask,
						  cp2, dam);
...
int docCopyRowProperties(RowProperties *rpTo, const RowProperties *rpFrom,
			 const DocumentAttributeMap *dam)
{
...
int docUpdRowProperties(PropertyMask *pRpDonePask, RowProperties *rpTo,
	...
	if (updCellLayout || updCellProps) {
		...
		if (updCellLayout) {
			while (rpTo->rpCellCount > rpFrom->rpCellCount) {
				docCleanCellProperties(&(
					rpTo->rpCells[rpTo->rpCellCount - 1]));
				PROPmaskADD(&rpDoneMask, RPpropCELL_LAYOUT);
				if (updCellProps) {
					PROPmaskADD(&rpDoneMask,
						    RPpropCELL_PROPS);
				}
				rpTo->rpCellCount--;
			}
		...
		for (col = 0;
			...
			if (docUpdCellProperties(&cpDoneMask, cpTo, &cpSetMask,
						 cpFrom, dam)) {
				LDEB(1);
				return -1;
			}
