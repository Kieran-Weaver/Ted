...
static int docAddTabToList(TabStopList *tsl)
{
	...
	if (tsl->tslTabStopCount % 10) {
		...
		newSize = tsl->tslTabStopCount + 11;
	}
...
int docCopyTabStopList(TabStopList *to, const TabStopList *from)
{
	...
	if (from->tslTabStopCount > 0) {
		...
		fresh = (TabStop *)malloc(from->tslTabStopCount *
					  sizeof(TabStop));
		if (!fresh) {
			LXDEB(from->tslTabStopCount, fresh);
			return -1;
		}
...
int docRulerMergeTabs(TabStopList *tslOld, int x0GeometryPixels, double xfac,
		      const TabStopList *tslNew)
{
	...
	if (tslNew->tslTabStopCount > 0) {
		...
		fresh = (TabStop *)malloc(tslNew->tslTabStopCount *
					  sizeof(TabStop));
		if (!fresh) {
			LXDEB(tslNew->tslTabStopCount, fresh);
			rval = -1;
			goto ready;
		}
	     ...
	     */
			while (n < tslNew->tslTabStopCount &&
				...
				int oPixels = x0GeometryPixels +
					      COORDtoGRID(xfac, oTwips);
				...
				fresh[n].tsTwips =
					tslOld->tslTabStops[o].tsTwips;
				fresh[n].tsAlignment =
					tslOld->tslTabStops[o].tsAlignment;
				fresh[n].tsLeader =
					tslOld->tslTabStops[o].tsLeader;
