...
extern int docIsTocBookmark(long *pId, const struct DocumentField *df);
...
extern int docBookmarkFromText(struct MemoryBuffer *markName, const char *text,
			       int len);

extern int docBookmarkSuffixIndex(const struct MemoryBuffer *markName,
				  int wanted);
