...
int docAllocateComponents(FieldInstructions *fi, int n)
{
	...
	fresh = (InstructionsComponent *)realloc(
		fi->fiComponents, n * sizeof(InstructionsComponent));
	if (!fresh) {
		LXDEB(n, fresh);
		return -1;
	}
			     ...
			     const FieldInstructions *fiFrom)
{
	...
	if (docAllocateComponents(fiTo, fiFrom->fiComponentCount)) {
		LDEB(fiFrom->fiComponentCount);
		return -1;
	}

	for (i = 0; i < fiTo->fiComponentCount; i++) {
		if (docCopyInstructionsComponent(fiTo->fiComponents + i,
...
static int docSplitFieldInstructions(InstructionsComponent *ic,
				     const char *bytes, int count)
{
	...
	for (;;) {
		...
		if (from[0] == '\\' && offset < count - 1 && from[1] != ' ') {
			if (ic) {
		...
		if (ic) {
			...
			if (unmatchedQuote && utilMemoryBufferAppendString(
						      &(ic->icBuffer), "\"")) {
				LDEB(unmatchedQuote);
				return -1;
			}
