...
int docGetMergeField(MergeField *mf, const DocumentField *df)
{
	...
	for (comp = 1; comp < fi->fiComponentCount; ic++, comp++) {
		...
		if (docComponentIsArgFlag(fi, comp, 'b')) {
			...
			if (utilCopyMemoryBuffer(&(mf->mfTextBefore),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
		...
		if (docComponentIsArgFlag(fi, comp, 'f')) {
			...
			if (utilCopyMemoryBuffer(&(mf->mfTextAfter),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
		...
		if (!ic->icIsFlag) {
			if (utilCopyMemoryBuffer(&(mf->mfFieldName),
						 &(ic->icBuffer))) {
				LDEB(comp);
			}
		...
		if (docComponentIsArgFlag(fi, comp, '*')) {
			...
			if (docFieldComponentCaseShift(&(mf->mfCaseShift),
						       ic)) {
				LDEB(comp);
			}
