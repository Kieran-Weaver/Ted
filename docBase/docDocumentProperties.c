...
const int DOCdocNOTE_PROP_MAP[FEPprop_COUNT] = {
	DPpropFOOTNOTE_STARTNR,	  DPpropFOOTNOTE_JUSTIFICATION,
	DPpropFOOTNOTE_PLACEMENT, DPpropFOOTNOTE_RESTART,
	DPpropFOOTNOTE_STYLE,

	DPpropENDNOTE_STARTNR,	  DPpropENDNOTE_JUSTIFICATION,
	DPpropENDNOTE_PLACEMENT,  DPpropENDNOTE_RESTART,
	DPpropENDNOTE_STYLE,
};
...
static const int DOC_TextProperties[] = {
	DPpropGENERATOR, DPpropTITLE,  DPpropSUBJECT, DPpropKEYWORDS,
	DPpropDOCCOMM,	 DPpropAUTHOR, DPpropCOMPANY, DPpropHLINKBASE,
};

static const int DOC_TextPropertyCount =
	sizeof(DOC_TextProperties) / sizeof(int);
...
static const int DOC_TimePropertyCount =
	sizeof(DOC_TimeProperties) / sizeof(int);

static const int DOC_BorderProperties[] = {
	DPpropTOP_BORDER,   DPpropBOTTOM_BORDER, DPpropLEFT_BORDER,
	DPpropRIGHT_BORDER, DPpropHEAD_BORDER,	 DPpropFOOT_BORDER,
};

static const int DOC_BorderPropertyCount =
	sizeof(DOC_BorderProperties) / sizeof(int);
			      ...
			      const DocumentProperties *from)
{
	...
	if (docUpdDocumentProperties((PropertyMask *)0, to, &dpSetMask, from,
				     dam0)) {
		LDEB(1);
		return -1;
	}
			     ...
			     const MemoryBuffer *filename)
{
	if (filename) {
		...
		utilEmptyMemoryBuffer(&(dp->dpFilename));
	}
...
int docAllocateDocumentColor(DocumentProperties *dp, const RGB8Color *rgb8)
{
	...
	color = utilPaletteInsertColor(dp->dpColorPalette, avoidZero, maxColors,
				       rgb8);
	if (color < 0) {
		LDEB(color);
		return -1;
...
int docGetDocumentProperty(const DocumentProperties *dp, int prop)
{
	...
	case DPpropFOOTNOTE_STYLE:
		return docGetNotesProperty(
			&(dp->dpNotesProps.fepFootnotesProps), NOTESpropSTYLE);
				 ...
				 const char *val, int vallen)
{
	...
	case DPpropGENERATOR:
		return utilMemoryBufferSetBytes(&(dp->dpGeneratorRead),
						...
						vallen);
	case DPpropTITLE:
		return utilMemoryBufferSetBytes(
			&(dp->dpTitle), (const unsigned char *)val, vallen);
	case DPpropSUBJECT:
		return utilMemoryBufferSetBytes(
			&(dp->dpSubject), (const unsigned char *)val, vallen);
	case DPpropKEYWORDS:
		return utilMemoryBufferSetBytes(
			&(dp->dpKeywords), (const unsigned char *)val, vallen);
	case DPpropDOCCOMM:
		return utilMemoryBufferSetBytes(
			&(dp->dpDoccomm), (const unsigned char *)val, vallen);
	case DPpropAUTHOR:
		return utilMemoryBufferSetBytes(
			&(dp->dpAuthor), (const unsigned char *)val, vallen);
	case DPpropCOMPANY:
		return utilMemoryBufferSetBytes(
			&(dp->dpCompany), (const unsigned char *)val, vallen);
	case DPpropHLINKBASE:
		return utilMemoryBufferSetBytes(
			&(dp->dpHlinkbase), (const unsigned char *)val, vallen);
