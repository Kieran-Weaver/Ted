...
#define docHasDocumentInfo(dp)                                         \
	((dp)->dpTitle.mbSize > 0 || (dp)->dpSubject.mbSize > 0 ||     \
	 (dp)->dpKeywords.mbSize > 0 || (dp)->dpDoccomm.mbSize > 0 ||  \
	 (dp)->dpHlinkbase.mbSize > 0 || (dp)->dpAuthor.mbSize > 0 ||  \
	 (dp)->dpCompany.mbSize > 0 || (dp)->dpCreatim.tm_mday != 0 || \
	 (dp)->dpRevtim.tm_mday != 0 || (dp)->dpPrintim.tm_mday != 0)
