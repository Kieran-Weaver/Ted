...
static int docInsertStyleByName(DocumentStyleSheet *dss, DocumentStyle *ds,
				const char *name, int nameLen)
{
	if (utilTreeStoreValue(dss->dssStylesByName, (void **)0,
			       (const char **)0, name, ds)) {
		SDEB(name);
		return -1;
	}

	if (nameLen == 9 && !strncmp(name, "heading ", 8) && name[8] >= '1' &&
	    name[8] <= '9') {
...
static int docInsertStyleIntoNames(DocumentStyleSheet *dss, DocumentStyle *ds)
{
	char *styleNames = ds->dsName;
	int rval = 0;
	...
	while (styleNames && styleNames[0]) {
		...
		if (comma) {
			*comma = '\0';
			...
			if (docInsertStyleByName(dss, ds, styleNames,
						 comma - styleNames)) {
				SDEB(styleNames);
				rval = 1;
			}
			      ...
			      const DocumentAttributeMap *dam)
{
	...
	if (n >= dss->dssStyleCount) {
		...
		fresh = (DocumentStyle **)realloc(
			dss->dssStyles, (n + 1) * sizeof(DocumentStyle *));

		if (!fresh) {
			LLDEB(dss->dssStyleCount, fresh);
			return (DocumentStyle *)0;
		}
		dss->dssStyles = fresh;
	...
	if (dss->dssStyles[n]) {
		...
		dss->dssStyles[n] =
			(DocumentStyle *)malloc(sizeof(DocumentStyle));
		if (!dss->dssStyles[n]) {
			XDEB(dss->dssStyles[n]);
			return (DocumentStyle *)0;
		}
...
void docCleanStyleSheet(DocumentStyleSheet *dss)
{
	...
	if (dss->dssStylesByName) {
		utilTreeFreeTree(dss->dssStylesByName, (UTIL_TREE_CALLBACK)0,
				 (void *)0);
	}
...
{
	if (dss->dssStylesByName) {
		return (DocumentStyle *)utilTreeGetEQ(
			dss->dssStylesByName, (const char **)0, styleName);
	}
