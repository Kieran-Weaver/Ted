...
static int docTocFieldParseLevels(unsigned char *pLevel0,
				  ...
				  const InstructionsComponent *ic)
{
	...
	if (l0 < 0 || l1 < 0 || l0 > PPoutlineDEEPEST ||
	    l1 > PPoutlineDEEPEST || l1 < l0) {
		LLDEB(l0, l1);
		rval = -1;
	}
...
static int docTocFieldParseStyleNames(TocField *tf)
{
	...
	ns /= 2;
	tf->tfStyleNameLevels =
		(StyleNameLevel *)malloc(ns * sizeof(StyleNameLevel));
	if (!tf->tfStyleNameLevels) {
		LXDEB(ns, tf->tfStyleNameLevels);
		return -1;
	}
	...
	while (*from) {
		...
		if (len == 0 || level < 0 || level > PPoutlineBODYTEXT) {
			...
			if (!snl->snlStyleName) {
				XDEB(snl->snlStyleName);
				return -1;
			}
			strncpy(snl->snlStyleName, from, len)[len] = '\0';
			snl->snlLevel = level;
			tf->tfStyleNameLevelCount++;
			snl++;
		}
...
int docFieldGetToc(TocField *tf, const DocumentField *df)
{
	...
	for (comp = 1; comp < fi->fiComponentCount; ic++, comp++) {
		...
		if (docComponentIsArgFlag(fi, comp, 'b')) {
			ic++, comp++;
			if (utilCopyMemoryBuffer(&(tf->tfMarkName),
						 &(ic->icBuffer))) {
				LDEB(1);
				return -1;
			}
		...
		if (docComponentIsArgFlag(fi, comp, 's')) {
			ic++, comp++;
			if (utilCopyMemoryBuffer(&(tf->tfSeqName),
						 &(ic->icBuffer))) {
				LDEB(1);
				return -1;
			}
...
static int docTocFieldAddLevels(FieldInstructions *fi, int flag, int level0,
				int level1)
{
	int rval = 0;
	...
	if (utilMemoryBufferPrintf(&mbScratch, "%d-%d", level0 + 1,
				   level1 + 1) < 0) {
		LDEB(1);
		rval = -1;
		goto ready;
...
int docTocFieldSetTocInstructions(FieldInstructions *fi, const TocField *tf)
{
	int flag;
	...
	case TOCtypeTOC:
		if (tf->tfUseStyleLevels) {
			if (docTocFieldAddLevels(fi, 'o', tf->tfStyleLevel0,
						 tf->tfStyleLevel1)) {
				LDEB(1);
				return -1;
			}
		...
		if (tf->tfUseStyleNames) {
			if (docFieldInstructionsAddArgFlagIfSet(
				    fi, 't', &(tf->tfStyleNamesBytes))) {
				LDEB(1);
				return -1;
			}
		}
		...
		if (tf->tfUseTcEntries) {
			...
			if (!utilMemoryBufferIsEmpty(&(tf->tfIdentifierName)) &&
			    docFieldInstructionsAddComponent(
				    fi, &(tf->tfIdentifierName))) {
				LDEB(1);
				return -1;
			}
	...
	if (tf->tfNLevel0 < PPoutlineBODYTEXT) {
		if (tf->tfNLevel0 > 0 || tf->tfNLevel1 < PPoutlineDEEPEST) {
			if (docTocFieldAddLevels(fi, 'n', tf->tfNLevel0,
						 tf->tfNLevel1)) {
				LDEB(1);
				return -1;
			}
