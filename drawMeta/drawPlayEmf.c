...
static int appEmfReadBmi(bmi *pbmi, SimpleInputStream *sis)
{
	int done = 0;

	pbmi->offBmi = sioEndianGetLeInt32(sis);
	done += 4;
	pbmi->cbBmi = sioEndianGetLeInt32(sis);
	done += 4;
	pbmi->offBits = sioEndianGetLeInt32(sis);
	done += 4;
	pbmi->cbBits = sioEndianGetLeInt32(sis);
	done += 4;
...
static int appEmfGetPolyPoly(SimpleInputStream *sis, const char *recordTypeStr,
			     int recordSize, DocumentRectangle *bounds,
			     int *pPolyCount, int *pPointCount, int **pCounts,
			     DeviceContext *dc)
{
	int polyCount;
	int pointCount;
	...
	polyCount = sioEndianGetLeInt32(sis);
	done += 4;
	pointCount = sioEndianGetLeInt32(sis);
	done += 4;

	WMFDEB(appDebug("%s( polyCount=%d, bounds=[%d..%d x %d..%d] )\n",
			recordTypeStr, polyCount, bounds->drX0, bounds->drX1,
			bounds->drY0, bounds->drY1));

	fresh = (int *)realloc(dc->dcCounts, polyCount * sizeof(int));
	if (!fresh) {
		LXDEB(polyCount, fresh);
		return -1;
	}
	dc->dcCounts = fresh;

	for (i = 0; i < polyCount; i++) {
		dc->dcCounts[i] = sioEndianGetLeInt32(sis);
		done += 4;
...
static int appEmfDrawPolyPolygonXX(SimpleInputStream *sis,
				   const char *recordTypeStr, void *through,
				   int recordSize, DocumentRectangle *bounds,
				   DeviceContext *dc, MetaGetPoints getPoints,
				   int fillInside, int drawBorder,
				   int closePath)
{
	int polyCount;
	int pointCount;
	...
	if (step < 0) {
		LLDEB(pointCount, step);
		return -1;
	}
	done += step;

	if ((*dc->dcDrawPolyPolygon)(dc, through, polyCount, counts,
				     dc->dcPoints, fillInside, drawBorder,
				     closePath)) {
		LDEB(polyCount);
		return -1;
	}
...
static int appEmfReadPolyPoints32(SimpleInputStream *sis,
				  DocumentRectangle *bounds, int *pPointCount,
				  Point2DI **pPoints, DeviceContext *dc)
{
	int done = 0;
	int step;
	int pointCount;
...
static int appEmfReadPen(DeviceContext *dc, int ob, SimpleInputStream *sis)
{
	...
	lp->lpStyle = sioEndianGetLeInt32(sis);
	done += 4;
	x = sioEndianGetLeInt32(sis);
	done += 4;
	y = sioEndianGetLeInt32(sis);
	done += 4;
	lp->lpWidth = (x + y) / 2;
...
static int appEmfReadLogPenEx(DeviceContext *dc, LogicalPen *lp,
			      SimpleInputStream *sis)
{
	int done = 0;
	...
	lp->lpWidth = sioEndianGetLeInt32(sis);
	done += 4;

	(void)/* BrushStyle= */ sioEndianGetLeInt32(sis);
	done += 4;
	...
	(void)/* BrushHatch= */ sioEndianGetLeInt32(sis);
	done += 4;

	NumStyleEntries = sioEndianGetLeInt32(sis);
	done += 4;
...
static int appEmfExtCreatePen(DeviceContext *dc, int ob, int skipped,
			      SimpleInputStream *sis)
{
	int step;
	int done = 0;
...
static int appEmfReadBrush(DeviceContext *dc, int ob, SimpleInputStream *sis)
{
	MetaFileObject *mfo = dc->dcObjects + ob;
	...
	lb->lbStyle = sioEndianGetLeInt32(sis);
	done += 4;

	switch (lb->lbStyle) {
	case BS_SOLID:
		style = "SOLID";
		bmEmfReadRgb8Color(&(lb->lbColor), sis);
		done += 4;
		lb->lbHatch = sioEndianGetLeInt32(sis);
		done += 4;
		break;
	case BS_HOLLOW:
		style = "HOLLOW";
	...
	style = style;
	hatch = hatch; /* use them */

	return done;
}

static int appEmfReadPatternBrush(DeviceContext *dc, int ob,
				  SimpleInputStream *sis, int skipped,
				  int expectBytes)
{
	MetaFileObject *mfo = dc->dcObjects + ob;
	...
	pb->pbUsage = sioEndianGetLeInt32(sis);
	done += 4;

	done += appEmfReadBmi(&bmi, sis);

	while (skipped + done < bmi.offBmi) {
		skip = sioEndianGetLeInt32(sis);
		done += 4;
	}
...
static int appEmfReadUtf16String(char *to, int expectChars,
				 SimpleInputStream *sis)
{
	int nchars;
	int nbytes;
	...
	for (nchars = 0; nchars < expectChars; nchars++) {
		int step;
		int c = sioEndianGetLeUint16(sis);
		done += 2;
...
static int appEmfReadLogFont(DeviceContext *dc, int ob, SimpleInputStream *sis,
			     int expectBytes)
{
	MetaFileObject *mfo = dc->dcObjects + ob;
	...
	lf->lfHeight = sioEndianGetLeInt32(sis);
	done += 4;
	lf->lfWidth = sioEndianGetLeInt32(sis);
	done += 4;
	lf->lfEscapement = sioEndianGetLeInt32(sis);
	done += 4;
	lf->lfOrientation = sioEndianGetLeInt32(sis);
	done += 4;
	lf->lfWeight = sioEndianGetLeInt32(sis);
	done += 4;

	lf->lfItalic = sioInGetByte(sis);
	done += 1;
	lf->lfUnderline = sioInGetByte(sis);
	done += 1;
	lf->lfStrikeOut = sioInGetByte(sis);
	done += 1;
	lf->lfCharSet = sioInGetByte(sis);
	done += 1;
	lf->lfOutPrecision = sioInGetByte(sis);
	done += 1;
	lf->lfClipPrecision = sioInGetByte(sis);
	done += 1;
	lf->lfQuality = sioInGetByte(sis);
	done += 1;
	lf->lfPitchAndFamily = sioInGetByte(sis);
	done += 1;
...
static int appEmfReadLogFontEx(DeviceContext *dc, int ob,
			       SimpleInputStream *sis, int expectBytes)
{
	int done = 0;
	int step;
...
static int appEmfReadDesignVector(DeviceContext *dc, SimpleInputStream *sis)
{
	int done = 0;
	...
	signature = sioEndianGetLeInt32(sis);
	done += 4;
	if (signature != 0x08007664) {
		XXDEB(signature, 0x08007664);
		return -1;
	}

	numAxes = sioEndianGetLeInt32(sis);
	done += 4;
	...
	for (i = 0; i < numAxes; i++) {
		(void)/* axis= */ sioEndianGetLeInt32(sis);
		done += 4;
	}
...
static int appEmfReadLogFontExDv(DeviceContext *dc, int ob,
				 SimpleInputStream *sis, int expectBytes)
{
	int step;
	int done = 0;
...
static int appEmfReadLogFontPanose(DeviceContext *dc, int ob,
				   SimpleInputStream *sis, int expectBytes)
{
	int step;
	int done = 0;
	...
	*/
		...
		(void)/* Version= */ sioEndianGetLeInt32(sis);
		done += 4;
		(void)/* StyleSize= */ sioEndianGetLeInt32(sis);
		done += 4;
		(void)/* Match= */ sioEndianGetLeInt32(sis);
		done += 4;
		(void)/* Reserved= */ sioEndianGetLeInt32(sis);
		done += 4;
		(void)/* VendirId= */ sioEndianGetLeInt32(sis);
		done += 4;
		(void)/* Culture= */ sioEndianGetLeInt32(sis);
		done += 4;

		memset(panose, 0, sizeof(panose));
		for (i = 0; i < 10; i++) {
			panose[i] = sioInGetByte(sis);
			done += 1;
		}
...
static int appEmfSelectStockObject(DeviceContext *dc, void *through,
				   unsigned long arg)
{
	...
	case SYSTEM_FIXED_FONT:
		...
		if (docGetFontByName(&(dc->dcFontList), lf.lfFaceNameUtf8) <
		    0) {
			LDEB(1);
		}
...
static int appEmfReadTextObject(DeviceContext *dc, int skipped,
				SimpleInputStream *sis, long *pX, long *pY)
{
	int step;
	int done = 0;
	...
	refX = sioEndianGetLeInt32(sis);
	done += 4;
	refY = sioEndianGetLeInt32(sis);
	done += 4;

	charCount = sioEndianGetLeInt32(sis);
	done += 4;
	offString = sioEndianGetLeInt32(sis);
	done += 4;
	options = sioEndianGetLeInt32(sis);
	done += 4;
...
static int appWmfReadSmallTextObject(DeviceContext *dc, int skipped,
				     SimpleInputStream *sis, long *pX, long *pY,
				     int *pByteCount)
{
	int step;
	...
	(void)/* iGraphicsMode= */ sioEndianGetLeInt32(sis);
	done += 4;
	(void)/* exScale= */ sioEndianGetLeFloat(sis);
	done += 4;
	(void)/* eyScale= */ sioEndianGetLeFloat(sis);
	done += 4;
...
static int appEmfStretchBlt(DeviceContext *dc, SimpleInputStream *sis,
			    void *through, int skipped, int expectBytes)
{
	int step;
	int done = 0;
	...
	(void)/* BitBltRasterOperation= */ sioEndianGetLeInt32(sis);
	done += 4;

	drSrc.drX0 = sioEndianGetLeInt32(sis);
	done += 4;
	drSrc.drY0 = sioEndianGetLeInt32(sis);
	done += 4;
	...
	(void)/* BkColorSrc= */ sioEndianGetLeInt32(sis);
	done += 4;

	/* What is this? */
	(void)/* skip= */ sioEndianGetLeInt32(sis);
	done += 4;

	done += appEmfReadBmi(&bmi, sis);

	w = sioEndianGetLeInt32(sis);
	done += 4;
	h = sioEndianGetLeInt32(sis);
	done += 4;
	...
	if (appMetaDrawRasterImage(sis, through, expectBytes, dc, &drSrc,
				   &drDest)) {
		LDEB(expectBytes);
		return -1;
	}
...
static void appWinMetaUpdateTransformEmf(DeviceContext *dc)
{
	...
	if (geoAffineTransformForTriangles(
		    &logicalToDevice, t->dctLogicalRect.drX0,
		    t->dctLogicalRect.drY0, t->dctLogicalRect.drX1,
		    t->dctLogicalRect.drY0, t->dctLogicalRect.drX0,
		    t->dctLogicalRect.drY1,
...
static int appEmfGetArc(DocumentRectangle *bounds, Point2DI *pS, Point2DI *pE,
			DeviceContext *dc, SimpleInputStream *sis)
{
	int done = 0;
	...
	s.y = sioEndianGetLeInt32(sis);
	done += 4;
	s.x = sioEndianGetLeInt32(sis);
	done += 4;
	e.y = sioEndianGetLeInt32(sis);
	done += 4;
	e.x = sioEndianGetLeInt32(sis);
	done += 4;
...
int appMetaPlayEmf(DeviceContext *dc, void *through)
{
	...
	for (;;) {
		...
		case EMR_SETWINDOWEXTEX:
			...
			WMFDEB(appDebug(
				"SetWindowExt(yWinExt=%ld,xWinExt=%ld)\n", h,
				w));
		...
		case EMR_SETWINDOWORGEX:
			x0 = sioEndianGetLeInt32(sis);
			done += 4;
			y0 = sioEndianGetLeInt32(sis);
			done += 4;

			WMFDEB(appDebug(
				"SetWindowOrg(yWinOrg=%ld,xWinOrg=%ld)\n", y0,
				x0));
		...
		case EMR_SETVIEWPORTORGEX:
			...
			WMFDEB(appDebug(
				"SetViewportOrg(yViewOrg=%ld,xViewOrg=%ld)\n",
				y0, x0));
		...
		case EMR_SELECTOBJECT:
			...
			if (arg & 0x80000000) {
				...
				if (arg >= dc->dcObjectCount) {
					LLDEB(arg, dc->dcObjectCount);
					return -1;
				}

				if (appWinMetaSelectObject(dc, through, arg)) {
					LLDEB(recordSize, arg);
					return -1;
				}
			}
			goto checkSize;
		...
		case EMR_SETTEXTJUSTIFICATION:
			dc->dcJustificationAmount = sioEndianGetLeInt32(sis);
			done += 4;
			dc->dcJustificationSpaces = sioEndianGetLeInt32(sis);
			done += 4;
		...
		case EMR_RECTANGLE:
			...
			WMFDEB(appDebug("Rectangle(%d..%d,%d..%d)\n",
					bounds.drX0, bounds.drX1, bounds.drY0,
					bounds.drY1));
		...
		case EMR_ELLIPSE:
			...
			WMFDEB(appDebug("Ellipse(%d..%d,%d..%d)\n", bounds.drX0,
					bounds.drX1, bounds.drY0, bounds.drY1));
		...
		case EMR_POLYBEZIER: {
			...
			step = appEmfReadPolyPoints32(sis, &bounds, &count,
						      &points, dc);
			if (step < 0) {
				LDEB(step);
				return -1;
			...
			if ((*dc->dcDrawPolyBezier)(dc, through, count, points,
						    1, dc->dcFillInsides,
						    dc->dcDrawBorders,
						    dc->dcFillInsides)) {
				LDEB(count);
		...
		case EMR_POLYBEZIERTO16: {
			...
			step = appEmfReadPolyPoints16(sis, &bounds, &count,
						      &points, dc);
			if (step < 0) {
				LDEB(step);
				return -1;
		...
		case EMR_POLYLINE: {
			...
			if ((*dc->dcDrawPolyPolygon)(dc, through, 1, &count,
						     points, 0, 1, 0)) {
				LDEB(count);
				return -1;
			}
		...
		case EMR_POLYLINE16: {
			...
			WMFDEB(appDebug(
				...
				count, bounds.drX0, bounds.drX1, bounds.drY0,
				bounds.drY1));
		...
		case EMR_POLYPOLYGON:
			...
			step = appEmfDrawPolyPolygonXX(
				sis, "PolyPolygon", through, recordSize,
		...
		case EMR_POLYPOLYGON16:
			...
			step = appEmfDrawPolyPolygonXX(
				sis, "PolyPolygon16", through, recordSize,
		...
		case EMR_POLYPOLYLINE:
			...
			step = appEmfDrawPolyPolygonXX(
				sis, "PolyPolyLine", through, recordSize,
				&bounds, dc, appWinMetaGetPoints32, 0, 1, 0);
			if (step < 0) {
				LDEB(step);
				return -1;
			}
		...
		case EMR_POLYPOLYLINE16:
			...
			step = appEmfDrawPolyPolygonXX(
				sis, "PolyPolyLine16", through, recordSize,
				&bounds, dc, appWinMetaGetPoints16, 0, 1, 0);
			if (step < 0) {
				LDEB(step);
				return -1;
			}
		...
		case EMR_LINETO:
			x0 = dc->dcX;
			y0 = dc->dcY;
			x1 = sioEndianGetLeInt32(sis);
			done += 4;
			y1 = sioEndianGetLeInt32(sis);
			done += 4;
		...
		case EMR_SETTEXTCOLOR:
			bmEmfReadRgb8Color(&(dc->dcTextColor), sis);
			done += 4;
		...
		case EMR_SETBKCOLOR:
			bmEmfReadRgb8Color(&(dc->dcBkColor), sis);
			done += 4;
		...
		case EMR_CREATEDIBPATTERNBRUSHPT: {
			...
			step = appEmfReadPatternBrush(dc, arg, sis, done,
						      expectBytes);
			if (step < 0) {
				LDEB(step);
				return -1;
		...
		case EMR_EXTCREATEFONTINDIRECTW:
			...
			if (recordSize - done <= 320) {
				expectBytes = recordSize - done;
				step = appEmfReadLogFontPanose(dc, arg, sis,
							       expectBytes);
				if (step < 0) {
					LDEB(step);
					return -1;
				...
				expectBytes = recordSize - done;
				step = appEmfReadLogFontExDv(dc, arg, sis,
							     expectBytes);
				if (step < 0) {
					LDEB(step);
					return -1;
		...
		case EMR_BITBLT: {
			...
			(void)/* UsageSrc= */ sioEndianGetLeInt32(sis);
			done += 4;
			...
			if (appMetaDrawRasterImage(sis, through, expectBytes,
						   dc, &drSrc, &drDest)) {
				LDEB(1);
				return -1;
			}
		...
		case EMR_STRETCHBLT:
			...
			step = appEmfStretchBlt(dc, sis, through, done,
						expectBytes);
			if (step < 0) {
				LDEB(step);
				return -1;
		...
		case EMR_STRETCHDIBITS: {
			...
			drDest.drX0 = sioEndianGetLeInt32(sis);
			done += 4;
			drDest.drY0 = sioEndianGetLeInt32(sis);
			done += 4;
		...
		case EMR_ARC:
			...
			if ((*dc->dcDrawArc)(dc, through, &bounds, s.x, s.y,
					     e.x, e.y, dc->dcFillInsides,
					     dc->dcDrawBorders)) {
				LDEB(1);
				return -1;
			}
		...
		case EMR_PIE:
			...
			if ((*dc->dcDrawPie)(dc, through, &bounds, s.x, s.y,
					     e.x, e.y, dc->dcFillInsides,
					     dc->dcDrawBorders)) {
				LDEB(1);
				return -1;
		...
		case EMR_EXTTEXTOUTW: {
			...
			iGraphicsMode = sioEndianGetLeInt32(sis);
			done += 4;
			exScale = sioEndianGetLeFloat(sis);
			done += 4;
			eyScale = sioEndianGetLeFloat(sis);
			done += 4;
		...
		case EMR_SMALLTEXTOUT: {
			...
			step = appWmfReadSmallTextObject(dc, done, sis, &x0,
							 &y0, &count);
			if (step < 0) {
				LDEB(step);
				return -1;
		...
		case EMR_SETPIXELV: {
			...
			bounds.drX0 = sioEndianGetLeInt32(sis);
			done += 4;
			bounds.drY0 = sioEndianGetLeInt32(sis);
			done += 4;
			...
			WMFDEB(appDebug(
				"SetPixelV(%d,%d,rgba:%d,%d,%d,%d)\n",
				bounds.drX0, bounds.drY0, lb.lbColor.rgb8Red,
				lb.lbColor.rgb8Green, lb.lbColor.rgb8Blue,
				lb.lbColor.rgb8Alpha));
			...
			if (dc->dcCurrentBrushObject >= 0) {
				if (appWinMetaSelectObject(
					    dc, through,
					    dc->dcCurrentBrushObject)) {
					LDEB(dc->dcCurrentBrushObject);
					return -1;
				}
			}
		}
...
skipArguments:
			if (done > recordSize) {
				XLLDEB(recordType, done, recordSize);
				return -1;
			}
