...
static void appMetaSetColorPs(PrintingState *ps, DeviceContext *dc,
...
static void appMetaSetFontPs(PrintingState *ps, DeviceContext *dc,
			     TextAttribute ta, int fontSizeTwips)
{
...
static void appMetaIssueDotsPs(SimpleOutputStream *sos,
			       const unsigned char *dots, int dotCount)
{
	const char *before = "";

	sioOutPrintf(sos, "[");
	while (dotCount-- > 0) {
		sioOutPrintf(sos, "%s%d", before, *(dots++));
		before = " ";
	}
...
static void appMetaSetPenPs(DeviceContext *dc, void *through,
			    const LogicalPen *lp)
{
...
static void appWinMetaPushGraphics(PushedGraphics *pg, const DeviceContext *dc,
				   PrintingState *ps)
{
...
static void appWinMetaPushGraphicsForDraw(PushedGraphics *pg, DeviceContext *dc,
					  PrintingState *ps, int fillInsides,
					  int drawBorders)
{
	if (fillInsides) {
		if (!dc->dcFillPattern) {
...
static void appWinMetaPopGraphics(const PushedGraphics *pg, DeviceContext *dc,
				  PrintingState *ps)
{
...
static int appDrawMetaBitmapImagePs(DeviceContext *dc, void *through,
				    ...
				    const DocumentRectangle *drDest)
{
	...
	if (bmPsPrintBitmapImage(sos, 1, twipsWide, -twipsHigh, drDest->drX0,
				 (drDest->drY0 + twipsHigh), drSrc, onWhite,
				 ps->psUsePostScriptFilters,
				 ps->psUsePostScriptIndexedImages, bd,
				 abi->riBytes)) {
		LDEB(1);
		return -1;
	}
...
static int appCleanObjectPs(DeviceContext *dc, void *through,
			    MetaFileObject *mfo)
{
	return 0;
}

static int appWinMeta_FillPathPs(PrintingState *ps, DeviceContext *dc)
{
	SimpleOutputStream *sos = ps->psSos;
	...
	if (dc->dcFillHatched) {
		...
		det = geoAffineTransformDeterminant2D(
...
static int appMetafilePolygonPathPs(PrintingState *ps, const Point2DI *points,
				    int count, const char *command,
				    const char *psOperator, int closePath)
{
	SimpleOutputStream *sos = ps->psSos;
	...
	for (done = 0; done < count; done++) {
		...
		sioOutPrintf(sos, "%d %d %s%c", points[done].x - x0,
			     points[done].y - y0, command, sep);
	...
	if (closePath) {
		...
		sioOutPrintf(sos, "%d %d %s%c", points[0].x - x0,
			     points[0].y - y0, command, sep);
	}
...
static int appMeta_PolyPolygonPs(DeviceContext *dc, void *through,
				 ...
				 int drawBorders, int closePath)
{
	...
	if (fillInsides) {
		...
		for (poly = 0; poly < polyCount; poly++) {
			appMetafilePolygonPathPs(ps, points + done,
						 pointCounts[poly], command,
						 sep, closePath);
			command = "mt";
...
static int appMeta_PolyBezierPs(DeviceContext *dc, void *through, int count,
				const Point2DI *points, int startPath,
				int fillInsides, int drawBorders, int closePath)
{
	return 0;
}

static int appMetaDrawStringPs(DeviceContext *dc, void *through, int x0, int y0,
			       const MemoryBuffer *text)
{
	...
	wide = psCalculateStringExtents(&drText, s, len, fontSizeTwips,
					withKerning, vswap, afi);
	drText.drX0 += x0;
	drText.drX1 += x0;
	drText.drY0 += y0;
	drText.drY1 += y0;
	...
	case TA_TOP:
		psFontBBox(&drFontBBox, &drFontAscDesc, fontSizeTwips, vswap,
			   afi);
...
static int appMetaPatBltPs(DeviceContext *dc, void *through, long rop,
...
static void appMetaGetArcPs(DeviceContext *dc, int *pW, int *pH, int *pXm,
			    int *pYm, int *pXs, int *pYs, double *pas,
			    double *pae, const DocumentRectangle *dr, int xs,
...
static int appMetaDrawArcPs(DeviceContext *dc, void *through,
			    const DocumentRectangle *dr, int xs, int ys, int xe,
			    int ye, int fillInside, int drawBorder)
{
	PrintingState *ps = (PrintingState *)through;
	double as;
	double ae;
	...
	appMetaGetArcPs(dc, &w, &h, &xm, &ym, &xs, &ys, &as, &ae, dr, xs, ys,
			xe, ye);
...
static int appMetaDrawPiePs(DeviceContext *dc, void *through,
			    const DocumentRectangle *dr, int xs, int ys, int xe,
			    int ye, int fillInside, int drawBorder)
{
	PrintingState *ps = (PrintingState *)through;
	...
	if (drawBorder) {
		...
		sioOutPrintf(ps->psSos, "%d 0 %d %f %f arc ", xm, w / 2, as,
			     ae);
		sioOutPrintf(ps->psSos, "%d  0 lineto ", xm);
...
static int appMetaDrawChordPs(DeviceContext *dc, void *through,
			      ...
			      int xe, int ye, int fillInside, int drawBorder)
{
	...
	if (drawBorder) {
		...
		sioOutPrintf(ps->psSos,
			     "%d 0 %d %f %f arc %d %d lineto stroke\n", xm,
			     w / 2, as, ae, xs, ys);
	}
...
static int appMetaDrawEllipsePs(DeviceContext *dc, void *through,
...
static int appMetaDrawRoundedRectanglePs(DeviceContext *dc, void *through,
...
static int appMetaIgnoreColorPs(DeviceContext *dc, void *through, int r, int g,
				int b)
{
	return 0;
}

static int appMetaSetPolyFillModePs(DeviceContext *dc, void *through, int mode)
{
	return 0;
}
...
static int appMetaSelectPatternBrushObjectPs(DeviceContext *dc, void *through,
					     PatternBrush *pb)
{
	PrintingState *ps = (PrintingState *)through;
	...
	bmPsOpenBitmapPrinter(&bp, sos, &(abi->riDescription), useFilters,
			      indexedImages);

	{
		...
		if (bmPsWriteBitmapData(&bp, &drSel, &(abi->riDescription),
					abi->riBytes)) {
			LDEB(1);
			return -1;
		}
	...
	bmPsWriteImageInstructions(sos, &(abi->riDescription), onWhite,
				   abi->riDescription.bdPixelsWide,
				   abi->riDescription.bdPixelsHigh,
				   "{ /fill-data load } bind", indexedImages);
...
static int appMetaStartPs(DeviceContext *dc, int objectCount,
			  const MetafilePlayer *player,
			  const DocumentRectangle *drLogical, int mapMode)
{
	if (appMetaInitDeviceContext(dc, player, objectCount, drLogical,
				     mapMode, player->mpTwipsWide,
				     player->mpTwipsHigh, player->mpTwipsWide,
				     player->mpTwipsHigh)) {
		LDEB(objectCount);
		return -1;
	}
...
int appMetaPlayWmfPs(PrintingState *ps, const MetafilePlayer *player)
{
	int rval = 0;
...
int appMetaPlayEmfPs(PrintingState *ps, const MetafilePlayer *player)
{
	int rval = 0;
...
int appMacPictPlayFilePs(PrintingState *ps, const MetafilePlayer *player)
{
	MacPictHeader mph;
	int rval = 0;
	...
	if (appMetaStartPs(&dc, handleCount, player, &(mph.mphFrame),
			   MM_ANISOTROPIC)) {
		LDEB(handleCount);
		return -1;
	}
	...
	WMFDEB(appDebug("PS PICT LOGICAL: [%d..%d x %d..%d] TWIPS: [%d x %d]\n",
			mph.mphFrame.drX0, mph.mphFrame.drX1, mph.mphFrame.drY0,
			mph.mphFrame.drY1, player->mpTwipsWide,
			player->mpTwipsHigh));
