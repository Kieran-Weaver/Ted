...
typedef int (*MetaGetPoints)(struct DeviceContext *dc, int count,
			     SimpleInputStream *sis);

/************************************************************************/

typedef int (*MetaDrawRasterImage)(struct DeviceContext *dc, void *through,
				   const RasterImage *abi,
				   const DocumentRectangle *drSrc,
				   const DocumentRectangle *drDest);

typedef int (*MetaCleanObject)(struct DeviceContext *dc, void *through,
			       MetaFileObject *mfo);

typedef int (*MetaSelectPenObject)(struct DeviceContext *dc, void *through,
				   LogicalPen *lp);

typedef int (*MetaSelectBrushObject)(struct DeviceContext *dc, void *through,
				     LogicalBrush *lb);

typedef int (*MetaSelectFontObject)(struct DeviceContext *dc, void *through,
				    LogicalFont *lf);

typedef int (*MetaSelectPatternBrushObject)(struct DeviceContext *dc,
					    void *through, PatternBrush *pb);

typedef int (*MetaDrawPolyPolygon)(struct DeviceContext *dc, void *through,
				   int polyCount, const int *pointCounts,
				   const Point2DI *points, int fillInside,
				   int drawBorder, int closePath);
typedef int (*MetaDrawPolyBezier)(struct DeviceContext *dc, void *through,
				  int pointCounts, const Point2DI *points,
				  int hasStart, int fillInside, int drawBorder,
				  int closePath);

typedef int (*MetaDrawRectangle)(struct DeviceContext *dc, void *through,
				 const DocumentRectangle *dr, int fillInside,
				 int drawBorder);

typedef int (*MetaDrawRoundedRectangle)(struct DeviceContext *dc, void *through,
					const DocumentRectangle *dr, int dx,
					int dy, int fillInside, int drawBorder);

typedef int (*MetaDrawString)(struct DeviceContext *dc, void *through, int x,
			      int y, const MemoryBuffer *text);

typedef int (*MetaPatBlt)(struct DeviceContext *dc, void *through, long rop,
			  const DocumentRectangle *drFill);

typedef int (*MetaDrawArc)(struct DeviceContext *dc, void *through,
			   const DocumentRectangle *drArc, int xs, int ys,
			   int xe, int ye, int fillInside, int drawBorder);

typedef int (*MetaSetColor)(struct DeviceContext *dc, void *through, int r,
			    int g, int b);

typedef int (*MetaSetPolyFillMode)(struct DeviceContext *dc, void *through,
				   int mode);

typedef int (*MetaBeginPath)(struct DeviceContext *dc, void *through);

typedef int (*MetaClosePath)(struct DeviceContext *dc, void *through);

typedef int (*MetaEndPath)(struct DeviceContext *dc, void *through);

typedef int (*MetaStrokePath)(struct DeviceContext *dc, void *through);

typedef int (*MetaFillPath)(struct DeviceContext *dc, void *through);
...
#define DC_xViewport(x, y, dc) \
	(AT2_X((x), (y), &((dc)->dcTransform.dctLogicalToOutput)))
#define DC_yViewport(x, y, dc) \
	(AT2_Y((x), (y), &((dc)->dcTransform.dctLogicalToOutput)))

/*HACK*/
#define DC_wViewport(w, dc) \
	(DC_xViewport((w), 0, (dc)) - DC_xViewport(0, 0, (dc)))
#define DC_hViewport(h, dc) \
	(DC_yViewport(0, (h), (dc)) - DC_yViewport(0, 0, (dc)))
...
extern void appMetaCleanDeviceContext(DeviceContext *dc, void *through);

int appMetaInitDeviceContext(DeviceContext *dc, const MetafilePlayer *player,
			     int objectCount,
			     const DocumentRectangle *drLogical, int mapMode,
			     int outputWide, int outputHigh, int twipsWide,
			     int twipsHigh);

extern void appMetaCleanObject(MetaFileObject *mfo);

extern int appWinMetaRememberFontInList(DeviceContext *dc, TextAttribute *ta,
					const LogicalFont *lf);

extern int appMetaReadBitmapImage(int *pSkip, RasterImage **pAbi,
				  SimpleInputStream *sis, int expectBytes);

extern int appMetaSetTextAlign(DeviceContext *dc, int textAlignment);

extern int appMetaSaveDC(DeviceContext *dc);
extern int appMetaRestoreDC(DeviceContext *dc);

extern int appMetaExcludeClipRect(DeviceContext *dc, int recordSize,
				  SimpleInputStream *sis);

extern int appMetaIntersectClipRect(DeviceContext *dc, int recordSize,
				    SimpleInputStream *sis);

extern int appWinMetaGetPoints16(DeviceContext *dc, int count,
				 SimpleInputStream *sis);
extern int appWinMetaGetPoints32(DeviceContext *dc, int count,
				 SimpleInputStream *sis);

extern int appMetaPlayWmf(DeviceContext *dc, void *through);

extern int appMetaPlayEmf(DeviceContext *dc, void *through);

extern int appMetaDrawRasterImage(SimpleInputStream *sis, void *through,
				  int expectBytes, DeviceContext *dc,
				  const DocumentRectangle *drSrc,
				  const DocumentRectangle *drDest);

extern int appWinMetaSelectObject(DeviceContext *dc, void *through, int ob);

extern int appWinMetaDrawRectangle(DeviceContext *dc, void *through,
				   const DocumentRectangle *dr, int fillInside,
				   int drawBorder);

extern int appWinMetaDrawEllipse(DeviceContext *dc, void *through,
				 const DocumentRectangle *dr, int fillInside,
				 int drawBorder);

extern void appWinMetaTransformRectangle(DocumentRectangle *drOutput,
					 DeviceContext *dc,
					 const DocumentRectangle *drLogical);

extern void appWinMetaUpdateTransform(DeviceContext *dc);

extern int appWinMetaOutputSize(DeviceContext *dc, int logicalSize);

extern int appWinMetaReadUtf16Text(DeviceContext *dc, int expectChars,
				   SimpleInputStream *sis);

extern int appWinMetaReadLegacyText(DeviceContext *dc, int count,
				    int expectBytes, SimpleInputStream *sis);

extern int appMacPictPlayPict(DeviceContext *dc, void *through);
