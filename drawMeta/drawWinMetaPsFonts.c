...
static int appDrawMetaBitmapImagePsList(DeviceContext *dc, void *through,
...
static int appMetaSelectPenObjectPsList(DeviceContext *dc, void *through,
					LogicalPen *lp)
{
	return 0;
}

static int appCleanObjectPsList(DeviceContext *dc, void *through,
				MetaFileObject *mfo)
{
	return 0;
}

static int appMetaSelectBrushObjectPsList(DeviceContext *dc, void *through,
					  LogicalBrush *lb)
{
	return 0;
}

static int appMetaDrawRoundedRectanglePsList(DeviceContext *dc, void *through,
					     const DocumentRectangle *dr,
					     int wide, int high, int fillInside,
					     int drawBorder)
{
	return 0;
}

static int appMeta_PolyPolygonPsList(DeviceContext *dc, void *through,
...
static int appMeta_PolyBezierPsList(DeviceContext *dc, void *through, int count,
...
static int appMetaDrawArcPsList(DeviceContext *dc, void *through,
...
static int appMetaSelectFontObjectPsList(DeviceContext *dc, void *through,
					 LogicalFont *lf)
{
	...
	dc->dcAfi = (*player->mpGetFontForAttribute)(
		&unicodesWanted, &(lf->lfTextAttribute), dfl, psfl);
	if (!dc->dcAfi) {
		XDEB(dc->dcAfi);
		return -1;
	}
	...
	if (psRememberPostsciptFace(lmf->lmfPstl, dc->dcAfi,
				    &(lf->lfTextAttribute), lmf->lmfPrefix,
				    appearsInText)) {
		SDEB(dc->dcAfi->afiFontName);
		return -1;
	}
...
static int appMetaDrawRectanglePsList(DeviceContext *dc, void *through,
				      const DocumentRectangle *dr,
...
static int appMetaDrawStringPsList(DeviceContext *dc, void *through, int x0,
				   int y0, const MemoryBuffer *text)
{
	return 0;
}

static int appMetaPatBltPsList(DeviceContext *dc, void *through, long rop,
			       const DocumentRectangle *dr)
{
	return 0;
}

static int appMetaIgnoreColorPsList(DeviceContext *dc, void *through, int r,
				    int g, int b)
{
	return 0;
}

static int appMetaSetPolyFillModePsList(DeviceContext *dc, void *through,
					int mode)
{
	return 0;
}

static int appMetaStartPsList(DeviceContext *dc, int objectCount,
...
int appWmfListFontsPs(PostScriptTypeList *pstl, const MetafilePlayer *player,
		      const char *prefix)
{
	int rval = 0;
	...
	if (appMetaStartPsList(&dc, wh.wh_objectCount, player, &drLogical,
			       player->mpMapMode)) {
...
int appEmfListFontsPs(PostScriptTypeList *pstl, const MetafilePlayer *player,
		      const char *prefix)
{
	int rval = 0;
	...
	if (appMetaStartPsList(&dc, eh.eh_numOfHandles, player, &(eh.ehBounds),
			       MM_ANISOTROPIC))
	...
	WMFDEB(appDebug(
		...
		eh.ehBounds.drY1, player->mpTwipsWide, player->mpTwipsHigh));
