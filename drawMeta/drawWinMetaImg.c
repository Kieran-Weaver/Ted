...
static int appMetaBitmapImageImg(RasterImage **pAbi, SimpleInputStream *sis,
				 int expectBytes, int srcX, int srcY,
				 int srcXExt, int srcYExt, int dstX, int dstY,
				 int dstXExt, int dstYExt)
{
	int skip;
...
static int appWinMetaPlayRecordsImg(SimpleInputStream *sis, RasterImage **pAbi)
{
	RasterImage *abi = (RasterImage *)0;
	...
	for (;;) {
		...
		case META_STRETCHBLT:
			...
			if (appMetaBitmapImageImg(
				    &abi, sis, 2 * (recordSize - 3 - 2 - 8 * 1),
...
int appMetaPlayWmfImg(BitmapDescription *pBd, unsigned char **pBuffer,
