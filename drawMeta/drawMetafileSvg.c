...
static int appMetaSelectBrushObjectSvg(DeviceContext *dc, void *through,
				       LogicalBrush *lb)
{
...
static int appMetaSvgEmitFill(XmlWriter *xw, int fillInsides,
...
static int appMetaSvgEmitStroke(XmlWriter *xw, int drawBorders,
				DeviceContext *dc)
{
	int widthTwips;
...
static int appDrawMetaBitmapImageSvg(DeviceContext *dc, void *through,
...
static int appCleanObjectSvg(DeviceContext *dc, void *through,
			     MetaFileObject *mfo)
{
	return 0;
}

static int appMeta_PolyPolygonSvg(DeviceContext *dc, void *through,
...
static int appMeta_PolyBezierSvg(DeviceContext *dc, void *through, int count,
...
static int appMetaDrawStringSvg(DeviceContext *dc, void *through, int x0,
				int y0, const MemoryBuffer *text)
{
	...
	wide = psCalculateStringExtents(&drText, s, len, fontSizeTwipsGeo,
	...
	case TA_TOP:
		psFontBBox(&drFontBBox, &drFontAscDesc, fontSizeTwipsGeo, vswap,
			   afi);
...
static int appMetaPatBltSvg(DeviceContext *dc, void *through, long rop,
...
static int appMetaDrawArcSvg(DeviceContext *dc, void *through,
			     ...
			     int xe, int ye, int fillInside, int drawBorder)
{
	...
	sioOutPrintf(xw->xwSos,
		     ...
		     "%d %d %d "
		     "%d,%d",
		     xs, ys, rx, ry, xAxisRotation, largeArcFlag, sweepFlag, xe,
		     ye);
...
static int appMetaDrawPieSvg(DeviceContext *dc, void *through,
			     const DocumentRectangle *dr, int xs, int ys,
			     int xe, int ye, int fillInside, int drawBorder)
{
	SvgWriter *sw = (SvgWriter *)through;
...
static int appMetaDrawChordSvg(DeviceContext *dc, void *through,
			       const DocumentRectangle *dr, int xs, int ys,
			       int xe, int ye, int fillInside, int drawBorder)
{
	SvgWriter *sw = (SvgWriter *)through;
...
static int appMetaDrawEllipseSvg(DeviceContext *dc, void *through,
...
static int appMetaDrawRoundedRectangleSvg(DeviceContext *dc, void *through,
...
static int appMetaIgnoreColorSvg(DeviceContext *dc, void *through, int r, int g,
				 int b)
{
	return 0;
}

static int appMetaSetPolyFillModeSvg(DeviceContext *dc, void *through, int mode)
{
	return 0;
}
...
static int appMetaSelectPatternBrushObjectSvg(DeviceContext *dc, void *through,
					      PatternBrush *pb)
{
	SvgWriter *sw = (SvgWriter *)through;
	XmlWriter *xw = &(sw->swXmlWriter);
	...
	if (pb->pbTileNumber < 0) {
		...
		sprintf(viewBox, "0 0 %d %d", drSrc.drX1 - drSrc.drX0 + 1,
		...
		xmlWriteIntAttribute(xw, "width",
				     drDest.drX1 - drDest.drX0 + 1);
		xmlWriteIntAttribute(xw, "height",
				     drDest.drY1 - drDest.drY0 + 1);
		xmlWriteStringAttribute(xw, "viewBox", viewBox);
		xmlPutString(">", xw);
		xmlNewLine(xw);

		if (appDrawMetaBitmapImageSvg(dc, through, abi, &drSrc,
					      &drSrc)) {
			LDEB(1);
		}
...
static int appMetaStartSvg(DeviceContext *dc, int objectCount,
...
int appMetaPlayWmfSvg(SvgWriter *sw, const MetafilePlayer *player)
{
	int rval = 0;
...
int appMetaPlayEmfSvg(SvgWriter *sw, const MetafilePlayer *player)
{
	int rval = 0;
...
int appMacPictPlayFileSvg(SvgWriter *sw, const MetafilePlayer *player)
{
	MacPictHeader mph;
	int rval = 0;
	...
	if (appMetaStartSvg(&dc, handleCount, player, &(mph.mphFrame),
			    MM_ANISOTROPIC)) {
		LDEB(handleCount);
		return -1;
	}
	...
	WMFDEB(appDebug(
		"SVG PICT LOGICAL: [%d..%d x %d..%d] TWIPS: [%d x %d]\n",
		mph.mphFrame.drX0, mph.mphFrame.drX1, mph.mphFrame.drY0,
		mph.mphFrame.drY1, player->mpTwipsWide, player->mpTwipsHigh));
