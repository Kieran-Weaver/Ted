...
static int appWmfGetCounts(SimpleInputStream *sis, int recordSize,
...
static int appWmfDrawPolyPolygon(SimpleInputStream *sis, void *through,
				 int recordSize, DeviceContext *dc)
{
	int polyCount;
	int pointCount;
	int *pointCounts;

	if (appWmfGetCounts(sis, recordSize, &polyCount, &pointCount,
			    &pointCounts, dc)) {
		LDEB(1);
		return -1;
	}
	...
	if ((*dc->dcDrawPolyPolygon)(dc, through, polyCount, pointCounts,
...
static int appWmfReadCountAndPoints(DeviceContext *dc, int *pCount,
				    SimpleInputStream *sis, int recordSize)
{
	int step;
	int done = 0;
	int count;
...
static int appWmfCreatePenIndirect(DeviceContext *dc, int recordSize,
				   SimpleInputStream *sis)
{
	...
	WMFDEB(appDebug("CreatePenIndirect( style=%d,x,y=%d,%d) ob=%d\n",
			dc->dcObjects[ob].mfoLogicalPen.lpStyle, x, y, ob));
...
static int appWmfCreateBrushIndirect(DeviceContext *dc, int recordSize,
				     SimpleInputStream *sis)
{
	int ob;
	LogicalBrush *lb;
...
static int appWmfCreateFontIndirect(DeviceContext *dc, int bytesLeft,
				    SimpleInputStream *sis)
{
	int ob;
	LogicalFont *lf;
...
static int appWmfReadTextObject(SimpleInputStream *sis, int recordSize,
				int *pX0, int *pY0, int *pStyle,
				DocumentRectangle *bgRect, DeviceContext *dc)
{
	int x0;
	int y0;
	int count;
	...
	if (style) {
		WMFDEB(appDebug("rect=[%d..%d,%d..%d], ", bgRect->drX0,
				bgRect->drX1, bgRect->drY0, bgRect->drY1));
	}
...
static int appWmfTextOut(SimpleInputStream *sis, int recordSize, int *pX0,
			 int *pY0, DeviceContext *dc)
{
	int x0;
	int y0;
	int step;
	...
	WMFDEB(appDebug("TextOut( \"%.*s\", x=%d, y= %d )\n",
			dc->dcCollectedText.mbSize,
			(char *)dc->dcCollectedText.mbBytes, x0, y0));
...
static int appWmfCreatePatternBrush(DeviceContext *dc, int recordSize,
				    SimpleInputStream *sis)
{
	int ob;
	int got;
	...
	pb->pbType = sioEndianGetLeInt16(sis);
	done++;
	pb->pbUsage = sioEndianGetLeInt16(sis);
	done++;
...
static int appMetaScaleViewportExt(DeviceContext *dc, int recordSize,
				   SimpleInputStream *sis)
{
	int yd;
	int yn;
...
static int appMetaScaleWindowExt(DeviceContext *dc, int recordSize,
				 SimpleInputStream *sis)
{
	int yd;
	int yn;
...
static int appWmfCreatePalette(DeviceContext *dc, int recordSize,
			       SimpleInputStream *sis)
{
	int ob;
	int done;
...
int appMetaPlayWmf(DeviceContext *dc, void *through)
{
	...
	for (;;) {
		...
		case META_OFFSETWINDOWORG:
			...
			WMFDEB(appDebug(
				"OffsetWindowOrg(yWinOrg+=%d,xWinOrg+=%d)\n",
				y0, x0));
		...
		case META_SETVIEWPORTORG:
			...
			WMFDEB(appDebug(
				"SetViewportOrg(yViewOrg=%d,xViewOrg=%d)\n", y0,
				x0));
		...
		case META_SETVIEWPORTEXT:
			...
			WMFDEB(appDebug(
				"SetViewportExt(yViewExt=%d,xViewExt=%d)\n", h0,
				w0));
		...
		case META_OFFSETCLIPRGN:
			...
			WMFDEB(appDebug(
				"OffsetClipRgn(yClpOrg+=%d,xClpOrg+=%d)\n", y0,
				x0));
		...
		case META_DIBBITBLT: {
			...
			bytesDone += 4;
			/*  source	*/
			drSrc.drX0 = sioEndianGetLeInt16(sis);
			bytesDone += 2;
			drSrc.drY0 = sioEndianGetLeInt16(sis);
			bytesDone += 2;
			/*  destination	*/
			...
			WMFDEB(appDebug(
				"BitBlt([%d..%d,%d..%d]->[%d..%d,%d..%d],..)\n",
				drSrc.drX0, drSrc.drX1, drSrc.drY0, drSrc.drY1,
				drDest.drX0, drDest.drX1, drDest.drY0,
				drDest.drY1));

			if (appMetaDrawRasterImage(
				    sis, through,
				    2 * (recordSize - 3 - 2 - 6 * 1), dc,
				    &drSrc, &drDest)) {
				LDEB(1);
				return -1;
			}
		...
		case META_CREATEFONTINDIRECT:
			WMFDEB(appDebug("CreateFontIndirect() recordSize=%ld\n",
					recordSize));
			step = appWmfCreateFontIndirect(
				dc, 2 * recordSize - bytesDone, sis);
			if (step < 0) {
				LDEB(step);
				return -1;
			}
		...
		case META_SETBKCOLOR:
			bmWmfGetColor(sis, &(dc->dcBkColor));
			bytesDone += 4;
		...
		case META_SETTEXTCOLOR:
			bmWmfGetColor(sis, &(dc->dcTextColor));
			bytesDone += 4;
		...
		case META_POLYLINE:
			step = appWmfReadCountAndPoints(dc, &count, sis,
							recordSize);
			if (step < 0) {
				LLDEB(recordSize, step);
				return -1;
			}
			bytesDone += step;
		...
		case META_POLYGON:
			...
			if ((*dc->dcDrawPolyPolygon)(
				    dc, through, 1, &count, dc->dcPoints,
		...
		case META_EXTTEXTOUT: {
			...
			step = appWmfReadTextObject(sis, recordSize, &x0, &y0,
						    &style, &bgRect, dc);
			if (step < 0) {
				LDEB(step);
				return -1;
		...
		case META_PIE:
			...
			if ((*dc->dcDrawPie)(dc, through, &bounds, xs, ys, xe,
					     ye, dc->dcFillInsides,
					     dc->dcDrawBorders)) {
				LDEB(1);
				return -1;
		...
		case META_ARC:
			...
			if ((*dc->dcDrawArc)(dc, through, &bounds, xs, ys, xe,
					     ye, dc->dcFillInsides,
					     dc->dcDrawBorders)) {
				LDEB(1);
				return -1;
		...
		case META_CHORD:
			...
			if ((*dc->dcDrawChord)(dc, through, &bounds, xs, ys, xe,
					       ye, dc->dcFillInsides,
					       dc->dcDrawBorders)) {
				LDEB(1);
				return -1;
		...
		case META_ROUNDRECT:
			...
			if ((*dc->dcDrawRoundedRectangle)(
				    dc, through, &bounds, w0, h0,
		...
		case META_SETPIXEL: {
			...
			bounds.drY0 = sioEndianGetLeInt16(sis);
			bytesDone += 2;
			bounds.drX0 = sioEndianGetLeInt16(sis);
			bytesDone += 2;
			bounds.drX1 = bounds.drX0;
			bounds.drY1 = bounds.drY0;

			WMFDEB(appDebug(
				"SetPixel(%d,%d,rgba:%d,%d,%d,%d)\n",
		...
		case META_POLYPOLYGON:
			if (appWmfDrawPolyPolygon(sis, through, recordSize,
						  dc)) {
				LDEB(1);
				return -1;
			}
