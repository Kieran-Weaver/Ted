...
static void appWinMetaRefreshTransform(DeviceContext *dc)
{
	...
	if (geoAffineTransformForTriangles(
		    &(t->dctDeviceToOutput), t->dctDeviceRect.drX0,
		    t->dctDeviceRect.drY0, t->dctDeviceRect.drX1,
		    t->dctDeviceRect.drY0, t->dctDeviceRect.drX0,
		    t->dctDeviceRect.drY1,
			     ...
			     int twipsHigh)
{
	...
	if (objectCount > 0) {
		...
		dc->dcObjects = (MetaFileObject *)malloc(
			objectCount * sizeof(MetaFileObject));
		if (!dc->dcObjects) {
			XDEB(dc->dcObjects);
			return -1;
		}
...
void appMetaCleanDeviceContext(DeviceContext *dc, void *through)
{
	int ob;
...
int appWinMetaRememberFontInList(DeviceContext *dc, TextAttribute *ta,
				 const LogicalFont *lf)
{
	...
	ta->taFontNumber = 0;
	if (lf->lfHeight < 0) {
		ta->taFontSizeHalfPoints = -lf->lfHeight;
	} /* Value not used */
	else {
		ta->taFontSizeHalfPoints = lf->lfHeight;
	} /* Value not used */
	...
	case FF_ROMAN:
		familyStyle = DFstyleFROMAN;
		break;
	case FF_SWISS:
		familyStyle = DFstyleFSWISS;
		break;
	case FF_MODERN:
		familyStyle = DFstyleFMODERN;
		break;
	case FF_SCRIPT:
		familyStyle = DFstyleFSCRIPT;
		break;
	case FF_DECORATIVE:
		familyStyle = DFstyleFDECOR;
		break;
	case 0x60:
		familyStyle = DFstyleFTECH;
		break;
	...
	if (fontNum < 0) {
		SLDEB(lf->lfFaceNameUtf8, fontNum);
		goto ready;
	}
	...
	WMFDEB(appDebug("CreateFontIndirect(h=%d,w=%d,..\"%s\")\n",
			lf->lfHeight, lf->lfWidth, lf->lfFaceNameUtf8));
...
int appMetaSaveDC(DeviceContext *dc)
{
	...
	dct = (DeviceCoordinateTransform *)malloc(
		sizeof(DeviceCoordinateTransform));
	if (!dct) {
		XDEB(dct);
		return -1;
	}
...
int appWinMetaGetPoints16(DeviceContext *dc, int count, SimpleInputStream *sis)
{
	Point2DI *xp;
	int done = 0;
...
int appWinMetaGetPoints32(DeviceContext *dc, int count, SimpleInputStream *sis)
{
	Point2DI *xp;
	int done = 0;
...
int appMetaExcludeClipRect(DeviceContext *dc, int recordSize,
...
int appMetaIntersectClipRect(DeviceContext *dc, int recordSize,
			     SimpleInputStream *sis)
{
	DocumentRectangle dr;
...
int appMetaSetTextAlign(DeviceContext *dc, int arg)
{
	...
	case TA_NOUPDATECP:
		WMFDEB(appDebug(" NOUPDATECP"));
		break;
	case TA_UPDATECP:
		WMFDEB(appDebug(" UPDATECP"));
		break;
	default:
	...
	case TA_LEFT:
		WMFDEB(appDebug(" LEFT"));
		break;
	case TA_RIGHT:
		WMFDEB(appDebug(" RIGHT"));
		break;
	case TA_CENTER:
		WMFDEB(appDebug(" CENTER"));
		break;
	default:
	...
	case TA_TOP:
		...
		break;
	case TA_BOTTOM:
		WMFDEB(appDebug(" BOTTOM"));
		break;
	case TA_BASELINE:
		WMFDEB(appDebug(" BASELINE"));
		break;
	default:
...
int appMetaReadBitmapImage(int *pSkip, RasterImage **pRi,
...
int appMetaDrawRasterImage(SimpleInputStream *sis, void *through,
			   ...
			   const DocumentRectangle *drDest)
{
	...
	if (drSrcAdjusted.drX0 < 0) {
		drSrcAdjusted.drX0 = 0;
		adjusted = 1;
	}
	if (drSrcAdjusted.drY0 < 0) {
		drSrcAdjusted.drY0 = 0;
		adjusted = 1;
	}

	if (drSrcAdjusted.drX1 > bd->bdPixelsWide - 1) {
		drSrcAdjusted.drX1 = bd->bdPixelsWide - 1;
		adjusted = 1;
	}
	if (drSrcAdjusted.drY1 > bd->bdPixelsHigh - 1) {
		drSrcAdjusted.drY1 = bd->bdPixelsHigh - 1;
		adjusted = 1;
	}
	...
	if (drSrcAdjusted.drX0 > drSrcAdjusted.drX1) {
		LLDEB(drSrcAdjusted.drX0, drSrcAdjusted.drX1);
		rval = -1;
		goto ready;
	}
	if (drSrcAdjusted.drY0 > drSrcAdjusted.drY1) {
		LLDEB(drSrcAdjusted.drY0, drSrcAdjusted.drY1);
		rval = -1;
		goto ready;
	}

	rval = (*dc->dcDrawRasterImage)(dc, through, abi, &drSrcAdjusted,
					&drOutput);
	if (rval) {
		LDEB(rval);
	}
...
int appWinMetaSelectObject(DeviceContext *dc, void *through, int ob)
{
	...
	case MFtypePEN: {
		...
		WMFDEB(appDebug(
			...
			ob, lp->lpWidth, lp->lpColor.rgb8Red,
			lp->lpColor.rgb8Green, lp->lpColor.rgb8Blue,
			lp->lpColor.rgb8Alpha));
	...
	case MFtypeBRUSH:
		...
		if ((*dc->dcSelectBrushObject)(dc, through,
					       &(mfo->mfoLogicalBrush))) {
			LDEB(ob);
			return -1;
		}
	...
	case MFtypePATTERNBRUSH:
		...
		if ((*dc->dcSelectPatternBrushObject)(
			    dc, through, &(mfo->mfoPatternBrush))) {
			LDEB(ob);
			return -1;
		}
		dc->dcCurrentBrushObject = ob;
	...
	case MFtypeFONT:
		...
		if ((*dc->dcSelectFontObject)(dc, through,
					      &(mfo->mfoLogicalFont))) {
			LDEB(ob);
			return -1;
		}
...
void appWinMetaTransformRectangle(DocumentRectangle *drOutput,
				  DeviceContext *dc,
...
int appWinMetaDrawRectangle(DeviceContext *dc, void *through,
			    const DocumentRectangle *drLogical, int fillInside,
			    int drawBorder)
{
	DocumentRectangle drOutput;
	Point2DI points[4];
...
int appWinMetaDrawEllipse(DeviceContext *dc, void *through,
			  const DocumentRectangle *drLogical, int fillInside,
			  int drawBorder)
{
	DocumentRectangle drOutput;
	...
	if ((*dc->dcDrawEllipse)(dc, through, &drOutput, fillInside,
				 drawBorder)) {
		LDEB(1);
		return -1;
	}
...
void appWinMetaUpdateTransform(DeviceContext *dc)
{
	...
	if (geoAffineTransformForTriangles(
		    &(t->dctLogicalToOutput), t->dctLogicalRect.drX0,
		    t->dctLogicalRect.drY0, t->dctLogicalRect.drX1,
		    t->dctLogicalRect.drY0, t->dctLogicalRect.drX0,
		    t->dctLogicalRect.drY1,
...
int appWinMetaOutputSize(DeviceContext *dc, int logicalSize)
{
