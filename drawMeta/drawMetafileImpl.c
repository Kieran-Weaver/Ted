...
int appMetaSelectPenObjectImpl(DeviceContext *dc, void *through, LogicalPen *lp)
{
...
int appMetaSelectBrushObjectImpl(DeviceContext *dc, void *through,
				 LogicalBrush *lb)
{
...
int appMetaSelectFontObjectImpl(DeviceContext *dc, void *through,
				LogicalFont *lf)
{
	...
	dc->dcAfi = (*player->mpGetFontForAttribute)(
		&unicodesWanted, &(lf->lfTextAttribute), dfl,
...
int appMetaGetPenWidth(DeviceContext *dc, const LogicalPen *lp)
{
...
int appMetaGetPenDots(DeviceContext *dc, unsigned char dots[6],
		      const LogicalPen *lp)
{
	int outputWidth;
	int dash;
