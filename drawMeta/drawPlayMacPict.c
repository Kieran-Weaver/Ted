...
static const char *appMacPictCommentName(int n)
{
	...
	switch (n) {
	case PICTCMT_Proprietary:
		return "Proprietary";
	case PICTCMT_MacDrawBegin:
		return "MacDrawBegin";
	case PICTCMT_MacDrawEnd:
		return "MacDrawEnd";
	case PICTCMT_GroupedBegin:
		return "GroupedBegin";
	case PICTCMT_GroupedEnd:
		return "GroupedEnd";
	case PICTCMT_BitmapBegin:
		return "BitmapBegin";
	case PICTCMT_BitmapEnd:
		return "BitmapEnd";
	case PICTCMT_ArrowAtEnd:
		return "ArrowAtEnd";
	case PICTCMT_ArrowAtStart:
		return "ArrowAtStart";
	case PICTCMT_ArrowBothEnds:
		return "ArrowBothEnds";
	case PICTCMT_ArrowEnd:
		return "ArrowEnd";
	case PICTCMT_SetGray:
		return "SetGray";
	case PICTCMT_TextBegin:
		return "TextBegin";
	case PICTCMT_TextEnd:
		return "TextEnd";
	case PICTCMT_StringBegin:
		return "StringBegin";
	case PICTCMT_StringEnd:
		return "StringEnd";
	case PICTCMT_TextCenter:
		return "TextCenter";
	case PICTCMT_LineLayoutOff:
		return "LineLayoutOff";
	case PICTCMT_LineLayoutOn:
		return "LineLayoutOn";
	case PICTCMT_ClientLineLayout:
		return "ClientLineLayout";
	case PICTCMT_PolyBegin:
		return "PolyBegin";
	case PICTCMT_PolyEnd:
		return "PolyEnd";
	case PICTCMT_PolyIgnore:
		return "PolyIgnore";
	case PICTCMT_PolySmooth:
		return "PolySmooth";
	case PICTCMT_PolyClose:
		return "PolyClose";
	case PICTCMT_RotateBegin:
		return "RotateBegin";
	case PICTCMT_RotateEnd:
		return "RotateEnd";
	case PICTCMT_RotateCenter:
		return "RotateCenter";
	...
	case PICTCMT_SetLineWidth:
		return "SetLineWidth";
	case PICTCMT_PostScriptBegin:
		return "PostScriptBegin";
	case PICTCMT_PostScriptEnd:
		return "PostScriptEnd";
	case PICTCMT_PostScriptHandle:
		return "PostScriptHandle";
	case PICTCMT_PostScriptFile:
		return "PostScriptFile";
	case PICTCMT_TextIsPostScript:
		return "TextIsPostScript";
	case PICTCMT_ResourcePS:
		return "ResourcePS";
	case PICTCMT_PSBeginNoSave:
		return "PSBeginNoSave";
	case PICTCMT_FormsPrinting:
		return "FormsPrinting";
	case PICTCMT_EndFormsPrinting:
		return "EndFormsPrinting";
	case PICTCMT_CMBeginProfile:
		return "CMBeginProfile";
	case PICTCMT_CMEndProfile:
		return "CMEndProfile";
	case PICTCMT_CMEnableMatching:
		return "CMEnableMatching";
	case PICTCMT_CMDisableMatching:
...
static int appMacPictGetRect(DocumentRectangle *dr, SimpleInputStream *sis)
{
	int done = 0;

	dr->drY0 = sioEndianGetBeInt16(sis);
	done += 2;
	dr->drX0 = sioEndianGetBeInt16(sis);
	done += 2;
	dr->drY1 = sioEndianGetBeInt16(sis);
	done += 2;
	dr->drX1 = sioEndianGetBeInt16(sis);
	done += 2;
...
static int appMacPictGetOvSize(DeviceContext *dc, SimpleInputStream *sis)
{
...
static int appMacPictGetArc(DeviceContext *dc, SimpleInputStream *sis)
{
	double as;
	double ae;
	int x;
...
static int appMacPictGetColor(RGB8Color *rgb8, SimpleInputStream *sis)
{
...
static int appMacPictGetPattern(DeviceContext *dc, SimpleInputStream *sis,
				RGB8Color *pSolidColor, int *pIsSolid,
...
static int appMacPictGetShortLine(int *pX0, int *pY0, int *pX1, int *pY1,
...
static int appMacPictGetShortLineFrom(int *pX0, int *pY0, int *pX1, int *pY1,
				      DeviceContext *dc, SimpleInputStream *sis)
{
	int y0 = dc->dcY;
	int x0 = dc->dcX;
...
static int appMacPictGetLineFrom(int *pX0, int *pY0, int *pX1, int *pY1,
				 DeviceContext *dc, SimpleInputStream *sis)
{
	int y0 = sioEndianGetBeInt16(sis);
	int x0 = sioEndianGetBeInt16(sis);
...
static int appMacPictGetLine(int *pX0, int *pY0, int *pX1, int *pY1,
			     DeviceContext *dc, SimpleInputStream *sis)
{
	int y0 = sioEndianGetBeInt16(sis);
	int x0 = sioEndianGetBeInt16(sis);
...
static int appMacPictAddFont(DeviceContext *dc, const char *name)
{
	int rval = 0;
...
static int appMacPictGetFontName(DeviceContext *dc, SimpleInputStream *sis)
{
	int rval = 0;
...
static int appMacPictGetTxFont(DeviceContext *dc, SimpleInputStream *sis)
{
	int rval = 0;
...
static int appMacPictGetTxFace(DeviceContext *dc, SimpleInputStream *sis)
{
...
static int appMacPictGetGlyphState(DeviceContext *dc, SimpleInputStream *sis)
{
	int dataLength = sioEndianGetBeInt16(sis);
	int i;
	...
	PICTDEB(appDebug("GlyphState( %d:%x,%x,%x,%x )\n", dataLength,
			 outlinePreferred, preserveGlyph, fractionalWidth,
			 scalingDisabled));
...
static int appMacPictGetString(DeviceContext *dc, SimpleInputStream *sis)
{
	int done;
	...
	for (done = 0; done < count; nbytes++, done++) {
		...
		if (c == '\r') {
			...
			unicode = uniMacRomanGlyphUnicodes[c];
		}
...
static int appMacPictGetDHText(DeviceContext *dc, int *pDh,
			       SimpleInputStream *sis)
{
	...
	PICTDEB(appDebug("DHText( %d, %d: \"%.*s\" )\n", dh,
			 dc->dcCollectedText.mbSize, dc->dcCollectedText.mbSize,
...
static int appMacPictGetDVText(DeviceContext *dc, int *pDv,
			       SimpleInputStream *sis)
{
...
static int appMacPictGetDHDVText(DeviceContext *dc, int *pDh, int *pDv,
				 SimpleInputStream *sis)
{
	int dh = sioInGetByte(sis);
	int dv = sioInGetByte(sis);
...
static int appMacPictGetLongText(DeviceContext *dc, SimpleInputStream *sis)
{
...
static int appMacPictGetPoly(DeviceContext *dc, SimpleInputStream *sis)
{
	int count;
	int done = 0;
	int step;
	...
	for (i = 0; i < count; xp++, i++) {
		...
		y = sioEndianGetBeInt16(sis);
		done += 2;
		x = sioEndianGetBeInt16(sis);
		done += 2;
...
static int appMacPictGetLongComment(DeviceContext *dc, SimpleInputStream *sis)
{
	int i;
	...
	PICTDEB(appDebug("LongComment( %d:%s, %d, .. )\n", kind,
			 appMacPictCommentName(kind), size));
	...
	case PICTCMT_TextBegin: {
		...
		PICTDEB(appDebug(
			...
			appMacPictCommentName(kind), justify, flip, angle,
			angleFixed));
	} break;

	case PICTCMT_DashedLine:
		dc->dcObjects[MACPICThandleFRAME_PEN].mfoLogicalPen.lpStyle =
			PS_DASH;
		for (i = 0; i < size; i++) {
	...
	case PICTCMT_SetLineWidth:
		...
		}
		{
			...
			PICTDEB(appDebug(
				"LongComment( %d:%s, %d bytes, %d/%d )\n", kind,
				appMacPictCommentName(kind), size, x, y));
		}
		break;
...
static int appMacPictGetTxSize(DeviceContext *dc, SimpleInputStream *sis)
{
	int size = sioEndianGetBeInt16(sis);
...
static int appMacPictDrawString(DeviceContext *dc, int dh, int dv,
				void *through)
{
...
static int appMacPictDrawImage(DeviceContext *dc, void *through,
			       SimpleInputStream *sis, int packed, int direct,
			       int clipRegion)
{
	int rval = 0;
	int step;
	...
	/*  2  */
	step = bmMacPictGetImage(&abi, &drSrc, &drDest, sis, packed, direct,
				 clipRegion);
	if (step < 0) {
		LDEB(step);
		rval = -1;
...
static int appMacPictDrawLine(DeviceContext *dc, void *through, int x0, int y0,
			      int x1, int y1)
{
	int count = 2;
	Point2DI points[2];
	...
	if ((*dc->dcDrawPolyPolygon)(dc, through, 1, &count, points, 0, 1, 0)) {
		LDEB(count);
		return -1;
	}
...
static int appMacPictSetPatternBrush(DeviceContext *dc, PatternBrush *pb,
				     const unsigned char *pattern)
{
	...
	if (foreBlack && backWhite) {
		...
		if (foreWhite && backBlack) {
			pb->pbAbi->riDescription.bdSamplesPerPixel = 1;
			pb->pbAbi->riDescription.bdColorEncoding =
				BMcoWHITEBLACK;
		} else {
			ColorPalette *cp =
				&(pb->pbAbi->riDescription.bdPalette);
...
static int appMacPictSetPattern(DeviceContext *dc, void *through,
				int brushHandle, int penHandle, int solid,
				...
				const unsigned char *pattern)
{
	...
	if (solid) {
		if (brushHandle != MACPICThandle_NONE) {
			...
			mfo->mfoLogicalBrush.lbTilePixmap =
				(struct DrawingSurface *)0;
		...
		if (brushHandle != MACPICThandle_NONE) {
			...
			mfo->mfoPatternBrush.pbTilePixmap =
				(struct DrawingSurface *)0;
			mfo->mfoPatternBrush.pbTileNumber = -1;
			if (appMacPictSetPatternBrush(
				    dc, &(mfo->mfoPatternBrush),
...
static int appMacPictRefreshDrawingGear(DeviceContext *dc, void *through)
{
	if (appMacPictSetPattern(
		    dc, through, MACPICThandlePAINT_BRUSH,
		    MACPICThandleFRAME_PEN, dc->dcMacPictPenIsSolid,
		    &(dc->dcMacPictPenSolid), dc->dcMacPictPenPattern)) {
		LDEB(1);
		return -1;
	}

	if (appMacPictSetPattern(dc, through, MACPICThandleFILL_BRUSH,
				 MACPICThandle_NONE, dc->dcMacPictFillIsSolid,
	...
	if (appMacPictSetPattern(dc, through, MACPICThandleERASE_BRUSH,
				 MACPICThandle_NONE, dc->dcMacPictBackIsSolid,
...
static int appMacPictChooseGear(DeviceContext *dc, void *through,
				int brushHandle, int penHandle)
{
	if (brushHandle != MACPICThandle_NONE) {
...
int appMacPictPlayPict(DeviceContext *dc, void *through)
{
	...
	{
		MetaFileObject *mfo =
			&(dc->dcObjects[MACPICThandleDUMMY_BRUSH]);
	...
	for (;;) {
		...
		case MACPICT_PnSize: {
			...
			dc->dcObjects[MACPICThandleFRAME_PEN]
				.mfoLogicalPen.lpWidth = (w + h) / 2;
		}
			continue;
		...
		case MACPICT_Clip: {
			...
			for (skip = 2; skip < regionBytes; skip += 2) {
				(void)/* regionCoord= */ sioEndianGetBeInt16(
					sis);
			}
		}
			continue;
		...
		case MACPICT_RGBFgCol:
			if (appMacPictGetColor(&(dc->dcMacPictForeColor),
					       sis)) {
				LDEB(1);
				rval = -1;
				goto ready;
		...
		case MACPICT_RGBBkCol:
			if (appMacPictGetColor(&(dc->dcMacPictBackColor),
					       sis)) {
				LDEB(1);
				rval = -1;
				goto ready;
		...
		case MACPICT_PnPat:
			if (appMacPictGetPattern(dc, sis,
		...
		case MACPICT_OpColor: {
			...
			PICTDEB(appDebug(
				"OpColor(%u,%u,%u)\n", dc->dcMacPictRedMax,
				dc->dcMacPictGreenMax, dc->dcMacPictBlueMax));
		}
			continue;
		...
		case MACPICT_FrameSameOval: {
			PICTDEB(appDebug("FrameOval([%d..%d x %d..%d)\n",
					 dc->dcMacPictOvalRect.drX0,
					 dc->dcMacPictOvalRect.drX1,
					 dc->dcMacPictOvalRect.drY0,
					 dc->dcMacPictOvalRect.drY1));
			...
			if (appWinMetaDrawEllipse(
				    dc, through, &(dc->dcMacPictOvalRect),
		...
		case MACPICT_FramePoly: {
			...
			if ((*dc->dcDrawPolyPolygon)(
				    dc, through, 1, &(dc->dcPointCount),
				    dc->dcPoints, dc->dcFillInsides,
				    dc->dcDrawBorders, 1)) {
				LDEB(1);
				return -1;
			}
		...
		case MACPICT_ShortLine: {
			...
			if (appMacPictGetShortLine(&x0, &y0, &x1, &y1, dc,
						   sis)) {
				LDEB(1);
				rval = -1;
				goto ready;
		...
		case MACPICT_ShortLineFrom: {
			...
			if (appMacPictGetShortLineFrom(&x0, &y0, &x1, &y1, dc,
						       sis)) {
				LDEB(1);
				rval = -1;
				goto ready;
		...
		case MACPICT_LineFrom: {
			...
			if (appMacPictGetLineFrom(&x0, &y0, &x1, &y1, dc,
						  sis)) {
				LDEB(1);
				rval = -1;
				goto ready;
		...
		case MACPICT_FrameSameRRect: {
			...
			PICTDEB(appDebug("FrameRRect([%d..%d x %d..%d])\n",
					 dc->dcMacPictRRectRect.drX0,
					 dc->dcMacPictRRectRect.drX1,
					 dc->dcMacPictRRectRect.drY0,
					 dc->dcMacPictRRectRect.drY1));
			...
			if ((*dc->dcDrawRoundedRectangle)(dc, through, &dr, w,
							  h, dc->dcFillInsides,
							  dc->dcDrawBorders)) {
				LDEB(1);
				return -1;
		...
		case MACPICT_FrameSameRect: {
			PICTDEB(appDebug("FrameRect([%d..%d x %d..%d])\n",
					 dc->dcMacPictRectRect.drX0,
					 dc->dcMacPictRectRect.drX1,
					 dc->dcMacPictRectRect.drY0,
					 dc->dcMacPictRectRect.drY1));
			...
			if (appWinMetaDrawRectangle(
				    dc, through, &(dc->dcMacPictRectRect),
				    dc->dcFillInsides, dc->dcDrawBorders)) {
				LDEB(1);
				rval = -1;
				goto ready;
		...
		case MACPICT_FrameSameArc: {
			PICTDEB(appDebug(
				"FrameArc([%d..%d x %d..%d], %d -> %d)\n",
				dc->dcMacPictArcRect.drX0,
				dc->dcMacPictArcRect.drX1,
				dc->dcMacPictArcRect.drY0,
				dc->dcMacPictArcRect.drY1, dc->dcMacPictArcA0,
				dc->dcMacPictArcA1));
			...
			if ((*dc->dcDrawArc)(
				    dc, through, &(dc->dcMacPictArcRect),
		...
		case MACPICT_BitsRect: {
			...
			if (appMacPictDrawImage(dc, through, sis, packed,
						direct, clipRegion)) {
				LDEB(1);
				rval = -1;
				goto ready;
...
int appMacPictGetDeviceHeader(MacPictHeader *mph, SimpleInputStream *sis)
{
	int c;
	int i;
