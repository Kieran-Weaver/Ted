...
static int docCheckTreeIsEmpty(int *pIsEmpty, const BufferDocument *bd,
	...
	if (dt && dt->dtRoot) {
		...
		if (!docNextPosition(&dpNext)) {
			...
			const ParagraphProperties *pp =
				&(dpBegin.dpNode->biParaProperties);

			if (pp->ppTableNesting > 0) {
				isEmpty = 0;
			}

			if (docBorderNumberIsBorder(bd,
			...
			if (docBorderNumberIsBorder(
				    bd, pp->ppBetweenBorderNumber)) {
				isEmpty = 0;
			}
...
DocumentTree *docSectionHeaderFooter(const BufferItem *bodySectNode,
...
int docSectionHeaderFooterFirstPage(int *pUsedByDocument,
				    ...
				    int treeType, const DocumentProperties *dp)
{
	...
	if (bodySectNode->biNumberInParent > 0) {
		prevBi =
			bodySectNode->biParent
				->biChildren[bodySectNode->biNumberInParent - 1];
...
int docGetHeaderFooter(DocumentTree **pEi, BufferItem **pBodySectNode,
...
int docHeaderFooterPage(const BufferDocument *bd,
			const BufferItem *bodySectNode, int currentPage,
			int treeType)
{
	switch (treeType) {
		int isEmpty;
...
void docCleanSectHeadersFooters(BufferDocument *bd, SectHeadersFooters *shf)
{
