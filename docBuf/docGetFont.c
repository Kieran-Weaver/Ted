...
static int docGetPositionAttributes(PropertyMask *pUpdMask,
				  ...
				  const BufferItem *paraNode, int part)
{
	...
	if (paraNode->biParaListOverride > 0) {
		...
		if (!docDelimitParaHeadField((DocumentField **)0, &dsInside,
					     &dsAround, &headPart, &tailPart,
					     paraNode, bd) &&
		    part > headPart && part < tailPart) {
			const ListLevel *ll;

			if (docGetListLevelOfParagraph(
				    (int *)0, (int *)0,
				    (struct ListOverride **)0,
				    (struct DocumentList **)0, &ll,
...
int docGetSelectionAttributes(BufferDocument *bd, const DocumentSelection *ds,
			      PropertyMask *pUpdMask, TextAttribute *ta)
{
	BufferItem *paraNode;
	int part;
	const TextParticule *tp;
	...
	/*  1  */
	if (docFindParticuleOfPosition(&part, &flags, &(ds->dsHead),
				       PARAfindLAST)) {
	...
	if (paraNode->biParaListOverride > 0) {
		...
		if (docDelimitParaHeadField(&dfBullet, &dsInsideBullet,
					    &dsAroundBullet, &bulletPartBegin,
		...
		if (part <= bulletPartEnd) {
			if (!docSelectionInsideParagraph(ds) ||
			    ds->dsTail.dpStroff >=
				    dsAroundBullet.dsTail.dpStroff) {
	...
	if (IBar && part > 0 && (flags & POSflagPART_HEAD) &&
	...
	for (;;) {
		...
		if (ds->dsCol0 < 0 || ds->dsCol1 < 0 ||
			...
			while (part < paraNode->biParaParticuleCount) {
				...
				docGetTextAttributeByNumber(&ta, bd,
							    tp->tpTextAttrNr);
				...
				utilAttributeDifference(&pm, &taNew, &pmAll,
							&ta);
		...
		if (paraNode->biParaListOverride > 0) {
			...
			if (docDelimitParaHeadField(
				    ...
				    &bulletPartBegin, &bulletPartEnd, paraNode,
				    bd)) {
				LDEB(1);
			}
