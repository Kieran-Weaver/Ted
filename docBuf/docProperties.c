...
/*  a  */
static int docMergeScanParaAttributes(DocumentSelection *ds,
				      BufferItem *paraNode, BufferDocument *bd,
...
static int docMergeListTables(BufferDocument *bdTo, BufferDocument *bdFrom,
			      ...
			      const int *colorMap, const int *rulerMap)
{
	...
	listStylesAdded = docMergeListAdmins(laTo, laFrom, lsUsed, lsMap,
					     listUsed, fontMap, colorMap,
					     rulerMap);
	if (listStylesAdded < 0) {
		LDEB(listStylesAdded);
		return -1;
	}

	if (listStylesAdded > 0 &&
	    docClaimListNumberTrees(
		    &(bdTo->bdBody.dtListNumberTrees),
...
static void docCountListFontsUsed(BufferDocument *bdFrom, const int *lsUsed,
				  int *listUsed, int *fontUsed)
{
	const DocumentProperties *dpFrom = &(bdFrom->bdProperties);
	const ListAdmin *laFrom = dpFrom->dpListAdmin;
			  ...
			  const int *colorMap, const int *rulerMap)
{
	...
	if (dflFrom->dflFontCount > 0) {
		...
		if (!fontMap) {
			LXDEB(dflFrom->dflFontCount, fontMap);
			rval = -1;
			goto ready;
		}
		fontUsed = (int *)malloc(dflFrom->dflFontCount * sizeof(int));
		if (!fontUsed) {
			LXDEB(dflFrom->dflFontCount, fontUsed);
			rval = -1;
			goto ready;
		}
	...
	if (lotFrom->lotOverrideCount > 0) {
		...
		if (!lsUsed) {
			LXDEB(lotFrom->lotOverrideCount, lsUsed);
			rval = -1;
			goto ready;
		}
		lsMap = (int *)malloc(lotFrom->lotOverrideCount * sizeof(int));
		if (!lsMap) {
			LXDEB(lotFrom->lotOverrideCount, lsMap);
			rval = -1;
			goto ready;
		}
	...
	if (dltFrom->dltListCount > 0) {
		...
		if (!listUsed) {
			LXDEB(dltFrom->dltListCount, listUsed);
			rval = -1;
			goto ready;
		}
	...
	if (docMergeListTables(bdTo, bdFrom, lsUsed, lsMap, listUsed, fontMap,
			       colorMap, rulerMap)) {
		LDEB(1);
		rval = -1;
		goto ready;
			...
			const BufferDocument *bdFrom)
{
	...
	return utilMergeColorPalettes(pColorMap, cpTo, cpFrom, avoidZero,
				      maxColors);
}
