...
void docCleanNote(BufferDocument *bd, DocumentNote *dn)
{
...
static void docSetNote(DocumentNote *fresh, int autoNumber)
{
	docInitNote(fresh);
...
int docInsertNote(DocumentNote **pDn, BufferDocument *bd, DocumentField *dfNote,
		  int autoNumber)
{
	...
	fresh = (DocumentNote *)realloc(
		nl->nlNotes, (nl->nlNoteCount + 1) * sizeof(DocumentNote));
	if (!fresh) {
		LXDEB(nl->nlNoteCount, fresh);
		return -1;
	}
	nl->nlNotes = fresh;
...
DocumentField *docGetFirstNoteFromColumn(DocumentNote **pDn,
					 const BufferDocument *bd, int page,
					 int column, int treeType)
{
...
DocumentField *docGetFirstNoteInColumn(DocumentNote **pDn,
				       const BufferDocument *bd, int page,
				       int column, int treeType)
{
	DocumentField *dfNote;
...
DocumentField *docGetLastNoteInColumn(DocumentNote **pDn,
				      const BufferDocument *bd, int page,
				      int column, int treeType)
{
...
DocumentField *docGetFirstNoteOnPage(DocumentNote **pDn,
				     const BufferDocument *bd, int page,
				     int treeType)
{
	DocumentField *dfNote;
...
static DocumentField *docGetNextSectionNote(DocumentField *df,
					    DocumentNote **pDn,
					    const BufferDocument *bd, int sect,
					    int treeType)
{
...
DocumentField *docGetFirstNoteOfSection(DocumentNote **pDn,
					const BufferDocument *bd, int sect,
					int treeType)
{
	const ChildFields *rootFields = &(bd->bdBody.dtRootFields);
...
DocumentField *docGetNextNoteInSection(DocumentNote **pDn,
				       const BufferDocument *bd, int sect,
				       DocumentField *df, int treeType)
{
	const ChildFields *rootFields = &(bd->bdBody.dtRootFields);
...
static DocumentField *docGetNextDocumentNote(DocumentField *df,
					     DocumentNote **pDn,
...
DocumentField *docGetFirstNoteOfDocument(DocumentNote **pDn,
					 const BufferDocument *bd, int treeType)
{
	const ChildFields *rootFields = &(bd->bdBody.dtRootFields);
	DocumentField *df;
...
DocumentField *docGetNextNoteInDocument(DocumentNote **pDn,
					const BufferDocument *bd,
					DocumentField *df, int treeType)
{
	const ChildFields *rootFields = &(bd->bdBody.dtRootFields);
...
static DocumentField *docGetPrevDocumentNote(DocumentField *df,
					     DocumentNote **pDn,
					     const BufferDocument *bd,
					     int treeType)
{
...
DocumentField *docGetLastNoteOfDocument(DocumentNote **pDn,
					const BufferDocument *bd, int treeType)
{
	const ChildFields *rootFields = &(bd->bdBody.dtRootFields);
	DocumentField *df;
...
DocumentField *docGetPrevNoteInDocument(DocumentNote **pDn,
					const BufferDocument *bd,
					DocumentField *df, int treeType)
{
	const ChildFields *rootFields = &(bd->bdBody.dtRootFields);
...
DocumentField *docGetSelectedNote(DocumentNote **pDn, int *pSelInNote,
				  BufferDocument *bd,
	...
	if (ds->dsSelectionScope.ssTreeType == DOCinBODY) {
		...
		if (!dfNote && docIsIBarSelection(ds)) {
			...
			if (!docPrevPosition(&dpPrev) &&
				...
				dfNote = docFindTypedFieldForPosition(
					bd, &dpPrev, DOCfkCHFTN, lastOne);
			}
		}
...
DocumentTree *docDocumentNoteSeparator(BufferDocument *bd, int treeType)
{
	switch (treeType) {
	case DOCinFTNSEP:
...
int docCheckNoteSeparatorItem(BufferDocument *bd, int sepTreeType)
{
	...
	if (!dt->dtRoot) {
		...
		if (!paraNode) {
			...
			if (docParaStringReplace(&stroffShift, paraNode, off,
						 off, "-", 1)) {
				LDEB(1);
			} else {
				...
				const BufferDocument *bd)
{
	...
	if (dfNote->dfSelectionScope.ssTreeType == DOCinBODY) {
		if (dfNote->dfNoteIndex < 0 ||
			...
			LLLDEB(dfNote->dfFieldNumber, dfNote->dfNoteIndex,
			       nl->nlNoteCount);
			return (DocumentNote *)0;
		}
