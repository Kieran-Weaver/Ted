...
void docFreeDocument(BufferDocument *bd)
{
	...
	if (bd->bdSeqFieldIdentifiers) {
		utilTreeFreeTree(bd->bdSeqFieldIdentifiers, utilTreeFreeValue,
				 (void *)0);
	}
			 ...
			 int colorNumber)
{
	...
	if (colorNumber != 0) {
		if (colorNumber < 1 || colorNumber >= cp->cpColorCount) {
			LLDEB(colorNumber, cp->cpColorCount);
		} else {
			*rgb8 = cp->cpColors[colorNumber];
		}
	}
...
static int docSetupDocumentBody(BufferDocument *bd)
{
	...
	if (!bd->bdBody.dtRoot) {
		XDEB(bd->bdBody.dtRoot);
		return -1;
	}

	docInitNode(bd->bdBody.dtRoot, (struct BufferItem *)0, bd, 0,
		    DOClevBODY, DOCinBODY);
...
BufferDocument *docNewDocument(const BufferDocument *bdRef)
{
	...
	bd->bdProperties.dpColorPalette =
		&(bd->bdPropertyLists->dplColorPalette);
			   ...
			   const DocumentGeometry *dg)
{
	...
	if (textAttributeNumber < 0) {
		SLDEB(defaultFontName, textAttributeNumber);
		goto failed;
	}
...
{
	NumberedPropertiesList *taList =
		&(bd->bdPropertyLists->dplTextAttributeList);
	const DocumentProperties *dp = &(bd->bdProperties);
...
{
	NumberedPropertiesList *isList =
		&(bd->bdPropertyLists->dplItemShadingList);

	return docItemShadingNumberImpl(isList, is);
}

void docGetItemShadingByNumber(ItemShading *is, const BufferDocument *bd, int n)
{
	NumberedPropertiesList *isList =
		&(bd->bdPropertyLists->dplItemShadingList);
...
int docBorderPropertiesNumber(BufferDocument *bd, const BorderProperties *bp)
{
	NumberedPropertiesList *bpList =
		&(bd->bdPropertyLists->dplBorderPropertyList);
...
{
	NumberedPropertiesList *fpList =
		&(bd->bdPropertyLists->dplFramePropertyList);
...
int docTabStopListNumber(BufferDocument *bd, const TabStopList *tsl)
{
	NumberedPropertiesList *tslList =
		&(bd->bdPropertyLists->dplTabStopListList);

	return docTabStopListNumberImpl(tslList, tsl);
}

void docGetTabStopListByNumber(TabStopList *tsl, const BufferDocument *bd,
			       int n)
{
	NumberedPropertiesList *tslList =
		&(bd->bdPropertyLists->dplTabStopListList);
			  ...
			  const DocumentGeometry *dgFrom)
{
	...
	for (sect = 0; sect < bd->bdBody.dtRoot->biChildCount; sect++) {
		...
		utilOverridePaperSize(&(sectNode->biSectDocumentGeometry),
				      dgFrom);
	}
