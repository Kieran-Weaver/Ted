...
int docParaStringReplace(int *pSizeShift, BufferItem *paraNode, int stroffBegin,
...
int docParaNextWord(const BufferItem *paraNode, int stroff)
{
...
int docParaPrevWord(const BufferItem *paraNode, int stroff)
{
...
int docParaHeadOfWord(int *pStroff, const BufferItem *paraNode, int part)
{
	const TextParticule *tp = paraNode->biParaParticules + part;
	int stroff = tp->tpStroff;

	while (part > 0 && tp[-1].tpKind == DOCkindSPAN &&
...
int docParaTailOfWord(int *pStroff, const BufferItem *paraNode, int part)
{
	const TextParticule *tp = paraNode->biParaParticules + part;
...
int docMakeCapsString(char **pUpperString, int **pSegments, int *pSegmentCount,
		      const TextAttribute *ta, const char *printString, int len)
{
	int rval = 0;
	...
	*pUpperString = upperString;
	upperString = (char *)0; /* steal */
	*pSegments = segments;
	segments = (int *)0; /* steal */
...
int docParaPastLastNonBlank(const BufferItem *paraNode, int from, int upto)
{
	while (upto > from && paraNode->biParaString[upto - 1] == ' ') {
		upto--;
	}
...
static int docDelimitParticule(TextParticule *tpNew, const char *from,
			       int strLen)
{
	int len = 0;
...
int docRedivideStringInParticules(BufferItem *paraNode, int strOff, int strLen,
				  ...
				  int textAttributeNumber)
{
	...
	while (bytesDone < strLen) {
		...
		len = docDelimitParticule(
			&tpNew, (const char *)paraNode->biParaString + strOff,
			strLen - bytesDone);
		if (len < 0) {
			LDEB(len);
			return -1;
		...
		if (partsDone < partsFree) {
			...
			tp = docInsertTextParticule(
				paraNode, part, tpNew.tpStroff, tpNew.tpStrlen,
		...
		appDebug("%s %3d: [%4d..%4d] %s \"%.*s\" len= %d\n", label,
			 part, tp->tpStroff, tp->tpStroff + tp->tpStrlen,
			 docKindStr(tp->tpKind), (int)tp->tpStrlen,
			 docParaString(paraNode, tp->tpStroff), tp->tpStrlen);
#endif
...
static int docParaAppendBytes(void *vParaNode, int offset, const char *bytes,
			      int count)
{
	BufferItem *paraNode = (BufferItem *)vParaNode;
	int stroffShift = 0;

	if (docParaStringReplace(&stroffShift, paraNode, offset, offset, bytes,
				 count)) {
		LDEB(docParaStrlen(paraNode));
		return -1;
	}
...
int docParaDivideAppendedText(BufferItem *paraNode, int textAttributeNumber,
			      int stroff, int upto)
{
	...
	if (npart > 0) {
		const TextParticule *tp =
			paraNode->biParaParticules + npart - 1;
	...
	if (docRedivideStringInParticules(paraNode, stroff, upto - stroff, part,
					  partsFree, textAttributeNumber) < 0) {
		LLDEB(upto - stroff, paraNode->biParaParticuleCount);
		return -1;
	}
...
int docParaAppendText(BufferDocument *bd, BufferItem *paraNode,
		      ...
		      const char *text, int len)
{
	...
	upto = textConverterConvertToUtf8(tc, (void *)paraNode, &consumed,
					  stroff, text, len);
	if (upto < 0) {
		LDEB(upto);
		return -1;
	...
	if (docParaDivideAppendedText(paraNode, textAttributeNumber, stroff,
				      upto)) {
		LDEB(1);
		return -1;
	}
...
int docParaFixStroff(const BufferItem *paraNode, int stroff)
{
...
int docParaNextStroff(const BufferItem *paraNode, int stroff)
{
	const char *from = (const char *)docParaString(paraNode, stroff);
...
int docParaPrevStroff(const BufferItem *paraNode, int stroff)
{
	unsigned short unicode;
	const char *from;
	int step;
