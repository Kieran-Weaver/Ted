			...
			const NoteProperties *np, int noteNumber)
{
	...
	default:
		SDEB(docTreeTypeStr(np->npTreeType));
		return -1;
	}
				 ...
				 const RecalculateFields *rf)
{
	...
	if (dn) {
		...
		if (docFormatChftnField(
			    mbResult, &(bodySectNode->biSectNotesProperties),
			    &(dn->dnNoteProperties), dn->dnNoteNumber)) {
			LDEB(dn->dnNoteNumber);
			*pCalculated = 0;
			return 0;
		}
					  ...
					  const RecalculateFields *rf)
{
	...
	ret = docRecalculateParaStringTextParticules(pCalculated, pPartShift,
						     pStroffShift, paraBi, part,
						     partCount, df, rf);
