...
int docSplitTextParticule(TextParticule **pTpPart, TextParticule **pTpNext,
			  BufferItem *paraNode, int part, int stroff)
{
	...
	if (part < 0 || part >= paraNode->biParaParticuleCount) {
		LLDEB(part, paraNode->biParaParticuleCount);
		return -1;
	}
	...
	if (tpPart->tpKind != DOCkindSPAN || stroff <= tpPart->tpStroff ||
	    stroff >= tpPart->tpStroff + tpPart->tpStrlen) {
		SLLLDEB(docKindStr(tpPart->tpKind), stroff, tpPart->tpStroff,
			tpPart->tpStrlen);
		docListNode(0, paraNode, 0);
		return -1;
	}

	tpPart = docInsertTextParticule(paraNode, part, tpScratch.tpStroff,
...
TextParticule *docInsertParticules(BufferItem *paraNode, int part, int count)
{
	int i;
	TextParticule *tp;
	...
	if (!tp) {
		LLDEB(paraNode->biParaParticuleCount, tp);
		return tp;
	}
	...
	if (part < 0) {
		...
		tp = paraNode->biParaParticules +
		     paraNode->biParaParticuleCount + count - 1;
...
TextParticule *docInsertTextParticule(BufferItem *paraNode, int part, int off,
				      int len, int kind,
...
void docDeleteParticules(BufferItem *paraNode, int first, int count)
{
...
TextParticule *docMakeSpecialParticule(BufferItem *paraNode, int n, int stroff,
				       int kind, int textAttrNr)
{
	...
	tp = docInsertTextParticule(paraNode, n, stroff, count, kind,
				    textAttrNr);
	if (!tp) {
...
int docSaveSpecialParticule(BufferDocument *bd, BufferItem *paraNode,
...
int docInsertAdminParticule(BufferDocument *bd, BufferItem *paraNode, int n,
			    int stroff, int objectNumber, int kind,
...
static void docShiftParticuleOffset(BufferDocument *bd,
				    ...
				    TextParticule *tp, int stroffShift)
{
...
int docShiftParticuleOffsets(BufferDocument *bd, BufferItem *paraNode,
			       ...
			       const DocumentPosition *dp, int lastOne)
{
	...
	case PARAfindLAST:
		...
		while (l < m) {
			...
			m = (l + r) / 2;
			tp = paraNode->biParaParticules + m;
		}
		...
		case DOCkindOBJECT: {
			...
			if (!io) {
				...
				if (io->ioKind == DOCokDRAWING_SHAPE) {
					docDeleteDrawingShape(
						bd, io->ioDrawingShape);
				}
			}
		} break;
		}
...
int docIntersectSelectionWithParagraph(DocumentSelection *pDsPara,
				       ...
				       const DocumentSelection *ds)
{
	...
	if (pDsPara) {
		...
		docSetRangeSelection(pDsPara, &(dsPara.dsHead),
				     &(dsPara.dsTail), direction);
	}
