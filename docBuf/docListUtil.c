...
int docGetListOfParagraph(ListOverride **pLo, DocumentList **pDl, int ls,
...
int docGetListLevelOfParagraph(int *startPath, int *formatPath,
			       ListOverride **pLo, DocumentList **pDl,
			       const ListLevel **pLl,
			       ...
			       const BufferDocument *bd)
{
	...
	if (docGetListForStyle(&lo, &dl, pp->ppListOverride,
		...
		LDEB(pp->ppListOverride);
		return -1;
	}

	if (docListGetFormatPath(startPath, formatPath, &ll, pp->ppListLevel,
				 dl, lo)) {
		LDEB(pp->ppListLevel);
		return -1;
	}
...
int docRemoveParagraphFromList(BufferItem *paraNode, DocumentTree *dt,
			       BufferDocument *bd)
{
	int rval = 0;
	...
	if (paraNode->biParaOutlineLevel < PPoutlineBODYTEXT) {
		if (docListNumberTreeDeleteParagraph(&(dt->dtOutlineTree),
						     paraNr)) {
			LDEB(paraNr);
			rval = -1;
		}
...
static int docFindListEnterNode(BufferItem *node, const DocumentSelection *ds,
				const BufferItem *bodySectNode, void *through)
{
	...
	if (node->biLevel == DOClevPARA) {
		...
		if (fl->fl_ls < 0 && pp->ppListOverride >= 1) {
			   ...
			   BufferDocument *bd)
{
	...
	if (ds) {
		if (docScanSelection(bd, ds, docFindListEnterNode,
				     (NodeVisitor)0, flags, &fl) < 0) {
			LDEB(1);
			return -1;
		}
	} else {
		if (docScanTree(bd, tree, docFindListEnterNode, (NodeVisitor)0,
				flags, &fl) < 0) {
			LDEB(1);
			return -1;
		}
...
static int docAdaptParagraphPropsToListLevel(int *pChanged,
...
int docAdaptParagraphToListLevel(int *pChanged, BufferItem *paraNode,
				 const BufferDocument *bd)
{
	int indentChanged = 0;
	...
	if (docGetListLevelOfParagraph(startPath, formatPath, &lo, &dl, &ll, pp,
		...
		if (docAdaptParagraphPropsToListLevel(
			    &indentChanged, &(paraNode->biParaProperties),
			    ll)) {
			LDEB(1);
		}
	}
...
static int docApplyListRulerLevel(const ListNumberTreeNode *lntn, int level,
				  ...
				  const DocumentTree *dt)
{
	...
	for (i = 0; i < lntn->lntnChildCount; i++) {
		...
		if (!paraNode) {
			LXDEB(child->lntnParaNr, paraNode);
			rval = -1;
			continue;
		}

		if (paraNode->biParaListLevel != level) {
			LLLDEB(child->lntnParaNr, paraNode->biParaListLevel,
			       level);
			rval = -1;
		}
		...
		if (docAdaptParagraphPropsToListLevel(
			    &changed, &(paraNode->biParaProperties), ll)) {
			LDEB(1);
			return -1;
		}
...
static int docApplyListRulerToTree(const DocumentList *dl,
				   const ListOverride *lo, BufferDocument *bd,
				   const DocumentTree *dt)
{
	if (!dt->dtRoot || dt->dtListNumberTrees.lntTreeCount <= lo->loIndex) {
		return 0;
	}

	return docApplyListRulerLevel(
		dt->dtListNumberTrees.lntTrees + lo->loIndex, 0, dl, lo, dt);
}
...
int docApplyListRuler(const DocumentList *dl, const ListOverride *lo,
		      BufferDocument *bd)
{
	int i;
	int rval = 0;
	...
	for (i = 0; i < bodyBi->biChildCount; i++) {
		...
		if (shf) {
			if (docApplyListRulerToTree(
				    dl, lo, bd, &(shf->shfFirstPageHeader))) {
				LDEB(1);
				rval = -1;
			}
			if (docApplyListRulerToTree(
				    dl, lo, bd, &(shf->shfLeftPageHeader))) {
				LDEB(1);
				rval = -1;
			}
			if (docApplyListRulerToTree(
				    dl, lo, bd, &(shf->shfRightPageHeader))) {
				LDEB(1);
				rval = -1;
			}

			if (docApplyListRulerToTree(
				    dl, lo, bd, &(shf->shfFirstPageFooter))) {
				LDEB(1);
				rval = -1;
			}
			if (docApplyListRulerToTree(
				    dl, lo, bd, &(shf->shfLeftPageFooter))) {
				LDEB(1);
				rval = -1;
			}
			if (docApplyListRulerToTree(
				    dl, lo, bd, &(shf->shfRightPageFooter))) {
				LDEB(1);
				rval = -1;
			}
		}
