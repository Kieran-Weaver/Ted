...
int docSelectWholeSection(DocumentSelection *ds, int direction)
{
...
int docSelectRow(DocumentSelection *ds, int direction, int allColumns)
{
	...
	if (!rowNode0 || !rowNode1) {
		XXDEB(rowNode0, rowNode1);
		return -1;
	}

	if (rowNode0->biParent != rowNode1->biParent) {
		XXDEB(rowNode0->biParent, rowNode1->biParent);
		return -1;
	}
...
int docSelectWholeCell(DocumentSelection *ds, int direction, int allRows)
{
	BufferItem *cellNode0 = docGetCellNode(ds->dsHead.dpNode);
	BufferItem *cellNode1 = docGetCellNode(ds->dsTail.dpNode);
	BufferItem *rowNode0;
	...
	if (allRows) {
		...
		if (docDelimitTable(ds->dsHead.dpNode, &parentNode, &col, &row0,
				    &row, &row1)) {
			LDEB(1);
			return -1;
		}
		...
		if (cellNode0->biNumberInParent >= rowNode0->biChildCount) {
			LLDEB(cellNode0->biNumberInParent,
			      rowNode0->biChildCount);
			return -1;
		}
		cellNode0 = rowNode0->biChildren[cellNode0->biNumberInParent];

		if (cellNode1->biNumberInParent >= rowNode1->biChildCount) {
			LLDEB(cellNode1->biNumberInParent,
			      rowNode1->biChildCount);
			return -1;
		}
		cellNode1 = rowNode1->biChildren[cellNode1->biNumberInParent];
	}
	...
	if (cellNode0 == cellNode1) {
		...
		if (rowspan > 1 || colspan > 1) {
			rowNode1 =
				parentNode
					->biChildren[rowNode0->biNumberInParent +
						     rowspan - 1];
			cellNode1 =
				rowNode1->biChildren[cellNode0->biNumberInParent +
						     colspan - 1];
...
int docSelectColumn(DocumentSelection *ds, int col)
{
	...
	if (col < 0 || col >= rowNode->biChildCount) {
		LLDEB(col, rowNode->biChildCount);
		return -1;
	}
...
int docSelectWholeTable(DocumentSelection *ds)
{
	...
	if (docDelimitTable(ds->dsHead.dpNode, &parentNode, (int *)0, &row0,
			    (int *)0, &row1)) {
		LDEB(1);
		return -1;
	}
