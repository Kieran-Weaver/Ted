...
int docRecalculateIncludePictureField(int *pCalculated, int *pPartShift,
				      int *pStroffShift, BufferItem *paraBi,
				      ...
				      const RecalculateFields *rf)
{
	...
	if (partCount == 1 && tp->tpKind == DOCkindOBJECT) {
		*pCalculated = 0;
		goto ready;
	}
	...
	if (res) {
		...
		if (docFieldReplaceContents(&stroff, &stroffShift,
					    &textAttributeNumber, paraBi, part,
					    partCount, *pStroffShift,
					    (const char *)fullName.mbBytes,
					    fullName.mbSize, rf)) {
			LDEB(1);
			rval = -1;
			goto ready;
		...
		tp = docInsertTextParticule(paraBi, part + 1, stroff,
					    fullName.mbSize, DOCkindSPAN,
					    textAttributeNumber);
		if (!tp) {
			XDEB(tp);
			rval = -1;
		...
		if (docFieldReplaceContents(
			    &stroff, &stroffShift, &textAttributeNumber, paraBi,
			    part, partCount, *pStroffShift, " ", 1, rf)) {
			LDEB(1);
			rval = -1;
			goto ready;
		}
