...
static int docCharsUsedGotSpan(BufferDocument *bd, BufferItem *paraNode,
			       ...
			       int from, int upto, void *through)
{
	...
	if (fontNumber < 0 || fontNumber >= dfl->dflFontCount) {
...
static int docCharsListObject(const BufferDocument *bd, BufferItem *paraNode,
...
int docGetCharsUsed(BufferDocument *bd)
{
	...
	for (i = 0; i < lot->lotOverrideCount; lo++, i++) {
		...
		for (level = 0; level < lo->loLevelCount; lol++, level++) {
			...
			if (docInludeFontOfAttribute(bd,
						     &(ll->llTextAttribute))) {
				XDEB(0x20);
				return -1;
			}
	...
	for (i = 0; i < dlt->dltListCount; dl++, i++) {
		...
		for (level = 0; level < dl->dlLevelCount; ll++, level++) {
			...
			if (ll->llTextAttribute.taFontNumber < 0) {
				LLLDEB(i, level,
				       ll->llTextAttribute.taFontNumber);
				continue;
			}
