...
void docDeleteShapeFromList(DrawingShapeList *dsl, struct DrawingShape *ds)
{
	...
	utilPagedListDeleteItemByNumber(&(dsl->dslPagedList),
					ds->dsShapeNumber);
...
DrawingShape *docClaimShapeInParent(DrawingShapeList *dsl, DrawingShape *parent,
				    int n, int kind)
{
	int i;
	...
	if (!freshChildren) {
		LXDEB(newSize, freshChildren);
		return (DrawingShape *)0;
	}
