...
static int docCheckChild(const BufferItem *parent, const BufferItem *child,
			 int i, const LayoutPosition *lpTop,
			 LayoutPosition *lpBelow, int checkGeometry)
{
	int rval = 0;
	...
	if (docIsRowNode(child)) {
		...
		belowChild = &(child->biBelowPosition);
	}
	...
	if (checkGeometry) {
		...
		if (docIsRowNode(parent)) {
			...
			belowParent = &(parent->biBelowPosition);
		}

		if (lpTop) {
			int d = docCompareLayoutPositions(
				&(child->biTopPosition), lpTop);
			...
			if (direction) {
				...
				LLDEB(child->biTopPosition.lpPage,
				      lpTop->lpPage);
				LLDEB(child->biTopPosition.lpColumn,
				      lpTop->lpColumn);
				LLDEB(child->biTopPosition.lpPageYTwips,
				      lpTop->lpPageYTwips);
				rval = -1;
			}
		}
...
int docCheckNode(const BufferItem *node, int checkGeometry)
{
	int i;
	int rval = 0;
	...
	if (node->biLevel == DOClevPARA) {
		...
		for (part = 0; part < node->biParaParticuleCount;
		     tp++, part++) {
			if (tp->tpStroff > docParaStrlen(node) ||
			    tp->tpStroff != stroff) {
				appDebug("############## %s:\n",
					 docLevelStr(node->biLevel));
				LLLLDEB(part, tp->tpStroff, stroff,
					docParaStrlen(node));
				rval = -1;
			}

			stroff = tp->tpStroff + tp->tpStrlen;
			if (stroff < 0 || stroff > docParaStrlen(node)) {
				appDebug("############## %s:\n",
					 docLevelStr(node->biLevel));
				LLLLDEB(part, stroff, tp->tpStrlen,
					docParaStrlen(node));
				rval = -1;
			}
		}
			...
			if (!docHeadPosition(&dp, (BufferItem *)node)) {
				...
				for (;;) {
					if (docNumberOfParagraph(paraBi) != n) {
						LLDEB(docNumberOfParagraph(
							      paraBi),
						      n);
						rval = -1;
					}

					paraBi = docNextParagraph(
						(BufferItem *)paraBi);
					if (!paraBi) {
						break;
					}
	...
	for (i = 0; i < node->biChildCount; i++) {
		...
		if (docCheckChild(node, node->biChildren[i], i, &lpHere,
				  &lpBelowChild, checkGeometry)) {
			rval = -1;
		}
...
int docCheckRootNode(const BufferItem *node, int checkGeometry)
{
	while (node->biParent) {
		node = node->biParent;
	}
...
static void docListChildren(int indent, const BufferItem *node,
			    int checkGeometry)
{
	int i;
	...
	for (i = 0; i < node->biChildCount; i++) {
		docCheckChild(node, node->biChildren[i], i, &lp, &lp,
			      checkGeometry);
	...
	if (checkGeometry && !docIsRowNode(node) &&
...
static void docListParaNode(int indent, const BufferItem *node)
{
	int i;
	const TextLine *tl;
	LayoutPosition lpHere;
	...
	appDebug("%*s{ NR= %d TN=%d brk=%s %d particules, %d lines\n",
		 IS * indent + IS, "", docNumberOfParagraph(node),
		 node->biParaTableNesting,
		 docBreakKindStr(node->biParaBreakKind),
		 node->biParaParticuleCount, node->biParaLineCount);
...
#if LIST_PARA_STRING
	{
		...
		while (stroff < length) {
			...
			appDebug("%*s\"%.*s\"\n", IS * indent + IS, "", (int)l,
				 (char *)docParaString(node, stroff));
	...
	for (i = 0; i < node->biParaLineCount; tl++, i++) {
		...
		if (docCompareLayoutPositions(&(tl->tlTopPosition), &lpHere) <
		    0) {
			appDebug("###### Line= %d\n", i);
			LDEB(i);
			listLine = 1;
...
void docListNode(int indent, const BufferItem *node, int checkGeometry)
{
	int i;
	...
	appDebug("NODE %4d 0x%08lx: %s(%s) LEFT= %d PG: %d:%d..%d:%d\n",
		 node->biNumberInParent, (unsigned long)node,
		 docLevelStr(node->biLevel), docTreeTypeStr(node->biTreeType),
		 node->biLeftParagraphs, node->biTopPosition.lpPage,
		 node->biTopPosition.lpColumn, node->biBelowPosition.lpPage,
	...
	case DOClevROW:
		...
		if (node->biRowTableFirst >= 0 || node->biRowTablePast >= 0 ||
		    node->biRowTableHeaderRow >= 0) {
			appDebug("%*s  Table %d .. %d Header@%d\n",
				 IS * indent + IS, "", node->biRowTableFirst,
				 node->biRowTablePast,
				 node->biRowTableHeaderRow);

			if (node->biRowTableFirst >=
			    node->biParent->biChildCount) {
				SLLDEB("####", node->biRowTableFirst,
				       node->biParent->biChildCount);
			}
			if (node->biRowTablePast >
			    node->biParent->biChildCount) {
				SLLDEB("####", node->biRowTablePast,
				       node->biParent->biChildCount);
			}
		}
...
void docListRootNode(int indent, const BufferItem *node, int checkGeometry)
{
	while (node->biParent) {
		node = node->biParent;
	}
