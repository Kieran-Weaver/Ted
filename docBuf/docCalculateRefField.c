			      ...
			      const BufferDocument *bd)
{
	...
	if (docFindBookmarkInDocument(&dsInside, &headPart, &tailPart, bd,
				      markName)) {
		const char *ehh1 = "<<? ";
		const char *ehh2 = "?>> ";

		utilMemoryBufferAppendBytes(mbResult, (unsigned char *)ehh1,
					    strlen(ehh1));

		utilMemoryAppendBuffer(mbResult, markName);

		utilMemoryBufferAppendBytes(mbResult, (unsigned char *)ehh2,
					    strlen(ehh2));
	} else {
		int beginMoved = 0;
		int endMoved = 0;
	...
	*/
		...
		for (; part < paraBiFrom->biParaParticuleCount; tp++, part++) {
			...
			if (tp->tpKind == DOCkindSPAN) {
				if (skipping) {
					utilMemoryBufferAppendBytes(
						mbResult, (unsigned char *)" ",
						1);
				}
