...
static TextParticule *docInsertObject(BufferDocument *bd, BufferItem *paraBi,
				      int n, int off, const TextAttribute *ta)
{
	TextParticule *tp;
	InsertedObject *io;
	...
	tp = docInsertTextParticule(paraBi, n, off, 1, DOCkindOBJECT,
				    textAttributeNumber);
	if (!tp) {
		XDEB(tp);
...
TextParticule *docAppendObject(BufferDocument *bd, BufferItem *paraBi,
...
static void docCleanParaObjects(CleanNodeObjects *cno, BufferItem *paraNode)
{
	int part;
	TextParticule *tp;
	...
	if (paraNode->biParaListOverride > 0 ||
		...
		if (docRemoveParagraphFromList(paraNode, cno->cnoTree,
					       cno->cnoDocument)) {
			LDEB(1);
		}
...
static void docCleanTreeObjects(int *pBulletsDeleted, int *pParagraphCount,
				DocumentTree *dt, BufferDocument *bd,
				DOC_CLOSE_OBJECT closeObject)
{
	if (dt->dtRoot) {
		docCleanNodeObjects(pBulletsDeleted, pParagraphCount, dt, bd,
				    dt->dtRoot, closeObject);
	}
}
...
{
	docCleanTreeObjects(&(cno->cnoBulletsDeleted),
			    &(cno->cnoParagraphCount),
			    &(shf->shfFirstPageHeader), cno->cnoDocument,
			    cno->cnoCloseObject);

	docCleanTreeObjects(&(cno->cnoBulletsDeleted),
			    &(cno->cnoParagraphCount),
			    &(shf->shfLeftPageHeader), cno->cnoDocument,
			    cno->cnoCloseObject);

	docCleanTreeObjects(&(cno->cnoBulletsDeleted),
			    &(cno->cnoParagraphCount),
			    &(shf->shfRightPageHeader), cno->cnoDocument,
			    cno->cnoCloseObject);

	docCleanTreeObjects(&(cno->cnoBulletsDeleted),
			    &(cno->cnoParagraphCount),
			    &(shf->shfFirstPageFooter), cno->cnoDocument,
			    cno->cnoCloseObject);

	docCleanTreeObjects(&(cno->cnoBulletsDeleted),
			    &(cno->cnoParagraphCount),
			    &(shf->shfLeftPageFooter), cno->cnoDocument,
			    cno->cnoCloseObject);

	docCleanTreeObjects(&(cno->cnoBulletsDeleted),
			    &(cno->cnoParagraphCount),
			    &(shf->shfRightPageFooter), cno->cnoDocument,
			    cno->cnoCloseObject);

	return;
}

static int docVisitCleanNode(BufferItem *node, const DocumentSelection *ds,
			     const BufferItem *bodySectNode, void *vcno)
{
	...
	case DOClevSECT:
		if (node->biSectHeadersFooters) {
			docCleanHeaderFooterObjects(node->biSectHeadersFooters,
						    cno);
		}
		break;
...
void docCleanNodeObjects(int *pBulletsDeleted, int *pParagraphCount,
			 DocumentTree *dt, BufferDocument *bd, BufferItem *node,
			 DOC_CLOSE_OBJECT closeObject)
{
	const int flags = 0;
	...
	if (docScanTreeNode(bd, node, docVisitCleanNode, (NodeVisitor)0, flags,
			    &cno)) {
		LDEB(1);
	} else {
		if (pBulletsDeleted) {
...
void docCleanDocumentObjects(BufferDocument *bd, DOC_CLOSE_OBJECT closeObject)
{
	...
	docCleanTreeObjects(&bulletsDeleted, &paragraphCount, &(bd->bdBody), bd,
			    closeObject);

	docCleanTreeObjects(&bulletsDeleted, &paragraphCount, &(bd->bdEiFtnsep),
			    bd, closeObject);

	docCleanTreeObjects(&bulletsDeleted, &paragraphCount,
			    &(bd->bdEiFtnsepc), bd, closeObject);

	docCleanTreeObjects(&bulletsDeleted, &paragraphCount, &(bd->bdEiFtncn),
			    bd, closeObject);
	...
	docCleanTreeObjects(&bulletsDeleted, &paragraphCount, &(bd->bdEiAftncn),
			    bd, closeObject);
