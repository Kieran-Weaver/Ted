...
static int docCalculateListtextParaString(char *target, int maxsize, int ilvl,
...
int docRecalculateParaListtextTextParticules(int *pCalculated, int *pPartShift,
					     int *pStroffShift,
					     ...
					     const RecalculateFields *rf)
{
	...
	if (pp->ppListOverride <= 0) { /*LDEB(pp->ppListOverride);*/
		*pCalculated = 0;
		goto ready;
	}
	...
	/*  6  */
	addedStrlen = docCalculateListtextParaString(addedString,
						     sizeof(addedString) - 1,
						     ilvl, numberPath,
						     startPath, formatPath, ll);
	if (addedStrlen < 0) {
		LLDEB(ilvl, addedStrlen);
		*pCalculated = 0;
		goto ready;
	}
	...
	if (docFieldReplaceContents(&stroff, &stroffShift, &textAttrNr,
				    paraNode, part, partCount, *pStroffShift,
				    addedString, addedStrlen, rf)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
	...
	if (ll->llFollow == DOCllfSPACE &&
	    (PROPmaskISSET(&(ll->llTextAttributeMask), TApropTEXTUNDERLINED) ||
	     PROPmaskISSET(&(ll->llTextAttributeMask), TApropSTRIKETHROUGH))) {
		makeSep = 1;
		sepKind = DOCkindSPAN;
	}

	tp = docInsertTextParticule(paraNode, part + 1, stroff,
				    addedStrlen - makeSep, DOCkindSPAN,
				    textAttrNr);
	if (!tp) {
		XDEB(tp);
		*pCalculated = 0;
