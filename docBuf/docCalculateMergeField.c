...
static int docCalculateMergefieldFieldString(int *pCalculated,
					     ...
					     const RecalculateFields *rf)
{
	...
	if (rf->rfFieldDataProvider) {
		...
		const char *fieldName =
			utilMemoryBufferGetString(&(mf.mfFieldName));
...
int docRecalculateMergeField(int *pCalculated, int *pPartShift,
			     ...
			     int part, int partCount, DocumentField *df,
			     const RecalculateFields *rf)
{
	int rval = 0;
	MemoryBuffer mbResult;
	...
	if (docRecalculateFieldParticulesFromString(pCalculated, pPartShift,
