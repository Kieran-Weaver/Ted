...
{
	appDebug("%*s%s %3d: [S %4d..%4d] A=%-3d %s %s", indent, "", label, n,
		 tp->tpStroff, tp->tpStroff + tp->tpStrlen, tp->tpTextAttrNr,
		 docParticuleFlagsStr(tp->tpFlags), docKindStr(tp->tpKind));
...
void docListTextLine(int indent, const char *label, int n,
		     const BufferItem *paraBi, const TextLine *tl)
{
	...
	appDebug("%*s%s %3d: PG=%2d:%d Y=%d H=%d S:[%4d..%4d] P:[%3d..%3d]%s ",
		 indent, "", label, n, tl->tlTopPosition.lpPage,
		 tl->tlTopPosition.lpColumn, tl->tlTopPosition.lpPageYTwips,
		 tl->tlLineStride, tl->tlStroff, tl->tlStroff + tl->tlStrlen,
		 tl->tlFirstParticule,
		 tl->tlFirstParticule + tl->tlParticuleCount, flags);
	...
	for (part = tl->tlFirstParticule;
	     part < tl->tlFirstParticule + tl->tlParticuleCount; tp++, part++) {
...
void docListNotes(const BufferDocument *bd)
{
	...
	for (i = 0; i < bd->bdNotesList.nlNoteCount; dn++, i++) {
		...
		appDebug("%4d %-8s %3d: NR=%3d PG=%3d:%d\n", i,
			 docTreeTypeStr(dn->dnNoteProperties.npTreeType), i,
			 dn->dnNoteNumber, dn->dnReferringPage,
