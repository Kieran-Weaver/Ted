...
static int docAttributesUsedGotSpan(BufferDocument *bd,
				    struct BufferItem *paraNode, int textAttrNr,
				    const TextAttribute *ta, int from, int upto,
				    void *through)
{
...
int docGetAttributesUsedInShape(BufferDocument *bd, const DrawingShape *ds,
				IndexSet *isUsed)
{
	int child;
	...
	for (child = 0; child < ds->dsChildCount; child++) {
		if (docGetAttributesUsedInShape(bd, ds->dsChildren[child],
						isUsed)) {
			LDEB(child);
			return -1;
		}
