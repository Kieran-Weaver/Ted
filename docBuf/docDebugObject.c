...
static int docListObject(int n, void *vio, void *through)
{
	...
	if (io->ioKind == DOCokDRAWING_SHAPE) {
		...
		if (!io->ioDrawingShape) {
			...
			SLDEB(docTreeTypeStr(ss->ssTreeType),
			      ss->ssOwnerSectNr);

			if (ss->ssOwnerNumber != ds->dsShapeNumber) {
				SLLDEB("###", ss->ssOwnerNumber,
				       ds->dsShapeNumber);
			}
		}
	}
...
{
	utilPagedListForAll(&(bd->bdObjectList.iolPagedList), docListObject,
			    (void *)0);
}

static void docListShape(int indent, const DrawingShape *ds)
{
...
static int docListShapeX(int n, void *vds, void *through)
{
...
static int docFindParaObjects(DocumentSelection *ds, BufferItem *paraBi,
			      BufferDocument *bd,
			      const DocumentPosition *dpFrom, void *through);

static void docListShapeObjects(const DrawingShape *dshp, BufferDocument *bd)
{
	...
	docFindFindNextInCurrentTree(&ds, &(ds.dsHead), bd, docFindParaObjects,
				     (void *)0);
}

static void docListParaObjects(const BufferItem *paraBi, BufferDocument *bd)
{
...
void docListDocumentObjects(BufferDocument *bd)
{
	...
	docFindFindNextInDocument(&ds, &(ds.dsHead), bd, docFindParaObjects,
				  (void *)0);
	return;
}
