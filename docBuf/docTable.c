...
int docDelimitTable(const BufferItem *paraNode, BufferItem **pParentNode,
		    int *pCol, int *pRow0, int *pRow, int *pRow1)
{
	BufferItem *rowNode;
	...
	if (paraNode->biLevel != DOClevPARA ||
	    paraNode->biParaTableNesting ==
		    0) { /* LLDEB(paraNode->biLevel,paraNode->biParaInTable); */
		return -1;
	}
...
int docGetCellBottom(LayoutPosition *lpBottom, const BufferItem *cellNode)
{
	...
	if (cp->cpVerticalMerge == CELLmergeFOLLOW) {
		...
		if (cp->cpVerticalMerge == CELLmergeHEAD) {
			...
			if (col == 0) {
				...
				l = cp[-1].cpRightBoundaryTwips;
			}
			r = docGetCellRight(&csp, cellNode);

			for (nRow = row0 + 1; nRow < rowp; nRow++) {
				int ncsp = 1;
				const BufferItem *nRowBi =
					parentNode->biChildren[nRow];
				int nCol;
				...
				const BufferItem *cellNode)
{
	...
	if (cp->cpHorizontalMerge == CELLmergeFOLLOW) {
		...
		if (cp->cpHorizontalMerge == CELLmergeHEAD) { /* right= */
			docGetCellRight(&colspan, cellNode);
		}
	}
...
int docGetTableRectangle(TableRectangle *tr, const DocumentSelection *ds)
{
	...
	/*  1  */
	if (docDelimitTable(ds->dsHead.dpNode, &parentNode0, &(trHead.trCol0),
			    &(trHead.trRow00), &(trHead.trRow0),
			    &(trHead.trRow11))) { /* LDEB(1); */
		return 1;
	}

	/*  2  */
	if (docDelimitTable(ds->dsTail.dpNode, &parentNode1, &(trTail.trCol0),
			    &(trTail.trRow00), &(trTail.trRow0),
			    &(trTail.trRow11))) { /* LDEB(1); */
		return 1;
	}

	/*  3  */
	if (parentNode0 != parentNode1) { /* XXDEB(parentNode0,parentNode1); */
		return 1;
	}
	...
	tr->trIsColSlice =
		(tr->trRow0 == tr->trRow00 && tr->trRow1 == tr->trRow11);

	tr->trIsRowSlice = (tr->trCol0 == 0 && tr->trCol1 == tr->trCol11);
	...
	if (tr->trIsSingleCell) {
		const BufferItem *rowNode0 =
			parentNode0->biChildren[trHead.trRow0];
		const BufferItem *cellNode0 = rowNode0->biChildren[tr->trCol0];

		docTableDetermineCellspans(&(tr->trCellRowspan),
					   &(tr->trCellColspan), cellNode0);
	}
...
void docDelimitTables(BufferItem *parentNode, int recursively)
{
	int n;
	int headerRow = -1;
	...
	while (n < parentNode->biChildCount) {
		...
		if (!docIsRowNode(rowNodeFirst)) {
			if (recursively &&
			    rowNodeFirst->biLevel != DOClevPARA) {
...
int docGetCellRight(int *pColspan, const BufferItem *cellNode)
{
	const BufferItem *rowNode = cellNode->biParent;
	int col = cellNode->biNumberInParent;
...
int docGetMatchingCell(int *pColspan, const BufferItem *rowNode, int l, int r)
{
	int col;
	int ll, rr;
	...
	if (rowNode->biLevel != DOClevROW) {
		SDEB(docLevelStr(rowNode->biLevel));
		return -1;
	}
	...
	for (col = 0; col < rowNode->biChildCount; cp++, col++) {
		...
		ll = rr;
		rr = cp->cpRightBoundaryTwips;
		if (cp->cpHorizontalMerge == CELLmergeHEAD) {
			rr = docGetCellRight(&colspan,
					   ...
					   const BufferItem *cellNode)
{
	...
	if (!docIsRowNode(rowNode)) {
		SSDEB(docLevelStr(rowNode->biLevel),
		      docLevelStr(cellNode->biLevel));
				   ...
				   const DocumentAttributeMap *dam)
{
	...
	if (tr->trRow1 > tr->trRow0) {
		...
		rp1 = &(rowNode->biParent->biChildren[tr->trRow1]
				->biRowProperties);
