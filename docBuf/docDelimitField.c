...
static DocumentField *docGetLeftOuterField(DocumentField *df,
...
static DocumentField *docGetRightOuterField(DocumentField *df,
					    const EditPosition *epHead,
					    const EditPosition *epTail)
{
	DocumentField *rval = (DocumentField *)0;
...
int docBalanceFieldSelection(DocumentField **pLeftField,
			     DocumentField **pRightField, int *pHeadMoved,
			     int *pTailMoved, int *pHeadPart, int *pTailPart,
			     DocumentPosition *dpHead, DocumentPosition *dpTail,
...
int docCountParticulesInField(const BufferItem *paraBi, int *pClosed, int part,
			      int partUpto)
{
	int fieldLevel = 0;
	int partCount = 0;
...
static int docFindFieldParticules(const BufferDocument *bd, int *pPart0,
				  int *pPart1, DocumentSelection *dsInside,
				  DocumentSelection *dsAround,
				  ...
				  const DocumentField *df)
{
	...
	/*  2  */
	if (docFindParticuleOfPosition(&part0, (int *)0, &(dsField->dsHead),
				       PARAfindFIRST)) {
		LDEB(1);
		return -1;
	}
	...
	/*  3  */
	if (docFindParticuleOfPosition(&part1, (int *)0, &(dsField->dsTail),
				       PARAfindLAST)) {
		LDEB(1);
		return -1;
	}
	tp = dsField->dsTail.dpNode->biParaParticules + part1;
	while (part1 >= 0 && tp->tpStroff == dsField->dsTail.dpStroff) {
		if (tp->tpKind == DOCkindFIELDTAIL &&
		    tp->tpObjectNumber == df->dfFieldNumber) {
			break;
		}
	...
	if (part1 < 0 || tp->tpStroff != dsField->dsTail.dpStroff) {
			 ...
			 const BufferDocument *bd, const DocumentField *df)
{
	...
	if (docSelectionForEditPositionsInDoc(
		    &dsField, bd, &(df->dfSelectionScope),
	...
	return docFindFieldParticules(bd, pPart0, pPart1, dsInside, dsAround,
				      &dsField, df);
}
...
int docDelimitParaHeadField(DocumentField **pDfHead,
			    ...
			    int *pTailPart, const BufferItem *paraBi,
