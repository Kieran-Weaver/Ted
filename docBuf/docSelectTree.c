...
int docGetTreeOfNode(DocumentTree **pTree, BufferItem **pBodySectNode,
		     BufferDocument *bd, BufferItem *node)
{
	...
	if (docGetRootOfSelectionScope(&dt, &bodySectNode, bd,
				       &(selSectBi->biSectSelectionScope))) {
		LDEB(1);
		return -1;
	}
...
BufferItem *docGetSelectionRoot(DocumentTree **pTree,
				BufferItem **pBodySectNode, BufferDocument *bd,
				      ...
				      const BufferDocument *bd)
{
	...
	if (ss->ssTreeType == DOCinBODY) {
		...
		sect = ss->ssOwnerSectNr;
	}

	if (sect < 0 || sect >= bd->bdBody.dtRoot->biChildCount) {
		SLLDEB(docTreeTypeStr(ss->ssTreeType), sect,
		       bd->bdBody.dtRoot->biChildCount);
		return (BufferItem *)0;
	}
...
BufferItem *docGetBodySectNode(BufferItem *node, const BufferDocument *bd)
{
	int sect;
	...
	if (node->biTreeType == DOCinBODY) {
		...
		sect = node->biSectSelectionScope.ssOwnerSectNr;
	}
