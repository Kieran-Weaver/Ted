...
static int docScanNode(TreeScanner *ts, BufferItem *node)
{
	int i;
	int ret;
	int isRow;
    ...
    */
	...
	if (node->biTreeType == DOCinBODY && node->biLevel == DOClevSECT) {
	...
	for (i = 0; i < node->biChildCount; i++) {
		...
		if (isRow) {
			...
			ret = docCompareCellPositionToSelection(
				&selected, node->biChildren[i],
				ts->tsSelection);
		} else {
		     ...
		     void *through)
{
	...
	if (ds) {
		...
		if (docGetTreeOfNode((DocumentTree **)0, &bodySectNode, bd,
				     root)) {
			LDEB(1);
			return -1;
		}
...
int docScanTreeNode(BufferDocument *bd, BufferItem *node, NodeVisitor enterNode,
		    NodeVisitor leaveNode, int flags, void *through)
{
	TreeScanner ts;
	int res;
	BufferItem *bodySectNode = (BufferItem *)0;
