		      ...
		      const DocumentRectangle *drTwips, const DrawingShape *ds)
{
	*drHere = *drTwips;
	...
	if (DSflipHORIZONTAL(ds)) {
		int sw = drHere->drX0;
		drHere->drX0 = drHere->drX1;
		drHere->drX1 = sw;
	}

	if (DSflipVERTICAL(ds)) {
		int sw = drHere->drY0;
		drHere->drY0 = drHere->drY1;
		drHere->drY1 = sw;
	}
			  ...
			  const DocumentRectangle *dr, const DrawingShape *ds)
{
	...
	drChild->drX0 = dr->drX0 + xscale * (sdChild->sdRelRect.drX0 -
					     sd->sdGroupRect.drX0);
	drChild->drY0 = dr->drY0 + yscale * (sdChild->sdRelRect.drY0 -
					     sd->sdGroupRect.drY0);
	drChild->drX1 = dr->drX0 + xscale * (sdChild->sdRelRect.drX1 -
					     sd->sdGroupRect.drX0);
	drChild->drY1 = dr->drY0 + yscale * (sdChild->sdRelRect.drY1 -
					     sd->sdGroupRect.drY0);
				 ...
				 int ySize)
{
	...
	geoTranslationAffineTransform2D(&att, (dr->drX1 + dr->drX0) / 2.0,
					(dr->drY1 + dr->drY0) / 2.0);
...
int docShapeGetLine(int *pLine, RGB8Color *rgb8, const DrawingShape *ds)
{
	const ShapeDrawing *sd = &(ds->dsDrawing);
	int line = 1;
