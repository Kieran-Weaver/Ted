...
DocumentField *docFindFieldForPosition(BufferDocument *bd,
...
DocumentField *docFindTypedFieldForPosition(BufferDocument *bd,
					    const DocumentPosition *dp,
					    ...
					    int kind, int lastOne)
{
	...
	if (docGetRootOfSelectionScope(&dt, (BufferItem **)0, bd,
				       &(ds->dsSelectionScope))) {
		LDEB(1);
		return (DocumentField *)0;
	}
...
int docSuggestNewBookmarkName(MemoryBuffer *markName, const BufferDocument *bd,
			      const DocumentSelection *ds)
{
	...
	if (paraBi) {
		...
		if (paraBi == ds->dsTail.dpNode) {
			...
			stroffTail = docParaStrlen(paraBi);
		}
...
int docMakeBookmarkUnique(const BufferDocument *bd, MemoryBuffer *markName)
{
	DocumentField *df;
	...
	if (docFindBookmarkField(&df, dfl, markName) >= 0) {
		...
		for (i = 0; i < 1000000; i++) {
			...
			utilMemoryBufferReplaceBytes(markName, offset,
						     markName->mbSize,
...
int docFieldKindFromInstructions(const DocumentField *df)
{
	...
	if (DOC_FieldKindCount != DOCfk_COUNT) {
		LLDEB(DOC_FieldKindCount, DOCfk_COUNT);
		return -1;
	}
...
static void docShiftFieldStart(DocumentField *df, int sectFrom, int paraFrom,
...
static void docShiftFieldEnd(DocumentField *df, int sectFrom, int paraFrom,
			     int stroffFrom, int sectShift, int paraShift,
			     int stroffShift)
{
	/*  earlier paragraph  */
...
static void docShiftChildFieldReferences(const ChildFields *cf, int sectFrom,
					 int paraFrom, int stroffFrom,
					 int sectShift, int paraShift,
					 int stroffShift)
{
	int l;
	int r;
			    ...
			    int stroffShift)
{
    ...
    */
	...
	docShiftChildFieldReferences(&(dt->dtRootFields), sectFrom, paraFrom,
				     stroffFrom, sectShift, paraShift,
				     stroffShift);
	return 0;
}
...
static void docDeleteFieldChildren(int *pUpdateFlags, BufferDocument *bd,
				   DocumentField *dfp)
{
...
int docDeleteFieldFromParent(DocumentTree *dt, DocumentField *df)
{
	...
	if (df->dfChildFields.cfChildCount == 0 && cf->cfChildCount == 1) {
		cf->cfChildCount--;
	} else {
			...
			DocumentField *dfLeft, DocumentField *dfRight)
{
	...
	for (i = 0; i < count; i++) {
		...
		bcm = docCompareEditPositions(&(df->dfHeadPosition),
					      &(er->erHead));
		ecm = docCompareEditPositions(&(df->dfTailPosition),
					      &(er->erTail));

		if (bcm < 0 || ecm > 0) {
			docDeleteFieldRange(pUpdateFlags, bd, er,
					    &(df->dfChildFields), dfLeft,
					    dfRight);
		} else {
...
int docInsertParaHeadField(DocumentField **pDfHead,
			   ...
			   DocumentSelection *dsAroundHead, int *pHeadPart,
			   int *pTailPart, BufferItem *paraBi,
			   ...
			   int textAttrNr)
{
	...
	while (head + 1 < paraBi->biParaParticuleCount &&
	       ...
	       docGetFieldKindByNumber(
		       dfl, paraBi->biParaParticules[head + 1].tpObjectNumber ==
				    DOCfkBOOKMARK)) {
		head++;
	}

	/*  4  */
	if (docParaStringReplace(&stroffShift, paraBi, stroffHead, stroffHead,
				 (const char *)"?", 1)) {
		LDEB(docParaStrlen(paraBi));
		return -1;
	}
	...
	if (paraBi->biParaParticuleCount > 0 && wasEmpty) {
		...
		tpText = docInsertTextParticule(paraBi, head, stroffHead, 1,
						DOCkindSPAN, textAttrNr);
		if (!tpText) {
			LXDEB(paraBi->biParaParticuleCount, tpText);
			return -1;
		}
	...
	/*  2,3  */
	df = docMakeField(bd, dt, &dsField, head, head + 1, textAttrNr,
			  textAttrNr);
	if (!df) {
		XDEB(df);
		return -1;
	...
	if (docDelimitFieldInDoc(dsInsideHead, dsAroundHead, pHeadPart,
				 pTailPart, bd, df)) {
		LDEB(1);
		return -1;
	}
			    ...
			    int part1, int attr0, int attr1)
{
...

	{
		...
		tp = docMakeSpecialParticule(dsInput->dsTail.dpNode, part1,
					     dsInput->dsTail.dpStroff,
					     DOCkindFIELDTAIL, attr1);
		if (!tp) {
			XDEB(tp);
			goto ready;
		}
		...
		docShiftParticuleOffsets(
			bd, dsInput->dsTail.dpNode, part1 + 1,
			dsInput->dsTail.dpNode->biParaParticuleCount,
			tp->tpStrlen);
	}

	{
		...
		tp = docMakeSpecialParticule(dsInput->dsHead.dpNode, part0,
					     dsInput->dsHead.dpStroff,
					     DOCkindFIELDHEAD, attr0);
		if (!tp) {
			XDEB(tp);
			goto ready;
		}
		...
		docShiftParticuleOffsets(
			bd, dsInput->dsHead.dpNode, part0 + 1,
			dsInput->dsHead.dpNode->biParaParticuleCount,
			tp->tpStrlen);
...
int docParaHeadFieldKind(const BufferItem *paraBi, const BufferDocument *bd)
{
	int fieldKind = -1;
