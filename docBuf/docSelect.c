...
int docGotoFirstPosition(DocumentPosition *dp, BufferItem *node)
{
	while (node) {
		if (node->biLevel == DOClevPARA) {
			docSetDocumentPosition(dp, node,
					       docParaFirstStroff(node));
			return 0;
		}

		if (node->biChildCount == 0) { /*LDEB(node->biChildCount);*/
			return -1;
		}
...
int docHeadPosition(DocumentPosition *dp, BufferItem *node)
{
	while (node) {
		if (node->biLevel == DOClevPARA) {
...
int docGotoLastPosition(DocumentPosition *dp, BufferItem *node)
{
	while (node) {
		if (node->biLevel == DOClevPARA) {
			docSetDocumentPosition(dp, node,
					       docParaLastStroff(node));
			return 0;
		}
...
int docTailPosition(DocumentPosition *dp, BufferItem *node)
{
	while (node) {
		if (node->biLevel == DOClevPARA) {
...
#define docIsMergedCell(node)                                           \
	((node)->biParent && docIsRowNode((node)->biParent) &&          \
	 (node)->biNumberInParent < (node)->biParent->biRowCellCount && \
	 CELL_MERGED(                                                   \
		 &((node)->biParent->biRowCells[(node)->biNumberInParent])))

static BufferItem *docNextNode(BufferItem *node, int level)
{
	for (;;) {
		if (!node->biParent) {
			return (BufferItem *)0;
...
static BufferItem *docPrevNode(BufferItem *node, int level)
{
	for (;;) {
		if (!node->biParent) {
			return (BufferItem *)0;
...
int docLineUp(int *pLine, DocumentPosition *dp, int line)
{
	...
	while (node) {
		if (node->biParaLineCount == 0) {
			LLDEB(docNumberOfParagraph(node),
		...
		if (node->biLevel == DOClevPARA &&
			...
			if (docDelimitTable(node, &parentNode, &col, &row0,
					    &row, &row1)) {
				LDEB(1);
				return -1;
			}
			...
			if (row > row0) {
				...
				if (col < rowNode->biChildCount) {
					BufferItem *cellNode =
						rowNode->biChildren[col];

					node = cellNode->biChildren
						       [cellNode->biChildCount -
							1];
...
int docLineHead(DocumentPosition *dp, int positionFlags)
{
	int line;
	int flags = 0;
...
int docLineTail(DocumentPosition *dp, int positionFlags)
{
	int line;
	int flags = 0;
...
void docLineSelection(DocumentSelection *dsLine, int *pPartLineBegin,
		      int *pPartLineEnd, const BufferItem *node, int line)
{
	const TextLine *tl = node->biParaLines + line;

	if (node->biLevel != DOClevPARA) {
		LLDEB(node->biLevel, DOClevPARA);
		return;
	}
	if (line < 0 || line >= node->biParaLineCount) {
		LLDEB(line, node->biParaLineCount);
		return;
	}
		      ...
		      const DocumentPosition *dpAround)
{
	...
	if (docFindParticuleOfPosition(&part, (int *)0, dpAround,
				       PARAfindLAST)) {
		LDEB(dpAround->dpStroff);
		return;
	}
	...
	if (tp->tpStroff == dpAround->dpStroff && part > 0 &&
	    tp[-1].tpKind == DOCkindOBJECT) {
		docSetParaSelection(dsWord, paraBi, 1, tp[-1].tpStroff,
				    tp[-1].tpStrlen);
		*pIsObject = 1;
		return;
	}
	...
	if (tp->tpKind == DOCkindOBJECT) {
		docSetParaSelection(dsWord, paraBi, 1, tp->tpStroff,
				    tp->tpStrlen);
		*pIsObject = 1;
		return;
	} else {
		...
		docSetParaSelection(dsWord, paraBi, 1, stroffHead,
				    stroffTail - stroffHead);
	}
...
int docTableRectangleSelection(DocumentSelection *ds, BufferItem **pSelParentBi,
			       BufferDocument *bd, const TableRectangle *tr)
{
	...
	if (tr->trRow0 < 0 || tr->trRow0 >= selParentBi->biChildCount) {
		LLDEB(tr->trRow0, selParentBi->biChildCount);
		return -1;
	}
	rowNode = selParentBi->biChildren[tr->trRow0];

	if (tr->trCol0 < 0 || tr->trCol0 >= rowNode->biChildCount) {
		LLDEB(tr->trCol0, rowNode->biChildCount);
		return -1;
	}
	...
	if (tr->trRow1 < 0 || tr->trRow1 >= selParentBi->biChildCount) {
		LLDEB(tr->trRow0, selParentBi->biChildCount);
		return -1;
	}
	rowNode = selParentBi->biChildren[tr->trRow1];

	if (tr->trCol1 < 0 || tr->trCol1 >= rowNode->biChildCount) {
		LLDEB(tr->trCol1, rowNode->biChildCount);
		return -1;
	}
...
int docGetObjectSelection(const DocumentSelection *ds, const BufferDocument *bd,
			  int *pPart, DocumentPosition *dpObject,
			  InsertedObject **pIo)
{
	if (ds->dsHead.dpNode && ds->dsTail.dpNode == ds->dsHead.dpNode) {
		BufferItem *paraBi = ds->dsHead.dpNode;
		int part;
		TextParticule *tp;

		if (docFindParticuleOfPosition(&part, (int *)0, &(ds->dsHead),
					       PARAfindLAST)) {
...
int docSelectFrameOfPosition(DocumentSelection *ds, const DocumentPosition *dp)
{
	...
	while (para0 > 0) {
		if (cellNode->biChildren[para0 - 1]->biParaFrameNumber !=
		    frameNumber) {
			break;
		}
		para0--;
	...
	while (para1 < cellNode->biChildCount - 1) {
		if (cellNode->biChildren[para1 + 1]->biParaFrameNumber !=
		    frameNumber) {
			break;
		}
		para1++;
