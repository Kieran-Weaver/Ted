				      ...
				      const DocumentSelection *ds)
{
	...
	if (!ds || ((ds->dsCol0 < 0 || col >= ds->dsCol0) &&
...
int docSetDocumentPosition(DocumentPosition *dp, BufferItem *node, int stroff)
{
	dp->dpNode = node;
	dp->dpStroff = stroff;
				      ...
				      const EditPosition *epTail)
{
	...
	if (docGetRootOfSelectionScope(&dt, &bodySectBi, (BufferDocument *)bd,
				       ss)) {
		LDEB(1);
		return -1;
	}
