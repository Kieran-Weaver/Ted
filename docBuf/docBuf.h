...
int docGetHeaderFooter(DocumentTree **pTree,
			      struct BufferItem **pBodySectNode,
...
int docSelectWholeCell(DocumentSelection *bs, int direction,
			      int allRows);
...
struct BufferItem *docGetParagraphByNumber(const DocumentTree *dt,
						  int n);
...
int docCountParticulesInField(const struct BufferItem *paraNode,
...
int docGetListOfParagraph(struct ListOverride **pLo,
...
int docMakeCapsString(char **pUpperString, int **pSegments,
			     int *pSegmentCount, const TextAttribute *ta,
			     const char *printString, int len);

int docChangeRowProperties(PropertyMask *rpDonePask,
...
int docGetMatchingCell(int *pColspan, const struct BufferItem *rowNode,
			      int l, int r);
...
const DocumentFont *docRtfGetCurrentFont(BufferDocument *bd,
						TextAttribute *ta);
