...
{
	appDebug("%*s%s %3d: [S %4d..%4d] %s", indent, "", label, n,
		 tp->tpStroff, tp->tpStroff + tp->tpStrlen,
		 docKindStr(tp->tpKind));

	if (tp->tpStrlen > 0) {
		appDebug(" \"%.*s\"", (int)tp->tpStrlen,
			 docParaString(bi, tp->tpStroff));
	}

	if (tp->tpKind == DOCkindOBJECT || tp->tpKind == DOCkindFIELDHEAD ||
	    tp->tpKind == DOCkindFIELDTAIL || tp->tpObjectNumber >= 0) {
...
static int docCheckFieldHeadParticule(int stroff, int part, int paraNr,
				      ...
				      const DocumentField *df)
{
	...
	if (tp->tpKind != DOCkindFIELDHEAD) {
		SLLDEB("####", tp->tpKind, DOCkindFIELDHEAD);
		wrong = 1;
	}
...
static int docCheckFieldTailParticule(int stroff, int part, int paraNr,
				      const BufferItem *paraNode,
				      const DocumentField *df)
{
	int wrong = 0;
	...
	if (tp->tpKind != DOCkindFIELDTAIL) {
		SLLDEB("####", tp->tpKind, DOCkindFIELDTAIL);
		wrong = 1;
	}
...
void docCheckFieldOffsets(const BufferDocument *bd, const BufferItem *bi)
{
...
static void docCheckNoteOfField(int indent, FieldPosition *fp,
				const DocumentField *df,
				const BufferDocument *bd)
{
	...
	if (dn->dnReferringPage < fp->fpPage ||
		...
		appDebug("%*s## NOTE %d:%d PG/COL BEFORE PREVIOUS %d:%d\n",
			 indent + 15, "", dn->dnReferringPage,
			 dn->dnReferringColumn, fp->fpPage, fp->fpColumn);
	}
...
static void docCheckFieldPosition(int indent, FieldPosition *fp,
				  const DocumentField *df,
				  const BufferDocument *bd)
{
	...
	appDebug("%*s {%*s%4d:%-4d .. %4d:%-4d %s #%d:%s\n", indent, "", fix,
		 "", df->dfHeadPosition.epParaNr, df->dfHeadPosition.epStroff,
		 df->dfTailPosition.epParaNr, df->dfTailPosition.epStroff,
		 endString, df->dfFieldNumber, docFieldKindStr(df->dfKind));

	/*
    docListFieldInstructions( indent+ 8, &(df->dfInstructions ) );
    */

	if (docCompareEditPositions(&(fp->fpPosition), &(df->dfHeadPosition)) >
	    0) {
...

	{
		...
		if (docGetRootOfSelectionScope(&dt, &bodySectNode,
			...
			SXDEB("##", bodySectNode);
			SSDEB("##",
			      docTreeTypeStr(df->dfSelectionScope.ssTreeType));
...
static void docCheckFieldTree(int indent, const BufferDocument *bd,
			      const DocumentTree *dt, FieldPosition *fp,
			      ...
			      const ChildFields *cf)
{
	...
	for (i = 0; i < cf->cfChildCount; i++) {
		...
		if (df->dfChildFields.cfChildCount > 0) {
			...
			docCheckFieldTree(indent + 1, bd, dt, fp, df,
					  &(df->dfChildFields));
			...
			appDebug("%*s }%*s%4d:%-4d   #%d:%s\n", indent, "", fix,
				 ...
				 df->dfTailPosition.epStroff, df->dfFieldNumber,
...
static void docScanNotes(const BufferDocument *bd, int treeType)
{
