...
static int docScanParaSpans(DocumentSelection *ds, BufferItem *paraNode,
			    BufferDocument *bd, const DocumentPosition *dpFrom,
			    void *vsdf);

static int docScanShapeSpans(const DrawingShape *ds, BufferDocument *bd,
			    ...
			    void *vsdf)
{
	...
	while (part < paraNode->biParaParticuleCount) {
		...
		case DOCkindOPT_HYPH:
			...
			if ((*sdf->sdfDocListSpanFont)(
				    bd, paraNode, attributeNumber, &ta, from,
				    upto, sdf->sdfThrough)) {
				LLDEB(from, tp->tpStroff);
				return -1;
			}
