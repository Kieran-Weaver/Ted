				  ...
				  const RecalculateFields *rf)
{
	...
	if (utilMemoryBufferEqualsString(&(sf.sfFontName),
					 "ITC Zapf Dingbats") ||
	...
	case SYMBOLencANSI:
		if (sf.sfSymbol >= 0 && sf.sfSymbol < 256 &&
					   ...
					   const RecalculateFields *rf)
{
	...
	rval = docRecalculateParaStringTextParticules(pCalculated, pPartShift,
						      pStroffShift, paraBi,
						      part, partCount, df, rf);
	...
	if (*pCalculated) {
		...
		if (!utilMemoryBufferIsEmpty(&(sf.sfFontName))) {
			...
			fontNumber =
				docGetFontByName(dp->dpFontList, allocated);
			if (fontNumber < 0) {
		...
		if (!utilPropMaskIsEmpty(&taSetMask)) {
			if (docChangeParticuleAttributes(
				    (int *)0, (PropertyMask *)0, rf->rfDocument,
				    paraBi, part + 1,
				    part + 1 + partCount + *pPartShift, &taSet,
				    &taSetMask)) {
				LDEB(part);
				rval = -1;
				goto ready;
