...
typedef int (*STEP_TREE)(DocumentPosition *dp, int *pPage, int *pColumn,
			 int samePhase, const BufferDocument *bd);

typedef BufferItem *(*STEP_PARA)(BufferItem *bi);

typedef int (*START_PARA)(DocumentPosition *dp, BufferItem *bi);
...
static DocumentTree *docFindGetHeaderFooter(BufferItem *bodySectNode,
...
static int docFindInitFindJob(DocumentFindJob *dfj, int reverse,
			      ...
			      const BufferDocument *bd)
{
	...
	if (dpFrom->dpNode) {
		...
		if (reverse) {
			if (docTailPosition(&(dfj->dfjCurrentPosition),
					    bodyBi)) {
				LDEB(1);
				return 1;
			}
		} else {
			if (docHeadPosition(&(dfj->dfjCurrentPosition),
					    bodyBi)) {
				LDEB(1);
				return 1;
			}
...
static int docFindNextHeaderFooter(DocumentPosition *dpNew, int *pExtItPage,
				   int *pExtItColumn, const BufferDocument *bd,
				   int sectFrom, int hfFrom)
{
	BufferItem *bodyBi = bd->bdBody.dtRoot;
	...
	for (sect = sectFrom; sect < bodyBi->biChildCount; sect++) {
		...
		for (hf = hfFrom; hf < DOC_HeaderFooterTypeCount; hf++) {
			...
			page = docSectionHeaderFooterFirstPage(
				&usedInDoc, bodySectNode,
...
static int docFindNextNote(DocumentPosition *dp, const BufferDocument *bd,
...
static int docFindNextSeparator(DocumentPosition *dp, int *pExtItPage,
				int *pExtItColumn, const BufferDocument *bd,
				int sepFrom)
{
	int sep;
	...
	for (sep = sepFrom; sep < DOC_FindDocNoteItemCount; sep++) {
		...
		case DOCinFTNSEP:
			if (!docGetFirstNoteOfDocument(&dn, bd,
						       DOCinFOOTNOTE)) {
				continue;
			}
...
static int docFindNextNextTree(DocumentPosition *dp, int *pPage, int *pColumn,
			       int samePhase, const BufferDocument *bd)
{
	int phaseTo;
	int phaseFrom;
	...
	case DOCinRIGHT_FOOTER:
		...
		if (hfFrom < 0) {
			LLDEB(selSectBi->biTreeType, hfFrom);
			return -1;
		}
		hfFrom++;
	...
	case DOCinAFTNCN:
		...
		if (sepFrom < 0) {
			LLDEB(selSectBi->biTreeType, sepFrom);
			return -1;
		}
		sepFrom++;
	...
	if (phaseTo <= 1) {
		res = docFindNextHeaderFooter(dp, pPage, pColumn, bd, sectFrom,
					      hfFrom);
		if (res < 0) {
			LDEB(res);
			return res;
...
static int docFindPrevHeaderFooter(DocumentPosition *dpNew, int *pExtItPage,
				   int *pExtItColumn, const BufferDocument *bd,
				   int sectFrom, int hfFrom)
{
	BufferItem *bodyBi = bd->bdBody.dtRoot;
...
static int docFindPrevNote(DocumentPosition *dp, const BufferDocument *bd,
			   DocumentField *dfNote)
{
	DocumentNote *dn;
...
static int docFindPrevSeparator(DocumentPosition *dp, int *pExtItPage,
				int *pExtItColumn, const BufferDocument *bd,
				int sepFrom)
{
	int sep;
...
static int docFindPrevPrevTree(DocumentPosition *dp, int *pPage, int *pColumn,
			       int samePhase, const BufferDocument *bd)
{
	int phaseTo;
	int phaseFrom;
	...
	if (phaseTo >= 1) {
		res = docFindPrevHeaderFooter(dp, pPage, pColumn, bd, sectFrom,
					      hfFrom);
		if (res < 0) {
			LDEB(res);
			return res;
...
static int docFindInCurrentTree(DocumentSelection *ds,
				const DocumentFindJob *dfj)
{
	...
	for (;;) {
		ret = (*dfj->dfjFindString)(ds, bi, dfj->dfjDocument, &dpHere,
					    dfj->dfjThrough);
...
typedef int (*FIND_IN_TREE)(DocumentSelection *ds,
			    const DocumentPosition *dpFrom,
			    const BufferDocument *bd, PARA_FIND_STRING findNext,
			    void *through);

static int docFindStepInDocument(DocumentSelection *ds, DocumentFindJob *dfj)
{
	int ret = -1;
...
int docFindFindNextInDocument(DocumentSelection *ds,
			      const DocumentPosition *dpFrom,
			      BufferDocument *bd, PARA_FIND_STRING findNext,
			      void *through)
{
	const int reverse = 0;
	DocumentFindJob dfj;
...
int docFindFindPrevInDocument(DocumentSelection *ds,
			      const DocumentPosition *dpFrom,
			      BufferDocument *bd, PARA_FIND_STRING findPrev,
			      void *through)
{
	const int reverse = 1;
	DocumentFindJob dfj;
