...
static int indPartition(IND *ind, int *clas, int *part, int *pngroup, int *gend)
{
	int turn = 0;
	int ngroup = *pngroup;
	...
	for (;;) {
				...
				for (i = first + 1; i < past; i++) {
					...
					if (intr == fntr) {
						for (cnt = 0; cnt < intr;
						     cnt++) {
							int cf =
								fnode->tn_transitions +
								cnt;
							int ci =
								inode->tn_transitions +
								cnt;
		...
		for (group = 0; group < ngroup; group++) {
			...
			LOG(appDebug(
				"TURN %3d CLASS %6d: %6d..%6d (%6d members)\n",
	...
	for (r = 0; r < ngroup; r++) {
		...
		while (first < past) {
			...
			for (cnt = 0; cnt < fntr; cnt++) {
				int cf = LINK(ind, fnode->tn_transitions + cnt)
						 ->tl_to;
				if (clas[cf] != r) {
					break;
				}
	...
	for (r = 0; r < ngroup; r++) {
		...
		if (node->tn_ntrans > 0) {
			...
			for (l = 0; l < node->tn_ntrans; l++) {
				TrieLink *link =
					LINK(ind, node->tn_transitions + l);
				LINK(rval, transitions + l)->tl_to =
					clas[link->tl_to];
				LINK(rval, transitions + l)->tl_key =
					link->tl_key;
			}
		} else {
			transitions = -1;
