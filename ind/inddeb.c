...
void indTLprint(IND *ind, int tl)
{
	TrieLink *link;
	char flags[3] = "  ";
...
void indTNprint(IND *ind, int tn)
{
...
void indINDprint(IND *ind)
{
	...
	appDebug("INDEX: root= %d, %d nodes, %d links firstFreeLink: %d\n",
		 ind->ind_start, ind->indNodeCount, ind->indAllocatedLinks,
    ...
    */
	...
	for (i = 0; i <= ind->indLastHeadLinkSlot && i < ind->indAllocatedLinks;
	     i++) {
		indTLprint(ind, i);
	}
}
