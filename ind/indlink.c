...
static int indMergeFreeSlots(IND *ind, const int slot)
{
	TrieLink *const link = LINK(ind, slot);

	if (link->tl_to != TLgFREE) {
		LLLDEB(slot, link->tl_to, TLgFREE);
		return -1;
	}
...
static int indSlotCapacity(IND *ind, const int from)
{
	int next;
	int space;
...
static int indSplit(IND *ind, int slot, int size, int upto)
{
...
static void indClaimSlot(IND *ind, int slot, int size)
{
	TrieLink *link = LINK(ind, slot);
...
static int indClaimLinkPages(IND *ind, int n)
{
	int linksNeeded;
	int pagesNeeded;
	TrieLink **fresh;
	...
	while (ind->indAllocatedLinks < linksNeeded) {
		...
		link->tl_key =
			ind->indAllocatedLinks - ind->indLastHeadLinkSlot - 1;
		link->tl_to = TLgFREE;
	}
...
static void indReallocLinks(IND *ind, int slot, int from)
{
	TrieLink *link = LINK(ind, from);
