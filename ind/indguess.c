...
static int indSpelRememberGuess(const GuessContext *gc, int len, int score)
{
	...
	if (scc->sccForgotInd &&
	    indGetUtf8(&accepted, scc->sccForgotInd, copy) >= 0 && accepted) {
		return 0;
	}
...
static int indGuessAcceptToEndCost(const GuessContext *gc, int to, int from)
{
	int cost;
	...
	if (from > 0 && from == gc->gcSourceLen - 1 &&
		...
		cost = gc->gcDelCost * (gc->gcSourceLen - from);
	}

	return cost;
}

static int indGuessCanSkip(const GuessContext *gc, int *pN, int *pCost, int to,
			   int from)
{
	int cost;
	int n = 1;
...
static int indGuessInsertCost(const GuessContext *gc, int to, int from)
{
	if (from < gc->gcSourceLen && gc->gcSource[from] == gc->gcTarget[to]) {
...
static int indGuessCanReplace(const GuessContext *gc, int *pCost, int to,
			      int from)
{
	if (from < gc->gcSourceLen) {
...
static int indGuessCanSwap(const GuessContext *gc, int *pNext, int *pCost,
			   int next, int to, int from)
{
	if (from + 1 < gc->gcSourceLen &&
		...
		next = indINDstep(&ignored, gc->gcInd, next,
				  gc->gcSource[from]);
		if (next >= 0) {
			*pCost = gc->gcSwapCost;
			*pNext = next;
...
static void indGuessLogChar(const GuessContext *gc, int key)
{
...
static void indGuessLogTo(const char *label, const GuessContext *gc, int to)
{
	char scratch[100];
...
static void indGuessLogFrom(const char *label, const GuessContext *gc, int to)
{
	char scratch[100];
...
static void indIndGuessForNode(const GuessContext *gc, int tn, int to, int from,
			       const int score)
{
	if (tn < 0) {
		LDEB(tn);
	...
	while (tn >= 0) {
		...
		for (j = 0; j < (int)node->tn_ntrans; j++) {
			TrieLink *link =
				LINK(gc->gcInd, node->tn_transitions + j);
			int n;
			...
			if (from < gc->gcSourceLen &&
				...
				if (indGuessCanReplace(gc, &cost, to, from) &&
				    score >= cost) {
					indIndGuessForNode(gc, link->tl_to,
							   to + 1, from + 1,
							   score - cost);
				}

				/*  4  */
				if (indGuessCanSwap(gc, &next, &cost,
						    link->tl_to, to, from) &&
				    score >= cost) {
					gc->gcTarget[to + 1] =
						gc->gcSource[from];
					indIndGuessForNode(gc, next, to + 2,
							   from + 2,
							   score - cost);
				}
			}
...
int indINDguess(IND *ind, const unsigned short *source, int sourceLen,
		SpellGuessContext *sgc, int how)
{
	...
	if (ind->ind_start >= 0) {
		...
		indIndGuessForNode(&gc, ind->ind_start, 0, 0,
				   (3 * sourceLen) / 2 + 10);
	}
		      ...
		      const char *word)
{
	...
	if (igl->iglGuessCount > 6) {
		limit = (igl->iglGuesses[0].igsScore +
			 2 * igl->iglGuesses[igl->iglGuessCount - 1].igsScore) /
			3;
	} else {
		limit = 0;
	}
