...
void indINDfree(IND *ind)
{
	...
	if (ind->indMmappedFile) {
		...
		ret = vm_deallocate(task_self(),
				    (vm_address_t)ind->indMmappedFile,
				    ind->indMmappedSize);
		if (ret != KERN_SUCCESS) {
			LDEB(ret);
		}
...
/************************************************************************/
static int indWriteData(IND *ind, int fd, int swap)
{
	int n;
	int i;
	unsigned sz;
	...
	for (n = 0; n < ind->indAllocatedNodes; n += TNsBLOCK) {
		...
		if (swap) {
			for (i = 0; i < TNsBLOCK; i++) {
				...
				indSwapInt(
					(unsigned char *)&tn->tn_transitions);
			}
		}
...
int indINDwrite(IND *ind, const char *filename)
{
	...
	if (sizeof(TrieLink) != 8) {
		LLDEB(sizeof(TrieLink), 8);
		return -1;
	}
	if (sizeof(TrieNode) != 8) {
		LLDEB(sizeof(TrieNode), 8);
		return -1;
	}
	...
	utilEndianStoreLeInt32(ind->indAllocatedNodes,
			       hd + POS_NODES_ALLOCATED);
	utilEndianStoreLeInt32(ind->indAllocatedLinks,
			       hd + POS_LINKS_ALLOCATED);
	utilEndianStoreLeInt32(ind->indFirstFreeLinkSlot, hd + POS_FREE_LINK);
	utilEndianStoreLeInt32(ind->indLastHeadLinkSlot,
			       hd + POS_LAST_HEAD_LINK);
	...
	if (!rval) {
		...
		utilEndianStoreBeInt32(ind->indAllocatedNodes,
				       hd + POS_NODES_ALLOCATED);
		utilEndianStoreBeInt32(ind->indAllocatedLinks,
				       hd + POS_LINKS_ALLOCATED);
		utilEndianStoreBeInt32(ind->indFirstFreeLinkSlot,
				       hd + POS_FREE_LINK);
		utilEndianStoreBeInt32(ind->indLastHeadLinkSlot,
				       hd + POS_LAST_HEAD_LINK);
...
IND *indINDread(const char *filename, int readOnly)
{
	...
	if (machineBigEndian) {
		...
		if (ind->ind_magic == INDMAGIC) {
			ind->ind_start =
				utilEndianExtractBeInt32(hd + POS_START);
			ind->indNodeCount =
				utilEndianExtractBeInt32(hd + POS_NODE_COUNT);
			ind->indAllocatedNodes = utilEndianExtractBeInt32(
				hd + POS_NODES_ALLOCATED);
			ind->indAllocatedLinks = utilEndianExtractBeInt32(
				hd + POS_LINKS_ALLOCATED);
			ind->indFirstFreeLinkSlot =
				utilEndianExtractBeInt32(hd + POS_FREE_LINK);
			ind->indLastHeadLinkSlot = utilEndianExtractBeInt32(
				hd + POS_LAST_HEAD_LINK);
			...
			if (ind->ind_magic == INDMAGIC_R) {
				fileOff = sizeof(hd);
				fileOff += utilEndianExtractLeInt32(
						   hd + POS_NODES_ALLOCATED) *
					   sizeof(TrieNode);
				fileOff += utilEndianExtractLeInt32(
						   hd + POS_LINKS_ALLOCATED) *
				...
				ind->ind_magic = utilEndianExtractBeInt32(hd);
				if (ind->ind_magic != INDMAGIC) {
					XXDEB(ind->ind_magic, INDMAGIC);
					goto failure;
				}

				ind->ind_start = utilEndianExtractBeInt32(
					hd + POS_START);
				ind->indNodeCount = utilEndianExtractBeInt32(
					hd + POS_NODE_COUNT);
				ind->indAllocatedNodes =
		...
		if (ind->ind_magic == INDMAGIC) {
			ind->ind_start =
				utilEndianExtractLeInt32(hd + POS_START);
			ind->indNodeCount =
				utilEndianExtractLeInt32(hd + POS_NODE_COUNT);
			ind->indAllocatedNodes = utilEndianExtractLeInt32(
				hd + POS_NODES_ALLOCATED);
			ind->indAllocatedLinks = utilEndianExtractLeInt32(
				hd + POS_LINKS_ALLOCATED);
			ind->indFirstFreeLinkSlot =
				utilEndianExtractLeInt32(hd + POS_FREE_LINK);
			ind->indLastHeadLinkSlot = utilEndianExtractLeInt32(
				hd + POS_LAST_HEAD_LINK);
			...
			if (ind->ind_magic == INDMAGIC_R) {
				fileOff = sizeof(hd);
				fileOff += utilEndianExtractBeInt32(
						   hd + POS_NODES_ALLOCATED) *
					   sizeof(TrieNode);
				fileOff += utilEndianExtractBeInt32(
						   hd + POS_LINKS_ALLOCATED) *
				...
				ind->ind_start = utilEndianExtractLeInt32(
					hd + POS_START);
				ind->indNodeCount = utilEndianExtractLeInt32(
					hd + POS_NODE_COUNT);
				ind->indAllocatedNodes =
					utilEndianExtractLeInt32(
						hd + POS_NODES_ALLOCATED);
				ind->indAllocatedLinks =
					utilEndianExtractLeInt32(
						hd + POS_LINKS_ALLOCATED);
				ind->indFirstFreeLinkSlot =
					utilEndianExtractLeInt32(hd +
								 POS_FREE_LINK);
				ind->indLastHeadLinkSlot =
					utilEndianExtractLeInt32(
						hd + POS_LAST_HEAD_LINK);
	...
	mmapSize = memOff + ind->indAllocatedNodes * sizeof(TrieNode) +
	...
	if (readOnly) {
		...
		ret = map_fd(fd, (vm_offset_t)mmapOffset,
			     (vm_address_t *)&(ind->indMmappedFile), 1,
			     mmapSize);
		if (ret != KERN_SUCCESS) {
			LDEB(ret);
		} else {
	...
	if (readOnly) {
		...
		mmapResult = mmap((void *)0, mmapSize, PROT_READ, MAP_SHARED,
				  fd, mmapOffset);
