...
static int indRenumberNode(int tn, IND *oldIndex, IND *newIndex,
			   int *newNumbers)
{
	TrieNode *oldNode;
	TrieNode *newNode;
	...
	if (oldNode->tn_ntrans > 0) {
		int newTransitions =
			indTLalloc(newIndex, -1, oldNode->tn_ntrans);
		...
		for (i = 0; i < oldNode->tn_ntrans; i++) {
			TrieLink *oldLink =
				LINK(oldIndex, oldNode->tn_transitions + i);
			int newTo;
			TrieLink *newLink = LINK(newIndex, newTransitions + i);

			newTo = indRenumberNode(oldLink->tl_to, oldIndex,
						newIndex, newNumbers);
			if (newTo < 0) {
				LDEB(newTo);
				return -1;
...
IND *indINDrenumber(IND *ind)
{
	...
	rval->ind_start =
		indRenumberNode(ind->ind_start, ind, rval, newNumbers);
	if (rval->ind_start < 0) {
		LDEB(rval->ind_start);
