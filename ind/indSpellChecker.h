...
typedef void (*SpellComplain)(int error, const char *subject, void *through);

typedef int (*SpellCheckerSetup)(struct SpellChecker *sc,
				 SpellComplain complain, void *through);

typedef int (*SpellCheckerFindNext)(void *target, MemoryBuffer *mbFound,
				    const struct SpellChecker *sc,
				    SpellDictionary *sd);

typedef void (*SpellCheckerCorrect)(void *target, const char *guess);

typedef int (*SpellCheckerOpenDict)(SpellDictionary *sd, SpellComplain complain,
				    void *through,
				    const struct SpellChecker *sc);
...
typedef int (*SpellCheckerGuessWord)(IndGuessList *igl, SpellDictionary *sd,
				     SpellComplain complain, void *through,
				     const struct SpellChecker *sc,
				     const char *word);
