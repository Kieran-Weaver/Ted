...
static int docRtfSetRowProperties(RtfReader *rr, BufferItem *rowNode)
{
	PropertyMask rpDoneMask;

	rr->rrcRowProperties.rpShadingNumber =
		docItemShadingNumber(rr->rrDocument, &(rr->rrcRowShading));
...
static int docRtfCheckParagraph(RtfReader *rr, BufferItem *paraNode)
{
	...
	if (addEmptyParticule) {
		...
		if (!docInsertTextParticule(paraNode, part, stroff, count,
...
static int docRtfCloseSect(RtfReader *rr, BufferItem *sectNode)
{
	if (sectNode->biTreeType == DOCinBODY) {
		...
		if (docUpdSectProperties(
			    (PropertyMask *)0, &(sectNode->biSectProperties),
...
static int docRtfCloseRow(RtfReader *rr, BufferItem *rowNode)
{
	...
	if (docIsRowNode(rowNode) &&
		...
		while (rowNode->biRowCellCount < rowNode->biChildCount) {
			...
			const DocumentAttributeMap *const dam =
				(const DocumentAttributeMap *)0;

			docInitCellProperties(&cp);
			docCopyCellProperties(
				&cp,
				&(rowNode->biRowCells[rowNode->biRowCellCount -
						      1]),
				dam);

			/* Add zero width dummy columns to avoid crashes */
			while (rowNode->biRowCellCount <
			       rowNode->biChildCount) {
				const int shiftTail = 0;

				if (docInsertRowColumn(
					    &(rowNode->biRowProperties),
					    rowNode->biRowCellCount, shiftTail,
					    &cp, dam)) {
...
static int docRtfCloseCell(RtfReader *rr, BufferItem *cellNode)
{
	if (cellNode->biChildCount == 0) {
		LDEB(cellNode->biChildCount);
		return 0;
	}
...
static int docRtfCloseParagraph(RtfReader *rr, BufferItem *paraNode)
{
	RtfReadingState *rrs = rr->rrcState;
	...
	if (paraNode->biParaListOverride > 0) {
		...
		if (paraNode->biParaListOverride >= lot->lotOverrideCount) {
			LLDEB(paraNode->biParaListOverride,
			      lot->lotOverrideCount);
			return -1;
		}
		...
		if (docDelimitParaHeadField(&dfHead, &dsInsideHead,
					    ...
					    paraNode, bd)) {
			if (docInsertParaHeadField(&dfHead, &dsInsideHead,
						   &dsAroundHead, &partBegin,
						   &partEnd, paraNode, bd,
						   rr->rrcTree, DOCfkLISTTEXT,
						   textAttributeNumber)) {
		...
		if (!root) {
			LXDEB(paraNode->biParaListOverride, root);
			return -1;
		}

		if (docListNumberTreeInsertParagraph(
			    root, paraNode->biParaListLevel, paraNr)) {
			LDEB(paraNr);
		}
	}
...
static int docRtfCloseNode(RtfReader *rr, BufferItem *node)
{
	switch (node->biLevel) {
	case DOClevSECT:
...
static int docRtfPopNode(RtfReader *rr, BufferItem *node)
{
	while (rr->rrcNode) {
...
static int docRtfHierarchyCell(const RtfControlWord *rcw, RtfReader *rr)
{
	...
	if (rcw->rcwID == DOClevCELL && rr->rrcNode &&
	...
	if (!levelNode || (rr->rrcNode->biLevel <= DOClevCELL && paraInCell)) {
		...
		if (paraNode->biParaTableNesting == 0) {
			...
			if (rr->rrcState) {
				rr->rrcState->rrsParagraphProperties
					.ppTableNesting =
...
int docRtfHierarchy(const RtfControlWord *rcw, int arg, RtfReader *rr)
{
	...
	case DOClevNESTROW:
		...
		if (!levelNode) {
			LSDEB(rr->rrCurrentLine, rcw->rcwWord);
			return 0;
		}
		if (docRtfPopNode(rr, levelNode)) {
			SDEB(rcw->rcwWord);
...
static int docRtfStartParagraph(RtfReader *rr)
{
	...
	if (!paraNode) {
		SXDEB(docLevelStr(rr->rrcLevel), paraNode);
		return -1;
	}
	rr->rrcNode = paraNode;
	...
	if (paraNode->biParaFirstIndentTwips + paraNode->biParaLeftIndentTwips <
	    0) {
		paraNode->biParaFirstIndentTwips =
			-paraNode->biParaLeftIndentTwips;
	}
...
static int docRtfStartSect(RtfReader *rr)
{
	...
	if (!sectNode) {
		SXDEB(docLevelStr(rr->rrcLevel), sectNode);
		return -1;
	}
	rr->rrcNode = sectNode;
...
static int docRtfStartRow(RtfReader *rr, int forTable)
{
	BufferItem *rowNode;

	rowNode = docInsertNode(rr->rrDocument, rr->rrcNode, -1, DOClevROW);
	if (!rowNode) {
		SXDEB(docLevelStr(rr->rrcLevel), rowNode);
		return -1;
	}
...
static int docRtfStartCell(RtfReader *rr)
{
	...
	if (!cellNode) {
		SXDEB(docLevelStr(rr->rrcLevel), cellNode);
		return -1;
	}
	rr->rrcNode = cellNode;
...
static int docRtfStartNextCell(int *pRowNesting, int tableNestingTo,
			       RtfReader *rr)
{
	int rowNestingTo;
	...
	if (tableNestingTo > 0 && rr->rrcNode->biParent->biLevel == DOClevROW &&
	...
	if (rowNestingTo < *pRowNesting) {
		LLDEB(rowNestingTo, *pRowNesting);
		return -1;
	}
...
static int docRtfStartNode(int *pRowNesting, RtfReader *rr)
{
	RtfReadingState *rrs = rr->rrcState;
	int tableNestingTo;
