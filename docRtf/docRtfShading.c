...
static void docRtfSaveShading(RtfWriter *rwc, const ItemShading *is,
			      const char *const *patTags, int patTagCount,
			      const char *foreTag, const char *backTag,
			      const char *levelTag)
{
	if (is->isPattern != DOCspSOLID) {
		docRtfWriteEnumTag(rwc, patTags, is->isPattern, patTagCount,
				   PSshd_COUNT);
	}
			       ...
			       const char *levelTag)
{
	...
	docRtfSaveShading(rwc, &is, patTags, patTagCount, foreTag, backTag,
			  levelTag);
