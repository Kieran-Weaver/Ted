...
static int docRtfStyleName(RtfReader *rrc, const char *name, int len)
{
	...
	if (docRtfStoreSavedText(&(rrc->rrcStyle.dsName), &size, rrc,
				 removeSemicolon)) {
		LDEB(len);
		return -1;
	}
	...
	ds = docInsertStyle(&(rrc->rrDocument->bdStyleSheet),
			    rrc->rrcStyle.dsStyleNumber, &(rrc->rrcStyle),
			    dam0);
	if (!ds) {
		XDEB(ds);
		return -1;
...
static int docRtfStyleGroup(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	RtfReadingState *rrs = rrc->rrcState;
...
static int docRtfReadWordGroup(RtfReader *rrc, int gotArg, int arg,
...
int docRtfStylesheet(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	RtfReadingState *rrs = rrc->rrcState;
	int res;
	...
	for (;;) {
		...
		case RTFfiCTRLGROUP:
			rcw = docRtfFindWord(controlWord,
					     docRtfStylesheetGroups);
			if (!rcw) {
				if (docRtfReadWordGroup(rrc, gotArg, arg,
							controlWord,
							(RtfControlWord *)0,
							docRtfStyleName)) {
					SDEB(controlWord);
...
groupFound:
				res = docRtfApplyControlWord(rcw, gotArg, arg,
							     rrc);
				if (res < 0) {
					LDEB(res);
					SDEB(controlWord);
		...
		case RTFfiTEXTGROUP: {
			...
			if (docRtfReadGroupX(
				    (const RtfControlWord *)0, rcwApplyFirst,
				    gotArgApplyFirst, argApplyFirst, rrc,
				    docRtfStylesheetGroups, docRtfStyleName,
				    (RtfCommitGroup)0)) {
				LDEB(1);
				return -1;
			}
...
int docRtfStoreStyleProperties(RtfReader *rrc)
{
	...
	/*  CELL	*/
	ds->dsCellProps.cpShadingNumber =
		docItemShadingNumber(rrc->rrDocument, &(rrc->rrcCellShading));
