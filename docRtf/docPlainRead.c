...
static int docPlainReadParaContents(BufferDocument *bd, BufferItem *paraNode,
				    int c, SimpleInputStream *sis,
				    PlainReadingContext *prc)
{
	...
	for (;;) {
		...
		if (done > 0) {
			...
			if (docParaAppendText(bd, paraNode,
					      &(prc->prcTextAttribute),
					      &(prc->prcTextConverter),
					      (char *)bytes, size)) {
				LDEB(size);
				rval = -1;
				goto ready;
...
static int docPlainReadParagraph(BufferDocument *bd, SimpleInputStream *sis,
				 PlainReadingContext *prc)
{
	int c;
	BufferItem *paraNode;
	...
	if (!prc->prcParaNode) {
		...
		paraNode = docInsertNode(bd, prc->prcParaNode->biParent, -1,
					 DOClevPARA);
		if (!paraNode) {
			XDEB(paraNode);
			return -1;
	...
	if (paraNode->biParaParticuleCount == 0) {
		textAttrNr =
			docTextAttributeNumber(bd, &(prc->prcTextAttribute));

		if (!docInsertTextParticule(paraNode, 0, 0, 0, DOCkindSPAN,
					    textAttrNr)) {
				 ...
				 const DocumentGeometry *dgPaper)
{
    ...
    */
	...
	if (lc_ctype) {
		textConverterSetNativeEncodingName(&(prc.prcTextConverter),
						   lc_ctype);
	}

	bd = docNewFile(&(prc.prcTextAttribute), font, 2 * 9,
