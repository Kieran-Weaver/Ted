...
static int docRtfReadControlWord(RtfReader *rrc, int *pC, char *controlWord,
...
int docRtfFindControl(RtfReader *rrc, int *pC, char *controlWord, int *pGotArg,
		      int *pArg)
{
	SimpleInputStream *sis = rrc->rrcSis;
	...
	for (;;) {
		...
		case '\\':
			sioInUngetLastRead(sis);
			res = docRtfReadControlWord(rrc, &c, controlWord,
						    pGotArg, pArg);
		...
		case '{':
			...
			if (c == '\\') {
				sioInUngetLastRead(sis);
				res = docRtfReadControlWord(
					rrc, &c, controlWord, pGotArg, pArg);
				if (res < 0) {
					LDEB(res);
					return -1;
				}
...
static int docRtfAddParticule(RtfReader *rrc, const char *collectedText,
			      int len)
{
	RtfReadingState *rrs = rrc->rrcState;

	if (!rrc->rrcAddParticule) {
		XDEB(rrc->rrcAddParticule);
		return -1;
	}
...
static int docRtfReadText(int c, int *pC,

			  char *controlWord, int *pGotArg, int *pArg,
			  ...
			  RtfReader *rrc)
{
	...
	for (;;) {
		...
		case '{':
			sioInUngetLastRead(sis);
			res = docRtfFindControl(rrc, &c, controlWord, pGotArg,
						pArg);
			if (res < 0) {
				LDEB(res);
				return res;
		...
		case '\\':
			...
			if (res != RTFfiCHAR) {
				if (docRtfAddParticule(rrc, collectedText,
						       len)) {
					LDEB(len);
					return -1;
				}
		...
		default:
			if (len >= collectedSize) {
				...
				fresh = (char *)realloc(collectedText,
							size + 2);
				if (!fresh) {
					LXDEB(size, fresh);
					return -1;
...
int docRtfSkipGroup(const RtfControlWord *rcw, int ignored_arg, RtfReader *rrc)
{
	int rval = 0;
...
int docRtfReadGroupX(const RtfControlWord *rcw,
		     ...
		     RtfReader *rrc, const RtfControlWord *groupWords,
		     ...
		     RtfCommitGroup commitGroup)
{
	...
	rval = docRtfConsumeGroup(applyFirst, gotArg, arg, rrc, groupWords,
				  addParticule);
		    ...
		    RtfCommitGroup commitGroup)
{
	...
	rval = docRtfConsumeGroup((const RtfControlWord *)0, gotArg, arg, rrc,
				  groupWords, addParticule);
...
static int docRtfConsumeWord(RtfReader *rrc, const RtfControlWord *rcw, int *pC,
			     char *controlWord, int *pGotArg, int *pArg)
{
	int resAhead = -2;
	...
	if (rcw->rcwDetailWords) {
		for (;;) {
			...
			resAhead = docRtfFindControl(rrc, &c, controlWord,
						     &gotArgAhead, &argAhead);
			...
			if (rcwAhead->rcwType == RTCtypeDEST) {
				SLDEB(rcwAhead->rcwWord, rcwAhead->rcwType);
				break;
			}
...
int docRtfReadUnknownGroup(RtfReader *rrc)
{
	...
	if (docRtfReadGroup((const RtfControlWord *)0, 0, 0, rrc,
			    (RtfControlWord *)0, docRtfIgnoreText,
			    (RtfCommitGroup)0)) {
		LDEB(1);
		return -1;
	}
		       ...
		       RtfAddTextParticule addParticule)
{
	...
	if (applyFirst) {
		res = docRtfConsumeWord(rrc, applyFirst, &c, controlWord,
					&gotArg, &arg);
		if (res < 0) {
			LDEB(res);
			rval = -1;
	...
	for (;;) {
		...
		case RTFfiCHAR:
			res = docRtfReadText(c, &c, controlWord, &gotArg, &arg,
					     rrc);
			if (res < 0) {
				SLDEB(controlWord, res);
				rval = -1;
				goto ready;
			}
			break;
		...
		case RTFfiWORD:
			...
			if (!rcw) {
				...
				res = docRtfFindControl(rrc, &c, controlWord,
							&gotArg, &arg);
				if (res < 0) {
					LDEB(res);
					rval = -1;
				...
				if (rcw->rcwType == RTCtypeDEST) {
					...
					res = docRtfConsumeWord(rrc, rcw, &c,
								controlWord,
								&gotArg, &arg);
					if (res < 0) {
						LDEB(res);
...
groupFound:
					if (rrc->rrcInIgnoredGroup) {
						...
						res = docRtfApplyControlWord(
							rcw, gotArg, arg, rrc);
					}
					if (res < 0) {
						LSDEB(res, controlWord);
						rval = -1;
						goto ready;
					}
				} else {
					if (docRtfReadGroupX(
						    rcw, rcw, gotArg, arg, rrc,
						    groupWords, addParticule,
						    (RtfCommitGroup)0)) {
						SDEB(rcw->rcwWord);
						return -1;
			...
			c = 0x0;
			res = docRtfFindControl(rrc, &c, controlWord, &gotArg,
						&arg);
			if (res < 0) {
				LDEB(res);
				rval = -1;
...
textGroup:
			if (docRtfReadGroupX((const RtfControlWord *)0,
					     rcwApplyFirst, gotArgApplyFirst,
					     argApplyFirst, rrc, groupWords,
				   ...
				   RtfReader *rrc)
{
	...
	if (docRtfReadGroup(rcw, gotArg, arg, rrc, (const RtfControlWord *)0,
			    docRtfSaveDocEncodedText, rcw->rwcCommitGroup)) {
		SDEB(rcw->rcwWord);
		return -1;
	}
...
int docRtfApplyRawBytesGroup(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	...
	if (docRtfReadGroup(rcw, gotArg, arg, rrc, (const RtfControlWord *)0,
			    docRtfSaveRawBytes, rcw->rwcCommitGroup)) {
		SDEB(rcw->rcwWord);
		return -1;
	}
...
int docRtfIgnoreWord(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	return 0;
}
