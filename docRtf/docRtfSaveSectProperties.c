...
void docRtfSaveSectionProperties(RtfWriter *rw, const PropertyMask *spMask,
				 const SectionProperties *sp)
{
	...
	if (PROPmaskISSET(spMask, SPpropNUMBER_STYLE)) {
		docRtfWriteEnumTag(rw, DOCrtf_PageNumberStyleTags,
				   ...
				   DOCrtf_PageNumberStyleTagCount,
				   DOCpgn_COUNT);
	}
	...
	if (PROPmaskISSET(spMask, SPpropNUMBER_HYPHEN)) {
		docRtfWriteEnumTag(rw, DOCrtf_PageNumberHyphenTags,
				   ...
				   DOCrtf_PageNumberHyphenTagCount,
				   DOCpgnh_COUNT);
	}
	...
	if (PROPmaskISSET(spMask, SPpropCOLUMNS) || sp->spColumnCount > 1) {
		int i;
		...
		for (i = 0; i < sp->spColumnCount; sc++, i++) {
			...
			if (sc->scColumnWidthTwips != 0) {
				docRtfWriteArgTag(rw, "colw",
						  sc->scColumnWidthTwips);
			}
			if (sc->scSpaceToRightTwips != 0) {
				docRtfWriteArgTag(rw, "colsr",
						  sc->scSpaceToRightTwips);
			}
		}
	}
...
int docRtfSaveSectionPropertiesOfNode(RtfWriter *rw,
...
static int docRtfSaveSectHeaderFooter(RtfWriter *rw, const BufferItem *sectNode,
				      const BufferItem *prevBi, const char *tag,
				      int treeType)
{
	...
	if (dt) {
		if (docRtfSaveDocumentTree(rw, tag, dt, evenIfAbsent,
					   forcePar)) {
			LSDEB(treeType, tag);
			return -1;
		}
...
int docRtfSaveSectHeadersFooters(RtfWriter *rw, const BufferItem *sectNode)
{
	...
	if (sectNode->biParent && sectNode->biNumberInParent > 0) {
		prevBi = sectNode->biParent
				 ->biChildren[sectNode->biNumberInParent - 1];
	}

	/*  Word 11+ uses right header/footer anyway. For compatibility: */
	if (!dp->dpHasFacingPages) {
		if (docRtfSaveSectHeaderFooter(rw, sectNode, prevBi, "header",
					       DOCinRIGHT_HEADER)) {
			LDEB(1);
			return -1;
		}
	...
	/*  Word 11+ uses right header/footer anyway. For compatibility: */
	if (!dp->dpHasFacingPages) {
		if (docRtfSaveSectHeaderFooter(rw, sectNode, prevBi, "footer",
					       DOCinRIGHT_FOOTER)) {
			LDEB(1);
			return -1;
		}
...
int docRtfSaveHeaderFooter(RtfWriter *rw, const DocumentTree *dt)
{
	const int forcePar = 1;
	...
	for (hdft = 0; hdft < HeaderFooterTypeCount; hdft++) {
		...
		if (docRtfSaveDocumentTree(rw, hft->hftTag, dt, evenIfAbsent,
					   forcePar)) {
			SDEB(hft->hftTag);
			return -1;
		}
