...
int docRtfObjectProperty(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	...
	if (!io) {
		LSXDEB(rrc->rrCurrentLine, rcw->rcwWord,
		       rrc->rrcInsertedObjectNr);
		return 0;
	}
...
int docRtfPictureProperty(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
...
static int docRtfReadPicprop(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int rval;
	...
	rval = docRtfConsumeGroup((const RtfControlWord *)0, 0, -1, rrc,
				  docRtfPicpropGroups, docRtfIgnoreText);
	if (rval) {
...
static int docRtfSaveObjectData(RtfReader *rrc, const char *text, int len)
{
	InsertedObject *io;

	io = docGetObject(rrc->rrDocument, rrc->rrcInsertedObjectNr);
	if (!io) {
		LXDEB(rrc->rrcInsertedObjectNr, io);
		return -1;
	}
...
static int docRtfSaveResultData(RtfReader *rrc, const char *text, int len)
{
	InsertedObject *io;
...
int docRtfReadPict(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int rval = 0;
...
int docRtfReadShpXPict(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int rval;
	int ignore = 0;
	...
	rval = docRtfConsumeGroup((const RtfControlWord *)0, 0, -1, rrc,
				  (const RtfControlWord *)0, docRtfIgnoreText);
	if (rval) {
		SLDEB(rcw->rcwWord, rval);
	}
...
static int docRtfReadResultPict(const RtfControlWord *rcw, int arg,
				RtfReader *rrc)
{
	int rval;
	...
	if (rrc->rrcInIgnoredGroup) {
		...
		addParticule = docRtfSaveResultData;
	}
...
static int docRtfReadResultXPpict(const RtfControlWord *rcw, int arg,
				  RtfReader *rrc)
{
	int rval;
	int ignore = 0;
...
static int docRtfSaveObjectClass(RtfReader *rrc, const char *text, int len)
{
	InsertedObject *io;
...
static int docRtfSaveObjectName(RtfReader *rrc, const char *text, int len)
{
	InsertedObject *io;
...
static int docRtfObjectClass(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int rval;

	rval = docRtfConsumeGroup((const RtfControlWord *)0, 0, -1, rrc,
				  (RtfControlWord *)0, docRtfSaveObjectClass);
...
static int docRtfObjectName(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int rval;

	rval = docRtfConsumeGroup((const RtfControlWord *)0, 0, -1, rrc,
				  (RtfControlWord *)0, docRtfSaveObjectName);
	if (rval) {
		SLDEB(rcw->rcwWord, rval);
	}
...
static int docRtfObjectData(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int rval;

	rval = docRtfConsumeGroup((const RtfControlWord *)0, 0, -1, rrc,
				  (RtfControlWord *)0, docRtfSaveObjectData);
	if (rval) {
		SLDEB(rcw->rcwWord, rval);
	}
...
static int docRtfObjectResult(const RtfControlWord *rcw, int arg,
			      RtfReader *rrc)
{
	int rval;

	rval = docRtfConsumeGroup((const RtfControlWord *)0, 0, -1, rrc,
				  docRtfResultGroups, docRtfIgnoreText);
	if (rval) {
		SLDEB(rcw->rcwWord, rval);
	}
...
int docRtfReadObject(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	...
	rval = docRtfConsumeGroup((const RtfControlWord *)0, 0, -1, rrc,
				  docRtfObjectGroups, docRtfSaveObjectData);
