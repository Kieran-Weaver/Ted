	...
	for (i = 0; i < dss->dssStyleCount; i++) {
		...
		case DOClevSPAN:
			docRtfWriteArgDestinationBegin(rwc, "*\\cs",
						       ds->dsStyleNumber);
			docRtfSaveTextAttribute(rwc, &(ds->dsTextMask),
						&(ds->dsTextAttribute));
			break;

		case DOClevPARA:
			if (ds->dsStyleNumber > 0) {
				docRtfWriteArgDestinationBegin(
					rwc, "s", ds->dsStyleNumber);
			} else {
		...
		case DOClevSECT:
			...
			docRtfSaveSectionProperties(rwc, &(ds->dsSectMask),
						    &(ds->dsSectProps));
			docRtfSaveParagraphProperties(rwc, &(ds->dsParaMask),
						      &(ds->dsParaProps));
			docRtfSaveTextAttribute(rwc, &(ds->dsTextMask),
		...
		docRtfWriteDocEncodedString(rwc, ds->dsName,
					    strlen(ds->dsName));
