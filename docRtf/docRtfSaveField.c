				      ...
				      const InstructionsComponent *ic)
{
	...
	docRtfWriteDocEncodedString(rwc, (const char *)ic->icBuffer.mbBytes,
				    ic->icBuffer.mbSize);
	if (ic->icIsQuoted) {
		docRtfWriteDocEncodedString(rwc, "\"", 1);
	}
...
static void docRtfWriteStartField(RtfWriter *rwc, const DocumentField *df)
{
...
static void docRtfBookmarkTag(RtfWriter *rwc, const char *tag,
			      const DocumentField *df)
{
	...
	if (!docFieldGetBookmark(&mbBookmark, df)) {
		docRtfWriteDocEncodedStringDestination(
			rwc, tag, (const char *)mbBookmark->mbBytes,
...
static void docRtfStartTc(RtfWriter *rwc, const DocumentField *df)
{
...
static int docRtfSaveNoteField(RtfWriter *rw, int *pCount,
			       const BufferItem *paraNode, const int part,
			       const TextParticule *tp, const DocumentField *df)
{
	...
	if (df->dfNoteIndex >= 0 && dn->dnDocumentTree.dtRoot) {
		...
		if (paraNode->biTreeType == DOCinFOOTNOTE ||
			...
			case DOCinFOOTNOTE:
				if (docRtfSaveDocumentTree(
					    rw, RTFtag_footnote,
					    &(dn->dnDocumentTree), evenIfAbsent,
					    forcePar)) {
					LDEB(np->npTreeType);
					return -1;
				}
...
int docRtfSaveFieldHead(RtfWriter *rwc, const BufferItem *paraNode, int part)
{
	const DocumentField *df;
	...
	if (df->dfKind == DOCfkLISTTEXT && paraNode->biParaListOverride > 0) {
		if (docRtfPushAttribute(rwc)) {
			LDEB(1);
		}
	...
	if (fki->fkiIsDestinationInRtf && fki->fkiRtfTag) {
		docRtfWriteDestinationBegin(rwc, fki->fkiRtfTag);

		docRtfWriteDocEncodedString(rwc, (char *)df->dfData.mbBytes,
					    df->dfData.mbSize);
...
int docRtfSaveFieldTail(RtfWriter *rwc, const BufferItem *paraNode, int part)
{
	const DocumentField *df;
	const FieldKindInformation *fki;
	const TextParticule *tp = paraNode->biParaParticules + part;
