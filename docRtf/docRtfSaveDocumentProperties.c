...
static int docRtfSaveInfo(RtfWriter *rw, const char *tag,
...
int docRtfSaveDocumentProperties(RtfWriter *rw, int fet,
				 ...
				 const DocumentProperties *dp)
{
    ...
    */
	...
	if (PROPmaskISSET(dpMask, DPpropLISTOVERRIDETABLE)) {
		docRtfWriteNextLine(rw);
		docRtfWriteListOverrideTable(
			rw, &(dp->dpListAdmin->laListOverrideTable));
	}
	...
	if (PROPmaskISSET(dpMask, DPpropTITLE) ||
		...
		if (PROPmaskISSET(dpMask, DPpropHLINKBASE)) {
			docRtfSaveInfo(rw, RTFtag_hlinkbase,
				       &(dp->dpHlinkbase));
		}
	...
	if (PROPmaskISSET(dpMask, DPpropTOP_BORDER)) {
		docRtfSaveBorderByNumber(rw, "pgbrdrt", dp->dpTopBorderNumber,
					 anyway);
	}
	...
	if (PROPmaskISSET(dpMask, DPpropLEFT_BORDER)) {
		docRtfSaveBorderByNumber(rw, "pgbrdrl", dp->dpLeftBorderNumber,
					 anyway);
	}
	if (PROPmaskISSET(dpMask, DPpropRIGHT_BORDER)) {
		docRtfSaveBorderByNumber(rw, "pgbrdrr", dp->dpRightBorderNumber,
					 anyway);
	}
...
int docRtfSaveDocNotesSeparators(RtfWriter *rw, const BufferDocument *bd)
{
	...
	if (docRtfSaveDocumentTree(rw, "ftncn", &(bd->bdEiFtncn), evenIfAbsent,
				   forcePar)) {
		LDEB(1);
		return -1;
	}
...
int docRtfDocPropMask(PropertyMask *dpSaveMask, const DocumentProperties *dpDoc)
{
	int rval = 0;
	...
	if (docUpdDocumentProperties(dpSaveMask, &dpDef, &dpSetMask, dpDoc,
				     dam0)) {
		LDEB(1);
		rval = -1;
		goto ready;
