...
static void docRtfInitReader(RtfReader *rr)
{
	...
	rr->rrcColor.rgb8Red = rr->rrcColor.rgb8Green = rr->rrcColor.rgb8Blue =
		0;
	...
	utilStartPagedList(&(rr->rrcEncodedFontList), sizeof(EncodedFont),
...
void docRtfPushReadingState(RtfReader *rr, RtfReadingState *rrs)
{
	...
	if (rr->rrcState) {
		...
		docCopyTabStopList(&(rrs->rrsTabStopList),
				   &(above->rrsTabStopList));
