...
static int docRtfSetAnsicpg(struct TextConverter *tc, int ansicpg)
{
	char scratch[20];
...
static int docRtfReadSetDocumentCharset(RtfReader *rrc, int arg)
{
...
int docRtfCommitDocPropText(const RtfControlWord *rcw, RtfReader *rrc)
{
	int rval = 0;
	...
	if (rrc->rrcTraceInProps) {
		...
		if (docSetDocumentPropertyString(
			    &(rrc->rrDocument->bdProperties), rcw->rcwID, text,
			    size)) {
			SDEB(rcw->rcwWord);
			rval = -1;
			goto ready;
...
int docRtfDocTimeGroup(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
...
int docRtfReadTimeField(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	switch (rcw->rcwID) {
	case RTFidHR:
...
int docRtfSelectionOpen(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
