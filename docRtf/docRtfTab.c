...
int docRtfApplyRawBytesGroup(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	return 0;
}
...
#define RTF_BORDER(s, id, sp)                                 \
	{                                                     \
		s, id, RTCtypeANY, sp, 0, docRtfBeginBorder,  \
			(RtfCommitGroup)0, docRtfBorderWords, \
	}
...
#define RTF_TEXT_BORDER(s, id) RTF_BORDER(s, id, docRtfRememberTextProperty)
...
#define RTF_CELL_BORDER(s, id) RTF_BORDER(s, id, docRtfRememberCellProperty)
...
#define RTF_ROW_BORDER(s, id) RTF_BORDER(s, id, docRtfRememberRowProperty)
...
#define RTF_DOC_BORDER(s, id) RTF_BORDER(s, id, docRtfRememberDocProperty)
...
#define RTF_SECT_FLAG(s, id)                                       \
	...
	RTF_SECT_ENUM("sftnrstpg", SPpropFOOTNOTE_RESTART, FTN_RST_PER_PAGE),
	...
	RTF_DOC_ENUM("endnotes", DPpropFOOTNOTE_PLACEMENT, FTNplaceSECT_END),
	RTF_DOC_ENUM("enddoc", DPpropFOOTNOTE_PLACEMENT, FTNplaceDOC_END),
	RTF_DOC_ENUM("ftntj", DPpropFOOTNOTE_JUSTIFICATION,
		     FTNjustifyBELOW_TEXT),
	RTF_DOC_ENUM("ftnbj", DPpropFOOTNOTE_JUSTIFICATION,
		     FTNjustifyPAGE_BOTTOM),

	RTF_DOC_ENUM("aendnotes", DPpropENDNOTE_PLACEMENT, FTNplaceSECT_END),
	RTF_DOC_ENUM("aenddoc", DPpropENDNOTE_PLACEMENT, FTNplaceDOC_END),
	...
	RTF_DOC_ENUM("ftnrstpg", DPpropFOOTNOTE_RESTART, FTN_RST_PER_PAGE),
	RTF_DOC_ENUM("ftnrestart", DPpropFOOTNOTE_RESTART, FTN_RST_PER_SECTION),
	RTF_DOC_ENUM("ftnrstcont", DPpropFOOTNOTE_RESTART, FTN_RST_CONTINUOUS),

	RTF_DOC_ENUM("aftnrstcont", DPpropENDNOTE_RESTART, FTN_RST_CONTINUOUS),
	RTF_DOC_ENUM("aftnrestart", DPpropENDNOTE_RESTART, FTN_RST_PER_SECTION),
...
#define RTF_LIST_FLAG(s, id)                                    \
	...
	RTF_DEST_XX("list", DPpropLISTTABLE, docRtfRememberList),
	RTF_DEST_XX("listlevel", DLpropLEVELS, docRtfRememberListLevel),

	RTF_TEXT_GROUP("leveltext", LLpropTEXT, docRtfCommitListLevelText),
...
#define RTF_SHAPE_ENUM(s, id, v)                          \
	...
	RTF_DEST_XX("listoverridetable", DPpropLISTOVERRIDETABLE,
    ...
    */
	...
	RTF_TEXT_GROUP(RTFtag_hlinkbase, DPpropHLINKBASE,
		       docRtfCommitDocPropText),
	/****************************************/
...
#define RTF_NEW_SCOPE_NUMBER(s, id)                   \
	{                                             \
		s, SSprop_COUNT + id, RTCtypeVALUE,   \
...
const char *const DOCrtf_DocFootNotesNumberStyleTags[] = {
	"ftnnar", "ftnnalc", "ftnnauc", "ftnnrlc", "ftnnruc", "ftnnchi",
};
...
const char *const DOCrtf_DocEndNotesNumberStyleTags[] = {
	"aftnnar", "aftnnalc", "aftnnauc", "aftnnrlc", "aftnnruc", "aftnnchi",
};
...
const char *const DOCrtf_SectFootNotesNumberStyleTags[] = {
	"sftnnar", "sftnnalc", "sftnnauc", "sftnnrlc", "sftnnruc", "sftnnchi",
};
...
const char *const DOCrtf_SectEndNotesNumberStyleTags[] = {
	"saftnnar",  "saftnnalc", "saftnnauc",
	"saftnnrlc", "saftnnruc", "saftnnchi",
};
...
const char *const DOCrtf_BorderStyleTags[] = {
	"brdrnone",	"brdrs",	"brdrth",     "brdrsh",
	"brdrdb",	"brdrhair",	"brdrdot",    "brdrdash",
	"brdrdashd",	"brdrdashdd",	"brdrtriple", "brdrtnthsg",
	"brdrthtnsg",	"brdrtnthtnsg", "brdrtnthmg", "brdrthtnmg",
	"brdrtnthtnmg", "brdrtnthlg",	"brdrthtnlg", "brdrtnthtnlg",
	"brdrwavy",	"brdrwavydb",	"brdrdashsm", "brdrdashdotstr",
	"brdremboss",	"brdrengrave",	"brdrframe",  "brdroutset",
	"brdrtbl",	"brdrnil",
};
...
const char *const DOCrtf_ParaShadingTags[] = {
	(const char *)0, /*  solid	*/
	"bghoriz",	 "bgvert",    "bgfdiag",   "bgbdiag",
	"bgcross",	 "bgdcross",  "bgdkhoriz", "bgdkvert",
...
const char *const DOCrtf_TextShadingTags[] = {
	(const char *)0, /*  solid	*/
	"chbghoriz",	 "chbgvert",	"chbgfdiag",   "chbgbdiag",
	"chbgcross",	 "chbgdcross",	"chbgdkhoriz", "chbgdkvert",
	"chbgdkfdiag",	 "chbgdkbdiag", "chbgdkcross", "chbgdkdcross",
};
...
const char *const DOCrtf_RowShadingTags[] = {
	(const char *)0, /*  solid	*/
	"trbghoriz",	 "trbgvert",	"trbgfdiag",   "trbgbdiag",
	"trbgcross",	 "trbgdcross",	"trbgdkhor",   "trbgdkvert",
	"trbgdkfdiag",	 "trbgdkbdiag", "trbgdkcross", "trbgdkdcross",
};
...
const char *const DOCrtf_SectBreakTags[] = {
	"sbknone", "sbkcol", "sbkpage", "sbkeven", "sbkodd",
};
...
const char *const DOCrtf_PageNumberHyphenTags[] = {
	"pgnhnsh", "pgnhnsp", "pgnhnsc", "pgnhnsm", "pgnhnsn",
};
...
const char *const DOCrtf_TabLeaderTags[] = {
	(const char *)0, "tldot", "tlul", "tlhyph", "tlth", "tleq",
};
...
static int docRtfMakeTagHash(void)
{
	...
	keyStrings = (const unsigned char **)malloc(
		keyStringCount * sizeof(const unsigned char *));
	if (!keyStrings) {
		LXDEB(keyStringCount, keyStrings);
		rval = -1;
		goto ready;
	}
	...
	for (i = 0; i < keyStringCount; i++) {
		keyStrings[i] =
			(const unsigned char *)docRtfPropertyWords[i].rcwWord;
	}

	if (utilJenkinsPerfectHash(keyStrings, keyStringCount,
				   "docRtfTagHash.c", "\"docRtfTagHash.h\"",
				   "docRtfTagHash", "docRtfTagIndex")) {
		LDEB(keyStringCount);
		rval = 0;
	}
...
static int docRtfCheckTagHash(void)
{
	...
	for (i = 0; i < keyStringCount; i++) {
		...
		if (idx != i) {
			...
			LSLLDEB(i, docRtfPropertyWords[i].rcwWord, idx, hash);
			rval = -1;
		}
	}
...
const RtfControlWord *docRtfFindPropertyWord(const char *controlWord)
{
	...
	if (checked > 0) {
		...
		keyStringCount =
			sizeof(docRtfPropertyWords) / sizeof(RtfControlWord) -
			1;
