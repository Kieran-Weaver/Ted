...
void docRtfSaveCellProperties(RtfWriter *rw, const PropertyMask *cpSetMask,
			      const CellProperties *cpSet, int shiftLeft)
{
	const int anyway = 0;

	if (PROPmaskISSET(cpSetMask, CLpropVALIGN)) {
		docRtfWriteEnumTag(rw, DOCrtf_CellValignTags, cpSet->cpValign,
	...
	if (PROPmaskISSET(cpSetMask, CLpropBOTTOM_PADDING_UNIT) ||
		...
		docRtfSaveAutoSpace(rw, "clpadfb", cpSet->cpBottomPaddingUnit,
				    "clpadb", cpSet->cpBottomPadding);
	}
	if (PROPmaskISSET(cpSetMask, CLpropCLFTS_WIDTH) ||
	    PROPmaskISSET(cpSetMask, CLpropCLW_WIDTH)) {
		docRtfSaveAutoSpace(rw, "clftsWidth",
				    cpSet->cpPreferredWidthUnit, "clwWidth",
				    cpSet->cpPreferredWidth);
	}

	if (PROPmaskISSET(cpSetMask, CLpropSHADING)) {
		docRtfSaveShadingByNumber(rw, cpSet->cpShadingNumber,
					  DOCrtf_CellShadingTags,
					  DOCrtf_CellShadingTagCount, "clcfpat",
					  "clcbpat", "clshdng");
	}
