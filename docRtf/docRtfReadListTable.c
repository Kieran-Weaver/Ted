...
int docRtfCommitListLevelText(const RtfControlWord *rcw, RtfReader *rr)
{
	...
	if (docRtfMemoryBufferSetText(&(rr->rrcListLevelText), rr,
				      removeSemicolon)) {
		LDEB(1);
		return -1;
	}
...
int docRtfCommitListLevelNumbers(const RtfControlWord *rcw, RtfReader *rr)
{
	...
	if (docRtfMemoryBufferSetText(&(rr->rrcListLevelNumbers), rr,
				      removeSemicolon)) {
		LDEB(1);
		return -1;
	}
...
int docRtfCommitListName(const RtfControlWord *rcw, RtfReader *rr)
{
	...
	if (docRtfMemoryBufferSetText(&(rr->rrcDocumentList.dlListName), rr,
				      removeSemicolon)) {
		LDEB(1);
		return -1;
	}
...
int docRtfRememberListLevelProperty(const RtfControlWord *rcw, int arg,
				    RtfReader *rr)
{
	if (docSetListLevelProperty(&(rr->rrcDocumentListLevel), rcw->rcwID,
				    arg)) {
		SDEB(rcw->rcwWord);
	}
					   ...
					   RtfReader *rr)
{
	...
	if (docListLevelSetStyle(
		    &(rr->rrcDocumentListLevel), &(rr->rrcStyle.dsParaMask),
		    &(rrs->rrsParagraphProperties), &(rr->rrcStyle.dsTextMask),
...
int docRtfRememberListLevel(const RtfControlWord *rcw, int arg, RtfReader *rr)
{
	...
	if (docDocumentListAddLevel(&(rr->rrcDocumentList),
				    &(rr->rrcDocumentListLevel), copyIds,
...
{
	if (docRtfReadGroup(rcw, 0, 0, rr, (const RtfControlWord *)0,
