...
static int docRtfSaveColor(RtfReader *rrc, const char *text, int len)
{
...
int docRtfColorIgnored(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	return 0;
}

int docRtfColorComp(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	switch (rcw->rcwID) {
	case RGBAcompRED:
...
int docRtfColorTable(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	rrc->rrcGotComponent = 0;
