				      ...
				      RtfReader *rrc)
{
	...
	if (docSetShadingProperty(&(rrs->rrsParagraphShading), rcw->rcwID,
				  arg) < 0) {
...
{
	rrs->rrsParagraphProperties.ppShadingNumber =
		docItemShadingNumber(bd, &(rrs->rrsParagraphShading));

	rrs->rrsParagraphProperties.ppTabStopListNumber =
		docTabStopListNumber(bd, &(rrs->rrsTabStopList));

	rrs->rrsParagraphProperties.ppFrameNumber =
		docFramePropertiesNumber(bd, &(rrs->rrsParaFrameProperties));
			    ...
			    int breakOverride)
{
	...
	/*  3  */
	if (docUpdParaProperties((PropertyMask *)0, pp, &ppSetMask,
				...
				RtfReadingState *rrs, int breakOverride)
{
	...
	if (docRtfSetParaProperties(&(paraNode->biParaProperties), bd,
				    mindTable, rrs, breakOverride)) {
		LDEB(1);
		return -1;
	}
	...
	if (paraNode->biParaTableNesting > 0) {
		if (!rowNode->biRowForTable) {
			...
			if (paraNode->biNumberInParent > 0 ||
				...
				docCleanRowProperties(
					&(rowNode->biRowProperties));
				docInitRowProperties(
					&(rowNode->biRowProperties));
			}
		...
		if (rowNode->biRowForTable) {
			if (rowNode->biChildCount == 1) {
				...
				LLDEB(paraNode->biParaTableNesting,
