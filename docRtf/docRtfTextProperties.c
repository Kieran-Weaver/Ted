...
static int docRtfRememberTextPropertyImpl(TextAttribute *ta,
					  const RtfControlWord *rcw, int arg,
					  RtfReader *rrc)
{
	switch (rcw->rcwID) {
	case TAprop_NONE:
				      ...
				      RtfReader *rrc)
{
	...
	if (docSetShadingProperty(&(rrs->rrsTextShading), rcw->rcwID, arg) <
	    0) {
		SLDEB(rcw->rcwWord, arg);
		return -1;
	}
			       ...
			       RtfReader *rrc)
{
	...
	if (rcw->rcwID == TApropTEXT_STYLE) {
		...
		if (arg >= 0 && arg < dss->dssStyleCount) {
			...
			if (ds && ds->dsLevel == DOClevSPAN) {
				...
				utilUpdateTextAttribute(
					&doneMask, &(rrs->rrsTextAttribute),
					&(ds->dsTextMask),
					&(ds->dsTextAttribute));
			}
		}
	...
	if (rcw->rcwID == TApropFONT_NUMBER) {
		...
		if (!(rrc->rrReadFlags & RTFflagUNENCODED)) {
			if (!docRtfReadMapFont(
				    rrc,
				    &(rrc->rrcCurrentEncodedFont
					      .ecBufFontNumber),
	...
	if (docRtfRememberTextPropertyImpl(&(rrs->rrsTextAttribute), rcw, arg,
					   rrc)) {
		SDEB(rcw->rcwWord);
		return -1;
	}
				 ...
				 RtfReader *rrc)
{
	...
	if (docRtfRememberTextPropertyImpl(
		    &(rrc->rrcParagraphNumber.pnTextAttribute), rcw, arg,
		    rrc)) {
		SDEB(rcw->rcwWord);
		return -1;
	}
...
void docRtfRefreshTextShading(RtfReader *rrc, RtfReadingState *rrs)
{
	rrs->rrsTextAttribute.taShadingNumber =
		docItemShadingNumber(rrc->rrDocument, &(rrs->rrsTextShading));
	rrs->rrsTextShadingChanged = 0;
