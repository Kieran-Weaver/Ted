					...
					const TextAttribute *ta)
{
	...
	if (PROPmaskISSET(updMask, TApropBORDER)) {
		...
		docRtfSaveBorderByNumber(rw, "chbrdr", ta->taBorderNumber,
					 anyway);
	}
	...
	if (PROPmaskISSET(updMask, TApropSHADING)) {
		...
		if (is.isBackColor > 0 && is.isPattern == DOCspSOLID &&
		    is.isLevel == 0) {
			docRtfWriteArgTag(rw, "cb", is.isBackColor);
		}

		docRtfSaveShadingByNumber(rw, ta->taShadingNumber,
					  DOCrtf_TextShadingTags,
					  DOCrtf_TextShadingTagCount, "chcfpat",
					  "chcbpat", "chshdng");
	}
	...
	if (PROPmaskISSET(updMask, TApropBASELINE_SHIFT)) {
		if (ta->taBaselineShiftHalfPoints >= 0) {
			docRtfWriteArgTag(rw, "up",
					  ta->taBaselineShiftHalfPoints);
		} else {
			docRtfWriteArgTag(rw, "dn",
					  -ta->taBaselineShiftHalfPoints);
		}
	}
...
void docRtfSaveTextAttribute(RtfWriter *rw, const PropertyMask *updMask,
			     const TextAttribute *taDoc)
{
	...
	if (!(rw->rwSaveFlags & RTFflagUNENCODED) &&
		...
		if (df) {
			...
			fileFontNumber =
				docRtfWriteGetCharset(rw, &charset, df, symbol);
			if (fileFontNumber < 0) {
				SLXDEB(df->dfName, fileFontNumber, symbol);
			} else {
				taFile.taFontNumber = fileFontNumber;
			}
		}
	}
...
void docRtfWriteSwitchTextAttributes(RtfWriter *rw, int textAttrNr,
				     const char *first)
{
	const BufferDocument *bd = rw->rwDocument;
	TextAttribute ta;
	...
	if (ta.taFontNumber >= 0) {
		...
		if (PROPmaskISSET(&doneMask, TApropSHADING)) {
			if (!docShadingNumberIsShading(bd,
						       ta.taShadingNumber)) {
				docRtfWriteTag(rw, RTFtag_plain);
				utilInitTextAttribute(&(rw->rwTextAttribute));
				utilPropMaskClear(&doneMask);
				utilUpdateTextAttribute(&doneMask,
							&(rw->rwTextAttribute),
							&taSetMask, &ta);
			}
		}
		...
		if (!(rw->rwSaveFlags & RTFflagUNENCODED)) {
			...
			if (df) {
				...
				fileFontNumber = docRtfWriteGetCharset(
					rw, &charset, df, symbol);
				if (fileFontNumber < 0) {
					SLXDEB(df->dfName, fileFontNumber,
					       symbol);
				} else {
					ta.taFontNumber = fileFontNumber;
				}

				if (rw->rwTextCharset != charset) {
					rw->rwTextCharset = charset;
					PROPmaskADD(&doneMask,
						    TApropFONT_NUMBER);
				}
