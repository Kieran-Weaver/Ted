...
static int docRtfCommitFaltName(const RtfControlWord *rcw, RtfReader *rrc)
{
	const int removeSemicolon = 0;
	int size = 0;

	if (docRtfStoreSavedText(&(rrc->rrcCurrentFont.dfAltName), &size, rrc,
				 removeSemicolon)) {
		LDEB(1);
		return -1;
	}
...
static int docRtfCommitFontPanoseText(const RtfControlWord *rcw, RtfReader *rrc)
{
	...
	memcpy(rrc->rrcCurrentFont.dfPanose, rrs->rrsSavedText.mbBytes,
...
static int docRtfAddCurrentFontToList(RtfReader *rrc)
{
	...
	if (docRtfStoreSavedText(&(rrc->rrcCurrentFont.dfName), &size, rrc,
				 removeSemicolon)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (rrc->rrcCurrentFont.dfName) {
		...
		efIn->ecBufFontNumber =
			docGetFontByName(dfl, rrc->rrcCurrentFont.dfName);
		if (efIn->ecBufFontNumber < 0) {
			SLDEB(rrc->rrcCurrentFont.dfName,
			      efIn->ecBufFontNumber);
			rval = -1;
			goto ready;
		}
		...
		if (!df) {
			SXDEB(rrc->rrcCurrentFont.dfName, df);
			rval = -1;
			goto ready;
		}
		...
		if (!efOut) {
			LXDEB(efIn->ecFileFontNumber, efOut);
			rval = -1;
			goto ready;
		}
		*efOut = *efIn;
...
static int docRtfFontName(RtfReader *rrc, const char *name, int len)
{
	...
	if (rrc->rrcCurrentEncodedFont.ecFileFontNumber < 0) {
		LDEB(rrc->rrcCurrentEncodedFont.ecFileFontNumber);
		return 0;
	}
...
static int docRtfCommitCurrentFont(const RtfControlWord *rcw, RtfReader *rrc)
{
	return docRtfAddCurrentFontToList(rrc);
}

int docRtfFontProperty(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	switch (rcw->rcwID) {
...
static int docRtfFontGroup(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	docRtfRestartFont(rrc);

	rrc->rrcCurrentEncodedFont.ecFileFontNumber = arg;

	if (docRtfReadGroup(rcw, 0, 0, rrc, docRtfFontGroupGroups,
...
static int docRtfThemeFontGroup(const RtfControlWord *rcw, int arg,
				RtfReader *rrc)
{
	docRtfRestartFont(rrc);
...
static int docRtfCommitFontTable(const RtfControlWord *rcw, RtfReader *rrc)
{
...
int docRtfFontTable(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	if (docRtfReadGroup(rcw, 0, 0, rrc, docRtfFontTableGroups,
			    docRtfFontName, docRtfCommitFontTable)) {
		SLDEB(rcw->rcwWord, arg);
		return -1;
	}
...
int docRtfReadMapFont(const RtfReader *rrc, int *pDocFontNum, int *pCharset,
		      int fileFontNum)
{
	EncodedFont *ef;
