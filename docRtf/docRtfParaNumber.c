...
static int docRtfCommitPnTxta(const RtfControlWord *rcw, RtfReader *rrc)
{
	int size;
	const int removeSemicolon = 0;

	if (docRtfStoreSavedText(&(rrc->rrcParagraphNumber.pnTextAfter), &size,
				 rrc, removeSemicolon)) {
		LDEB(1);
		return -1;
	}
...
static int docRtfCommitPnTxtb(const RtfControlWord *rcw, RtfReader *rrc)
{
	int size;
	const int removeSemicolon = 0;

	if (docRtfStoreSavedText(&(rrc->rrcParagraphNumber.pnTextBefore), &size,
				 rrc, removeSemicolon)) {
		LDEB(1);
		return -1;
	}
...
int docRtfReadPnseclvl(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int res;
	int level = arg - 1;
	...
	res = docRtfReadGroup(rcw, 0, 0, rrc, docRtfPndescGroups,
	...
	if (level >= rrc->rrcParagraphNumberCount) {
		...
		fresh = (ParagraphNumber *)realloc(
			rrc->rrcParagraphNumbers,
...
int docRtfSkipPn(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int res;
