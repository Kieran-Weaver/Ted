...
static int docRtfSaveBytes(void *vmb, int offset, const char *bytes, int count)
{
	MemoryBuffer *mb = (MemoryBuffer *)vmb;

	if (utilMemoryBufferAppendBytes(mb, (const unsigned char *)bytes,
					count)) {
		LDEB(mb->mbSize);
		return -1;
	}
...
int docRtfSaveDocEncodedText(RtfReader *rr, const char *text, int len)
{
	...
	upto = textConverterConvertToUtf8(rr->rrRtfTextConverter,
					  ...
					  &consumed, rrs->rrsSavedText.mbSize,
					  text, len);
	if (upto < 0) {
		LDEB(upto);
		return -1;
			 ...
			 int removeSemicolon)
{
	...
	if (removeSemicolon && size > 0 && fresh[size - 1] == ';') {
		fresh[--size] = '\0';
	}
...
int docRtfMemoryBufferAppendText(MemoryBuffer *mb, RtfReader *rr)
{
	...
	if (utilMemoryBufferAppendBytes(mb, (const unsigned char *)text,
					size)) {
		LDEB(1);
		return -1;
	}
...
static int docRtfReadAdaptToFontEncoding(RtfReader *rr, RtfReadingState *rrs)
{
	...
	textConverterSetNativeEncodingName(rr->rrTextTextConverter,
					   encodingName);
...
int docRtfTextParticule(RtfReader *rr, const char *text, int len)
{
	...
	if (docParaStrlen(paraNode) == 0 || rr->rrAfterParaHeadField) {
...
static int docRtfTextUnicodeValue(const RtfControlWord *rcw, int arg,
				  RtfReader *rr)
{
	RtfReadingState *rrs = rr->rrcState;
	...
	if (rr->rrcAddParticule == docRtfSaveRawBytes) {
		XXDEB(rr->rrcAddParticule, docRtfSaveRawBytes);
		return 0;
	}
	...
	if (rr->rrcAddParticule == docRtfTextParticule) {
		...
		textAttributeNumber =
			docTextAttributeNumber(bd, &(rrs->rrsTextAttribute));
		...
		if (docParaDivideAppendedText(paraNode, textAttributeNumber,
					      stroff, stroff + count)) {
			LLDEB(count, paraNode->biParaParticuleCount);
			return -1;
		}
	} else {
		if (utilMemoryBufferAppendBytes(&(rrs->rrsSavedText),
	...
	case DOCkindCOLUMNBREAK: {
		...
		if (rr->rrParagraphBreakOverride == -1 &&
			...
			if (rcw->rcwID == DOCkindPAGEBREAK) {
				rr->rrParagraphBreakOverride = DOCibkPAGE;
				done = 1;
			}

			if (rcw->rcwID == DOCkindCOLUMNBREAK) {
				rr->rrParagraphBreakOverride = DOCibkCOL;
				done = 1;
			}
		}

		if (!done && docSaveSpecialParticule(rr->rrDocument, paraNode,
						     &(rrs->rrsTextAttribute),
						     rcw->rcwID)) {
			LDEB(1);
			return -1;
