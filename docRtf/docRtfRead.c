...
static int docRtfReadDoc(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int rval = 0;
	int changed = 0;
	...
	rval = docRtfReadGroup(rcw, 0, 0, rrc, docRtfDocumentGroups,
...
static RtfControlWord docRtfOutsideGroups[] = { RTF_DEST_XX("rtf", 0,
							    docRtfReadDoc),
...
BufferDocument *docRtfReadFile(SimpleInputStream *sis, unsigned int flags)
{
	...
	if (!(rr->rrReadFlags & RTFflagLENIENT)) {
		...
		while (c != EOF) {
			if (c != '\r' && c != '\n' && c != '\0' && c != ' ') {
				...
				LCSDEB(rr->rrCurrentLine, c, message);
				goto ready;
			}
	...
	if (!rr->rrcGotDocGeometry && bd->bdBody.dtRoot->biChildCount > 0) {
...
int docRtfRememberProperty(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
