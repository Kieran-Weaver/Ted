...
static int docRtfPushTable(RtfWriter *rw, const BufferItem *rowNode,
			   const DocumentSelection *ds)
{
...
static int docRtfEnterRowNode(RtfWriter *rw, const BufferItem *rowNode,
			      const DocumentSelection *ds)
{
...
static int docRtfLeaveRowNode(RtfWriter *rw, const BufferItem *rowNode,
			      const DocumentSelection *ds)
{
	int tableNesting = docTableNesting(rowNode);
	...
	if (tableNesting > 1) {
		...
		if (nip == cellBi->biChildCount - 1 ||
			...
			docRtfSavePropertiesOfRow(
				rw, &(cellBi->biParent->biRowProperties), ds);
		}
	} else {
...
static int docRtfEnterParaNode(RtfWriter *rw, const BufferItem *paraNode,
			       const DocumentSelection *ds)
{
	int flattenCell = 0;
	int firstInRow = 0;
	...
	/*  0  */
	if (ds && paraNode->biTreeType == DOCinBODY &&
	...
	if (!(rw->rwSaveFlags & RTFflagEMIT_CELL)) {
		...
		if (paraNode->biParaTableNesting > 0 && ds &&
		    docPositionsInsideCell(&(ds->dsHead), &(ds->dsTail))) {
	...
	if (paraNode->biParaTableNesting > 0) {
		...
		if (!rowNode || docHeadPosition(&dpFirst, rowNode)) {
			...
			firstInRow = paraNode == dpFirst.dpNode;
		}
	}
...
static int docRtfEnterNode(BufferItem *node, const DocumentSelection *ds,
			   const BufferItem *bodySectNode, void *voidrw)
{
	...
	case DOClevSECT:
		...
		if (!ds && node->biTreeType == DOCinBODY &&
...
static int docRtfLeaveNode(BufferItem *node, const DocumentSelection *ds,
			   const BufferItem *bodySectNode, void *voidrw)
{
	RtfWriter *rw = (RtfWriter *)voidrw;
...
int docRtfSaveDocumentTree(RtfWriter *rw, const char *tag,
			   ...
			   int forcePar)
{
	...
	if (dt->dtRoot->biLevel != DOClevSECT) {
		SDEB(docLevelStr(dt->dtRoot->biLevel));
		return -1;
	}
	...
	if (docScanTree(rw->rwDocument, dt, docRtfEnterNode, docRtfLeaveNode,
			flags, (void *)rw) < 0) {
		LDEB(1);
		return -1;
	}
...
int docRtfWriteSelection(RtfWriter *rw, const DocumentSelection *ds)
{
	...
	if (ds) {
		if (docScanSelection(rw->rwDocument, ds, docRtfEnterNode,
				     docRtfLeaveNode, flags, (void *)rw) < 0) {
			LDEB(1);
			return -1;
		}
	} else {
		if (docScanTree(rw->rwDocument, &(rw->rwDocument->bdBody),
				docRtfEnterNode, docRtfLeaveNode, flags,
				(void *)rw) < 0) {
			LDEB(1);
			return -1;
		}
