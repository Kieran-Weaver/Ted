...
int docRtfReadDocumentTree(const RtfControlWord *rcw, DocumentTree *dt,
			   int *pExtItKind, RtfReader *rrc, int ignoreEmpty,
			   const SelectionScope *ss)
{
	...
	/*  2  */
	if (!dt->dtRoot && docMakeDocumentTree(rrc->rrDocument, dt, ss,
	...
	if (docRtfReadGroup(rcw, 0, 0, rrc, docRtfDocumentGroups,
	...
	/*  5  */
	{
		...
		if (docTailPosition(&dp, dt->dtRoot)) {
			if (ignoreEmpty) {
				...
				if (!docInsertEmptyParagraph(
					    rrc->rrDocument, dt->dtRoot,
					    textAttributeNumber)) {
...
int docRtfReadExtTree(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	BufferItem *sectNode;
...
static int docExtractFixedTextNote(DocumentNote *dn, BufferDocument *bd,
				   BufferItem *ownerNode, int fixedStroff,
				   int fixedStrlen, const char *fieldinst,
				   int fieldsize)

{
	...
	if (utilMemoryBufferSetBytes(&(dn->dnNoteProperties.npFixedText),
				     docParaString(ownerNode, fixedStroff),
				     fixedStrlen)) {
		LDEB(fixedStrlen);
	}

	if (docHeadPosition(&(dsField.dsHead), dn->dnDocumentTree.dtRoot)) {
		LDEB(1);
		return -1;
	}
	...
	if (docParaStrlen(noteBi) >= fixedStrlen &&
		...
		df = docMakeField(bd, &(dn->dnDocumentTree), &dsField, part0,
				  part1, tp1->tpTextAttrNr, tp1->tpTextAttrNr);
		if (!df) {
			XDEB(df);
			return -1;
		}
...
int docRtfReadFootnote(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int rval = 0;
	BufferDocument *bd = rrc->rrDocument;
	...
	if (rrc->rrcAfterNoteref) {
		...
		dfNote = docRtfSpecialField(DOCfkCHFTN, fieldinst, fieldsize,
					    fieldRslt, rrc);
		if (!dfNote) {
			SDEB(rcw->rcwWord);
			rval = -1;
		...
		if (docFindParticuleOfPosition(&part0, (int *)0, &dp,
					       PARAfindFIRST)) {
			LDEB(dfNote->dfHeadPosition.epStroff);
			rval = -1;
			goto ready;
		}
		...
		if (ownerNode->biParaParticules[part0].tpKind == DOCkindSPAN &&
		    ownerNode->biParaParticules[part0 + 1].tpKind ==
			    DOCkindFIELDHEAD) {
			fixedStroff =
				ownerNode->biParaParticules[part0].tpStroff;
			fixedStrlen =
				ownerNode->biParaParticules[part0].tpStrlen;

			dfNote->dfHeadPosition.epStroff = fixedStroff;

			ownerNode->biParaParticules[part0 + 1].tpKind =
				DOCkindSPAN;
			ownerNode->biParaParticules[part0 + 1].tpObjectNumber =
				-1;
			ownerNode->biParaParticules[part0 + 1].tpStroff =
				fixedStroff;
			ownerNode->biParaParticules[part0 + 1].tpStrlen =
				fixedStrlen;

			ownerNode->biParaParticules[part0].tpKind =
				DOCkindFIELDHEAD;
		...
		autoNumber = 0;
		if (fixedStrlen > 0) {
			PROPmaskADD(&(rrc->rrcNotePropertyMask),
				    NOTEpropFIXED_TEXT);
		}
	}

	dfNote = docGetFieldByNumber(&(bd->bdFieldList),
				     rrc->rrcLastFieldNumber);
	if (!dfNote) {
		LPDEB(rrc->rrcLastFieldNumber, dfNote);
		return -1;
	}
	...
	if (docRtfReadDocumentTree(rcw, &(dn->dnDocumentTree), &treeType, rrc,
				   ignoreEmpty, &ss)) {
		SDEB(rcw->rcwWord);
		rval = -1;
		goto ready;
	...
	if (!autoNumber && fixedStrlen > 0 &&
	    docExtractFixedTextNote(dn, bd, ownerNode, fixedStroff, fixedStrlen,
				    fieldinst, fieldsize)) {
		LDEB(1);
		rval = -1;
		goto ready;
