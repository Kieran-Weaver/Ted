...
void docRtfSaveRowProperties(RtfWriter *rw, const PropertyMask *rpSetMask,
			     const RowProperties *rpSet, int col0, int col1)
{
	...
	if (PROPmaskISSET(rpSetMask, RPpropHORIZ_BORDER)) {
		docRtfSaveBorderByNumber(
			rw, "trbrdrh", rpSet->rpHorizontalBorderNumber, anyway);
	}

	/**/
	if (PROPmaskISSET(rpSetMask, RPpropSHADING)) {
		docRtfSaveShadingByNumber(rw, rpSet->rpShadingNumber,
					  DOCrtf_RowShadingTags,
					  DOCrtf_RowShadingTagCount, "trcfpat",
					  "trcbpat", "trshdng");
	}
	...
	if (PROPmaskISSET(rpSetMask, RPpropCELL_LAYOUT) ||
		...
		if (rw->rwCpExtraMask) {
			utilPropMaskOr(&cpAllMask, &cpAllMask,
				       rw->rwCpExtraMask);
		}

		if (col0 > 0) {
			shiftLeft = docColumnLeft(rpSet, col0) -
				    docColumnLeft(rpSet, 0);
		}
		...
		for (col = 0; col < rpSet->rpCellCount; cp++, col++) {
			if ((col0 < 0 || col >= col0) &&
				...
				docCellPropertyDifference(&cpSetMask, &cpRef,
							  &cpAllMask, cp, dam0);

				docRtfSaveCellProperties(rw, &cpSetMask, cp,
							 shiftLeft);
			}
		}
