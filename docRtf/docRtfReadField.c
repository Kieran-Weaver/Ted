...
static int docRtfReadFldrslt(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int res;

	res = docRtfReadGroup(rcw, 0, 0, rrc, docRtfDocumentGroups,
...
static int docRtfCommitFldinst(const RtfControlWord *rcw, RtfReader *rrc)
{
...
static int docRtfReadFldinst(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int res;

	res = docRtfReadGroup(rcw, 0, 0, rrc, docRtfFldinstGroups,
			      docRtfSaveDocEncodedText, docRtfCommitFldinst);
...
static int docRtfReadPushField(int *pPart, DocumentField **pField, int kind,
			       RtfReader *rrc, const char *fieldinst, int size)
{
	int rval = 0;
	...
	if (docInsertAdminParticule(bd, paraNode, rfsl->rfslParticule,
	...
	*pPart = part;
	rrc->rrcFieldStack = rfsl;
	rfsl = (RtfFieldStackLevel *)0; /* steal */
...
static int docRtfTerminateField(const RtfFieldStackLevel *rfsl, RtfReader *rrc)
{
	...
	if (docInsertAdminParticule(
		    bd, bi, part, stroff, rfsl->rfslField->dfFieldNumber,
		    DOCkindFIELDTAIL, &(rrs->rrsTextAttribute))) {
		LDEB(1);
		return -1;
	}
	...
	if (rfsl->rfslPrev) {
		if (docAddChildToField(rfsl->rfslField,
				       rfsl->rfslPrev->rfslField)) {
			LDEB(1);
			return -1;
		}
	} else {
...
static int docRtfPopFieldFromFieldStack(DocumentField *df, RtfReader *rrc)
{
...
int docRtfPopParaFromFieldStack(RtfReader *rrc, int paraNr)
{
	RtfFieldStackLevel *rfsl;
...
int docRtfReadField(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int res;
	...
	res = docRtfReadGroup(rcw, 0, 0, rrc, docRtfFieldGroups,
...
int docRtfLookupWord(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	RtfFieldStackLevel *rfsl = rrc->rrcFieldStack;
	char scratch[40];
	...
	case RTFlookupTCF:
	case RTFlookupTCL:
		while (rfsl && rfsl->rfslField->dfKind != DOCfkTC &&
		       rfsl->rfslField->dfKind != DOCfkTCN) {
			rfsl = rfsl->rfslPrev;
		}
		if (!rfsl) {
			SXDEB(rcw->rcwWord, rfsl);
			return 0;
		}
		break;
	...
	case RTFlookupBXE:
	case RTFlookupIXE:
		while (rfsl && rfsl->rfslField->dfKind != DOCfkXE) {
			rfsl = rfsl->rfslPrev;
		}
		if (!rfsl) {
			SXDEB(rcw->rcwWord, rfsl);
	...
	case RTFlookupTCL:
		...
		if (docAddToFieldData(rfsl->rfslField, scratch,
				      strlen(scratch))) {
			SDEB(rcw->rcwWord);
			return -1;
		}
...
int docRtfLookupEntry(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int res;
	...
	/*  docRtfReadPushField() adjusts level */
	if (docRtfReadPushField(&startParticule, &df, fieldKind, rrc, fieldinst,
				size)) {
		SDEB(rcw->rcwWord);
		return -1;
	}
...
static int docRtfCommitBookmarkName(const RtfControlWord *rcw, RtfReader *rrc)
{
...
int docRtfBkmkStart(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int res;
...
int docRtfBkmkEnd(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	BufferDocument *bd = rrc->rrDocument;
	...
	fieldNumber = docFindBookmarkField(&df, &(bd->bdFieldList),
					   &(rrc->rrcBookmark));
	if (fieldNumber < 0) { /*SDEB((char *)rrc->rrcBookmarkName);*/
		return 0;
	}
...
int docRtfTextSpecialToField(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	...
	if (!docRtfSpecialField(rcw->rcwID, fieldinst, fieldsize, fieldRslt,
				rrc)) {
		SDEB(rcw->rcwWord);
		return -1;
	}
...
int docRtfReadTextField(const RtfControlWord *rcw, int arg, RtfReader *rrc)
{
	int res;
				...
				RtfReader *rrc)
{
	...
	if (!rfsl) {
		SXDEB(rcw->rcwWord, rrc->rrcFieldStack);
		return 0;
	}
