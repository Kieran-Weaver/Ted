		...
		case DOCkindFIELDHEAD:
			...
			if (df->dfKind == DOCfkCHFTN) {
				...
				if (paraNode->biTreeType == DOCinBODY) {
					sprintf(scratch, "[%d]",
						pwc->pwcNoteRefCount + 1);
					pwc->pwcNoteRefCount++;
				} else {
					sprintf(scratch, "[%d]",
						pwc->pwcNoteDefCount + 1);
					pwc->pwcNoteDefCount++;
				}
...
static int docPlainEnterNode(BufferItem *node, const DocumentSelection *ds,
			     const BufferItem *bodySectNode, void *voidpwc)
{
			 ...
			 const DocumentSelection *ds, int fold)
{
	...
	if (ds) {
		if (docScanSelection(bd, ds, docPlainEnterNode, (NodeVisitor)0,
				     flags, (void *)&pwc) < 0) {
			LDEB(1);
			return -1;
		}
	...
	if (!ds && pwc.pwcNoteRefCount > 0) {
		...
		for (dfNote = docGetFirstNoteOfDocument(&dn, bd, -1); dfNote;
		     dfNote = docGetNextNoteInDocument(&dn, bd, dfNote, -1)) {
			if (docScanTree(bd, &(dn->dnDocumentTree),
					docPlainEnterNode, (NodeVisitor)0,
					flags, (void *)&pwc) < 0) {
				LDEB(1);
				return -1;
			}
