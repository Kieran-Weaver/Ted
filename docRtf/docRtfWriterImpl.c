...
static void docRtfInitWritingContext(RtfWriter *rw)
{
	...
	utilStartPagedList(&(rw->rwcEncodedFontList), sizeof(EncodedFont),
...
void docRtfReserveColumns(RtfWriter *rw, int cols)
{
...
int docRtfWriteTag(RtfWriter *rw, const char *tag)
{
	int len = strlen(tag);
...
int docRtfWriteDestinationBegin(RtfWriter *rw, const char *tag)
{
	int len = strlen(tag);
...
int docRtfWriteArgDestinationBegin(RtfWriter *rw, const char *tag, int arg)
{
	char scratch[20];
	int len;
...
int docRtfWriteArgTag(RtfWriter *rw, const char *tag, int arg)
{
	char scratch[20];
	int len;
...
void docRtfWriteFlagTag(RtfWriter *rw, const char *tag, int arg)
{
	if (arg) {
		docRtfWriteTag(rw, tag);
	} else {
		docRtfWriteArgTag(rw, tag, arg);
	}
}

void docRtfWriteAltTag(RtfWriter *rw, const char *yesTag, const char *noTag,
		       int arg)
{
	if (arg) {
		docRtfWriteTag(rw, yesTag);
	} else {
		docRtfWriteTag(rw, noTag);
	}
}

void docRtfWriteEnumTag(RtfWriter *rw, const char *const *tags, int arg,
			int tagCount, int enumCount)
{
	if (tagCount != enumCount) {
		LLSDEB(tagCount, enumCount, tags[0]);
		return;
	}

	if (arg < 0 || arg >= tagCount) {
		LLSDEB(tagCount, arg, tags[0]);
		return;
	}
...
int docRtfWriteMemoryBuffer(RtfWriter *rw, const MemoryBuffer *mb)
{
...
void docRtfSaveAutoSpace(RtfWriter *rw, const char *unitTag, int unit,
...
void docRtfWriteDocEncodedStringDestination(RtfWriter *rw, const char *tag,
					    const char *s, int n,
					    int addSemicolon)
{
...
void docRtfWriteRawBytesDestination(RtfWriter *rw, const char *tag,
				    const char *s, int n)
{
	docRtfWriteDestinationBegin(rw, tag);
