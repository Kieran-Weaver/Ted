...
static void docRtfParaSaveProperties(RtfWriter *rw, int flattenCell,
				     int firstInRow, int *pPropertyChange,
				     int *pTabsSaved,
				     ...
				     const ParagraphProperties *prevPP)
{
	...
	if (utilPropMaskIsEmpty(&ppDifMask)) {
		if (!flattenCell && firstInRow && newPP->ppTableNesting > 0) {
...
static int docRtfSaveObject(RtfWriter *rwc, const BufferItem *paraNode,
	...
	case DOCokOLEOBJECT:
		...
		if (io->ioObjectClass) {
			...
			docRtfWriteDocEncodedStringDestination(
				rwc, "*\\objclass", io->ioObjectClass,
				strlen(io->ioObjectClass), addSemicolon);
		}
		...
		if (io->ioObjectName) {
			...
			docRtfWriteDocEncodedStringDestination(
				rwc, "*\\objname", io->ioObjectName,
				strlen(io->ioObjectName), addSemicolon);
		}
...
static int docRtfSaveTagParticule(RtfWriter *rwc, int kind)
{
	switch (kind) {
...
int docRtfSaveParaNode(RtfWriter *rwc, const BufferItem *paraNode,
		       ...
		       const int firstInRow)
{
	...
	docRtfParaSaveProperties(rwc, flattenCell, firstInRow, &paraPropChange,
				 &tabsSaved, &(paraNode->biParaProperties),
...

	{
		...
		if (docIntersectSelectionWithParagraph(
			    &dsPara, &part, &partUpto, &headFlags, &tailFlags,
			    paraNode, ds)) {
			LDEB(1);
			return -1;
		}
	...
	for (; part < paraNode->biParaParticuleCount; part++, tp++) {
		int pastSel = (tailFlags & POSflagPARA_FOUND) &&
			      tp->tpStroff >= stroffUpto;
		...
		if (!pastSel ||
		    (tp->tpKind == DOCkindSPAN && tp->tpStrlen == 0)) {
			const char *first = (const char *)0;

			if (tp->tpStroff < paralen) {
				first = (const char *)docParaString(
					paraNode, tp->tpStroff);
			}

			if (tp->tpKind != DOCkindFIELDTAIL) {
				docRtfWriteSwitchTextAttributes(
					rwc, tp->tpTextAttrNr, first);
			}
		}
		...
		case DOCkindSPAN:
...

			{
				...
				if ((tailFlags & POSflagPARA_FOUND) &&
					...
					n = tp->tpStroff + tp->tpStrlen -
					    stroff;
				}

				if (rwc->rwCol > 0 && rwc->rwCol + n > 72 &&
				    n < 72) {
					docRtfWriteNextLine(rwc);
				}
...

	{
		...
		if ((rwc->rwSaveFlags & RTFflagSAVE_SOMETHING) && ds &&
		    ds->dsHead.dpNode == paraNode && docIsIBarSelection(ds)) {
			saveSpace = 1;
		}
	...
	/*  8  */
	if (!(tailFlags & POSflagPARA_FOUND) || stroffUpto == dpLast.dpStroff ||
	    ds) {
