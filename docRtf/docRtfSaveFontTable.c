...
static void docRtfWriteEncodedFont(RtfWriter *rw, const DocumentFont *df,
				   const char *suffix, int fontnum, int charset)
{
...
static const EncodedFont *
dowRtfWriteGetEncodedFont(RtfWriter *rw, const DocumentFont *df, int csIdx)
{
...
int docRtfWriteGetCharset(RtfWriter *rw, int *pCharset, const DocumentFont *df,
			  int symbol)
{
	int csIdx;
	const EncodedFont *ef;
...
void docRtfWriteFontTable(RtfWriter *rw)
{
	...
	for (fnt = 0; fnt < dfl->dflFontCount; fnt++) {
		...
		for (cs = 0; cs < df->dfCharsetIdxUsedCount; cs++) {
			...
			ef = dowRtfWriteGetEncodedFont(
				rw, df, df->dfCharsetIdxUsed[cs]);
			if (!ef) {
				LXDEB(cs, ef);
				continue;
			}

			oc = utilGetOfficeCharsetByIndex(
				df->dfCharsetIdxUsed[cs]);
			if (!oc) {
				LXDEB(df->dfCharsetIdxUsed[cs], oc);
				continue;
			}
			...
			docRtfWriteEncodedFont(rw, df, suffix,
					       ef->ecFileFontNumber,
					       oc->ocOfficeCharset);
