...
void docRtfSaveParagraphProperties(RtfWriter *rw, const PropertyMask *updMask,
				   const ParagraphProperties *pp)
{
	...
	if (PROPmaskISSET(updMask, PPpropWIDCTLPAR)) {
		docRtfWriteAltTag(rw, "widctlpar", "nowidctlpar",
				  pp->ppWidowControl);
	}
	...
	if (PROPmaskISSET(updMask, PPpropALIGNMENT)) {
		docRtfWriteEnumTag(rw, DOCrtf_ParaAlignTags, pp->ppAlignment,
	...
	if (PROPmaskISSET(updMask, PPpropTOP_BORDER)) {
		docRtfSaveBorderByNumber(rw, "brdrt", pp->ppTopBorderNumber,
					 anyway);
	}

	if (PROPmaskISSET(updMask, PPpropBOTTOM_BORDER)) {
		docRtfSaveBorderByNumber(rw, "brdrb", pp->ppBottomBorderNumber,
					 anyway);
	}

	if (PROPmaskISSET(updMask, PPpropLEFT_BORDER)) {
		docRtfSaveBorderByNumber(rw, "brdrl", pp->ppLeftBorderNumber,
					 anyway);
	}

	if (PROPmaskISSET(updMask, PPpropRIGHT_BORDER)) {
		docRtfSaveBorderByNumber(rw, "brdrr", pp->ppRightBorderNumber,
					 anyway);
	}
	...
	if (PROPmaskISSET(updMask, PPpropBAR_BORDER)) {
		docRtfSaveBorderByNumber(rw, "brdrbar", pp->ppBarNumber,
					 anyway);
	}
	...
	if (PROPmaskISSET(updMask, PPpropSHADING)) {
		docRtfSaveShadingByNumber(rw, pp->ppShadingNumber,
					  DOCrtf_ParaShadingTags,
					  DOCrtf_ParaShadingTagCount, "cfpat",
					  "cbpat", "shading");
	}
