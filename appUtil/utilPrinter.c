...
/*  0  */
static int utilPrinterCompareNames(const void *vpd1, const void *vpd2)
{
...
/*  2,3 */
static int utilPrinterSetLpCommand(PrintDestination *pd, const char *name,
				   int l)
{
...
/*  1  */
static int utilPrinterGetLprPrinters(const char *command, int *pDefaultPrinter,
				     int *pPrinterCount,
				     PrintDestination **pDestinations)
{
	...
	while (fgets(scratch, 250, f)) {
		...
		fresh = (PrintDestination *)realloc(
			*pDestinations, (count + 1) * sizeof(PrintDestination));
		if (!fresh) {
			XDEB(fresh);
			return -1;
		}
	...
	sprintf( fresh->pdCommand, "lpr -r -s -P '%s' '%s'",
							scratch, "%f" );
#else
		sprintf(fresh->pdCommand, "lpr -r -P '%s' '%s'", scratch, "%f");
#endif
...
/*  2  */
static int utilPrinterGetAixPrinters(const char *command, int *pDefaultPrinter,
				     int *pPrinterCount,
				     PrintDestination **pDestinations)
{
	char scratch[250 + 1];
	...
	if (fgets(scratch, 250, f) && fgets(scratch, 250, f)) {
		while (fgets(scratch, 250, f)) {
			...
			*pDestinations = fresh;
			...
			if (utilPrinterSetLpCommand(fresh + count, scratch,
						    l)) {
				LDEB(l);
				return -1;
			}
...
/*  3  */
static int utilPrinterGetLpPrinters(const char *command, int *pDefaultPrinter,
				    int *pPrinterCount,
				    PrintDestination **pDestinations)
{
	char scratch[250 + 1];
...
static int utilPrinterSetCustom(PrintDestination *pd, const char *customCommand,
				const char *customName)
{
	...
	if (!pd->pdCommand && !pd->pdPrinterName) {
		XXDEB(pd->pdCommand, pd->pdPrinterName);
		return -1;
	}
			   ...
			   const char *customCommand2, const char *customName2)
{
	...
	/*  1b  */
	if (count == 0) {
		utilPrinterGetLprPrinters(
			"( /usr/sbin/lpc status ) 2>/dev/null", &defaultPrinter,
			&count, &pd);
	}
	...
	if (extra > 0) {
		...
		fresh = (PrintDestination *)realloc(
			pd, (count + extra) * sizeof(PrintDestination));
		if (!fresh) {
			LLXDEB(count, extra, fresh);
			return -1;
		}
		pd = fresh;
	}
	...
	if (customName && customCommand) {
		if (utilPrinterSetCustom(pd + count, customCommand,
					 customName)) {
			LLDEB(extra, count);
			return -1;
		} else {
	...
	if (customName2 && customCommand2) {
		if (utilPrinterSetCustom(pd + count, customCommand2,
					 customName2)) {
			LLDEB(extra, count);
			return -1;
		} else {
