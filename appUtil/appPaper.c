...
int utilPaperGetInfoByNumber(int n, int *pWide, int *pHigh, const char **pLabel)
{
#if USE_LIBPAPER
	...
	if (n < 0 || n >= (int)(sizeof(UtilPaperSizes) / sizeof(PaperSize))) {
		return -1;
	}
...
int utilPaperGetNumberBySize(int wide, int high)
{
	int gw = wide / 200;
	int gh = high / 200;
...
int utilPaperSizeFromString(int *pFormat, int *pWide, int *pHigh, int unitType,
			    const char *paperString)
{
	...
	if (sp) {
		for (i = 0; i < UtilPaperCount; i++) {
			if (sp == UtilPapers[i]) {
				if (pWide) {
					*pWide = (int)((paperpswidth(sp) *
							20.0) +
						       0.499);
				}
				if (pHigh) {
					*pHigh = (int)((paperpsheight(sp) *
							20.0) +
						       0.499);
				}
				if (pFormat) {
	...
	for (i = 0; i < sizeof(UtilPaperSizes) / sizeof(PaperSize); i++) {
		if (!strcmp(UtilPaperSizes[i].psMnemonic, paperString)) {
			utilPaperGetInfoByNumber(i, pWide, pHigh,
						 (const char **)0);
			if (pFormat) {
				*pFormat = i;
			}
	...
	for (i = 0; i < sizeof(UtilPaperSizes) / sizeof(PaperSize); i++) {
		if (tolower(paperString[0]) ==
		    ...
		    !strcmp(UtilPaperSizes[i].psMnemonic + 1,
			    paperString + 1)) {
			utilPaperGetInfoByNumber(i, pWide, pHigh,
						 (const char **)0);
			if (pFormat) {
