...
static int sioInMsGetPacketBytes(MsLz77InputStream *mis, unsigned char *buffer,
				 int count)
{
	int done = 0;
	int got;

	while (done < count && !mis->misExhausted && mis->misFlagBit < 8) {
		int byte1;
		int byte2;
		...
		if (!(mis->misFlagByte & mis->misFlagMask)) {
			...
			got = sioInMsLz77GetWindowBytes(mis, buffer,
							count - done);
			if (got <= 0) {
				LDEB(got);
				break;
			...
			mis->misInputWindow[mis->misBytesRead % MSLZ77_WINSZ] =
				byte1;
		...
		mis->misFlagBit++;
		mis->misFlagMask = mis->misFlagMask << 1;
	}
...
static int sioInMsLz77ReadBytes(void *voidmis, unsigned char *buffer,
				unsigned int count)
{
