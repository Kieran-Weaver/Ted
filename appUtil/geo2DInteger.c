			 ...
			 int pointCount)
{
	...
	for (;;) {
		...
		for (i = 0; i < pointCount; vp++, i++) {
			/*  2  */
			if (vp[0].x == vp[1].x && vp[0].y == vp[1].y) {
				continue;
			}

			/*  3  */
			res = geo2DIIntersectSegments(&p1, &p2, &xab1, &xab2,
						      &xvp1, &xvp2, ab, vp);
			if (res == 0) {
				continue;
			}
			...
			if (xvp1 < 0.001 || xvp1 > 0.999 || xvp2 < 0.001 ||
			    xvp2 > 0.999) {
				if (attempts < 60) {
					break;
				}
					 ...
					 const DocumentRectangle *dr)
{
	...
	side[1].x = dr->drX1;
	side[1].y = dr->drY0;
	if (geo2DIIntersectSegments(&pa, &pb, &xpa, &xpb, &xsa, &xsb, vp,
				    side) == 1 &&
	    xpa < xp) {
		p = pa;
		xp = xpa;
