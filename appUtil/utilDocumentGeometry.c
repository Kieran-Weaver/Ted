...
void utilUpdDocumentGeometry(PropertyMask *dpDoneMask, DocumentGeometry *dgTo,
			     ...
			     const DocumentGeometry *dgSet)
{
	...
	if (PROPmaskISSET(dgSetMask, DGpropHEADER_POSITION)) {
		if (dgTo->dgHeaderPositionTwips !=
		    dgSet->dgHeaderPositionTwips) {
			dgTo->dgHeaderPositionTwips =
				dgSet->dgHeaderPositionTwips;
	...
	if (PROPmaskISSET(dgSetMask, DGpropFOOTER_POSITION)) {
		if (dgTo->dgFooterPositionTwips !=
		    dgSet->dgFooterPositionTwips) {
			dgTo->dgFooterPositionTwips =
				dgSet->dgFooterPositionTwips;
