...
static const char *utilFontmapGet(const char *from, const FontmapEntry *entries,
				  int entryCount)
{
	int turn;
...
{
	return utilFontmapGet(from, UTIL_FontmapEntries,
...
int utilFontmapReadMap(const char *filename)
{
	...
	if (recursive == 1 &&
	    UTIL_FontmapEntryCount >
		    0) { /* LDEB(UTIL_FontmapEntryCount); rval= -1; */
		goto ready;
	}
	...
	while (c != EOF) {
		...
		case '(':
			...
			if (appAbsoluteName(&runfile, &local, 1, &fileBuf) <
			    0) {
				SSDEB(filename, scratch);
				rval = -1;
				goto ready;
			}
			...
			if (!appTestFileExists(&runfile) &&
			    utilFontmapReadMap(
				    utilMemoryBufferGetString(&runfile))) {
				LDEB(1);
				rval = -1;
				goto ready;
			}
		...
		fe = (FontmapEntry *)realloc(UTIL_FontmapEntries,
					     (UTIL_FontmapEntryCount + 1) *
						     sizeof(FontmapEntry));
		if (!fe) {
			LXDEB(UTIL_FontmapEntryCount, fe);
			rval = -1;
			goto ready;
		}
