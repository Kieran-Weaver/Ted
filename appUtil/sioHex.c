...
static int sioInHexReadBytes(void *voidsis, unsigned char *buffer,
			     unsigned int count)
{
...
SimpleInputStream *sioInHexOpen(SimpleInputStream *sisHex)
{
	...
	if (SioHexIndices[0] == 0) {
		...
		while (SioHexDigitsU[i]) {
			SioHexIndices[SioHexDigitsU[i]] = i;
			i++;
		}
		i = 0;
		while (SioHexDigitsL[i]) {
			SioHexIndices[SioHexDigitsL[i]] = i;
			i++;
		}
	}
...
static int sioOutHexWriteBytes(void *voidhos, const unsigned char *buffer,
			       int count)
{
	HexOutputStream *hos = (HexOutputStream *)voidhos;
	SimpleOutputStream *sos = hos->hosSosHex;
	...
	while (done < count) {
		...
		if (sioOutPutByte(SioHexDigitsL[((*buffer) >> 4) & 0x0f], sos) <
		    0) {
			return -1;
		}
		if (sioOutPutByte(SioHexDigitsL[((*buffer) >> 0) & 0x0f], sos) <
		    0) {
			return -1;
		}
...
{
	HexOutputStream *hos =
		(HexOutputStream *)malloc(sizeof(HexOutputStream));
