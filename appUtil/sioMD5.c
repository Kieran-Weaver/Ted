...
static int sioInMD5ReadBytes(void *voidfis, unsigned char *buffer,
			     unsigned int count)
{
	MD5InputStream *fis = (MD5InputStream *)voidfis;
	int done = 0;

	if (fis->fisExhausted) { /*LDEB(fis->fisExhausted);*/
		return -1;
	}
...
SimpleInputStream *sioInMD5Open(unsigned char *digest, SimpleInputStream *sisIn)
{
...
static int sioOutMD5WriteBytes(void *voidfos, const unsigned char *buffer,
			       int count)
{
	MD5OutputStream *fos = (MD5OutputStream *)voidfos;
