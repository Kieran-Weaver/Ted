...
static TreeNode *treeGetLeaf(TreeNode *tn, const char *key)
{
	/*  1  */
	while (!tn->no_L.nsKey) {
		int cmp;
...
int utilTreeStoreValue(void *tree, void **pPrevVal, const char **pStoredKey,
...
static void *treeGetLX(void *tree, const char **pKey, const char *key, int skew)
{
	TreeMapping *tm = (TreeMapping *)tree;
...
static void *treeGetGX(void *tree, const char **pKey, const char *key, int skew)
{
	TreeMapping *tm = (TreeMapping *)tree;
...
void *utilTreeGetEQ(const void *tree, const char **pStoredKey, const char *key)
{
	TreeMapping *tm = (TreeMapping *)tree;
...
static void trfillslots(const NodeSlot *hs, TreeNode *no, int n, int leaf)
{
	if (leaf) {
		no->no_L = *(hs++);
		no->no_M = *(hs++);
		     ...
		     void *through)
{
	...
	if (!utilTreeGetEQ(tree, &storedKey, key) && !storedKey) {
		return -1;
	}
			  ...
			  void *through)
{
	...
	if (lfcu == no->no_L.nsKey) {
		...
		if (callback) {
			(void)(*callback)(no->no_L.nsKey, no->no_L.sl_val,
					  through);
		}
		...
		if (lfcu == no->no_M.nsKey) {
			if (callback) {
				(void)(*callback)(no->no_M.nsKey,
						  no->no_M.sl_val, through);
			}
			...
			if (callback) {
				(void)(*callback)(no->no_R.nsKey,
						  no->no_R.sl_val, through);
			}
		   ...
		   void *through)
{
	...
	while (no) /* for all leaves	*/
	{
		...
		if (no->no_M.nsKey) {
			if ((*callback)(no->no_M.nsKey, no->no_M.sl_val,
					through)) {
				rval = -1;
				if (stopOnFailure) {
					break;
			...
			if (no->no_R.nsKey) {
				if ((*callback)(no->no_R.nsKey, no->no_R.sl_val,
						through)) {
					rval = -1;
					if (stopOnFailure) {
						break;
...
int utilTreeFreeTree(void *tree, UTIL_TREE_CALLBACK callback, void *through)
{
	...
	while (no) {
		...
		if (no->no_M.nsKey) {
			if (callback) {
				(*callback)(no->no_M.nsKey, no->no_M.sl_val,
					    through);
			}
			if (tm->trOwnKeys) {
				treeFreeKey(no->no_M.nsKey);
