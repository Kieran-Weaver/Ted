...
static void appGeoLengthNumberToString(char *target, int twips, int unitInt)
{
...
int geoRectangleFromString(const char *s, int defaultUnitInt, int *pWidth,
			   int *pHeight)
{
	double width;
	double height;
	...
	got = sscanf(s, "%d %d/%d x %d %d/%d%s", &w_ival, &w_nom, &w_div,
	...
	got = sscanf(s, "%lg x %d %d/%d%s", &width, &h_ival, &h_nom, &h_div,
		     scratch);
	if (got == 4) {
		height = h_ival + (double)h_nom / (double)h_div;
	...
	got = sscanf(s, "%d %d/%d x %lg%s", &w_ival, &w_nom, &w_div, &height,
		     scratch);
	if (got == 4) {
		width = w_ival + (double)w_nom / (double)w_div;
