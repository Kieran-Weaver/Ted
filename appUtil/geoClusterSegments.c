...
static int geoFreeSegmentCluster(int *pDelete, int x0, int y0, void *voidsc,
				 void *voidcs)
{
...
int geoClusterSegmentsAddSegment(ClusterSegments *cs, int x0, int y0, int x1,
				 int y1)
{
	int rval = 0;
	SegmentCluster *sc;
...
static void geoClusterSegmentsCalculateXY01(SegmentCluster *sc)
{
	...
	for (seg = 0; seg < sc->scSegmentCount; ls++, seg++) {
		...
		l = AT2_X(ls->ls2diX0 + 0.499, ls->ls2diY0 + 0.499,
			  &(sc->scTransform));
		c = AT2_Y(ls->ls2diX0 + 0.499, ls->ls2diY0 + 0.499,
			  &(sc->scTransform));
		sumc += c;
		if (l0 > l) {
			l0 = l;
		...
		l = AT2_X(ls->ls2diX1 + 0.499, ls->ls2diY1 + 0.499,
			  &(sc->scTransform));
		c = AT2_Y(ls->ls2diX1 + 0.499, ls->ls2diY1 + 0.499,
			  &(sc->scTransform));
		sumc += c;
		if (l0 > l) {
			l0 = l;
...
static int bmClusterCandidate(int *pDelete, int x, int y, void *voidsc,
			      void *voidcs)
{
	...
	geoAddLineFitters(&lfMerged, &(sc->scLineFitter),
			  &(curSc->scLineFitter));
	...
	geoLineAffineTransform2D(&(curSc->scTransform), curSc->scA, curSc->scB,
				 curSc->scC);
...
static int bmGrowCluster(int *pDelete, int x, int y, void *voidsc, void *voidcs)
{
	SegmentCluster *sc = (SegmentCluster *)voidsc;
	ClusterSegments *cs = (ClusterSegments *)voidcs;
	...
	if (sc->scSegments[0].ls2diX0 != x) {
		LLDEB(sc->scSegments[0].ls2diX0, x);
		return -1;
	}
	if (sc->scSegments[0].ls2diY0 != y) {
		LLDEB(sc->scSegments[0].ls2diY0, y);
		return -1;
	}
	...
	qtForAll(cs->csClusterTree, bmCurrentFilter, bmClusterCandidate,
		 voidcs);
...
static int bmPrintAllSegments(int *pDelete, int x, int y, void *voidsc,
			      void *voidcs)
{
	ClusterSegments *cs = (ClusterSegments *)voidcs;
	SegmentCluster *sc = (SegmentCluster *)voidsc;
	...
	for (seg = 0; seg < sc->scSegmentCount; ls++, seg++) {
		fprintf(cs->csFile, "%d %d %d %d ls\n",
			ls->ls2diX1 - ls->ls2diX0, ls->ls2diY1 - ls->ls2diY0,
...
static int geoPrintClusterSegments(int *pDelete, int x, int y, void *voidsc,
				   void *voidcs)
{
	ClusterSegments *cs = (ClusterSegments *)voidcs;
	SegmentCluster *sc = (SegmentCluster *)voidsc;
				   ...
				   const rect *dr)
{
	...
	if (!cs->csClusterTree) {
		XDEB(cs->csClusterTree);
		return -1;
	}

	cs->csLongClusterTree = qtMakeTree(dr);
	if (!cs->csLongClusterTree) {
		XDEB(cs->csLongClusterTree);
		return -1;
	}
...
static int geoFinishClusterSegments(ClusterSegments *cs)
{
	...
	qtForAll(cs->csClusterTree, (QuadForAllFilter)0, bmPrintAllSegments,
		 (void *)cs);

	if (cs->csLongClusterCount > 0) {
		cs->csLongClusters = (SegmentCluster **)malloc(
			cs->csLongClusterCount * sizeof(SegmentCluster *));
		if (!cs->csLongClusters) {
			LXDEB(cs->csLongClusterCount, cs->csLongClusters);
			return -1;
		}
...
int geoRunClusterSegments(ClusterSegments *cs)
{
	...
	while (cs->csMerged) {
		...
		qtForAll(cs->csClusterTree, (QuadForAllFilter)0, bmGrowCluster,
			 (void *)cs);
	}
