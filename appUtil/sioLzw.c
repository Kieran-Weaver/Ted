...
static void sioLzwInitState(LzwState *ls, int codeSize)
{
...
static int sioInLzwReadCode(LzwInputStream *lis, int *pCode)

{
	LzwState *ls = &(lis->lisState);

	static unsigned int CodeMasks[] = { 0x0000, 0x0001, 0x0003, 0x0007,
	...
	while (ls->lsBitsAccumulated < ls->lsCurrentCodeSize) {
		...
		ls->lsAccumulator |= ((unsigned long)byte)
				     << ls->lsBitsAccumulated;
...
static int sioInLzwGetPrefixChar(const unsigned int *Prefix, int Code,
				 int ClearCode)
{
	int i = 0;
...
static int sioInLzwReadBytes(void *voidlis, unsigned char *buffer,
			     unsigned int count)
{
	int i = 0;
	int j;
			...
			if (CrntCode < ClearCode) {
				...
				if (Prefix[CrntCode] == NO_SUCH_CODE) {
					...
					if (CrntCode == ls->lsNextCode - 2) {
						CrntPrefix = LastCode;
						Suffix[ls->lsNextCode -
						       2] = Stack[StackPtr++] =
							sioInLzwGetPrefixChar(
								Prefix,
								LastCode,
								ClearCode);
					} else {
...
static const unsigned char SIO_LZW_OutMasks[] = { 0x00, 0x01, 0x03, 0x07, 0x0f,
						  0x1f, 0x3f, 0x7f, 0xff };
...
static void _InsertHashTable(GifHashTableType *HashTable, unsigned long Key,
			     int Code)
{
...
static int _ExistsHashTable(GifHashTableType *HashTable, unsigned long Key)
{
	int HKey = KeyItem(Key);
...
static int sioOutLzwGifWriteCode(LzwGifOutputStream *los, int code)
{
	...
	while (ls->lsBitsAccumulated >= 8) {
		/*  1  */
		if (sioOutPutByte(ls->lsAccumulator & 0xff, los->losSosLzw) ==
		    EOF) {
			rval = -1;
		}
...
static int sioOutLzwGifWriteBytes(void *voidlos, const unsigned char *buffer,
				  int count)
{
	LzwGifOutputStream *los = (LzwGifOutputStream *)voidlos;
	LzwState *ls = &(los->losState);
	...
	if (ls->lsCurrentCode == FIRST_CODE) {
		ls->lsCurrentCode = buffer[i++] &
				    SIO_LZW_OutMasks[ls->lsInitialCodeSize];
	}
	...
	while (i < count) {
		...
		if (nextCode >= 0) {
			...
			if (ls->lsCurrentCodeSize > 12) {
				...
				ls->lsNextCode = ls->lsEOFCode + 1;
				ls->lsCurrentCodeSize =
					ls->lsInitialCodeSize + 1;
				ls->lsMaxCode1 = 1 << ls->lsCurrentCodeSize;

				_ClearHashTable(&(los->losHashTable));
			} else {
				_InsertHashTable(&(los->losHashTable), nextKey,
						 ls->lsNextCode);
				ls->lsNextCode++;
			}
		}
