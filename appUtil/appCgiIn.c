...
static void *appCgiInGetHeaderList(void)
{
	...
	while (*e) {
		...
		*eq = '\0';
		if (utilTreeStoreValue(tree, &prev, (const char **)0, *(e + 5),
				       val)) {
			*eq = '=';
			SDEB(*e);
			goto ready;
...
static void *appCgiInGetEnvironmentList(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (utilTreeStoreValue(tree, &prev, (const char **)0, key,
				       value)) {
			SDEB(key);
			goto ready;
		}
...
static void utilCgiSave(char *to, const char *from, const char *upto)
{
	while (from < upto) {
		int c = *(from++);
...
static void *appCgiInReadValueList(FILE *from, unsigned int contentLength)
{
	void *tree = (void *)0;
...
static void *appCgiInAnalyseCookieString(const char *cookieString)
{
	...
	while (*cookieString) {
		...
		strncpy(key, cookieString,
			val - cookieString - 1)[val - cookieString - 1] = '\0';
...
static int appCgiInGetRequestMethod(const char **pValue, void *tree)
{
	const char *s;
	int n;
...
CGIRequest *appCgiInGetRequest(void)
{
	...
	if (!strcmp(cgir->cgirRequestMethod, "POST") ||
		...
		if (!strcmp(contentType, "application/x-www-form-urlencoded")) {
			...
			cgir->cgirQueryValues = appCgiInReadValueList(
				stdin, (int)contentLength);
	...
	if (!strcmp(cgir->cgirRequestMethod, "GET")) {
		...
		if (!n && queryString[0]) {
			cgir->cgirQueryValues =
				appCgiInAnalyseQueryString(queryString);
...

	{
		...
		if (res == 0 && !n && cookieString[0]) {
			cgir->cgirCookies =
				appCgiInAnalyseCookieString(cookieString);
...
{
	if (cgir->cgirHeaderValues) {
		utilTreeFreeTree(cgir->cgirHeaderValues, utilTreeFreeValue,
				 (void *)0);
	}
	if (cgir->cgirEnvironmentValues) {
		utilTreeFreeTree(cgir->cgirEnvironmentValues, utilTreeFreeValue,
				 (void *)0);
	}
	if (cgir->cgirQueryValues) {
		utilTreeFreeTree(cgir->cgirQueryValues, utilTreeFreeValue,
				 (void *)0);
	}
	if (cgir->cgirCookies) {
		utilTreeFreeTree(cgir->cgirCookies, utilTreeFreeValue,
				 (void *)0);
	}
