...
static int sioInMemoryReadBytes(void *voidsmp, unsigned char *buffer,
				unsigned int count)
{
...
static int sioOutMemoryWriteBytes(void *voidsmp, const unsigned char *buffer,
				  int count)
{
	SioMemoryPrivate *smp = (SioMemoryPrivate *)voidsmp;
	MemoryBuffer *mb = smp->smpBuffer;
	...
	if (smp->smpPosition + count > mb->mbSize) {
		...
		fresh = (unsigned char *)realloc(mb->mbBytes,
						 smp->smpPosition + count + 1);
		if (!fresh) {
			LXDEB(smp->smpPosition + count, fresh);
			return -1;
		}
