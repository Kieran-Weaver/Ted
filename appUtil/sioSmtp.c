...
static int sioSmtpSayRecipients(int fd, const char *recipients, void *through,
				APP_COMPLAIN complain);

static int mmWriteHeader(int fd, const char *name, const char *value,
			 void *through, APP_COMPLAIN complain);

static int mmWrite(int fd, const char *buf, unsigned int len, void *through,
		   APP_COMPLAIN complain);

static int mmWriteCRLF(int fd, const char *buf, void *through,
		       APP_COMPLAIN complain);

static int sioOutSmtpWriteBytes(void *voidmc, const unsigned char *buf,
				int len);

static int sioOutSmtpClose(void *voidmc);

static int mmReadCRLF(int fd, char *buf, void *through, APP_COMPLAIN complain);
...
static void sioOutSmtpAbortMessage(MailContext *mc, void *through,
				   APP_COMPLAIN complain)
{
...
static int sioOutSmtpChallengeResponse(MailContext *mc, const char *response,
				       int responseLen, void *through,
				       APP_COMPLAIN complain)
{
	int rval = 0;
	...
	sioOutClose(sosb64);
	sosb64 = (SimpleOutputStream *)0;
	sioOutClose(sosmb);
	sosmb = (SimpleOutputStream *)0;
...
sioOutSmtpOpen(const char *mailHost, const char *mailPort, const char *user,
	       ...
	       const char *typeSlashSubtype, const char *mimeBoundary,
	       void *through, APP_COMPLAIN complain)
{
    ...
    */
	...
	do {
		...
		if (strncmp(MMscratch, "250", 3)) {
			if (strncmp(MMscratch, "500", 3) &&
				...
				SDEB(MMscratch);
				(void)(*complain)(through, APP_SYSTEMeSMTP,
						  MMscratch);
				goto mmError;
			} else {
				haveEsmtp = 0;
	...
	if (!haveEsmtp) {
		...
		do {
			if (mmReadCRLF(mc->mcFd, MMscratch, through,
				       complain)) {
				LDEB(1);
				goto mmError;
			}
	...
	if (user && pass && haveAuthLogin) {
		...
		for (;;) {
			...
			if (!strcmp(MMscratch + 4, "VXNlcm5hbWU6")) {
				if (sioOutSmtpChallengeResponse(
					    mc, user, strlen(user), through,
					    complain)) {
					LDEB(1);
					goto mmAbort;
				}
	...
	if (mmWriteHeader(mc->mcFd, "From", from, through, complain) ||
	    ...
	    mmWriteHeader(mc->mcFd, "Subject", subject, through, complain)) {
		LDEB(1);
		goto mmAbort;
	}
	...
	if (mimeBoundary) {
		...
		if (mmWrite(mc->mcFd, name2, len, through, complain) ||
		    mmWrite(mc->mcFd, mimeBoundary, strlen(mimeBoundary),
			    through, complain) ||
		    mmWriteCRLF(mc->mcFd, "\"", through, complain)) {
			LDEB(1);
			goto mmAbort;
...
static int sioOutSmtpWriteBytes(void *voidmc, const unsigned char *buf, int len)
{
	...
	while (todo > 0) {
		...
		if (todo > 0) {
			if (mmWrite(mc->mcFd, crlf, 2, mc->mcThrough,
				    mc->mcComplain)) {
				LDEB(1);
				return -1;
			}
				...
				APP_COMPLAIN complain)
{
	...
	while (recipients[0]) {
		...
		if (!comma) {
			comma = recipients + strlen(recipients);
		} /*lie*/
		...
		/*  3  */
		sprintf(MMscratch, "RCPT TO:<%.*s>", (int)(past - start),
			start);
		       ...
		       APP_COMPLAIN complain)
{
	...
	while (*start) {
		...
		if (past > start) {
			if (mmWrite(fd, start, past - start, through,
				    complain)) {
				LDEB(1);
				return -1;
			}
