...
static int sioHttpCloseHeaderOutput(void *voidhc)
{
	...
	if (hc->hcCommunicationPhase == CP_WriteHeader) {
		...
		while (hc->hcCommunicationPhase < CP__COUNT &&
		       !(hc->hcPhases[hc->hcCommunicationPhase] &
			 HCphaseUSED)) {
			hc->hcCommunicationPhase++;
		}
	} else {
		LDEB(hc->hcCommunicationPhase);
		rval = -1;
	}
...
static int sioOutHttpWriteHeaderBytes(void *voidhc, const unsigned char *buffer,
				      int count)
{
	HttpConnection *hc = (HttpConnection *)voidhc;

	if (hc->hcCommunicationPhase == CP_WriteHeader) {
		if (utilMemoryBufferAppendBytes(&(hc->hcRequestHeaders), buffer,
						count)) {
			LDEB(count);
			return -1;
		}
...
static int sioHttpCloseBodyOutput(void *voidhc)
{
	...
	if (hc->hcCommunicationPhase == CP_WriteBody) {
		...
		if (done != hc->hcRequestHeaders.mbSize) {
			LLDEB(hc->hcRequestHeaders.mbSize, done);
			return -1;
		}
		...
		if (done != hc->hcRequestBody.mbSize) {
			LLDEB(hc->hcRequestBody.mbSize, done);
			return -1;
		}
...
static int sioOutHttpWriteBodyBytes(void *voidhc, const unsigned char *buffer,
				    int count)
{
	HttpConnection *hc = (HttpConnection *)voidhc;

	if (hc->hcCommunicationPhase == CP_WriteBody) {
		if (utilMemoryBufferAppendBytes(&(hc->hcRequestBody), buffer,
						count)) {
			LDEB(count);
			return -1;
		}
...
static int sioHttpUseHeaderBuffer(HttpConnection *hc, unsigned char *buffer,
				  int count)
{
	int use;
...
static int sioInHttpReadHeaderBytes(void *voidhc, unsigned char *buffer,
				    unsigned int count)
{
	HttpConnection *hc = (HttpConnection *)voidhc;
	...
	if (hc->hcCommunicationPhase == CP_ReadHeader) {
		...
		if (hc->hcPhases[CP_ReadHeader] &
			...
			LXDEB(hc->hcCommunicationPhase,
			      hc->hcPhases[CP_ReadHeader]);
			return -1;
		}
		...
		while (done < count) {
			...
			if (hc->hcUseHeaderBuffer > 0) {
				got = sioHttpUseHeaderBuffer(hc, buffer + done,
							     count - done);
...
static int sioInHttpReadBodyBytes(void *voidhc, unsigned char *buffer,
				  unsigned int count)
{
	HttpConnection *hc = (HttpConnection *)voidhc;
...
static int sioHttpMakeRequest(int fd, const char *url, const char *method)
{
	char *request = (char *)0;
	int requestLength;
...
int sioHttpOpen(SimpleInputStream **pSisBody, SimpleInputStream **pSisHeader,
		SimpleOutputStream **pSosBody, SimpleOutputStream **pSosHeader,
		const char *host, const char *port, const char *url,
		const char *method, void *through, APP_COMPLAIN complain)
{
	int fd = -1;
