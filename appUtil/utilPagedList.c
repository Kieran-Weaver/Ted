...
static int utilPagedListCleanItem(int n, void *vpl)
{
...
void utilCleanPagedList(PagedList *pl)
{
	...
	if (pl->plCleanItem) {
		utilIndexSetForAll(&(pl->plItemUsed), utilPagedListCleanItem,
				   (void *)pl);
	}
...
static int utilPagedListForOne(int n, void *vfat)
{
			...
			void *through)
{
	...
	return utilIndexSetForAll(&(pl->plItemUsed), utilPagedListForOne,
				  (void *)&fat);
}
