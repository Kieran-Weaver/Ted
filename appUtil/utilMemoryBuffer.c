...
int utilMemoryBufferSetString(MemoryBuffer *mb, const char *s)
{
	...
	return utilMemoryBufferSetBytes(mb, (const unsigned char *)s,
					strlen(s));
}
...
int utilMemoryBufferAppendString(MemoryBuffer *mb, const char *s)
{
	if (!s) {
		XDEB(s);
		return -1;
	...
	return utilMemoryBufferAppendBytes(mb, (const unsigned char *)s,
					   strlen(s));
}
				 ...
				 int addedCount)
{
	...
	if (offHead < 0 || offHead > mb->mbSize) {
		LLDEB(offHead, mb->mbSize);
		return -1;
	}

	if (sizeShift > 0 &&
	    utilMemoryBufferSetSize(mb, mb->mbSize + sizeShift)) {
		LLDEB(mb->mbSize, sizeShift);
		return -1;
	}
	...
	if (oldSize > 0) {
		...
		if (n < 0) {
			...
			memmove(mb->mbBytes + offPast + sizeShift,
				mb->mbBytes + offPast, n);
		}
	}
...
int utilMemoryBufferLastIndexOf(const MemoryBuffer *mb, int chr)
{
	const char *p;
...
int utilMemoryBufferIndexOf(const MemoryBuffer *mb, int chr)
{
	const char *p;
