...
static int sioInPfbReadBytes(void *voidbfs, unsigned char *buffer,
			     unsigned int count)
{
	...
	while (done < count) {
		...
		if (bfs->bfsKindOfBlock == BINARY) {
			while (bfs->bfsBytesLeftInBlock > 0 && done < count) {
			...
			if (bfs->bfsBytesLeftInBlock > 0 && done < count) {
				c = sioInGetByte(sisPfb);
				if (c == EOF) {
					return done;
				}
