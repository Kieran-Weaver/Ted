				   ...
				   double r_1, double r_2)
{
	...
	if (x_2 == y_2 || x_2 == z_2) {
		...
		if (xy_1 / xy_2 == xz_1 / xz_2) {
			FFFFDEB(xy_1, xy_2, xz_1, xz_2);
			return -1;
		}

		at.at2Axx = (pq_1 / xy_2 - pr_1 / xz_2) /
			    (xy_1 / xy_2 - xz_1 / xz_2);
		at.at2Axy = (pq_2 / xy_2 - pr_2 / xz_2) /
			    (xy_1 / xy_2 - xz_1 / xz_2);
	}
	...
	if (x_1 == y_1 || x_1 == z_1) {
		...
		at.at2Ayx = (pq_1 / xy_1 - pr_1 / xz_1) /
			    (xy_2 / xy_1 - xz_2 / xz_1);
		at.at2Ayy = (pq_2 / xy_1 - pr_2 / xz_1) /
			    (xy_2 / xy_1 - xz_2 / xz_1);
	}
...
static int geoAffineTransformTest(const AffineTransform *at, double x_1,
				  double x_2, double p_1, double p_2)
{
	double d_1;
	double d_2;
	...
	if (d_1 < -0.00009 || d_1 > 0.00009 || d_2 < -0.00009 ||
	    d_2 > 0.00009) {
		FFFFDEB(x_1, x_2, p_1, p_2);
		FFDEB(d_1, d_2);
		return -1;
										...
										for (q_2 = 0;
											...
											for (r_1 = 0;
												...
												for (r_2 = 0;
													...
													geoAffineTransformForTriangles(
														&at,
														x_1,
														x_2,
														y_1,
...
void geoTranslationAffineTransform2D(AffineTransform2D *at2, double x, double y)
{
	geoInitAffineTransform2D(at2);
...
{
	return at2->at2Axx * at2->at2Ayy - at2->at2Axy * at2->at2Ayx;
}
...
int geoLineConstants(double *pA, double *pB, double *pC, double x0, double y0,
		     double x1, double y1)
{
	if (x1 == x0 && y1 == y0) {
		FFFFDEB(x0, y0, x1, y1);
...
#define TT(x, y, ex, ey)                                                                           \
	appDebug(                                                                                  \
		"(%8.3f, %8.3f) -> (%8.3f, %8.3f)\n                 Expect (%8.3f, %8.3f) %s\n\n", \
		(double)x, (double)y, AT2_X(x, y, &at), AT2_Y(x, y, &at),                          \
		(double)ex, (double)ey,                                                            \
		(about(AT2_X(x, y, &at), ex) && about(AT2_Y(x, y, &at), ey)) ?                     \
			"" :                                                                       \
			" !!!")
				 ...
				 const AffineTransform3D *a)
{
	...
	/**/
	c.at3Axx = b->at3Axx * a->at3Axx + b->at3Ayx * a->at3Axy +
		   b->at3Azx * a->at3Axz;
	c.at3Axy = b->at3Axy * a->at3Axx + b->at3Ayy * a->at3Axy +
		   b->at3Azy * a->at3Axz;
	c.at3Axz = b->at3Axz * a->at3Axx + b->at3Ayz * a->at3Axy +
		   b->at3Azz * a->at3Axz;

	/**/
	c.at3Ayx = b->at3Axx * a->at3Ayx + b->at3Ayx * a->at3Ayy +
		   b->at3Azx * a->at3Ayz;
	c.at3Ayy = b->at3Axy * a->at3Ayx + b->at3Ayy * a->at3Ayy +
		   b->at3Azy * a->at3Ayz;
	c.at3Ayz = b->at3Axz * a->at3Ayx + b->at3Ayz * a->at3Ayy +
		   b->at3Azz * a->at3Ayz;

	/**/
	c.at3Azx = b->at3Axx * a->at3Azx + b->at3Ayx * a->at3Azy +
		   b->at3Azx * a->at3Azz;
	c.at3Azy = b->at3Axy * a->at3Azx + b->at3Ayy * a->at3Azy +
		   b->at3Azy * a->at3Azz;
	c.at3Azz = b->at3Axz * a->at3Azx + b->at3Ayz * a->at3Azy +
		   b->at3Azz * a->at3Azz;

	/**/
	c.at3Tx = b->at3Axx * a->at3Tx + b->at3Ayx * a->at3Ty +
		  b->at3Azx * a->at3Tz + b->at3Tx;
	c.at3Ty = b->at3Axy * a->at3Tx + b->at3Ayy * a->at3Ty +
		  b->at3Azy * a->at3Tz + b->at3Ty;
	c.at3Tz = b->at3Axz * a->at3Tx + b->at3Ayz * a->at3Ty +
		  b->at3Azz * a->at3Tz + b->at3Tz;
