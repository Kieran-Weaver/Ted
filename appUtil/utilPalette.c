			  ...
			  int a)
{
	...
	if (col >= cp->cpColorCount && cp->cpColorCount >= maxCount) {
		LLDEB(cp->cpColorCount, maxCount);
		return -1;
	}
...
static int utilPaletteFindNearestColor(const ColorPalette *cp,
				       const RGB8Color *rgb8, int avoidZero)
{
	int d = 257;
	int i;
	int color = -1;
			   ...
			   int maxColors)
{
	...
	if (cpFrom->cpColorCount > avoidZero) {
		...
		if (!colorMap) {
			LXDEB(cpFrom->cpColorCount, colorMap);
			rval = -1;
			goto ready;
		}
	...
	for (from = avoidZero; from < cpFrom->cpColorCount;
	     rgb8From++, from++) {
		if (colorMap[from] >= 0) {
			continue;
		}
		...
		/*  2  */
		to = utilPaletteInsertColor(cpTo, avoidZero, maxColors,
					    rgb8From);
		if (to < 0) {
			LDEB(to);
			rval = -1;
