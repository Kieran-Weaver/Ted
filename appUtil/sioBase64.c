...
static int sioInBase64ReadBytes(void *voidbis, unsigned char *buffer,
				unsigned int count)
{
	...
	if (done < count && bis->bisCount > 0) {
		...
		memmove(bis->bisBytes, bis->bisBytes + todo,
			bis->bisCount - todo);
		bis->bisCount -= todo;
	}
...
static int sioOutBase64Close(void *voidbos)
{
	...
	if (bos->bosCount > 0) {
		...
		for (i = 0; i < bos->bosCount + 1; i++) {
			if (sioOutPutByte(UTIL_Base64Digits[d[i]],
					  bos->bosSosBase64) < 0) {
				rval = -1;
			}
		}
...
static int sioOutBase64WriteBytes(void *voidbos, const unsigned char *buffer,
				  int count)
{
	Base64OutputStream *bos = (Base64OutputStream *)voidbos;
	int done = 0;
	...
	/*  1  */
	while (bos->bosCount < 3 && done < count) {
		bos->bosBytes[bos->bosCount++] = *(buffer++);
		done++;
	}
	...
	util_Base64Make(d, bos->bosBytes);
	for (i = 0; i < 4; i++) {
		if (sioOutPutByte(UTIL_Base64Digits[d[i]], bos->bosSosBase64) <
		    0) {
			return -1;
		}
	}
