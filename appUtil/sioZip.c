...
void sioZipInitOutput(ZipOutput *zo)
{
	...
	utilStartPagedList(&(zo->zoFileList), sizeof(ZipOutputStream),
...
static int sioZipWriteLocalFileHeader(const ZipOutputStream *zos)
{
	...
	sioEndianPutLeUint32(ZIPsignatureMEMBER, sosZip);
	done += 4;
	sioEndianPutLeUint16(zos->zosVersion, sosZip);
	done += 2;
	sioEndianPutLeUint16(zos->zosFlags, sosZip);
	done += 2;
	sioEndianPutLeUint16(zos->zosMethod, sosZip);
	done += 2;
	sioEndianPutLeUint16(zos->zosDosTime, sosZip);
	done += 2;
	sioEndianPutLeUint16(zos->zosDosDate, sosZip);
	done += 2;
	sioEndianPutLeUint32(zos->zosAdlerCrc, sosZip);
	done += 4;
	sioEndianPutLeUint32(zos->zosCollectedSize, sosZip);
	done += 4;
	sioEndianPutLeUint32(zos->zosUncompressedSize, sosZip);
	done += 4;
	sioEndianPutLeUint16(fileNameLength, sosZip);
	done += 2;
	sioEndianPutLeUint16(extraLength, sosZip);
	done += 2;
...
static int sioZipWriteDirectoryEntry(const ZipOutputStream *zos)
{
	...
	sioEndianPutLeUint32(ZIPsignatureDIRECTORY_ENTRY, sosZip);
	done += 4;
	sioEndianPutLeUint16(zos->zosVersion, sosZip);
	done += 2;
	sioEndianPutLeUint16(zos->zosVersion, sosZip);
	...
	sioEndianPutLeUint16(commentLength, sosZip);
	done += 2;
	sioEndianPutLeUint16(diskNumberStart, sosZip);
	done += 2;
	sioEndianPutLeUint16(ZIPattributesINTERNAL, sosZip);
	done += 2;
	sioEndianPutLeUint32(ZIPattributesEXTERNAL, sosZip);
	done += 4;

	sioEndianPutLeUint32(zos->zosHeaderOffset, sosZip);
	done += 4;
...
static int sioZipWriteDirectoryEnd(const ZipOutput *zo)
{
	...
	sioEndianPutLeUint32(ZIPsignatureDIRECTORY_END, sosZip);
	done += 4;
	/*  number of this disk */
	sioEndianPutLeUint16(diskNumber, sosZip);
	done += 2;
	...
	/*  Total entry count */
	sioEndianPutLeUint16(zo->zoEntryCount, sosZip);
	done += 2;
	sioEndianPutLeUint32(zo->zoDirectorySize, sosZip);
	done += 4;
	sioEndianPutLeUint32(zo->zoDirectoryOffset, sosZip);
	done += 4;
	sioEndianPutLeUint16(commentLength, sosZip);
	done += 2;
...
static int sioZipEmitDirectoryEntry(int n, void *voidzos, void *voidzs)
{
...
int sioZipFlushOutput(ZipOutput *zo)
{
	...
	n = utilPagedListForAll(&(zo->zoFileList), sioZipEmitDirectoryEntry,
				(void *)&zs);
	if (n < 0) {
		LDEB(n);
		return -1;
...
static int sioOutZipFlushBytesDeflated(ZipOutputStream *zos, int n)
{
	z_stream *c_stream = &(zos->zosZstream);
	const unsigned char *b = zos->zosOutputBuffer;

	if (n > 0 &&
	    utilMemoryBufferAppendBytes(&(zos->zosCollectedBuffer), b, n)) {
		LDEB(n);
		return -1;
	}
				       ...
				       const unsigned char *buffer, int count)
{
	...
	while (c_stream->avail_in > 0) {
		...
		if (c_stream->avail_out == 0) {
			if (sioOutZipFlushBytesDeflated(zos, SIOsizBUF) !=
			    SIOsizBUF) {
				LDEB(SIOsizBUF);
				return -1;
			}
...
static int sioOutZipWriteBytesStored(void *voidzos, const unsigned char *buffer,
				     int count)
{
	ZipOutputStream *zos = (ZipOutputStream *)voidzos;
	...
	if (utilMemoryBufferAppendBytes(&(zos->zosCollectedBuffer), buffer,
					count)) {
		LDEB(count);
		return -1;
	}
...
static void sioOutZipFinishFile(ZipOutputStream *zos, int rval)
{
	ZipOutput *zo = zos->zosZipOutput;
	...
	if (rval) {
		if (zos->zosIndex >= 0) {
			utilPagedListDeleteItemByNumber(&(zo->zoFileList),
							zos->zosIndex);
		}
	} else {
		zo->zoEntryCount++;
...
static int sioZipStartDeflated(ZipOutputStream *zos)
{
	...
	ret = deflateInit2(&(zos->zosZstream), Z_DEFAULT_COMPRESSION,
...
SimpleOutputStream *sioOutZipOpen(ZipOutput *zo, const char *fileName,
				  int deflated)
{
...

	{
		...
		zos = (ZipOutputStream *)utilPagedListClaimNewItem(
			&n, &(zo->zoFileList));
		if (!zos) {
			XDEB(zos);
			goto ready;
	...
	if (deflated) {
		...
		sos = sioOutOpen((void *)zos, sioOutZipWriteBytesDeflated,
				 sioOutZipCloseDeflated);
		if (!sos) {
			LXDEB(deflated, sos);
			goto ready;
		}
		...
		sos = sioOutOpen((void *)zos, sioOutZipWriteBytesStored,
				 sioOutZipCloseStored);
		if (!sos) {
			LXDEB(deflated, sos);
			goto ready;
		}
