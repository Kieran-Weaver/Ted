...
static int sioInBlockedReadBytes(void *voidbis, unsigned char *buffer,
				 unsigned int count)
{
	...
	while (done < count) {
		...
		if (todo > count - done) {
			...
			if (sioInReadBytes(bis->bisBlocked, buffer, todo) !=
			    todo) {
				LDEB(todo);
				bis->bisExhausted = 1;
				return -1;
...
static int sioOutBlockedWriteBytes(void *voidbos, const unsigned char *buffer,
				   int count)
{
	BlockedOutputStream *bos = (BlockedOutputStream *)voidbos;
	int done = 0;
