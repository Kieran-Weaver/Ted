...
static int sioInFileioReadBytes(void *vfi, unsigned char *buffer,
				unsigned int count)
{
	FileIo *fi = (FileIo *)vfi;
...
static int sioFdOpenFileS(const char *filename, int read, int write, int append,
		  ...
		  int create, int exclusive)
{
	...
	return sioFdOpenFileS(utilMemoryBufferGetString(filename), read, write,
			      append, create, exclusive);
}
...
SimpleInputStream *sioInFileioOpenS(const char *filename)
{
	...
	fi->fiFd = sioFdOpenFileS(filename, read, write, append, create,
				  exclusive);
	if (fi->fiFd < 0) {
		SLDEB(filename, fi->fiFd);
		goto ready;
	}
	fi->fiOwnFd = 1;
...
static int sioOutFileioWriteBytes(void *vfi, const unsigned char *buffer,
				  int count)
{
	FileIo *fi = (FileIo *)vfi;
...
static SimpleOutputStream *sioOutFileioOpenSAt(const char *filename, int append,
					       long offset)
{
...
SimpleOutputStream *sioOutFileioOpen(const MemoryBuffer *filename)
{
	...
	return sioOutFileioOpenSAt(utilMemoryBufferGetString(filename), append,
				   offset);
}

SimpleOutputStream *sioOutFileioOpenForAppend(const MemoryBuffer *filename)
{
	const int append = 1;
	const long offset = 0L;
...
static SimpleOutputStream *sioOutFdOpenAtX(int fd, int seek, long offset)
{
	...
	if (seek) {
		if (lseek(fi->fiFd, offset, SEEK_SET) != offset) {
			LLLSDEB(fi->fiFd, offset, errno, strerror(errno));
			goto ready;
		}

		if (ftruncate(fi->fiFd, offset)) {
			LLSDEB(offset, errno, strerror(errno));
			goto ready;
		}
	}
