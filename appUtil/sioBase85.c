...
static int sioInBase85ReadBytes(void *voidbis, unsigned char *buffer,
				unsigned int count)
{
...
static int sioOutBase85WriteBytes(void *voidbos, const unsigned char *buffer,
				  int count)
{
	Base85OutputStream *bos = (Base85OutputStream *)voidbos;
	int done = 0;
	...
	/*  1  */
	while (bos->bosCount < 4 && done < count) {
