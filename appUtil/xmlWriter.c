...
void xmlWriteStringAttribute(XmlWriter *xw, const char *name, const char *value)
{
	if (xw->xwColumn > 1 && xw->xwColumn + 1 + strlen(name) + 1 + 3 > 76) {
		sioOutPutString("\n", xw->xwSos);
		xw->xwColumn = 0;
	} else {
			   ...
			   const RGB8Color *rgb8)
{
	...
	sprintf(scratch, "#%02x%02x%02x", rgb8->rgb8Red, rgb8->rgb8Green,
		rgb8->rgb8Blue);
...
void xmlEscapeCharacters(XmlWriter *xw, const char *ss, int len)
{
	...
	while (len > 0) {
		...
		if ((*us == '-' && xw->xwColumn == 0) || *us == '<' ||
		    *us == '>' || *us == '&' || *us == '\'' || *us == '"') {
			sprintf(scratch, "&#%03d;", *us);
			escaped = scratch;
		}
