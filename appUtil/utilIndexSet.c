...
static unsigned char *utilIndexSetClaimPage(IndexSet *is, int page)
{
	...
	if (!is->isPages[page]) {
		...
		fresh = (unsigned char *)malloc(ISET_PSZB *
...
static void utilIndexSetReleasePage(IndexSet *is, int page)
{
...
static void utilIndexSetTrimResult(IndexSet *res, int upto)
{
...
static int utilIndexSetCopyPage(IndexSet *to, const IndexSet *from, int page)
{
	int n = 0;
...
int utilIndexSetForAll(const IndexSet *is, IndexSetForOne forOne, void *through)
{
	int page;
	int n = 0;
...
int utilIndexSetAddArray(IndexSet *is, const int *indexes, int indexCount)
{
	...
	for (g = 0; g < indexCount; g++) {
		if (indexes[g] >= 0 && utilIndexSetAdd(is, indexes[g])) {
			LLDEB(g, indexes[g]);
		}
	}
