...
static const int LanguageCodeCount =
	sizeof(LanguageCodes) / sizeof(LanguageCode);
...
const char *utilLangForLanguageCode(int code)
{
	...
	while (l < m) {
		if (LanguageCodes[LanguageCodes[m].lcCodeIndex].lcCode <=
		    code) {
			l = m;
		} else {
			r = m;
	...
	if (code != LanguageCodes[LanguageCodes[m].lcCodeIndex].lcCode) {
		...
		return LanguageCodes[LanguageCodes[m].lcCodeIndex].lcLang;
	}
}
...
int utilLanguageCodeForLang(const char *lang)
{
	...
	if (strcmp(lang, LanguageCodes[m].lcLang) != 0) {
		...
		return LanguageCodes[m].lcCode;
	}
}

#if GENERATE

static int utilCompareLanguageCodesByNumber(const void *v1, const void *v2)
{
...
static int utilCompareLanguageCodesByString(const void *v1, const void *v2)
{
	const LanguageCode *lc1 = (const LanguageCode *)v1;
	const LanguageCode *lc2 = (const LanguageCode *)v2;
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < LanguageCodeCount; i++) {
		...
		printf("    { %5d, \"%s\",%s%4d }, /* 0x%04x */\n",
		       lcOut->lcCode, lcOut->lcLang, tabs + col,
		       lcOut->lcCodeIndex, LanguageCodes[i].lcCode);
	}
