...
int csvAddSheet(InputBook *ib, const char *fileName, const char *sheetName)
{
	InputSheet *fresh;

	fresh = (InputSheet *)realloc(ib->ibSheets, (ib->ibSheetCount + 1) *
							    sizeof(InputSheet));
	if (!fresh) {
		LXDEB(ib->ibSheetCount, fresh);
		return -1;
	}
	ib->ibSheets = fresh;
	...
	if (!fresh->isFileName || !fresh->isSheetName) {
...
int csvGetSheet(const InputBook *ib, const char *sheetName)
{
...
int csvGetInclude(int *pSheet, const InputBook *ib, const char *field0)
{
...
int csvReadRow(InputRow *ir, int sep, FILE *csvInFile)
{
	int c;
	...
	for (;;) {
		...
		if (ir->irFieldCount >= fieldCount) {
			...
			int *fresh = (int *)realloc(ir->irFields,
						    count * sizeof(int));
		...
		for (;;) {
			...
			if (ir->irByteCount >= byteCount) {
				...
				char *fresh =
					(char *)realloc(ir->irBytes, count + 1);
...
int csvReadSheet(InputSheet *is, int sep, FILE *csvInFile)
{
	int rowCount = 0;
	...
	for (;;) {
		...
		if (is->isRowCount >= rowCount) {
			...
			InputRow *fresh = (InputRow *)realloc(
				is->isRows, count * sizeof(InputRow));
			int i;
...
int csvWriteCsvOutFile(const InputSheet *is, int sep, FILE *csvOutFile)
{
	int row;
	int col;
...
int csvFindRowByLabel(const InputSheet *is, const char *label, int col,
		      int filterColumn)
{
	int row;
	const InputRow *ir;
	...
	if (row >= is->isRowCount) { /* SDEB(label); */
		return -1;
	}
...
int csvFindColumnByLabel(const InputRow *ir, const char *label)
{
	int col;
	int len;
	...
	if (col >= ir->irFieldCount) { /* SDEB(label); */
		return -1;
	}
...
int csvFindRowByLabelIndirect(int *pSheet, int *pRow, int *pNrFound, int *pNr,
			      const InputBook *ib, int sheet, const char *label,
			      int filterColumn)
{
	int row;
	...
	for (row = 0; row < is->isRowCount; ir++, row++) {
		...
		if (!strncmp(field0, "::include ", 10)) {
			...
			ret = csvFindRowByLabelIndirect(pSheet, pRow, pNrFound,
							&nr, ib, n, label,
							filterColumn);
			if (ret < 0) {
				LDEB(ret);
...
int csvNormaliseNumber(char *field)
{
	...
	*to = '\0';
	...
	if (to - field > 8 && !gotDot && from - field > 10) {
		strcpy(to, ".0");
	}
...
void csvStripLength(const char **pS, int *pL)
{
	const char *s = *pS;
	const char *r;
	int i;
...
int csvGetIntFromRow(int *pI, const InputRow *ir, int col)
{
	int i;
	char *past;

	if (col >= ir->irFieldCount) {
		LLDEB(col, ir->irFieldCount);
		return -1;
	}
	...
	if (past == ir->irBytes + ir->irFields[col] || *past) {
		SDEB(ir->irBytes + ir->irFields[col]);
		return -1;
	}
...
int csvGetDoubleFromRow(double *pD, const InputRow *ir, int col)
{
	double d;
	char *past;
