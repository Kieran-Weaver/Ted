			    ...
			    const DocumentSelection *dsSet)
{
	...
	rootNodeSet =
		docGetSelectionRoot(&treeSet, &bodySectNodeSet, bd, dsSet);
	if (!rootNodeSet) {
		XDEB(rootNodeSet);
		return -1;
	}
	...
	case DOCinENDNOTE: {
		...
		if (docGetTreeOfNode(&tree, &bodySectNodeSet, bd,
				     rootNodeSet)) {
			LDEB(1);
			return -1;
		}
	...
	case DOCinAFTNCN: {
		...
		if (docGetBoxAroundTree(drExternalSet, bodySectNodeSet, treeSet,
					justUsed, page, column, lc)) {
			LDEB(rootNodeSet->biTreeType);
			redrawTreeSet = 0;
		}
