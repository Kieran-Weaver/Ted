...
static int docGetBitmap(bmReadBitmap readBitmap, RasterImage *ri,
...
int docCheckObjectLayout(int *pFixed, InsertedObject *io)
{
	...
	if (io->ioKind == DOCokDRAWING_SHAPE) {
		...
		if (!ds) {
			...
			if (io->ioTwipsWide == 0) {
				io->ioTwipsWide =
					drTwips.drX1 - drTwips.drX0 + 1;
				fixed = 1;
			}
			if (io->ioTwipsHigh == 0) {
				io->ioTwipsHigh =
					drTwips.drY1 - drTwips.drY0 + 1;
				fixed = 1;
			}
		}
	}

	if ((pip->pipTwipsWide == 0 || pip->pipTwipsHigh == 0) &&
	    (io->ioKind == DOCokPICTPNGBLIP ||
	     io->ioKind == DOCokPICTJPEGBLIP)) {
		if (!io->ioRasterImage.riBytes && docGetBitmapForObject(io)) {
			LDEB(1);
		} else {
			bmRectangleSizeTwips(
	...
	const DocumentRectangle *drParaContent)
{
	...
	if (io->ioScaleXUsed == io->ioScaleXSet &&
	    ...
	    objectWideTwips <= textWideTwips && objectHighTwips <= pageHigh) {
		*pChanged = 0;
		return;
	}
	...
	if (scaleMax < io->ioScaleXUsed || scaleMax < io->ioScaleYUsed) {
...
void docScaleObjectToParagraph(BufferDocument *bd, BufferItem *paraBi,
			       double xfac, InsertedObject *io)
{
	...
	docLayoutScaleObjectToFitParagraphFrame(&changed, io, pageHigh,
	...
	io->ioPixelsWide =
		COORDtoGRID(xfac, (io->ioScaleXUsed * pip->pipTwipsWide) / 100);
	io->ioPixelsHigh =
		COORDtoGRID(xfac, (io->ioScaleYUsed * pip->pipTwipsHigh) / 100);
