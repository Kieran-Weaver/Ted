...
void docBlockFrameTwips(BlockFrame *bf, BufferItem *node, BufferDocument *bd,
			int page, int column)
{
	DocumentTree *dt;
	int useColumns = 0;
	...
	const NotesProperties *npEndnotes =
		&(bd->bdProperties.dpNotesProps.fepEndnotesProps);
	...
	if (sectNode->biTreeType != DOCinBODY &&
	    sectNode->biTreeType != DOCinSHPTXT && bodySectNode) {
	...
	if (useColumns && sp->spColumnCount > 1) {
		...
		docSectGetColumnX(&xLine, &(bf->bfContentRect.drX0),
				  &(bf->bfContentRect.drX1), sp,
				  &(bf->bfPageGeometry), column);
	}
	...
	case DOCinRIGHT_FOOTER:
		...
		if (column != 0) {
			SLLDEB(docTreeTypeStr(sectNode->biTreeType), page,
			       column);
			column = 0;
		}
	...
	case DOCinSHPTXT:
		...
		bf->bfContentRect.drY1 = bf->bfPageGeometry.dgPageHighTwips -
					 bf->bfPageGeometry.dgBottomMarginTwips;
		break;
...
void docParaBlockFrameTwips(BlockFrame *bf, BufferItem *paraNode,
			    BufferDocument *bd, int page, int column)
{
	if (paraNode->biLevel != DOClevPARA) {
		...
		if (DOCisFRAME(&fp)) {
			...
			docLayoutSetTextFrame(bf, &lpScratch, &bfPage, &fp,
					      frameHigh);
...
static void docShiftCellFrame(ParagraphFrame *pf, const BufferItem *rowNode)
{
	...
	while (parentBi) {
		if (parentBi->biLevel == DOClevROW) {
			...
			if (nrInParent == 0) {
				...
				x0 = rp->rpCells[nrInParent - 1]
					     .cpRightBoundaryTwips;
			}
		       ...
		       const BufferItem *cellNode)
{
	...
	docGetCellWidth(&(pf->pfCellRect), &leftMargin, &rightMargin, cellNode,
			rp, bf);
	...
	if (rowNode->biRowCellCount < 1) {
		...
		pf->pfRedrawX1Twips = rp->rpCells[rowNode->biRowCellCount - 1]
					      .cpRightBoundaryTwips;
	}
	...
	if (rowNode->biRowHeightTwips < 0) {
		...
		pf->pfCellRect.drY1 =
			pf->pfCellRect.drY0 - rowNode->biRowHeightTwips;
	}
			   ...
			   const BufferItem *cellNode)
{
	...
	if (bf->bfPage > lpTop->lpPage ||
	    (bf->bfPage == lpTop->lpPage && bf->bfColumn > lpTop->lpColumn)) {
		if (sectNode->biTopPosition.lpPage == bf->bfPage) {
			pf.pfCellRect.drY0 =
				sectNode->biTopPosition.lpPageYTwips;
		} else {
	...
	if (bf->bfPage < lpBelow.lpPage ||
	    (bf->bfPage == lpBelow.lpPage && bf->bfColumn < lpBelow.lpColumn)) {
		if (sectNode->biBelowPosition.lpPage == bf->bfPage) {
			pf.pfCellRect.drY1 =
				sectNode->biBelowPosition.lpPageYTwips;
		} else {
...
void docLayoutSectColumnTop(LayoutPosition *lpTop, BlockFrame *bf,
			    BufferItem *bodySectNode, BufferDocument *bd)
{
	if (bodySectNode->biTreeType != DOCinBODY) {
		SDEB(docTreeTypeStr(bodySectNode->biTreeType));
	}

	docBlockFrameTwips(bf, bodySectNode, bd, lpTop->lpPage,
			   lpTop->lpColumn);
