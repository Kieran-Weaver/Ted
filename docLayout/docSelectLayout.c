...
static int docParaGetFirstInColumn(DocumentPosition *dp, int *pLineTop,
				   int *pPartTop, BufferItem *paraBi,
				   int lineUpto, int page, int column)
{
	int i;
	...
	if (paraBi->biParaLineCount > 0 && i >= paraBi->biParaLineCount) {
		...
		if (tl->tlFlags & TLflagBLOCKBREAK) {
			...
			if (nextBi && nextBi->biParaLineCount > 0) {
				i = 0;
				paraBi = nextBi;
				tl = paraBi->biParaLines;
			}
		}
	}

	if (i >= paraBi->biParaLineCount || tl->tlTopPosition.lpPage != page ||
...
int docGetFirstInColumnForNode(DocumentPosition *dp, int *pLineTop,
			       int *pPartTop, BufferItem *node, int page,
			       int column)
{
	int i;
	...
	while (node && node->biLevel != DOClevPARA) {
		...
		for (i = 0; i < node->biChildCount; i++) {
			if (node->biChildren[i]->biBelowPosition.lpPage >=
			    page) {
				if (node->biChildren[i]->biBelowPosition.lpPage >
				    page) {
					break;
				}
				if (node->biChildren[i]
					    ->biBelowPosition.lpColumn >=
				    column) {
					break;
				}
			}
	...
	return docParaGetFirstInColumn(dp, pLineTop, pPartTop, node,
				       node->biParaLineCount, page, column);
}
...
int docGetLastInColumnForNode(DocumentPosition *dp, int *pLineBot,
			      int *pPartBot, BufferItem *nodeIn, int page,
			      int column)
{
	int i;
	const TextLine *tl;
	...
	while (node && node->biLevel != DOClevPARA) {
		if (node->biTopPosition.lpPage >
		    page) { /*LLDEB(node->biBelowPosition.lpPage,page);*/
			return 1;
		}
		if (node->biBelowPosition.lpPage <
		    page) { /*LLDEB(node->biBelowPosition.lpPage,page);*/
			return 1;
		}
		...
		for (i = node->biChildCount - 1; i >= 0; i--) {
			if (node->biChildren[i]->biTopPosition.lpPage <= page) {
				if (node->biChildren[i]->biTopPosition.lpPage <
				    page) {
					break;
				}
				if (node->biChildren[i]->biTopPosition.lpColumn <=
				    column) {
					break;
				}
			}
	...
	if (i < 0 || tl->tlTopPosition.lpPage != page ||
	    tl->tlTopPosition.lpColumn != column) {
		SSDEB(docTreeTypeStr(node->biTreeType),
...
int docGetBottomOfColumn(DocumentPosition *dp, int *pPartBot,
			 BufferDocument *bd, int page, int column)
{
	int lineBot;
