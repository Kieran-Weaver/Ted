	...
	while (sectNr >= 0) {
		...
		if (sectNr == 0 || (sectNode->biSectBreakKind != DOCibkNONE &&
			...
			docPageRectsPixels(&drOutside, &drInside,
					   lc->lcPixelsPerTwip, sectNode,
					   lc->lcDocument);

			step = drOutside.drY1 - drOutside.drY0 + 1 +
			       lc->lcPageGapPixels;
				   ...
				   const LayoutContext *lc)
{
	...
	drPixelsB.drX0 = drPixelsB.drX1 =
		pgB->pgXPixels; /* Do not trust XTwips yet */

	docGetPixelRectForPos(&drPixelsE, lc, pgE->pgXTwips, pgE->pgXTwips,
			      &(pgE->pgTopPosition), &(pgE->pgBottomPosition));
	drPixelsE.drX0 = drPixelsE.drX1 =
		pgE->pgXPixels; /* Do not trust XTwips yet */
			...
			const BufferDocument *bd)
{
	...
	if (drIn) {
		...
		drIn->drX1 =
			drPage.drX1 - COORDtoGRID(xfac, dg->dgRightMarginTwips);
		drIn->drY1 = drPage.drY1 -
			     COORDtoGRID(xfac, dg->dgBottomMarginTwips);
	}
...
int docGetPageForYPixels(const LayoutContext *lc, int yPixels)
{
	...
	docPageRectsPixels(&drOutside, &drInside, lc->lcPixelsPerTwip, sectNode,
			   lc->lcDocument);
	...
	for (sectNr = 1; sectNr < bodyNode->biChildCount; sectNr++) {
		...
		if (sectNode->biSectBreakKind != DOCibkNONE &&
			...
			pagesBefore = topPage;
			candidate =
				pagesBefore + (yPixels - heightBefore) / step;
		}
	}
