...
static void docLayoutTopOfRowspan(ParagraphLayoutJob *plj, BufferItem *cellNode,
				  const BufferItem *rowNode, int l, int r)
{
	int topRow;
	...
	for (topRow = rowNode->biNumberInParent - 1;
	     topRow >= rowNode->biRowTableFirst; topRow--) {
		int colspan;
		int topCol;
...
void docRowLayoutRowspanAdmin(BufferItem *cellNode, const BufferItem *rowNode,
			      const BufferItem *nextRowBi, int nextRow,
	...
	if (nextRow < rowNode->biRowTablePast) {
		...
		if (nextCol >= 0) {
			...
			if (cp->cpVerticalMerge == CELLmergeFOLLOW &&
			    nextCp->cpVerticalMerge != CELLmergeFOLLOW) {
				docLayoutTopOfRowspan(plj, cellNode, rowNode, l,
						      r);
			}
		} else {
			if (cp->cpVerticalMerge == CELLmergeFOLLOW) {
