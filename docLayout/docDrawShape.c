...
int docDrawShapeGetLine(int *pLine, RGB8Color *rgb8, const DrawingShape *ds,
			DrawingContext *dc, void *through)
{
	int line = 1;
			       ...
			       void *through)
{
	...
	if (ds->dsDrawing.sdShapeType == SHPtyGROUP) {
		...
		for (child = 0; child < ds->dsChildCount; child++) {
			...
			if (docDrawDrawingShape(&drChild, bodySectNode, page,
						column, dsChild, dc, through)) {
				LDEB(child);
				rval = -1;
			}
		}
		...
		if (ds->dsDocumentTree.dtRoot &&
		    docDrawShapeText(&drHere, bodySectNode, page, column, ds,
				     dc, through)) {
			LDEB(page);
			rval = -1;
		}
		 ...
		 const InsertedObject *io)
{
	...
	docDrawDrawingShape(&drTwips, bodySectNode, page, column, ds, dc,
			    through);
	return 0;
}
