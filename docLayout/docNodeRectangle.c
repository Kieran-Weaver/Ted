		      ...
		      const LayoutContext *lc, const BlockOrigin *bo)
{
	...
	/********************************************************************/
	if (node->biLevel == DOClevBODY || lpTop.lpPage != lpBelow.lpPage ||
	    lpTop.lpColumn != lpBelow.lpColumn) {
		docGetPixelRectangleForPages(drPixels, lc, lpTop.lpPage,
					     lpBelow.lpPage);
		goto ready;
	}

	docBlockFrameTwips(&bf, node, lc->lcDocument, lpTop.lpPage,
			   lpTop.lpColumn);
	...
	docGetPixelRectForPos(drPixels, lc, drTwips.drX0, drTwips.drX1, &lpTop,
			      &lpBelow);
	if (drPixels->drX0 > 0)
