...
static void psSaveInfo(const char *tag, const char *info, PrintingState *ps)
{
	if (!info) {
		return;
	}
...
static void psSaveMbInfo(const char *tag, const MemoryBuffer *info,
			 PrintingState *ps)
{
...
static void psSaveDate(const char *tag, const struct tm *tm, PrintingState *ps)
{
	char scratch[40 + 1];
			...
			const DocumentProperties *dp)
{
	...
	utilMemoryBufferPrintf(&scratch, "%s: %s", applicationName,
...
static int docPsOutlinePdfmark(PrintingState *ps, BufferDocument *bd,
			       const TocEntry *te, int closeLevel, int count)
{
	int rval = 1;
	int childCount = 0;
	...
	psOutlinePdfmark(ps, childCount, te->teLevel >= closeLevel, &title,
			 te->teMarkName);
	...
	if (childCount > 0) {
		...
		for (c = 1; c < rval; c++) {
			if (te[c].teLevel == te[1].teLevel) {
				docPsOutlinePdfmark(ps, bd, te + c, closeLevel,
						    count - c);
			}
		}
	}
...
int docPsOutlinePdfmarks(PrintingState *ps, BufferDocument *bd)
{
	...
	while (done < ct.ctEntryCount) {
		...
		step = docPsOutlinePdfmark(ps, bd, ct.ctEntries + done, level,
					   ct.ctEntryCount - done);
		if (step <= 0) {
			LDEB(step);
			rval = -1;
			goto ready;
