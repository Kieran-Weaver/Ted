...
static void docDrawRunUnderlines(const DrawTextLine *dtl, int part, int upto,
				 const LayoutPosition *baseLine)
{
	...
	while (part < upto) {
		...
		if (docNextUnderlinedRun(&head, &past, &textAttrNr, &ta, bd,
					 paraNode, part, upto)) {
			break;
		}
		...
		(*dc->dcDrawUnderline)(dtl, head, past, textAttrNr, &ta,
...
static void docDrawRunStrikethrough(const DrawTextLine *dtl, int part, int upto,
				    const LayoutPosition *baseLine)
{
	DrawingContext *dc = dtl->dtlDrawingContext;
	const LayoutContext *lc = &(dc->dcLayoutContext);
	...
	while (part < upto) {
		...
		if (docNextStrikethroughRun(&head, &past, &textAttrNr, &ta, bd,
					    paraNode, part, upto)) {
			break;
		}
		...
		(*dc->dcDrawStrikethrough)(dtl, head, past, textAttrNr, &ta,
					   x0Twips, x1Twips, baseLine);
			      ...
			      DrawingContext *dc)
{
	...
	while (part < partUpto) {
		...
		next = docDrawBorderStretch(&ignored, &xTwips, &drText,
					    baseline, borderNumber, dc,
					    paraNode, part, partUpto);
		...
		if (next > part && xTwips > drText.drX0) {
			...
			if (DOCisBORDER(&(ornaments.boTopBorder))) {
				...
				ornaments.boBottomBorder =
					ornaments.boTopBorder;

				ornaments.boLeftBorderNumber =
					ornaments.boTopBorderNumber;
				ornaments.boRightBorderNumber =
					ornaments.boTopBorderNumber;
				ornaments.boBottomBorderNumber =
					ornaments.boTopBorderNumber;

				geoIntersectRectangle(&drText, &drText, drLine);

				thick = docBorderThick(
					&space, &(ornaments.boTopBorder));
				drInside = drText;
			      ...
			      DrawingContext *dc)
{
	...
	while (part < partUpto) {
		...
		next = docDrawShadingStretch(&ignored, &xTwips, &drText,
					     baseline, shadingNumber, dc,
					     paraNode, part, partUpto);
		...
		if (next > part && xTwips > drText.drX0) {
			...
			if ((*dc->dcDrawOrnaments)(&ornaments, page, &drText,
						   &drText, through, dc)) {
				LDEB(1);
				return -1;
			}
...
static int docDrawLineBackground(const BufferItem *paraNode, const TextLine *tl,
				 const ParagraphFrame *pf, void *through,
				 DrawingContext *dc, const BlockOrigin *bo)
{
	...
	if (tl->tlFlags & TLflagSHADING) {
		if (docDrawTextShading(paraNode, tl, baseline, &drLine, page,
				       xTwips, through, dc)) {
			LDEB(1);
			return -1;
		}
	...
	if (tl->tlFlags & TLflagBORDER) {
		if (docDrawTextBorders(paraNode, tl, baseline, &drLine, page,
				       xTwips, through, dc)) {
			LDEB(1);
			return -1;
		}
...
int docDrawTextLines(void *through, const ParagraphDrawingStrip *pds,
		     BufferItem *paraNode, const ParagraphFrame *pf,
		     DrawingContext *dc, const BlockOrigin *bo)
{
	const LayoutContext *lc = &(dc->dcLayoutContext);
	int done = 0;
	...
	while (line < paraNode->biParaLineCount) {
		...
		/*  6  */
		if (dc->dcSelection && !beforeSelectionBegin &&
		    !pastSelectionEnd &&
		...
		if (beforeSelectionBegin || pastSelectionEnd ||
			...
			if (dc->dcDrawOrnaments &&
				...
				docDrawLineBackground(paraNode, tl, pf, through,
						      dc, bo);
			}

			accepted = (*dc->dcDrawTextLine)(paraNode, line, pf,
							 &drLinePixels, through,
							 dc, bo);
		}
...
int docDrawLineParticules(const DrawTextLine *dtl, int *pXTwips, int part)
{
	...
	case DOCkindTAB:
		if (dc->dcDrawTab) {
			...
			if ((*dc->dcDrawTab)(dtl, part, textAttrNr, &ta, leader,
					     x0Twips, x1Twips, &spanBaseline)) {
				LDEB(1);
			}

			if (ta.taTextIsUnderlined && dc->dcDrawUnderline) {
				docDrawRunUnderlines(dtl, part, part + 1,
						     &spanBaseline);
			}
			if (ta.taHasStrikethrough && dc->dcDrawStrikethrough) {
				docDrawRunStrikethrough(dtl, part, part + 1,
							&spanBaseline);
			}
		}
	...
	case DOCkindOBJECT:
		if (dc->dcDrawObject) {
			...
			drawn = (*dc->dcDrawObject)(dtl, part, io, x0Twips,
						    x1Twips, &spanBaseline);
	...
	case DOCkindCHFTNSEPC:
		if (dc->dcDrawFtnsep) {
			(*dc->dcDrawFtnsep)(dtl, part, textAttrNr, &ta, x0Twips,
					    x1Twips, &spanBaseline);
		}
		(*pXTwips) += tp->tpTwipsWide;
		return drawn = 1;
...

	{
		...
		drawn = docLayoutDelimitRun(&tr, *pXTwips, bd, paraNode, part,
					    dtl->dtlPartUpto,
		...
		if (tr.trStrlenNb > 0 && dc->dcDrawSpan) {
			...
			printString =
				(char *)docParaString(paraNode, tp->tpStroff);
		...
		if (tr.trHasUnderline && dc->dcDrawUnderline) {
			docDrawRunUnderlines(dtl, tr.trPartFrom, tr.trPartUpto,
					     &spanBaseline);
		}
		if (tr.trHasStrikethrough && dc->dcDrawStrikethrough) {
			docDrawRunStrikethrough(dtl, tr.trPartFrom,
						tr.trPartUpto, &spanBaseline);
		}
