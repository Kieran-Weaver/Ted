...
int docLayoutDelimitRun(TextRun *tr, const int x0Twips, BufferDocument *bd,
			const BufferItem *paraNode, int part, int past,
			int separate)
{
	int count = past - part;
	int done;
	...
	textAttrNr =
		docGetEffectiveTextAttributes(&taFound, bd, paraNode, part);
	...
	for (done = 1; done < count; done++) {
		...
		nx = docGetEffectiveTextAttributes(&taNx, bd, paraNode,
						   part + done);

		if (nx != textAttrNr || tp[done].tpKind != DOCkindSPAN) {
			break;
		}
	...
	if (part + done < paraNode->biParaParticuleCount - 1 &&
	    tp[done].tpKind == DOCkindOPT_HYPH && tp[done].tpTwipsWide > 0) {
...
int docNextUnderlinedRun(int *pHead, int *pPast, int *pTextAttrNr,
			 ...
			 const BufferItem *paraNode, int part, int upto)
{
	int head = part;
	TextAttribute taFound;
