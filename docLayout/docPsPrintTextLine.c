...
static void docPsPrintString(const DrawTextLine *dtl, int baseline,
			     const ParticuleData *pd, int textAttr,
			     const TextAttribute *ta, const unsigned char *s,
			     int len)
{
	...
	if (len > 0) {
		...
		if (ta->taSuperSub == TEXTvaSUPERSCRIPT) {
			psGetSuperBaseline(&y, baseline, fontSizeTwips,
					   pd->pdAfi);
		}

		if (ta->taSuperSub == TEXTvaSUBSCRIPT) {
			psGetSubBaseline(&y, baseline, fontSizeTwips,
					 pd->pdAfi);
		}
...
static void psPrintSegment(const DrawTextLine *dtl, int textAttr,
			   const TextAttribute *ta, const unsigned char *s,
			   int len)
{
	PrintingState *ps = (PrintingState *)dtl->dtlThrough;
...
static void psPrintSegments(const DrawTextLine *dtl, int baseLine,
			    const ParticuleData *pd, int textAttr,
			    const TextAttribute *ta, const unsigned char *s,
			    const int *segments, int segmentCount)
{
	...
	if (segments[0] > 0) {
		docPsPrintString(dtl, baseLine, pd, textAttr, &taN, s,
				 segments[0]);
		s += segments[0];
		...
		docPsPrintString(dtl, baseLine, pd, textAttr, ta, s,
				 segments[1]);
		s += segments[1];
	}
	...
	for (seg = 1; seg < segmentCount; seg++) {
		if (segments[2 * seg + 0] > 0) {
			psPrintSegment(dtl, textAttr, &taN, s,
				       segments[2 * seg + 0]);
			s += segments[2 * seg + 0];
		}

		if (segments[2 * seg + 1] > 0) {
			psPrintSegment(dtl, textAttr, ta, s,
				       segments[2 * seg + 1]);
			s += segments[2 * seg + 1];
		}
	}
...
int docPsPrintRunStrikethrough(const DrawTextLine *dtl, int part, int upto,
		   ...
		   const TextAttribute *ta, const char *printString, int nbLen)
{
	...
	if (ta->taSmallCaps && !ta->taCapitals) {
		psPrintSegments(dtl, spanBaseline, pd, textAttrNr, ta,
				(unsigned char *)printString, segments,
				segmentCount);
	} else {
			 ...
			 const DocumentField *df)
{
	...
	if (df->dfKind == DOCfkCHFTN) {
		...
		utilMemoryBufferPrintf(&(ps->psNoteRef), "_NREF_%d",
				       df->dfFieldNumber + 1);
		utilMemoryBufferPrintf(&(ps->psNoteDef), "_NDEF_%d",
				       df->dfFieldNumber + 1);
	...
	if (df->dfKind == DOCfkHYPERLINK) {
		if (!docGetHyperlinkField(&hf, df)) {
			...
			if (utilCopyMemoryBuffer(&(ps->psLinkFile),
						 &(hf.hfFile))) {
				LDEB(1);
				rval = -1;
				goto ready;
			}
			if (utilCopyMemoryBuffer(&(ps->psLinkMark),
						 &(hf.hfBookmark))) {
				LDEB(1);
				rval = -1;
				goto ready;
			  ...
			  const DocumentField *df)
{
	...
	if (ps->psInsideLink) {
		psFlushLink(ps, pd->pdX0, pd->pdVisibleBBox.drX1, lineTop,
			    lineHeight);
		ps->psInsideLink = 0;
	}
		       ...
		       DrawingContext *dc, const BlockOrigin *bo)
{
	...
	boxLine = *tl;
	accepted =
		docLayoutLineBox(&boxLine, paraNode, part, lc, pd + part, pf);
	...
	dtl.dtlDrawParticulesSeparately = paraNode->biParaAlignment ==
					  DOCthaJUSTIFIED;
	...
	if (done > 0 && ps->psInsideLink) {
		...
		psFlushLink(ps, ppd->pdX0, ppd->pdVisibleBBox.drX1, lineTop,
			    lineHeight);
	}
