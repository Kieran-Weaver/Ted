...
static int docGetParaTopBorder(BorderProperties *bpTop, int *pNrAbove,
			       int *pFillBefore, const BufferDocument *bd,
			       ...
			       const BufferItem *cellNode)
{
	...
	if (docBorderNumberIsBorder(bd, paraNode->biParaTopBorderNumber)) {
		if (!prevNode ||
			...
			docGetBorderPropertiesByNumber(
				bpTop, bd, paraNode->biParaTopBorderNumber);
...
static int docGetParaBottomBorder(BorderProperties *bpBottom, int *pNrBelow,
				  int *pFillAfter, const BufferDocument *bd,
				  const BufferItem *paraNode,
				  const BufferItem *cellNode)
{
	int rval = 0;
				    ...
				    BufferItem *paraNode)
{
	...
	if (paraNode->biParaTableNesting > 0 &&
		...
		const CellProperties *cp =
			rp->rpCells + cellNode->biNumberInParent;

		if (!docIsRowNode(rowNode)) {
			LDEB(1);
			cp = (const CellProperties *)0;
		}
	...
	/*  5  */
	if (docGetParaTopBorder(&bpTop, &nrAbove, &fillBefore, bd, paraNode,
				cellNode)) {
				       ...
				       BufferItem *paraNode)
{
	...
	if (paraNode->biParaTableNesting > 0 &&
	    cellNode->biNumberInParent < rowNode->biRowProperties.rpCellCount &&
	...
	if (docGetParaBottomBorder(&bpBottom, &nrBelow, &fillAfter, bd,
				   paraNode, cellNode)) {
...
int docLayoutParagraphLineExtents(int *pFontSizeTwips, const LayoutContext *lc,
				  BufferItem *paraNode)
{
	...
	for (part = 0; part < paraNode->biParaParticuleCount; tp++, part++) {
		if (tp->tpKind != DOCkindSPAN && tp->tpKind != DOCkindTAB &&
		    tp->tpKind != DOCkindOBJECT) {
			continue;
		}

		if (tp->tpTextAttrNr < 0 || tp->tpTextAttrNr >= count) {
	...
	if (found >= 0) {
		...
		if (fontHigh < 2) {
			LLDEB(ta.taFontSizeHalfPoints, fontHigh);
			fontHigh = 2;
		}
					...
					const BufferDocument *bd)
{
	...
	for (col = 0; col < rowNode->biChildCount; cp++, col++) {
		...
		docGetCellBottomBorder(&bpBottom, &bottomNr, &useBelow, bd,
			 ...
			 int atParaTop, int atParaBottom)
{
	...
	if (atParaTop) {
		...
		docGetParaTopBorder(&bpTop, &nrAbove, &fillBefore, bd, paraNode,
				    paraNode->biParent);
	...
	if (atParaBottom) {
		...
		docGetParaBottomBorder(&bpBottom, &nrBelow, &fillAfter, bd,
				       paraNode, paraNode->biParent);
...
static int docStartParagraphLayout(const ParagraphFrame *pf,
				   const BlockFrame *bf, BufferItem *paraNode,
				   const LayoutContext *lc)
{
	...
	for (part = 0; part < paraNode->biParaParticuleCount; tp++, part++) {
		...
		docLayoutScaleObjectToFitParagraphFrame(
			&changed, io, pageHigh, &(pf->pfParaContentRect));
...
int docLayoutStartParagraph(const LayoutJob *lj, int *pStopCode,
			    BufferItem *paraNode, const BlockFrame *bf,
			    ParagraphLayoutPosition *plp)
{
	...
	if (docStartParagraphLayout(&(plp->plpParagraphFrame), bf, paraNode,
				    lc)) {
		LDEB(1);
		return -1;
	}
	...
	if (lj->ljStartScreenParagraph &&
	    (*lj->ljStartScreenParagraph)(paraNode, &(plp->plpParagraphFrame),
					  lc)) {
		LDEB(1);
		return -1;
	}
