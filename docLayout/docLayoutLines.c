...
static int docAboveLine(int *pStopCode, LayoutPosition *lp,
			const BufferItem *paraNode, int part,
			const BlockFrame *bf, const ParagraphFrame *pf)
{
	...
	if (lp->lpPageYTwips + spaceAboveLineTwips >=
	    pf->pfParaContentRect.drY1) {
		*pStopCode = FORMATstopFRAME_FULL;
		return 1;
	}
...
static int docBelowLine(int *pStopCode, LayoutPosition *lp,
			const LayoutPosition *lpLineTop, int lineStride,
			const BufferItem *paraNode, int partFrom, int partUpto,
			const BlockFrame *bf, const ParagraphFrame *pf,
			const NotesReservation *nrLine)
{
	...
	if (lineBottom > pf->pfParaContentRect.drY1 &&
	    lineHeight <
		    pf->pfParaContentRect.drY1 - pf->pfParaContentRect.drY0) {
		*pStopCode = FORMATstopFRAME_FULL;
		return 1;
	}
...
static int docPlaceShape(const BufferItem *paraNode,
			 const LayoutPosition *lpLineTop, InsertedObject *io,
			 const ParagraphFrame *pf, const BlockFrame *bf,
			 int xChar)
{
	...
	docShapePageRectangle(&(io->ioY0Position), &lpBelowShape,
			      &(io->ioX0Twips), &x1, ds, paraNode, lpLineTop,
			      pf, bf, xChar);
			  ...
			  const BlockFrame *bf)
{
	...
	docShapePageY(&(io->ioY0Position), &lpBelowShape, &bfShape, ds,
		      paraNode, lpLineTop, bf);
...
static int docPlaceLineInserts(BufferDocument *bd, const BufferItem *paraNode,
			       const TextLine *tl, const ParticuleData *pd,
			       const ParagraphFrame *pf, const BlockFrame *bf,
	...
	for (i = 0; i < tl->tlParticuleCount; tp++, i++) {
		...
		if (tp->tpKind == DOCkindOBJECT) {
			...
			if (io && io->ioKind == DOCokDRAWING_SHAPE) {
				if (docPlaceShape(paraNode,
						  &(tl->tlTopPosition), io, pf,
						  bf, pd->pdX0)) {
					LDEB(1);
				}
			}
...
static int docPlaceLineInsertsY(BufferDocument *bd, const BufferItem *paraNode,
				const TextLine *tl, const BlockFrame *bf,
				const LayoutPosition *lp)
{
	int i;
	const TextParticule *tp;
...
static int docLayout_Line(int *pStopCode, TextLine *resTl,
			  NotesReservation *pNrLine, const BlockFrame *bf,
			  BufferItem *paraNode, int redoLineLayout, int part,
			  ParticuleData *pd, const LayoutJob *lj,
			  const ParagraphFrame *pf, const LayoutPosition *lpTop,
			  LayoutPosition *lpBottom)
{
	...
	if (docLayoutCollectParaFootnoteHeight(
		    &nrLine, tl->tlTopPosition.lpPage,
		    tl->tlTopPosition.lpColumn, bd, lj->ljBodySectNode,
		    paraNode, tl->tlFirstParticule,
		    tl->tlFirstParticule + accepted)) {
		LDEB(1);
		return -1;
	}

	res = docBelowLine(pStopCode, &lpHere, &(tl->tlTopPosition),
			   tl->tlLineStride, paraNode, part, part + accepted,
			   bf, pf, &nrLine);
	if (res < 0) {
		LDEB(res);
		return -1;
	...
	if (redoLineLayout) {
		tl->tlStrlen = tp[accepted - 1].tpStroff +
			       tp[accepted - 1].tpStrlen - tp->tpStroff;
		...
		if (docPlaceLineInserts(bd, paraNode, tl, pd, pf, bf,
					&lpHere)) {
			LDEB(accepted);
		}
	} else {
				     ...
				     const TextLine *tlLine)
{
	...
	docGetPixelRectForPos(&drBox, lc, pf->pfRedrawX0Twips,
			      pf->pfRedrawX1Twips, &lpTop, &lpBottom);
	...
	if (tlLine) {
		...
		docGetPixelRectForPos(&drTl, lc, pf->pfRedrawX0Twips,
				      pf->pfRedrawX1Twips, &lpTop, &lpBottom);

		if (drTl.drX0 != drBox.drX0 || drTl.drX1 != drBox.drX1 ||
		    drTl.drY0 != drBox.drY0 || drTl.drY1 != drBox.drY1) {
...
#if SHOW_LINE_CHANGES
		appDebug("EXPOSE [%4d..%4d x %4d..%4d]\n", drChanged->drX0,
			 drChanged->drX1, drChanged->drY0, drChanged->drY1);
#endif
	}
}
...
int docLayoutParaLines(int *pStopCode, int isRedo, const ParagraphFrame *pf,
		       ...
		       const LayoutJob *lj, BufferItem *paraNode, int part)
{
	...
	while (part < paraNode->biParaParticuleCount) {
		...
		if (line < paraNode->biParaLineCount) {
			const TextParticule *tp =
				paraNode->biParaParticules + part;

			tlLine = paraNode->biParaLines + line;
			placeOldLine = 1;
			boxLine = *tlLine;

			if (tlLine->tlFirstParticule +
					    tlLine->tlParticuleCount >
		...
		accepted = docLayout_Line(&stopCode, &boxLine, &nrLine, bf,
					  paraNode, !placeOldLine, part, pd, lj,
					  pf, &lp, &lp);
		...
		/*  7  */
		if (drChanged) {
			docLayoutAddLineToExpose(drChanged, lj, pf, &boxLine,
						 tlLine);
		}
		...
		*tlLine = boxLine;
		...
		if (boxLine.tlFlags & TLflagBLOCKBREAK) {
			...
			part = boxLine.tlFirstParticule +
			       boxLine.tlParticuleCount - 1;
