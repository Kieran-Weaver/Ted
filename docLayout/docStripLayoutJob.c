...
int docParagraphLayoutPosClaimChildren(ParagraphLayoutPosition *plp, int n)
{
	...
	if (n > 0) {
		p = (ParagraphLayoutJob *)realloc(
			plp->pspChildren, n * sizeof(ParagraphLayoutJob));
		if (!p) {
			LXDEB(n, p);
			return -1;
		}
...
void docBeginParagraphLayoutProgress(ParagraphLayoutJob *plj, int child,
				     int line, int part, int paraUpto,
