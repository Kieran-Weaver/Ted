...
void docShapePageY(LayoutPosition *lpShapeTop, LayoutPosition *lpBelowShape,
		   BlockFrame *bfShape, const DrawingShape *ds,
		   const BufferItem *paraBi, const LayoutPosition *lpLineTop,
		   const BlockFrame *bf)
{
	...
	*lpShapeTop = *lpLineTop;
	...
	docLayoutFrameY(bfShape, yRefProp, ds->dsDrawing.sdYPosition,
			sp->spRect.drY0, lpLineTop, &(paraBi->biTopPosition),
...
void docShapePageRectangle(LayoutPosition *lpShapeTop,
			   ...
			   const DrawingShape *ds, const BufferItem *paraBi,
			   ...
			   int xChar)
{
	...
	docShapePageY(lpShapeTop, lpBelowShape, &bfShape, ds, paraBi, lpLineTop,
		      bfRef);
	...
	docLayoutFrameX(&bfShape, xRefProp, ds->dsDrawing.sdXPosition,
			sp->spRect.drX0,
			    ...
			    INIT_LAYOUT_EXTERNAL initLayoutExternal)
{
	...
	dgBox.dgRightMarginTwips =
		dgBox.dgPageWideTwips - dgBox.dgLeftMarginTwips - wide;
	...
	if (page != ds->dsDocumentTree.dtPageFormattedFor ||
		...
		if (docLayoutDocumentTree(&(ds->dsDocumentTree), drChanged,
					  page, column, dgBox.dgTopMarginTwips,
