				   ...
				   int x0, int baseLine)
{
	...
	drTo.drY0 = baseLine;
	drTo.drX1 =
		drTo.drX0 + (io->ioScaleXUsed / 100.0) * 20 * io->ioTwipsWide;
	drTo.drY1 =
		drTo.drY0 - (io->ioScaleYUsed / 100.0) * 20 * io->ioTwipsHigh;
...
static void psPrintObjectBox(DrawingContext *dc, PrintingState *ps,
			     const InsertedObject *io, int x0, int y0)
{
	int high;
	int wide;
...
static int docPsPrintBitmapObject(PrintingState *ps, int x0, int baseLine,
				 ...
				 const AffineTransform2D *at)
{
	...
	if (!ds->dsRasterImage.riBytes) {
		if (docGetBitmapForObjectData(kind, &(ds->dsRasterImage),
					      &(ds->dsPictureData))) {
	...
	if (ds->dsRasterImage.riBytes) {
		...
		if (bmPsPrintBitmapImage(ps->psSos, 1, xs, -ys, x0, y0, &drSel,
					 onWhite, ps->psUsePostScriptFilters,
...
static int docPsPrintJpegImage(PrintingState *ps, int x0, int baseLine,
			       InsertedObject *io, MemoryBuffer *mb)
{
	int rval = 0;
	...
	if (bmEpsTestJpegEmbeddable(&pixelsWide, &pixelsHigh, &componentCount,
				    &bitsPerComponent, sisBitmap)) {
		rval = 1;
		goto ready;
	}

	sioInClose(sisBitmap);
	sisBitmap = (SimpleInputStream *)0;
	sioInClose(sisMem);
	sisMem = (SimpleInputStream *)0;
	...
	if (bmPsPrintJpegImage(ps->psSos, pip->pipTwipsWide * scaleX,
			       -pip->pipTwipsHigh * scaleY, componentCount, x0,
			       baseLine, pixelsWide, pixelsHigh,
			       bitsPerComponent, sisBitmap)) {
		LDEB(1);
		rval = -1;
		goto ready;
...
static int docPsPrintBitmapImage(PrintingState *ps, DrawingContext *dc,
				 InsertedObject *io, int objectKind,
				 MemoryBuffer *mb, const int x0Twips,
				 int baseLine)
{
	...
	if (io->ioRasterImage.riBytes) {
		if (ps->psUsePostScriptFilters &&
			...
			if (!docPsPrintJpegImage(ps, x0Twips, baseLine, io,
						 mb)) {
...
int docPsPrintObject(const DrawTextLine *dtl, int part, InsertedObject *io,
		     const int x0Twips, const int x1Twips,
		     const LayoutPosition *baseLine)
{
	...
	case DOCokPICTPNGBLIP:
		...
		done = docPsPrintBitmapImage(ps, dc, io, io->ioKind,
					     &(io->ioObjectData), x0Twips,
	...
	case DOCokEPS_FILE:
		if (psPrintIncludeEpsObject(ps, io, x0Twips,
					    baseLine->lpPageYTwips)) {
			LDEB(1);
			break;
		}
