...
static void docLayoutAddExtraToPrevious(ParticuleData *pd, int extra)
{
...
static void docLayoutAddRightBorderToPrevious(ParticuleData *pd,
...
static void docLayoutAddSlantToPrevious(int *pX, ParticuleData *pd,
			 ...
			 int particuleUpto, int x0Word)
{
	...
	*pFattestWordB = 0;
	...
	if (part >= particuleUpto || tp->tpKind != DOCkindSPAN) {
		LLLDEB(part, particuleUpto, tp->tpKind);
		return -1;
	}
	...
	while (part < particuleUpto && tp->tpKind == DOCkindSPAN) {
		...
		if (!afi) {
			LLXDEB(docNumberOfParagraph(paraNode), tp->tpStroff,
			       afi);
		...
		if (spanFlags & TLflagBORDER) {
			...
			docGetBorderPropertiesByNumber(&bp, bd,
						       ta.taBorderNumber);
		...
		while (part < particuleUpto && textAttrNr == textAttrNr0 &&
			...
			if (ta.taBaselineShiftHalfPoints > 0) {
				drPart.drY0 -=
					10 * ta.taBaselineShiftHalfPoints;
			}
			if (ta.taBaselineShiftHalfPoints < 0) {
				drPart.drY1 -=
					10 * ta.taBaselineShiftHalfPoints;
			...
			if (!firstInSpan) {
				...
				pd->pdVisibleBBox.drX1 =
					x0Span + pd->pdVisibleBBox.drX1 - x;
			}

			if (firstInSpan && spanHasLeftBorder) {
				pd->pdLeftBorderWidth = thick;
				pd->pdFlags |=
					(TPflagLEFT_BORDER | TPflagXMATCH_LEFT);
			}
			...
			firstInSpan = 0;
			textAttrNr = docGetEffectiveTextAttributes(
				&taTmp, bd, paraNode, part);
		}
	...
	if (part < paraNode->biParaParticuleCount) {
		...
		/*  7  */
		if (part < particuleUpto && !ta.taFontIsSlanted &&
...
static int docLayoutShowOptionalHyphen(LineLayoutJob *llj, int part,
				       unsigned char *pLineFlags,
				       ParticuleData *pd, int x0)
{
	BufferDocument *bd = llj->lljLayoutContext->lcDocument;
	...
	if (docLayoutStringExtents(&width, &decWidth, &drPart, &ta, sizeTwips,
				   bd, afi, from, len)) {
		LDEB(len);
		return -1;
	}
...
static int docLayoutInsertOptionalHyphen(LineLayoutJob *llj, int part, int done,
					 int x0, ParticuleData *pd)
{
	BufferDocument *bd = llj->lljLayoutContext->lcDocument;
	...
	if (i == 0) {
		...
		for (l = 0; l < paraNode->biParaParticules[part].tpStrlen - 1;
		     l++) {
			int width;
			int decWidth;
			rect drPart;
			...
			if (docLayoutStringExtents(
				    ...
				    bd, afi,
				    (char *)docParaString(paraNode, from),
				    l + 1)) {
				LDEB(l + 1);
				return -1;
			}
	...
	if (i > 0) {
		...
		int attrNr =
			paraNode->biParaParticules[part + i - 1].tpTextAttrNr;

		docParaStringReplace(&stroffShift, paraNode, stroff, stroff,
				     (const char *)"-", 1);

		tpHyphen = docInsertTextParticule(paraNode, part + i, stroff, 1,
						  DOCkindOPT_HYPH, attrNr);
		if (!tpHyphen) {
			XDEB(tpHyphen);
			return -1;
		}

		docShiftParticuleOffsets(bd, paraNode, part + i + 1,
...
textCase:
					     ...
					     &addRightBorder, &rightBorderThick,
					     &fattestWordBorder, &visibleX1,
					     &lineFlags, pd, particuleUpto, x0);
			if (done < 1) {
				LDEB(done);
				return -1;
			}
			...
			if (visibleX1 >= drParaContent->drX1 &&
				...
				inserted = docLayoutInsertOptionalHyphen(
					llj, part, done, x0, pd);
				if (inserted < 0) {
					LDEB(inserted);
					return -1;
			...
			if (visibleX1 >= drParaContent->drX1 &&
				...
				if (accepted > 0) {
					/* WRONG! fix later */
					pd[-1].pdTwipsWide =
						pd[-1].pdVisibleBBox.drX1;
					docLayoutAddRightBorderToPrevious(
						pd, rightBorderThick);
				}
		...
		case DOCkindOBJECT: {
			...
			io = docGetObject(bd, paraNode->biParaParticules[part]
						      .tpObjectNumber);
			if (!io) {
				LPDEB(paraNode->biParaParticules[part]
					      .tpObjectNumber,
				      io);
			}
			...
			if (io && io->ioInline) {
				...
				done = docLayoutInlineObject(&ta, bd, paraNode,
							     part, io, &drWord,
							     &fattestWordBorder,
							     &x1, x0);
				...
				drWord.drY0 -=
					(150 *
					 paraNode->biParaMajorityFontSize) /
					100;
		...
		case DOCkindCHFTNSEPC: {
			...
			frameWide = pf->pfParaContentRect.drX1 -
				    pf->pfParaContentRect.drX0;
			if (width > frameWide) {
				width = frameWide;
			}
			...
			if (docLayoutFontAscDesc(
				    paraNode, &ta, &drWord, &fattestWordBorder,
		...
		case DOCkindOPT_HYPH: {
			...
			afi = docLayoutGetAfi(&textAttrNr, &ta,
					      &ignoredLineFlags,
					      llj->lljLayoutContext, paraNode,
					      part);
			if (!afi) {
				XDEB(afi);
				return -1;
		...
		case DOCkindCOLUMNBREAK:
			...
			if (paraNode->biParaParticules[part].tpKind !=
				    DOCkindLINEBREAK &&
				   ...
				   const int tabX1)
{
	...
	if (done > 0) {
		...
		visibleSinceTab = pdd[done - 1].pdX0 + visibleX1 - tabX0;
	}
...
static int docLayoutBulletAsTab(int *pParticuleUpto, int *pX0, int *pTabX0,
				int *pTabX1, int *pTabParticule, int *pTabKind,
				...
				const BufferDocument *bd)
{
	...
	if (docGetListLevelOfParagraph(startPath, formatPath, &lo, &dl, &ll, pp,
				       bd)) {
		...
		if (ll->llFollow != DOCllfTAB && tabKind != DOCtaLEFT) {
			...
			if (docDelimitParaHeadField(&dfHead, &dsInsideHead,
						    &dsAroundHead, &partBegin,
						    &partEnd, paraNode, bd)) {
				LDEB(1);
			} else {
				particuleUpto = partEnd + 1;
			}
		}
	}
			       ...
			       ParticuleData *pdFrom, int acceptAtLeast, int x0)
{
	...
	*pFattestLineB = 0;
	...
	if (part == 0 && paraNode->biParaListOverride > 0) {
		if (docLayoutBulletAsTab(&particuleUpto, &x0, &tabX0, &tabX1,
					 &tabParticule, &tabKind, pf, paraNode,
					 bd)) {
			LDEB(1);
		}
	}
	...
	while (accepted < paraNode->biParaParticuleCount) {
		...
		done = docPsLayoutText(llj, part, &found, &textWordCount,
				       &drText, &fattestTextBorder, &lineFlags,
				       pd, particuleUpto, acceptAtLeast, x0);
		if (done < 0) {
			LDEB(done);
			return -1;
		}
		...
		if (docLayoutAlignTextToTab(
			    pd, paraNode->biParaParticules + part, part, done,
			    pdTab, tpTab, tabKind, &drText, tabX0, tabX1)) {
			LDEB(done);
		}
		...
		tab = -1;
		if (docNextTabStop(
			    &ts, &nextTabX1, &tab, &(llj->lljTabStopList),
...
static void docJustifyLine(const BufferItem *paraNode,
			   const TextParticule *tpFrom, ParticuleData *pdFrom,
			   int accepted, int x1TextLines)
{
	int i;
	int extra;
	int totalWeight;
	...
	if (extra < 0) {
		...
		for (i = 0; i < accepted; tp++, pd++, i++) {
			appDebug("%-4s: %5d..%5d \"%*.*s\"\n",
				 docKindStr(tp->tpKind), pd->pdX0,
				 pd->pdX0 + pd->pdVisibleBBox.drX1,
	...
	for (i = 0; i < accepted - 1; tp++, pd++, i++) {
		...
		if (docParaPastLastNonBlank(paraNode, tp->tpStroff,
			...
			pd->pdWhiteUnits = sqrt((double)tp[0].tpTwipsWide +
						(double)visibleWidth);
...
static int docLayoutGetLineX0(const BufferItem *paraNode, int part,
			      const ParagraphFrame *pf)
{
	int x0;
...
int docLayoutLineBox(TextLine *tl, BufferItem *paraNode, int part,
		     ...
		     const ParagraphFrame *pf)
{
	...
	/*  1  */
	accepted = docLayoutParticules(&llj, part, &found, &wordCount, &drLine,
				       &fattestBorder, &lineFlags, pd, 1, x0);
	if (accepted <= 0) {
		LDEB(accepted);
		return -1;
	}
	...
	case DOCthaJUSTIFIED:
		...
		if (found == PSfoundLINEFULL) {
			docJustifyLine(paraNode,
				       paraNode->biParaParticules + part, pd,
				       accepted, pf->pfParaContentRect.drX1);
		}
		break;
	default:
	...
	for (p = 0; p < accepted; p++) {
		...
		paraNode->biParaParticules[part + p].tpTwipsWide =
			pd[p].pdTwipsWide;
	...
	/*  9 */
	if (found == PSfoundBLOCKBREAK && paraNode->biTreeType == DOCinBODY &&
	    paraNode->biParaTableNesting == 0) {
