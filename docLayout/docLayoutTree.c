			       ...
			       const LayoutContext *lc)
{
	...
	dfNote = docGetFirstNoteInColumn(&dnFirstNote, lc->lcDocument, page,
					 column, noteItKind);
	...
	if (docNoteSeparatorRectangle(dr, &eiNoteSep, &y0Twips, dnFirstNote,
				      noteSepKind, lc)) {
		LDEB(1);
		return -1;
	}
			...
			int column, const LayoutContext *lc)
{
...

	{
		...
		docBlockFrameTwips(&bf, tree->dtRoot, lc->lcDocument, page,
				   column);
		...
		docGetPixelRectForPos(&drBox, lc, bf.bfContentRect.drX0,
				      bf.bfContentRect.drX1, &lpTop, &lpBot);
	}
	...
	case DOCinFTNCN:
		...
		if (docNoteSeparatorBox(&drBox, DOCinFOOTNOTE,
					treeRoot->biTreeType, page, column,
					lc)) {
			LLDEB(page, column);
			rval = -1;
			goto ready;
	...
	case DOCinAFTNCN:
		...
		if (docNoteSeparatorBox(&drBox, DOCinENDNOTE,
					treeRoot->biTreeType, page, column,
					lc)) {
			LLDEB(page, column);
			rval = -1;
...
static void docPlaceHeader(DocumentTree *tree, const DocumentGeometry *dgSect)
{
...
static void docPlaceFooter(DocumentTree *tree, const DocumentGeometry *dgSect)
{
...
int docTreePrelayout(DocumentTree *tree, const BufferItem *bodySectNode,
		     LayoutJob *lj)
{
	int rval = 0;
	...
	docBlockFrameTwips(&bf, tree->dtRoot, lc->lcDocument, treeLp.lpPage,
			   treeLp.lpColumn);
	...
	if (docLayoutNodeImplementation(&treeLp, &treeLp, tree->dtRoot, &bf,
					&treeLj)) {
		LDEB(1);
		rval = -1;
		goto ready;
...
int docSectHeaderFooterPrelayout(BufferItem *bodySectNode, LayoutJob *lj)
{
	int hdft;
	int redraw = 0;
	...
	for (hdft = 0; hdft < DOC_HeaderFooterTypeCount; hdft++) {
		...
		dtHdFt = docSectionHeaderFooter(
			bodySectNode, &applies,
...
static int docGetY0ForSelectedNoteSeparator(int *pY0Twips,
					    BufferItem **pBodySectNode,
					    const LayoutContext *lc,
					    const DocumentTree *selTree,
					    int noteItKind, int sepItKind)
{
	struct DocumentNote *dnFirstNote;
	DocumentTree *eiBody;
	DocumentTree *eiNoteSep;
	...
	if (docNoteSeparatorRectangle(&drExtern, &eiNoteSep, &y0Twips,
				      dnFirstNote, sepItKind, lc)) {
		LDEB(1);
		return -1;
	}
			       ...
			       INIT_LAYOUT_EXTERNAL initLayoutExternal)
{
	...
	case DOCinRIGHT_FOOTER:
		...
		selRootBodySectNode =
			docGetBodySectNodeOfScope(selRootScope, lc->lcDocument);
	...
	case DOCinFTNSEP:
		...
		if (selTree->dtPageSelectedUpon < 0) {
			LDEB(selTree->dtPageSelectedUpon);
			return -1;
		}

		if (docGetY0ForSelectedNoteSeparator(
			    &y0Twips, &selRootBodySectNode, lc, selTree,
	...
	if (selTree->dtPageSelectedUpon < 0) {
		LSDEB(selTree->dtPageSelectedUpon,
		      docTreeTypeStr(selTree->dtRoot->biTreeType));
		return -1;
	}
	...
	if (docLayoutDocumentTree(
		    selTree, drChanged, selTree->dtPageSelectedUpon,
		    selTree->dtColumnSelectedIn, y0Twips, selRootBodySectNode,
		    lc, initLayoutExternal, adjustDocument)) {
		LDEB(selTree->dtPageSelectedUpon);
		return -1;
	}
			  ...
			  int adjustDocument)
{
	...
	if (docRecalculateTextLevelFieldsInDocumentTree(&rf, tree, bodySectNode,
							page)) {
		LDEB(page);
		return -1;
	}
    ...
    */
	...
	if (docLayoutNodeImplementation(&lpHere, &lpHere, tree->dtRoot, &bf,
					&lj)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (adjustDocument && tree->dtY0UsedTwips != oldY1) {
		const DocumentGeometry *dgSect =
			&(bodySectNode->biSectDocumentGeometry);
