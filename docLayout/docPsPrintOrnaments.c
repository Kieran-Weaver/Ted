...
static void psSolidBorderProc(SimpleOutputStream *sos, const char *name)
{
...
static void psDashHorBorderProc(SimpleOutputStream *sos, const char *name,
...
static void psDashVerBorderProc(SimpleOutputStream *sos, const char *name,
				const unsigned char *dashes, int dashCount)
{
	int i;
	int dashLength = 0;
...
void psDefineBorderProcs(SimpleOutputStream *sos)
{
	...
	psDashHorBorderProc(sos, "h-brdrdash", PSborderDASH,
			    sizeof(PSborderDASH));
	psDashHorBorderProc(sos, "h-brdrdot", PSborderDOT, sizeof(PSborderDOT));
	psDashHorBorderProc(sos, "h-brdrdashd", PSborderDASHD,
			    sizeof(PSborderDASHD));
	psDashHorBorderProc(sos, "h-brdrdashdd", PSborderDASHDD,
			    sizeof(PSborderDASHDD));

	psSolidBorderProc(sos, "v-brdrs");

	psDashVerBorderProc(sos, "v-brdrdash", PSborderDASH,
			    sizeof(PSborderDASH));
	psDashVerBorderProc(sos, "v-brdrdot", PSborderDOT, sizeof(PSborderDOT));
	psDashVerBorderProc(sos, "v-brdrdashd", PSborderDASHD,
			    sizeof(PSborderDASHD));
	psDashVerBorderProc(sos, "v-brdrdashdd", PSborderDASHDD,
			    sizeof(PSborderDASHDD));
}
				    ...
				    struct DrawingContext *dc)
{
	...
	case DOCbsDASH:
		sioOutPrintf(ps->psSos, "%d %d %d %d v-brdrdash ", dr->drX0,
			     dr->drY0, wide, high);
		break;

	case DOCbsDOT:
		sioOutPrintf(ps->psSos, "%d %d %d %d v-brdrdot ", dr->drX0,
			     dr->drY0, wide, high);
		break;

	case DOCbsDASHD:
		sioOutPrintf(ps->psSos, "%d %d %d %d v-brdrdashd ", dr->drX0,
			     dr->drY0, wide, high);
		break;

	case DOCbsDASHDD:
		sioOutPrintf(ps->psSos, "%d %d %d %d v-brdrdashdd ", dr->drX0,
			     dr->drY0, wide, high);
		break;

	default:
		sioOutPrintf(ps->psSos, "%d %d %d %d v-brdrs ", dr->drX0,
			     dr->drY0, wide, high);
		break;
	}
				      ...
				      struct DrawingContext *dc)
{
	...
	case DOCbsDASH:
		sioOutPrintf(ps->psSos, "%d %d %d %d h-brdrdash ", dr->drX0,
			     dr->drY0, wide, high);
		break;

	case DOCbsDOT:
		sioOutPrintf(ps->psSos, "%d %d %d %d h-brdrdot ", dr->drX0,
			     dr->drY0, wide, high);
		break;

	case DOCbsDASHD:
		sioOutPrintf(ps->psSos, "%d %d %d %d h-brdrdashd ", dr->drX0,
			     dr->drY0, wide, high);
		break;

	case DOCbsDASHDD:
		sioOutPrintf(ps->psSos, "%d %d %d %d h-brdrdashdd ", dr->drX0,
			     dr->drY0, wide, high);
		break;

	default:
		sioOutPrintf(ps->psSos, "%d %d %d %d h-brdrs ", dr->drX0,
			     dr->drY0, wide, high);
		break;
	}
			...
			struct DrawingContext *dc)
{
	...
	if (PROPmaskISSET(&(bo->boPropMask), ORNdrawSHADE)) {
		...
		if (isFilled) {
			...
			psFillRectangle(ps, drInside->drX0, drInside->drY0,
		...
		if (bo->boShading.isPattern != DOCspSOLID) {
			...
			if (bo->boShading.isForeColor > 0 &&
			    bo->boShading.isForeColor <
				    dp->dpColorPalette->cpColorCount) {
				cf = dp->dpColorPalette->cpColors
					     [bo->boShading.isForeColor];
			}

			docDrawSetColorRgb(dc, (void *)ps, &cf);

			psRectanglePath(ps, drInside->drX0, drInside->drY0,
					drInside->drX1 - drInside->drX0 + 1,
					drInside->drY1 - drInside->drY0 + 1);
	...
	if (PROPmaskISSET(&(bo->boPropMask), ORNdrawTOP_BORDER)) {
		...
		docPsPrintHorizontalBorder(&(bo->boTopBorder), &drBorder, ps,
					   dc);
		done = 1;
	}
	...
	if (PROPmaskISSET(&(bo->boPropMask), ORNdrawLEFT_BORDER)) {
		...
		docPsPrintVerticalBorder(&(bo->boLeftBorder), &drBorder, ps,
					 dc);
		done = 1;
	}
	...
	if (PROPmaskISSET(&(bo->boPropMask), ORNdrawRIGHT_BORDER)) {
		...
		docPsPrintVerticalBorder(&(bo->boRightBorder), &drBorder, ps,
					 dc);
		done = 1;
	}
	...
	if (PROPmaskISSET(&(bo->boPropMask), ORNdrawBOTTOM_BORDER)) {
		...
		docPsPrintHorizontalBorder(&(bo->boBottomBorder), &drBorder, ps,
					   dc);
		done = 1;
	}
