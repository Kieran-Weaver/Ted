...
static int docPsPrintArrowHead(PrintingState *ps, int lineWidth,
			       const DrawShapeArrow *dsa)
{
	...
	case DSarrowDIAMOND:
		sioOutPrintf(sos, "%g %g bp ", dsa->dsaPath[0].x,
			     dsa->dsaPath[0].y);
...
static void docPsPrintGetLine(int *pLine, int *pLineWidth,
			      const DrawingShape *ds, DrawingContext *dc,
			      PrintingState *ps, const AffineTransform2D *at)
{
			       ...
			       PrintingState *ps)
{
	...
	if (line && !sp->spClosed && nv >= 2) {
		...
		if (sd->sdLineTailArrow.saArrowHead != DSarrowNONE) {
			...
			shaftTail[0].x = AT2_X(sv[nv - 2].x - xm,
					       sv[nv - 2].y - ym, &at);
			shaftTail[0].y = AT2_Y(sv[nv - 2].x - xm,
					       sv[nv - 2].y - ym, &at);
			shaftTail[1].x = AT2_X(sv[nv - 1].x - xm,
					       sv[nv - 1].y - ym, &at);
			shaftTail[1].y = AT2_Y(sv[nv - 1].x - xm,
					       sv[nv - 1].y - ym, &at);
	...
	if (drawHeadArrow) {
		...
		docPsEmitFirstVertex(sos, &at, &x0, &y0, sp->spVertices, xm,
				     ym);
	}

	docPsEmitVertices(sos, &at, &x0, &y0, sp->spVertices, upto, x0, y0, xm,
			  ym);

	if (drawTailArrow) {
		sioOutPrintf(sos, "%g %g rl ", dsaTail.dsaShaft[1].x - x0,
			       ...
			       DrawingShape *ds, DrawingContext *dc, void *vps)
{
	...
	case SHPtyPICTURE_FRAME: {
		...
		docShapeStartShapeTransform(&at, ds, drTwips, sp->spXSize,
					    sp->spYSize);
	...
	case SHPtyFLOW_CHART_EXTRACT:
		if (docPsPrintShapePath(ds, &SP_ISOSCELES_TRIANGLE, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyFLOW_CHART_MERGE:
		if (docPsPrintShapePath(ds, &SP_FLOW_CHART_MERGE, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyRIGHT_TRIANGLE:
		if (docPsPrintShapePath(ds, &SP_RIGHT_TRIANGLE, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyFLOW_CHART_INPUT_OUTPUT:
		if (docPsPrintShapePath(ds, &SP_PARALLELOGRAM, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyNOTCHED_RIGHT_ARROW:
		if (docPsPrintShapePath(ds, &SP_NOTCHED_RIGHT_ARROW, drTwips,
					dc, ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyRIGHT_ARROW_CALLOUT:
		if (docPsPrintShapePath(ds, &SP_RIGHT_ARROW_CALLOUT, drTwips,
					dc, ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyLEFT_ARROW_CALLOUT:
		if (docPsPrintShapePath(ds, &SP_LEFT_ARROW_CALLOUT, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyUP_ARROW_CALLOUT:
		if (docPsPrintShapePath(ds, &SP_UP_ARROW_CALLOUT, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyDOWN_ARROW_CALLOUT:
		if (docPsPrintShapePath(ds, &SP_DOWN_ARROW_CALLOUT, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyUP_DOWN_ARROW_CALLOUT:
		if (docPsPrintShapePath(ds, &SP_UP_DOWN_ARROW_CALLOUT, drTwips,
					dc, ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyQUAD_ARROW_CALLOUT:
		if (docPsPrintShapePath(ds, &SP_QUAD_ARROW_CALLOUT, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyLEFT_RIGHT_ARROW:
		if (docPsPrintShapePath(ds, &SP_LEFT_RIGHT_ARROW, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyUP_DOWN_ARROW:
		if (docPsPrintShapePath(ds, &SP_UP_DOWN_ARROW, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyLEFT_RIGHT_UP_ARROW:
		if (docPsPrintShapePath(ds, &SP_LEFT_RIGHT_UP_ARROW, drTwips,
					dc, ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyLEFT_UP_ARROW:
		if (docPsPrintShapePath(ds, &SP_LEFT_UP_ARROW, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyBENT_UP_ARROW:
		if (docPsPrintShapePath(ds, &SP_BENT_UP_ARROW, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyWEDGE_RECT_CALLOUT:
		if (docPsPrintShapePath(ds, &SP_WEDGE_RECT_CALLOUT, drTwips, dc,
					ps)) {
			LDEB(1);
			rval = -1;
		}
	...
	case SHPtyRIGHT_BRACE: {
		...
		sioOutPrintf(sos,
			     "newpath 0 -0.75 moveto 0 0.5 rlineto stroke\n");
		...
		sioOutPrintf(sos,
			     "newpath 0  0.25 moveto 0 0.5 rlineto stroke\n");
		sioOutPrintf(sos, "gsave 0  0.75 translate 1 0.25 scale");
	...
	case SHPtyFREEFORM_OR_NON_AUTOSHAPE:
		if (sd->sdVertexCount > 1) {
			...
			docShapeStartShapeTransform(
				&at, ds, drTwips, drTwips->drX1 - drTwips->drX0,
			...
			docDrawShapeGetFill(&fill, &rgb8Fill, ds, dc,
					    (void *)ps);
			if (fill) {
				sioOutPrintf(sos,
					     "gsave closepath fill grestore\n");
			}
	...
	default:
		LLSDEB(ds->dsShapeNumber, sd->sdShapeType,
		       docShapeTypeString(sd->sdShapeType));
		break;
	}
