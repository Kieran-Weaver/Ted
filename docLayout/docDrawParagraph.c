...
static int docDrawParaOrnaments(void *through, const ParagraphDrawingStrip *pds,
				...
				const BlockOrigin *bo)
{
	...
	if (paraNode->biParaTableNesting > 0) {
		...
		if (!rowBi) {
			...
			int x0 = pf->pfParaContentRect.drX0 -
				 rowBi->biRowHalfGapWidthTwips;
			int x1 = pf->pfParaContentRect.drX1 +
				 rowBi->biRowHalfGapWidthTwips;
	...
	docGetParaOrnaments(&ornaments, &drOutside, &drInside, &drPara, bd,
			    paraNode, pds->pdsAtParaTop, pds->pdsAtParaBottom);

	if ((*dc->dcDrawOrnaments)(&ornaments, lpTop.lpPage, &drOutside,
				   &drInside, through, dc)) {
		LDEB(1);
		return -1;
	}
...
static int docDelimitParagraphDrawingStrip(
	ParagraphDrawingStrip *pds, const BufferItem *paraNode, int countAfter,
	const LayoutPosition *lpShadeTop, int lineFrom,
	const LayoutPosition *lpThisFrame, const BlockOrigin *bo)
{
	const TextLine *tl;
	LayoutPosition lp;
	...
	while (line < paraNode->biParaLineCount) {
		...
		afterPageBreak =
			paraNode->biParaParticules[tl->tlFirstParticule +
						   tl->tlParticuleCount - 1]
				.tpKind == DOCkindPAGEBREAK;
	...
	if (line == paraNode->biParaLineCount) {
		if (!afterPageBreak) {
			...
			if (!countAfter) {
				lp.lpPageYTwips -=
					paraNode->biParaSpaceAfterTwips;
			}
...
int docDrawParagraphStrip(void *through, ParagraphDrawingStrip *pds,
			  BufferItem *paraNode, int countAfter,
			  const LayoutPosition *lpShadeTop, int lineFrom,
			  const ParagraphFrame *pf, DrawingContext *dc,
			  ...
			  const BlockOrigin *bo)
{
	if (docDelimitParagraphDrawingStrip(pds, paraNode, countAfter,
					    lpShadeTop, lineFrom, lpThisFrame,
					    bo)) {
		LDEB(1);
		return -1;
	}
...
int docDrawParaNode(LayoutPosition *lpBelow, BufferItem *paraNode,
		    void *through, DrawingContext *dc, const BlockOrigin *bo)
{
	...
	docParaBlockFrameTwips(&bf, paraNode, bd,
	...
	while (line < paraNode->biParaLineCount) {
		...
		if (DOC_COLUMN_AFTER(&(paraNode->biBelowPosition), &lpTop)) {
			...
			ret = docDrawToNextColumn(bodyNode, bodyNode, through,
						  &lpTop, &bf, dc);
			if (ret < 0) {
	...
	if (lpBelow && line >= paraNode->biParaLineCount) {
		docLayoutPushBottomDownShifted(
			lpBelow, &(paraNode->biBelowPosition), bo);
	}
