...
static void appSymbolPickerGetDimensions(int *pPixelsWide, int *pPixelsHigh,
					 SymbolPicker *sp, int cellsWide,
					 int cellsHigh)
{
...
static void appSymbolPickerCalculateGridSize(SymbolPicker *sp, int pixelsWide,
					     int pixelsHigh)
{
...
static int appSymbolGetSymbol(SymbolPicker *sp, int row, int col)
{
	const IndexSet *is;
...
static int appSymbolGetRowCol(int *pRow, int *pCol, SymbolPicker *sp,
			      int symbol)
{
	const IndexSet *is;
	int step;
...
static int appSymbolSetBlockOffset(SymbolPicker *sp, int sym)
{
...
static int appSymbolAdaptDrawingToFont(SymbolPicker *sp)
{
	...
	sp->spFontInfo =
		appGetFontInfoForAttribute(&unicodesWanted, ta, dfl, psfl);
	if (!sp->spFontInfo) {
		XDEB(sp->spFontInfo);
		return -1;
	}
	...
	screenFont = drawOpenScreenFont(ds, sp->spFontInfo, pixelSize,
					unicodesWanted);
	if (screenFont < 0) {
		LDEB(screenFont);
	} else {
...
static void appSymbolAdaptBlockMenuToFont(SymbolPicker *sp)
{
	...
	while (block < sp->spUnicodeBlockOptCount) {
		...
		while (block < sp->spUnicodeBlockOptCount &&
			...
			appGuiSetWidgetVisible(sp->spUnicodeBlockOpts[block],
					       0);
			block++;
		}
	...
	if (fontBlock >= 0) {
		if (sp->spSymbolOffset < UCD_Blocks[fontBlock].ubFirst ||
			...
			appSymbolSetBlockOffset(sp,
						UCD_Blocks[fontBlock].ubFirst);
		} else {
...
static void appSymbolPickerInsert(SymbolPicker *sp)
{
	...
	if (sp->spSymbolSelected < 0 || !sp->spInsert) {
		LXDEB(sp->spSymbolSelected, sp->spInsert);
		return;
	}
...
static void appSymbolDrawSymbol(SymbolPicker *sp, int sym, int x0, int y0)
{
	...
	y = (drCell.drY1 + drCell.drY0) / 2 -
	    (drText.drY1 + drText.drY0 + 1) / 2;
	x = (drCell.drX1 + drCell.drX0) / 2 -
				...
				const DocumentRectangle *drClip)
{
	...
	if (sp->spEnabled) {
		...
		drawSetForegroundColor(ds, &disabledColor);
	}
	...
	if (geoIntersectRectangle(&drIgn, &drSym, drClip)) {
		...
		bytes = utilMemoryBufferGetBytes(&size,
						 &(sp->spCollectedString));
		if (bytes && size > 0) {
			int x, y;
			...
			drawString(ds, x, y, sp->spScreenFont, (char *)bytes,
				   size);
		}
	}
	...
	if (sym >= 0) {
		...
		if (seq == 1 || !sameAsPrevious) {
			...
			if (step < 1) {
				...
				sp->spPreviousSize =
					sp->spCollectedString.mbSize;

				if (utilMemoryBufferAppendBytes(
					    &(sp->spCollectedString),
					    (unsigned char *)scratch, step)) {
					LDEB(step);
				}

				guiEnableWidget(sp->spClearButton,
						sp->spEnabled);
				guiEnableWidget(sp->spInsertButton,
						sp->spEnabled);
			}
		}
	}
...
/*  1  */
static void appSymbolFontFamilyChosen(int sortIndex, void *voidsp)
{
	SymbolPicker *sp = (SymbolPicker *)voidsp;
	DocumentFontList *dfl = &(sp->spDocumentFontList);
	TextAttribute *ta = &(sp->spTextAttribute);
	...
	if (sortIndex < 0 || sortIndex >= dfl->dflFontCount) {
		LLDEB(sortIndex, dfl->dflFontCount);
		return;
	}
...
/*  1  */
static void appSymbolUnicodeBlockChosen(int block, void *voidsp)
{
	SymbolPicker *sp = (SymbolPicker *)voidsp;

	if (block < 0 || block >= sp->spUnicodeBlockOptCount) {
		LLDEB(block, sp->spUnicodeBlockOptCount);
		return;
	}
				  ...
				  SymbolPicker *sp)
{
	...
	appMakeOptionmenuInColumn(&(sp->spFontOptionmenu), sp->spFontPaned,
				  appSymbolFontFamilyChosen, sp);
	appMakeOptionmenuInColumn(&(sp->spBlockOptionmenu), sp->spFontPaned,
...
static void appSymbolMakeSymbolPart(APP_WIDGET parent, SymbolPicker *sp)
{
	int wide;
	int high;
	...
	appSymbolPickerGetDimensions(&wide, &high, sp, MIN_CELLS_WIDE,
				     MIN_CELLS_HIGH);

	appGuiMakeDrawingAreaInColumn(&(sp->spSymbolDrawing), parent, wide,
				      high, heightResizable, appSymbolRedraw,
				      (void *)sp);

	guiDrawSetButtonPressHandler(sp->spSymbolDrawing, appSymbolMousePress,
				     (void *)sp);
...
static int appSymbolFillBlockMenu(SymbolPicker *sp)
{
	...
	for (block = 0; block < UCD_BlockCount; block++) {
		...
		sp->spUnicodeBlockOpts[block] =
			appAddItemToOptionmenu(&(sp->spBlockOptionmenu),
...
/*  1  */
static void appSymbolFillFontMenu(int *pEnabled,
				  const SymbolPickerResources *spr,
				  SymbolPicker *sp)
{
	int fam;
...
int appAdaptSymbolPickerToFontFamily(void *voidsp, unsigned int documentId,
				     ...
				     const TextAttribute *taSet)
{
	...
	utilUpdateTextAttribute(&doneMask, &(sp->spTextAttribute), taSetMask,
				taSet);
	...
	if (sp->spTextAttribute.taFontNumber >= 0) {
		...
		df = docFontListGetFontByNumber(
			dfl, sp->spTextAttribute.taFontNumber);
		if (df) {
	...
	if (sp->spFontFamilyChosen != familyIndex) {
		...
		sortIndex = utilDocumentFontListGetSortIndex(
			dfl, sp->spFontFamilyChosen);
...
static AppConfigurableResource APP_SymbolSubjectResourceTable[] = {
	...
	APP_RESOURCE("symbolPickerClear",
		     offsetof(InspectorSubjectResources, isrRevert), "Clear"),
...
static AppConfigurableResource APP_SymbolPickerResourceTable[] = {
	APP_RESOURCE("symbolPickerFont",
		     offsetof(SymbolPickerResources, sprFont), "Font"),
	APP_RESOURCE("symbolPickerBlock",
		     offsetof(SymbolPickerResources, sprBlock), "Block"),
	APP_RESOURCE("symbolPickerNone",
		     offsetof(SymbolPickerResources, sprNone), "None"),
};
