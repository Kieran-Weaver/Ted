...
void appGuiMakeListInColumn(Widget *pList, Widget column, int visibleItems,
			    ...
			    APP_BUTTON_CALLBACK_T actionCallback, void *through)
{
	...
	ac++;
	XtSetArg(al[ac], XmNallowResize, False);
	ac++;
	if (visibleItems > 0) {
		XtSetArg(al[ac], XmNvisibleItemCount, visibleItems);
		ac++;
	}

	XtSetArg(al[ac], XmNwidth, 25);
	ac++;
	...
	if (listCallback) {
		XtAddCallback(list, XmNbrowseSelectionCallback, listCallback,
			      through);
	}

	if (actionCallback) {
		XtAddCallback(list, XmNdefaultActionCallback, actionCallback,
			      through);
	}
...
void appGuiReplaceValueInListWidget(APP_WIDGET list, int pos, const char *value)
{
	int *selected = (int *)0;
	int selectedCount = 0;
	int selectNew = 0;
	...
	if (selectNew) {
		...
		XmListAddItemUnselected(list, labelString, pos + 1);
	}
...
void appGuiAddValueToListWidget(Widget list, int pos, const char *value)
{
	XmString labelString;
...
static void appGuiScrollListToPosMotif(APP_WIDGET list, int pp1)
{
	int firstVisible;
	int nVisible;

	/* If the selected item is not visible, scroll the list */
	XtVaGetValues(list, XmNtopItemPosition, &firstVisible,
...
int appGuiGetPositionFromListCallback(APP_WIDGET list, void *voidlcs)
{
...
char *appGuiGetStringFromListCallback(APP_WIDGET list, void *voidlcs)
{
	XmListCallbackStruct *lcs = (XmListCallbackStruct *)voidlcs;
	char *text = (char *)0;
