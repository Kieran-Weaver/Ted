...
static int drawPrepareRoundRect(Arc2DI arcs[4], Point2DI points[13],
				const DocumentRectangle *dr, int w, int h)
{
...
int drawFillRoundedRect(DrawingSurface ds, const DocumentRectangle *dr, int w,
			int h)
{
	int a;
	Arc2DI arcs[4];
...
int drawRoundedRect(DrawingSurface ds, const DocumentRectangle *dr, int w,
		    int h)
{
	int a;
	Arc2DI arcs[4];
	...
	for (a = 0; a < 4; a++) {
		...
		drawLine(ds, points[o].x, points[o].y, points[o + 1].x,
			 points[o + 1].y);
	}
