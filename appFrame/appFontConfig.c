...
static void appFcScaleInt(int *pV, int upm)
{
	*pV = (1000 * *pV) / upm;
}
...
static int appFcGetEncodingsFromUnicode(AfmFontInfo *afi, const FT_Face ftFace)
{
	FT_ULong charcode;
	FT_UInt glyphIdx;
	...
	while (glyphIdx != 0) {
		if (glyphIdx < afi->afiMetricCount) {
			if (charcode == 'x') {
				afi->afiXHeight =
					afi->afiMetrics[glyphIdx]->acmBBox.drY1;
			}
			if (charcode == 'H') {
				afi->afiCapHeight =
					afi->afiMetrics[glyphIdx]->acmBBox.drY1;
			...
			/*  Only support UTF16 */
			if (charcode <= 0xffff) {
				if (psFontInfoSetGlyphUnicode(afi, glyphIdx,
							      charcode)) {
					SLXDEB(afi->afiFontName, glyphIdx,
					       charcode);
					return -1;
				}
...
static int appFcGetEncodingsFromAdobeDingbats(AfmFontInfo *afi,
	...
	while (glyphIdx != 0) {
		if (charcode < 256) {
			...
			if (unicode >= 0 &&
				...
				LLLDEB(glyphIdx, charcode, unicode);
				return -1;
			}
		} else {
			LDEB(charcode);
...
static int appFcGetDefaultEncoding(AfmFontInfo *afi, const FT_Face ftFace)
{
	int ch;
...
int appFcGetFontMetrics(AfmFontInfo *afi)
{
	...
	/*  1  */
	fterror =
		FT_New_Face(APP_FT_Library,
			    utilMemoryBufferGetString(&(afi->afiFontFileName)),
			    afi->afiFontFileIndex, &ftFace);
	if (fterror) {
	...
	for (glyphIdx = 0; glyphIdx < ftFace->num_glyphs; glyphIdx++) {
		...
		/* TTF default char */
		if (afi->afiFallbackGlyph < 0 && N && !strcmp(N, ".notdef")) {
...
static int appFcListOneFont(PostScriptFontList *psfl, FcPattern *font)
{
	int rval = 0;
	...
	/************************/
	{
		...
		fontName = (char *)malloc(strlen((char *)family) + 1 +
					  strlen((char *)style) + 1);
		fullName = (char *)malloc(strlen((char *)family) + 1 +
					  strlen((char *)style) + 1);

		if (strlen((char *)style) > 0) {
			sprintf(fontName, "%s-%s", (char *)family,
				(char *)style);
			sprintf(fullName, "%s %s", (char *)family,
				(char *)style);
		} else {
	...
	afi->afiFontName = fontName;
	fontName = (char *)0; /* steal */
	afi->afiFullName = fullName;
	fullName = (char *)0; /* steal */
...
int appFcListFonts(PostScriptFontList *psfl)
{
	...
	os = FcObjectSetBuild(FC_FAMILY, FC_WIDTH, FC_STYLE, FC_FILE, FC_INDEX,
			      FC_CHARSET, FC_SLANT, FC_WEIGHT, FC_SPACING,
			      (char *)0);
	if (!os) {
		XDEB(os);
		rval = -1;
...
static int appFcAddChar(int unicode, void *vcharset)
{
					     ...
					     const PostScriptFontList *psfl)
{
	...
	appDebug("TRY: file=\"%s\" family= \"%s\" style=\"%s\"\n", (char *)file,
		 (char *)family, (char *)style);
#endif

	afi = psPostScriptFontListGetFontInfoByFaceFile(psfl, (char *)file);
	if (!afi) { /*SXDEB((char *)file,afi);*/
	}

	return afi;
}

static AfmFontInfo *appFcGetFontInfoFromList(FcPattern *pat,
					     const PostScriptFontList *psfl)
{
	FcResult fcResult;
	FcFontSet *fs = (FcFontSet *)0;
					  ...
					  const PostScriptFontList *psfl)
{
	...
	if (unicodesUsed) {
		...
		if (utilIndexSetForAll(unicodesUsed, appFcAddChar, fcCharset) <
		    0) {
			LDEB(1);
			goto ready;
		}
...
static int appFontAddOneCharToSet(int val, void *voidfcs)
{
...
XftFont *drawOpenXftFont(DrawScreenFont *dsf)
{
	...
	if (dsf->dsfUnicodesUsed) {
		...
		/* IndexSet -> FcCharSet */
		utilIndexSetForAll(dsf->dsfUnicodesUsed, appFontAddOneCharToSet,
				   (void *)fcCharset);
	}
	...
	/*  2  */
	rval = XftFontOpen(
		dsf->dsfDisplay, screen, FC_FAMILY, FcTypeString, scratch,
		FC_WIDTH, FcTypeInteger, afi->afiWidthInt, FC_PIXEL_SIZE,
		FcTypeDouble, (double)dsf->dsfSizePixels, FC_SLANT,
		FcTypeInteger,
		(FONT_IS_SLANTED(afi) ? FC_SLANT_ITALIC : FC_SLANT_ROMAN),
		FC_WEIGHT, FcTypeInteger,
		(FONT_IS_BOLD(afi) ? FC_WEIGHT_BOLD : FC_WEIGHT_REGULAR),
		fc_charset, typeCharset, fcCharset, NULL);
