...
int appCallPrintFunction(SimpleOutputStream *sos, const PrintJob *pj)
{
...
static int appDocBuildTmpfileCommand(MemoryBuffer *target, const char *format,
...
int appPrintDocument(int printer, const PrintJob *pj)
{
	int rval = 0;
	EditApplication *ea = pj->pjApplication;
	...
	case APPprinterTMPFILE:
		...
		if (appDocBuildTmpfileCommand(&command, pd->pdCommand,
					      &scratchName)) {
...
void appPrintJobForEditDocument(PrintJob *pj, EditDocument *ed,
...
void appApplicationSettingsToPrintGeometry(PrintGeometry *pg,
					   EditApplication *ea)
{
	pg->pgUsePostScriptFilters = ea->eaUsePostScriptFiltersInt > 0;
	pg->pgUsePostScriptIndexedImages = ea->eaUsePostScriptIndexedImagesInt >
					   0;
...
static int appPrintJobForCommand(PrintJob *pj, PrintGeometry *pg,
				 EditApplication *ea, const char *fromName)
{
	int rval = 0;
	...
	if ((*ea->eaOpenDocument)(ea, pj->pjPrivateData, &(pj->pjFormat),
				  ...
				  readOnly, suggestStdin, formatHint,
				  &filename)) {
		SDEB((char *)fromName);
		rval = -1;
		goto ready;
...
static void appPrintFinishCommandRun(EditApplication *ea, PrintJob *pj)
{
...
static int appPrintStartCommandRun(EditApplication *ea, PrintJob *pj,
		   ...
		   const char *toName, const char *paperString)
{
	...
	if (!strcmp(toName, "-ps-") || !strcmp(toName, "-")) {
		sos = sioOutStdoutOpen();
	} else {
		sos = sioOutFileioOpenS(toName);
	}
