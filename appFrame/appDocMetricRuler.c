			     ...
			     APP_EVENT_HANDLER_T mouseDown)
{
	...
	ed->edTopRuler = appMakeMetricRuler(
		...
		ea->eaMagnification, magnification, /*  magnification	*/
	...
	guiDrawSetConfigureHandler(ed->edTopRulerWidget,
				   appHorizontalRulerConfigure,
				   (void *)ed->edTopRuler);

	guiDrawSetRedrawHandler(ed->edTopRulerWidget,
				appRedrawHorizontalMetricRuler,
				(void *)ed->edTopRuler);
...
void appDocSetMetricLeftRuler(EditDocument *ed, int leftRulerWidth,
			      double magnification, int unitInt,
			      APP_EVENT_HANDLER_T mouseDown)
{
	EditApplication *ea = ed->edApplication;
