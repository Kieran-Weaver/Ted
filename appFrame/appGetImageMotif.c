...
static int appGetIndexedColors(XColor **pColors, Display *display,
...
static int appGetCalculatedColors(XColor **pColors, int depth,
				  ...
				  unsigned long blue_mask)
{
	...
	if (red_mask == 0 || green_mask == 0 || blue_mask == 0) {
		LLLDEB(red_mask, green_mask, blue_mask);
		return -1;
	}
...
static int appGetRGB8Description(BitmapDescription *bd, double pixelsPerTwip,
				 const APP_IMAGE *xim, int count)
{
	int bitsPerPixel;
	...
	if (!bd->bdPalette.cpColors) {
		LXDEB(count, bd->bdPalette.cpColors);
		return -1;
	}
...
static int appGetRGBDescription(BitmapDescription *bd, double pixelsPerTwip,
				const APP_IMAGE *xim)
{
	bd->bdPixelsWide = xim->width;
	bd->bdPixelsHigh = xim->height;
...
static int appGetRGB8Palette(XColor *xc, int count, RGB8Color *palette)
{
	unsigned int from;
	unsigned int to;
...
int appGetImageMotif(RasterImage *abi, const APP_IMAGE *xim, Display *display)
{
	int bitsPerPixel;
	int bytesPerLine;
	char *data;
	...
	case 8:
		...
		bd.bdPalette.cpColorCount =
			appGetRGB8Palette(xc, count, bd.bdPalette.cpColors);
	...
	case 16:
		...
		for (row = 0; row < bd.bdPixelsHigh; row++) {
			psh = (unsigned short *)(buffer +
						 row * bd.bdBytesPerRow);

			memcpy(psh, data + row * bytesPerLine,
			       bd.bdBytesPerRow);
		      ...
		      APP_SELECTION_EVENT *event)
{
	...
	ret = XGetWindowProperty(selEvent->display, selEvent->requestor,
				 selEvent->property, 0, 1, False,
				 AnyPropertyType, &typeFound, &formatFound,
	...
	ret = XGetGeometry(selEvent->display, pmap, &wignored, &xignored,
			   &yignored, &width, &height, &bignored, &depth);
	/*
    if  ( ret != Success )
	{ LLDEB(ret,Success); return -1;	}
    */
	xim = XGetImage(selEvent->display, pmap, 0, 0, width, height, AllPlanes,
			ZPixmap);
