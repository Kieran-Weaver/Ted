...
void guiExposeDrawingWidget(APP_WIDGET nativeWidget)
{
	if (!XtWindow(nativeWidget)) {
		XDEB(XtWindow(nativeWidget));
		return;
	}
	if (!XtIsRealized(nativeWidget)) {
		LDEB(XtIsRealized(nativeWidget));
		return;
	}

	XClearArea(XtDisplay(nativeWidget), XtWindow(nativeWidget), 0, 0, 0, 0,
		   True);
				     ...
				     const DocumentRectangle *drExpose)
{
	...
	if (XSendEvent(XtDisplay(nativeWidget), XtWindow(nativeWidget), 0,
		       ExposureMask, &xev) != Success) { /*LDEB(1);*/
	}
...
static void guiCollectExposuresMotif(DocumentRectangle *drClip,
				     ...
				     APP_EVENT *event)
{
	...
	appDebug("E=====: [%4d+%4d]x[%4d+%4d]\n", drClip->drX0,
		 drClip->drX1 - drClip->drX0 + 1, drClip->drY0,
	...
	while (QLength(display) > 0) {
		...
		drMore.drX1 =
			(int)nextEvent.xexpose.x + (int)nextEvent.xexpose.width;
		drMore.drY1 = (int)nextEvent.xexpose.y +
			      (int)nextEvent.xexpose.height;

#if LOG_EXPOSES
		appDebug("E+++++: [%4d+%4d]x[%4d+%4d]\n", drMore.drX0,
			 drMore.drX1 - drMore.drX0 + 1, drMore.drY0,
...
void guiCollectExposures(DocumentRectangle *drClip, APP_WIDGET nativeWidget,
			 APP_EVENT *event)
{
	guiCollectExposuresMotif(drClip, XtDisplay(nativeWidget),
				 XtWindow(nativeWidget), event);
}
...
static void guiGetColor(RGB8Color *rgb8, const char *xmName,
			APP_WIDGET nativeWidget)
{
	...
	XtVaGetValues(nativeWidget, xmName, &(xc.pixel), NULL);
