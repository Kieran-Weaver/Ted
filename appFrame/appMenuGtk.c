			   ...
			   unsigned int *pKeyCode)
{
	...
	for (;;) {
		...
		if (!strncmp(s, "Ctrl", 4)) {
			keyMask |= GDK_CONTROL_MASK;
			s += 4;
			continue;
		}
		if (!strncmp(s, "Shift", 5)) {
			keyMask |= GDK_SHIFT_MASK;
			s += 5;
			continue;
		}
		if (!strncmp(s, "Lock", 4)) {
			keyMask |= GDK_LOCK_MASK;
			s += 4;
			continue;
		}
		if (!strncmp(s, "Mod1", 4)) {
			keyMask |= GDK_MOD1_MASK;
			s += 4;
			continue;
		}
		if (!strncmp(s, "Mod2", 4)) {
			keyMask |= GDK_MOD2_MASK;
			s += 4;
			continue;
		}
		if (!strncmp(s, "Mod3", 4)) {
			keyMask |= GDK_MOD3_MASK;
			s += 4;
			continue;
		}
		if (!strncmp(s, "Mod4", 4)) {
			keyMask |= GDK_MOD4_MASK;
			s += 4;
			continue;
		}
		if (!strncmp(s, "Mod5", 4)) {
			keyMask |= GDK_MOD5_MASK;
			s += 4;
			continue;
		}
			  ...
			  void *target)
{
	...
	if (ami->amiCallback) {
		gtk_signal_connect(GTK_OBJECT(rval), "activate",
				   (GtkSignalFunc)ami->amiCallback, target);
	}
				...
				AppMenuItem *ami, void *target)
{
	...
	gtk_check_menu_item_set_show_toggle(GTK_CHECK_MENU_ITEM(rval), TRUE);

	gtk_menu_append(GTK_MENU(menu), rval);
	gtk_widget_show(rval);

	if (ami->amiCallback) {
		gtk_signal_connect(GTK_OBJECT(rval), "toggled",
				   (GtkSignalFunc)ami->amiCallback, target);
	}
