...
void drawStartScreenFontList(NumberedPropertiesList *sfl)
{
	...
	utilStartNumberedPropertyList(
		...
		sizeof(DrawScreenFont), (InitPagedListItem)drawInitScreenFont,
		      ...
		      const AfmFontInfo *afi, int pixelSize)
{
	...
	if (screenFont >= 0) {
		...
		psStrikethroughGeometry(
			&(dsfFound->apfStrikethroughPositionPixels),
		...
		psGetSuperBaseline(&(dsfFound->apfSuperBaseline),
				   baselinePixels, dsfFound->dsfSizePixels,
				   dsfFound->apfPsFontInfo);
		psGetSubBaseline(&(dsfFound->apfSubBaseline), baselinePixels,
				 dsfFound->dsfSizePixels,
				 dsfFound->apfPsFontInfo);
			     ...
			     const char *s, int len)
{
	...
	if (!fullDsf) {
		LXDEB(fullScreenFont, fullDsf);
		return -1;
	}

	scapsDsf = drawGetScreenFontByNumber(npl, scapsScreenFont);
	if (!scapsDsf) {
		LXDEB(scapsScreenFont, scapsDsf);
		return -1;
	}

#ifdef USE_XFT
	if (fullDsf->dsfXftFont && scapsDsf->dsfXftFont &&
	    !drawTextExtentsXft(drText, x, y, scapsDsf, fullDsf, s, len)) {
		return 0;
	}
#endif
