...
typedef struct AppPageTool {
	...
	struct DrawingSurface *aptDrawingSurface;
...
static AppConfigurableResource APP_PageToolresourceTable[] = {
	APP_RESOURCE("unit", offsetof(AppPageToolResources, aptrUnitType),
		     "inch"),
	APP_RESOURCE("pageToolPaper", offsetof(AppPageToolResources, aptrPaper),
		     "Paper"),
	APP_RESOURCE("pageToolCustom",
		     offsetof(AppPageToolResources, aptrCustom), "Custom"),
	APP_RESOURCE("pageToolPortrait",
		     offsetof(AppPageToolResources, aptrPortrait), "Portrait"),
	...
	APP_RESOURCE("pageToolRevert",
		     offsetof(AppPageToolResources, aptrRevert), "Revert"),
	APP_RESOURCE("pageToolSet", offsetof(AppPageToolResources, aptrSet),
		     "Set"),
#if 0
	For use as a Dialog.
...
static APP_EVENT_HANDLER_H(appPageToolDrawPage, w, voidapt, exposeEvent)
{
	...
	struct DrawingSurface *ds = apt->aptDrawingSurface;
...
static void appPageToolPaperRectChanged(PaperChooser *pc, void *voidapt,
...
static APP_BUTTON_CALLBACK_H(appPageToolRevertPushed, w, voidapt)
{
	...
	appMarginToolShowMargins(&(apt->aptMarginTool), apt->aptUnitType,
				 &(apt->aptGeometryChosen));
...
static APP_BUTTON_CALLBACK_H(appPageToolSetPushed, w, voidapt)
{
    ...
    */
	...
	if (appMarginToolGetMargins(&margChgMask, &margUpdMask,
				    apt->aptUnitType, &(apt->aptMarginTool),
				    &dg)) {
		return;
	}

	if (appPaperChooserGetSize(&sizeChgMask, &(apt->aptPaperChooser),
				   &dg)) {
		LDEB(1);
		return;
	}
...
static APP_TXACTIVATE_CALLBACK_H(appPageToolMarginChanged, w, voidapt)
{
	...
	if (appMarginToolGetMargins(&dgChgMask, &dgUpdMask, apt->aptUnitType,
				    &(apt->aptMarginTool), &dg)) {
		return;
	}
...
static void appPageToolMakePaperFrame(APP_WIDGET parent,
				      ...
				      AppPageTool *apt)
{
	appMakePaperChooserWidgets(parent, aptr->aptrPaper, apt->aptUnitType,
				   &(apt->aptPaperChooser),
				   appPageToolPaperRectChanged, (void *)apt);
					   ...
					   AppPageTool *apt)
{
	...
	appMakeButtonInRow(&revertButton, row, aptr->aptrRevert,
			   appPageToolRevertPushed, (void *)apt, 0, colspan,
			   buttonIsDefault);

	appMakeButtonInRow(&setButton, row, aptr->aptrSet, appPageToolSetPushed,
			   (void *)apt, 1, colspan, buttonIsDefault);
...
void *appMakePageTool(EditApplication *ea, APP_WIDGET pageOption,
		      const char *pixmapName)
{
	...
	if (!gotResources) {
		appGuiGetResourceValues(
			&gotResources, ea, &aptr, APP_PageToolresourceTable,
	...
	if (apt->aptUnitType < 0) {
		SDEB(aptr.aptrUnitType);
		apt->aptUnitType = UNITtyINCH;
	}
	...
	apt->aptPixelsPerTwip =
		(apt->aptPageHighMm * ea->eaPixelsPerTwip) / A3_MM_HIGH;

	apt->aptPageDrawing = appMakePageDrawing(
		apt->aptMainWidget, ea->eaPixelsPerTwip, apt->aptPageHighMm,
