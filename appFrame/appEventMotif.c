...
int guiDrawGetSizeFromConfigureEvent(int *pWide, int *pHigh, Widget w,
				     XEvent *event)
{
		...
		*pUpDown = +1;
		...
		if (w == prevWidget) {
			if (bevent->time != lastTime &&
				...
				seq++;
				if (bevent->time - prevTime <
				    2 * multiClickInterval) {
					seq++;
				}
			}
...
int guiGetCoordinatesRelativeToWidget(int *pX, int *pY, const APP_WIDGET w)
{
	Window root;
	Window child;
	int rootX;
...
int guiGetCoordinatesFromMouseMoveEvent(int *pX, int *pY, Widget w,
					XEvent *event)
{
...
void guiDrawSetConfigureHandler(APP_WIDGET w, APP_EVENT_HANDLER_T handler,
				void *through)
{
	XtAddEventHandler(w, StructureNotifyMask, False, handler, through);
}

void guiDrawSetButtonPressHandler(APP_WIDGET w, APP_EVENT_HANDLER_T handler,
				  void *through)
{
