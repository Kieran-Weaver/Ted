...
GtkWidget *appListGtkMakeParent(GtkAdjustment **pVAdjustment, GtkWidget *column)
{
	...
	scrolled_window =
		gtk_scrolled_window_new((GtkAdjustment *)0, (GtkAdjustment *)0);
	...
	gtk_container_set_focus_vadjustment(GTK_CONTAINER(scrolled_window),
					    vadjust);
...
void appListGtkSetVisibleSize(GtkWidget *list, GtkWidget *scrolled_window,
			      int visibleItems)
{
	int pixelSize = 9;
			      ...
			      gpointer voidla)
{
	...
	if (la->laListHigh > 0 && la->laParentHigh > 0 &&
...
static void appListGtkParentResized(GtkWidget *w, GdkRectangle *allocation,
				    gpointer voidla)
{
	ListAction *la = (ListAction *)voidla;
...
static void appListGtkSetFocusChild(GtkTreeView *parent, GtkWidget *w,
				    gpointer voidla)
{
	ListAction *la = (ListAction *)voidla;
...
ListAction *appListGtkSetCallbacks(GtkWidget *widget,
				   GtkWidget *scrolled_window,
				   ...
				   void *through)
{
	...
	if (la->laParentWidget) {
		gtk_signal_connect(GTK_OBJECT(la->laParentWidget),
				   "size-allocate",
				   (GCallback)appListGtkParentResized,
				   (void *)la);
	}
...
{
	return (ListAction *)gtk_object_get_data(GTK_OBJECT(widget),
						 APP_ListAction);
}
...
void appGuiGtkSetListAdjustment(ListAction *la, int position)
{
	...
	if (la->laItemCount == 0) { /*LLDEB(position,la->laItemCount);*/
		return;
	}

	increment = la->laListHigh / la->laItemCount;
	if (increment == 0) { /*LDEB(increment);*/
		return;
	}
