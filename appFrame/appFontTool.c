...
static int EditFontToolSizesHalfPoints[] = { 2 * 8,  2 * 9,  2 * 10, 2 * 11,
					     2 * 12, 2 * 14, 2 * 16, 2 * 18,
					     2 * 24, 2 * 36, 2 * 48, 2 * 64 };
...
static void appFontToolFormatSize(char *scratch, int halfPoints)
{
	if (halfPoints % 2) {
		sprintf(scratch, "%d.5", halfPoints / 2);
	} else {
		sprintf(scratch, "%d", halfPoints / 2);
	}
}
...
static APP_EVENT_HANDLER_H(appFontRedraw, w, voidafc, exposeEvent)
{
	...
	if (afc->afcScreenFont <
	    0) { /*SLDEB(afc->afcChoiceText,afc->afcScreenFont);*/
		return;
	}
	...
	x = y = 0;
	drawGetTextExtents(&drText, ds, x, y, afc->afcScreenFont,
			   afc->afcChoiceText, l);

	y = (drWidget.drY1 + drWidget.drY0) / 2 -
	    (drText.drY1 + drText.drY0 + 1) / 2;
	x = (drWidget.drX1 + drWidget.drX0) / 2 -
	    (drText.drX1 + drText.drX0 + 1) / 2;
...
static void appFontShowSizeInList(AppFontChooser *afc, int fontSizeHalfPoints)
{
	int pos;
	int sizeChosen = -1;
	...
	for (pos = 0; pos < EditFontToolSizeCount; pos++) {
		if (fontSizeHalfPoints > 0 &&
		    EditFontToolSizesHalfPoints[pos] == fontSizeHalfPoints) {
			sizeChosen = pos;
			break;
		}
	}
	...
	if (sizeChosen >= 0) {
		...
		appGuiRemoveSelectionFromListWidget(afc->afcSizeList);
	}
...
static void appFontShowSizeInText(AppFontChooser *afc, int fontSizeHalfPoints)
{
	char scratch[80];
...
static int appFontToolGetSize(int *pFontSizeHalfPoints, int anyway,
			      AppFontChooser *afc)
{
	...
	if (isEmpty) {
		...
		if (appGetLengthFromTextWidget(afc->afcSizeText, &sizeTwips,
					       &changed, UNITtyPOINTS, minValue,
					       adaptToMin, maxValue,
					       adaptToMax)) {
			if (anyway) {
				sizeHalfPoints = -1;
...
static int appFontToolGetBaseline(int *pBaselineHalfPoints, int *pEmpty,
				  int anyway, AppFontChooser *afc)
{
	ExpandedTextAttribute *eta = &(afc->afcTextAttributeChosen);
	TextAttribute *ta = &(eta->etaTextAttribute);
	...
	if (*pEmpty) {
		...
		if (appGetLengthFromTextWidget(
			    afc->afcBaselineText, &baselineTwips, &changed,
			    UNITtyPOINTS, minValue, adaptToMin, maxValue,
			    adaptToMax)) {
			if (anyway) {
				baselineHalfPoints = -1;
			} else {
...
static int appFontGetCurrent(int *pFamily, int *pFace, int *pFontSizeHalfPoints,
			     int anyway, AppFontChooser *afc)
{
	...
	if (PROPmaskISSET(chosenMask, TApropFONT_NUMBER) &&
		...
		fontChosen = utilDocumentFontListGetArrayIndex(
			dfl, afc->afcFontSortIndexChosen);
	}
...
static void appFontFormatCurrent(char *target, AppFontChooser *afc,
				 const char *familyName, int face,
				 int sizeHalfPoints)
{
	...
	if (sizeHalfPoints > 0) {
		if (sizeHalfPoints % 2) {
			...
			sprintf(sizeStr, "%d", sizeHalfPoints / 2);
		}
	}
...
static void appFontShowExampleOfCurrent(AppFontChooser *afc)
{
	...
	appFontFormatCurrent(afc->afcChoiceText, afc, etaC->etaFontName, face,
			     fontSizeHalfPoints);
	...
	if (afi) {
		...
		appGuiChangeLabelText(afc->afcPsName, "<?>");
	}
...
static void appFontReflectFamily(AppFontChooser *afc)
{
	...
	if (afc->afcFaceChosen >= 0) {
		appGuiSelectPositionInListWidget(
			afc->afcFaceList,
...
static void appFontReflectProperties(AppFontChooser *afc)
{
	...
	if (PROPmaskISSET(chosenMask, TApropFONT_NUMBER) && etaC->etaFontName) {
		int fontNumber = docGetFontByName(dfl, etaC->etaFontName);

		if (fontNumber >= 0) {
			fontIndex = utilDocumentFontListGetSortIndex(
				dfl, fontNumber);
		}
	}
...

	{
		if (PROPmaskISSET(chosenMask, TApropBASELINE_SHIFT)) {
			...
			appFontToolFormatSize(scratch,
					      taC->taBaselineShiftHalfPoints);
...
static APP_TOGGLE_CALLBACK_H(appFontSmallcapsToggled, w, voidafc, voidtbcs)
{
	...
	if (set) {
		...
		if (resetOther) {
			taC->taCapitals = 0;
			appGuiSetToggleState(afc->afcCapitalsToggle,
					     taC->taCapitals);
...
static APP_TOGGLE_CALLBACK_H(appFontCapitalsToggled, w, voidafc, voidtbcs)
{
	...
	if (set) {
		...
		if (resetOther) {
			taC->taSmallCaps = 0;
			appGuiSetToggleState(afc->afcSmallcapsToggle,
					     taC->taSmallCaps);
			PROPmaskADD(&(afc->afcChosenMask), TApropSMALLCAPS);
		}
	} else {
...
static APP_LIST_CALLBACK_H(appFontFamilyChosen, w, voidafc, voidlcs)
{
	...
	if (fontIndex < 0 || fontIndex >= dfl->dflFontCount) {
		LLDEB(fontIndex, dfl->dflFontCount);
		return;
	}
	...
	if (afc->afcFaceChosen >= 0 && !df->dfPsFontInfo[afc->afcFaceChosen]) {
		int faceChosen;
				  ...
				  AppFontChooser *afc)
{
	...
	afc->afcChooseRow =
		appMakeRowInColumn(parent, colcount, heightResizable);
	...
	appMakeLabelInColumn(&(afc->afcFamilyLabel), afc->afcFamilyColumn,
			     aftr->aftrFamily);

	appGuiMakeListInColumn(&(afc->afcFamilyList), afc->afcFamilyColumn,
			       listHeight, appFontFamilyChosen,
			       (APP_BUTTON_CALLBACK_T)0, (void *)afc);
	...
	appMakeLabelInColumn(&(afc->afcFaceLabel), afc->afcFaceColumn,
			     aftr->aftrFace);

	appGuiMakeListInColumn(&(afc->afcFaceList), afc->afcFaceColumn,
			       listHeight, appFontFaceChosen,
			       (APP_BUTTON_CALLBACK_T)0, (void *)afc);

	{
		...
		appGuiMakeListInColumn(&(afc->afcSizeList), afc->afcSizeColumn,
				       0, appFontSizeChosen,
				       (APP_BUTTON_CALLBACK_T)0, (void *)afc);
		...
		appGuiSetGotValueCallbackForText(
			afc->afcSizeText, appFontToolSizeChanged, (void *)afc);

		appGuiSetTypingCallbackForText(
			afc->afcSizeText, appFontToolSizeTyped, (void *)afc);
	}
...
static void appFontMakePreviewDrawing(APP_WIDGET parent, AppFontChooser *afc,
				      int twipsSize)
{
	const int wide = -1;
	...
	appGuiMakeDrawingAreaInColumn(&(afc->afcSampleDrawing), parent, wide,
				      high, heightResizable, appFontRedraw,
				      (void *)afc);
...
static int appFontToolSetCurrentFont(AppFontChooser *afc,
			       ...
			       const ColorPalette *cp)
{
	...
	if (afc->afcCurrentDocumentId != documentId) {
		...
		for (i = 0; i < dflTo->dflFontCount; i++) {
			...
			appGuiAddValueToListWidget(afc->afcFamilyList, -1,
						   df->dfName);
		}
			 ...
			 const InspectorSubjectResources *isr)
{
...

	{
		...
		guiToolMake3ToggleRow(&(afc->afcToggleRow1), pageWidget,
				      ...
				      appFontSmallcapsToggled, (void *)afc);
		...
		guiToolMake3ToggleRow(&(afc->afcToggleRow2), pageWidget,
				      ...
				      aftr->aftrSubscript, aftr->aftrCapitals,

				      appFontStrikethroughToggled,
				      appFontSubscriptToggled,
				      appFontCapitalsToggled, (void *)afc);
		...
		guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
				      &(is->isRevertButton),
				      &(is->isApplyButton), isr->isrRevert,
				      isr->isrApplyToSubject,
				      appFontRevertPushed, appFontSetPushed,
				      afc);
...
static AppConfigurableResource APP_FontToolResourceTable[] = {
	APP_RESOURCE("fontToolFamily",
		     offsetof(AppFontToolResources, aftrFamily), "Family"),
	APP_RESOURCE("fontToolFace", offsetof(AppFontToolResources, aftrFace),
		     "Typeface"),
	APP_RESOURCE("fontToolSize", offsetof(AppFontToolResources, aftrSize),
		     "Size"),
	...
	APP_RESOURCE("fontToolBoldSlanted",
		     offsetof(AppFontToolResources,
			      aftrFaces[FONTfaceBOLD_SLANTED]),
		     "Bold Italic"),
	...
	APP_RESOURCE("fontToolCapitals",
		     offsetof(AppFontToolResources, aftrCapitals), "All caps"),

	APP_RESOURCE("fontToolBaseline",
		     offsetof(AppFontToolResources, aftrBaseline), "Raise"),
};
