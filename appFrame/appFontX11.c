...
/*  2  */
#define EditFontFamilyTemplate "-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s"
...
static AppFontEncoding AppFontXEncodings[] = {
...

	{
		"iso8859-15",
		1,
		uniIso8859_15GlyphUnicodes,
	},
	{
		"iso8859-2",
		1,
		uniIso8859_2GlyphUnicodes,
	},
	{
		"iso8859-5",
		1,
		uniIso8859_5GlyphUnicodes,
	},
	{
		"iso8859-7",
		1,
		uniIso8859_7GlyphUnicodes,
	},
	{
		"iso8859-9",
		1,
		uniIso8859_9GlyphUnicodes,
	},
	{
		"iso8859-13",
		1,
		uniIso8859_13GlyphUnicodes,
	},
	{
		"koi8-u",
		1,
		uniKoi8RGlyphUnicodes,
	},
	{
		"koi8-r",
		1,
...
static void appFontQueryString(char *target, const char *foundry,
			       const char *family, const char *weight,
			       const char *slant, const char *swdth,
			       const char *pxlsz, const char *registry_encoding)
{
	...
	sprintf(target, EditFontFamilyTemplate, foundry, family, weight, slant,
		swdth, /* swdth		*/
		EditFontAnyTemplate, /* adstyl		*/
		pxlsz, anySizeTemplate, /* ptsz			*/
...
static int appFontExtractString(const char *s, const char **pPast, char *target)
{
	if (*s != '-') {
		SDEB(s);
		return -1;
...
static int appFontSkipString(const char *s, const char **pPast)
{
	if (*s != '-') {
		SDEB(s);
		return -1;
...
static int appFontSplitXFont(const char *fontname, char *foundry, char *family,
			     char *weight, char *slant, char *swdth,
			     char *pxlszStr, char *spacing, char *avgwdth,
			     char *registry, char *encoding)
{
	const char *s;
...
static int appCmpFacePixelSizes(const void *vxffs1, const void *vxffs2)
{
...
static int appFontXCatalog(Display *display, AppFontEncoding *afe)
{
	char fontFormat[120];
	...
	afe->afeXFontList =
		XListFonts(display, fontFormat, 4000, &(afe->afeXFontCount));
	...
	for (idx = 0; idx < afe->afeXFontCount; idx++) {
		...
		if (appFontSplitXFont(afe->afeXFontList[idx], foundry, family,
				      weightStr, slantStr, swdthStr, pxlszStr,
				      spacing, avgwdth, ignore, /*  registry  */
				      ignore)) /*  encoding  */
		{
			LSDEB(idx, afe->afeXFontList[idx]);
			continue;
		}
		...
		if (strcmp(pxlszStr, "0") &&
		    !strcmp(avgwdth, "0")) { /*SSDEB(pxlszStr,avgwdth);*/
			continue;
		}

		if (psFontGetWidth(&swdth, swdthStr)) {
			SSDEB(swdthStr, afe->afeXFontList[idx]);
			continue;
		}
		...
		if (fam >= afe->afeXfontFamilyCount) {
			...
			xff = (XFontFamily *)realloc(afe->afeXfontFamilies,
						     (afe->afeXfontFamilyCount +
						      1) * sizeof(XFontFamily));
			if (!xff) {
				LXDEB(afe->afeXfontFamilyCount, xff);
				return -1;
			}
		...
		if (face >= xff->xffFaceCount) {
			xft = (XFontTypeface *)realloc(
				xff->xffFaces, (xff->xffFaceCount + 1) *
						       sizeof(XFontTypeface));
			if (!xft) {
				LXDEB(xff->xffFaceCount, xft);
				return -1;
			}
			xff->xffFaces = xft;
			...
			if (psFontGetWeight(&(xft->xftWeight), weightStr)) {
				SSDEB(afe->afeXFontList[idx], weightStr);
				continue;
			}
		...
		*/
			...
			xffs = (XFontFaceSize *)realloc(
				xft->xftFaceSizes, (xft->xftFaceSizeCount +
						    1) * sizeof(XFontFaceSize));
			if (!xffs) {
				LXDEB(xft->xftFaceSizeCount, xffs);
				return -1;
			}
		...
		for (face = 0; face < xff->xffFaceCount; xft++, face++) {
			if (xft->xftFaceSizeCount > 1) {
				qsort(xft->xftFaceSizes, xft->xftFaceSizeCount,
				      sizeof(XFontFaceSize),
				      appCmpFacePixelSizes);
			}
		}
	}
...
static int appFontXFontUsingQuery(char *target, const char *queryFormat,
				  int pixelSize)
{
...
static int appFontX11ScoreSimilarity(const XFontFamily *xff, const char *psName,
				     int psNameLen, int psWidth)
{
	int common;
	int wdif;
	...
	return (100 * common) / psNameLen + (30 * (wrange - wdif)) / wrange;
}
...
static int appFontXFontName(char *target, Display *display,
			    const AfmFontInfo *afi, AppFontEncoding *afe,
			    int anyFont, int pixelSize, const char *psName,
			    int psNameLen, int psWidth)
{
	...
	for (f = 0; f < afi->afiX11FontCount; f++) {
		...
		if (o11 > 0 && afi->afiX11Fonts[f][o11 - 1] == '-' &&
		    !strcmp(afi->afiX11Fonts[f] + o11, afe->feX11Reg_Enc)) {
			t1Entry = afi->afiX11Fonts[f];
			break;
		}
	}
	...
	for (f = 0; f < afe->afeXfontFamilyCount; xff++, f++) {
		...
		score = appFontX11ScoreSimilarity(xff, psName, psNameLen,
						  psWidth);
	...
	if (fam < 0) {
		if (t1Entry) {
			if (appFontXFontUsingQuery(target, t1Entry,
						   pixelSize)) {
				SDEB(t1Entry);
				return -1;
			}
...

	{
		...
		for (face = 0; face < xff->xffFaceCount; xft++, face++) {
			...
			if (difCount < difCountFound && difCount < 2 &&
			    distance < distanceFound && distance < 0.3) {
				faceFound = face;
	...
	if (xft->xftFaceSizeCount > 0) {
		...
		if (i > 0 && xft->xftFaceSizes[i].xffsPixelSize > pixelSize) {
			int d0;
			int d1;

			d0 = (100 * pixelSize) /
			     xft->xftFaceSizes[i - 1].xffsPixelSize;
			d1 = (100 * xft->xftFaceSizes[i].xffsPixelSize) /
			     pixelSize;
		...
		if (dv <= 115 || (!t1Entry && xft->xftScalableListIndex < 0)) {
...
static int appFontGetEncodingInfo(const IndexMapping **pIm,
				  const IndexSet **pIs, const int **pUnicodes,
				  AppFontEncoding *fe, const char *fontName)
{
...
static int appXfontHasCode(const XFontStruct *xfs, int code)
{
...
static void appXFontSymbols(EncodedScreenFontList *esfl,
			    const EncodedScreenFont *sf, IndexSet *isRequired,
			    const int *unicodes, int font)
{
			...
			for (symbol = 0; symbol < 256 * 256; symbol++) {
				if (appXfontHasCode(xfs, symbol)) {
					if (utilIndexMappingGet(
						    &(esfl->esflSymbolToFont),
						    symbol) >= 0) {
						continue;
					}

					if (utilIndexMappingPut(
						    &(esfl->esflSymbolToFont),
						    symbol, font)) {
						LLDEB(symbol, font);
					}
...
static int appFontX11TryEncoding(EncodedScreenFont *sf,
				 ...
				 DrawScreenFont *dsf, int font,
				 AppFontEncoding *fe, const AfmFontInfo *afi,
				 int sizePixels, const char *psName,
				 int psNameLen, int psWidth, int anyFont,
				 IndexSet *isRequired)
{
	int rval = 0;
	int res;
	...
	res = appFontGetEncodingInfo(&im, &isProvided, &unicodes, fe,
				     afi->afiFamilyName);
	if (res < 0) {
		LDEB(res);
		rval = -1;
	...
	if (!dsf->apfFontName[0]) {
		if (strlen(sf->esfFontName) >= X_FONTNAME_MAX) {
			...
			strcpy(dsf->apfFontName, sf->esfFontName);
		}
	}
...
int appFontOpenX11Fonts(DrawScreenFont *dsf)
{
	...
	for (anyFont = 0; anyFont <= 1; anyFont++) {
		...
		for (encIdx = 0; encIdx < AppFontXEncodingCount;
		     fe++, encIdx++) {
			int res;

			res = appFontX11TryEncoding(sf, esfl, dsf, count, fe,
						    afi, dsf->dsfSizePixels,
						    psName, psNameLen, psWidth,
						    anyFont, &isRequired);
			if (res < 0) {
				LDEB(res);
				rval = -1;
	...
	esfl->esfFonts = esfs;
	esfs = (EncodedScreenFont *)0; /* steal*/
	esfl->esfFontCount = count;
	count = 0; /* steal */
...
int appFindX11Fonts(APP_WIDGET topw, PostScriptFontList *psfl)
{
	int rval = 0;
	int i;
	...
	appFontQueryString(
		fontFormat, EditFontAnyTemplate, /* foundry,		*/
		EditFontAnyTemplate, /* family,		*/
		EditFontAnyTemplate, /* weight,		*/
		EditFontAnyTemplate, /* slant,		*/
		EditFontAnyTemplate, /* swdth,		*/
	...
	fontNames = XListFontsWithInfo(display, fontFormat, 6000, &fontCount,
				       &fontInfos);
	...
	for (i = 0; i < fontCount; i++) {
		...
		if (XGetFontProperty(&(fontInfos[i]), XA_FACE_NAME,
				     &faceNameA)) {
			char *faceName = XGetAtomName(display, faceNameA);
			AfmFontInfo *afi;

			afi = (AfmFontInfo *)utilTreeGetEQ(
				full2afi, (const char **)0, faceName);
			if (afi) {
...
void addDrawX11GetFontProperties(EncodedScreenFont *esf, DrawScreenFont *dsf)
{
	...
	static const char *an[] = { "FOUNDRY",		"FAMILY_NAME",
				    "WEIGHT_NAME",	"SLANT",
				    "SETWIDTH_NAME",	"ADD_STYLE_NAME",
				    "PIXEL_SIZE",	"POINT_SIZE",
				    "RESOLUTION_X",	"RESOLUTION_Y",
				    "SPACING",		"AVERAGE_WIDTH",
	...
	for (i = 0; i < AppFontXEncodingCount; fe++, i++) {
		if (!strcmp(esf->esfFontName + prevDash + 1,
			    fe->feX11Reg_Enc)) {
			IndexMapping *im = &(fe->feSymbolToByteMapping);
			const int *unicodes = fe->feGlyphUnicodes;
			...
			if (unicodes) {
				...
				if (utilIndexMappingIsEmpty(im) &&
				    utilIndexMappingBuildBackward(im, unicodes,
								  256)) {
					LDEB(1);
					return;
				}
...
int guiSystemFont(DrawingSurface ds, APP_FONT *xfs)
{
	...
	if (!dsf->dsfEncodedFonts.esfFonts) {
		XDEB(dsf->dsfEncodedFonts.esfFonts);
		return -1;
	}
