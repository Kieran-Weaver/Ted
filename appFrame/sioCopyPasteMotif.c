...
static int sioXpropPasteClose(void *voidxps)
{
	...
	if (xps->xpsDeleteOnClose) {
		XDeleteProperty(xps->xpsDisplay, xps->xpsWindow,
				xps->xpsProperty);
	}
...
static int sioInXpropReadBytes(void *voidxps, unsigned char *buffer,
			       unsigned int count)
{
	XpropPasteStream *xps = (XpropPasteStream *)voidxps;
	...
	ret = XGetWindowProperty(xps->xpsDisplay, xps->xpsWindow,
				 xps->xpsProperty, xps->xpsOffset / 4,
				 count / 4, False, AnyPropertyType, &typeFound,
				 &formatFound, &itemsReturned, &itemsLeft,
				 &dataReturned);
...
SimpleInputStream *sioInOpenPaste(APP_WIDGET w, APP_EVENT *event)
{
...
static int sioOutCopyWriteBytes(void *voidxcs, const unsigned char *buffer,
				int count)
{
	XpropCopyStream *xcs = (XpropCopyStream *)voidxcs;

	XChangeProperty(xcs->xcsDisplay, xcs->xcsRequestor, xcs->xcsProperty,
			xcs->xcsTarget, 8, xcs->xcsPropMode, buffer, count);
...
SimpleOutputStream *sioOutOpenCopy(APP_WIDGET w, APP_EVENT *event)
{
	XSelectionEvent *selEvent = &(event->xselection);
