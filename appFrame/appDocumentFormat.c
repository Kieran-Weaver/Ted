...
int appDocumentTestCanSave(EditApplication *ea, const AppFileExtension *afe,
			   void *privateData, unsigned int useFlags, int format)
{
	if ((afe->afeUseFlags & useFlags) != useFlags) {
		return -1;
	}

	if (privateData && ea->eaCanSaveDocument &&
			     ...
			     unsigned int useFlags, int format)
{
	...
	if (utilMemoryBufferIsEmpty(&ext) && filename->mbSize >= 3 &&
	    filename->mbBytes[0] == '-' &&
	    filename->mbBytes[filename->mbSize - 1] == '-') {
		if (utilMemoryBufferGetRange(&ext, filename, 1,
					     filename->mbSize - 2)) {
	...
	if (format >= 0 && format < ea->eaFileExtensionCount) {
		...
		if (!appDocumentTestCanSave(ea, afe, privateData, useFlags,
					    format)) {
			if (utilMemoryBufferIsEmpty(&ext) ||
			    !afe->afeExtension ||
			    utilMemoryBufferEqualsString(&ext,
							 afe->afeExtension)) {
				rval = format;
				goto ready;
			}
	...
	for (i = 0; i < ea->eaFileExtensionCount; afe++, i++) {
		...
		if (utilMemoryBufferIsEmpty(&ext) || !afe->afeExtension ||
		   ...
		   const MemoryBuffer *toName)
{
	...
	if ((*ea->eaOpenDocument)(ea, privateData, &fromFormat,
				  ...
				  readOnly, suggestStdin, fromFormat,
				  fromName)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	toFormat = appDocumentGetSaveFormat(&suggestStdout, ea, toName,
					    privateData, APPFILE_CAN_SAVE,
					    fromFormat);
	if (toFormat < 0) {
		LDEB(toFormat);
		rval = -1;
				 ...
				 int extensionCount)
{
	...
	acr = (AppConfigurableResource *)malloc(
		extensionCount * sizeof(AppConfigurableResource));
	if (!acr) {
		LXDEB(extensionCount, acr);
		return -1;
	}
