...
static int appDocMakeMainWindow(EditDocument *ed)
{
	...
	gtk_window_set_position(GTK_WINDOW(ed->edToplevel.atTopWidget),
				GTK_WIN_POS_CENTER);
	gtk_window_set_policy(GTK_WINDOW(ed->edToplevel.atTopWidget), TRUE,
			      TRUE, FALSE);

	gtk_widget_realize(ed->edToplevel.atTopWidget);

	gdk_window_set_icon(ed->edToplevel.atTopWidget->window,
			    ed->edToplevel.atTopWidget->window, pixmap, mask);
	...
	gtk_box_pack_start(GTK_BOX(ed->edMainWindow), ed->edMenuBar, FALSE,
			   TRUE, 0);
...
static int appDocMakeScrolledWindow(EditDocument *ed)
{
	...
	gtk_box_pack_start(GTK_BOX(ed->edMainWindow), ed->edScrolledWindow,
			   TRUE, TRUE, 0);
...
int appFinishDocumentWindow(EditDocument *ed)
{
	...
	gtk_signal_connect(GTK_OBJECT(dw->dwWidget), "configure_event",
			   (GtkSignalFunc)appDocConfigure, (void *)ed);

	gtk_signal_connect(GTK_OBJECT(ed->edToplevel.atTopWidget), "map_event",
			   (GtkSignalFunc)appDocTopConfigure, (void *)ed);

	gtk_signal_connect(GTK_OBJECT(dw->dwWidget), "selection_clear_event",
			   (GtkSignalFunc)appDocCopyPasteHandler, (void *)ed);

	gtk_signal_connect(GTK_OBJECT(dw->dwWidget), "selection_received",
			   (GtkSignalFunc)appDocGotPasteReplyGtk, (void *)ed);

	gtk_signal_connect(GTK_OBJECT(dw->dwWidget), "selection_get",
			   (GtkSignalFunc)appDocReplyToCopyRequest, (void *)ed);
	...
	while (gl) {
		...
		if (voided == (void *)ed) {
			appGuiSetToggleItemLabel(
				child, utilMemoryBufferGetString(title));
		}
