					       ...
					       int high)
{
	...
	if (!ds->dsAvoidFontconfig) {
		ds->dsXftDrawable = appMotifXftDrawCreate(
			ds->dsDisplay, ds->dsScreen, ds->dsDrawable,
			&(ds->dsXftColorList));
		if (!ds->dsXftDrawable) {
...
DrawingSurface drawMakeDrawingSurfaceForImageAndParent(
	...
	const DocumentRectangle *drSrc, int pixelsWide, int pixelsHigh)
{
	DrawingSurface ds;
	...
	if (drawUtilMotifMakeImage(ds->dsDisplay, ds->dsScreen, &xim,
				   pixelsWide, pixelsHigh, ds->dsColors, abi,
				   drSrc)) {
		drawFreeDrawingSurface(ds);
		return (DrawingSurface)0;
	}
		    ...
		    const RasterImage *abi, const DocumentRectangle *drSrc)
{
	...
	XPutImage(ds->dsDisplay, ds->dsDrawable, ds->dsGc, xim, 0, 0,
		  drDest->drX0, drDest->drY0, pixelsWide, pixelsHigh);
...
int drawSetForegroundColor(DrawingSurface ds, const RGB8Color *rgb8)
{
	...
	if (appColorRgb(&xc, ds->dsColors, rgb8->rgb8Red, rgb8->rgb8Green,
			rgb8->rgb8Blue)) {
		LDEB(1);
		return -1;
	}
...
void drawFillRectangle(DrawingSurface ds, const DocumentRectangle *dr)
{
	...
	XFillRectangle(ds->dsDisplay, ds->dsDrawable, ds->dsGc, dr->drX0,
		       dr->drY0, dr->drX1 - dr->drX0 + 1,
		       dr->drY1 - dr->drY0 + 1);
}

void drawRectangle(DrawingSurface ds, const DocumentRectangle *dr)
{
	XDrawRectangle(ds->dsDisplay, ds->dsDrawable, ds->dsGc, dr->drX0,
		       dr->drY0, dr->drX1 - dr->drX0 + 1,
		       dr->drY1 - dr->drY0 + 1);
}
			 ...
			 int pointCount)
{
	...
	if (ds->dsPointCount < pointCount) {
		APP_POINT *fresh = (APP_POINT *)realloc(
			ds->dsPoints, pointCount * sizeof(APP_POINT));
		if (!fresh) {
			LXDEB(pointCount, fresh);
			return -1;
		}
...
int drawFillPolygon(DrawingSurface ds, const Point2DI *points, int pointCount)
{
	...
	XFillPolygon(ds->dsDisplay, ds->dsDrawable, ds->dsGc, ds->dsPoints,
...
static const int CapStyleMap[LineCap_Count] = { CapNotLast, CapButt, CapRound,
						CapProjecting };

static const int JoinStyleMap[LineJoin_Count] = { JoinMiter, JoinRound,
						  JoinBevel };
			  ...
			  const unsigned char *dashList, int dashCount)
{
	XSetLineAttributes(ds->dsDisplay, ds->dsGc, lineWidth,
			   LineStyleMap[lineStyle], CapStyleMap[capStyle],
	...
	if (lineStyle != LineStyleSolid) {
		if (!dashList || dashCount == 0) {
			...
			XSetDashes(ds->dsDisplay, ds->dsGc, 0, (char *)dashList,
				   dashCount);
		}
	}
...
{
	XDrawArc(ds->dsDisplay, ds->dsDrawable, ds->dsGc, arc->a2diRect.drX0,
		 arc->a2diRect.drY0,
...
{
	XFillArc(ds->dsDisplay, ds->dsDrawable, ds->dsGc, arc->a2diRect.drX0,
		 arc->a2diRect.drY0,
		 arc->a2diRect.drX1 - arc->a2diRect.drX0 + 1,
		 arc->a2diRect.drY1 - arc->a2diRect.drY0 + 1,
		 arc->a2diAngleFrom, arc->a2diAngleStep);
...
void drawSetClipRect(DrawingSurface ds, const DocumentRectangle *drClip)
{
	...
	if (xRect.width < 1 || xRect.height < 1) {
		LLDEB(xRect.width, xRect.height);
		return;
	}
...
{
	if (drChild->drX1 >= drChild->drX0 && drChild->drY1 >= drChild->drY0) {
		XCopyArea(ds->dsDisplay, child->dsDrawable, ds->dsDrawable,
			  ds->dsGc, drChild->drX0, drChild->drY0,
			  drChild->drX1 - drChild->drX0 + 1,
			  drChild->drY1 - drChild->drY0 + 1, xDest, yDest);
	}
		  ...
		  const DocumentRectangle *drSrc)
{
	XCopyArea(ds->dsDisplay, ds->dsDrawable, ds->dsDrawable, ds->dsGc,
		  drSrc->drX0, drSrc->drY0, drSrc->drX1 - drSrc->drX0 + 1,
		  drSrc->drY1 - drSrc->drY0 + 1, xDest, yDest);
}
