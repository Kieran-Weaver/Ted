			...
			const char *labelText)
{
	...
	XtVaGetValues(hbox, XmNchildren, &children, XmNnumChildren, &childCount,
		      NULL);
	...
	if (childCount == 0) {
		...
		XtSetArg(al[ac], XmNleftOffset, 0);
		ac++;
		XtSetArg(al[ac], XmNleftWidget, children[childCount - 1]);
		ac++;
	}
...
void appMakeLabelInColumn(APP_WIDGET *pLabel, APP_WIDGET column,
			  const char *labelText)
{
	Widget label;
...
void appGuiChangeLabelText(APP_WIDGET labelWidget, const char *label)
{
	XmString labelString;

	labelString = XmStringCreateLocalized((char *)label);

	XtVaSetValues(labelWidget, XmNlabelString, labelString, NULL);
