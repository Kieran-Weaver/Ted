...
static void appDocScrollVertically(EditDocument *ed, int scrolledY)
{
	...
	struct DrawingSurface *ds = ed->edDrawingSurface;
	...
	if (ed->edLeftRuler && ea->eaScrollLeftRuler) {
		(*ea->eaScrollLeftRuler)(ed->edLeftRuler, ed->edLeftRulerWidget,
					 scrolledY);
	}

	if (ed->edRightRuler && ea->eaScrollRightRuler) {
		(*ea->eaScrollRightRuler)(ed->edRightRuler,
					  ed->edRightRulerWidget, scrolledY);
	}
...
static void appDocScrollHorizontally(EditDocument *ed, int scrolledX)
{
	...
	if (ed->edTopRuler && ea->eaScrollTopRuler) {
		(*ea->eaScrollTopRuler)(ed->edTopRuler, ed->edTopRulerWidget,
					scrolledX);
	}

	if (ed->edBottomRuler && ea->eaScrollBottomRuler) {
		(*ea->eaScrollBottomRuler)(ed->edBottomRuler,
					   ed->edBottomRulerWidget, scrolledX);
	}
			  ...
			  int *pScrolledX, int *pScrolledY)
{
	...
	if (noy != ooy) {
		appGuiSetScrollbarValues(ed->edVerticalScrollbar, noy,
					 sliderSize);
		changed = 1;
	}
    ...
    */
	...
	if (nox != oox) {
		appGuiSetScrollbarValues(ed->edHorizontalScrollbar, nox,
					 sliderSize);
		changed = 1;
	}
...
static void appFileAdaptHorizontalRulerRange(EditDocument *ed, int wide)
{
	EditApplication *ea = ed->edApplication;

	if (ed->edTopRuler && ea->eaSetTopRulerRange) {
		(*ea->eaSetTopRulerRange)(ed->edTopRuler, ed->edTopRulerWidget,
					  ed->edFullRect.drX1,
...
static void appFileAdaptVerticalRulerRange(EditDocument *ed, int high)
{
	EditApplication *ea = ed->edApplication;
...
static void appDocAdaptToWidgetSize(EditDocument *ed, int wide, int high)
{
	int changed = 0;
	int d;
...
void appDocumentRulerWidth(EditApplication *ea, EditDocument *ed)
{
	...
	/*  1  */
	ed->edLeftRulerWidePixels = (int)(ea->eaLeftRulerWidthMM * pixPerMM);
	ed->edTopRulerHighPixels = (int)(ea->eaTopRulerHeightMM * pixPerMM);
	ed->edRightRulerWidePixels = (int)(ea->eaRightRulerWidthMM * pixPerMM);
	...
	if (mult) {
		ed->edBottomRulerHighPixels =
			mult *
			((ed->edBottomRulerHighPixels + mult - 1) / mult);
	}
