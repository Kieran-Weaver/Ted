...
{
	appLengthToTextWidget(amt->amtLeftMarginText, dg->dgLeftMarginTwips,
			      unitInt);

	appLengthToTextWidget(amt->amtTopMarginText, dg->dgTopMarginTwips,
			      unitInt);

	appLengthToTextWidget(amt->amtRightMarginText, dg->dgRightMarginTwips,
			      unitInt);

	appLengthToTextWidget(amt->amtBottomMarginText, dg->dgBottomMarginTwips,
			      unitInt);
			    ...
			    DocumentGeometry *dg)
{
	...
	if (PROPmaskISSET(chgMask, DGpropLEFT_MARGIN)) {
		if (appGetLengthFromTextWidget(
			    amt->amtLeftMarginText, &(dg->dgLeftMarginTwips),
			    &changed, unitType, minValue, adaptToMin, maxValue,
			    adaptToMax)) {
			return -1;
		}
		if (changed) {
	...
	if (PROPmaskISSET(chgMask, DGpropTOP_MARGIN)) {
		if (appGetLengthFromTextWidget(
			    amt->amtTopMarginText, &(dg->dgTopMarginTwips),
			    &changed, unitType, minValue, adaptToMin, maxValue,
			    adaptToMax)) {
			return -1;
		}
	...
	if (PROPmaskISSET(chgMask, DGpropRIGHT_MARGIN)) {
		if (appGetLengthFromTextWidget(
			    amt->amtRightMarginText, &(dg->dgRightMarginTwips),
			    &changed, unitType, minValue, adaptToMin, maxValue,
			    adaptToMax)) {
			return -1;
		}
...
void appMakeMarginToolWidgets(APP_WIDGET parent, const MarginToolResources *mtr,
			      AppMarginTool *amt,
			      APP_TXACTIVATE_CALLBACK_T gotValue, void *through)
{
	APP_WIDGET label;
	APP_WIDGET row;
	...
	guiToolMakeLabelAndTextRow(&row, &label, &(amt->amtBottomMarginText),
				   amt->amtRightColumn,
				   mtr->mtrBottomMarginText, textWidth,
				   textEnabled);
