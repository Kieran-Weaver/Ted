...
static int appColorRgb111(AllocatorColor *ac, ColorAllocator *ca,
...
static int appColorRgb222(AllocatorColor *ac, ColorAllocator *ca,
			  unsigned int r, unsigned int g, unsigned int b)
{
	AllocatorColor acScratch;
	AllocatorColor *acRes;
...
static int appColorRgb555(AllocatorColor *ac, ColorAllocator *ca,
			  unsigned int r, unsigned int g, unsigned int b)
{
	AllocatorColor acScratch;
	AllocatorColor *acRes;
...
static int appColorRgb332(AllocatorColor *ac, ColorAllocator *ca,
			  unsigned int r, unsigned int g, unsigned int b)
{
	AllocatorColor acScratch;
	AllocatorColor *acRes;
...
int appColorRgb(XColor *xc, AppColors *acSys, unsigned int r, unsigned int g,
		unsigned int b)
{
	AllocatorColor ac;
...
int appColorNamed(XColor *xc, AppColors *acSys, const char *name)
{
	XColor exact;
	XColor screen;

	if (!XLookupColor(acSys->acDisplay, acSys->acColormap, name, &exact,
			  &screen)) {
		SDEB(name);
		return -1;
	}

	return appColorRgb(xc, acSys, exact.red / 257, exact.green / 257,
			   exact.blue / 257);
}
...
static int appPixCmp(const void *vpix1, const void *vpix2)
{
...
static void appTry222Color(Display *display, Colormap cmap, ColorAllocator *ca,
			   AppColors *acSys, double dists[64], Pixel l)
{
	XColor xc;
	int col222;
	...
	col222 = C222((int)(xc.red / 256), (int)(xc.green / 256),
		      (int)(xc.blue / 256));
	...
	if (ca->ca222Colors[col222].acAllocated == AC_ALLOCATED) {
		...
		c = xc.red - (256 * 256 - 1) * (((col222 & 0x30) >> 4) / 3);
		d += c * c;
		c = xc.green - (256 * 256 - 1) * (((col222 & 0x0c) >> 2) / 3);
		d += c * c;
		c = xc.blue - (256 * 256 - 1) * (((col222 & 0x03) >> 0) / 3);
		d += c * c;
	...
	if (XAllocColor(display, cmap, &xc)) {
		if (xc.pixel == l) {
			if (ca->ca222Colors[col222].acAllocated ==
			    AC_UNALLOCATED) {
				ca->ca222Colors[col222].acAllocated =
					AC_ALLOCATED;
				ca->ca222Colors[col222].acRed = xc.red;
				ca->ca222Colors[col222].acGreen = xc.green;
				ca->ca222Colors[col222].acBlue = xc.blue;
				ca->ca222Colors[col222].acColorNumber =
					xc.pixel;
			} else {
				XFreeColors(display, cmap, &(xc.pixel), 1, 0L);
			}
		} else {
			XFreeColors(display, cmap, &(xc.pixel), 1, 0L);
		}
...
static int appAllocateColors(Display *display, AppColors *acSys)
{
	...
	case GrayScale:
		...
		case 8:
			break;
		default:
			SLDEB(APP_VisualClasses[acSys->acVisualClass],
			      ca->caDepth);
		}
		break;
	...
	while (l < m) {
		if (!XAllocColorCells(display, cmap, False, (unsigned long *)0,
				      0, pxx, (unsigned)m)) {
			r = m;
		} else {
...
int appColorFindRgb(XColor *xc, AppColors *acSys, unsigned int r,
