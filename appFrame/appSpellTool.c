...
static void appSpellToolSomethingFound(SpellTool *ast, int yes_no)
{
...
static void appSpellToolGotAlternative(SpellTool *ast, int gotAlternative)
{
...
static void appSpellToolComplain(int error, const char *subject, void *voidast)
{
	...
	if (subject) {
		appQuestionRunErrorDialog(ast->astApplication, topWidget,
					  (APP_WIDGET)0, message);
	} else {
		appQuestionRunSubjectErrorDialog(ast->astApplication, topWidget,
						 (APP_WIDGET)0, subject,
						 message);
	}
...
static void appSpellMakeGuesses(void *voidast, const char *word)
{
	SpellTool *ast = (SpellTool *)voidast;
	SpellChecker *sc;
	SpellDictionary *sd;
	...
	if (ast->astCurrentDictionary < 0) {
		LDEB(ast->astCurrentDictionary);
		return;
	}
	if (!(sc = ast->astSpellChecker)) {
		XDEB(ast->astSpellChecker);
		return;
	}
	if (!sc->scGuessWord) {
		XDEB(sc->scGuessWord);
	...
	count = (*sc->scGuessWord)(&igl, sd, appSpellToolComplain, (void *)ast,
				   sc, word);
	if (count < 0) {
		LDEB(count);
		return;
...
static void appSpellToolFindNext(SpellTool *ast)
{
	...
	if (ast->astCurrentDictionary < 0 ||
		...
		LLDEB(ast->astCurrentDictionary, sc->scDictionaryCount);
		return;
	}
...
static APP_BUTTON_CALLBACK_H(appSpellToolCorrect, w, voidast)
{
	...
	if (!sc->scCorrect || ast->astReadOnly) {
		XLDEB(sc->scCorrect, ast->astReadOnly);
		return;
	}
...
static void appSpellDictionaryChosen(int dict, void *voidast)
{
	SpellTool *ast = (SpellTool *)voidast;
	SpellChecker *sc;
	...
	if (dict < 0 || dict >= sc->scDictionaryCount) {
		LLDEB(dict, sc->scDictionaryCount);
		return;
	}
					      ...
					      SpellTool *ast)
{
	...
	appMakeColumnFrameInColumn(&frame, &paned, parent,
				   astr->astrDictionary);

	appMakeOptionmenuInColumn(&(ast->astDictionaryOptionmenu), paned,
				  appSpellDictionaryChosen, (void *)ast);

	guiToolMake2BottonRow(&buttonRow, paned, &(ast->astLearnButton),
			      &(ast->astForgetButton), astr->astrLearn,
			      astr->astrForget, appSpellToolLearnPushed,
			      appSpellToolForgetPushed, (void *)ast);
				       ...
				       const SpellToolResources *astr)
{
	...
	for (dict = 0; dict < sc->scDictionaryCount; sd++, dict++) {
		appAddItemToOptionmenu(&(ast->astDictionaryOptionmenu),
				       sd->sdLocaleLabel);

		if (ast->astApplication && ast->astApplication->eaLocaleName &&
				    ...
				    SpellTool *ast)
{
	...
	appGuiMakeListInColumn(&list, parent, visibleItems, appSpellGuessChosen,
				   ...
				   SpellTool *ast)
{
	...
	guiToolMake2BottonRow(&buttonRow, spellForm, &(ast->astIgnoreButton),
			      &(ast->astFindNextButton), astr->astrIgnore,
			      astr->astrFindNext, appSpellToolIgnorePushed,
			      appSpellToolFindNextPushed, (void *)ast);

	guiToolMake2BottonRow(&buttonRow, spellForm, &(ast->astGuessButton),
			      &(ast->astCorrectButton), astr->astrGuess,
			      astr->astrCorrect, appSpellGuessButtonPushed,
			      appSpellToolCorrect, (void *)ast);
...
static int appSpellGetLocaleNames(EditApplication *ea, SpellChecker *sc)
{
	static int gotResources;

	static LocaleName *localeNames = (LocaleName *)0;
	static int localeNameCount;
	static AppConfigurableResource *localeRes =
		(AppConfigurableResource *)0;
	...
	if (!gotResources) {
		...
		for (i = 0; i < sc->scDictionaryCount; i++) {
			...
			localeNames[i].lnLocale =
				strdup(sc->scDictionaries[i].sdLocale);
			if (!localeNames[i].lnLocale) {
				XDEB(localeNames[i].lnLocale);
				return -1;
			}
	...
	for (i = 0; i < sc->scDictionaryCount; i++) {
		...
		for (j = 0; j < localeNameCount; j++) {
			...
			if (!strcmp(localeNames[n].lnLocale,
				    sc->scDictionaries[i].sdLocale)) {
				indSpellDictionarySetLocaleLabel(
					&(sc->scDictionaries[i]),
...
static AppConfigurableResource APP_SpellToolResourceTable[] = {
	...
	APP_RESOURCE("spellToolDictTitle",
		     ...
		     "Dictionary"),
	APP_RESOURCE("spellToolLearn", offsetof(SpellToolResources, astrLearn),
		     "Learn"),
	APP_RESOURCE("spellToolForget",
		     offsetof(SpellToolResources, astrForget), "Forget"),
	APP_RESOURCE("spellToolGuesses",
		     offsetof(SpellToolResources, astrGuesses), "Guesses"),
	APP_RESOURCE("spellToolIgnore",
		     offsetof(SpellToolResources, astrIgnore), "Ignore"),
	APP_RESOURCE("spellToolFindNext",
		     offsetof(SpellToolResources, astrFindNext), "Find Next"),
	APP_RESOURCE("spellToolGuess", offsetof(SpellToolResources, astrGuess),
		     "Guess"),
	APP_RESOURCE("spellToolCorrect",
		     offsetof(SpellToolResources, astrCorrect), "Correct"),
	APP_RESOURCE("spellToolNoDicts",
		     offsetof(SpellToolResources, astrNoDicts), "None"),
