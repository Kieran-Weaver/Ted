		...
		while (gl) {
			...
			if (GTK_IS_MENU_ITEM(c)) {
				...
				if (GTK_IS_CONTAINER(c)) {
					rval = appGuiGetOptionmenuFindItem(c,
									   it);
...
static void appOptionMenuGtkCallback(GtkMenuItem *it, void *voidaom)
{
...
static void appComboBoxGtkCallback(GtkComboBox *combo, void *voidaom)
{
	AppOptionmenu *aom = (AppOptionmenu *)voidaom;
	int idx;
...
APP_WIDGET appAddItemToOptionmenu(AppOptionmenu *aom, const char *label)
{
	...
	if (GTK_IS_COMBO_BOX(aom->aomInplace)) {
		...
		gtk_combo_box_append_text(GTK_COMBO_BOX(aom->aomInplace),
					  label);
	}
	...
	if (aom->aomCallback) {
		gtk_signal_connect(GTK_OBJECT(option), "activate",
				   (GtkSignalFunc)appOptionMenuGtkCallback,
				   aom);
	}
...
void appSetOptionmenu(AppOptionmenu *aom, int num)
{
	if (GTK_IS_OPTION_MENU(aom->aomInplace)) {
		gtk_option_menu_set_history(GTK_OPTION_MENU(aom->aomInplace),
					    num);
	}
