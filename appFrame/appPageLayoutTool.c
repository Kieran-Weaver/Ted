...
static void appPageLayoutChangeLayout(PageLayoutTool *plt, int wholeDocument)
{
	EditApplication *ea = plt->pltApplication;
	...
	if (appMarginToolGetMargins(&margUpdMask, &margChgMask,
				    plt->pltUnitType, &(plt->pltMarginTool),
				    &dgNew)) {
		return;
	}
...
static void appSectHeaderFooterPosChanged(PageLayoutTool *plt, APP_WIDGET w)
{
	EditApplication *ea = plt->pltApplication;
...
static APP_TXACTIVATE_CALLBACK_H(appPaperMarginChanged, w, voidplt)
{
	...
	if (appMarginToolGetMargins(&dgUpdMask, &dgChgMask, plt->pltUnitType,
				    &(plt->pltMarginTool), &dg)) {
		return;
	}
...
{
	PageLayoutTool *plt = (PageLayoutTool *)voidplt;
	struct DrawingSurface *ds = plt->pltDrawingSurface;
	DocumentGeometry *dg = &(plt->pltGeometryChosen);
...
static void appPageLayoutToolPaperRectChanged(PaperChooser *pc, void *voidplt,
...
static void
appPageLayoutMakeHeaderFooterWidgets(APP_WIDGET parent,
				     ...
				     PageLayoutTool *plt)
{
	...
	appMakeRowFrameInColumn(&(plt->pltHeaderFooterFrame),
				&(plt->pltHeaderFooterInside), parent, 2,
				plpr->plprPositionsText);
			       ...
			       const InspectorSubjectResources *isr)
{
	...
	if (plt->pltUnitType < 0) {
		SDEB(plpr->plprUnitType);
		plt->pltUnitType = UNITtyINCH;
	}
	...
	plt->pltPixelsPerTwip =
		(plt->pltPageHighMm * ea->eaPixelsPerTwip) / A3_MM_HIGH;

	plt->pltPageDrawing = appMakePageDrawing(
		pageWidget, ea->eaPixelsPerTwip, plt->pltPageHighMm,
		appPageLayoutToolDrawPage, (void *)plt);

	appMakePaperChooserWidgets(pageWidget, plpr->plprPage, plt->pltUnitType,
				   &(plt->pltPaperChooser),
	...
	appMakeMarginToolWidgets(pageWidget, &(plpr->plprMarginToolResources),
				 &(plt->pltMarginTool), appPaperMarginChanged,
				 (void *)plt);
	...
	guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
			      ...
			      appPageLayoutChangeDocumentPushed, (void *)plt);
...
static AppConfigurableResource APP_PageToolResourceTable[] = {
	...
	APP_RESOURCE("tableToolChangeSelection",
		     ...
		     "Apply to Selection"),
	/**/
	APP_RESOURCE("unit", offsetof(PageLayoutPageResources, plprUnitType),
		     "inch"),
	APP_RESOURCE("formatToolPageLayoutPage",
		     offsetof(PageLayoutPageResources, plprPage), "Page"),
