		    ...
		    const RasterImage *abi, const DocumentRectangle *drSrc)
{
	...
	if (drawUtilGtkMakeImage(&xim, pixelsWide, pixelsHigh, ds->dsColors,
				 abi, drSrc)) {
		LLDEB(pixelsWide, pixelsHigh);
		return -1;
	}

	gdk_draw_image(ds->dsDrawable, ds->dsGc, xim, 0, 0, drDest->drX0,
		       drDest->drY0, pixelsWide, pixelsHigh);
...
void drawFillRectangle(DrawingSurface ds, const DocumentRectangle *dr)
{
	...
	gdk_draw_rectangle(ds->dsDrawable, ds->dsGc, TRUE, dr->drX0, dr->drY0,
	      ...
	      int close)
{
	...
	for (i = 0; i < pointCount; i++) {
		gdk_draw_line(ds->dsDrawable, ds->dsGc, ds->dsPoints[i + 0].x,
			      ds->dsPoints[i + 0].y, ds->dsPoints[i + 1].x,
			      ds->dsPoints[i + 1].y);
	}

	if (close) {
		gdk_draw_line(ds->dsDrawable, ds->dsGc, ds->dsPoints[i].x,
			      ds->dsPoints[i].y, ds->dsPoints[0].x,
			      ds->dsPoints[0].y);
	}
...
int drawFillPolygon(DrawingSurface ds, const Point2DI *points, int pointCount)
{
	...
	gdk_draw_polygon(ds->dsDrawable, ds->dsGc, TRUE, ds->dsPoints,
			 pointCount);
			  ...
			  const unsigned char *dashList, int dashCount)
{
	...
	if (lineStyle != LineStyleSolid) {
		if (!dashList || dashCount == 0) {
			...
			gdk_gc_set_dashes(ds->dsGc, 0, (gint8 *)dashList,
					  dashCount);
		}
	}
