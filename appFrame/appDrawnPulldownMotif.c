...
static APP_EVENT_HANDLER_H(appDrawnPulldownPulldown, w, voidadp, mouseEvent)
{
	...
	XtTranslateCoords(adp->adpInplaceDrawing, mouseX, mouseY, &screenX,
			  &screenY);
	...
	XtTranslateCoords(adp->adpInplaceDrawing, inplaceX, inplaceY, &screenX,
			  &screenY);

	ac = 0;
	XtSetArg(al[ac], XmNx, screenX);
	ac++;
	XtSetArg(al[ac], XmNy, screenY);
	ac++;
...
static APP_EVENT_HANDLER_H(appDrawnPulldownMouseUpDown, w, voidadp, mouseEvent)
{
	...
	/*  3  */
	if (upDown < 0 && adp->adpMouseX == screenX &&
	    adp->adpMouseY == screenY) {
		return;
	}

	/*  4  */
	XtVaGetValues(w, XmNwidth, &wide, XmNheight, &high, NULL);
	...
	if (adp->adpClickHandler) {
		(*adp->adpClickHandler)(w, adp->adpThrough, mouseEvent,
					pRefused);
	}
				   ...
				   void *through)
{
	...
	if (redrawInplace) {
		XtAddEventHandler(adp->adpInplaceDrawing, ExposureMask, False,
	...
	ac = 0;
	XtSetArg(al[ac], XmNmwmDecorations, 0);
	ac++;
	XtSetArg(al[ac], XmNmwmFunctions, 0);
	ac++;
	XtSetArg(al[ac], XmNoverrideRedirect, True);
	ac++;
	XtSetArg(al[ac], XmNallowShellResize, True);
	ac++;

	adp->adpPulldownShell =
		XtCreatePopupShell(WIDGET_NAME, transientShellWidgetClass,
	...
	adp->adpPulldownDrawing =
		XmCreateDrawingArea(adp->adpPulldownShell, WIDGET_NAME, al, ac);

	if (redrawPulldown) {
		XtAddEventHandler(adp->adpPulldownDrawing, ExposureMask, False,
	...
	XtAddEventHandler(adp->adpPulldownShell,
			  ButtonReleaseMask | ButtonPressMask, False,
			  appDrawnPulldownMouseUpDown, (void *)adp);

	XtAddEventHandler(adp->adpPulldownShell, FocusChangeMask, False,
			  appDrawnPulldownFocusChange, (void *)adp);

	XtAddEventHandler(adp->adpInplaceDrawing, ButtonPressMask, False,
			  appDrawnPulldownPulldown, (void *)adp);
				  ...
				  APP_WIDGET column, void *through)
{
	...
	adp->adpInplaceDrawing =
		XmCreateDrawingArea(column, WIDGET_NAME, al, ac);
...
void appGuiEnableDrawnPulldown(AppDrawnPulldown *adp, int enabled)
{
...
{
	XtVaSetValues(adp->adpPulldownShell, XmNheight, height, NULL);
	return;
}
...
void appGuiSetDrawnPulldownStrips(AppDrawnPulldown *adp, int strips)
{
	...
	XtVaGetValues(adp->adpInplaceDrawing, XmNwidth, &inplaceWidth,
		      XmNheight, &inplaceHeight, NULL);

	XtVaSetValues(adp->adpPulldownShell, XmNwidth, inplaceWidth, XmNheight,
		      strips * inplaceHeight, NULL);
	return;
}

int appGuiDrawnPulldownGetStrip(int *pStrip, const AppDrawnPulldown *adp,
				const APP_WIDGET w, const APP_EVENT *mouseEvent)
{
	int mouseX;
	int mouseY;
	...
	XtVaGetValues(adp->adpInplaceDrawing, XmNheight, &inplaceHeight,
		      XmNwidth, &inplaceWidth, NULL);
