...
static int appMetaCleanObjectX11(DeviceContext *dc, void *through,
				 MetaFileObject *mfo)
{
	PatternBrush *pb;
	LogicalBrush *lb;
...
static int
appMetaInitDeviceContextX11(DeviceContextX11 *dcx, DeviceContext *dc,
			    DrawingSurface ds, const MetafilePlayer *player,
			    int objectCount, const DocumentRectangle *drLogical,
			    int mapMode, int twipsWide, int twipsHigh,
			    int pixelsWide, int pixelsHigh)
{
	if (appMetaInitDeviceContext(dc, player, objectCount, drLogical,
				     mapMode, pixelsWide, pixelsHigh, twipsWide,
				     twipsHigh)) {
		LDEB(objectCount);
		return -1;
	}
...
static int appMetaSetDeviceDefaultsX11(DeviceContextX11 *dcx, DeviceContext *dc)
{
	...
	if (appColorRgb(&(dcx->dcxPenColor), ds->dsColors,
			dc->dcPen.lpColor.rgb8Red, dc->dcPen.lpColor.rgb8Green,
	...
	if (appColorRgb(&(dcx->dcxBkColor), ds->dsColors, dc->dcBkColor.rgb8Red,
			dc->dcBkColor.rgb8Green, dc->dcBkColor.rgb8Blue)) {
		LDEB(1);
		return -1;
	}

	if (appColorRgb(&(dcx->dcxTextColor), ds->dsColors,
			dc->dcTextColor.rgb8Red, dc->dcTextColor.rgb8Green,
...
static int appMetaSelectBrushObjectX11(DeviceContext *dc, void *through,
				       LogicalBrush *lb)
{
	...
	case BS_PATTERN:
		if (!lb->lbTilePixmap) {
			...
			case HS_FDIAGONAL:
				drawLine(lb->lbTilePixmap, 0, 0, wide - 1,
					 high - 1);
				break;

			case HS_BDIAGONAL:
				drawLine(lb->lbTilePixmap, 0, high - 1,
					 wide - 1, 0);
				break;
...
static int appMetaSelectPatternBrushObjectX11(DeviceContext *dc, void *through,
					      PatternBrush *pb)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	DrawingSurface ds = dcx->dcxDrawingSurface;
	...
	if (!pb->pbTilePixmap) {
		...
		pb->pbTilePixmap = drawMakeDrawingSurfaceForImageAndParent(
			ds, abi, (DocumentRectangle *)0, dstXExt, dstYExt);
		if (!pb->pbTilePixmap) {
			XDEB(pb->pbTilePixmap);
			return -1;
		}
	}
...
static int appMetaSelectFontObjectX11(DeviceContext *dc, void *through,
				      LogicalFont *lf)
{
	...
	if (lf->lfPrivateFont < 0) {
		...
		afi = (*player->mpGetFontForAttribute)(
			&unicodesWanted, &(lf->lfTextAttribute),
			&(dc->dcFontList), player->mpPostScriptFontList);
		if (!afi) {
			SXDEB(lf->lfFaceNameUtf8, afi);
			return -1;
		}
		...
		lf->lfPrivateFont =
			drawOpenScreenFont(ds, afi, pixelSize, unicodesWanted);
		if (lf->lfPrivateFont < 0) {
			SLDEB(lf->lfFaceNameUtf8, lf->lfPrivateFont);
			return 0;
		}
	}
...
static void appMetaX11SetWindowsLineStyle(DrawingSurface ds, int *pDraw, int ps,
					  int lpW, int width)
{
	int drawBorders = 0;
...
solid:
		...
		drawSetLineAttributes(ds, width, LineStyleSolid, LineCapButt,
				      LineJoinMiter, (const unsigned char *)0,
				      0);
		break;
	...
	case PS_DASH:
		...
		drawSetLineAttributes(ds, 1, LineStyleSingleDash, LineCapButt,
				      LineJoinMiter, dash, sizeof(dash));
		break;
	...
	case PS_DASHDOT:
		...
		drawSetLineAttributes(ds, 1, LineStyleSingleDash, LineCapButt,
				      LineJoinMiter, dashdot, sizeof(dashdot));
		break;
...
static int appMetaSelectPenObjectX11(DeviceContext *dc, void *through,
				     LogicalPen *lp)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	DrawingSurface ds = dcx->dcxDrawingSurface;
	...
	appMetaX11SetWindowsLineStyle(ds, &(dc->dcDrawBorders), lp->lpStyle,
				      lp->lpWidth, outputWidth);

	if (dc->dcDrawBorders) {
		if (appColorRgb(&dcx->dcxPenColor, ac, lp->lpColor.rgb8Red,
				lp->lpColor.rgb8Green, lp->lpColor.rgb8Blue)) {
			LDEB(1);
			return -1;
		}
...
static int appDrawMetaBitmapImageX11(DeviceContext *dc, void *through,
...
static int appMetaDrawRoundedRectangleX11(DeviceContext *dc, void *through,
					  const DocumentRectangle *dr, int wide,
					  int high, int fillInside,
					  int drawBorder)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	DrawingSurface ds = dcx->dcxDrawingSurface;
...
static void appMetaGetArcX11(DeviceContextX11 *dcx, Arc2DI *arc,
			     const DocumentRectangle *dr, int xs, int ys,
			     int xe, int ye)
...
static int appMetaDrawArcX11(DeviceContext *dc, void *through,
			     const DocumentRectangle *dr, int xs, int ys,
			     int xe, int ye, int fillInside, int drawBorder)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	DrawingSurface ds = dcx->dcxDrawingSurface;
...
static int appMetaDrawChordX11(DeviceContext *dc, void *through,
			       const DocumentRectangle *dr, int xs, int ys,
			       int xe, int ye, int fillInside, int drawBorder)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
...
static int appMetaDrawPieX11(DeviceContext *dc, void *through,
			     const DocumentRectangle *dr, int xs, int ys,
			     int xe, int ye, int fillInside, int drawBorder)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
...
static int appMetaDrawEllipseX11(DeviceContext *dc, void *through,
				 const DocumentRectangle *dr, int fillInside,
				 int drawBorder)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	DrawingSurface ds = dcx->dcxDrawingSurface;
...
static void appMeta_SetFillX11(DeviceContextX11 *dcx, int before)
{
	DrawingSurface ds = dcx->dcxDrawingSurface;
...
static int appMeta_PolyPolygonX11(DeviceContext *dc, void *through,
				  int polyCount, const int *pointCounts,
				  const Point2DI *points, int fillInsides,
				  int drawBorders, int closePath)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	DrawingSurface ds = dcx->dcxDrawingSurface;
	int poly;
...
static int appMeta_PolyBezierX11(DeviceContext *dc, void *through, int count,
				 const Point2DI *points, int startPath,
				 int fillInsides, int drawBorders,
				 int closePath)
{
	return 0;
}

static int appMetaDrawStringX11(DeviceContext *dc, void *through, int x0,
				int y0, const MemoryBuffer *text)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	DrawingSurface ds = dcx->dcxDrawingSurface;
	...
	if (dc->dcFont.lfTextAttribute.taTextIsUnderlined) {
		...
		drawGetUnderlineGeometry(&thick, &y, ds, dcx->dcxScreenFont,
					 y0);
		drUnderline.drY0 = y;
...
static int appMetaPatBltX11(DeviceContext *dc, void *through, long rop,
...
static int appMetaSetPolyFillModeX11(DeviceContext *dc, void *through, int mode)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	DrawingSurface ds = dcx->dcxDrawingSurface;
...
static int appMetaSetTextColorX11(DeviceContext *dc, void *through, int r,
				  int g, int b)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	AppColors *ac = dcx->dcxColors;
...
static int appMetaSetBkColorX11(DeviceContext *dc, void *through, int r, int g,
				int b)
{
	DeviceContextX11 *dcx = (DeviceContextX11 *)through;
	AppColors *ac = dcx->dcxColors;
...
static int appMetaStartX11DeviceContext(DeviceContextX11 *dcx,
					DeviceContext *dc, DrawingSurface ds,
					const MetafilePlayer *player,
					int objectCount,
					const DocumentRectangle *drLogical,
					int mapMode)
{
	appMetaInitDeviceContextX11(dcx, dc, ds, player, objectCount, drLogical,
				    mapMode, player->mpTwipsWide,
				    player->mpTwipsHigh, player->mpPixelsWide,
				    player->mpPixelsHigh);
...
int appMetaPlayWmfX11(void **pPrivate, DrawingSurface ds,
		      const MetafilePlayer *player)
{
	WmfHeader wh;
	...
	*/
	...
	WMFDEB(appDebug(
		...
		drLogical.drX1, drLogical.drY1, player->mpPixelsWide,
		player->mpPixelsHigh, player->mpTwipsWide,
		player->mpTwipsHigh));

	if (appMetaStartX11DeviceContext(&dcx, &dc, ds, player,
					 wh.wh_objectCount, &drLogical,
					 player->mpMapMode)) {
		LDEB(1);
		return -1;
	}
...
int appMetaPlayEmfX11(void **pPrivate, DrawingSurface ds,
		      const MetafilePlayer *player)
{
	EmfHeader eh;
	...
	WMFDEB(appDebug(
		"X11 EMF LOGICAL: [%d..%d x %d..%d] PIXELS: [%d x %d] TWIPS: [%d x %d]\n",
		eh.ehBounds.drX0, eh.ehBounds.drX1, eh.ehBounds.drY0,
		eh.ehBounds.drY1, player->mpPixelsWide, player->mpPixelsHigh,
		player->mpTwipsWide, player->mpTwipsHigh));

	if (appMetaStartX11DeviceContext(&dcx, &dc, ds, player,
					 eh.eh_numOfHandles, &(eh.ehBounds),
					 MM_ANISOTROPIC)) {
		LDEB(1);
		return -1;
	}
...
int appMacPictPlayFileX11(void **pPrivate, DrawingSurface ds,
			  const MetafilePlayer *player)
{
	MacPictHeader mph;
	...
	WMFDEB(appDebug(
		...
		player->mpXWinExt, player->mpYWinExt, player->mpPixelsWide,
		player->mpPixelsHigh, player->mpTwipsWide,
		player->mpTwipsHigh));
