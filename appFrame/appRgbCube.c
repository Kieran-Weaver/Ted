...
typedef void (*HANDLE_FACE)(RgbCube *rc, RgbColorBlock *rcb, void *through,
			    int square, double scale, const double xo[8],
...
static void appRgbCubeGeometry(double *pScale, int *pSquare, const RgbCube *rc,
			       int wide, int high)
{
	int drsz;
	int square;
...
static void appTryRgbColorFace(RgbCube *rc, RgbColorBlock *rcb, void *through,
			       int square, double scale, const double xo[8],
			       const double yo[8], int canSplit,
			       const int nodes[4])
{
...
static void appDrawRgbColorFace(RgbCube *rc, RgbColorBlock *rcb, void *through,
				int square, double scale, const double xo[8],
				const double yo[8], int canSplit,
				const int nodes[4])
{
	...
	drawSetLineAttributes(ds, linewidth, LineStyleSolid, LineCapButt,
...
static void appHandleRgbColorBlock(double k, int square, double scale,
				   RgbCube *rc, void *through,
				   HANDLE_FACE handleFace, RgbColorBlock *rcb)
{
	/*
	  6----7
	 /:   /|
    ...
    */
	...
	for (i = 0; i < 6; i++) {
		...
		faces[i].bfSplitCandidate = faces[i].bfOnOutside &&
	...
	if (rc->rcSplitColor == RCsplitGREEN) {
		...
		/*  top  */
		if (rcb->rcbRgbColor.rgb8Green ==
		    rc->rcBSplitValues.rgb8Green) {
	...
	*/
		...
		if (nz > 0) {
			(*handleFace)(rc, rcb, through, square, scale, xo, yo,
		      ...
		      const DocumentRectangle *drClip, DrawingSurface ds)
{
	...
	for (i = 0; i < rc->rcColorBlockCount; rcb++, i++) {
		appHandleRgbColorBlock(k, square, scale, rc, (void *)ds,
				       appDrawRgbColorFace, rcb);
	}
			...
			int mouseX, int mouseY, int wide, int high)
{
	...
	rcb = rc->rcColorBlocks;
	for (i = 0; i < rc->rcColorBlockCount; rcb++, i++) {
		appHandleRgbColorBlock(k, square, scale, rc, (void *)&fc,
				       appTryRgbColorFace, rcb);
	}
...
static void appRgbColorBlockSetC(double *pC0, double *pC1, int c, int split,
				 int splitValue, int step)
{
...
static int appRgbCubeZCmp(const void *vrcb1, const void *vrcb2)
{
...
static void appRgbCubeCalculateBlocks(RgbCube *rc)
{
	...
	qsort(rc->rcColorBlocks, rc->rcColorBlockCount, sizeof(RgbColorBlock),
	      appRgbCubeZCmp);
			...
			const RGB8Color *splitValues)
{
	...
	case RCsplitRED:
		...
		rc->rcBSplitValues.rgb8Red =
			(255 * (n - 1)) / (rc->rcRedSteps - 1);
		break;
	...
	case RCsplitGREEN:
		...
		rc->rcSplitValues.rgb8Green =
			(255 * n) / (rc->rcGreenSteps - 1);
		rc->rcBSplitValues.rgb8Green =
			(255 * (n - 1)) / (rc->rcGreenSteps - 1);
		break;
	...
	case RCsplitBLUE:
		...
		rc->rcBSplitValues.rgb8Blue =
			(255 * (n - 1)) / (rc->rcBlueSteps - 1);
		break;
		      ...
		      int greenSteps, int blueSteps)
{
	...
	if (redSteps < 2 || greenSteps < 2 || blueSteps < 2) {
		LLLDEB(redSteps, greenSteps, blueSteps);
		return -1;
	}
