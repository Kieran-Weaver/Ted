...
static int appReadProperties(void *properties, const MemoryBuffer *dir,
...
int appReadUserProperties(EditApplication *ea)
{
	...
	if (appReadProperties(ea->eaUserProperties, &homeDirectory,
			      &relative)) {
		LDEB(1);
		rval = -1;
		goto ready;
...
int appReadSystemProperties(EditApplication *ea)
{
	...
	if (appAbsoluteName(&appDirectory, &appRelative, relativeIsFile,
			    &pkgDirectory) < 0) {
		SSDEB(ea->eaApplicationName, PKGDIR);
		rval = -1;
		goto ready;
	}
	...
	if (appReadProperties(ea->eaSystemProperties, &appDirectory,
			      &relative)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (ea->eaLocaleName) {
		...
		if (uscore) {
			if (utilMemoryBufferPrintf(
				    &relative, "%s-%.*s.properties",
...
static void appSetResourceDefault(EditApplication *ea, void *tree,
				  char *scratch, AppConfigurableResource *acr)
{
	const char *key;
	void *value;
			    ...
			    int acrCount)
{
	...
	for (i = 0; i < acrCount; i++) {
		...
		if (ea->eaUserProperties) {
			appSetResourceDefault(ea, ea->eaUserProperties, scratch,
					      acr + i);
		}
	}
...
static int appSetProperty(EditApplication *ea, void *properties,
...
int appSetUserProperty(EditApplication *ea, const char *name, const char *value)
{
	if (appMakeUserProperties(ea)) {
		XDEB(ea->eaUserProperties);
		return -1;
