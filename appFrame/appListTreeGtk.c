...
static gboolean appGuiGtkHandlePotentialListAction(GtkTreeView *treev,
						   GtkTreePath *path,
...
static gboolean
appGuiGtkHandlePotentialListCallback(GtkTreeSelection *selection,
				     gpointer voidla)
{
	ListAction *la = (ListAction *)voidla;
...
static gboolean appGuiGtkListCursorChanged(GtkTreeView *treev, gpointer voidla)
{
	ListAction *la = (ListAction *)voidla;
...
void appGuiMakeListInColumn(APP_WIDGET *pList, APP_WIDGET column,
			    ...
			    APP_BUTTON_CALLBACK_T actionCallback, void *through)
{
	...
	*/
	...
	gtk_tree_view_insert_column_with_attributes(
		GTK_TREE_VIEW(tree), -1, "col", gtk_cell_renderer_text_new(),
		"text", 0, NULL);
	...
	if (listCallback) {
		g_signal_connect(
			...
			(GtkSignalFunc)appGuiGtkHandlePotentialListCallback,
			voidla);
	}

	if (actionCallback) {
		gtk_signal_connect(
			GTK_OBJECT(tree), "row-activated",
			(GtkSignalFunc)appGuiGtkHandlePotentialListAction,
			voidla);
	}
...
void appGuiAddValueToListWidget(APP_WIDGET tree, int position,
				const char *value)
{
	const int col = 0;
...
void appGuiSelectPositionInListWidget(APP_WIDGET tree, int position)
{
	GtkTreeView *treev = GTK_TREE_VIEW(tree);
	GtkTreePath *path;
	GtkTreeSelection *selection;
...
void appGuiReplaceValueInListWidget(APP_WIDGET tree, int position,
				    const char *value)
{
	const int col = 0;
...
static gboolean appGuiGtkFindValue(GtkTreeModel *model, GtkTreePath *path,
				   GtkTreeIter *iter, void *vfv)
{
...
void appGuiSelectValueInListWidget(APP_WIDGET tree, const char *value)
{
	GtkTreeView *treev = GTK_TREE_VIEW(tree);
	GtkTreeModel *model = gtk_tree_view_get_model(treev);
