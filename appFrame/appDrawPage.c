			...
			const DocumentGeometry *dg)
{
	...
	/*  1  */
	drText.drX0 = wide / 2 - pixelsPerTwip * (dg->dgPageWideTwips / 2 -
						  dg->dgLeftMarginTwips);
	drText.drX1 = wide / 2 + pixelsPerTwip * (dg->dgPageWideTwips / 2 -
						  dg->dgRightMarginTwips);
	drText.drY0 = high / 2 - pixelsPerTwip * (dg->dgPageHighTwips / 2 -
						  dg->dgTopMarginTwips);
	drText.drY1 = high / 2 + pixelsPerTwip * (dg->dgPageHighTwips / 2 -
				   ...
				   const DocumentRectangle *drSheet)
{
	if (rotate) {
		...
		appDrawDirectionArrow(ds, rotate, drSheet->drX0 - ARROWD,
				      (drSheet->drY0 + drSheet->drY1) / 2);
		appDrawDirectionArrow(ds, rotate, drSheet->drX1 + ARROWD,
				      (drSheet->drY0 + drSheet->drY1) / 2);
	}
}
		       ...
		       const PrintGeometry *pg)
{
	...
	if (utilNupSetSchema(&ns, rotateSheet, &atPage, pg, pixelsPerTwip,
			     dgPage)) {
		LLDEB(pg->pgGridRows, pg->pgGridCols);
		goto ready;
	}
	...
	if (pg->pgGridRows * pg->pgGridCols == 1) {
		...
		x0 = AT2_X(dgPage->dgLeftMarginTwips, dgPage->dgTopMarginTwips,
			   &atPage) +
		     0.49;
		y0 = AT2_Y(dgPage->dgLeftMarginTwips, dgPage->dgTopMarginTwips,
			   &atPage) +
		     0.49;
		...
		for (i = 0; i < pg->pgGridRows * pg->pgGridCols; i++) {
			...
			x0 = AT2_X(dgPage->dgLeftMarginTwips,
				   dgPage->dgTopMarginTwips, &atPage) +
			     0.49;
			y0 = AT2_Y(dgPage->dgLeftMarginTwips,
				   dgPage->dgTopMarginTwips, &atPage) +
			     0.49;

			x1 = AT2_X(dgPage->dgPageWideTwips -
					   dgPage->dgRightMarginTwips,
				   dgPage->dgPageHighTwips -
					   dgPage->dgBottomMarginTwips,
				   &atPage) +
			     0.49;
			y1 = AT2_Y(dgPage->dgPageWideTwips -
					   dgPage->dgRightMarginTwips,
				   dgPage->dgPageHighTwips -
					   dgPage->dgBottomMarginTwips,
				   &atPage) +
			...
			appDrawNupTextPage(ds, backColor, screenFont, scratch,
					   &drText);
		}
	}
...
APP_WIDGET appMakePageDrawing(APP_WIDGET parent, double pixelsPerTwip,
			      int widgetHighMm, APP_EVENT_HANDLER_T redraw,
			      void *through)
{
	APP_WIDGET drawing;
	...
	appGuiMakeDrawingAreaInColumn(&drawing, parent, pixelsWide, pixelsHigh,
