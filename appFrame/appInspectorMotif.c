...
void appInspectorChoosePage(AppInspector *ai, int andMenu, int subject)
{
	WidgetList children;
	...
	childCount = 0;
	XtVaGetValues(ai->aiPageParent, XmNchildren, &children, XmNnumChildren,
		      &childCount, NULL);

	for (i = 0; i < childCount; i++) {
		XtVaSetValues(children[i], XmNmappedWhenManaged, i == subject,
			      NULL);
	}
...
void appInspectorDeleteSubject(AppInspector *ai, int subject)
{
	...
	childCount = 0;
	XtVaGetValues(ai->aiSubjectOptionmenu.aomPulldown, XmNchildren,
	...
	if (subject < 0 || subject >= childCount) {
		...
		XtDestroyWidget(children[subject]);
	}
...
void appInspectorEnablePage(AppInspector *ai, int subject, int enabled)
{
	WidgetList children;
	Cardinal childCount = 0;
	...
	if (subject < 0 || subject >= childCount) {
		...
		XtSetSensitive(children[subject], enabled != 0);
	}
