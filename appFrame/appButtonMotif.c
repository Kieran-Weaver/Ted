...
void appMakeButtonInRow(APP_WIDGET *pButton, APP_WIDGET row, const char *text,
			...
			int position, int colspan, int showAsDefault)
{
	Arg al[20];
	int ac = 0;
	...
	if (showAsDefault) {
		XtSetArg(al[ac], XmNdefaultButtonShadowThickness, 1);
		ac++;
		XtSetArg(al[ac], XmNshowAsDefault, showAsDefault != 0);
		ac++;
	} else {
		XtSetArg(al[ac], XmNdefaultButtonShadowThickness, 0);
		ac++;
	}
	...
	if (showAsDefault) {
		XtVaSetValues(row, XmNdefaultButton, button, NULL);
	}
			   ...
			   int showAsDefault)
{
	...
	if (showAsDefault) {
		XtVaSetValues(column, XmNdefaultButton, button, NULL);
	}
