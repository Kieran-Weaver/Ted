...
static void appDocTopConfigure(APP_WIDGET w, void *voided, XEvent *event,
			       Boolean *pRefused)
{
	...
	if (event->type == ConfigureNotify) {
		...
		if (!ed->edMapped && cevent->y < 0) {
			XtVaSetValues(ed->edToplevel.atTopWidget, XmNy, 40,
				      NULL);
		}
	}
...
static int appDocMakeMainWindow(EditDocument *ed)
{
	...
	ac = 0;
	XtSetArg(al[ac], XmNtitle, utilMemoryBufferGetString(&fullTitle));
	ac++;
	XtSetArg(al[ac], XmNiconName, utilMemoryBufferGetString(&iconName));
	ac++;
    ...
    */
	...
	if (pixmap) {
		XtSetArg(al[ac], XmNiconPixmap, pixmap);
		ac++;
	}

#ifdef USE_X11_R5
	ed->edToplevel.atTopWidget = XtAppCreateShell(
		ea->eaApplicationName, ea->eaDocumentWidgetName,
		applicationShellWidgetClass, display, al, ac);
#else
	ed->edToplevel.atTopWidget =
		XtAppCreateShell(ea->eaApplicationName, WIDGET_NAME,
				 topLevelShellWidgetClass, display, al, ac);
#endif
	...
	ed->edMainWindow = XmCreatePanedWindow(ed->edToplevel.atTopWidget,
					       WIDGET_NAME, al, ac);
...
static int appDocMakeScrolledWindow(EditDocument *ed)
{
	...
	ac = 0;
	XtSetArg(al[ac], XmNscrollingPolicy, XmAPPLICATION_DEFINED);
	ac++;
	...
	ac++;
	XtSetArg(al[ac], XmNprocessingDirection, XmMAX_ON_BOTTOM);
	ac++;
	...
	ac++;
	XtSetArg(al[ac], XmNprocessingDirection, XmMAX_ON_RIGHT);
	ac++;
	XtSetArg(al[ac], XmNtraversalOn, False);
	ac++;
	XtSetArg(al[ac], XmNnavigationType, XmNONE);
...
int appMakeDocumentWidget(EditApplication *ea, EditDocument *ed)
{
	...
	/*  2  */
	ed->edWorkWidget =
		XmCreateForm(ed->edScrolledWindow, WIDGET_NAME, al, ac);
	...
	ac++;
	XtSetArg(al[ac], XmNbottomOffset, -ed->edTopRulerHighPixels);
	ac++;
	...
	ac++;
	XtSetArg(al[ac], XmNtopOffset, -ed->edBottomRulerHighPixels);
	ac++;
	XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM);
	ac++;
	XtSetArg(al[ac], XmNrightOffset, 0);
	...
	ac++;
	XtSetArg(al[ac], XmNtopWidget, ed->edTopRulerWidget);
	ac++;
	XtSetArg(al[ac], XmNtopOffset, 0);
	ac++;

	XtSetArg(al[ac], XmNrightAttachment, XmATTACH_OPPOSITE_FORM);
	ac++;
	XtSetArg(al[ac], XmNrightOffset, -ed->edLeftRulerWidePixels);
	ac++;

	XtSetArg(al[ac], XmNbottomAttachment, XmATTACH_WIDGET);
	ac++;
	XtSetArg(al[ac], XmNbottomWidget, ed->edBottomRulerWidget);
	ac++;
	XtSetArg(al[ac], XmNbottomOffset, 0);
	ac++;
	...
	ac++;
	XtSetArg(al[ac], XmNleftOffset, -ed->edRightRulerWidePixels);
	ac++;
	XtSetArg(al[ac], XmNtopAttachment, XmATTACH_WIDGET);
	ac++;
	XtSetArg(al[ac], XmNtopWidget, ed->edTopRulerWidget);
    ...
    */
	...
	ac++;
	XtSetArg(al[ac], XmNleftWidget, ed->edLeftRulerWidget);
	ac++;
	XtSetArg(al[ac], XmNleftOffset, 0);
	ac++;
	XtSetArg(al[ac], XmNtopAttachment, XmATTACH_WIDGET);
	...
	ac++;
	XtSetArg(al[ac], XmNrightWidget, ed->edRightRulerWidget);
	ac++;
	XtSetArg(al[ac], XmNrightOffset, 0);
	ac++;
	XtSetArg(al[ac], XmNbottomAttachment, XmATTACH_WIDGET);
	...
	XtAddCallback(ed->edDocumentWidget.dwWidget, XmNdestroyCallback,
...
int appFinishDocumentWindow(EditDocument *ed)
{
	...
	/*  3  */
	if (appDocMakeMainWindow(ed) || appDocMakeToolbar(ed) ||
	...
	XtAddEventHandler(ed->edDocumentWidget.dwWidget, StructureNotifyMask,
			  False, appDocConfigure, (void *)ed);
	XtAddEventHandler(ed->edToplevel.atTopWidget, StructureNotifyMask,
			  False, appDocTopConfigure, (void *)ed);

	/*  4  */
	XtAddEventHandler(ed->edDocumentWidget.dwWidget, PropertyChangeMask,
			  True, appDocCopyPasteHandler, ed);
...
int appFinishDocumentSetup(EditDocument *ed)
{
	...
	if (ea->eaObserveFocus) {
		appGuiSetFocusChangeHandler(dw->dwWidget, ea->eaObserveFocus,
					    (void *)ed);
	}

	if (ea->eaDocumentMouseHandler) {
		guiDrawSetButtonPressHandler(
			dw->dwWidget, ea->eaDocumentMouseHandler, (void *)ed);
	}

	if (ea->eaDocumentScrollHandler) {
		guiDrawSetScrollHandler(
			dw->dwWidget, ea->eaDocumentScrollHandler, (void *)ed);
	}
...
void appSetWindowsItemState(APP_WIDGET menu, EditDocument *ed, int changed)
{
	...
	XtVaGetValues(menu, XmNchildren, &children, XmNnumChildren, &childCount,
		      NULL);

	for (j = childCount - 1; j >= 0; j--) {
		void *voided = (void *)0;

		XtVaGetValues(children[j], XmNuserData, &voided, NULL);
...
void appRemoveWindowsOption(APP_WIDGET menu, EditDocument *oldEd)
{
	...
	for (i = childCount - 1; i >= 0; i--) {
		...
		XtVaGetValues(children[i], XmNuserData, &voided, NULL);
			    ...
			    const MemoryBuffer *title)
{
	...
	for (i = childCount - 1; i >= 0; i--) {
		...
		if (voided == (void *)ed) {
			appGuiSetToggleItemLabel(
				children[i], utilMemoryBufferGetString(title));
		}
	}
}
