				...
				int tick, double units, int ox)
{
	...
	if (iitem != 0 && tick % rd->rdTicksPerText == 0) {
		...
		if (rd->rdScreenFont >= 0 &&
			...
			drawString(ds, iitem - ox - wide / 2, fontBottom,
				   rd->rdScreenFont, scratch, len);
				   ...
				   DocumentRectangle *drClip, int ox)
{
	...
	for (;;) {
		...
		if (iitem - ox < rd->rdMinUsed || iitem - ox >= maxUsed) {
			units += rd->rdUnitsPerTick;
			tick++;
			continue;
		}

		appHorizontalRulerDrawMark(rd, fontBottom, tagY0, tagY1, tickY0,
					   tickY1, iitem, tick, units, ox);
    ...
    */
	...
	if (mr->mrV0Pixels >= 0 && mr->mrV0Pixels - ox - 2 >= rd->rdMinUsed &&
	    mr->mrV0Pixels - ox < maxUsed) {
		drMark.drX0 = drMark.drX1 = mr->mrV0Pixels - ox - 1;
		drawFillRectangle(ds, &drMark);
	}

	if (mr->mrV1Pixels >= 0 && mr->mrV1Pixels - ox >= rd->rdMinUsed &&
				 ...
				 DocumentRectangle *drClip, int oy)
{
	...
	for (;;) {
		...
		if (iitem - oy < rd->rdMinUsed || iitem - oy >= maxUsed) {
			units += rd->rdUnitsPerTick;
			tick++;
			continue;
		}
    ...
    */
	...
	if (mr->mrV0Pixels >= 0 && mr->mrV0Pixels - oy - 2 >= rd->rdMinUsed &&
	    mr->mrV0Pixels - oy < maxUsed) {
		drMark.drY0 = drMark.drY1 = mr->mrV0Pixels - oy - 1;
		drawFillRectangle(ds, &drMark);
	}

	if (mr->mrV1Pixels >= 0 && mr->mrV1Pixels - oy >= rd->rdMinUsed &&
...
static void appHorizontalRulerSetValue(MetricRulerDrag *mrd, APP_WIDGET w,
				       int newValue)
{
...
static void appHorizontalRulerDragItem(int *pValue, int *pChanged, APP_WIDGET w,
				       EditApplication *ea,
				       APP_EVENT *downEvent, MetricRuler *mr,
				       int ox, EditDocument *ed, int direction)
{
	SelectRectangle *sr = &(ed->edSelectRectangle);
	MetricRulerDrag mrd;
	...
	appRunDragLoop(w, ea, downEvent, appHorizontalRulerMouseUp,
		       appHorizontalRulerMouseMove, 0, (APP_TIMER_CALLBACK)0,
		       (void *)&mrd);
...
void appHorizontalRulerTrackMouse(int *pX0pixels, int *pX1pixels, int *pChanged,
				  APP_WIDGET w, EditApplication *ea,
				  ...
				  EditDocument *ed)
{
	...
	if (mr->mrV0Pixels >= 0 && x + ox < mr->mrV0Pixels + xp &&
	    x + ox >= mr->mrV0Pixels - HAIR_THICK(sizeAcross) - xp) {
		appHorizontalRulerDragItem(&mr->mrV0Pixels, &changed, w, ea,
					   downEvent, mr, ox, ed, DOCselW);
		goto ready;
	}

	if (mr->mrV1Pixels >= 0 && x + ox >= mr->mrV1Pixels - xp &&
	    x + ox < mr->mrV1Pixels + HAIR_THICK(sizeAcross) + xp) {
		/* asUpper= 1; */

		appHorizontalRulerDragItem(&mr->mrV1Pixels, &changed, w, ea,
					   downEvent, mr, ox, ed, DOCselE);
	...
	if (mr->mrV0Pixels >= 0) {
		...
		if (x + ox < mr->mrV1Pixels) {
			...
			changed = 1;
			appHorizontalRulerDragItem(&mr->mrV0Pixels, &changed, w,
						   ea, downEvent, mr, ox, ed,
						   DOCselW);
			...
			appHorizontalRulerDragItem(&mr->mrV1Pixels, &changed, w,
						   ea, downEvent, mr, ox, ed,
						   DOCselE);
...
static void appVerticalRulerSetValue(MetricRulerDrag *mrd, APP_WIDGET w,
				     int newValue)
{
	MetricRuler *mr = mrd->mrdMr;
	RulerData *rd = &(mr->mrRulerData);
...
static void appVerticalRulerDragItem(int *pValue, int *pChanged, APP_WIDGET w,
				     EditApplication *ea, APP_EVENT *downEvent,
				     MetricRuler *mr, int oy, EditDocument *ed,
				     int direction)
{
	SelectRectangle *sr = &(ed->edSelectRectangle);
	MetricRulerDrag mrd;
	...
	appRunDragLoop(w, ea, downEvent, appVerticalRulerMouseUp,
		       appVerticalRulerMouseMove, 0, (APP_TIMER_CALLBACK)0,
		       (void *)&mrd);
	if (pChanged && *pValue != startValue) {
		*pChanged = 1;
	}
...
void appVerticalRulerTrackMouse(int *pY0pixels, int *pY1pixels, int *pChanged,
				APP_WIDGET w, EditApplication *ea,
				APP_EVENT *downEvent, void *voidmr,
				EditDocument *ed)
{
	...
	if (mr->mrV0Pixels >= 0 && y + oy < mr->mrV0Pixels + xp &&
	    y + oy >= mr->mrV0Pixels - HAIR_THICK(sizeAcross) - xp) {
		/* asUpper= 0; */

		appVerticalRulerDragItem(&(mr->mrV0Pixels), &changed, w, ea,
					 downEvent, mr, oy, ed, DOCselN);

		goto ready;
	}

	if (mr->mrV1Pixels >= 0 && y + oy >= mr->mrV1Pixels - xp &&
	    y + oy < mr->mrV1Pixels + HAIR_THICK(sizeAcross) + xp) {
		/* asUpper= 1; */

		appVerticalRulerDragItem(&mr->mrV1Pixels, &changed, w, ea,
					 downEvent, mr, oy, ed, DOCselS);
	...
	if (mr->mrV0Pixels >= 0) {
		...
		if (y + oy < mr->mrV1Pixels) {
			...
			changed = 1;
			appVerticalRulerDragItem(&mr->mrV0Pixels, &changed, w,
						 ea, downEvent, mr, oy, ed,
						 DOCselN);
			...
			changed = 1;
			appVerticalRulerDragItem(&mr->mrV1Pixels, &changed, w,
						 ea, downEvent, mr, oy, ed,
						 DOCselS);
