...
static void appGuiDebugKey(const char *how, XKeyPressedEvent *keyEvent,
			   APP_KEY_VALUE keysym, char *b)
{
...
static void appGuiHandleKeysym(DocumentWidget *dw, char *scratch,
			       APP_KEY_VALUE keysym, int state)
{
	if (keysym != 0 && (keysym & 0xff00) != 0xff00) {
		...
		if (unicode >= 0) {
			...
			if (step < 1) {
				...
				scratch[step] = '\0';
				(*dw->dwGotString)(dw->dwOwner, (char *)scratch,
						   step);
				return;
			}
		}
...
static APP_EVENT_HANDLER_H(appKeyPressed, w, voiddw, event)
{
	...
	if (dw->dwInputContext) {
		...
		(void)/* gotString= */ XmbLookupString(
			dw->dwInputContext, keyEvent, (char *)scratch,
			sizeof(scratch) - 1, &keysym, &status);
		switch (status) {
		case XBufferOverflow:
			LSDEB(status, "XBufferOverflow");
			return;
		case XLookupNone:
			LSDEB(status, "XLookupNone");
			return;
		case XLookupChars:
			LSDEB(status, "XLookupChars");
			return;
		default:
			LSDEB(status, "default");
			return;

		case XLookupKeySym:
		case XLookupBoth:
			scratch[0] = '\0';
			appGuiHandleKeysym(dw, scratch, keysym,
					   keyEvent->state);
			if (debugKeys > 0) {
				appGuiDebugKey(status == XLookupBoth ? "Both" :
								       "KeySym",
		...
		(void)/* gotString= */ XLookupString(keyEvent, (char *)scratch,
						     sizeof(scratch) - 1,
						     &keysym,
						     (XComposeStatus *)0);
...
static XIMStyle appXimPreferences[] = {
	XIMPreeditCallbacks, XIMPreeditPosition, XIMPreeditArea,
	XIMPreeditNothing,   XIMStatusCallbacks, XIMStatusArea,
	XIMStatusNothing,
};

void appDocumentSetInputContext(APP_INPUT_METHOD im, DocumentWidget *dw)
{
	XtAddEventHandler(dw->dwWidget, KeyPressMask, False, appKeyPressed,
			  (void *)dw);
	...
	if (im) {
		...
		XGetIMValues(im, XNQueryInputStyle, &availableStyles, NULL);

		supportedStyles = XIMPreeditArea | XIMPreeditNothing |
				  XIMPreeditNone | XIMStatusArea |
				  XIMStatusNothing | XIMStatusNone;

		likedStyle = 0;
		for (style = 0; style < availableStyles->count_styles;
		     style++) {
			XIMStyle foundStyle =
				availableStyles->supported_styles[style];
			unsigned j;
			...
			if (!likedStyle) {
				likedStyle = foundStyle;
				continue;
			}

			for (j = 0;
			     j < sizeof(appXimPreferences) / sizeof(XIMStyle);
			     j++) {
		...
		if (likedStyle) {
			dw->dwInputContext = XCreateIC(im, XNClientWindow,
						       XtWindow(dw->dwWidget),
