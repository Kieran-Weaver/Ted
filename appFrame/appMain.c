...
void appGuiGetResourceValues(int *pGotResources, EditApplication *ea,
			     void *pValues, AppConfigurableResource *acrList,
			     int acrCount)
{
...
APP_CLOSE_CALLBACK_H(appAppWmClose, w, voidea)
{
	...
	if (ea->eaMainVisibleAsAbout) {
		...
		appQuitApplication((APP_WIDGET)0, ea->eaToplevel.atTopWidget,
				   ea);
	}
}
...
static void appAskCloseDocuments(APP_WIDGET option, EditApplication *ea)
{
	int i;
...
static void appHighlightCurrentDocument(EditApplication *ea, EditDocument *ed,
					int current)
{
...
void appDocumentChanged(EditDocument *ed, int changed)
{
	...
	if (ed->edHasBeenChanged != changed) {
		...
		for (i = 0; i < ea->eaOpenCount; i++) {
			...
			appSetWindowsItemState(
				ea->eaOpenDocuments[i]->edWindowMenu, ed,
				changed);
		}
...
void appRemoveDocument(EditApplication *ea, EditDocument *ed)
{
	...
	for (i = 0; i < ea->eaOpenCount; i++) {
		...
		appRemoveWindowsOption(ea->eaOpenDocuments[i]->edWindowMenu,
				       ed);
			      ...
			      const MemoryBuffer *title)
{
	...
	for (i = 0; i < ea->eaOpenCount; i++) {
		...
		appRenameWindowsOption(ea->eaOpenDocuments[i]->edWindowMenu, ed,
				       title);
	}
...
static void appAppSetWindowsOption(APP_WIDGET menu, EditDocument *ed,
				   const MemoryBuffer *title)
{
	AppMenuItem ami;
	...
	if (ed->edHasBeenChanged) {
		...
		ami.amiItemType = ITEMtyTOGGLE_OFF;
	}
...
void appSetDocument(EditApplication *ea, EditDocument *newEd)
{
	EditDocument **fresh;
	int i;
...
static AppConfigurableResource APP_ApplicationResourceTable[] = {
	APP_RESOURCE("unit", offsetof(EditApplication, eaUnitString), "inch"),
	APP_RESOURCE("paper", offsetof(EditApplication, eaPaperString),
		     "letter"),
	APP_RESOURCE("leftMargin",
		     offsetof(EditApplication, eaLeftMarginString), "90pt"),
	APP_RESOURCE("rightMargin",
		     offsetof(EditApplication, eaRightMarginString), "90pt"),
	APP_RESOURCE("topMargin", offsetof(EditApplication, eaTopMarginString),
		     "72pt"),
	APP_RESOURCE("bottomMargin",
		     offsetof(EditApplication, eaBottomMarginString), "72pt"),
	APP_RESOURCE("magnification",
		     offsetof(EditApplication, eaMagnificationString), "100%"),
	...
	APP_RESOURCE("usePostScriptIndexedImages",
		     offsetof(EditApplication,
			      eaUsePostScriptIndexedImagesString),
		     "1"),
	APP_RESOURCE("sevenBitsPostScript",
		     offsetof(EditApplication, ea7BitsPostScriptString), "0"),

	APP_RESOURCE("skipEmptyPages",
		     offsetof(EditApplication, eaSkipEmptyPagesString), "0"),
	APP_RESOURCE("skipBlankPages",
		     offsetof(EditApplication, eaSkipBlankPagesString), "0"),
	...
	APP_RESOURCE("embedFonts",
		     offsetof(EditApplication, eaEmbedFontsString), "1"),
	APP_RESOURCE("useKerning",
		     offsetof(EditApplication, eaUseKerningString), "1"),
	APP_RESOURCE("styleTool", offsetof(EditApplication, eaStyleToolString),
		     "1"),
	...
	APP_RESOURCE("customPrinterName",
		     offsetof(EditApplication, eaCustomPrinterName), (char *)0),
...
static void appDetermineGeometrySettings(EditApplication *ea)
{
	...
	if (ea->eaUnitInt < 0) {
		SDEB(ea->eaUnitString);
		ea->eaUnitInt = UNITtyINCH;
	}
	...
	if (ea->eaMagnificationString) {
		...
		if (mag < 0.1 || ea->eaMagnification < 0.1 ||
	...
	if (ea->eaTopMarginString) {
		if (geoLengthFromString(
			    ea->eaTopMarginString, ea->eaUnitInt,
...
void appGetApplicationResourceValues(EditApplication *ea)
{
	...
	if (!ea->eaGotResourceTable) {
		appGuiGetResourceValues(&(ea->eaGotResourceTable), ea,
					ea->eaResourceData, ea->eaResourceTable,
					ea->eaResourceCount);
	}

	if (!ea->eaGotFileMessageResourceTable) {
		appGuiGetResourceValues(&(ea->eaGotFileMessageResourceTable),
					ea, &(ea->eaFileMessageResources),
	...
	if (!ea->eaGotApplicationResources) {
		appGuiGetResourceValues(
			&(ea->eaGotApplicationResources), ea, (void *)ea,
	...
	appDetermineBoolean(&(ea->eaUseKerningInt), ea->eaUseKerningString);
	appDetermineBoolean(&(ea->eaStyleToolInt), ea->eaStyleToolString);
...
static int appFinishApplicationWindow(EditApplication *ea)
{
	...
	ea->eaFileMenu = appMakeMenu(&(ea->eaFileMenuButton), &(ea->eaToplevel),
				     ea, ea->eaMenuBar,
				     *(ea->eaAppFileMenuText), 0,
				     ea->eaAppFileMenuItems,
				     ea->eaAppFileMenuItemCount, (void *)ea);

	ea->eaWinMenu = appMakeMenu(&(ea->eaWinMenuButton), &(ea->eaToplevel),
				    ea, ea->eaMenuBar, *(ea->eaAppWinMenuText),
				    0, ea->eaAppWinMenuItems,
				    ea->eaAppWinMenuItemCount, (void *)ea);
	...
	if (ea->eaAppHelpMenuItems) {
		ea->eaHelpMenu =
			appMakeMenu(&(ea->eaHelpMenuButton), &(ea->eaToplevel),
				    ea, ea->eaMenuBar, *(ea->eaAppHelpMenuText),
				    1, ea->eaAppHelpMenuItems,
	...
	if (ea->eaNameAndVersion) {
		...
		if (0 && ea->eaPlatformCompiled) {
			...
			if (!ident) {
				...
				sprintf(ident, "%s - %s", ea->eaNameAndVersion,
	...
	if (ea->eaReference) {
		appMakeLabelInColumn(&label, ea->eaMainWindow, ea->eaReference);
	}
...
static int app_version(EditApplication *ea, const char *prog, const char *call,
		       int argc, char **argv)
{
	printf("%s\n", ea->eaNameAndVersion);
	return 0;
...
static int app_platform(EditApplication *ea, const char *prog, const char *call,
			int argc, char **argv)
{
	printf("%s\n", ea->eaPlatformCompiled);
	return 0;
...
static int app_build(EditApplication *ea, const char *prog, const char *call,
		     int argc, char **argv)
{
...
static int app_fullVersion(EditApplication *ea, const char *prog,
			   const char *call, int argc, char **argv)
{
	printf("%s %s %s %s\n", ea->eaNameAndVersion, ea->eaOptionalComponents,
	       ea->eaPlatformCompiled, ea->eaHostDateCompiled);

	return 0;
}

static int app_saveTo(EditApplication *ea, const char *prog, const char *call,
		      int argc, char **argv)
{
	int rval = 2;
...
static int app_printToFile(EditApplication *ea, const char *prog,
			   const char *call, int argc, char **argv)
{
	if (argc < 2) {
		SLDEB(call, argc);
...
static int app_printToFilePaper(EditApplication *ea, const char *prog,
				const char *call, int argc, char **argv)
{
	if (argc < 3) {
		SLDEB(call, argc);
...
static int app_print(EditApplication *ea, const char *prog, const char *call,
		     int argc, char **argv)
{
	if (argc < 1) {
		SLDEB(call, argc);
...
static int app_printPaper(EditApplication *ea, const char *prog,
			  const char *call, int argc, char **argv)
{
	if (argc < 2) {
		SLDEB(call, argc);
...
static int app_printToPrinter(EditApplication *ea, const char *prog,
			      const char *call, int argc, char **argv)
{
	if (argc < 2) {
		SLDEB(call, argc);
...
static int app_printToPrinterPaper(EditApplication *ea, const char *prog,
				   const char *call, int argc, char **argv)
{
	if (argc < 3) {
		SLDEB(call, argc);
...
static const SpecialCall AppSpecialCalls[] = {
	...
	{
		"printToPrinterPaper",
		app_printToPrinterPaper,
	},
};

static const int AppSpecialCallCount =
	sizeof(AppSpecialCalls) / sizeof(SpecialCall);
...
static const SpecialCall *appGetSpecialCall(const SpecialCall *calls,
					    int callCount, const char *call)
{
	int c;
...
static int appMainHandleSpecialCalls(EditApplication *ea, const char *prefix,
				     int getResources, const char *prog,
				     int argc, char *argv[])
{
	...
	while (done < argc) {
		...
		off = done + 1;
		args = (*sc->scExecuteCall)(ea, prog, call, argc - off,
					    argv + off);
		if (args < 0) {
			SLDEB(argv[1 + done], args);
			return -1;
		}
		done += 1 + args;
	}
...
static int appOpenExistingDocument(EditApplication *ea, int readOnly,
				   int formatHint, const MemoryBuffer *filename)
{
	...
	if (appAbsoluteName(&absolute, filename, relativeIsFile,
			    fileRelativeTo) < 0) {
		SDEB(utilMemoryBufferGetString(filename));
		ed = appOpenDocumentFile(ea, ea->eaToplevel.atTopWidget,
					 (APP_WIDGET)0, readOnly, suggestStdin,
					 formatHint, filename);
	} else {
		ed = appOpenDocumentFile(ea, ea->eaToplevel.atTopWidget,
					 (APP_WIDGET)0, readOnly, suggestStdin,
...
static int appSetProperties(EditApplication *ea, int argc, char *argv[])
{
	int arg;
	int argTo = 1;

	for (arg = 1; arg < argc; arg++) {
		if (arg + 3 <= argc && !strcmp(argv[arg], "--setProperty")) {
			if (appSetUserProperty(ea, argv[arg + 1],
					       argv[arg + 2])) {
		...
		if (!strncmp(argv[arg], "--", 2)) {
			...
			if (eq) {
				...
				*eq = '\0';
				res = appSetUserProperty(ea, argv[arg] + 2,
							 eq + 1);
				*eq = '=';
...
int appMain(EditApplication *ea, int argc, char *argv[])
{
	...
	for (arg = 1; arg < argc; arg++) {
		...
		if (!appTestDirectory(&filename)) {
			appRunOpenChooser(
				(APP_WIDGET)0, ea->eaToplevel.atTopWidget,
		...
		if (appTestFileExists(&filename)) {
			...
			if (argc == 2 && openFormat >= 0 && suggestStdin) {
				appOpenDocumentFile(ea,
						    ea->eaToplevel.atTopWidget,
						    (APP_WIDGET)0, readOnly,
						    suggestStdin, openFormat,
						    &filename);

				continue;
			}

			saveFormat = appDocumentGetSaveFormat(
				&suggestStdout, ea, &filename, (void *)0,
			...
			if (createNew) {
				...
				ynRes = appQuestionRunSubjectYesNoDialog(
					ea, ea->eaToplevel.atTopWidget,
					(APP_WIDGET)0, argv[arg],
					afmr->afmrMakeItQuestion);
				...
				appQuestionRunFilenameErrorDialog(
					ea, ea->eaToplevel.atTopWidget,
					(APP_WIDGET)0, &filename,
					afmr->afmrNoSuchFileMessage);
			}
		} else {
			(void)appOpenExistingDocument(ea, readOnly, openFormat,
						      &filename);
		}
	}
