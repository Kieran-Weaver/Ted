...
/*  1  */
static void appDialogSetFocus(Widget w, XtPointer voidqc, XEvent *event,
			      Boolean *pRefused)
{
...
#if FIX_DIALOG_SIZE
/*  2  */
static void appFixDialogSize(Widget w, void *through, XEvent *event,
			     Boolean *pRefused)
{
	...
	XtRemoveEventHandler(w, StructureNotifyMask, False, appFixDialogSize,
			     through);
...
void appMakeVerticalDialog(AppDialog *ad, Widget *pPaned, EditApplication *ea,
			   ...
			   void *through)
{
	...
	shell = XmCreateDialogShell(ea->eaToplevel.atTopWidget, WIDGET_NAME, al,
				    ac);
...
#if FIX_DIALOG_SIZE
	See Above XtAddEventHandler(shell, StructureNotifyMask, False,
    ...
    */
	...
	XtAddEventHandler(shell, StructureNotifyMask, False, appDialogSetFocus,
			  (void *)0);
...
void appGuiRunDialog(AppDialog *ad, int initial, EditApplication *ea)
{
	...
	while (ad->adContinue && XtIsRealized(ad->adTopWidget)) {
...
{
	XtVaSetValues(ad->adDialog, XmNdefaultButton, button, NULL);
}

void appGuiSetCancelButtonForDialog(AppDialog *ad, APP_WIDGET button)
{
	XtVaSetValues(ad->adDialog, XmNcancelButton, button, NULL);
}

static void appDialogRelative(Widget relative, Widget dialog)
{
	Dimension x;
	Dimension y;
	Dimension width;
	...
	XtVaGetValues(relative, XmNx, &x, XmNy, &y, XmNwidth, &width, XmNheight,
		      &height, NULL);

	XtVaSetValues(dialog, XmNx, x + width / 5, XmNy, y + height / 5, NULL);

	XtVaSetValues(XtParent(dialog), XmNtransientFor, relative, XmNx,
		      x + width / 5, XmNy, y + height / 5, NULL);
}

static void appRelativeMapCallback(Widget w, XtPointer voidRelative,
				   XtPointer voidcbs)
{
	...
	XtRemoveCallback(w, XmNmapCallback, appRelativeMapCallback,
			 voidRelative);
}
...
void appGuiShowDialog(EditApplication *ea, AppDialog *ad, Widget relative)
{
	if (relative) {
...
void appGuiHideDialog(AppDialog *ad)
{
	...
	XtVaSetValues(ad->adTopWidget, XmNtransientFor, (Widget)0, NULL);
}
