			...
			APP_WIDGET w, int fontSizeHintPixels)
{
	...
	if (w) {
		...
		if (gs) {
			if (gs->font_desc) {
				...
				fam = pango_font_description_get_family(
					gs->font_desc);
				if (fam) {
					fontFamilyName = fam;
				}
				...
				if (fontSizeHintPixels <= 0 &&
					...
					sz = pango_font_description_get_size(
						gs->font_desc);
					if (sz > 0) {
						pixelSize = sz;
					}
				...
				style = pango_font_description_get_style(
					gs->font_desc);
				...
				weight = pango_font_description_get_weight(
					gs->font_desc);
	...
	afi = appGetFontInfo(fontFamilyName, DFstyleFNIL, (IndexSet *)0, &ta,
			     psfl);
	if (!afi) {
		SXDEB(fontFamilyName, afi);
		return -1;
	}
