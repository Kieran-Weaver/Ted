...
static APP_EVENT_HANDLER_H(appColorChooserRedrawInplace, w, voidcc, exposeEvent)
{
	...
	appDrawnPulldownDrawArrow(&drClip, &wide, &high, w, &(cc->ccPulldown));
	...
	if (!cc->ccColorExplicit) {
		...
		drawString(dsI, cc->ccXShift + 4,
			   (row + 1) * cc->ccStripHigh - 7,
			   cc->ccInplaceScreenFont, ccr->ccrAutomaticColor,
...
static int appColorChooserGetColor(ColorChooser *cc, RGB8Color **pRgb8, int row,
				   int col)
{
	int color;
	...
	if (color < 0 || color >= cc->ccColorCount) {
		LLDEB(color, cc->ccColorCount);
		return 1;
	}

	ccpc = cc->ccColors + color;
	if (ccpc->ccpcStatus == CCstatusFREE) { /*LDEB(ccpc->ccpcStatus);*/
		return 1;
	}
...
static int appColorChooserAllocatePullDownDrawingData(ColorChooser *cc)
{
	...
	if (cc->ccPulldownScreenFont < 0) {
		cc->ccPulldownScreenFont = guiGetLabelFont(
			...
			cc->ccPostScriptFontList, cc->ccLabelWidget);
	}
...
static APP_EVENT_HANDLER_H(appColorChooserRedrawPulldown, w, voidcc,
			   exposeEvent)
{
	ColorChooser *cc = (ColorChooser *)voidcc;
	DrawingSurface *dsP;
	...
	if (cc->ccPulldownScreenFont < 0) {
		...
		if (cc->ccHasAutomatic) {
			row = 0;
			drawString(dsP, cc->ccXShift + 4,
				   (row + 1) * cc->ccStripHigh - 5,
		...
		row = cc->ccStrips - 1;
		drawString(dsP, cc->ccXShift + 4,
			   (row + 1) * cc->ccStripHigh - 8,
			   cc->ccPulldownScreenFont, ccr->ccrMoreColors,
			   strlen(ccr->ccrMoreColors));
	}
	...
	for (row = cc->ccHasAutomatic; row < cc->ccStrips - 1; row++) {
		...
		for (col = 0; col < cc->ccColumns; col++) {
			...
			if (cc->ccColorExplicit && cc->ccColorSet &&
	...
	drawSetLineAttributes(dsP, 1, LineStyleSolid, LineCapButt,
			      LineJoinMiter, (const unsigned char *)0, 0);
	...
	if (currRow >= 0 && currCol >= 0) {
		...
		drawSetLineAttributes(dsP, 3, LineStyleSolid, LineCapButt,
...
static APP_EVENT_HANDLER_H(appColorChooserClickedPulldown, w, voidcc,
			   mouseEvent)
{
	ColorChooser *cc = (ColorChooser *)voidcc;
	...
	/*  1  */
	if (appGuiDrawnPulldownGetStrip(&row, &(cc->ccPulldown), w,
					mouseEvent)) {
		return;
	}
	...
	if (cc->ccCallback) {
		...
		appColorChooserSetColor(cc, colorExplicit,
					&(ccpc->ccpcRGB8Color));

		(*cc->ccCallback)(cc, cc->ccWhich, cc->ccTarget, CHOICEccCOLOR,
				  &(ccpc->ccpcRGB8Color));
	}
...
static int appColorChooserAllocateColor(const ColorChooser *cc,
					const RGB8Color *rgb8, int status)
{
	int i;
	ColorChooserPaletteColor *ccpc;
...
static int appColorChooserFindNearest(const RGB8Color *colors, int colorCount,
				      int dist, const RGB8Color *rgb8)
{
	int i;
	int found = -1;
	int d = 256;
...
static int appColorChooserTryColor(ColorChooser *cc, const RGB8Color *colors,
				   int colorCount, int dist, int r, int g,
				   int b)
{
	int found;
	int status;
...
static int appColorChooserCompareHue(const void *vccpc1, const void *vccpc2)
{
	const ColorChooserPaletteColor *ccpc1;
	const ColorChooserPaletteColor *ccpc2;
...
static int appColorChooseMoreColors(ColorChooser *cc, const RGB8Color *colors,
				    int colorCount, int *rs, int rc, int *gs,
				    int gc, int *bs, int bc)
{
	int r;
	int g;
	int b;
	...
	for (b = 0; b < bc; b++) {
		for (r = 0; r < rc; r++) {
			for (g = 0; g < gc; g++) {
				if (appColorChooserTryColor(
					    cc, colors, colorCount, d, rs[r],
					    gs[g], bs[b])) {
					return 1;
				}
			}
				   ...
				   const ColorPalette *cp)
{
	...
	if (cc->ccColorCount < useColors) {
		ccpc = (ColorChooserPaletteColor *)realloc(
			cc->ccColors,
			useColors * sizeof(ColorChooserPaletteColor));

		if (!ccpc) {
			LXDEB(useColors, ccpc);
			return;
		}
	...
	if (appColorChooserTryColor(cc, colors, colorCount, dist, 255, 255,
				    255)) {
		goto ready;
	}
	...
	if (appColorChooserTryColor(cc, colors, colorCount, dist, 0, 255,
				    255)) {
		goto ready;
	}
	if (appColorChooserTryColor(cc, colors, colorCount, dist, 255, 0,
				    255)) {
		goto ready;
	}
	if (appColorChooserTryColor(cc, colors, colorCount, dist, 255, 255,
	...
	for (i = 0; i < 6; i++) {
		...
		if (appColorChooserTryColor(cc, colors, colorCount, dist, c, c,
					    c)) {
			goto ready;
		}
	}
	...
	/*  4  */
	{
		...
		if (appColorChooseMoreColors(cc, colors, colorCount, rr0,
					     sizeof(rr0) / sizeof(int), gg0,
					     sizeof(gg0) / sizeof(int), bb0,
					     sizeof(bb0) / sizeof(int))) {
			goto ready;
		}

		if (appColorChooseMoreColors(cc, colors, colorCount, rr1,
					     sizeof(rr1) / sizeof(int), gg1,
					     sizeof(gg1) / sizeof(int), bb1,
					     sizeof(bb1) / sizeof(int))) {
			goto ready;
		}

		if (appColorChooseMoreColors(cc, colors, colorCount, all,
					     sizeof(all) / sizeof(int), all,
					     sizeof(all) / sizeof(int), all,
					     sizeof(all) / sizeof(int))) {
			goto ready;
		}
	}
...
ready:
	...
	qsort(cc->ccColors, cc->ccColorCount, sizeof(ColorChooserPaletteColor),
...
void appFinishColorChooser(ColorChooser *cc, const PostScriptFontList *psfl,
...
void appMakeColorChooserInRow(ColorChooser *cc, int hasAutomatic,
			      ...
			      ColorChooserCallback callback, int which,
			      void *through)
{
	...
	appMakeDrawnPulldownInRow(&(cc->ccPulldown),
				  ...
				  appColorChooserClickedPulldown, pullDown, row,
				  col, colspan, (void *)cc);
...
void appMakeLabelAndColorChooserRow(APP_WIDGET *pRow, APP_WIDGET *pLabel,
				    ...
				    APP_WIDGET column, const char *labelText,
				    const ColorChooserResources *ccr,
...
void appMakeToggleAndColorChooserRow(APP_WIDGET *pRow, APP_WIDGET *pToggle,
				     ColorChooser *cc, int hasAutomatic,
				     APP_WIDGET column, const char *toggleText,
				     const ColorChooserResources *ccr,
				     ...
				     int which, void *through)
{
	...
	toggle = appMakeToggleInRow(row, toggleText, toggleCallback, through,
				    toggleColumn, toggleColspan);
	appMakeColorChooserInRow(cc, hasAutomatic, row, ccColumn, ccColspan,
				 ccr, colorCallback, which, through);
...
void appColorChooserColorChosen(PropertyMask *isSetMask, int *pChanged,
				...
				const RGB8Color *rgb8Set, int colorExplicit,
				int which)
{
	if (colorExplicit) {
		if (!*pExplicit) {
