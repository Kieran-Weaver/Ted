...
static void appChooserDestroyed(APP_WIDGET fileChooser, GdkEvent *event,
				void *voidaci)
{
...
static void appFileChooserSetFilter(AppChooserInformation *aci, int filter)
{
...
static void appChooserSetFileExtension(const MemoryBuffer *filename,
				       ...
				       GtkFileChooser *gfs, int selectFile)
{
...
static int appMakeFileChooser(AppChooserInformation **pAci,
			      ...
			      const gchar *actionText, APP_WIDGET relative,
			      EditApplication *ea, APP_WIDGET option)
{
	...
	aci->aciWidget = gtk_file_chooser_dialog_new(
		title, GTK_WINDOW(relative), action, GTK_STOCK_CANCEL,
		GTK_RESPONSE_CANCEL, actionText, GTK_RESPONSE_ACCEPT, NULL);
...
static int appMakeFileSelection(AppChooserInformation **pAci,
				APP_WIDGET relative, EditApplication *ea,
				APP_WIDGET option)
{
	AppChooserInformation *aci;
	const char *title = "<@v@>";
...
static int appRunFileChooser(EditApplication *ea, int acceptResponse,
			     AppChooserInformation *aci, APP_WIDGET relative)
{
				   ...
				   void *through)
{
	...
	if (!ret &&
	    (*openDocument)(aci->aciApplication, through, aci->aciRelativeTo,
				    ...
				    void *through)
{
	...
	if (filenames) {
		while (fn) {
			...
			appFileChooserOpenFile(aci, &filename, openDocument,
					       through);
...
static void appFileChooserOpenFiles(AppChooserInformation *aci,
				    APP_WIDGET option, GtkFileChooser *gfs,
				    APP_OPEN_DOCUMENT openDocument,
				    void *through)
{
		       ...
		       void *through)
{
	...
	if (withFilter) {
		...
		if (appFileFilterGetDescriptions(ea, extensions,
						 extensionCount)) {
			LDEB(extensionCount);
		}
		...
		for (i = 0; i < extensionCount; afe++, i++) {
			...
			if (defaultFilter &&
			    !strcmp(afe->afeFilter, defaultFilter)) {
				defaultFileFilter = fileFilter;
			}
		}
...
static int appChooserGetFileName(MemoryBuffer *fileName, GtkFileChooser *gfs)
{
...
static int appSaveChooserSave(AppChooserInformation *aci, APP_WIDGET relative,
			      unsigned int useFlags, GtkFileChooser *gfs,
			      EditDocument *ed)
{
	int rval = 0;
	EditApplication *ea = ed->edApplication;
...
static void appFileFilterChosen(int ext, void *voidaci)
{
	AppChooserInformation *aci = (AppChooserInformation *)voidaci;
	GtkFileChooser *gfs = GTK_FILE_CHOOSER(aci->aciWidget);
	...
	if (fnm) {
		...
		appChooserSetFileExtension(&filename, newExtension, gfs,
					   selectFile);
	}
...
static void appFillSaveFilter(AppChooserInformation *aci, unsigned int useFlags,
			      EditDocument *ed)
{
	EditApplication *ea = ed->edApplication;
	...
	if (appFileFilterGetDescriptions(ea, ea->eaFileExtensions,
			     ...
			     void *through)
{
	...
	if (!utilMemoryBufferIsEmpty(&(ed->edFilename))) {
		...
		appChooserSetFileExtension(&(ed->edFilename), psExt, gfs,
					   selectFile);
		appChooserSetDirectory(&(ed->edFilename), gfs);
	}
