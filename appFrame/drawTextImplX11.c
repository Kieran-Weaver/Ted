...
void drawTextSegment8X11(TextProgress *tp, DrawScreenFont *dsf, APP_FONT *xfs,
			 const char *s, int l)
{
	gdk_draw_text(dsf->dsfDrawable, xfs, dsf->dsfGc, tp->tpX, tp->tpY, s,
		      l);
}
...
{
	gdk_draw_text(dsf->dsfDrawable, xfs, dsf->dsfGc, tp->tpX, tp->tpY,
		      (gchar *)ws, 2 * wl);
}
...
static int appDrawX11SetWS(int *pUsed, const char *ss, int len)
{
	int wl = 0;
	int used = 0;
...
static int appDrawX11SetBS(int *pUsed, const IndexMapping *im, const char *ss,
			   int len)
{
	int bl = 0;
	int used = 0;
	...
	if (bc < len) {
		unsigned char *fresh = (unsigned char *)realloc(
			bs, len * sizeof(unsigned char));
...
void drawHandleTextSegmentsX11(TextProgress *tp, DrawScreenFont *dsf,
			       const char *s, int len, HandleText8 handleText8,
			       ...
			       const EncodedScreenFontList *esfl)
{
	...
	while (len > 0) {
		...
		if (efs->esfIsTwoByte) {
			...
			if (bl < 0) {
				SLLDEB(efs->esfFontName, len, bl);
				return;
			}

			(*handleText8)(tp, dsf, efs->esfFontStruct, (char *)bs,
				       bl);
		}
...
void drawVerticalTextExtentsX11(DocumentRectangle *drText, int y,
				const EncodedScreenFontList *esfl)
{
