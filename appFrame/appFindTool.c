...
static void appFindToolSetFindResult(FindTool *aft, int result)
{
	int sensitive;
...
static int appFindToolSetProgram(FindTool *aft)
{
	...
	if (!aft->aftSetPattern) {
		XDEB(aft->aftSetPattern);
		return -1;
	}

	pattern = appGetStringFromTextWidget(aft->aftPatternText);

	res = (*aft->aftSetPattern)(aft->aftApplication, pattern,
				    aft->aftUseRegex);
...
static void
appFindToolMakeButtonRow(APP_WIDGET *pRow, APP_WIDGET parent,
			 ...
			 APP_BUTTON_CALLBACK_T rightCallback, FindTool *aft)
{
	APP_WIDGET row;
	...
	appMakeButtonInRow(&(leftButton), row, leftLabel, leftCallback,
			   (void *)aft, 0, colspan, showAsDefault);
				       ...
				       FindTool *aft)
{
	...
	appFindToolMakeButtonRow(&row, paned, &(aft->aftFindNextButton),
				 &(aft->aftFindPrevButton),
...
static void appFindMakeReplaceFrame(APP_WIDGET parent,
				    const FindToolResources *aftr,
				    FindTool *aft)
{
	APP_WIDGET row;
	...
	appMakeColumnFrameInColumn(&(aft->aftReplaceFrame), &paned, parent,
				   aftr->aftrReplaceTitle);
	...
	appFindToolMakeButtonRow(&row, paned, &(aft->aftReplaceButton),
				 &(aft->aftReplaceNextButton),
				 aftr->aftrReplaceFound, aftr->aftrReplaceNext,
				 appFindToolReplaceSelectionPushed,
				 appFindToolReplaceNext, aft);
}
		     ...
		     const InspectorSubjectResources *isr)
{
	...
	if (aftr->aftrExplicitFocus && !strcmp(aftr->aftrExplicitFocus, "1")) {
...
void appFindToolSetPattern(FindTool *aft, const char *pattern, int useRegex)
{
...
static AppConfigurableResource APP_FindToolResourceTable[] = {
	APP_RESOURCE("findToolFindTitle",
		     offsetof(FindToolResources, aftrFindTitle), "Find"),
	...
	APP_RESOURCE("findToolReplaceTitle",
		     offsetof(FindToolResources, aftrReplaceTitle), "Replace"),

	APP_RESOURCE("findToolReplaceFound",
		     offsetof(FindToolResources, aftrReplaceFound), "Replace"),
	APP_RESOURCE("findToolReplaceNext",
		     offsetof(FindToolResources, aftrReplaceNext),
		     "Replace, Next"),

	APP_RESOURCE("findToolExplicitFocus",
		     offsetof(FindToolResources, aftrExplicitFocus), "1"),
};

void appFindToolGetResourceTable(EditApplication *ea, FindToolResources *spr,
