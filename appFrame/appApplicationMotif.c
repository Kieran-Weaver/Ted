...
int appGuiInitApplication(EditApplication *ea, int *pArgc, char ***pArgv)
{
	...
	ea->eaToplevel.atTopWidget =
		XtAppInitialize(&(ea->eaContext), ea->eaApplicationName, NULL,
				0, &argc, argv, NULL, al, ac);
#else
	cloneCommand[0] = argv[0];
	cloneCommand[1] = (char *)0;
	XtSetArg(al[ac], XtNcloneCommand, cloneCommand);
	ac++;

	ea->eaToplevel.atTopWidget = XtOpenApplication(
		&(ea->eaContext), ea->eaApplicationName, NULL, 0, &argc, argv,
		NULL, sessionShellWidgetClass, al, ac);
#endif

	/*  2  */
	ea->eaScreenPixelsWide = DisplayWidth(
		XtDisplay(ea->eaToplevel.atTopWidget),
		DefaultScreen(XtDisplay(ea->eaToplevel.atTopWidget)));
	ea->eaScreenPixelsHigh = DisplayHeight(
		XtDisplay(ea->eaToplevel.atTopWidget),
		DefaultScreen(XtDisplay(ea->eaToplevel.atTopWidget)));

	ea->eaInputMethod = XOpenIM(XtDisplay(ea->eaToplevel.atTopWidget), NULL,
				    NULL, NULL);
	...
	appSetCloseCallback(ea->eaToplevel.atTopWidget, ea, appAppWmClose,
			    (void *)ea);
