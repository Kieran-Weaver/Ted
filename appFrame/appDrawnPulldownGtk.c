				   ...
				   void *through)
{
	...
	if (redrawInplace) {
		guiDrawSetRedrawHandler(adp->adpInplaceDrawing, redrawInplace,
					through);
	}
	...
	gtk_drawing_area_size(GTK_DRAWING_AREA(adp->adpPulldownDrawing), 100,
			      100);

	if (redrawPulldown) {
		guiDrawSetRedrawHandler(adp->adpPulldownDrawing, redrawPulldown,
					through);
	}
				  ...
				  APP_WIDGET column, void *through)
{
	...
	gtk_box_pack_start(GTK_BOX(column), adp->adpInplaceDrawing, expand,
			   fill, 0);
