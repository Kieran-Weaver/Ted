		      ...
		      APP_BITMAP_IMAGE *pPixmap, APP_BITMAP_MASK *pMask)
{
	...
	if (i >= ea->eaNamedPictureCount) {
		SLDEB(name, ea->eaNamedPictureCount);
		return -1;
	}
	...
	res = XpmCreatePixmapFromData(
		display, RootWindow(display, DefaultScreen(display)),
		(char **)np->npBuffer, &pixmap, &mask, &APP_XpmAttributes);

	while (res == XpmColorFailed &&
	       APP_XpmAttributes.red_closeness < 10000) {
...
void appDiscardImagePixmaps(EditApplication *ea)
{
	...
	for (i = 0; i < ea->eaNamedPictureCount; np++, i++) {
		if (np->npPixmap) {
			XFreePixmap(display, np->npPixmap);
			np->npPixmap = None;
		}
		if (np->npMask) {
			XFreePixmap(display, np->npMask);
			np->npMask = None;
		}
	}
