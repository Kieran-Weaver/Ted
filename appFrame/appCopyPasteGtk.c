...
static void appAskForTargetTypes(GtkWidget *widget, const char *selection)
{
...
static int appGetResponseType(AppSelectionType **pAst,
			      ...
			      int astCount, APP_ATOM selection, APP_ATOM target)
{
	int i;
		       ...
		       AppSelectionTargetType *targets, int targetCount)
{
	...
	for (i = 0; i < targetCount; i++) {
		if (!targets[i].asttTargetAtom) {
			targets[i].asttTargetAtom =
				gdk_atom_intern(targets[i].asttTargetString, 0);
	...
	if (!gtk_selection_owner_set(ed->edDocumentWidget.dwWidget,
				     ast->astSelectionAtom, GDK_CURRENT_TIME)) {
		return -1;
	}
...
static int appAskForPaste(APP_WIDGET w, EditApplication *ea,
			    ...
			    void *voided)
{
	...
	if (appGetResponseType(
		    &ast, &astt, &targetFound, ea->eaDocSelectionTypes,
		    ea->eaDocSelectionTypeCount, gsd->selection, gsd->target)) {
		LDEB(1);
		return;
	}
...
void appAllocateCopyPasteTargetAtoms(EditApplication *ea)
{
	...
	for (i = 0; i < ea->eaDocSelectionTypeCount; ast++, i++) {
		if (!ast->astSelectionAtom) {
			ast->astSelectionAtom =
				gdk_atom_intern(ast->astSelectionString, 0);
			if (!ast->astSelectionAtom) {
