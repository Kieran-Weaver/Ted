					...
					int *pChanged, const char *fontName)
{
	...
	if (etaTo->etaFontName) {
		if (!fontName || strcmp(etaTo->etaFontName, fontName)) {
	...
	if (PROPmaskISSET(setMask, TApropTEXT_COLOR)) {
		if (taFrom->taTextColorNumber == 0) {
			...
			if (taFrom->taTextColorNumber < 0 ||
				...
				LLDEB(taFrom->taTextColorNumber,
				      cp->cpColorCount);
			      ...
			      DocumentFontList *dfl, ColorPalette *cp)
{
	...
	if (PROPmaskISSET(setMask, TApropTEXT_COLOR)) {
		...
		if (etaFrom->etaTextColorExplicit) {
			color = utilPaletteInsertColor(
				cp, avoidZero, maxColors,
				&(etaFrom->etaTextColor));
			if (color < 0) {
				LDEB(color);
				color = 0;
	...
	if (PROPmaskISSET(setMask, TApropFONT_NUMBER)) {
		if (!etaFrom->etaFontName) {
			...
			int fontNumber =
				docGetFontByName(dfl, etaFrom->etaFontName);
