...
static void utilMatchPsFacesToDocFont(DocumentFont *df, const PsFontFamily *psf)
{
	int fac;
	...
	if (!df->dfPsFontInfo[0]) {
		for (fac = 0; fac < FONTface_COUNT; fac++) {
			if (df->dfPsFontInfo[fac]) {
				df->dfPsFontInfo[0] = df->dfPsFontInfo[fac];
				break;
			}
		}
	}
			    ...
			    const PostScriptFontList *psfl)
{
	...
	for (fam = 0; fam < psfl->psflFamilyCount; fam++) {
		...
		df = utilAddFontToDocList(dfl, aff->psfFontFamilyName, styleInt,
					  pitch);
		if (!df) {
			SXDEB(aff->psfFontFamilyName, df);
			return -1;
		}
...
int utilAddBase35FontsToDocList(DocumentFontList *dfl)
{
	...
	for (fam = 0; fam < UtilBase35FamilyCount; b35f++, fam++) {
		...
		df = utilAddFontToDocList(dfl, b35f->b35fName, b35f->b35fStyle,
					  pitch);
		if (!df) {
			SXDEB(b35f->b35fName, df);
			return -1;
		}
	}
...
static int docFontSetUnicodeCharsetIdx(int unicode, void *vfc)
{
	const FontCharset *fc = (const FontCharset *)vfc;

	if (utilIndexMappingPut(&(fc->fcFont->dfUnicodeToCharset), unicode,
				fc->fcCharsetIdx)) {
		LLDEB(unicode, fc->fcCharsetIdx);
		return -1;
	}
...
int docFontFindLegacyEncodings(DocumentFont *df)
{
	...
	for (csIdx = 0; csIdx < CHARSETidx_COUNT; csIdx++) {
		...
		hitCount[csIdx] = utilIndexSetIntersect(
			&(legacyUsed[csIdx]), &(df->dfUnicodesUsed), legacySet);
		if (hitCount[csIdx] < 0) {
			LLDEB(csIdx, hitCount[csIdx]);
			rval = -1;
			goto ready;
		}
	...
	while (bestIndex >= 0) {
		...
		for (csIdx = 0; csIdx < CHARSETidx_COUNT; csIdx++) {
			...
			hitsLeft[csIdx] = utilIndexSetRemoveAll(
				&(legacyUsed[csIdx]), &(legacyUsed[csIdx]),
				subtract);
			if (hitsLeft[csIdx] < 0) {
				LLDEB(csIdx, hitsLeft[csIdx]);
				rval = -1;
				goto ready;
			}
	...
	while (bestIndex >= 0) {
		...
		if (utilIndexSetForAll(&(legacyUsed[fc.fcCharsetIdx]),
				       docFontSetUnicodeCharsetIdx,
				       (void *)&fc) < 0) {
			LDEB(fc.fcCharsetIdx);
			rval = -1;
			goto ready;
		}
