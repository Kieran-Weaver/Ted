...
int docFontSetFamilyStyle(DocumentFont *df, int styleInt)
{
...
int docCopyDocumentFont(DocumentFont *to, const DocumentFont *from)
{
	...
	if (utilCopyIndexSet(&(copy.dfUnicodesUsed), &(from->dfUnicodesUsed)) <
	    0) {
		LDEB(1);
		return -1;
	}
	...
	*to = copy;
	docInitDocumentFont(&copy); /* steal */
...
int utilRemoveCharsetFromFontName(DocumentFont *df, int charset)
{
	...
	if (suffix) {
		...
		if (lenN > lenS && !strcmp(df->dfName + lenN - lenS, suffix)) {
