...
static DocumentFont *docFontListClaimFont(DocumentFontList *dfl, int n)
{
	int page = n / DFL_PGSZ;
	DocumentFont *fontPage;
	...
	if (page >= dfl->dflFontPageCount) {
		...
		DocumentFont **fresh = (DocumentFont **)realloc(
			dfl->dflFontPages,
...
int docCopyFontList(DocumentFontList *to, const DocumentFontList *from)
{
	...
	docCleanFontList(to);
	*to = copy;
	docInitFontList(&copy); /* steal contents */
...
static int docFontListFindOpenSlot(const DocumentFontList *dfl)
{
	int page;
...
int docGetFontByName(DocumentFontList *dflTo, const char *fontFamilyName)
{
...
static int utilFontCompareFontIdx(const void *vdfs1, const void *vdfs2)
{
...
static int utilDocumentFontListBuildSortIndex(DocumentFontList *dfl)
{
	...
	dfl->dflFontSortIndex =
		(DocFontSort *)malloc(dfl->dflFontCount * sizeof(DocFontSort));
	if (!dfl->dflFontSortIndex) {
		LXDEB(dfl->dflFontCount, dfl->dflFontSortIndex);
		return -1;
	}
	...
	dflsort = dfl;
	qsort(dfl->dflFontSortIndex, dfl->dflFontCount, sizeof(DocFontSort),
	      utilFontCompareFontIdx);
...
utilDocumentFontListGetFontBySortIndex(DocumentFontList *dfl, int idx)
{
	if (idx < 0 || idx >= dfl->dflFontCount) {
		LLDEB(idx, dfl->dflFontCount);
		return (const DocumentFont *)0;
	}

	if (!dfl->dflFontSortIndex && utilDocumentFontListBuildSortIndex(dfl)) {
		LDEB(1);
		return (const DocumentFont *)0;
	}

	return docFontListGetFontByNumber(
		dfl, dfl->dflFontSortIndex[idx].arrayIndex);
}

int utilDocumentFontListGetSortIndex(DocumentFontList *dfl, int aidx)
{
	if (aidx < 0 || aidx >= dfl->dflFontCount) {
		LLDEB(aidx, dfl->dflFontCount);
		return -1;
	}
