...
static int tedReplaceSelectionWithObject(TedEditOperation *teo, int objectNr,
					 InsertedObject *io)
{
	TextParticule *tp;
				    ...
				    const PictureProperties *pipSet)
{
	...
	if (docUpdPictureProperties(&pipDoneMask, pipTo, pipSetMask, pipSet)) {
		LDEB(1);
		return -1;
	}

	if (PROPmaskISSET(&pipDoneMask, PIPpropPICSCALE_X)) {
		io->ioScaleXSet = io->ioScaleXUsed = pipTo->pipScaleXUsed =
			pipTo->pipScaleXSet;
	}
	if (PROPmaskISSET(&pipDoneMask, PIPpropPICSCALE_Y)) {
		io->ioScaleYSet = io->ioScaleYUsed = pipTo->pipScaleYUsed =
			pipTo->pipScaleYSet;
	}
...
int tedObjectSetImageProperties(EditDocument *ed, InsertedObject *io,
				const DocumentPosition *dpObj, int partObj,
				const PropertyMask *pipSetMask,
				const PictureProperties *pipSet, int traced)
{
	int rval = 0;
			     ...
			     const PictureProperties *pipSet, int traced)
{
	...
	tedObjectSetImageProperties(ed, io, &dpObj, partObj, pipSetMask, pipSet,
				    traced);
...
int tedReplaceSelectionWithRasterImage(EditDocument *ed, RasterImage *ri,
				       int traced)
{
	int rval = 0;
	InsertedObject *io;
...
InsertedObject *tedObjectMakeRasterObject(int *pObjectNumber, EditDocument *ed,
					  BufferItem *bi,
					  const LayoutContext *lc,
					  RasterImage *ri)
{
	...
	if (ri->riFormat >= 0 && !includedAsBitmap &&
	...
	if (ri->riFormat >= 0 && !includedAsBitmap &&
	    (!strcmp(typeId, "jpgFile") || !strcmp(typeId, "jpegFile"))) {
