...
static int tedStartUndoOperation(TedEditOperation *teo, SelectionGeometry *sg,
				 ...
				 const EditStep *es)
{
...

	{
		...
		if (docGetRootOfSelectionScope(&(eo->eoTree), &bodySectNode, bd,
					       &(es->esNewSelectionScope))) {
			LDEB(1);
			return -1;
		}
	...
	docDescribeSelection(sd, &ds, bd, sg->sgHead.pgTopPosition.lpPage,
			     sg->sgTail.pgTopPosition.lpPage, ed->edDocumentId,
			     ed->edFileReadOnly);
...
static int tedUndoFinishOld(TedEditOperation *teo, const EditStep *es)
{
	EditOperation *eo = &(teo->teoEo);
	...
	if (docGetRootOfSelectionScope(&dt, (BufferItem **)0, eo->eoDocument,
...
static int tedUndoReplace(TedEditOperation *teo, EditDocument *ed,
			  const EditStep *es, int posWhere,
			  int copyTailParaProps)
{
	...
	if (!docHeadPosition(&dpFrom, es->esSourceDocument->bdBody.dtRoot)) {
		if (docSetTraceDocumentCopyJob(&dcj, eo,
					       es->esSourceDocument)) {
			LDEB(1);
			rval = -1;
			goto ready;
	...
	case SELposHEAD:
		...
		break;
	case SELposALL:
		tedEditFinishRange(teo, es->esOldCol0, es->esOldCol1,
				   &(es->esOldEditRange));
		break;
	default:
		LDEB(posWhere);
...
static int tedUndoDeletePara(TedEditOperation *teo, EditDocument *ed,
			     const EditStep *es)
{
	int rval = 0;
...
static int tedUndoDeleteColumn(TedEditOperation *teo, EditDocument *ed,
			       const EditStep *es)
{
	int rval = 0;
	...
	if (docDelimitTable(dpFrom.dpNode, &parentFrom, &colFrom, &row0From,
			    &rowFrom, &row1From)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (docDelimitTable(eo->eoHeadDp.dpNode, &parentTo, &colTo, &row0To,
			    &rowTo, &row1To)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (row0From != 0 || row1To - row0To != row1From - row0From) {
		LLLLDEB(row0To, row1To, row0From, row1From);
		rval = -1;
		goto ready;
	}
...
static int tedUndoDeleteRow(TedEditOperation *teo, EditDocument *ed,
			    const EditStep *es)
{
	int rval = 0;
...
static int tedUndoDeleteSect(TedEditOperation *teo, EditDocument *ed,
			     const EditStep *es)
{
	int rval = 0;
...
static int tedUndoInsertNode(TedEditOperation *teo, const EditStep *es,
			     BufferItem *node)
{
	EditOperation *eo = &(teo->teoEo);
	...
	if (node->biLevel == DOClevSECT && node->biTreeType == DOCinBODY &&
	    node->biNumberInParent == node->biParent->biChildCount - 1) {
		eo->eoSelectionScope.ssSectNr = node->biNumberInParent - 1;
	}

	docEditDeleteNodes(eo, &sectionsDeleted, &firstParaDeleted,
			   &paragraphsDeleted, node->biParent,
			   node->biNumberInParent, 1);
...
static int tedUndoInsertPara(TedEditOperation *teo, EditDocument *ed,
			     const EditStep *es)
{
	int rval = 0;
...
static int tedUndoInsertSect(TedEditOperation *teo, EditDocument *ed,
			     const EditStep *es)
{
	int rval = 0;
...
static int tedUndoInsertRow(TedEditOperation *teo, EditDocument *ed,
			    const EditStep *es)
{
	int rval = 0;
...
static int tedUndoInsertColumn(TedEditOperation *teo, EditDocument *ed,
			       const EditStep *es)
{
	int rval = 0;
	...
	if (docDelimitTable(eo->eoHeadDp.dpNode, &parentNode, &col, &row0, &row,
			    &row1)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	for (row = row0; row <= row1; row++) {
		...
		docEditDeleteNodes(eo, &sectionsDeleted, &firstParaDeleted,
				   &paragraphsDeleted, rowNode, col, 1);
...
static int tedUndoInsertHeaderFooter(TedEditOperation *teo, EditDocument *ed,
				     const EditStep *es)
{
	int rval = 0;
...
static int tedUndoDeleteHeaderFooter(TedEditOperation *teo, EditDocument *ed,
				     const EditStep *es)
{
	int rval = 0;
	...
	for (i = 0; i < DOC_HeaderFooterTypeCount; i++) {
		...
		dt = docSectionHeaderFooter(
			sectNodeFrom, (unsigned char *)0,
	...
	/*  3  */
	dtTo->dtRoot = docMakeTreeRoot(eo->eoDocument, dtTo, eo->eoBodySectNode,
				       ownerNumber, dtFrom->dtRoot->biTreeType);
	if (!dtTo->dtRoot) {
		XDEB(dtTo->dtRoot);
		rval = -1;
		goto ready;
...
static int tedUndoUpdateField(TedEditOperation *teo, EditDocument *ed,
			      const EditStep *es)
{
	int rval = 0;
...
static int tedUndoSetField(TedEditOperation *teo, EditDocument *ed,
			   const EditStep *es)
{
	int rval = 0;
	...
	if (docDelimitFieldInDoc(&dsInsideField, &dsAroundField, &headPart,
				 &tailPart, eo->eoDocument, df)) {
		SDEB(docFieldKindStr(df->dfKind));
		rval = -1;
		goto ready;
	}

	if (tedFlattenFieldImpl(teo, (DocumentSelection *)0, &dsAroundField,
...
static int tedUndoDelField(TedEditOperation *teo, EditDocument *ed,
			   const EditStep *es)
{
	int rval = 0;
	...
	if (docEditSurroundTextSelectionByField(
		    eo, &dfTo, &dsInside, &dsAround, &headPart, &tailPart,
		    (PropertyMask *)0, (TextAttribute *)0, dfFrom->dfKind,
...
static int tedUndoUpdPropsImpl(TedEditOperation *teo, const EditStep *es)
{
	int rval = 0;
	EditOperation *eo = &(teo->teoEo);
	...
	docSetRangeSelection(&dsTo, &(eo->eoHeadDp), &(eo->eoTailDp),
			     direction);
	docSelectWholeBody(&dsFrom, es->esSourceDocument);

	if (haveTextUpdate) {
		if (haveNodeUpdate) {
			if (docCopySelectionProperties(
				    &dcj, &dsTo, &dsFrom, (PropertyMask *)0,
...
static int tedUndoUpdProps(TedEditOperation *teo, EditDocument *ed,
			   const EditStep *es)
{
	int rval = 0;
...
static int tedUndoShiftIndent(TedEditOperation *teo, EditDocument *ed,
			      const EditStep *es)
{
	int rval = 0;
...
static int tedUndoUpdObject(TedEditOperation *teo, EditDocument *ed,
			    const EditStep *es)
{
...
static int tedUndoUpdNote(TedEditOperation *teo, EditDocument *ed,
			  const EditStep *es)
{
	int rval = 0;
	...
	if (tedChangeNoteImpl(eo, dfTo, dnTo, selInNote, &(es->esNotePropMask),
...
static int tedUndoUpdList(TedEditOperation *teo, EditDocument *ed,
			  const EditStep *es)
{
	int rval = 0;
...
static int tedUndoSetNewList(TedEditOperation *teo, EditDocument *ed,
			     const EditStep *es)
{
	int rval = 0;
...

	{
		...
		if (docListOverrideTableDeleteOverride(
			    &(la->laListOverrideTable), lo->loIndex)) {
			LDEB(lo->loIndex);
			rval = -1;
		}
	}
...
static int tedUndoShiftRows(TedEditOperation *teo, EditDocument *ed,
			    const EditStep *es)
{
	int rval = 0;
