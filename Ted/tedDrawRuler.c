...
#define HBUTTON_APEX(s) (HBUTTON_BASE(s) + BUTTON_RECTH(s) + BUTTON_TRIH(s))

#define SBUTTON_BASE(s) ((s)-BUTTON_DIST(s))
#define SBUTTON_APEX(s) (SBUTTON_BASE(s) - BUTTON_RECTH(s) - BUTTON_TRIH(s))
			     ...
			     const RulerData *rd, int kind)
{
	...
	if (kind == DOCtaRIGHT || kind == DOCtaCENTER || kind == DOCtaDECIMAL) {
		/*  laying left	*/
...
int tedRulerMakeDrawingSurface(TedTopRuler *ttr, APP_WIDGET w)
{
	...
	appRulerMakeDrawingSurface(rd, vertical, FONT_HEIGHT(rd->rdSizeAcross),
