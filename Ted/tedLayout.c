...
int tedAdjustParagraphLayout(TedEditOperation *teo, DocumentTree *dt)
{
	...
	if (docAdjustParaLayout(paraNode, !eo->eoIBarSelectionOld, stroffFrom,
				stroffShift, stroffUpto, &lj)) {
		LDEB(1);
		return -1;
	}
...
void tedSetScreenLayoutContext(LayoutContext *lc, EditDocument *ed)
{
			   ...
			   const LayoutContext *lc, const EditDocument *ed)
{
	...
	docGetPixelRectForPos(drObject, lc, pg->pgXTwips,
			      pg->pgXTwips + io->ioTwipsWide,
