...
static void tedFormatFillPages(const TedFormatToolResources *tftr,
			       TedFormatTool *tft, AppInspector *ai)
{
#if USE_FONT_TOOL
	appFontToolFillPage(&(tft->tftFontTool), &(tftr->tttrFontToolResources),
			    TEDtsiFONT, &(ai->aiSubjects[TEDtsiFONT]),
			    ai->aiSubjects[TEDtsiFONT].isPage,
			    &(tftr->tttrSubjectResources[TEDtsiFONT]));
#endif

	tedFormatFillTextOrnamentsPage(
		&(tft->tftTextOrnamentsTool),
		&(tftr->tttrTextOrnamentsToolResources), ai, TEDtsiTEXT_ORN,
		&(ai->aiSubjects[TEDtsiTEXT_ORN]),
		ai->aiSubjects[TEDtsiTEXT_ORN].isPage,
		&(tftr->tttrSubjectResources[TEDtsiTEXT_ORN]));

	tedFormatFillParagraphLayoutPage(
		&(tft->tftParagraphLayoutTool),
	...
	tedFormatFillParagraphOrnamentsPage(
		&(tft->tftParagraphOrnamentsTool),
		&(tftr->tttrParaOrnamentsToolResources), ai, TEDtsiPARA_ORN,
	...
	tedFormatFillHeaderFooterPage(
		&(tft->tftHeaderFooterTool), &(tftr->tttrHeadFootToolResources),
		&(ai->aiSubjects[TEDtsiHEADFOOT]),
		ai->aiSubjects[TEDtsiHEADFOOT].isPage,
		&(tftr->tttrSubjectResources[TEDtsiHEADFOOT]));

	tedFormatFillThisNotePage(
		&(tft->tftThisNoteTool), &(tftr->tttrThisNoteToolResources),
...
#if USE_LINK_TOOL
	tedFillLinkTool(&(tft->tftLinkTool), &(tftr->tttrLinkToolResources),
	...
	tedFormatFillFramePage(&(tft->tftFrameTool),
			       &(tftr->tttrFrameToolResources), ai, TEDtsiFRAME,
			       &(ai->aiSubjects[TEDtsiFRAME]),
			       ai->aiSubjects[TEDtsiFRAME].isPage,
			       &(tftr->tttrSubjectResources[TEDtsiFRAME]));
#endif

	tedFormatFillTocPage(&(tft->tftTocTool), &(tftr->tttrTocToolResources),
			     ai, TEDtsiTOC, &(ai->aiSubjects[TEDtsiTOC]),
			     ai->aiSubjects[TEDtsiTOC].isPage,
			     &(tftr->tttrSubjectResources[TEDtsiTOC]));

	appFillSymbolPicker(&(tft->tftSymbolPicker),
			    &(tftr->tttrSymbolPickerResources), ai,
			    TEDtsiSYMBOL, &(ai->aiSubjects[TEDtsiSYMBOL]),
			    ai->aiSubjects[TEDtsiSYMBOL].isPage,
			    &(tftr->tttrSubjectResources[TEDtsiSYMBOL]));

#if USE_FIND_TOOL
	tedFillFindTool(&(tft->tftFindTool), &(tftr->tttrFindToolResources), ai,
			TEDtsiFIND, &(ai->aiSubjects[TEDtsiFIND]),
			ai->aiSubjects[TEDtsiFIND].isPage,
			&(tftr->tttrSubjectResources[TEDtsiFIND]));
#endif

#if USE_SPELL_TOOL
	tedFillSpellTool(&(tft->tftSpellTool), &(tftr->tttrSpellToolResources),
			 ai, TEDtsiSPELL, &(ai->aiSubjects[TEDtsiSPELL]),
...
static void tedFormatToolNotifySubject(AppInspector *ai, int subjectFrom,
				       int subjectTo)
{
...
void tedShowFormatTool(APP_WIDGET option, EditApplication *ea)
{
	AppInspector *ai;
	TedFormatTool *tft;
	...
	if (!gotResources) {
#if USE_FONT_TOOL
		tedFontToolGetResourceTable(
			ea, &(tftr.tttrFontToolResources),
			&(tftr.tttrSubjectResources[TEDtsiFONT]));
#endif

		tedFormatToolGetTextOrnamentsResourceTable(
			ea, &(tftr.tttrTextOrnamentsToolResources),
			&(tftr.tttrSubjectResources[TEDtsiTEXT_ORN]));

		tedFormatToolGetParaLayoutResourceTable(
			ea, &(tftr.tttrParaLayoutToolResources),
			&(tftr.tttrSubjectResources[TEDtsiPARA_LAY]));

#if USE_TABS_TOOL
		tedFormatToolGetTabsResourceTable(
			ea, &(tftr.tttrTabsToolResources),
			&(tftr.tttrSubjectResources[TEDtsiTABS]));
#endif

		tedFormatToolGetParaOrnamentsResourceTable(
			ea, &(tftr.tttrParaOrnamentsToolResources),
			&(tftr.tttrSubjectResources[TEDtsiPARA_ORN]));

#if USE_LIST_TOOL
		tedFormatToolGetListsResourceTable(
			ea, &(tftr.tttrListsToolResources),
			&(tftr.tttrSubjectResources[TEDtsiLISTS]));
#endif

		tedFormatToolGetSectResourceTable(
			ea, &(tftr.tttrSectToolResources),
			&(tftr.tttrSubjectResources[TEDtsiSECT]));

		tedFormatToolGetTableResourceTable(
			ea, &(tftr.tttrTableToolResources),
			&(tftr.tttrSubjectResources[TEDtsiTABLE]));

		tedFormatToolGetRowResourceTable(
			ea, &(tftr.tttrRowToolResources),
			&(tftr.tttrSubjectResources[TEDtsiROW]));

		tedFormatToolGetColumnResourceTable(
			ea, &(tftr.tttrColumnToolResources),
			&(tftr.tttrSubjectResources[TEDtsiCOLUMN]));

		tedFormatToolGetCellResourceTable(
			ea, &(tftr.tttrCellToolResources),
			&(tftr.tttrSubjectResources[TEDtsiCELL]));

#if USE_PAGE_TOOL
		appPageLayoutPageGetResourceTable(
			ea, &(tftr.tttrPageLayoutToolResources),
			&(tftr.tttrSubjectResources[TEDtsiPAGE]));
#endif

		tedFormatToolGetHeaderFooterResourceTable(
			ea, &(tftr.tttrHeadFootToolResources),
			&(tftr.tttrSubjectResources[TEDtsiHEADFOOT]));

		tedFormatToolGetThisNoteResourceTable(
			ea, &(tftr.tttrThisNoteToolResources),
			&(tftr.tttrSubjectResources[TEDtsiTHIS_NOTE]));

		tedFormatToolGetNotesResourceTable(
			ea, &(tftr.tttrNotesToolResources),
			&(tftr.tttrSubjectResources[TEDtsiFOOT_NOTES]));
...
#if USE_LINK_TOOL
		tedLinkToolGetResourceTable(
			ea, &(tftr.tttrLinkToolResources),
			&(tftr.tttrSubjectResources[TEDtsiLINK]));
#endif

#if USE_BOOKMARK_TOOL
		tedBookmarkToolGetResourceTable(
			ea, &(tftr.tttrBookmarkToolResources),
			&(tftr.tttrSubjectResources[TEDtsiBOOKMARK]));
#endif

		tedImageToolGetResourceTable(
			ea, &(tftr.tttrImageToolResources),
			&(tftr.tttrSubjectResources[TEDtsiIMAGE]));

#if USE_FRAME_TOOL
		tedFrameToolGetResourceTable(
			ea, &(tftr.tttrFrameToolResources),
			&(tftr.tttrSubjectResources[TEDtsiFRAME]));
#endif

		tedTocToolGetResourceTable(
			ea, &(tftr.tttrTocToolResources),
			&(tftr.tttrSubjectResources[TEDtsiTOC]));

		appSymbolPickerGetResourceTable(
			ea, &(tftr.tttrSymbolPickerResources),
			&(tftr.tttrSubjectResources[TEDtsiSYMBOL]));

#if USE_FIND_TOOL
		appFindToolGetResourceTable(
			ea, &(tftr.tttrFindToolResources),
			&(tftr.tttrSubjectResources[TEDtsiFIND]));
#endif

#if USE_SPELL_TOOL
		appSpellToolGetResourceTable(
			ea, &(tftr.tttrSpellToolResources),
			&(tftr.tttrSubjectResources[TEDtsiSPELL]));
#endif

		appRgbChooserPageGetResourceTable(
			ea, &(tftr.tttrRgbChooserToolResources),
			&(tftr.tttrSubjectResources[TEDtsiRGB]));
	...
	ai = appMakeInspector(ea, option, tftr.tttrSubjectResources,
			      TEDtsi_COUNT, tedDestroyFormatTool, (void *)tft);
	if (!ai) {
...
#endif
	tedParaOrnamentsToolFillChoosers(
		&(tft->tftParagraphOrnamentsTool),
...
#endif
	tedFormatFillTextOrnamentsChoosers(
		&(tft->tftTextOrnamentsTool),
...
static void tedFormatRefreshToolPages(int *enabled, int *prefs,
				      TedFormatTool *tft, AppInspector *ai,
				      EditDocument *ed, DocumentTree *ei,
				      ...
				      const unsigned char *cmdEnabled)
{
...
#if USE_FONT_TOOL
	tedRefreshFontTool(&(tft->tftFontTool), enabled + TEDtsiFONT,
			   prefs + TEDtsiFONT, ai->aiSubjects + TEDtsiFONT, ds,
			   sd, bd, ed->edDocumentId, cmdEnabled);
#endif

	tedRefreshTextOrnamentsTool(&(tft->tftTextOrnamentsTool),
				    enabled + TEDtsiTEXT_ORN,
				    prefs + TEDtsiTEXT_ORN,
				    ai->aiSubjects + TEDtsiTEXT_ORN, ed, ds, sg,
				    sd, cmdEnabled);

	tedRefreshParaLayoutTool(&(tft->tftParagraphLayoutTool),
				 enabled + TEDtsiPARA_LAY,
				 prefs + TEDtsiPARA_LAY,
				 ai->aiSubjects + TEDtsiPARA_LAY, ds, sg, sd,
				 cmdEnabled);

#if USE_TABS_TOOL
	tedRefreshTabsTool(&(tft->tftTabsTool), enabled + TEDtsiTABS,
			   prefs + TEDtsiTABS, ai->aiSubjects + TEDtsiTABS, ds,
			   sg, sd, bd, cmdEnabled);
#endif
	...
	tedRefreshParaOrnamentsTool(&(tft->tftParagraphOrnamentsTool),
				    ...
				    ai->aiSubjects + TEDtsiPARA_ORN, ds, sg, sd,
				    cmdEnabled);

#if USE_LIST_TOOL
	tedRefreshListTool(&(tft->tftListsTool), enabled + TEDtsiLISTS,
			   prefs + TEDtsiLISTS, ai->aiSubjects + TEDtsiLISTS,
			   ds, sd, bd, cmdEnabled);
#endif

	tedRefreshTableTool(&(tft->tftTableTool), enabled + TEDtsiTABLE,
			    prefs + TEDtsiTABLE, ai->aiSubjects + TEDtsiTABLE,
			    ds, sg, sd, bd, cmdEnabled);

	tedRefreshColumnTool(&(tft->tftColumnTool), enabled + TEDtsiCOLUMN,
			     prefs + TEDtsiCOLUMN,
			     ai->aiSubjects + TEDtsiCOLUMN, ds, sg, sd,
			     cmdEnabled);

	tedRefreshRowTool(&(tft->tftRowTool), enabled + TEDtsiROW,
			  prefs + TEDtsiROW, ai->aiSubjects + TEDtsiROW, ds, sg,
			  sd, bd, cmdEnabled);

	tedRefreshCellTool(&(tft->tftCellTool), enabled + TEDtsiCELL,
			   prefs + TEDtsiCELL, ai->aiSubjects + TEDtsiCELL, ds,
			   sd, bd, cmdEnabled);

	tedRefreshSectionTool(&(tft->tftSectionTool), enabled + TEDtsiSECT,
			      prefs + TEDtsiSECT, ai->aiSubjects + TEDtsiSECT,
			      ds, sd, bd, cmdEnabled);

#if USE_PAGE_TOOL
	tedRefreshPageLayoutTool(&(tft->tftPageLayoutTool),
				 enabled + TEDtsiPAGE, prefs + TEDtsiPAGE,
				 ai->aiSubjects + TEDtsiPAGE, ds, sd, bd,
				 cmdEnabled);
#endif
	...
	tedRefreshHeaderFooterTool(&(tft->tftHeaderFooterTool),
				   ...
				   ai->aiSubjects + TEDtsiHEADFOOT, ds, sd, sg,
				   bd, cmdEnabled);

	tedRefreshThisNoteTool(&(tft->tftThisNoteTool),
			       enabled + TEDtsiTHIS_NOTE,
			       prefs + TEDtsiTHIS_NOTE,
			       ai->aiSubjects + TEDtsiTHIS_NOTE, ds, sd, bd,
			       cmdEnabled);
	tedRefreshNotesTool(&(tft->tftFootNotesTool),
			    enabled + TEDtsiFOOT_NOTES,
			    prefs + TEDtsiFOOT_NOTES,
			    ai->aiSubjects + TEDtsiFOOT_NOTES, ds, sd, bd,
			    cmdEnabled);
	tedRefreshNotesTool(&(tft->tftEndNotesTool), enabled + TEDtsiEND_NOTES,
			    prefs + TEDtsiEND_NOTES,
			    ai->aiSubjects + TEDtsiEND_NOTES, ds, sd, bd,
			    cmdEnabled);

#if USE_LINK_TOOL
	tedRefreshLinkTool(&(tft->tftLinkTool), enabled + TEDtsiLINK,
			   prefs + TEDtsiLINK, ai->aiSubjects + TEDtsiLINK, ds,
			   sd, bd, cmdEnabled);
#endif

#if USE_BOOKMARK_TOOL
	tedRefreshBookmarkTool(&(tft->tftBookmarkTool),
			       enabled + TEDtsiBOOKMARK, prefs + TEDtsiBOOKMARK,
			       ai->aiSubjects + TEDtsiBOOKMARK, ds, sd, ei, bd,
			       cmdEnabled);
#endif

	tedRefreshImageTool(&(tft->tftImageTool), enabled + TEDtsiIMAGE,
			    prefs + TEDtsiIMAGE, ai->aiSubjects + TEDtsiIMAGE,
			    ds, sd, bd, cmdEnabled);

#if USE_FRAME_TOOL
	tedRefreshFrameTool(&(tft->tftFrameTool), enabled + TEDtsiFRAME,
			    prefs + TEDtsiFRAME, ai->aiSubjects + TEDtsiFRAME,
			    ds, sd, bd);
#endif

	tedRefreshTocTool(&(tft->tftTocTool), enabled + TEDtsiTOC,
			  prefs + TEDtsiTOC, ai->aiSubjects + TEDtsiTOC, ds, sd,
			  bd, cmdEnabled);

	tedRefreshSymbolPicker(&(tft->tftSymbolPicker), enabled + TEDtsiSYMBOL,
			       prefs + TEDtsiSYMBOL,
			       ai->aiSubjects + TEDtsiSYMBOL, ds, sd, bd,
			       ed->edDocumentId, cmdEnabled);

#if USE_FIND_TOOL
	tedRefreshFindTool(&(tft->tftFindTool), enabled + TEDtsiFIND,
			   prefs + TEDtsiFIND, ai->aiSubjects + TEDtsiFIND, ds,
			   sd, bd, ed->edDocumentId, cmdEnabled);
#endif

#if USE_SPELL_TOOL
	tedRefreshSpellTool(&(tft->tftSpellTool), enabled + TEDtsiSPELL,
			    prefs + TEDtsiSPELL, ai->aiSubjects + TEDtsiSPELL,
				   ...
				   const unsigned char *cmdEnabled)
{
	...
	if (!ds->dsHead.dpNode) {
		...
		tedFormatRefreshToolPages(enabled, prefs, tft, ai, ed, ei, ds,
					  sg, sd, cmdEnabled);
	...
	*/
	...
	for (subject = 0; subject < TEDtsi_COUNT; subject++) {
		...
		if (enabled[subject] && prefs[subject] > highest) {
			preferred = subject;
			highest = prefs[subject];
		}
	}

	if (ai->aiCurrentSubject < 0 || choosePage ||
	...
	guiEnableWidget(td->tdToolsSpellingOption, enabled[TEDtsiSPELL]);
	guiEnableWidget(td->tdToolsSymbolOption, enabled[TEDtsiSYMBOL]);
}
