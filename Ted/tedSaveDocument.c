...
static int tedRefreshRecalculateFields(int *pReachedBottom,
				       ...
				       TedDocument *td, const LayoutContext *lc)
{
	...
	if (docRecalculateTextLevelFieldsInDocumentTree(
		    rf, &(bd->bdBody), bd->bdBody.dtRoot->biChildren[0],
		    page)) {
		LDEB(1);
		return -1;
	}
...
int tedPrintDocument(SimpleOutputStream *sos, const PrintJob *pj,
		     const PrintGeometry *pg)
{
	...
	if (docPsPrintDocument(sos, pj->pjTitle, ea->eaApplicationName,
			       ea->eaReference, &fontDir, shadingMesh,
			       emitOutline, &lc, pg)) {
		LDEB(1);
		rval = -1;
		goto ready;
					     ...
					     const MemoryBuffer *filename)
{
	...
	if (format == TEDdockindPDF) {
		...
		if (!sosDoc) { /* SXDEB(filename,sosDoc); */
			goto ready;
		}
	} else {
		if (suggestStdout) {
	...
	rval = sosDoc;
	sosDoc = (SimpleOutputStream *)0; /* steal */
...
static int tedSaveDocumentImpl(EditApplication *ea, struct DrawingSurface *ds,
			       ...
			       const MemoryBuffer *filename)
{
	...
	case TEDdockindTEXT_SAVE_WIDE:
		if (docPlainSaveDocument(sos, bd, (const DocumentSelection *)0,
					 0)) {
			LDEB(1);
			return -1;
		}
		    ...
		    const MemoryBuffer *filename, int isDocName)
{
	...
	if (isDocName) {
		...
		if (renameTrace) {
			...
			if (fdTrace < 0 && etNew.etTraceStatus == TRACING_NO) {
				LCDEB(fdTrace, etNew.etTraceStatus);
				rval = -1;
				goto ready;
			}
			if (fdTrace < 0 &&
			    etNew.etTraceStatus == TRACING_EXIST) {
				LCDEB(fdTrace, etNew.etTraceStatus);
				rval = -1;
				goto ready;
			}
	...
	if (tedSaveDocumentImpl(ea, ds, sos, td, format, documentTitle,
				filename)) {
		LDEB(format);
		rval = -1;
		goto ready;
	...
	if (sosMD5) {
		sioOutClose(sosMD5);
		sosMD5 = (SimpleOutputStream *)0;

		utilMD5ToBase64(etNew.etBaseMD5Digest64, digest);
	}

	if (renameTrace && etNew.etTraceStatus == TRACING_ACCES &&
	...
	if (renameTrace) {
		...
		if (etNew.etTraceStatus == TRACING_ACCES) {
			fdTrace =
				docEditTraceTryTemp(&etNew, TedTraceExtension);
		...
		sioFdCloseFile(etNew.etTraceFileHandle);
		etNew.etTraceFileHandle = -1;
		sioFdCloseFile(etOld->etTraceFileHandle);
		etOld->etTraceFileHandle = -1;
		...
		if (docEditTraceOpenTrace(etOld, restart, exclusive) < 0) {
			SDEB(utilMemoryBufferGetString(
				&(etOld->etTraceFileName)));
			rval = -1;
			goto ready;
		}
