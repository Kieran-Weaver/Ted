...
static void tedSectionToolRefreshColumns(SectionTool *st)
{
	...
	if (sp->spColumnCount < 2) {
		...
		appLengthToTextWidget(st->stColwTexts[0],
				      docSectGetColumnWidth((int *)0, (int *)0,
							    sp, 0),
				      st->stUnitInt);
...
static void tedFormatToolRefreshSectionPage(SectionTool *st)
{
	...
	if (sp->spColumnCount >= SECT_MAX_COLUMNS) {
		...
		appSetOptionmenu(&(st->stColumnCountMenu),
				 sp->spColumnCount - 1);
	}
...
static int tedSectToolGetColSpacing(int *pChanged, SectionTool *st, int col,
				    APP_WIDGET w)
{
	SectionProperties *sp = &(st->stPropertiesChosen);
	...
	if (appGetLengthFromTextWidget(w, &value, &changed, st->stUnitInt,
				       minValue, adaptToMin, maxValue,
				       adaptToMax)) {
		return 1;
	}
	if (!changed) {
...
static int tedSectToolGetColWidth(int *pChanged, SectionTool *st, int col,
				  APP_WIDGET w)
{
	SectionProperties *sp = &(st->stPropertiesChosen);
...
static int tedSectToolGetChosen(SectionTool *st)
{
	...
	if (spChosen->spRestartPageNumbers) {
		...
		if (appGetIntegerFromTextWidget(st->stPageRestartText, &n, 1, 0,
						INT_MAX, 0)) {
			return -1;
		}
	...
	if (spChosen->spColumnCount > 1) {
		...
		for (col = 0; col < upto; col++) {
			if (tedSectToolGetColSpacing(&changed, st, col,
						     st->stColsrTexts[col])) {
				return -1;
			}
		}
		...
		for (col = 0; col < upto; col++) {
			if (tedSectToolGetColWidth(&changed, st, col,
						   st->stColwTexts[col])) {
				return -1;
			}
		}
...
static APP_BUTTON_CALLBACK_H(tedFormatChangeSectPushed, w, voidst)
{
	...
	if (tedAppChangeSectionProperties(st->stApplication, &spDifMask,
					  spChosen)) {
		LDEB(1);
	}
...
static void tedSectNumberStyleChosen(int style, void *voidst)
{
	SectionTool *st = (SectionTool *)voidst;
	SectionProperties *sp = &(st->stPropertiesChosen);

	if (style < 0 || style >= DOCpgn_COUNT) {
		LLDEB(style, DOCpgn_COUNT);
		return;
	}
...
static void tedSectBreakKindChosen(int breakKind, void *voidst)
{
	SectionTool *st = (SectionTool *)voidst;
	SectionProperties *sp = &(st->stPropertiesChosen);

	if (breakKind < 0 || breakKind >= DOCibk_COUNT) {
		LLDEB(breakKind, DOCibk_COUNT);
		return;
	}
...
static void tedColumnCountChosen(int val, void *voidst)
{
	SectionTool *st = (SectionTool *)voidst;
	SectionProperties *sp = &(st->stPropertiesChosen);

	if (val < 0 || val >= SECT_MAX_COLUMNS) {
		LLDEB(val, SECT_MAX_COLUMNS);
		return;
	}
...
static void tedSectionToolFillColumnsFrame(SectionTool *st,
					   ...
					   APP_WIDGET frameColumn)
{
	...
	/**************/
	guiToolMakeLabelAndMenuRow(&row, &(st->stColumnCountMenu), &rowLabel,
				   frameColumn, spr->sprColumnCount,
				   tedColumnCountChosen, (void *)st);

	guiToolMake2ToggleRow(
		&row, frameColumn, &(st->stEqualWidthToggle),
		&(st->stLineBetweenColumnsToggle), spr->sprColumnsEqualWidth,
		spr->sprLineBetweenColumns, tedSectEqualWidthToggled,
		tedSectLineBetweenToggled, (void *)st, (void *)st);
	...
	for (col = 0; col < SECT_MAX_COLUMNS; col++) {
		...
		appMakeTextInRow(&(st->stColwTexts[col]), st->stColumnRows[col],
				 1, 2, textColumns, textEnabled);
		appMakeTextInRow(&(st->stColsrTexts[col]),
				 st->stColumnRows[col], 3, 2, textColumns,
				 textEnabled);

		appGuiSetGotValueCallbackForText(st->stColwTexts[col],
						 tedSectColWidthChanged,
						 (void *)st);
		appGuiSetGotValueCallbackForText(st->stColsrTexts[col],
						 tedSectColSpacingChanged,
						 (void *)st);
	}
			      ...
			      const InspectorSubjectResources *isr)
{
	...
	if (st->stUnitInt < 0) {
		SDEB(spr->sprUnitType);
		st->stUnitInt = UNITtyPOINTS;
	}
	...
	/**************/
	guiToolMakeLabelAndTextRow(&(st->stNumberRow), &(st->stNumberLabel),
				   &(st->stNumberText), pageWidget,
				   isr->isrSubjectName, textColumns, 0);

	/**************/
	guiToolMakeLabelAndMenuRow(&(st->stBreakKindRow),
				   &(st->stBreakKindOptionmenu), &rowLabel,
				   pageWidget, spr->sprBreakKind,
				   tedSectBreakKindChosen, (void *)st);
	/**************/
	guiToolMakeLabelAndMenuRow(&(st->stNumberStyleRow),
				   &(st->stNumberStyleOptionmenu), &rowLabel,
				   pageWidget, spr->sprPageNumberStyle,
				   tedSectNumberStyleChosen, (void *)st);
	/**************/
	guiToolMakeToggleAndTextRow(
		&(st->stPageRestartRow), &(st->stPageRestartToggle),
		&(st->stPageRestartText), pageWidget, spr->sprNumberingRestarts,
		tedFormatPageRestartToggled, (void *)st, textColumns, 1);

	/**************/
	appMakeColumnFrameInColumn(&(st->stColumnsFrame), &(st->stColumnsPaned),
	...
	guiToolMake2BottonRow(&row, pageWidget, &(is->isSelectButton),
			      &(is->isDeleteButton), isr->isrSelectButtonText,
			      isr->isrDeleteButtonText, tedFormatSelectSect,
			      tedFormatDeleteSect, (void *)st);

	guiToolMake2BottonRow(&row, pageWidget, &(is->isInsertButton),
			      &(is->isAppendButton), isr->isrInsertButtonText,
			      isr->isrAppendButtonText, tedFormatInsertSect,
			      tedFormatAppendSect, (void *)st);

	guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
			      &(is->isRevertButton), &(is->isApplyButton),
			      isr->isrRevert, isr->isrApplyToSubject,
			      tedFormatRevertSectPushed,
			      tedFormatChangeSectPushed, (void *)st);
				  ...
				  const SectionPageResources *spr)
{
	...
	appFillInspectorMenu(DOCpgn_COUNT, DOCpgnDEC, st->stNumberStyleItems,
			     spr->sprNumberStyleMenuTexts,
			     &(st->stNumberStyleOptionmenu));

	appFillInspectorMenu(DOCibk_COUNT, DOCibkPAGE, st->stBreakKindItems,
			     spr->sprBreakKindMenuTexts,
	...
	appFillInspectorMenu(SECT_MAX_COLUMNS + 1, 1, st->stColumnCountItems,
...
static AppConfigurableResource TED_TedSectionToolResourceTable[] = {
	/**/
	APP_RESOURCE("unit", offsetof(SectionPageResources, sprUnitType),
		     "inch"),
	...
	APP_RESOURCE("formatToolSectNumberStyleDec",
		     offsetof(SectionPageResources,
			      sprNumberStyleMenuTexts[DOCpgnDEC]),
		     "1, 2, 3"),
	APP_RESOURCE("formatToolSectNumberStyleUcrm",
		     offsetof(SectionPageResources,
			      sprNumberStyleMenuTexts[DOCpgnUCRM]),
		     "I, II, III"),
	APP_RESOURCE("formatToolSectNumberStyleLcrm",
		     offsetof(SectionPageResources,
			      sprNumberStyleMenuTexts[DOCpgnLCRM]),
		     "i, ii, iii"),
	APP_RESOURCE("formatToolSectNumberStyleUcltr",
		     offsetof(SectionPageResources,
			      sprNumberStyleMenuTexts[DOCpgnUCLTR]),
		     "A, B, C"),
	APP_RESOURCE("formatToolSectNumberStyleLcltr",
		     offsetof(SectionPageResources,
			      sprNumberStyleMenuTexts[DOCpgnLCLTR]),
		     "a, b, c"),
	/**/
	APP_RESOURCE("formatToolSectBreakKind",
		     offsetof(SectionPageResources, sprBreakKind), "Begins"),
	APP_RESOURCE("formatToolSectBreakKindNone",
		     offsetof(SectionPageResources,
			      sprBreakKindMenuTexts[DOCibkNONE]),
		     "Below Previous"),
	APP_RESOURCE("formatToolSectBreakKindCol",
		     offsetof(SectionPageResources,
			      sprBreakKindMenuTexts[DOCibkCOL]),
		     "In Next Column"),
	APP_RESOURCE("formatToolSectBreakKindPage",
		     offsetof(SectionPageResources,
			      sprBreakKindMenuTexts[DOCibkPAGE]),
		     "On New Page"),
	APP_RESOURCE("formatToolSectBreakKindEven",
		     offsetof(SectionPageResources,
			      sprBreakKindMenuTexts[DOCibkEVEN]),
		     "On Even Page"),
	APP_RESOURCE("formatToolSectBreakKindOdd",
		     offsetof(SectionPageResources,
			      sprBreakKindMenuTexts[DOCibkODD]),
		     "On Odd Page"),
	/**/
	APP_RESOURCE("formatToolSectColumns",
		     offsetof(SectionPageResources, sprColumns), "Columns"),
	APP_RESOURCE("formatToolSectColumnCount",
		     offsetof(SectionPageResources, sprColumnCount), "Columns"),
	APP_RESOURCE("formatToolSectColumn",
		     offsetof(SectionPageResources, sprColumn), "Column"),
	APP_RESOURCE("formatToolSectColumnWidth",
		     offsetof(SectionPageResources, sprColumnWidth), "Width"),
