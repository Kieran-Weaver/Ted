...
{
	utilMemoryBufferPrintf(&(dp->dpGeneratorWrite), "%s (%s);",
...
static int tedSetDocumentDefaults(TedDocument *td, EditApplication *ea)
{
...
int tedNewDocument(EditDocument *ed, const MemoryBuffer *filename)
{
	...
	if (filename) {
		...
		if (traced) {
			fdTrace = docEditTraceTryRelative(et, filename,
							  TedTraceExtension);
	...
	if (docEditIsTraced(et->etTraceStatus)) {
		...
		if (docRtfTraceVersion(EDITcmd_NEW, filename, (char *)0,
				       &(dp->dpRevtim), et)) {
			LDEB(1);
			rval = -1;
			goto ready;
...
static BufferDocument *tedTryRtfStdin(int *pFormat, EditApplication *ea)
{
	TedAppResources *tar = (TedAppResources *)ea->eaResourceData;
	BufferDocument *bd;
...
static BufferDocument *tedTryTxtStdin(int *pFormat, EditApplication *ea)
{
	BufferDocument *bd;
	int longestPara;
	...
	if (longestPara > 76) {
		...
		*pFormat = TEDdockindTEXT_SAVE_FOLDED;
	}

	return bd;
}

static BufferDocument *tedTryRtfFile(unsigned char *digest, int *pFormat,
				     int *pFailedFile, EditApplication *ea,
				     ...
				     APP_WIDGET relative, APP_WIDGET option)
{
	...
	if (!bd) {
		...
		resp = appQuestionRunFilenameOkCancelDialog(
			ea, relative, option, filename, tar->tarFileNotRtf);
...
static BufferDocument *tedTryTxtFile(unsigned char *digest, int *pFormat,
				     int *pFailedFile, EditApplication *ea,
				     const MemoryBuffer *filename, int complain,
				     APP_WIDGET relative, APP_WIDGET option)
{
			...
			APP_WIDGET relative, APP_WIDGET option)
{
	...
	if (!bd && formatHint == TEDdockindRTF) {
		/*  1  */
		bd = tedTryRtfFile(digest, &format, &failedFile, ea, filename,
	...
	if (!bd && formatHint == TEDdockindTEXT_OPEN) {
		bd = tedTryTxtFile(digest, &format, &failedFile, ea, filename,
				   complain, relative, option);
		if (failedFile) {
			rval = -1;
			goto ready;
...
static int tedFindDigest(const TraceStep *ts, const EditStep *es, int step,
			 void *voidfd)
{
	...
	if (es->esCommand != EDITcmd_OPEN && es->esCommand != EDITcmd_SAVE) {
		return 0;
	}
...
static int tedRemebmerTraceStep(const TraceStep *ts, const EditStep *es,
				int step, void *voidfd)
{
	FindDigest *fd = (FindDigest *)voidfd;
	TraceStep *tsNew;
...
static int tedFindDocument(const TraceStep *ts, const EditStep *es, int step,
			   void *voidfd)
{
	int rval = 0;
	BufferDocument *bd = (BufferDocument *)0;
	...
	if (es->esCommand == EDITcmd_OPEN || es->esCommand == EDITcmd_SAVE) {
		...
		if (!tedOpenDocumentFile(digest, &format, &bd, ea, suggestStdin,
					 format, filename, complain,
...
static int tedFindBaseVersion(EditApplication *ea, TedDocument *td)
{
	int rval = 0;
...
static int tedOpenTraceFile(EditApplication *ea, TedDocument *td, int *pFormat,
			    const MemoryBuffer *filename)
{
	...
	fdTrace =
		sioFdOpenFile(filename, read, write, append, create, exclusive);
	if (fdTrace < 0) {
		LDEB(fdTrace);
		rval = -1;
		goto ready;
	...
	if (fd.fdBase >= 0 && fd.fdDocument) {
		...
		*pFormat = fd.fdFormat;
		...
		if (utilMemoryBufferSetString(&mbScratch,
					      tar->tarCannotRecover)) {
			LDEB(1);
			rval = -1;
			goto ready;
		}
		...
		td->tdDocument =
			docPlainReadFile(sisTrace, &longestPara,
					 &(ea->eaDefaultDocumentGeometry));
		sioInClose(sisTrace);
		sisTrace = (SimpleInputStream *)0;
		if (!td->tdDocument) {
...
int tedOpenDocument(EditApplication *ea, void *voidtd, int *pFormat,
		    APP_WIDGET relative, APP_WIDGET option, const int readOnly,
		    const int suggestStdin, const int formatHint,
		    const MemoryBuffer *filename)
{
	int rval = 0;
	...
	if (utilMemoryBufferEqualsString(&ext, TedTraceExtension)) {
		...
		if (traced && !readOnly) {
			...
			if (suggestStdin) {
				fdTrace = docEditTraceTryAnon(
					et, TedTraceExtension);
				if (fdTrace < 0) {
					LDEB(fdTrace);
					rval = -1;
					goto ready;
				...
				fdTrace = docEditTraceTryRelative(
					et, filename, TedTraceExtension);

				if (fdTrace < 0 &&
				    et->etTraceStatus == TRACING_NO) {
					LCDEB(fdTrace, et->etTraceStatus);
					rval = -1;
					goto ready;
				}
...
int tedDocRecover(EditDocument *ed)
{
	...
	if (!sisTrace) {
		LXDEB(et->etTraceFileHandle, sisTrace);
		rval = -1;
		goto ready;
	}
	...
	while (et->etIndex < fd.fdLastEdit) {
		if (tedDocRepeat(ed)) {
			LLDEB(et->etIndex, fd.fdLastEdit);
			rval = -1;
			goto ready;
		}
	}
...
int tedDocUnlock(EditDocument *ed)
{
	...
	if (et->etTraceStatus == TRACING_EXIST) {
		...
		if (docEditTraceOpenTrace(&(td->tdEditTrace), restart,
					  exclusive)) {
			LLDEB(restart, exclusive);
			return -1;
		}
