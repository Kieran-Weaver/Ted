...
static void tedBorderToolShowWide(BorderTool *bt)
{
	...
	if (ebp->ebpStyle != DOCbsNONE) {
		appLengthToTextWidget(bt->btWideText, ebp->ebpPenWideTwips,
				      UNITtyPOINTS);
	} else {
		appStringToTextWidget(bt->btWideText, "");
	}
				       ...
				       const BorderProperties *bpSet)
{
	...
	appColorChooserSuggestPalette(&(bt->btColorChooser), avoidZero,
				      dp->dpColorPalette);
	appColorChooserSetColor(&(bt->btColorChooser), ebp->ebpColorExplicit,
				&(ebp->ebpColor));
			       ...
			       const DocumentRectangle *drI)
{
	if (style != DOCbsNONE) {
		...
		tedDrawHorizontalBorderLine(ds, style, minThick, drI->drX0,
					    drI->drX1,
					    (drI->drY0 + drI->drY1) / 2);
	}
...
static APP_TOGGLE_CALLBACK_H(tedBorderToolOnOffToggled, w, voidbt, voidtbcs)
{
	...
	if (set) {
		...
		if (ebp->ebpPenWideTwips == 0) {
			...
			PROPmaskADD(&(bt->btPropertiesChanged),
				    BRDRpropPEN_WIDE);
			/*changed= 1;*/
		}
	} else {
...
static APP_EVENT_HANDLER_H(tedBorderToolClickedPulldown, w, voidbt, mouseEvent)
{
	...
	/*  1  */
	if (appGuiDrawnPulldownGetStrip(&style, &(bt->btStylePulldown), w,
					mouseEvent)) {
		return;
	}
...
static APP_TXACTIVATE_CALLBACK_H(tedFormatBorderWidthChanged, w, voidbt)
{
	...
	if (appGetLengthFromTextWidget(bt->btWideText, &wide, &changed,
...
static void tedBorderColorChosen(ColorChooser *cc, int which, void *voidbt,
				 int choice, const RGB8Color *rgb8Set)
{
	...
	if (choice == CHOICEccMORE) {
		appInspectorShowRgbPage(bt->btInspector, bt->btSubjectPage,
					which, &(ebp->ebpColor));
		return;
	}
...
void tedMakeBorderTool(BorderTool *bt, AppInspector *ai, APP_WIDGET pageWidget,
		       const char *title, const BorderToolResources *btr,
		       int subjectPage, int which)
{
	...
	/**/
	bt->btOnOffRow = appMakeRowInColumn(rowParent, ONOFF_ROW_COLUMNS,
					    heightResizable);

#if USE_TOGGLE_TITLE
	bt->btOnOffToggle = appMakeToggleInRow(bt->btOnOffRow, title,
					       tedBorderToolOnOffToggled,
					       (void *)bt, ONOFF_COLUMN,
					       ONOFF_COLSPAN);
#else
	...
	appMakeTextInRow(&(bt->btWideText), bt->btOnOffRow, THICK_COLUMN,
			 THICK_COLSPAN, THICK_CHARS, textEnabled);

	appGuiSetGotValueCallbackForText(
		bt->btWideText, tedFormatBorderWidthChanged, (void *)bt);
	...
	appMakeColorChooserInRow(&(bt->btColorChooser),
				 bordersHaveAutomaticColor, bt->btColorRow,
	...
	if (bt->btStyleRow) {
		...
		appMakeDrawnPulldownInRow(&(bt->btStylePulldown),
					  ...
					  bt->btStyleRow, valueColumn,
					  valueColspan, (void *)bt);
	}
...
void tedFinishBorderTool(BorderTool *bt, const PostScriptFontList *psfl)
{
	if (bt->btStyleRow) {
		...
		appGuiSetDrawnPulldownStrips(&(bt->btStylePulldown),
					     DOCbs_COUNT);
	}

	if (bt->btColorLabel) {
		appFinishColorChooser(&(bt->btColorChooser), psfl,
				      bt->btColorLabel);
	} else {
		appFinishColorChooser(&(bt->btColorChooser), psfl,
				      bt->btOnOffToggle);
				      ...
				      DocumentProperties *dp)
{
	...
	if (PROPmaskISSET(&(bt->btPropertiesChanged), BRDRpropPEN_WIDE) &&
		...
		if (appGetLengthFromTextWidget(
			    bt->btWideText, &wide, &changed, UNITtyPOINTS,
