...
static int tedSplitParaNode(TedEditOperation *teo, int splitLevel,
			    int onNewPage)
{
	...
	/*  3  */
	geoUnionRectangle(&(teo->teoChangedRect), &(teo->teoChangedRect),
			  &drPara);
...
void tedDocSplitParagraph(EditDocument *ed, int onNewPage, int traced)
{
...
int tedDocInsertSectBreak(EditDocument *ed, int traced)
{
	int rval = 0;
	...
	if (docFindParticuleOfPosition(&part, &partFlags, &(eo->eoHeadDp),
				       PARAfindFIRST)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if ((partFlags & POSflagPARA_HEAD) &&
		...
		docSetEditPosition(&(eo->eoReformatRange.erHead),
				   &(eo->eoHeadDp));
	...
	if (tedEditStartReplace(&dsTraced, &teo, EDITcmdSPLIT_SECT, DOClevSPAN,
				0)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (!(partFlags & (POSflagPARA_HEAD | POSflagPARA_TAIL))) {
		...
		if (tedEditInsertSection(&dpBefore, &dpAfter, &teo, split,
					 after)) {
			LDEB(after);
			rval = -1;
			goto ready;
