...
static void tedDrawSetBorderColor(DrawingContext *dc, ScreenDrawingData *sdd,
				  int colorNumber, int asGrid)
{
	if (asGrid) {
		     ...
		     struct DrawingContext *dc)
{
		...
		if (!dc->dcClipRect ||
			...
			if (is.isPattern != DOCspSOLID) {
				...
				if (bo->boShading.isForeColor == 0) {
					...
					rgb8.rgb8Red =
						((100 - level) * rgb8.rgb8Red) /
						100;
					rgb8.rgb8Green = ((100 - level) *
							  rgb8.rgb8Green) /
							 100;
					rgb8.rgb8Blue = ((100 - level) *
							 rgb8.rgb8Blue) /
							100;
					isFilled = 1;
				} else {
					...
					if (docGetSolidRgbShadeOfItem(&isFilled,
								      &rgb8, bd,
								      &is)) {
						LDEB(1);
					}
	...
	if (PROPmaskISSET(&(bo->boPropMask), ORNdrawTOP_BORDER) ||
		...
		if (!dc->dcClipRect ||
		    geoIntersectRectangle(&drBorder, &drBorder,
					  dc->dcClipRect)) {
			    ...
			    int pattern, const rect *drShade)
{
	if (!shadingPixmaps[pattern]) {
		...
		if (t > 0) {
			drawSetLineAttributes(shadingPixmaps[pattern], t,
					      LineStyleSolid, LineCapProjecting,
					      LineJoinMiter, (unsigned char *)0,
					      0);
		}
		...
		case PSshdDKHORIZ:
			y = 0;
			while (y < high) {
				drawLine(shadingPixmaps[pattern], 0, y + s,
					 wide - 1, y + s);
				y += d;
			}
			break;
		...
		case PSshdDKVERT:
			x = 0;
			while (x < wide) {
				drawLine(shadingPixmaps[pattern], x + s, 0,
					 x + s, high - 1);
				x += d;
			}
			break;
		...
		case PSshdDKBDIAG:
			x = 0;
			while (x < wide + high) {
				drawLine(shadingPixmaps[pattern], x, 0,
					 x - high, high);
				x += d;
			}
			break;
...
static void tedBorderSetLineSolid(struct DrawingSurface *ds, int thick)
{
	drawSetLineAttributes(ds, thick, LineStyleSolid, LineCapButt,
			      LineJoinMiter, (unsigned char *)0, 0);
}

static void tedBorderSetLineDashes(struct DrawingSurface *ds,
				   const unsigned char *dashes, int dashCount,
				   int thick)
{
	drawSetLineAttributes(ds, thick, LineStyleSingleDash, LineCapButt,
			      LineJoinMiter, dashes, dashCount);
}
				 ...
				 int x0, int x1, int y)
{
	...
	case DOCbsDOT:
		tedBorderSetLineDashes(ds, TED_Border_DOT,
				       sizeof(TED_Border_DOT), 1);
		drawLine(ds, x0, y, x1, y);
		break;

	case DOCbsDASH:
		tedBorderSetLineDashes(ds, TED_Border_DASH,
				       sizeof(TED_Border_DASH), 1);
		drawLine(ds, x0, y, x1, y);
		break;

	case DOCbsDASHD:
		tedBorderSetLineDashes(ds, TED_Border_DASHD,
				       sizeof(TED_Border_DASHD), 1);
		drawLine(ds, x0, y, x1, y);
		break;

	case DOCbsDASHDD:
		tedBorderSetLineDashes(ds, TED_Border_DASHDD,
				       sizeof(TED_Border_DASHDD), 1);
		drawLine(ds, x0, y, x1, y);
		break;
...
#define W 2
#define W2 4
		drawLine(ds, xx, y, xx + W, y + W);
		xx += W;
		while (xx < x1) {
			drawLine(ds, xx, y + W, xx + W2, y - W);
			xx += W2;
			drawLine(ds, xx, y - W, xx + W2, y + W);
			xx += W2;
		}
	...
	case DOCbsWAVYDB:
		...
		yy = y - 2;
		xx = x0;
		drawLine(ds, xx, yy, xx + W, yy + W);
		xx += W;
		while (xx < x1) {
			drawLine(ds, xx, yy + W, xx + W2, yy - W);
			xx += W2;
			drawLine(ds, xx, yy - W, xx + W2, yy + W);
			xx += W2;
		}
	...
	case DOCbsDASHSM:
		tedBorderSetLineDashes(ds, TED_Border_DASHSM,
				       sizeof(TED_Border_DASHSM), 1);
		drawLine(ds, x0, y, x1, y);
		break;
