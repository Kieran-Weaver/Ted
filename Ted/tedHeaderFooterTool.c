...
static void tedFormatToolRefreshHeadFootPage(HeaderFooterTool *hft)
{
	...
	if (hft->hftPagesChosen < 0 || hft->hftPagesChosen >= PAGES__COUNT ||
				...
				const unsigned char *cmdEnabled)
{
	...
	if (docUpdDocumentProperties((PropertyMask *)0,
				     &(hft->hftDocPropertiesChosen), &dpSetMask,
				     dpSet, dam0)) {
		LDEB(1);
		return;
	}
	if (docUpdDocumentProperties((PropertyMask *)0,
				     &(hft->hftDocPropertiesSet), &dpSetMask,
				     dpSet, dam0)) {
		LDEB(1);
		return;
	}
	...
	if (docUpdSectProperties((PropertyMask *)0,
				 &(hft->hftSectPropertiesChosen), &spSetMask,
				 spSet)) {
		LDEB(1);
		return;
	}
	if (docUpdSectProperties((PropertyMask *)0,
				 &(hft->hftSectPropertiesSet), &spSetMask,
				 spSet)) {
		LDEB(1);
		return;
	}
	...
	for (i = 0; i < PAGES__COUNT; i++) {
		...
		if (docSectionHasHeaderFooter(
			    bodySectNode, &(hft->hftPagesHeaderApplies[i]),
			    &(hft->hftDocPropertiesSet), DOC_HeaderScopes[i])) {
			hft->hftPagesHeaderExists[i] = 1;
		}
		if (docSectionHasHeaderFooter(
			    bodySectNode, &(hft->hftPagesFooterApplies[i]),
			    &(hft->hftDocPropertiesSet), DOC_FooterScopes[i])) {
	...
	if (sd->sdInHeaderFooter) {
		...
		hft->hftPagesOptionsEnabled[PAGES_SUBSEQUENT_PAGES] =
			spSet->spHasTitlePage && !dpSet->dpHasFacingPages;
	...
	if (sd->sdInHeaderFooter) {
		...
		pages = docWhatPagesForHeaderFooter(dpSet, spSet,
						    whatHeaderFooter);
		if (pages < 0) {
			LLDEB(whatHeaderFooter, pages);
			*pEnabled = 0;
			return;
		}
		...
		appSetOptionmenu(&(hft->hftPagesOptionmenu),
				 hft->hftPagesChosen);
	}
...
static APP_BUTTON_CALLBACK_H(tedHeadFootRevertDocPushed, w, voidhft)
{
	...
	if (docUpdDocumentProperties((PropertyMask *)0, dp, &dpSetMask,
				     &(hft->hftDocPropertiesSet), dam0)) {
		LDEB(1);
		return;
	}
...
static void tedHeadFootPagesChosen(int pages, void *voidhft)
{
	HeaderFooterTool *hft = (HeaderFooterTool *)voidhft;

	if (pages < 0 || pages >= PAGES__COUNT) {
		LLDEB(pages, PAGES__COUNT);
		return;
	}
...
static APP_BUTTON_CALLBACK_H(tedHeadFootDeleteHeaderPushed, w, voidhft)
{
	...
	if (hft->hftPagesChosen < 0 || hft->hftPagesChosen >= PAGES__COUNT) {
		LLDEB(hft->hftPagesChosen, PAGES__COUNT);
		return;
	}
				   ...
				   const InspectorSubjectResources *isr)
{
	...
	appMakeColumnFrameInColumn(&(hft->hftSectionFrame),
				   &(hft->hftSectionPaned), pageWidget,
				   hfpr->hfprSectionHeaderText);

	guiToolMakeLabelAndTextRow(&row, &(hft->hftSectionNumberLabel),
				   &(hft->hftSectionNumberText),
				   hft->hftSectionPaned,
				   hfpr->hfprSectionNumberText, textColumns, 0);
	...
	hft->hftTitlepgToggle = appMakeToggleInRow(row, hfpr->hfprTitlepgText,
						   tedHeaderFooterToggleTitlepg,
						   (void *)hft, 0, 1);

	guiToolMake2BottonRow(&(is->isApplyRow), hft->hftSectionPaned,
			      &(is->isRevertButton), &(is->isApplyButton),
			      isr->isrRevert, isr->isrApplyToSubject,
			      tedHeadFootRevertSectPushed,
			      tedHeadFootChangeSectPushed, (void *)hft);

	/**************/

	appMakeColumnFrameInColumn(&(hft->hftDocumentFrame),
				   &(hft->hftDocumentPaned), pageWidget,
				   hfpr->hfprDocumentHeaderText);
	...
	hft->hftFacingpToggle = appMakeToggleInRow(row, hfpr->hfprFacingpText,
						   tedHeaderFooterToggleFacingp,
						   (void *)hft, 0, 1);

	guiToolMake2BottonRow(&row, hft->hftDocumentPaned,
			      &(hft->hftRevertDocButton),
			      &(hft->hftChangeDocButton),
			      hfpr->hfprRevertDocText, hfpr->hfprChangeDocText,
			      tedHeadFootRevertDocPushed,
			      tedHeadFootChangeDocPushed, (void *)hft);

	/**************/

	appMakeColumnFrameInColumn(&(hft->hftPagesFrame), &(hft->hftPagesPaned),
				   pageWidget, hfpr->hfprPagesHeaderText);

	appMakeOptionmenuInColumn(&(hft->hftPagesOptionmenu),
				  hft->hftPagesPaned, tedHeadFootPagesChosen,
				  (void *)hft);

	guiToolMake2BottonRow(
		&row, hft->hftPagesPaned, &(hft->hftDeleteHeaderButton),
		&(hft->hftEditHeaderButton), hfpr->hfprDeleteHeaderText,
		hfpr->hfprEditHeaderText, tedHeadFootDeleteHeaderPushed,
		tedHeadFootEditHeaderPushed, (void *)hft);

	guiToolMake2BottonRow(
		&row, hft->hftPagesPaned, &(hft->hftDeleteFooterButton),
		&(hft->hftEditFooterButton), hfpr->hfprDeleteFooterText,
		hfpr->hfprEditFooterText, tedHeadFootDeleteFooterPushed,
		tedHeadFootEditFooterPushed, (void *)hft);
...
void tedFormatFillHeaderFooterChoosers(HeaderFooterTool *hft,
				       const HeaderFooterPageResources *hfpr)
{
	appFillInspectorMenu(PAGES__COUNT, PAGES_ALL_PAGES, hft->hftPagesItems,
			     hfpr->hfprPagesOptionTexts,
