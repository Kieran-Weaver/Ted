...
extern int tedDrawDrawingShape(const DocumentRectangle *drTwips, int page,
			       struct DrawingShape *ds, DrawingContext *dc,
			       void *vsdd);

extern int tedDrawTextReverse(struct BufferItem *paraBi, int line,
			      const ParagraphFrame *pf,
			      const DocumentRectangle *drLine, void *vsdd,
			      DrawingContext *dc, const BlockOrigin *bo);

extern int tedDrawTextSelected(struct BufferItem *paraBi, int line,
			       const ParagraphFrame *pf,
			       const DocumentRectangle *drLine, void *vsdd,
			       DrawingContext *dc, const BlockOrigin *bo);

extern int tedDrawTextLine(struct BufferItem *paraBi, int line,
			   const ParagraphFrame *pf,
			   const DocumentRectangle *drLine, void *vsdd,
			   DrawingContext *dc, const BlockOrigin *bo);
...
extern void tedDrawFreePixmap(struct DrawingSurface *sp);

extern int tedDrawIBar(const DocumentRectangle *drPixels,
		       const LayoutContext *lc);

extern void tedGetIBarRect(DocumentRectangle *drPixels,
			   const PositionGeometry *pg, const LayoutContext *lc);

extern int tedDrawObject(const DrawTextLine *dtl, int part, InsertedObject *io,
			 int x0Twips, int x1Twips,
			 const LayoutPosition *baseLine);

extern void tedDrawShadedRectangle(const LayoutContext *lc,
				   struct DrawingSurface *shadingPixmaps, int pattern,
				   const DocumentRectangle *drShade);

extern void tedDrawHorizontalBorderLine(struct DrawingSurface *ds, int style,
...
extern int tedDrawChftnsep(const DrawTextLine *dtl, int part, int textAttrNr,
			   const TextAttribute *ta, int x0Twips, int x1Twips,
			   const LayoutPosition *baseLine);

extern int tedDrawSpan(const DrawTextLine *dtl, int part, int count,
		       const LayoutPosition *baseLine, int textAttrNr,
		       const TextAttribute *ta, const char *printString,
		       int nbLen);

extern int tedDrawRunUnderline(const DrawTextLine *dtl, int part, int upto,
			       int textAttrNr, const TextAttribute *ta,
			       int x0Twips, int x1Twips,
			       const LayoutPosition *baseLine);

extern int tedDrawRunStrikethrough(const DrawTextLine *dtl, int part, int upto,
				   int textAttrNr, const TextAttribute *ta,
				   int x0Twips, int x1Twips,
				   const LayoutPosition *baseLine);

extern int tedDrawOrnaments(const BlockOrnaments *bo, int page,
			    const DocumentRectangle *drOutside,
			    const DocumentRectangle *drInside, void *through,
			    struct DrawingContext *dc);

extern int tedDrawSetColorRgb(DrawingContext *dc, void *vsdd,
