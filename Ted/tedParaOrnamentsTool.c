...
static void tedFormatToolRefreshParaOrnamentsPage(ParagraphOrnamentsTool *pot)
{
	...
	tedSetShadingToolByNumber(&(pot->potShadingTool), bd,
				  pp->ppShadingNumber);
...
static void tedParaOrnamentsGotColor(void *voidpot, int which,
				     const RGB8Color *rgb8)
{
	...
	case PPpropTOP_BORDER:
		tedBorderSetExplicitColorChoice(&(pot->potTopBorderTool), rgb8);
		break;
	case PPpropBOTTOM_BORDER:
		tedBorderSetExplicitColorChoice(&(pot->potBottomBorderTool),
						rgb8);
		break;
	case PPpropLEFT_BORDER:
		tedBorderSetExplicitColorChoice(&(pot->potLeftBorderTool),
						rgb8);
		break;
	case PPpropRIGHT_BORDER:
		tedBorderSetExplicitColorChoice(&(pot->potRightBorderTool),
						rgb8);
		break;
	...
	case PPprop_SHADE_FORE_COLOR:
		tedShadeSetExplicitColorChoice(&(pot->potShadingTool), which,
					       rgb8);
		break;
...
static void
tedParaOrnamentsToolMakeBorderFrame(ParagraphOrnamentsTool *pot,
				    ...
				    APP_WIDGET pageWidget)
{
	appMakeColumnFrameInColumn(&(pot->potBordersFrame),
				   &(pot->potBordersPaned), pageWidget,
				   popr->poprBorders);
	...
	tedMakeBorderTool(&(pot->potBottomBorderTool), ai, pot->potBordersPaned,
			  popr->poprBottomBorder,
			  &(popr->poprBorderToolResources), subjectPage,
			  PPpropBOTTOM_BORDER);
...
void tedFormatFillParagraphOrnamentsPage(
	...
	int subjectPage, InspectorSubject *is, APP_WIDGET pageWidget,
	const InspectorSubjectResources *isr)
{
	...
	/**/
	tedParaOrnamentsToolMakeBorderFrame(pot, popr, ai, subjectPage,
					    pageWidget);

	/**/
	tedFormatMakeShadingTool(&(pot->potShadingTool), ai, pageWidget,
				 /* popr->poprShading, */ (const char *)0,
				 &(popr->poprShadingResources), subjectPage,
				 PPprop_SHADE_FORE_COLOR,
				 PPprop_SHADE_BACK_COLOR,
	...
	guiToolMake2BottonRow(&row, pageWidget, &(is->isSelectButton),
			      ...
			      isr->isrDeleteButtonText, tedFormatSelectPara,
			      tedFormatDeletePara, pot);

	guiToolMake2BottonRow(&row, pageWidget, &(is->isInsertButton),
			      &(is->isAppendButton), isr->isrInsertButtonText,
			      isr->isrAppendButtonText, tedFormatInsertPara,
			      tedFormatAppendPara, pot);

	guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
			      &(is->isRevertButton), &(is->isApplyButton),
			      isr->isrRevert, isr->isrApplyToSubject,
			      tedFormatParaRevertPushed, tedFormatChangePara,
			      pot);
...
static AppConfigurableResource TED_TedParagraphToolResourceTable[] = {
	...
	APP_RESOURCE("formatToolParaBorderColorChooserAutomatic",
		     offsetof(ParagraphOrnamentsPageResources,
			      poprBorderToolResources.btrColorChooserResources
				      .ccrAutomaticColor),
		     "Automatic"),
	...
	APP_RESOURCE("formatToolParaShadingBackColorChooserNoBackground",
		     offsetof(ParagraphOrnamentsPageResources,
			      poprShadingResources.strBackColorChooserResources
				      .ccrAutomaticColor),
		     "No Background"),
	...
	APP_RESOURCE("formatToolParaShadingForeColorChooserAutomatic",
		     offsetof(ParagraphOrnamentsPageResources,
			      poprShadingResources.strForeColorChooserResources
				      .ccrAutomaticColor),
		     "Automatic"),
...
void tedFormatToolGetParaOrnamentsResourceTable(
	EditApplication *ea, ParagraphOrnamentsPageResources *popr,
