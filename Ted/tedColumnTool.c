...
static void tedFormatToolRefreshColumnPage(ColumnTool *ct)
{
	...
	appIntervalToTextWidget(ct->ctColumnText, tr->trCol0 + 1,
				tr->trCol1 + 1);
			  ...
			  const unsigned char *cmdEnabled)
{
	...
	ct->ctBlockFrameWide =
		sg->sgHead.pgBlockFrameX1 - sg->sgHead.pgBlockFrameX0;
	...
	guiEnableWidget(is->isNextButton, ct->ctTableRectangle.trCol1 <
...
static int tedFormatToolGetColumnWidth(ColumnTool *ct)
{
	...
	/*  2  */
	if (appGetLengthFromTextWidget(ct->ctWidthText, &width, &changed,
				       UNITtyPOINTS, minValue, adaptToMin,
				       maxValue, adaptToMax)) {
		return -1;
	}

	if (changed) {
		int left = ct->ctPropertiesSet.cpRightBoundaryTwips -
			   ct->ctWidthSet;
...
static int tedColumnToolGetChosen(ColumnTool *ct, BufferDocument *bd)
{
	CellProperties *cpChosen = &(ct->ctPropertiesChosen);
	int res;
...
static APP_TXACTIVATE_CALLBACK_H(tedColumnWidthChanged, w, voidct)
{
	...
	if (!tedFormatToolGetColumnWidth(ct)) {
		appLengthToTextWidget(ct->ctWidthText, ct->ctWidthChosen,
				      UNITtyPOINTS);
	}
}
...
static void tedColumnToolGotColor(void *voidct, int which,
...
static void tedColumnToolMakeBorderFrame(ColumnTool *ct,
			     ...
			     const InspectorSubjectResources *isr)
{
	...
	/**/
	guiToolMakeLabelAndTextRow(&(ct->ctColumnRow), &(ct->ctColumnLabel),
				   &(ct->ctColumnText), pageWidget,
				   isr->isrSubjectName, textColumns, 0);

	guiToolMakeLabelAndTextRow(&(ct->ctWidthRow), &(ct->ctWidthLabel),
				   &(ct->ctWidthText), pageWidget,
				   cpr->cprWidth, textColumns, 1);

	appGuiSetGotValueCallbackForText(ct->ctWidthText, tedColumnWidthChanged,
					 (void *)ct);
...
static AppConfigurableResource TED_TedColumnToolResourceTable[] = {
	APP_RESOURCE("tableToolWidth", offsetof(ColumnPageResources, cprWidth),
		     "Width"),

	/**/
	APP_RESOURCE("tableToolColumnBorders",
		     offsetof(ColumnPageResources, cprBorders), "Borders"),
	APP_RESOURCE("tableToolColumnBorderLeft",
		     offsetof(ColumnPageResources, cprLeftBorder), "Left"),
	APP_RESOURCE("tableToolColumnBorderRight",
		     offsetof(ColumnPageResources, cprRightBorder), "Right"),

	/**/
	APP_RESOURCE("formatToolColumnBorderWidth",
		     offsetof(ColumnPageResources,
