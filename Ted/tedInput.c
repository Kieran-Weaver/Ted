...
static void tedInputChangeSelection(EditDocument *ed, unsigned int keyState,
				    const DocumentSelection *dsOrig,
				    const DocumentPosition *dpSet, int lastLine)
{
...
static int tedInputSetIBarDelete(EditDocument *ed, int headAtLineHead,
...
static int tedMoveRightOnKey(DocumentPosition *dpNew, BufferDocument *bd,
			     int keyState)
{
...
static int tedMoveLeftOnKey(DocumentPosition *dpNew, BufferDocument *bd,
			    int keyState)
{
	if (keyState & KEY_CONTROL_MASK) {
...
void tedDocGotKey(void *voided, int keySym, unsigned int state)
{
	...
	headAfterBreak =
		(sg.sgHead.pgPositionFlags & POSflagPART_AFTER_BREAK) != 0;
	/*
...
shiftTab:
...

		{
			...
			tedEditInsertSpecialParticule(ed, DOCkindTAB,
						      EDITcmdREPLACE,
						      redoLayout, traced);
		}
	...
	case KEY_Delete:
		...
		if (sd.sdIsIBarSelection) {
			...
			if ((ds.dsHead.dpNode->biParaTableNesting > 0 ||
				...
				tedInputChangeSelection(ed, keyState, &ds,
							&dpNew, lastLine);
				goto ready;
			}

			tedInputSetIBarDelete(ed, headAtLineHead, &(ds.dsHead),
					      &dpNew);
	...
	case KEY_BackSpace:
		...
		if (sd.sdIsIBarSelection) {
			...
			tedInputSetIBarDelete(ed, headAtLineHead, &dpNew,
					      &(ds.dsTail));
	...
	case KEY_Home:
...

		{
			...
			tedInputChangeSelection(ed, state, &ds, &dpNew,
						lastLine);
		}
		goto ready;
	...
	case KEY_KP_Left:
	case KEY_Left:
		if (!state && sd.sdIsIBarSelection && ds.dsHead.dpStroff > 0 &&
		    headAtLineHead && !headAfterBreak && !headAtLineTail) {
			const int lastLine = 0;

			/*  To same position on previous line */
			tedInputChangeSelection(ed, state, &ds, &(ds.dsHead),
						lastLine);
			goto ready;
		}
		...
		if (state != 0 || !sd.sdIsListBullet || sd.sdIsIBarSelection) {
			if (tedMoveLeftOnKey(&dpNew, bd, state)) {
				goto ready;
			}
		}
	...
	case KEY_KP_Right:
	case KEY_Right:
		if (!state && sd.sdIsIBarSelection &&
		    ds.dsHead.dpStroff < docParaStrlen(ds.dsHead.dpNode) &&
		    headAtLineTail && !headAtLineHead) {
			const int lastLine = 1;
	...
	case KEY_Prior:
		...
		if (dpNew.dpNode->biTreeType != DOCinBODY) {
			...
			if (!docPrevSimilarRoot(&dpNew, &page, &column, bd)) {
				...
				if (docGetTreeOfNode(&dtPrev, &bodySectNode, bd,
						     dpNew.dpNode)) {
					LDEB(1);
					goto ready;
				}

				if (dtPrev && dtPrev->dtPageSelectedUpon < 0 &&
				    page >= 0) {
...

		{
			...
			while (page >= 0) {
				...
				res = docGetTopOfColumn(&dpNew, &lineNew,
							&partNew, bd, page, 0);
				if (res < 0) {
					LDEB(res);
					goto ready;
	...
	case KEY_Next:
		...
		if (dpNew.dpNode->biTreeType != DOCinBODY) {
			...
			if (!docNextSimilarRoot(&dpNew, &page, &column, bd)) {
				...
				if (docGetTreeOfNode(&dtNext, &bodySectNode, bd,
						     dpNew.dpNode)) {
					LDEB(1);
					goto ready;
				}

				if (dtNext && dtNext->dtPageSelectedUpon < 0 &&
				    page >= 0) {
...

		{
			...
			res = docGetBottomOfColumn(&dpNew, &partNew, bd, page,
						   0);
			if (res == 0) {
				tedInputChangeSelection(ed, state, &ds, &dpNew,
							lastLine);
	...
	case KEY_KP_Insert:
		...
		if (tedHasIBarSelection(ed)) {
			tedSetSelectionLow(ed, &ds, headAtLineHead, (int *)0,
					   (int *)0);
		}
		break;
...
void tedDocGotString(void *voided, const char *str, int length)
{
	EditDocument *ed = (EditDocument *)voided;
