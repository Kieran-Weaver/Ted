			 ...
			 const InsertedObject *io, const LayoutContext *lc)
{
	...
	static const int font_cursors[RESIZE_COUNT] = {
		...
		GDK_LEFT_SIDE,		 GDK_RIGHT_SIDE,

		GDK_TOP_LEFT_CORNER,	 GDK_TOP_SIDE,
	...
	if (!td->tdObjectWindow) {
		...
		for (i = 0; i < RESIZE_COUNT; i++) {
			...
			td->tdObjectResizeWindows[i] =
				gdk_window_new(td->tdObjectWindow, &xswa,
					       attributesMask | GDK_WA_CURSOR);

			/* The document will get the events */
			gdk_window_set_events(td->tdObjectResizeWindows[i],
					      (GdkEventMask)0);
		}
		...
		for (i = 0; i < RESIZE_COUNT; i++) {
			gdk_window_move(td->tdObjectResizeWindows[i], xp[i].x,
					xp[i].y);
		}
	}
...
static int tedShowIBar(void *voided)
{
	...
	td->tdHideIBarId =
		gtk_timeout_add(TED_BLINK_VISIBLE, tedHideIBar, (void *)ed);
	return 0;
}
...
static int tedHideIBar(void *voided)
{
	...
	td->tdShowIBarId =
		gtk_timeout_add(TED_BLINK_INVISIBLE, tedShowIBar, (void *)ed);
	return 0;
}
