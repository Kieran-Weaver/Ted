...
static void tedShadingToolShowLevel(ShadingTool *st, const ItemShading *is)
{
	if (!st->stLevelText) {
		return;
	}
	...
	if (is->isPattern == DOCspSOLID) {
		...
		appStringToTextWidget(st->stLevelText, "");
	}
...
void tedEnableShadingTool(ShadingTool *st, int enabled)
{
	...
	if (st->stFrame) {
		...
		guiEnableWidget(st->stRow, enabled);
	}
...
static void tedSetShadingTool(ShadingTool *st, const DocumentProperties *dp,
			      const ItemShading *is)
{
	...
	/**/
	appColorChooserSuggestPalette(&(st->stBackColorChooser), avoidZero,
				      dp->dpColorPalette);
...
static APP_EVENT_HANDLER_H(tedShadingRedrawInplace, w, voidst, exposeEvent)
{
	...
	drawFillRectangle(add, drI.drX0, drI.drY0, drI.drX1 - drI.drX0 + 1,
			  drI.drY1 - drI.drY0 + 1);
...
static APP_EVENT_HANDLER_H(tedShadingRedrawPulldown, w, voidst, exposeEvent)
{
	...
	if (!st->stPulldownDrawingDataSet) {
		appSetDrawingDataForWidget(
			st->stPatternPulldown.adpPulldownDrawing, 1.0,
			&(st->stPatternPulldownDrawingData));
...
static APP_EVENT_HANDLER_H(tedShadingClickedPulldown, w, voidst, mouseEvent)
{
	...
	if (appGuiDrawnPulldownGetStrip(&pattern, &(st->stPatternPulldown), w,
					mouseEvent)) {
		return;
	}

	if (pattern < 0 || pattern >= PSshd_COUNT) {
		LLDEB(pattern, PSshd_COUNT);
		return;
	}
...
static void tedShadeColorChosen(ColorChooser *cc, int which, void *voidst,
				int choice, const RGB8Color *rgb8Set)
{
	ShadingTool *st = (ShadingTool *)voidst;
	ExpandedItemShading *eis = &(st->stShadingChosen);
	int changed = 0;
	...
	if (which == st->stBackWhich) {
		appColorChooserColorChosen(&(st->stShadingChanged), &changed,
					   &(eis->eisBackColor),
					   &(eis->eisBackColorExplicit),
					   rgb8Set, choice != CHOICEccDEFAULT,
					   ISpropBACK_COLOR);
...
void tedFormatMakeShadingTool(ShadingTool *st, AppInspector *ai,
			      ...
			      int backWhich, int foreWhich,
			      TedShadingToolCallback callback, void *target)
{
    ...
    */
	...
	/****/
	if (title) {
		appMakeColumnFrameInColumn(&(st->stFrame), &(st->stPaned),
					   pageWidget, title);
		parent = st->stPaned;
	}
	...
	appMakeLabelInRow(&(st->stPatternLabel), row, labelColumn, labelColspan,
			  str->strPattern);
	appMakeDrawnPulldownInRow(&(st->stPatternPulldown),
				  tedShadingRedrawInplace,
				  tedShadingRedrawPulldown,
				  tedShadingClickedPulldown, row, valueColumn,
				  valueColspan, (void *)st);
	st->stShowPattern = 1;
#else
	st->stShowPattern = 0;
#endif
	...
	if (st->stLevelText) {
		appGuiSetTypingCallbackForText(
			st->stLevelText, tedShadingLevelTyped, (void *)st);
	}
	...
	appMakeToggleAndColorChooserRow(
		&(st->stRow), &(st->stOnOffToggle), &(st->stBackColorChooser),
		shadingHasAutomaticColor, parent, str->strBackColor,
		&(str->strBackColorChooserResources), tedShadeToggled,
		tedShadeColorChosen, backWhich, (void *)st);
#endif
...
void tedFinishShadingTool(ShadingTool *st, const PostScriptFontList *psfl)
{
	if (st->stShowPattern) {
		...
		appGuiSetDrawnPulldownStrips(&(st->stPatternPulldown),
					     PSshd_COUNT);
	}

	/**/
	if (st->stBackLabel) {
		appFinishColorChooser(&(st->stBackColorChooser), psfl,
				      st->stBackLabel);
	} else {
		appFinishColorChooser(&(st->stBackColorChooser), psfl,
				      st->stOnOffToggle);
	...
	if (st->stForeColorChooser.ccFilled) {
		appFinishColorChooser(&(st->stForeColorChooser), psfl,
				      st->stForeLabel);
	}
...
static int tedShadingToolGetShading(ExpandedItemShading *eisSet,
				    ...
				    const ShadingTool *st)
{
	...
	if (PROPmaskISSET(&(st->stShadingChanged), ISpropLEVEL) && changed) {
