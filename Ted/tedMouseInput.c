...
static int tedExtendSelectionToXY(DraggingContext *dc)
{
	...
	if (tedFindPosition(&dpTo, &pgTo, &pageFound, &columnFound,
			    dc->dcSelRootNode, lc, dc->dcMouseX + ox,
			    dc->dcMouseY + oy)) { /*LLDEB(mouseX,mouseY);*/
		return 0;
	}
...
static int tedLayoutSelectedRoot(const LayoutContext *lc,
				 ...
				 BufferItem *bodySectBiSet, int page,
				 int column)
{
	DocumentPosition dp;
	int inHeadFoot;
		...
		if (!treeFound) {
			...
			if (treeFound->dtPageFormattedFor != page) {
				...
				if (docLayoutDocumentTree(
					    treeFound, &drChanged, page,
					    column0, treeFound->dtY0UsedTwips,
					    bodySectBiSet, lc,
	...
	case DOCinAFTNCN:
		...
		if (!treeFound) {
			...
			if (docCheckPageOfSelectedTree(
				    &changed, &bodySectBiSetX, &drExternalSetX,
				    treeFound, lc,
				    docStartScreenLayoutForTree)) {
				LDEB(1);
			}
			...
			if (changed) {
				geoUnionRectangle(&drChanged, &drChanged,
						  &drExternalSetX);
			}
		}
		break;
...
static void tedHandleMultiClick(EditDocument *ed,
				...
				const PositionGeometry *pgClick, int seq)
{
...
static int tedSelectObject(EditDocument *ed, const LayoutContext *lc,
			   const DocumentSelection *dsWord, int docX)
{
...
static int tedFindMousePosition(APP_WIDGET w, EditDocument *ed,
				DraggingContext *dc,
				const DocumentSelection *dsOld, int *pSameRoot,
				int *pSameInstance, PositionGeometry *pgClick,
				int *pDocX, int *pSeq, unsigned int *pKeyState,
				APP_EVENT *downEvent)
{
	...
	if (tedFindRootForPosition(&(dc->dcAnchorPosition), pgClick,
				   &(dc->dcTree), &rootNodeFound, &page,
				   &column, ed, docX,
				   docY)) { /*LLDEB(docX,docY);*/
		return 1;
	}
	...
	if (sameRoot) {
		sameInstance =
			docSelectionSameInstance(dc->dcTree, page, column);
	}
	...
	if (!sameRoot || !sameInstance) {
		...
		if (tedLayoutSelectedRoot(&(dc->dcLayoutContext), dc->dcTree,
					  rootNodeFound, bodySectNode, page,
					  column)) {
			LDEB(page);
			return 1;
		}
...
static void tedAvoidReadonlyField(DraggingContext *dc,
				  PositionGeometry *pgClick, int docX)
{
	...
	docBalanceFieldSelection(&dfHead, &dfTail, &headMoved, &tailMoved,
				 &headPart, &tailPart, &dpHead, &dpTail,
				 dc->dcTree, dc->dcLayoutContext.lcDocument);

	if (headMoved) {
		PositionGeometry pg;

		tedPositionGeometry(&pg, &dpHead, PARAfindLAST,
				    &(dc->dcLayoutContext));
	...
	if (tailMoved) {
		...
		tedPositionGeometry(&pg, &dpTail, PARAfindFIRST,
				    &(dc->dcLayoutContext));
...
static int tedSelectMousePosition(unsigned int *pKeyState, APP_WIDGET w,
				  EditDocument *ed, DraggingContext *dc,
				  APP_EVENT *downEvent)
{
	TedDocument *td = (TedDocument *)ed->edPrivateData;
	BufferDocument *bd = td->tdDocument;
	...
	if (tedGetSelection(&dsOld, &sg, &sd, (DocumentTree **)0,
			    (BufferItem **)0, ed)) {
		LDEB(1);
		return 1;
	}
	...
	if (seq > 1) {
		tedHandleMultiClick(ed, &dsWord, &(dc->dcAnchorPosition),
				    &pgClick, seq);
		goto ready;
	}
	...
	if (wordIsObject &&
	    !tedSelectObject(ed, &(dc->dcLayoutContext), &dsWord, docX)) {
		goto ready;
	}
			      ...
			      APP_EVENT *downEvent)
{
	...
	if (ed->edFileReadOnly || (keyState & KEY_CONTROL_MASK)) {
		...
		dfHyperlink = docFindTypedFieldForPosition(
			bd, &(dc.dcAnchorPosition), DOCfkHYPERLINK, 0);
	...
	appRunDragLoop(w, ed->edApplication, downEvent, (APP_EVENT_HANDLER_T)0,
		       tedInputDragMouseMove, TED_DRAG_INTERVAL, tedTick,
		       (void *)&dc);
...
static void tedButton3Pressed(APP_WIDGET w, EditDocument *ed,
			      APP_EVENT *downEvent)
{
	TedDocument *td = (TedDocument *)ed->edPrivateData;
	...
	if (tedFindRootForPosition(&dpClick, (PositionGeometry *)0, &treeFound,
				   &rootNodeFound, &page, &column, ed, docX,
				   docY)) {
		LLDEB(docX, docY);
		return;
	}
