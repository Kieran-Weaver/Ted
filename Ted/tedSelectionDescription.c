...
int tedGetSelection(DocumentSelection *ds, SelectionGeometry *sg,
		    SelectionDescription *sd, DocumentTree **pEi,
		    BufferItem **pBodySectNode, const EditDocument *ed)
{
	const TedDocument *td = (const TedDocument *)ed->edPrivateData;
	...
	if (pEi || pBodySectNode) {
		if (docGetTreeOfNode(&ei, &bodySectNode, bd,
				     dss->dsHead.dpNode)) {
			LDEB(1);
			return -1;
		}
...
void tedDelimitCurrentSelection(EditDocument *ed)
{
	...
	lastLine = sdDoc.sdIsIBarSelection &&
		   (td->tdSelectionGeometry.sgHead.pgPositionFlags &
		    POSflagLINE_HEAD);
...
static void tedAdaptOptions(TedDocument *td, const unsigned char *cmdEnabled,
			    const SelectionDescription *sd)
{
	...
	guiEnableWidget(td->tdTabInsertTableOption,
			cmdEnabled[EDITcmdINSERT_TABLE]);
	guiEnableWidget(td->tdTabAddRowOption, cmdEnabled[EDITcmdAPPEND_ROW]);
...
void tedAdaptToolsToSelection(EditDocument *ed)
{
	...
	tedSetBottomRulerValues(ed->edBottomRuler, ed->edBottomRulerWidget,
				sd.sdTailPage, sd.sdDocumentPages,
				sd.sdTailSection, sd.sdDocumentSections);
