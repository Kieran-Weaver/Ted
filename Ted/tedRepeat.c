...
static int tedRepeatInsertHeaderFooter(EditDocument *ed, const EditStep *es,
				       int traced)
{
	...
	if (treeType != sd.sdHeaderTypeForSelection &&
		...
		SDEB(docTreeTypeStr(treeType));
		return -1;
	}

	if (docSectionHasHeaderFooter(
		    bodySectNode, (unsigned char *)0, dp,
		    treeType)) { /* SDEB(docTreeTypeStr(treeType)); */
		return 0;
	}
...
static int tedRepeatDeleteHeaderFooter(EditDocument *ed, const EditStep *es,
				       int traced)
{
	TedDocument *td = (TedDocument *)ed->edPrivateData;
	BufferDocument *bd = td->tdDocument;
...
static int tedRepeatUpdateField(EditDocument *ed, const EditStep *es,
				int traced)
{
	int rval = 0;
...
static int tedRepeatSetField(EditDocument *ed, const EditStep *es, int traced)
{
	int rval = 0;
...
static int tedRepeatUpdProps(EditDocument *ed, const EditStep *es, int traced)
{
	int rval = 0;
	...
	if (tedEditChangeSelectionProperties(
		    &teo, &ds, es->esPropLevel, es->esCommand,
		    &(es->esNewStyle.dsTextMask),
		    &(es->esNewStyle.dsTextAttribute),
...
static int tedRepeatUpdObject(EditDocument *ed, const EditStep *es, int traced)
{
	...
	return tedDocSetImageProperties(ed, &(es->esPicturePropMask), pipFrom,
					traced);
}
...
static int tedRepeatUpdNote(EditDocument *ed, const EditStep *es, int traced)
{
	const DocumentField *dfFrom;
...
static int tedRepeatInsNote(EditDocument *ed, const EditStep *es, int traced)
{
	const DocumentField *dfFrom;
	const DocumentNote *dnFrom = (DocumentNote *)0;
	...
	return tedDocInsertNote(ed, dnFrom->dnNoteProperties.npTreeType,
				traced);
}
...
static int tedRepeatInsertTable(EditDocument *ed, const EditStep *es,
				int traced)
{
	DocumentPosition dp;
	BufferItem *rowNode;
	...
	return tedInsertTable(
		ed, rowNode->biRowTablePast - rowNode->biRowTableFirst,
...
static int tedRepeatReplace(EditDocument *ed, const EditStep *es, int traced)
{
	int rval = 0;
	...
	if (docEditRangeIsIBar(&(es->esNewEditRange))) {
		...
		if (tedEditIncludeDocument(&teo, &dcj, es->esCommand,
					   es->esSelectionPosition)) {
			LDEB(1);
			rval = -1;
			goto ready;
		}
...
int tedDocRepeat(EditDocument *ed)
{
	...
	if (!isRepeat) {
		...
		if (docSelectionForEditPositionsInDoc(
			    &dsNew, bd, &(es.esOldSelectionScope),
	...
	case EDITcmdAPPEND_PARA:
		if (tedDocInsertParagraph(
			    ed, es.esCommand == EDITcmdAPPEND_PARA, isRepeat)) {
			LDEB(es.esCommand);
			rval = -1;
			goto ready;
		}
	...
	case EDITcmdAPPEND_COLUMN:
		if (tedDocAddColumnToTable(ed,
					   es.esCommand == EDITcmdAPPEND_COLUMN,
					   isRepeat)) {
			LDEB(es.esCommand);
			rval = -1;
			goto ready;
	...
	case EDITcmdAPPEND_ROW:
		if (tedDocAddRowToTable(ed, es.esCommand == EDITcmdAPPEND_ROW,
					isRepeat)) {
			LDEB(es.esCommand);
			rval = -1;
			goto ready;
	...
	case EDITcmdAPPEND_SECT:
		if (tedDocInsertSection(ed, es.esCommand == EDITcmdAPPEND_SECT,
					isRepeat)) {
			LDEB(es.esCommand);
			rval = -1;
			goto ready;
	...
	case EDITcmdDEL_FIELD:
		if (tedDocFlattenTypedField(
			    ed, es.esFieldKind, &(es.esNewStyle.dsTextMask),
			    &(es.esNewStyle.dsTextAttribute), isRepeat)) {
			SDEB(docFieldKindStr(es.esFieldKind));
			rval = -1;
			goto ready;
		}
		break;
	...
	case EDITcmdUPD_LIST:
		if (tedDocChangeCurrentList(ed, &(es.esDocumentList),
					    isRepeat)) {
			LDEB(es.esCommand);
			rval = -1;
			goto ready;
