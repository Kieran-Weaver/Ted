...
static void tedSetPositionFound(PositionFindJob *pfj, const TextLine *tl,
				int distance)
{
				     ...
				     const BlockOrigin *bo)
{
	...
	if (geo2DIXYInBox(pfj->pfjDocX, pfj->pfjDocY, drLinePixels)) {
		...
		if (tedFindPositionInLine(&(pfj->pfjDocumentPosition),
					  &(pfj->pfjPositionGeometry), lc,
					  paraBi, line, pfj->pfjDocX)) {
			LDEB(pfj->pfjDocX);
		} else {
	...
	if (paraBi->biTreeType == DOCinBODY) {
		...
		if (pfj->pfjDocY > yc) {
			...
			dy = drLinePixels->drY1 - pfj->pfjDocY;
		}
		...
		if (d < pfj->pfjDistance &&
		    geo2DIXYInBox(pfj->pfjDocX, pfj->pfjDocY, &drTry)) {
			pfj->pfjDocumentPosition.dpNode = (BufferItem *)paraBi;
			pfj->pfjDocumentPosition.dpStroff =
				tl->tlStroff + tl->tlStrlen;
...
int tedFindPositionInLine(DocumentPosition *dp, PositionGeometry *pg,
			  const LayoutContext *lc, const BufferItem *paraBi,
			  int line, int docXPixels)
{
	const TextLine *tl = paraBi->biParaLines + line;
...
static void tedSetFindPosition(PositionFindJob *pfj, const LayoutContext *lc,
			       int docXPixels, int docY)
...
int tedFindPosition(DocumentPosition *dp, PositionGeometry *pg, int *pPage,
		    int *pColumn, BufferItem *selRootNode,
		    const LayoutContext *lc, int docXPixels, int docY)
{
	...
	if (docDrawNode((LayoutPosition *)0, selRootNode, (void *)&pfj,
			&(pfj.pfjDrawingContext))) {
		LDEB(1);
		return 1;
	}
...
int tedFindRootForPosition(DocumentPosition *dpFound, PositionGeometry *pgFound,
			   DocumentTree **pTree, BufferItem **pSelSectNode,
			   int *pPage, int *pColumn, EditDocument *ed,
			   int docXPixels, int docY)
{
	LayoutContext lc;
	PositionFindJob pfj;
	...
	if (pfj.pfjFound) {
		...
		if (pTree) {
			if (docGetRootOfSelectionScope(
				    pTree, (BufferItem **)0, lc.lcDocument,
				    &(selSectNode->biSectSelectionScope))) {
				LDEB(1);
				return -1;
			}
		}
