...
static int docThisNoteToolDefaultText(const ThisNoteTool *tnt,
				      NoteProperties *np)
{
	if (docFormatChftnField(&(np->npFixedText),
				&(tnt->tntNotesPropertiesSet), np,
				tnt->tntNoteNumber)) {
...
static void tedThisNoteToolRefreshFixedTextSet(ThisNoteTool *tnt)
{
...
static void tedThisNoteToolRefreshFixedTextChosen(ThisNoteTool *tnt)
{
...
static void tedThisNoteToolRefreshWidgets(ThisNoteTool *tnt)
{
	...
	appGuiSetToggleState(tnt->tntFootnoteToggle,
			     tnt->tntNotePropertiesChosen.npTreeType ==
				     DOCinFOOTNOTE);
			    ...
			    const unsigned char *cmdEnabled)
{
	...
	if (dfNote) {
		...
		bodySectNode =
			docGetBodySectNode(dn->dnDocumentTree.dtRoot, bd);

		tnt->tntNotesPropertiesSet =
		...
		if (docThisNoteToolDefaultText(tnt,
...
static APP_BUTTON_CALLBACK_H(tedChangeNotePushed, w, voidtnt)
{
	...
	if (!utilPropMaskIsEmpty(&npSetMask)) {
		tedAppChangeCurrentNote(tnt->tntApplication, &npSetMask,
					&(tnt->tntNotePropertiesChosen));
	}
			       ...
			       const InspectorSubjectResources *isr)
{
	...
	guiToolMake2ToggleRow(&(tnt->tntNoteTypeRow), pageWidget,
			      &(tnt->tntFootnoteToggle),
			      &(tnt->tntEndnoteToggle), npr->nprFootnoteText,
			      npr->nprEndnoteText, tedFootnoteToggled,
			      tedEndnoteToggled, (void *)tnt, (void *)tnt);

	guiToolMakeToggleAndTextRow(
		&(tnt->tntFixedTextRow), &(tnt->tntFixedTextToggle),
		&(tnt->tntFixedTextText), pageWidget, npr->nprFixedTextText,
		tedFixedNoteTextToggled, (void *)tnt, textColumns, 1);

	guiToolMake2BottonRow(&(tnt->tntToOtherRow), pageWidget,
			      &(tnt->tntToNoteRefButton),
			      &(tnt->tntToNoteButton), npr->nprToNoteRefText,
			      npr->nprToNoteText, tedThisNoteToolToNoteRef,
			      tedThisNoteToolToNoteDef, (void *)tnt);
