...
static void tedFormatToolRefreshFramePage(FrameTool *ft)
{
	...
	/*  Y  */
	tedFormatEnableHeightChooser(&(ft->ftHeightChooser), ft->ftCanChange);
	tedFormatRefreshHeightChooser(&(ft->ftHeightChooser), fp->fpHighTwips);
			 ...
			 const SelectionDescription *sd, BufferDocument *bd)
{
	...
	docGetFramePropertiesByNumber(&(ft->ftPropertiesSet), bd,
...
static int tedFrameToolGetChosen(FrameTool *ft)
{
	...
	if (appGetLengthFromTextWidget(
		    ft->ftWidthText, &(fpChosen->fpWideTwips), &changed,
		    UNITtyPOINTS, minWidth, adaptToMin, maxValue, adaptToMax)) {
		return -1;
	}
	...
	if (fpChosen->fpXPosition == FXposXGIVEN) {
		if (appGetLengthFromTextWidget(ft->ftXText,
					       ...
					       UNITtyPOINTS, minPos, adaptToMin,
					       maxValue, adaptToMax)) {
			return -1;
		}
	}
...
static APP_BUTTON_CALLBACK_H(tedFormatChangeFramePushed, w, voidft)
{
	...
	if (tedDocChangeParagraphProperties(ed, &ppSetMask, &ppChosen,
					    traced)) {
		LDEB(1);
		goto ready;
	}
...
static APP_TXACTIVATE_CALLBACK_H(tedFrameWidthChanged, w, voidft)
{
	...
	tedFormatValidateDimension(&value, &changed, ft->ftWidthText,
				   fp->fpWideTwips);
...
static APP_TXACTIVATE_CALLBACK_H(tedFrameXChanged, w, voidft)
{
	...
	tedFormatValidateDimension(&value, &changed, ft->ftXText, fp->fpXTwips);
...
static APP_TXACTIVATE_CALLBACK_H(tedFrameYChanged, w, voidft)
{
	...
	tedFormatValidateDimension(&value, &changed, ft->ftYText, fp->fpYTwips);
...
static void tedFrameHeightChosen(int how, void *voidft)
{
	FrameTool *ft = (FrameTool *)voidft;
...
static void tedFrameXRefChosen(int ref, void *voidft)
{
	FrameTool *ft = (FrameTool *)voidft;
	FrameProperties *fp = &(ft->ftPropertiesChosen);

	if (ref < 0 || ref >= FXref__FRAME_COUNT) {
		LLDEB(ref, FXref__FRAME_COUNT);
		return;
	}
...
static void tedFrameXPosChosen(int pos, void *voidft)
{
	FrameTool *ft = (FrameTool *)voidft;
	FrameProperties *fp = &(ft->ftPropertiesChosen);

	if (pos < 0 || pos >= FXpos_COUNT) {
		LLDEB(pos, FXpos_COUNT);
		return;
	}
...
static void tedFrameYRefChosen(int ref, void *voidft)
{
	FrameTool *ft = (FrameTool *)voidft;
	FrameProperties *fp = &(ft->ftPropertiesChosen);

	if (ref < 0 || ref >= FYref__FRAME_COUNT) {
		LLDEB(ref, FYref__FRAME_COUNT);
		return;
	}
	...
	if (!docFrameYAllowed[fp->fpYPosition][fp->fpYReference]) {
		...
		for (pos = 0; pos < FYpos_COUNT; pos++) {
			if (docFrameYAllowed[pos][ref]) {
				fp->fpYPosition = pos;
				appSetOptionmenu(&(ft->ftYPosMenu),
						 fp->fpYPosition);
				tedFormatRefreshYPosText(ft);
			}
		}
...
static void tedFrameYPosChosen(int pos, void *voidft)
{
	FrameTool *ft = (FrameTool *)voidft;
	FrameProperties *fp = &(ft->ftPropertiesChosen);

	if (pos < 0 || pos >= FYpos_COUNT) {
		LLDEB(pos, FYpos_COUNT);
		return;
	}
	...
	if (!docFrameYAllowed[fp->fpYPosition][fp->fpYReference]) {
		...
		for (ref = 0; ref < FYref__FRAME_COUNT; ref++) {
			if (docFrameYAllowed[pos][ref]) {
				fp->fpYReference = ref;
				appSetOptionmenu(&(ft->ftYRefMenu),
						 fp->fpYReference);
			}
		}
	}
...
static void tedFillFramePositionTool(APP_WIDGET *pRow, APP_WIDGET *pText,
				     void *through, APP_WIDGET parent,
				     AppOptionmenu *aom,
				     ...
				     OptionmenuCallback menuCallback)
{
	...
	appMakeOptionmenuInRow(aom, row, menuColumn, menuColspan, menuCallback,
			       through);
...
static void tedFrameToolMakeHorizontalGroup(FrameTool *ft,
					    ...
					    APP_WIDGET pageWidget)
{
	...
	appMakeColumnFrameInColumn(&(ft->ftHorizontalFrame),
				   &(ft->ftHorizontalPaned), pageWidget,
				   fpr->fprHorizontal);

	guiToolMakeLabelAndTextRow(&row, &(ft->ftWidthLabel),
				   &(ft->ftWidthText), ft->ftHorizontalPaned,
				   fpr->fprWidth, textColumns, 1);
	appGuiSetGotValueCallbackForText(ft->ftWidthText, tedFrameWidthChanged,
					 (void *)ft);

	tedFillFramePositionTool(&row, &(ft->ftXText), (void *)ft,
				 ft->ftHorizontalPaned, &(ft->ftXPosMenu),
				 tedFrameXChanged, tedFrameXPosChosen);

	guiToolMakeLabelAndMenuRow(&row, &(ft->ftXRefMenu), &label,
				   ft->ftHorizontalPaned, fpr->fprXReference,
...
static void tedFrameToolMakeVerticalGroup(FrameTool *ft,
					  const FramePageResources *fpr,
					  APP_WIDGET pageWidget)
{
	APP_WIDGET label;
	...
	appMakeColumnFrameInColumn(&(ft->ftVerticalFrame),
				   &(ft->ftVerticalPaned), pageWidget,
				   fpr->fprVertical);

	tedFormatMakeHeightRow(ft, ft->ftVerticalPaned, &(ft->ftHeightChooser),
			       tedFrameHeightChanged, tedFrameHeightChosen);

	tedFillFramePositionTool(&row, &(ft->ftYText), (void *)ft,
				 ft->ftVerticalPaned, &(ft->ftYPosMenu),
				 tedFrameYChanged, tedFrameYPosChosen);

	guiToolMakeLabelAndMenuRow(&row, &(ft->ftYRefMenu), &label,
				   ft->ftVerticalPaned, fpr->fprYReference,
			    ...
			    const InspectorSubjectResources *isr)
{
	...
	guiToolMake2BottonRow(&row, pageWidget, &(is->isSelectButton),
			      ...
			      isr->isrDeleteButtonText, tedFormatSelectFrame,
			      tedFormatDeleteFrame, (void *)ft);

	guiToolMake2BottonRow(&row, pageWidget, &(is->isInsertButton),
			      &(is->isAppendButton), isr->isrInsertButtonText,
			      isr->isrAppendButtonText, tedFormatSetFrame,
			      tedFormatUnsetFrame, (void *)ft);
...
{
	tedFormatFillHeightChooser(&(ft->ftHeightChooser), fpr->fprHeightFree,
				   fpr->fprHeightAtLeast,
				   fpr->fprHeightExactly);

	appFillInspectorMenu(FXref__FRAME_COUNT, FXrefCOLUMN, ft->ftXRefItems,
			     fpr->fprXRefItemTexts, &(ft->ftXRefMenu));

	appFillInspectorMenu(FXpos_COUNT, FXposXL, ft->ftXPosItems,
			     fpr->fprXPosItemTexts, &(ft->ftXPosMenu));

	appFillInspectorMenu(FYref__FRAME_COUNT, FYrefMARGIN, ft->ftYRefItems,
			     fpr->fprYRefItemTexts, &(ft->ftYRefMenu));

	appFillInspectorMenu(FYpos_COUNT, FYposYT, ft->ftYPosItems,
...
static AppConfigurableResource TED_TedFrameToolResourceTable[] = {
	APP_RESOURCE("frameToolHorizontal",
		     offsetof(FramePageResources, fprHorizontal), "Horizontal"),
	APP_RESOURCE("frameToolVertical",
		     offsetof(FramePageResources, fprVertical), "Vertical"),

	APP_RESOURCE("frameToolWidth", offsetof(FramePageResources, fprWidth),
		     "Width"),
	...
	APP_RESOURCE("frameToolXPosGiven",
		     offsetof(FramePageResources,
			      fprXPosItemTexts[FXposXGIVEN]),
		     "X at"),
	...
	APP_RESOURCE("frameToolXRefMargin",
		     offsetof(FramePageResources,
			      fprXRefItemTexts[FXrefMARGIN]),
		     "Margin"),
	APP_RESOURCE("frameToolYPosGiven",
		     offsetof(FramePageResources,
			      fprYPosItemTexts[FYposYGIVEN]),
		     "Y at"),
...
void tedFrameToolGetResourceTable(EditApplication *ea, FramePageResources *fpr,
