		...
		if (!docGotoFirstPosition(&dpFirst, bd->bdBody.dtRoot)) {
			...
			if (tar->tarFindPattern &&
			    !docFindSetPattern(&(td->tdFindProg),
					       tar->tarFindPattern,
					       tar->tarFindRegex) &&
			    !docFindFindNextInDocument(
				    &(td->tdSelection), &dpFirst, bd,
				    docFindParaFindNext, td->tdFindProg)) {
				bodySectNode =
					docGetBodySectNode(dpFirst.dpNode, bd);
			} else {
				docAvoidParaHeadField(&dpFirst, (int *)0, bd);

				docSetIBarSelection(&(td->tdSelection),
						    &dpFirst);
    ...
    */
	...
	if (utilRGB8LumaChromaHue(&luma, &chroma, &hue,
		...
		if (changed) {
			...
			if (ed->edTopRuler) {
				tedTopRulerSetBackground(
					ed->edTopRuler,
					&(ed->edBackgroundColor));
			}
			if (ed->edLeftRuler) {
				tedVerticalRulerSetBackground(
					ed->edLeftRuler,
					&(ed->edBackgroundColor));
			}
			if (ed->edRightRuler) {
	...
	if (!utilMemoryBufferIsEmpty(&(td->tdRecoveredName))) {
		...
		while (n < et->etCount) {
			...
			n = docEditGetTraceStep(&ts, &isRepeat, direction, et,
						n + 1);
...
int tedMakeDocumentWidget(EditApplication *ea, EditDocument *ed)
{
	...
	if (!ea->eaDocumentCursor && tedMakeDocumentCursor(ea)) {
		LDEB(1);
	}
			      ...
			      DOC_CLOSE_OBJECT closeObject, BufferDocument *bd)
{
	...
	rf->rfUpdateFlags = FIELDdoDOC_INFO | FIELDdoDOC_COMPLETE |
			    FIELDdoCHFTN;
		      ...
		      const DocumentGeometry *dgDef)
{
	...
	*drVisible = drOutside;
	...
	if (docRecalculateTextLevelFieldsInDocumentTree(
		    &rf, &(bd->bdBody), rootNode->biChildren[0], page)) {
		LDEB(1);
		return -1;
	}
...
void tedSuggestPageSetup(PrintGeometry *pg, void *privateData, int sheetSize)
{
	TedDocument *td = (TedDocument *)privateData;
	BufferDocument *bd = td->tdDocument;
	DocumentProperties *dp = &(bd->bdProperties);
