...
static void tedIncludeFieldInRedraw(TedEditOperation *teo,
				    const DocumentSelection *dsField,
				    int headPart, int tailPart)
{
			...
			int tailPart, DocumentField *df)
{
	...
	/*  0,2  */
	docDeleteField(dsExInside, eo, dsAroundField->dsHead.dpNode,
		       dsAroundField->dsTail.dpNode, headPart, tailPart, df);
			   ...
			   const TextAttribute *taSet, int traced)
{
	...
	if (taSetMask) {
		...
		if (teo.teoEditTrace) {
			if (docRtfTraceNewProperties(
				    ...
				    (const PropertyMask *)0,
				    (const CellProperties *)0,
				    (const PropertyMask *)0,
				    (const RowProperties *)0,
				    (const PropertyMask *)0,
			...
			if (docRtfTraceOldContents(&dsTraced, eo, DOClevSPAN,
						   0)) {
				LDEB(1);
				rval = -1;
				goto ready;
		...
		if (docChangeParticuleAttributes(
			    (int *)0, (PropertyMask *)0, eo->eoDocument,
			    eo->eoHeadDp.dpNode, headPart + 1, tailPart, taSet,
			    taSetMask)) {
			LDEB(1);
		}
	} else {
	...
	if (tedFlattenFieldImpl(&teo, &dsExInside, dsAroundField, headPart,
				tailPart, df)) {
		LDEB(1);
		rval = -1;
		goto ready;
...
int tedInsertPageNumber(EditDocument *ed, int traced)
{
	int rval = 0;
	...
	/*  2  */
	if (tedDocReplaceSelectionWithField(&teo, &df, &headPart, &tailPart,
					    &dsInside, &dsAround, &fi,
					    DOCfkPAGE, &taSetMask, &taSet)) {
		LDEB(4);
		rval = -1;
		goto ready;
	}
...
int tedDocFlattenTypedField(EditDocument *ed, int fieldType,
			    const PropertyMask *taSetMask,
			    const TextAttribute *taSet, int traced)
{
	TedDocument *td = (TedDocument *)ed->edPrivateData;
	...
	tedGetSelection(&ds, &sg, &sd, (DocumentTree **)0,
	...
	tedFlattenField(ed, &dsAroundField, headPart, tailPart, df, taSetMask,
			taSet, traced);
