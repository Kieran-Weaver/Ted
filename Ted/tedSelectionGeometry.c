...
static int tedCalculateX(const BufferItem *paraNode, const TextLine *tl,
			 const TextParticule *tp, int part, int x0FramePixels,
			 const LayoutContext *lc, int stroff)
{
	...
	case DOCkindSPAN:
		...
		if (len == tp->tpStrlen &&
			...
			docXPixels =
				x0Pixels +
				docScreenTextWidth(screenFont, &ta, lc, s, len);
		}
...
static void tedSelectionRectangle(SelectionGeometry *sg,
				  ...
				  const BufferItem *bodySectNode)
{
	...
	if (tlBegin->tlTopPosition.lpPage != tlEnd->tlTopPosition.lpPage ||
		...
		if (useColumns && tlBegin->tlTopPosition.lpColumn <
					  bodySectNode->biSectColumnCount - 1) {
			docParaBlockFrameRectangle(
				&dr, ds->dsHead.dpNode, lc->lcDocument,
		...
		if (useColumns && tlEnd->tlTopPosition.lpColumn > 0) {
			docParaBlockFrameRectangle(
				&dr, ds->dsTail.dpNode, lc->lcDocument,
	...
	if (ds->dsTail.dpNode == ds->dsHead.dpNode) {
		...
		if (paraNode->biParaFirstIndentTwips < 0) {
			drSel->drX0 = docLayoutXPixels(
				lc, drParaContent->drX0 +
	...
	if (ds->dsHead.dpNode->biParaTableNesting == 0 ||
		...
		parentNode = docGetCommonParent(ds->dsHead.dpNode,
						ds->dsTail.dpNode);
		if (!parentNode) {
			XDEB(parentNode);
		} else {
	...
	if (ds->dsHead.dpNode->biParaTableNesting > 0) {
		...
		if (docSelectionInsideCell(ds)) {
			dr.drY0 = docLayoutYPixels(lc,
						   &(paraNode->biTopPosition));
			dr.drY1 = docLayoutYPixels(
				lc, &(paraNode->biBelowPosition));
		} else {
			BufferItem *rowBi = paraNode->biParent->biParent;

			dr.drY0 = docLayoutYPixels(lc, &(rowBi->biTopPosition));
			dr.drY1 =
				docLayoutYPixels(lc, &(rowBi->biBelowPosition));
		}
			 ...
			 int lastOne, const LayoutContext *lc)
{
	...
	pg->pgXPixels = tedCalculateX(dp->dpNode, tl, tp, part, x0FramePixels,
				      lc, dp->dpStroff);
...
void tedSelectionGeometry(SelectionGeometry *sg, const DocumentSelection *ds,
			  const BufferItem *bodySectNode, int lastLine,
			  const LayoutContext *lc)
{
