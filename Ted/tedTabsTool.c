...
static void tedTabToolRefreshCurrentTab(TabsTool *tt)
{
	...
	if (ts->tsTwips >= 0) {
		appLengthToTextWidget(tt->ttTabPositionText, ts->tsTwips,
				      UNITtyPOINTS);
	} else {
...
static void tedTabsToolSelectTab(TabsTool *tt, int n)
{
	const TabStopList *tsl = &(tt->ttTabStopsChosen);

	if (n >= 0 && n >= tsl->tslTabStopCount) {
		LLDEB(n, tsl->tslTabStopCount);
		n = -1;
	}
...
void tedRefreshTabsTool(TabsTool *tt, int *pEnabled, int *pPref,
			...
			const unsigned char *cmdEnabled)
{
	...
	appLengthToTextWidget(tt->ttTabDefaultText,
			      tt->ttDocPropertiesChosen.dpTabIntervalTwips,
			      UNITtyPOINTS);

	guiEnableWidget(tt->ttTabDefaultFrame,
...
static APP_BUTTON_CALLBACK_H(tedFormatTabsRevertParaPushed, w, voidtt)
{
	...
	if (docCopyTabStopList(&(tt->ttTabStopsChosen), &(tt->ttTabStopsSet))) {
		LDEB(1);
	}
...
static int tedTabToolValueTooCloseToExisting(const TabStopList *tsl,
					     int current, int value)
{
	...
	for (i = 0; i < tsl->tslTabStopCount; ts++, i++) {
		...
		if (value >= ts->tsTwips - 200 && value <= ts->tsTwips + 200) {
			return 1;
		}
	}
...
static APP_TXACTIVATE_CALLBACK_H(tedTabPositionChanged, w, voidtt)
{
	...
	tedFormatValidateDimension(&value, &changed, tt->ttTabPositionText,
				   value);
...
static APP_TXACTIVATE_CALLBACK_H(tedTabDeftabChanged, w, voidtt)
{
	...
	tedFormatValidateDimension(&value, &changed, tt->ttTabDefaultText,
				   value);
...
static APP_BUTTON_CALLBACK_H(tedTabToolAddTab, w, voidtt)
{
	...
	/*  1  */
	value = tt->ttTabStopValue.tsTwips;
	changed = 0;

	if (appGetLengthFromTextWidget(tt->ttTabPositionText, &value, &changed,
...
static APP_BUTTON_CALLBACK_H(tedFormatChangeParaTabs, w, voidtt)
{
	...
	/*  3  */
	if (tedTabToolValueTooCloseToExisting(tsl, tt->ttTabStopNumber,
					      value)) {
		appQuestionRunErrorDialog(tt->ttApplication,
					  tt->ttInspector->aiTopWidget, w,
					  tpr->tprTooCloseMessage);
...
static APP_BUTTON_CALLBACK_H(tedTabToolDeleteTab, w, voidtt)
{
	...
	if (tt->ttTabStopNumber < 0 ||
	    tt->ttTabStopNumber >= tsl->tslTabStopCount) {
		LLDEB(tt->ttTabStopNumber, tsl->tslTabStopCount);
		return;
	}
...
static APP_BUTTON_CALLBACK_H(tedFormatTabsApplyDocPushed, w, voidtt)
{
	...
	if (appGetLengthFromTextWidget(tt->ttTabDefaultText, &value, &changed,
				       UNITtyPOINTS, INT_MIN, 0, maxValue,
				       adaptToMax)) {
		return;
	}
...
static void tedTabAlignmentChosen(int alignment, void *voidtt)
{
	TabsTool *tt = (TabsTool *)voidtt;
	TabStop *ts = &(tt->ttTabStopValue);

	if (alignment < 0 || alignment >= DOCta_COUNT) {
		LLDEB(alignment, DOCta_COUNT);
		return;
	}
...
static void tedTabLeaderChosen(int leader, void *voidtt)
{
	TabsTool *tt = (TabsTool *)voidtt;
	TabStop *ts = &(tt->ttTabStopValue);

	if (leader < 0 || leader >= DOCtl_COUNT) {
		LLDEB(leader, DOCtl_COUNT);
		return;
	}
			   ...
			   const InspectorSubjectResources *isr)
{
	...
	appMakeColumnFrameInColumn(&(tt->ttTabDefaultFrame),
				   &(tt->ttTabDefaultPaned), pageWidget,
				   tpr->tprDefaultTabStopsLabel);

	guiToolMakeLabelAndTextRow(&row, &everyLabel, &(tt->ttTabDefaultText),
				   tt->ttTabDefaultPaned,
				   tpr->tprDefaultTabStopsEvery, textColumns,
				   1);

	appGuiSetGotValueCallbackForText(tt->ttTabDefaultText,
					 tedTabDeftabChanged, (void *)tt);

	guiToolMake2BottonRow(
		&row, tt->ttTabDefaultPaned, &(tt->ttTabDefaultRevertButton),
		&(tt->ttTabDefaultApplyButton), tpr->tprDefaultTabStopsRevert,
		tpr->tprDefaultTabStopsApply, tedFormatTabsRevertDocPushed,
		tedFormatTabsApplyDocPushed, tt);
	/**/
	appGuiMakeListInColumn(&(tt->ttTabPositionList), pageWidget,
			       visibleItems, tedTabsToolTabChosen,
			       (APP_BUTTON_CALLBACK_T)0, (void *)tt);

	/**/
	appMakeColumnFrameInColumn(&(tt->ttTabPropertyFrame),
				   &(tt->ttTabPropertyPaned), pageWidget,
				   tpr->tprTabStop);

	/**/
	guiToolMakeLabelAndTextRow(&row, &posLabel, &(tt->ttTabPositionText),
				   tt->ttTabPropertyPaned, tpr->tprPosition,
				   textColumns, 1);
	...
	guiToolMakeLabelAndMenuRow(&row, &(tt->ttAlignmentOptionmenu),
				   ...
				   tpr->tprAlignment, tedTabAlignmentChosen,
				   (void *)tt);

	/**/
	guiToolMakeLabelAndMenuRow(&row, &(tt->ttLeaderOptionmenu),
				   &leaderLabel, tt->ttTabPropertyPaned,
				   tpr->tprLeader, tedTabLeaderChosen,
				   (void *)tt);

	guiToolMake2BottonRow(&row, pageWidget, &(tt->ttDeleteTabButton),
			      &(tt->ttInsertTabButton),
			      isr->isrDeleteButtonText,
			      isr->isrInsertButtonText, tedTabToolDeleteTab,
			      tedTabToolAddTab, tt);

	guiToolMake2BottonRow(&row, pageWidget, &(is->isRevertButton),
			      &(tt->ttChangeTabButton), isr->isrRevert,
			      isr->isrApplyToSubject,
			      tedFormatTabsRevertParaPushed,
			      tedFormatChangeParaTabs, tt);
...
void tedTabsToolFillChoosers(TabsTool *tt)
{
	...
	appFillInspectorMenu(DOCta_COUNT, DOCtaLEFT, tt->ttAlignmentItems,
			     tpr->tprAlignmentOptionTexts,
			     &(tt->ttAlignmentOptionmenu));

	appFillInspectorMenu(DOCtl_COUNT, DOCtlNONE, tt->ttLeaderItems,
			     tpr->tprLeaderOptionTexts,
...
static AppConfigurableResource TED_TedTabsToolResourceTable[] = {
	...
	APP_RESOURCE("formatToolTabPositions",
		     offsetof(TabsPageResources, tprPositions), "Positions"),

	APP_RESOURCE("formatToolTabStop",
		     offsetof(TabsPageResources, tprTabStop), "Tab Stop"),

	APP_RESOURCE("formatToolTabPosition",
		     offsetof(TabsPageResources, tprPosition), "Position"),

	APP_RESOURCE("formatToolTabAlignment",
		     offsetof(TabsPageResources, tprAlignment), "Alignment"),
	/**/
	APP_RESOURCE("formatToolTabAlignmentLeft",
		     offsetof(TabsPageResources,
			      tprAlignmentOptionTexts[DOCtaLEFT]),
		     "Left"),
	APP_RESOURCE("formatToolTabAlignmentRight",
		     offsetof(TabsPageResources,
			      tprAlignmentOptionTexts[DOCtaRIGHT]),
		     "Right"),
	APP_RESOURCE("formatToolTabAlignmentCenter",
		     offsetof(TabsPageResources,
			      tprAlignmentOptionTexts[DOCtaCENTER]),
		     "Center"),
	APP_RESOURCE("formatToolTabAlignmentDecimal",
		     offsetof(TabsPageResources,
			      tprAlignmentOptionTexts[DOCtaDECIMAL]),
		     "Decimal"),

	/**/
	APP_RESOURCE("formatToolTabLeader",
		     offsetof(TabsPageResources, tprLeader), "Leader"),
	APP_RESOURCE("formatToolTabLeaderNone",
		     offsetof(TabsPageResources,
			      tprLeaderOptionTexts[DOCtlNONE]),
		     "None"),
	APP_RESOURCE("formatToolTabLeaderDots",
		     offsetof(TabsPageResources,
			      tprLeaderOptionTexts[DOCtlDOTS]),
		     "Dots"),
	APP_RESOURCE("formatToolTabLeaderUnderline",
		     offsetof(TabsPageResources,
			      tprLeaderOptionTexts[DOCtlUNDERLINE]),
		     "Line"),
	APP_RESOURCE("formatToolTabLeaderHyphens",
		     offsetof(TabsPageResources,
			      tprLeaderOptionTexts[DOCtlHYPH]),
		     "Dashes"),

	APP_RESOURCE("formatToolTabLeaderThick",
		     offsetof(TabsPageResources,
			      tprLeaderOptionTexts[DOCtlTHICK]),
		     "Thick Line"),
	APP_RESOURCE("formatToolTabLeaderEqual",
		     offsetof(TabsPageResources,
			      tprLeaderOptionTexts[DOCtlEQUAL]),
		     "Double Line"),
