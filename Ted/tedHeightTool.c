...
void tedFormatRefreshHeightChooser(HeightChooser *hc, int height)
{
	if (height == 0) {
		...
		if (height > 0) {
			...
			hc->hcHow = HC_EXACTLY;
			appLengthToTextWidget(hc->hcText, -height,
					      UNITtyPOINTS);
		}
	}
...
int tedFormatToolGetHeight(int *pHeight, HeightChooser *hc)
{
	...
	if (sign != 0) {
		if (appGetLengthFromTextWidget(
			    hc->hcText, &height, &changed, UNITtyPOINTS,
			    ...
			    OptionmenuCallback menuCallback)
{
	...
	appMakeOptionmenuInRow(&(hc->hcOptionmenu), hc->hcRow, menuColumn,
			       menuColspan, menuCallback, through);

	appMakeTextInRow(&(hc->hcText), hc->hcRow, textColumn, textColspan, 10,
			 0);
	if (textCallback) {
		appGuiSetGotValueCallbackForText(hc->hcText, textCallback,
						 through);
	}
				...
				const char *exactlyText)
{
	...
	hc->hcMenuItems[HC_FREE] =
		appAddItemToOptionmenu(&(hc->hcOptionmenu), freeText);

	hc->hcMenuItems[HC_AT_LEAST] =
		appAddItemToOptionmenu(&(hc->hcOptionmenu), atLeastText);

	hc->hcMenuItems[HC_EXACTLY] =
		appAddItemToOptionmenu(&(hc->hcOptionmenu), exactlyText);
