...
int tedInsertTable(EditDocument *ed, int rows, int columns, int traced)
{
	int rval = 0;
	...
	if (docFindParticuleOfPosition((int *)0, &headFlags, &(eo->eoHeadDp),
				       PARAfindFIRST)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
	if (docFindParticuleOfPosition((int *)0, &tailFlags, &(eo->eoTailDp),
				       PARAfindPAST)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if ((headFlags & POSflagPARA_HEAD) && (tailFlags & POSflagPARA_TAIL) &&
	    !docNextPosition(&(eo->eoTailDp))) {
		eo->eoLastDp = eo->eoTailDp;
		docSetEditPosition(&(eo->eoReformatRange.erTail),
				   &(eo->eoTailDp));
	...
	textAttrNr = docMapTextAttributeNumberFromTo(
		bdTable, eo->eoDocument,
	...
	if (docFillTableDocument(bdTable, sectNodeTable, textAttrNr, wide, rows,
				 columns)) {
		LLDEB(rows, columns);
		rval = -1;
		goto ready;
	...
	if (tedEditIncludeDocument(&teo, &dcj, EDITcmdINSERT_TABLE, posWhere)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
...
static int tedSplitColumnInRows(TedEditOperation *teo, DocumentPosition *dpNew,
				BufferItem *sectBi, int row0, int row, int row1,
				int col, int after)
{
	EditOperation *eo = &(teo->teoEo);
	...
	if (docSplitColumnInRows(&newParaBi, eo, sectBi, row0, row, row1, col,
				 after)) {
		LDEB(1);
		return -1;
	}
...
int tedDocAddColumnToTable(EditDocument *ed, int after, int traced)
{
	...
	if (docDelimitTable(eo->eoHeadDp.dpNode, &sectBi, &col, &row0, &row,
			    &row1)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (tedSplitColumnInRows(&teo, &dpNew, sectBi, row0, row, row1, col,
				 after)) {
		LDEB(1);
		rval = -1;
		goto ready;
