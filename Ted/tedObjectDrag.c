...
static void tedObjectHandleMove(TedObjectDrag *tod, int mouseX, int mouseY)
{
	...
	if (moved) {
		...
		if (drExp.drX1 >= drExp.drX0 && drExp.drY1 >= drExp.drY0) {
			guiExposeDrawingWidgetRectangle(
				ed->edDocumentWidget.dwWidget, &drExp);
		}
	}
...
int tedObjectDrag(APP_WIDGET w, EditDocument *ed, APP_EVENT *downEvent)
{
	...
	if (guiGetCoordinatesFromMouseButtonEvent(
		    &(tod.todMouseX), &(tod.todMouseY), &button, &upDown, &seq,
		    &keyState, w, downEvent)) {
		LDEB(1);
		return -1;
	}
	...
	if (tedGetObjectSelection(ed, &(tod.todParticule), &(tod.todDp),
				  &(tod.todIo))) {
		return 2;
	}

	/*  3  */
	tedPositionGeometry(&(tod.todPg), &(tod.todDp), PARAfindLAST,
			    &(tod.todLayoutContext));

	tedGetObjectRectangle(&drObj, xp, tod.todIo, &(tod.todPg),
			      &(tod.todLayoutContext), ed);

	/*  4  */
	if (seq > 1 || upDown < 1 || tod.todMouseX + ox < drObj.drX0 ||
	...
	appRunDragLoop(w, ea, downEvent, tedObjectDragMouseUp,
		       tedObjectDragMouseMove, 0, (APP_TIMER_CALLBACK)0,
		       (void *)&tod);
	...
	if (td->tdScaleChangedX != 0 || td->tdScaleChangedY != 0) {
		...
		if (td->tdObjectCornerMovedX != 0) {
			io->ioScaleXSet = (100.0 * (drResized.drX1 -
						    drResized.drX0 + 1)) /
		...
		if (td->tdObjectCornerMovedY != 0) {
			io->ioScaleYSet = (100.0 * (drResized.drY1 -
						    drResized.drY0 + 1)) /
		...
		if (td->tdObjectCornerMovedX != 0 ||
			...
			if (drExp.drX1 >= drExp.drX0 &&
			    drExp.drY1 >= drExp.drY0) {
				guiExposeDrawingWidgetRectangle(
					ed->edDocumentWidget.dwWidget, &drExp);
			}

			tedObjectSetImageProperties(
				ed, io, &(tod.todDp), tod.todParticule,
				&pipSetMask, &pipFrom,
	...
	tedSetObjectWindows(ed, &(tod.todPg), tod.todIo,
			    &(tod.todLayoutContext));
