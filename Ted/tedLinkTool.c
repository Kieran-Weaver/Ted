...
static void tedLinkAdaptToText(LinkTool *lt)
{
	...
	guiEnableWidget(bl->blMarkListWidget, lt->ltCanChange && bl->blIsLocal);
...
static void tedLinkAdaptToValues(LinkTool *lt, int adaptFile, int adaptMark,
				 int search, int select)
{
	...
	if (search) {
		tedBookmarkFindChosen(bl, lt->ltApplication,
				      &(hfChosen->hfBookmark));
	}
...
static APP_TXTYPING_CALLBACK_H(tedLinkFileChanged, w, voidlt)
{
	...
	if (!lt->ltInProgrammaticChange) {
		...
		tedLinkAdaptToValues(lt, adaptToFile, adaptToMark, search,
				     select);
	}
...
static APP_BUTTON_CALLBACK_H(tedLinkFollowPushed, w, voidlt)
{
	...
	if (tedAppFollowLink(w, ea, &(hfChosen->hfFile),
			     &(hfChosen->hfBookmark))) {
		LDEB(1);
	}
		     ...
		     const InspectorSubjectResources *isr)
{
	...
	appGuiSetTypingCallbackForText(lt->ltFileTextWidget, tedLinkFileChanged,
				       (void *)lt);
	appGuiSetTypingCallbackForText(bl->blMarkTextWidget, tedLinkMarkChanged,
				       (void *)lt);

	appGuiMakeListInColumn(&(bl->blMarkListWidget), pageWidget,
			       listVisibleItems, tedLinkBookmarkChosen,
			       (APP_BUTTON_CALLBACK_T)0, (void *)lt);

	guiToolMake2BottonRow(&row, pageWidget, &(lt->ltFollowLinkButton),
			      &(lt->ltRemoveLinkButton),
			      isr->isrSelectButtonText,
			      isr->isrDeleteButtonText, tedLinkFollowPushed,
			      tedLinkRemovePushed, (void *)lt);

	guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
			      &(is->isRevertButton), &(is->isApplyButton),
			      isr->isrRevert, isr->isrApplyToSubject,
			      tedLinkRevertPushed, tedLinkChangePushed,
			      (void *)lt);
			...
			const unsigned char *cmdEnabled)
{
	...
	if (dfHyperlink) {
		...
		if (docCopyHyperlinkField(&(lt->ltLinkChosen),
					  &(lt->ltLinkSet))) {
			LDEB(1);
			return;
		}
...
static AppConfigurableResource TED_TedLinkToolResourceTable[] = {
	APP_RESOURCE("hyperlinkDocument",
		     offsetof(LinkToolResources, lprFileText), "Document"),
	APP_RESOURCE("hyperlinkBookmark",
		     offsetof(LinkToolResources, lprMarkText), "Bookmark"),
};
