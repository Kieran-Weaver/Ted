...
int tedDocSetHyperlink(EditDocument *ed, const HyperlinkField *hf, int traced)
{
	...
	if (!dfHyperlink) {
		...
		if (tedDocSetField(&teo, &ds, EDITcmdSET_HYPERLINK,
				   DOCfkHYPERLINK, &fi, &taSetMask, &taSet)) {
			LDEB(1);
			rval = -1;
			goto ready;
		}
...
int tedDocRemoveHyperlink(EditDocument *ed, int traced)
{
	...
	return tedDocFlattenTypedField(ed, DOCfkHYPERLINK, &taSetMask, &taSet,
				       traced);
}
		      ...
		      int traced)
{
	...
	dfBookmark = docFindTypedFieldForPosition(bd, &(ds.dsHead),
						  DOCfkBOOKMARK, 0);
	if (!dfBookmark) {
		if (tedDocSetField(&teo, &ds, EDITcmdSET_BOOKMARK,
				   DOCfkBOOKMARK, &fi, (const PropertyMask *)0,
