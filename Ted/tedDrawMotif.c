...
void tedSetObjectWindows(EditDocument *ed, const PositionGeometry *pg,
			 const InsertedObject *io, const LayoutContext *lc)
{
	...
	if (!moveCursor) {
		...
		for (i = 0; i < RESIZE_COUNT; i++) {
			cursors[i] =
				XCreateFontCursor(display, font_cursors[i]);
		}
	}
	...
	if (!td->tdObjectWindow) {
		...
		td->tdObjectWindow = XCreateWindow(
			display, win, drObj.drX0 - lc->lcOx,
			drObj.drY0 - lc->lcOy, drObj.drX1 - drObj.drX0 + 1,
			drObj.drY1 - drObj.drY0 + 1, 0, CopyFromParent,
			InputOnly, CopyFromParent, 0L, &xswa);

		for (i = 0; i < RESIZE_COUNT; i++) {
			xswa.cursor = cursors[i];

			td->tdObjectResizeWindows[i] = XCreateWindow(
				display, td->tdObjectWindow, xp[i].x, xp[i].y,
				RESIZE_BLOCK, RESIZE_BLOCK, 0, CopyFromParent,
				CopyFromParent, CopyFromParent, CWCursor,
				&xswa);
		}
...
static void tedHideIBar(void *voided, XtIntervalId *intervalId);

static void tedShowIBar(void *voided, XtIntervalId *intervalId)
{
