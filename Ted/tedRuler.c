...
void *tedMakeTopRuler(int height, double magnifiedPixelsPerTwip,
		      ...
		      int documentC0, int documentC1, int marginC0,
		      int marginC1, int visibleC0, int visibleC1,
			    ...
			    const DocumentRectangle *drClip)
{
	...
	if (rd->rdVisibleC0 <= rd->rdVisibleC1) {
		...
		if (rd->rdVisibleC0 < drActive.drX0) {
			tedTopRulerDrawTicks(ttr, ox, rd->rdVisibleC0,
					     drActive.drX0);
		}
		if (drActive.drX1 < rd->rdVisibleC1) {
			tedTopRulerDrawTicks(ttr, ox, drActive.drX1,
					     rd->rdVisibleC1);
		}
	}
...

	{
		...
		for (col = 0; col < ttr->ttrColumnCount; cs++, col++) {
			tedRulerColumnControlRect(&drButton, cs->csX0, cs->csX1,
						  rd);

			if (drClip && !geoIntersectRectangle(
					      &drButton, &drButton, drClip)) {
				continue;
			}
	...
	if (ttr->ttrLeftIndent >= 0 && ttr->ttrLeftIndent >= rd->rdVisibleC0 &&
...

	{
		...
		for (tab = 0; tab < ttr->ttrTabStopList.tslTabStopCount;
		     ts++, tab++) {
...
static void tedTopRulerSetValue(TopRulerDrag *trd, int newValue)
{
...
static APP_EVENT_HANDLER_H(tedTopRulerMouseUp, w, vtrd, event)
{
	...
	if (guiGetCoordinatesFromMouseButtonEvent(&x, &y, &button, &upDown,
						  &seq, &keyState, w, event)) {
		return;
	}
...
static int tedTopRulerDragItem(int *pV0, int *pV1, int *pY, int candidateProp,
			       APP_WIDGET w, APP_EVENT *downEvent,
			       TedTopRuler *ttr, int xMin, int xMax, int v,
			       int ox, EditDocument *ed)
{
	...
	appRunDragLoop(w, ea, downEvent, tedTopRulerMouseUp,
		       tedTopRulerMouseMove, 0, (APP_TIMER_CALLBACK)0,
		       (void *)&trd);
...
static int tedCompareTabStopsPixels(const void *voidts1, const void *voidts2)
{
...
/*  2  */
static int tedTopRulerDragTab(APP_WIDGET w, APP_EVENT *downEvent,
			      TedTopRuler *ttr, int item, int x, int ox,
			      int mouseX, int mouseY, EditDocument *ed)
{
	...
	prop = tedTopRulerDragItem(&x1, (int *)0, &mouseY, PPpropTAB_STOPS, w,
				   downEvent, ttr, ttr->ttrParaFrameX0,
				   ttr->ttrParaFrameX1, x, ox, ed);
	...
	qsort(ttr->ttrTabStopList.tslTabStops,
	      ttr->ttrTabStopList.tslTabStopCount, sizeof(TabStop),
...
static int tedTopRulerTabClick(TabStopList *tsl, APP_WIDGET w,
			       APP_EVENT *downEvent, TedTopRuler *ttr, int item,
			       int x, int ox, int mouseX, int mouseY,
			       EditDocument *ed)
{
	...
	if (item >= 0) {
		/*  2  */
		property = tedTopRulerDragTab(w, downEvent, ttr, item,
					      mouseX + ox, ox, mouseX, mouseY,
					      ed);
		...
		tedTopRulerDragTab(w, downEvent, ttr, item, mouseX + ox, ox,
				   mouseX, mouseY, ed);
			 ...
			 EditDocument *ed)
{
	...
	/*  2  */
	if (mouseY >= drActive.drY0 && mouseY <= drActive.drY1) {
		int tab;
		const TabStop *ts;
	...
	if (mouseY >= drActive.drY0 && mouseY <= drActive.drY1) {
		...
		for (col = 0; col < ttr->ttrColumnCount; cs++, col++) {
			if (mouseX + ox >= cs->csX0 && mouseX + ox < cs->csX1) {
				*pIndex = col;
				return PPprop_COLUMNS;
			}
		}
	}
...
int tedTopRulerTrackMouse(int *pFirstIndent, int *pLeftIndent,
			  int *pRightIndent, TabStopList *tsl, int *pCsCount,
			  ColumnSeparator **pCs, APP_WIDGET w,
			  APP_EVENT *downEvent, EditDocument *ed)
{
	...
	case PPpropFIRST_INDENT:
		/*  4  */
		property = tedTopRulerDragItem(&(ttr->ttrFirstIndent), (int *)0,
					       (int *)0, property, w, downEvent,
					       ttr, ttr->ttrParaFrameX0,
					       ttr->ttrParaFrameX1, mouseX + ox,
					       ox, ed);
	...
	case PPpropLEFT_INDENT:
		/*  5  */
		property = tedTopRulerDragItem(&(ttr->ttrLeftIndent), (int *)0,
					       (int *)0, property, w, downEvent,
					       ttr, ttr->ttrParaFrameX0,
					       ttr->ttrParaFrameX1, mouseX + ox,
					       ox, ed);
	...
	case PPpropRIGHT_INDENT:
		/*  6  */
		property = tedTopRulerDragItem(&(ttr->ttrRightIndent), (int *)0,
					       (int *)0, property, w, downEvent,
					       ttr, ttr->ttrParaFrameX0,
					       ttr->ttrParaFrameX1, mouseX + ox,
					       ox, ed);
	...
	case PPprop_COLUMNS:
		/*  8  */
		{
			...
			property = tedTopRulerDragItem(
				&(ttr->ttrColumns[item].csX0),
				&(ttr->ttrColumns[item].csX1), (int *)0,
				property, w, downEvent, ttr, xMin, xMax,
				mouseX + ox, ox, ed);

			if (property == PPprop_COLUMNS) {
				*pCsCount = ttr->ttrColumnCount;
				*pCs = ttr->ttrColumns;
				guiExposeDrawingWidget(w);
			}
			return property;
	...
	case PPprop_TAB_KIND_BUTTON:
		/*  9  */
		{
			...
			case DOCtaLEFT:
				ttr->ttrTabKind = DOCtaRIGHT;
				break;
			case DOCtaRIGHT:
				ttr->ttrTabKind = DOCtaCENTER;
				break;
			case DOCtaCENTER:
				ttr->ttrTabKind = DOCtaDECIMAL;
				break;
			case DOCtaDECIMAL:
				ttr->ttrTabKind = DOCtaLEFT;
				break;
...
void tedAdaptTopRuler(void *voidttr, APP_WIDGET w, int documentC0,
		      int documentC1, int marginC0, int marginC1,
		      int firstIndent, int leftIndent, int rightIndent,
		      const TabStopList *tslSet, int x0GeometryPixels,
		      double xfac)
{
	TedTopRuler *ttr = (TedTopRuler *)voidttr;
	RulerData *rd = &(ttr->ttrRulerData);
	...
	if (ttr->ttrFirstIndent != firstIndent) {
		ttr->ttrFirstIndent = firstIndent;
		redraw = 1;
	}

	if (ttr->ttrLeftIndent != leftIndent) {
		ttr->ttrLeftIndent = leftIndent;
		redraw = 1;
	}

	if (ttr->ttrRightIndent != rightIndent) {
		ttr->ttrRightIndent = rightIndent;
		redraw = 1;
	}
	...
	for (i = 0; i < tslCopy.tslTabStopCount; i++) {
		tslCopy.tslTabStops[i].tsPixels =
			x0GeometryPixels +
		       ...
		       ColumnSeparator *cs, int csCount)
{
	...
	if (csCount != ttr->ttrColumnCount) {
		if (csCount > ttr->ttrColumnCount) {
			...
			fresh = (ColumnSeparator *)realloc(
				ttr->ttrColumns,
				csCount * sizeof(ColumnSeparator));
			if (!fresh) {
				LXDEB(csCount, fresh);
				return -1;
			}
	...
	for (i = 0; i < csCount; i++) {
		if (ttr->ttrColumns[i].csX0 != cs[i].csX0) {
			ttr->ttrColumns[i].csX0 = cs[i].csX0;
			changed = 1;
		}
		if (ttr->ttrColumns[i].csX1 != cs[i].csX1) {
			ttr->ttrColumns[i].csX1 = cs[i].csX1;
			changed = 1;
		}
	}
