...
int tedGetObjectSelection(const EditDocument *ed, int *pPartObj,
			  DocumentPosition *dpObj, InsertedObject **pIo)
{
	...
	rval = docGetObjectSelection(&(td->tdSelection), bd, pPartObj, dpObj,
				     pIo);
	if (rval) {
		return rval;
	}
...
static void tedScrollToPositions(EditDocument *ed, const PositionGeometry *pgB,
...
void tedScrollToSelection(EditDocument *ed, int *pScrolledX, int *pScrolledY)
{
	...
	if (tedGetSelection(&dsDoc, &sgDoc, &sdDoc, (DocumentTree **)0,
			    (BufferItem **)0, ed)) {
		LDEB(1);
		return;
	}

	tedScrollToPositions(ed, &(sgDoc.sgHead), &(sgDoc.sgTail), &lc,
...
static int tedExtendSelectionFromTable(EditDocument *ed,
				       ...
				       DocumentPosition *dpTail, int *pCol0,
				       int *pCol1)
{
	...
	const DocumentPosition *dpBegin, const DocumentPosition *dpEnd)
{
	...
	tedSelectionGeometry(&(td->tdSelectionGeometry), &(td->tdSelection),
	...
	if (exposeSelection) {
		...
		appDocExposeRectangle(ed, &(sgDoc.sgRectangle), scrolledX,
				      scrolledY);
	}
...
static void tedBalanceFieldSelection(BufferDocument *bd, DocumentTree *ei,
				     int *pBalanced, DocumentPosition *dpBegin,
				     DocumentPosition *dpEnd)
{
	...
	docBalanceFieldSelection(&dfLeft, &dfRight, &beginMoved, &endMoved,
				 &headPart, &tailPart, dpBegin, dpEnd, ei, bd);
				 ...
				 const DocumentPosition *dpFound)
{
	...
	if (directionToAnchor < 0) {
		...
		if (docPositionsInsideCell(&dpTo, &dpFrom)) {
			if (dpFrom.dpNode->biParaTableNesting > 0) {
				col0 = col1 = dpFrom.dpNode->biParent
						      ->biNumberInParent;
			}
		} else {
			if (dpTo.dpNode->biParaTableNesting > 0) {
				if (tedExtendSelectionFromTable(
					    ed, &dpTo, &dpFrom, &col0, &col1)) {
					LDEB(1);
					return -1;
				}
			} else {
				if (dpFrom.dpNode->biParaTableNesting > 0) {
					if (tedExtendSelectionIntoTable(
						    ed, &dpFrom)) {
						LDEB(1);
						return -1;
					}
		...
		tedBalanceFieldSelection(td->tdDocument, ei, &balanced, &dpTo,
					 &dpFrom);
		...
		/****************************************/
		if (directionEndAnchor > 0 || balanced || cellChanged) {
		...
		if (directionToBegin < 0) {
			dir = -1;
			exposeSelection = directionEndAnchor > 0 || balanced ||
					  cellChanged;

			tedSetExtendedSelection(ed, exposeSelection,
						&(sgDoc.sgRectangle), dir,
						bodySectNode, &dpTo,
						&(dsDoc.dsHead), &dpTo, &dpTo,
						&dpFrom);
		...
		if (directionToBegin > 0) {
			...
			tedSetExtendedSelection(ed, exposeSelection,
						&(sgDoc.sgRectangle), dir,
						bodySectNode, &(dsDoc.dsHead),
						&dpTo, &dpTo, &dpTo, &dpFrom);
	...
	if (directionToAnchor > 0) {
		...
		tedBalanceFieldSelection(td->tdDocument, ei, &balanced, &dpFrom,
					 &dpTo);
		...
		/****************************************/
		if (directionBeginAnchor < 0 || balanced || cellChanged) {
			appDocExposeRectangle(ed, &(sgDoc.sgRectangle),
					      scrolledX, scrolledY);
		}
		...
		if (directionToEnd > 0) {
			...
			exposeSelection = directionBeginAnchor < 0 ||
					  balanced || cellChanged;

			tedSetExtendedSelection(ed, exposeSelection,
						&(sgDoc.sgRectangle), dir,
						bodySectNode, &(dsDoc.dsTail),
						&dpTo, &dpTo, &dpFrom, &dpTo);
	...
	/********************/
	{
		...
		/*  Undraw selection after the anchor.	*/
		/****************************************/
		if (directionBeginAnchor < 0 || directionEndAnchor > 0) {
			appDocExposeRectangle(ed, &(sgDoc.sgRectangle),
					      scrolledX, scrolledY);
		}
		...
		dir = 0;
		exposeSelection = directionBeginAnchor < 0 ||
				  directionEndAnchor > 0 || balanced;

		tedSetExtendedSelection(ed, exposeSelection,
					&(sgDoc.sgRectangle), dir, bodySectNode,
					&dpFrom, &dpTo, &dpFrom, &dpFrom,
					&dpTo);
...
static int tedArrowFindPositionInLine(DocumentPosition *dp,
				      const LayoutContext *lc, int line,
				      int docXPixels)
{
	PositionGeometry pg;
...
int tedArrowUp(DocumentPosition *dp, const PositionGeometry *pg,
...
int tedArrowDown(DocumentPosition *dp, const PositionGeometry *pg,
		 const LayoutContext *lc)
{
	int line;
...
void tedSetSelectionLow(EditDocument *ed, const DocumentSelection *dsSet,
			int lastLine, int *pScrolledX, int *pScrolledY)
{
	...
	if (tedGetSelection(&dsOld, &sgOld, &sdOld, (DocumentTree **)0,
			    (BufferItem **)0, ed)) {
		LDEB(1);
		return;
	}
	...
	bodySectNr = docDescribeSetSelection(&treeSet, &bodySectBiSet,
					     &drExternalSet, &redrawTreeOld,
					     &redrawTreeSet, &lc, bd, &dsOld,
					     dsSet);
	if (bodySectNr < 0) {
		LDEB(bodySectNr);
		return;
	...
	if (hadSelection) {
		...
		if (redrawTreeOld) {
			geoUnionRectangle(&drExpose, &drExpose,
					  &(ed->edFullRect));
		}
	...
	treeSet->dtColumnSelectedIn =
		dsNew.dsHead.dpNode->biTopPosition.lpColumn;
...
void tedSetSelection(EditDocument *ed, const DocumentSelection *dsSet,
		     int lastLine, int *pScrolledX, int *pScrolledY)
{
...
void tedDescribeSelection(EditDocument *ed)
{
	...
	docDescribeSelection(
		sd, &(td->tdSelection), bd,
	...
	td->tdCurrentScreenFont =
		utilIndexMappingGet(a2s, sd->sdTextAttributeNumber);
...
void tedSetSelectedPosition(EditDocument *ed, const DocumentPosition *dpSet,
			    int lastLine, int *pScrolledX, int *pScrolledY)
{
	DocumentSelection dsNew;
