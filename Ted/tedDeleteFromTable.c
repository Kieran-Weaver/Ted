...
static int tedEditDeleteSelectedRows(DocumentPosition *dpNew, int *pSide,
				     const DocumentSelection *dsRows,
				     BufferItem *rowNode0, BufferItem *rowNode1,
				     TedEditOperation *teo)
{
	...
	if (child0 <= child1) {
		...
		docEditDeleteNodes(eo, &sectionsDeleted, &firstParaDeleted,
				   &paragraphsDeleted, parentNode, child0,
				   child1 - child0 + 1);
...
static int tedDeleteRowsFromTableOperation(TedEditOperation *teo, int command,
					   int row0, int row1)
{
	EditOperation *eo = &(teo->teoEo);
	...
	if (docDelimitTable(eo->eoHeadDp.dpNode, &parentNode, &col, &tabRow0,
			    &row, &tabRow1)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (row0 < tabRow0 || row0 > tabRow1) {
		LLLDEB(tabRow0, row0, tabRow1);
		rval = -1;
		goto ready;
	}
	if (row1 < tabRow0 || row1 > tabRow1) {
		LLLDEB(tabRow0, row1, tabRow1);
		rval = -1;
		goto ready;
	}
	...
	if (allChildren) {
		if (tedEditStartReplace(&dsTraced, teo, command, DOClevSECT,
					0)) {
			LDEB(1);
			goto ready;
		}
		...
		if (tedEditDeleteSelectedRows(&dpNew, &side, &dsRows, rowNode0,
					      rowNode1, teo)) {
			LDEB(1);
			rval = -1;
			goto ready;
...
static int tedDeleteRowsFromTable(EditDocument *ed, int command, int row0,
				  int row1, int traced)
{
	int rval = 0;
	TedEditOperation teo;
...
int tedDocDeleteTable(EditDocument *ed, int traced)
{
	...
	if (tedDeleteRowsFromTable(ed, EDITcmdDELETE_TABLE, tr.trRow00,
				   tr.trRow11, traced)) {
		LLDEB(tr.trRow00, tr.trRow11);
		return -1;
	}
...
int tedDocDeleteRow(EditDocument *ed, int traced)
{
	...
	if (tedDeleteRowsFromTable(ed, EDITcmdDELETE_ROW, tr.trRow0, tr.trRow1,
				   traced)) {
		LLDEB(tr.trRow00, tr.trRow11);
		return -1;
	}
...
static int tedDeleteColumnsFromRowsOperation(TedEditOperation *teo, int delRow0,
					     int delRow1, int delCol0,
					     int delCol1)
{
	EditOperation *eo = &(teo->teoEo);
	...
	if (tedEditStartReplace(&dsTraced, teo, EDITcmdDELETE_COLUMN,
				DOClevCOLUMN, 0)) {
		LDEB(1);
		goto ready;
	}
	...
	if (docDeleteColumnsFromRows(eo, parentNode, delRow0, delRow1, delCol0,
				     delCol1)) {
		LDEB(1);
		return -1;
	}
...
int tedDocDeleteColumn(EditDocument *ed, int traced)
{
	...
	if (tedDeleteColumnsFromRowsOperation(&teo, tr.trRow00, tr.trRow11,
					      tr.trCol0, tr.trCol1)) {
		LLDEB(tr.trRow00, tr.trRow11);
		rval = -1;
		goto ready;
...
int tedDeleteTableSliceSelection(EditDocument *ed, int traced)
{
	int rval = 0;
	...
	if (sd.sdTableRectangle.trIsRowSlice) {
		ret = tedDeleteRowsFromTableOperation(
			&teo, EDITcmdDELETE_ROW, sd.sdTableRectangle.trRow0,
	...
	if (sd.sdTableRectangle.trIsColSlice) {
		ret = tedDeleteColumnsFromRowsOperation(
			&teo, sd.sdTableRectangle.trRow0,
			sd.sdTableRectangle.trRow1, sd.sdTableRectangle.trCol0,
	...
	/*  4  */
	LLDEB(sd.sdTableRectangle.trIsRowSlice,
