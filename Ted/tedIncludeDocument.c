...
int tedIncludeDocument(TedEditOperation *teo, DocumentCopyJob *dcj)
{
	...
	if (eo->eoParagraphsInserted == 0) {
		...
		/*  C  */
		docSetParagraphAdjust(eo, eo->eoHeadDp.dpNode, stroffShift,
				      stroffTail);
	}
...
int tedIncludePlainDocument(EditDocument *ed, BufferDocument *bdFrom,
			    int traced)
{
	int rval = 0;
...
int tedIncludeRtfDocument(EditDocument *ed, BufferDocument *bdFrom, int traced)
{
	int rval = 0;
