...
static const int TED_FootnoteRestartCount =
...
static const int TED_EndnoteRestartCount =
...
static const int TED_EndnotePlacementCount =
	sizeof(TED_EndnotePlacements) / sizeof(int);
...
static void tedSetNotesMenu(AppOptionmenu *aom, int val, const int *values,
			    int valueCount)
{
	int i;
					    ...
					    const NotesProperties *np)
{
	if (np->npRestart == FTN_RST_CONTINUOUS) {
		appIntegerToTextWidget(nt->ntStartNumberText,
				       np->npStartNumber);
...
static void
tedNotesRefreshNotePropertiesTool(NotesTool *nt, const NotesProperties *np,
				  const int *restarts, int restartCount,
				  const int *placements, int placementCount)
{
	tedNotesRefreshNotesStartNumber(nt, np);

	if (placements) {
		guiEnableWidget(nt->ntPlacementRow,
				nt->ntCanChangeSelection ||
					nt->ntCanChangeDocument);
		appGuiEnableOptionmenu(&(nt->ntPlacementOptionmenu),
				       nt->ntCanChangeSelection ||
					       nt->ntCanChangeDocument);
		tedSetNotesMenu(&(nt->ntPlacementOptionmenu), np->npPlacement,
	...
	tedSetNotesMenu(&(nt->ntRestartOptionmenu), np->npRestart, restarts,
			restartCount);

	appSetOptionmenu(&(nt->ntJustifyOptionmenu), np->npJustification);
	appSetOptionmenu(&(nt->ntStyleOptionmenu), np->npNumberStyle);
...
static void tedNotesToolRefreshNotesWidgets(NotesTool *nt)
{
	if (nt->ntNoteTreeType == DOCinFOOTNOTE) {
		tedNotesRefreshNotePropertiesTool(
			nt, &(nt->ntNotesPropertiesChosen.fepFootnotesProps),
			TED_FootnoteRestarts, TED_FootnoteRestartCount,
			(int *)0, 0);
	}

	if (nt->ntNoteTreeType == DOCinENDNOTE) {
		tedNotesRefreshNotePropertiesTool(
			nt, &(nt->ntNotesPropertiesChosen.fepEndnotesProps),
			TED_EndnoteRestarts, TED_EndnoteRestartCount,
			TED_EndnotePlacements, TED_EndnotePlacementCount);
	}
...
static int tedNotesToolGetStartNumber(NotesTool *nt, NotesProperties *np)
{
	if (np->npRestart == FTN_RST_CONTINUOUS) {
		if (appGetIntegerFromTextWidget(nt->ntStartNumberText,
						&(np->npStartNumber), 1, 0,
						INT_MAX, 0)) {
			return -1;
		}
	}
...
static int tedNotesToolSetProperties(NotesTool *nt, int wholeDocument)
{
	int rval = 0;
...

	{
		...
		docFootEndNotesPropertyDifference(
			&dpSetMask, &(dpNew.dpNotesProps), &cmpMask,
...

	{
		...
		docFootEndNotesPropertyDifference(
			&spSetMask, &(spNew.spNotesProperties), &cmpMask,
			&(nt->ntNotesPropertiesSetSelection),
	...
	if (wholeDocument) {
		...
		if (tedAppChangeSectionProperties(nt->ntApplication, &spSetMask,
						  &spNew)) {
			LDEB(1);
			rval = -1;
			goto ready;
...
static void tedFootnoteStyleChosen(int n, void *voidnt)
{
	NotesTool *nt = (NotesTool *)voidnt;
	NotesProperties *np = &(nt->ntNotesPropertiesChosen.fepFootnotesProps);
...
static void tedEndnoteStyleChosen(int n, void *voidnt)
{
	NotesTool *nt = (NotesTool *)voidnt;
	NotesProperties *np = &(nt->ntNotesPropertiesChosen.fepEndnotesProps);
...
static void tedFootnoteRestartChosen(int n, void *voidnt)
{
	NotesTool *nt = (NotesTool *)voidnt;
	NotesProperties *np = &(nt->ntNotesPropertiesChosen.fepFootnotesProps);

	if (n < 0 || n >= TED_FootnoteRestartCount) {
		LLDEB(n, TED_FootnoteRestartCount);
		return;
	}
...
static void tedEndnoteRestartChosen(int n, void *voidnt)
{
	NotesTool *nt = (NotesTool *)voidnt;
	NotesProperties *np = &(nt->ntNotesPropertiesChosen.fepEndnotesProps);

	if (n < 0 || n >= TED_EndnoteRestartCount) {
		LLDEB(n, TED_EndnoteRestartCount);
		return;
	}
...
static void tedEndnotePlacementChosen(int n, void *voidnt)
{
	NotesTool *nt = (NotesTool *)voidnt;
	NotesProperties *np = &(nt->ntNotesPropertiesChosen.fepEndnotesProps);

	if (n < 0 || n >= TED_EndnotePlacementCount) {
		LLDEB(n, TED_EndnotePlacementCount);
		return;
	}

	if (TED_EndnotePlacements[n] < 0 ||
	    TED_EndnotePlacements[n] >= FTNplace_COUNT) {
		LDEB(TED_EndnotePlacements[n]);
		return;
	}
...
static void tedFootnoteJustifyChosen(int n, void *voidnt)
{
	NotesTool *nt = (NotesTool *)voidnt;
	NotesProperties *np = &(nt->ntNotesPropertiesChosen.fepFootnotesProps);
...
static void tedEndnoteJustifyChosen(int n, void *voidnt)
{
	NotesTool *nt = (NotesTool *)voidnt;
	NotesProperties *np = &(nt->ntNotesPropertiesChosen.fepEndnotesProps);
...
static void tedNotesMakeNotePropertiesTool(
	APP_WIDGET pageWidget, const char *title, NotesTool *nt,
	const NotesPageResources *npr, APP_TXACTIVATE_CALLBACK_T startCB,
	OptionmenuCallback justifyCB, OptionmenuCallback placementCB,
	OptionmenuCallback restartCB, OptionmenuCallback styleCB)
{
	const int textColumns = 6;

	guiToolMakeLabelAndMenuRow(&(nt->ntJustifyRow),
				   &(nt->ntJustifyOptionmenu),
				   &(nt->ntJustifyLabel), pageWidget,
				   npr->nprJustifyText, justifyCB, (void *)nt);

	if (placementCB) {
		guiToolMakeLabelAndMenuRow(&(nt->ntPlacementRow),
					   &(nt->ntPlacementOptionmenu),
					   &(nt->ntPlacementLabel), pageWidget,
					   npr->nprPositionText, placementCB,
					   (void *)nt);
	}

	guiToolMakeLabelAndMenuRow(&(nt->ntRestartRow),
				   &(nt->ntRestartOptionmenu),
				   &(nt->ntRestartLabel), pageWidget,
				   npr->nprRestartText, restartCB, (void *)nt);

	guiToolMakeLabelAndMenuRow(&(nt->ntStyleRow), &(nt->ntStyleOptionmenu),
				   &(nt->ntStyleLabel), pageWidget,
				   npr->nprStyleText, styleCB, (void *)nt);

	guiToolMakeLabelAndTextRow(&(nt->ntStartNumberRow),
				   &(nt->ntStartNumberLabel),
				   &(nt->ntStartNumberText), pageWidget,
				   npr->nprFirstNumberText, textColumns, 1);

	if (startCB) {
		appGuiSetGotValueCallbackForText(nt->ntStartNumberText, startCB,
						 (void *)nt);
	}
			    ...
			    const InspectorSubjectResources *isr, int treeType)
{
	...
	if (nt->ntNoteTreeType == DOCinFOOTNOTE) {
		tedNotesMakeNotePropertiesTool(
			pageWidget, npr->nprFootnotesText, nt, npr,
			tedNotesFootRestartChanged, tedFootnoteJustifyChosen,
			(OptionmenuCallback)0, tedFootnoteRestartChosen,
			tedFootnoteStyleChosen);
	}

	if (nt->ntNoteTreeType == DOCinENDNOTE) {
		tedNotesMakeNotePropertiesTool(
			pageWidget, npr->nprEndnotesText, nt, npr,
			tedNotesEndRestartChanged, tedEndnoteJustifyChosen,
			tedEndnotePlacementChosen, tedEndnoteRestartChosen,
			tedEndnoteStyleChosen);
	}

	/**************/

	guiToolMake2BottonRow(&row, pageWidget, &(nt->ntRevertSelectionWidget),
			      &(nt->ntChangeSelectionWidget),
			      npr->nprRevertSelectionText,
			      npr->nprChangeSelectionText,
			      tedNotesToolRevertSelectionPushed,
			      tedNotesToolChangeSelectionPushed, (void *)nt);

	guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
			      &(is->isRevertButton), &(is->isApplyButton),
			      isr->isrRevert, isr->isrApplyToSubject,
			      tedNotesToolRevertDocumentPushed,
			      tedNotesToolChangeDocumentPushed, (void *)nt);

	return;
}

static void tedNotePropertiesToolFillStyleChooser(NotesTool *nt,
						  const NotesPageResources *npr)
{
	...
	appFillInspectorMenu(FTNstyle_COUNT, FTNstyleNAR, nt->ntStyleOptions,
			     npr->nprNumberStyleMenuTexts,
...
static void
tedNotePropertiesToolFillChoosers(NotesTool *nt, const NotesPageResources *npr,
				  const int *restarts, int restartCount,
				  const int *placements, int placementCount)
{
	int i;
	...
	for (i = 0; i < restartCount; i++) {
		nt->ntRestartOptions[i] = appAddItemToOptionmenu(
			&(nt->ntRestartOptionmenu),
			npr->nprRestartMenuTexts[restarts[i]]);
	}

	for (i = 0; i < placementCount; i++) {
		nt->ntPlacementOptions[i] = appAddItemToOptionmenu(
			&(nt->ntPlacementOptionmenu),
...
void tedFormatFillNotesChoosers(NotesTool *nt, const NotesPageResources *npr)
{
	...
	if (nt->ntNoteTreeType == DOCinFOOTNOTE) {
		tedNotePropertiesToolFillChoosers(nt, npr, TED_FootnoteRestarts,
						  TED_FootnoteRestartCount,
						  (int *)0, 0);
	}

	if (nt->ntNoteTreeType == DOCinENDNOTE) {
		tedNotePropertiesToolFillChoosers(nt, npr, TED_EndnoteRestarts,
...
/**/
	...
	APP_RESOURCE("formatToolEndnotes",
		     offsetof(NotesPageResources, nprEndnotesText), "Endnotes"),
