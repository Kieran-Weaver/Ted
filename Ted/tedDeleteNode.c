...
static int tedEditDeleteSelectedParagaphs(DocumentPosition *dpNew, int *pSide,
...
int tedDocDeleteSelectedParagraphs(EditDocument *ed, int traced)
{
	...
	if (!sd.sdInContiguousParagraphs) {
		LDEB(sd.sdInContiguousParagraphs);
		rval = -1;
		goto ready;
	}
	...
	if (allChildren) {
		if (tedEditStartReplace(&dsTraced, &teo, EDITcmdDELETE_PARA,
					DOClevPARA, 0)) {
			LDEB(1);
			goto ready;
		}
...
static int tedEditDeleteSelectedSections(DocumentPosition *dpNew, int *pSide,
...
int tedDocDeleteSelectedSections(EditDocument *ed, int traced)
{
	...
	if (allChildren) {
		if (tedEditStartReplace(&dsTraced, &teo, EDITcmdDELETE_SECT,
					DOClevSECT, 0)) {
			LDEB(1);
			goto ready;
		}
		...
		if (tedEditDeleteSelectedSections(&dpNew, &side, sectNode0,
						  sectNode1, &teo)) {
			LDEB(1);
			rval = -1;
			goto ready;
