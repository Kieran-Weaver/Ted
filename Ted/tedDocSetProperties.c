	...
	const PropertyMask *dpSetMask, const DocumentProperties *dpSet)
{
	...
	if (docChangeSelectionProperties(
		    eo, ds, &selTaDoneMask, taSetMask, taSet, &selPpDoneMask,
		    ppSetMask, ppSet, &selCpDoneMask, cpSetMask, cpSet,
		    &selRpDoneMask, rpSetMask, rpSet, &selSpDoneMask, spSetMask,
		    spSet, &docDpDoneMask, dpSetMask, dpSet)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
	...
	const PropertyMask *dpSetMask, const DocumentProperties *dpSet)
{
	...
	if (teo->teoEditTrace) {
		if (docRtfTraceOldProperties(&dsTraced, eo, level, taSetMask,
					     ppSetMask, cpSetMask, rpSetMask,
					     spSetMask, dpSetMask)) {
			LDEB(1);
			rval = -1;
			goto ready;
		...
		if (docRtfTraceNewProperties(eo, taSetMask, taSet, ppSetMask,
					     ppSet, cpSetMask, cpSet, rpSetMask,
					     rpSet, spSetMask, spSet, dpSetMask,
					     dpSet)) {
			LDEB(1);
			rval = -1;
			goto ready;
	...
	if (tedEditChangeSelectionPropertiesImpl(
		    teo, &dsTraced, taSetMask, taSet, ppSetMask, ppSet,
		    cpSetMask, cpSet, rpSetMask, rpSet, spSetMask, spSet,
		    dpSetMask, dpSet)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (cpSetMask) {
		...
		if (docSelectionForEditPositionsInTree(
			    &dsNew, eo->eoTree, &(er->erHead), &(er->erTail))) {
			LDEB(1);
			return -1;
		}
	...
	if (docIsIBarSelection(ds) && taSetMask) {
		...
		if (!utilPropMaskIsEmpty(&taDoneMask)) {
			const IndexMapping *a2s =
				&(td->tdAttributeToScreenFont);

			sd->sdTextAttributeNumber =
				docTextAttributeNumber(bd, ta);

			td->tdCurrentScreenFont = utilIndexMappingGet(
				a2s, td->tdSelectionDescription
					     .sdTextAttributeNumber);
		}
	}
	...
	int traced)
{
	...
	if (tedEditChangeSelectionProperties(
		    &teo, &ds, level, command, taSetMask, taSet, ppSetMask,
		    ppSet, cpSetMask, cpSet, rpSetMask, rpSet, spSetMask, spSet,
		    dpSetMask, dpSet)) {
		LDEB(1);
		rval = -1;
		goto ready;
...
int tedDocChangeSectionProperties(EditDocument *ed,
				  ...
				  const SectionProperties *spSet, int traced)
{
	if (tedDocChangeSelectionProperties(
		    ed, EDITcmdUPD_SECT_PROPS, (const PropertyMask *)0,
		    ...
		    (const RowProperties *)0, spSetMask, spSet,
...
{
	if (tedDocChangeSelectionProperties(
		    ed, EDITcmdUPD_PARA_PROPS, (const PropertyMask *)0,
			       ...
			       const TextAttribute *taSet, int traced)
{
	...
	if (tedDocChangeSelectionProperties(
		    ed, EDITcmdUPD_SPAN_PROPS, taSetMask, taSet,
			     ...
			     const RowProperties *rpSet, int traced)
{
	...
	if (tedEditChangeSelectionProperties(
		    &teo, &ds, level, command, (const PropertyMask *)0,
		    (const TextAttribute *)0, (const PropertyMask *)0,
		    (const ParagraphProperties *)0, cpSetMask, cpSet, rpSetMask,
		    rpSet, (const PropertyMask *)0,
...
int tedDocSetParagraphTabs(EditDocument *ed, const TabStopList *tsl, int traced)
{
	TedDocument *td = (TedDocument *)ed->edPrivateData;
	BufferDocument *bd = td->tdDocument;
				     ...
				     int traced)
{
	...
	if (tedEditChangeSelectionProperties(
		    &teo, &ds, DOClevSECT, EDITcmdUPD_SECTDOC_PROPS,
...
{
	if (tedDocChangeSelectionProperties(
		    ed, EDITcmdUPD_DOC_PROPS, (const PropertyMask *)0,
		    (const TextAttribute *)0, (const PropertyMask *)0,
		    (const ParagraphProperties *)0, (const PropertyMask *)0,
		    (const CellProperties *)0, (const PropertyMask *)0,
		    (const RowProperties *)0, (const PropertyMask *)0,
		    (const SectionProperties *)0, dpSetMask, dpSet, traced)) {
		XDEB(dpSetMask);
		return -1;
	}
			 ...
			 int traced)
{
	...
	if (wholeDocument) {
		if (tedDocChangeAllSectionProperties(
			    ed, dgSetMask, &spSet, dgSetMask, &dpSet, traced)) {
			LDEB(wholeDocument);
			goto ready;
		}
	} else {
		if (tedDocChangeSectionProperties(ed, dgSetMask, &spSet,
						  traced)) {
			XDEB(dgSetMask);
			goto ready;
		}
...
int tedFormatShiftIndent(EditDocument *ed, int step, int traced)
{
	...
	if (teo.teoEditTrace) {
		if (docRtfTraceOldProperties(
			    &dsTraced, eo, DOClevPARA, (PropertyMask *)0,
			    &ppSetMask, (PropertyMask *)0, (PropertyMask *)0,
