...
extern void tedScrollToSelection(EditDocument *ed, int *pScrolledX,
				 int *pScrolledY);

extern void tedAdaptPageToolToDocument(EditApplication *ea, EditDocument *ed);
...
extern int tedSaveObjectPicture(RasterImage *abiTo, const InsertedObject *io);

extern void tedDrawRectangle(EditDocument *ed, DocumentRectangle *drClip,
			     int ox, int oy);
...
extern int tedMakeDocumentWidget(EditApplication *ea, EditDocument *ed);

extern void tedAdaptFormatToolToDocument(EditDocument *ed, int choosePage);
...
extern int tedNewDocument(EditDocument *ed, const MemoryBuffer *filename);

extern int tedPrintDocument(SimpleOutputStream *sos, const struct PrintJob *pj,
			    const struct PrintGeometry *pg);

extern void tedSuggestPageSetup(struct PrintGeometry *pg, void *privateData,
				int sheetSize);

extern void tedFreeDocument(void *voidtd, int format);

extern void tedRedoDocumentLayout(EditDocument *ed);

extern void tedFormatSetFindPattern(EditApplication *ea,
				    const char *findPattern, int findRegexp);

extern void tedHideObjectWindows(EditDocument *ed);

extern int tedSetIBarSelection(EditDocument *ed, const DocumentPosition *dp,
			       int lastLine, int *pScrolledX, int *pScrolledY);

extern int tedOpenDocumentFile(unsigned char *digest, int *pFormat,
...
extern int tedObjectDrag(APP_WIDGET w, EditDocument *ed, APP_EVENT *downEvent);

extern int tedOpenDocument(EditApplication *ea, void *voidtd, int *pFormat,
...
extern void tedToggleTextAttribute(EditDocument *ed, int set, int prop);

extern void tedSetFontSupersub(EditDocument *ed, int supersub);
...
extern void tedObserveFocus(EditDocument *ed, int inout);
...
extern void tedMakeDocumentReadonly(EditDocument *ed, int readonly);
