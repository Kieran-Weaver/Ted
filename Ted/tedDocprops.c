...
static AppConfigurableResource TED_PropertyDialogResourceTable[] = {
	...
	APP_RESOURCE("propDialogOk",
		     offsetof(DocumentPropertyPageResources, dpprOkText), "Ok"),
};
...
static void tedPropsDialogMakePropertyRow(APP_WIDGET *pRow, APP_WIDGET *pLabel,
					  ...
					  int textEnabled)
{
	...
	appMakeTextInRow(&text, row, textColumn, textColspan, textColumns,
			 textEnabled);
	...
	*pText = text;
	...
	if (textEnabled) {
		appGuiSetTypingCallbackForText(text, tedPropertyChanged,
					       (void *)dpd);
	}
...
static void
tedPropsDialogMakeTextForm(APP_WIDGET parent, PropertyDialog *dpd,
...
static APP_WIDGET
tedPropsDialogMakeButtonRow(APP_WIDGET parent, PropertyDialog *dpd,
			    const DocumentPropertyPageResources *dppr)
{
	APP_WIDGET row;
	...
	appMakeButtonInRow(&(cancelButton), row, dppr->dpprCancelText,
			   tedPropertiesCancelPushed, (void *)dpd, 1, colspan,
			   0);
					     ...
					     const char *pixmapName)
{
	...
	appMakeVerticalDialog(&(dpd->dpdDialog), &paned, ea,
			      tedClosePropertyDialog, (APP_DESTROY_CALLBACK_T)0,
			      (void *)dpd);

	appSetShellTitle(dpd->dpdDialog.adTopWidget, printOption,
...
static void tedDateToWidget(APP_WIDGET w, const struct tm *tm)
{
	if (tm->tm_mday == 0) {
...
int tedRunPropertyDialog(EditDocument *ed, const DocumentStatistics *ds,
			 APP_WIDGET option, const char *pixmapName)
{
	...
	/*  1  */
	if (!dpd) {
		dpd = TED_PropertyDialog =
			tedMakePropertyDialog(ed, option, pixmapName);
		if (!dpd) {
			XDEB(dpd);
			rval = -1;
			goto ready;
		...
		appSetShellTitle(dpd->dpdDialog.adTopWidget, option,
				 ea->eaApplicationName);

		appGuiShowDialog(ea, &(dpd->dpdDialog),
	...
	if (appBufferFromTextWidget(&(dpChosen.dpTitle), dpd->dpdTitleWidget)) {
		LDEB(1);
		rval = -1;
		goto ready;
	}
