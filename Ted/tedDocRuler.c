...
static int tedGetColumns(BufferItem *paraNode, const LayoutContext *lc,
			 const BlockFrame *bf, ColumnSeparator **pCs,
			 int *pCsCount)
{
	...
	if (!fresh) {
		LXDEB(rowNode->biRowCellCount, fresh);
		return -1;
	}
	cs = fresh;
	...
	for (i = 0; i < rowNode->biRowCellCount; fresh++, cp++, i++) {
		...
		if (i == 0) {
			fresh[0].csX0 =
				COORDtoGRID(xfac, pf.pfCellRect.drX0) - rg2p;
			fresh[0].csX1 =
				COORDtoGRID(xfac, pf.pfCellRect.drX0) + rg2p;
		}
	...
	*pCs = cs;
	*pCsCount = rowNode->biRowCellCount + 1;
	return 0;
}

static int tedUpdateTableColumns(EditDocument *ed, BufferItem *paraNode,
				 const BlockFrame *bf, const LayoutContext *lc,
				 int csCountNew, const ColumnSeparator *csNew)
{
	int rval = 0;
	...
	if (docDelimitTable(paraNode, &parentNode, (int *)0, &row0, &row,
			    &row1)) {
		LDEB(1);
		rval = -1;
		goto ready;
	...
	if (csNew[0].csX0 != cs[0].csX0) {
		...
		rowLeftIndentPixels =
			COORDtoGRID(xfac, rowNode->biRowLeftIndentTwips);

		rowLeftIndentPixels += (csNew[0].csX0 - cs[0].csX0);
		rpSet.rpLeftIndentTwips =
			GRIDtoCOORD(rowLeftIndentPixels, xfac);
		...
		for (csi = 1; csi < csCount; cp++, csi++) {
			if (csNew[csi].csX0 != cs[csi].csX0) {
				int rightBoundaryPixels = COORDtoGRID(
					xfac, cp->cpRightBoundaryTwips);

				rightBoundaryPixels +=
					(csNew[csi].csX0 - cs[csi].csX0);
	...
	if (changed) {
		...
		tedDocSetTableProperties(ed, wholeRow, wholeColumn,
					 ...
					 (const CellProperties *)0, &rpDifMask,
					 &rpSet, td->tdTraced);
	}
...
static int tedGetRulerTwips(const LayoutContext *lc, const BlockFrame *bf,
			    const ParagraphFrame *pf, BufferDocument *bd,
			    ParagraphProperties *ppNew, int firstIndentPixels,
			    int leftIndentPixels, int rightIndentPixels,
...
static APP_EVENT_HANDLER_H(tedTopRulerButtonDown, w, voided, downEvent)
{
	...
	firstIndentNew = COORDtoGRID(
		xfac, drParaContent->drX0 + paraNode->biParaFirstIndentTwips);
	rightIndentNew = COORDtoGRID(xfac, drParaContent->drX1);

	prop = tedTopRulerTrackMouse(&firstIndentNew, &leftIndentNew,
				     &rightIndentNew, &tslNew, &csCountNew,
				     &csNew, w, downEvent, ed);
	...
	case PPprop_COLUMNS:
		tedUpdateTableColumns(ed, paraNode, &bf, &lc, csCountNew,
				      csNew);
		goto ready;
	default:
		LDEB(prop);
	...
	if (tedGetRulerTwips(&lc, &bf, &pf, bd, &ppNew, firstIndentNew,
			     leftIndentNew, rightIndentNew, &tslNew)) {
		LDEB(1);
		goto ready;
	}
...
void tedDocAdaptTopRuler(EditDocument *ed, const DocumentSelection *ds,
			 ...
			 const BufferItem *bodySectNode)
{
	...
	if (sg->sgHead.pgLine >= paraNode->biParaLineCount) {
		LLDEB(sg->sgHead.pgLine, paraNode->biParaLineCount);
		return;
	}
	...
	firstIndent =
		COORDtoGRID(xfac, pf.pfParaContentRect.drX0 +
	...
	tedAdaptTopRuler(ed->edTopRuler, ed->edTopRulerWidget, drOutside.drX0,
			 drOutside.drX1, /*  doc */
			 drInside.drX0, drInside.drX1, /*  marg */
			 firstIndent, leftIndent, rightIndent, &tsl, pfX0Pixels,
			 xfac);
	...
	if (paraNode->biParaTableNesting > 0) {
		...
		if (docDelimitTable(paraNode, &parentBi, &col, &row0, &row,
				    &row1)) {
			LDEB(1);
			goto ready;
		}
		...
		tedSetRulerColumns(ed->edTopRulerWidget, ed->edTopRuler,
				   bfX0Pixels, bfX1Pixels, pfX0Pixels,
				   pfX1Pixels, cs, csCount);
	} else {
		tedSetRulerColumns(ed->edTopRulerWidget, ed->edTopRuler,
				   bfX0Pixels, bfX1Pixels, pfX0Pixels,
				   pfX1Pixels, (ColumnSeparator *)0, 0);
	}
...
int tedDocSetTopRuler(EditDocument *ed)
{
	...
	ed->edTopRuler = tedMakeTopRuler(
		...
		drOutside.drX0, drOutside.drX1, drInside.drX0, drInside.drX1,
		ed->edVisibleRect.drX0, ed->edVisibleRect.drX1,

		unitInt); /*  whatUnit		*/

	guiDrawSetConfigureHandler(ed->edTopRulerWidget, tedTopRulerConfigure,
				   (void *)ed->edTopRuler);

	guiDrawSetRedrawHandler(ed->edTopRulerWidget, tedRedrawTopRuler,
				(void *)ed->edTopRuler);
...
int tedDocSetBottomRuler(EditDocument *ed)
{
	...
	guiDrawSetConfigureHandler(ed->edBottomRulerWidget,
				   tedBottomRulerConfigure,
				   (void *)ed->edBottomRuler);

	guiDrawSetRedrawHandler(ed->edBottomRulerWidget, tedRedrawBottomRuler,
				(void *)ed->edBottomRuler);
