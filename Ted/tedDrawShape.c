...
static int tedShapeDrawArrowHead(const LayoutContext *lc, int widthPixels,
				 const DrawShapeArrow *dsa)
{
	...
	case DSarrowOPEN_ARROW:
		...
		drawSetLineAttributes(ds, widthPixels, LineStyleSolid,
...
static void tedShapeGetLine(int *pLine, int *pWidthPixels, int *pWidthTwips,
			    ...
			    ScreenDrawingData *sdd)
{
	...
	if (*pLine) {
		...
		drawSetLineAttributes(lc->lcDrawingSurface, widthPixels,
				      lineStyle, capStyle, joinStyle, dashList,
				      dashCount);
				...
				const DocumentRectangle *dr)
{
	...
	if (sd->sdRotation == 0) {
		...
		for (i = 0; i < np; sv++, i++) {
			...
			x = ((xs - sv->x) * dr->drX0 + sv->x * dr->drX1) / xs -
			    xm;
			y = ((ys - sv->y) * dr->drY0 + sv->y * dr->drY1) / ys -
			    ym;
			  ...
			  const DocumentRectangle *drPixels)
{
	...
	if (line) {
		...
		a2 = sd->sdAdjust2Value;
		a0 = SC - a2;
		xp[pp1].y = (a0 * xp[0].y + a2 * xp[2].y) / SC;
		pp1++;

		a2 = sd->sdAdjust3Value;
		a0 = SC - a2;
		xp[pp1].x = (a0 * xp[0].x + a2 * xp[2].x) / SC;
		a2 = sd->sdAdjust4Value;
		a0 = SC - a2;
		xp[pp1].y = (a0 * xp[0].y + a2 * xp[2].y) / SC;
		pp1++;
			...
			DrawingShape *ds, DrawingContext *dc, void *vsdd)
{
	...
	case SHPtyRIGHT_BRACE: {
	    ...
	    */
		...
		drawLine(drsf, xp[0].x + wide / 2,
			 (7 * xp[0].y + 1 * xp[2].y) / 8, xp[0].x + wide / 2,
			 (5 * xp[0].y + 3 * xp[2].y) / 8);
		...
		drawLine(drsf, xp[0].x + wide / 2,
			 (3 * xp[0].y + 5 * xp[2].y) / 8, xp[0].x + wide / 2,
			 (1 * xp[0].y + 7 * xp[2].y) / 8);
