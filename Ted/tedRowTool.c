...
static void tedFormatToolRefreshRowPage(RowTool *rt)
{
	...
	appIntervalToTextWidget(rt->rtRowText, tr->trRow0 - tr->trRow00 + 1,
				tr->trRow1 - tr->trRow00 + 1);

	guiEnableWidget(rt->rtRowRow, rt->rtCanChange);
	tedFormatEnableHeightChooser(&(rt->rtHeightChooser), rt->rtCanChange);
	...
	tedSetShadingToolByNumber(&(rt->rtShadingTool), bd,
				  rp->rpShadingNumber);
		       ...
		       const unsigned char *cmdEnabled)
{
	...
	docSetCellRectangleProperties(&(rt->rtCellPropertiesSet), rowNode,
				      &(rt->rtTableRectangle), dam0);
	...
	guiEnableWidget(is->isMoveUpButton, cmdEnabled[EDITcmdSHIFT_ROWS_UP]);
...
static APP_BUTTON_CALLBACK_H(tedFormatChangeRowPushed, w, voidrt)
{
	...
	tedDocSetTableProperties(ed, wholeRow, wholeColumn, &cpSetMask,
				 cpChosen, &rpSetMask, rpChosen, traced);
...
static void tedRowHeightChosen(int how, void *voidrt)
{
	RowTool *rt = (RowTool *)voidrt;
...
static void tedRowToolGotColor(void *voidrt, int which, const RGB8Color *rgb8)
{
	RowTool *rt = (RowTool *)voidrt;
	...
	case CLpropBOTTOM_BORDER:
		tedBorderSetExplicitColorChoice(&(rt->rtBottomBorderTool),
						rgb8);
		break;

	case RPprop_SHADE_BACK_COLOR:
	case RPprop_SHADE_FORE_COLOR:
		tedShadeSetExplicitColorChoice(&(rt->rtShadingTool), which,
					       rgb8);
		break;
...
static void tedRowToolMakeBorderFrame(RowTool *rt, const RowPageResources *rpr,
			  ...
			  const InspectorSubjectResources *isr)
{
	...
	/**************/
	guiToolMakeLabelAndTextRow(&(rt->rtRowRow), &(rt->rtRowLabel),
				   &(rt->rtRowText), pageWidget,
	...
	tedFormatMakeShadingTool(&(rt->rtShadingTool), ai, pageWidget,
				 ...
				 &(rpr->rprShadingResources), subjectPage,
				 RPprop_SHADE_FORE_COLOR,
				 RPprop_SHADE_BACK_COLOR,
				 (TedShadingToolCallback)0, (void *)rt);

	/**************/
	guiToolMake2ToggleRow(&(rt->rtKeepHeaderRow), pageWidget,
			      &(rt->rtKeepOnPageToggle),
			      &(rt->rtIsTableHeaderToggle), rpr->rprKeepOnPage,
			      rpr->rprIsTableHeader, tedRowToggleKeepOnPage,
			      tedRowToggleIsTableHeader, (void *)rt,
			      (void *)rt);

	guiToolMakeToggleAndLabelRow(&(rt->rtKeepfollowRow), pageWidget,
				     &(rt->rtKeepfollowToggle),
				     &(rt->rtKeepfollowLabel),
				     rpr->rprKeepWithNext, "",
	...
	guiToolMake2BottonRow(&(is->isNextPrevRow), pageWidget,
			      ...
			      tedFormatPreviousRow, tedFormatNextRow,
			      (void *)rt);

	{
		...
		guiToolMake2BottonRow(&row, pageWidget, &(is->isSelectButton),
				      ...
				      tedFormatSelectRow, tedFormatDeleteRow,
				      (void *)rt);
		...
		guiToolMake2BottonRow(&row, pageWidget, &(is->isInsertButton),
				      ...
				      tedFormatInsertRow, tedFormatAppendRow,
				      (void *)rt);

		guiToolMake2BottonRow(&row, pageWidget, &(is->isMoveUpButton),
				      &(is->isMoveDownButton),
				      isr->isrMoveUpButtonText,
				      isr->isrMoveDownButtonText,
				      tedRowToolMoveRowUp,
				      tedRowToolMoveRowDown, (void *)rt);
	}

	guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
			      &(is->isRevertButton), &(is->isApplyButton),
			      isr->isrRevert, isr->isrApplyToSubject,
			      tedFormatRevertRowPushed,
			      tedFormatChangeRowPushed, (void *)rt);
...
{
	tedFormatFillHeightChooser(&(rt->rtHeightChooser), rpr->rprHeightFree,
...
static AppConfigurableResource TED_TedRowToolResourceTable[] = {
	APP_RESOURCE("tableToolHeightFree",
		     offsetof(RowPageResources, rprHeightFree), "Height Free"),
	...
	APP_RESOURCE("tableToolCellMargin",
		     offsetof(RowPageResources,
			      rprMarginsResources.rmtrCellMargin),
		     "Cell Margin"),
	APP_RESOURCE("tableToolLeftIndent",
		     offsetof(RowPageResources,
			      rprMarginsResources.rmtrLeftIndent),
		     "Left Margin"),
	/**/
	APP_RESOURCE("tableToolRowBorders",
		     offsetof(RowPageResources, rprBorders), "Borders"),
	APP_RESOURCE("tableToolRowBorderTop",
		     offsetof(RowPageResources, rprTopBorder), "Top"),
	APP_RESOURCE("tableToolRowBorderBottom",
		     offsetof(RowPageResources, rprBottomBorder), "Bottom"),

	APP_RESOURCE("formatToolRowBorderColorChooserAutomatic",
		     offsetof(RowPageResources,
			      rprBorderToolResources.btrColorChooserResources
				      .ccrAutomaticColor),
		     "Automatic"),
	...
	APP_RESOURCE("formatToolRowShading",
		     offsetof(RowPageResources, rprShading), "Shading"),
	...
	APP_RESOURCE("formatToolRowShadingForeColor",
		     offsetof(RowPageResources,
			      rprShadingResources.strForeColor),
		     "Foreground"),
	APP_RESOURCE("formatToolRowShadingBackColor",
		     offsetof(RowPageResources,
			      rprShadingResources.strBackColor),
		     "Background"),

	/**/
	APP_RESOURCE("formatToolRowShadingBackColorChooserNoBackground",
		     offsetof(RowPageResources,
			      rprShadingResources.strBackColorChooserResources
				      .ccrAutomaticColor),
		     "No Background"),
	...
	APP_RESOURCE("formatToolRowShadingForeColorChooserAutomatic",
		     offsetof(RowPageResources,
			      rprShadingResources.strForeColorChooserResources
				      .ccrAutomaticColor),
		     "Automatic"),
	...
	APP_RESOURCE("formatToolRowBorderWidth",
		     offsetof(RowPageResources,
			      rprBorderToolResources.btrWidth),
		     "Width"),
	APP_RESOURCE("formatToolRowBorderStyle",
		     offsetof(RowPageResources,
			      rprBorderToolResources.btrStyle),
		     "Style"),
	APP_RESOURCE("formatToolRowBorderColor",
		     offsetof(RowPageResources,
			      rprBorderToolResources.btrColor),
		     "Color"),
};
