			   ...
			   const MemoryBuffer *markName)
{
	...
	sioOutClose(sosCmd);
	sosCmd = (SimpleOutputStream *)0;

	if (system(utilMemoryBufferGetString(&mbCmd))) {
		SDEB(utilMemoryBufferGetString(&mbCmd));
		rval = -1;
	}
...
static void tedAnalyseFileNamePart(int *pIsFile, int *pIsAbsolute,
				   int *pProtocolSize,
				   const MemoryBuffer *fileBuffer)
{
	...
	if (isAbsolute) {
		...
		while (protocolSize < fileSize &&
		       isalpha(fileName[protocolSize])) {
			protocolSize++;
		}
...
int tedDocFollowLink(APP_WIDGET option, EditDocument *edFrom,
		     const MemoryBuffer *fileName, const MemoryBuffer *markName)
{
	...
	if (!utilMemoryBufferIsEmpty(&(dpFrom->dpHlinkbase))) {
		tedAnalyseFileNamePart(&baseIsFile, &baseIsAbsolute,
				       &baseProtocolSize,
				       &(dpFrom->dpHlinkbase));
	...
	if (fileName) {
		...
		tedAnalyseFileNamePart(&fileIsFile, &fileIsAbsolute,
				       &fileProtocolSize, fileName);
		if (!fileIsFile) {
		...
		if (utilMemoryBufferGetRange(&file, fileName, fileProtocolSize,
					     fileName->mbSize)) {
			LDEB(1);
			rval = -1;
			goto ready;
		...
		if (fileIsAbsolute || utilMemoryBufferIsEmpty(&base)) {
			edTo = appOpenDocument(ea, relative, option, readOnly,
					       &file);
			if (!edTo) {
				SXDEB(scratch, edTo);
			}
