...
extern int tedDocReplace(EditDocument *ed, const char *word);

extern int tedDocAddRowToTable(EditDocument *ed, int after, int traced);

extern int tedDocAddColumnToTable(EditDocument *ed, int after, int traced);

extern int tedDocSetTableProperties(EditDocument *ed, int wholeRow,
				    int wholeColumn,
...
extern int tedDocSetImageProperties(EditDocument *ed,
				    const PropertyMask *pipSetMask,
				    const PictureProperties *pip, int traced);

extern int tedDocSetHyperlink(EditDocument *ed, const HyperlinkField *hf,
			      int traced);

extern int tedDocRemoveHyperlink(EditDocument *ed, int traced);

extern int tedDocFollowLink(APP_WIDGET option, EditDocument *ed,
			    const MemoryBuffer *fileName,
			    const MemoryBuffer *markName);

extern int tedDocDeleteSelectedParagraphs(EditDocument *ed, int traced);

extern int tedDocDeleteSelectedSections(EditDocument *ed, int traced);

extern int tedDocInsertParagraph(EditDocument *ed, int after, int traced);

extern int tedDocInsertSection(EditDocument *ed, int after, int traced);

extern int tedDocSetDocumentProperties(EditDocument *ed,
				       const PropertyMask *dpSetMask,
				       const DocumentProperties *dpSet,
				       int traced);

extern int tedDocChangeCurrentNote(EditDocument *ed,
				   const PropertyMask *npSetMask,
				   const NoteProperties *npSet, int traced);

extern int tedDocChangeSectionProperties(EditDocument *ed,
					 const PropertyMask *spUpdMask,
					 const SectionProperties *spNew,
					 int traced);

extern int tedDocChangeAllSectionProperties(EditDocument *ed,
					    const PropertyMask *spUpdMask,
					    const SectionProperties *spNew,
					    const PropertyMask *dpSetMask,
...
extern int tedDocSetParagraphTabs(EditDocument *ed,
				  const struct TabStopList *tsl, int traced);

extern int tedDocSetNewList(EditDocument *ed, int traced);

extern int tedDocChangeCurrentList(EditDocument *ed,
				   const struct DocumentList *dlNew,
				   int traced);

extern int tedDocDeleteHeaderFooter(EditDocument *ed, int treeType, int traced);

extern int tedDocInsertHeaderFooter(EditDocument *ed, int treeType, int traced);

extern void tedDocEditHeaderFooter(EditDocument *ed, APP_WIDGET relative,
				   APP_WIDGET option, int treeType);

extern int tedDocInsertNote(EditDocument *ed, int noteTreeType, int traced);
...
extern void tedDocSelectRow(EditDocument *ed, int direction, int allColumns);
extern void tedDocSelectWholeCell(EditDocument *ed, int direction, int allRows);

extern int tedDocSelectColumn(EditDocument *ed, int col);

extern int tedDocDeleteTable(EditDocument *ed, int traced);
extern int tedDocDeleteRow(EditDocument *ed, int traced);
extern int tedDocDeleteColumn(EditDocument *ed, int traced);

extern int tedDocSelectCurrentFrame(EditDocument *ed);
extern int tedDocDeleteCurrentFrame(EditDocument *ed, int traced);

extern int tedDocSetBookmark(EditDocument *ed, const MemoryBuffer *markName,
			     int traced);

extern int tedDocGoToBookmark(EditDocument *ed, const MemoryBuffer *markName);

extern int tedDocRemoveBookmark(EditDocument *ed, int traced);

extern int tedDocFindBookmarkField(DocumentField **pDf, EditDocument *ed,
				   const MemoryBuffer *markName);

extern int tedDocSetTocField(EditDocument *ed, const TocField *tf, int traced);

extern void tedDocDeleteTocField(EditDocument *ed, int traced);

extern void tedDocAddTocField(EditDocument *ed, const TocField *tf, int traced);

extern void tedDocChangeTextAttribute(EditDocument *ed,
				      const PropertyMask *taSetMask,
				      const TextAttribute *taSet, int traced);

extern int tedDocShiftRowsInTable(EditDocument *ed, int direction);

extern int tedDocListFontToolSet(EditDocument *ed,
				 const PropertyMask *taSetMask,
				 const ExpandedTextAttribute *etaSet);

extern int tedDocFontToolSet(EditDocument *ed, const PropertyMask *taSetMask,
			     const ExpandedTextAttribute *etaSet);

extern void tedDocFormatSetParaAlignment(EditDocument *ed, int align,
					 int traced);

extern void tedDocSetParaOutlineLevel(EditDocument *ed, int level, int traced);

extern void tedDocSetPageLayout(EditDocument *ed, const PropertyMask *dgSetMask,
				const DocumentGeometry *dgSet,
				int wholeDocument, int traced);

extern int tedFormatShiftIndent(EditDocument *ed, int step, int traced);

extern int tedObjectOpenImage(EditApplication *ea, void *voided,
...
extern void tedClipboardLost(APP_WIDGET w, void *voided, APP_EVENT *clrEvent);

extern void tedPrimaryLost(APP_WIDGET w, void *voided, APP_EVENT *clrEvent);
...
extern void tedDocGotKey(void *voided, int keySym, unsigned int state);

extern void tedDocGotString(void *voided, const char *string, int length);

extern void tedDocSplitParagraph(struct EditDocument *ed, int onNewPage,
				 int traced);

extern int tedDocInsertSectBreak(struct EditDocument *ed, int traced);
