...
static int tedRowToolGetLeftIndent(int *pValue, RowMarginsTool *rmt,
				   const RowProperties *rp)
{
	...
	if (appGetLengthFromTextWidget(rmt->rmtLeftIndentText, &value, &changed,
	...
	if (value < -rmt->rmtParentLeftMargin) {
		...
		appLengthToTextWidget(rmt->rmtLeftIndentText, value,
				      UNITtyPOINTS);
	...
	if (value + 2 * rp->rpHalfGapWidthTwips >=
		...
		value = rp->rpCells[0].cpRightBoundaryTwips -
			3 * rp->rpHalfGapWidthTwips;
			      ...
			      int canChange)
{
	...
	appLengthToTextWidget(rmt->rmtCellMarginText, rp->rpHalfGapWidthTwips,
			      UNITtyPOINTS);

	guiEnableWidget(rmt->rmtLeftIndentRow, canChange);
	guiEnableText(rmt->rmtLeftIndentText, canChange);
	appLengthToTextWidget(rmt->rmtLeftIndentText, rp->rpLeftIndentTwips,
			      UNITtyPOINTS);
...
int tedRowMarginToolGetValues(RowProperties *rp, RowMarginsTool *rmt)
{
	...
	if (appGetLengthFromTextWidget(rmt->rmtCellMarginText, &value, &changed,
				       UNITtyPOINTS, minValue, adaptToMin,
				       maxValue, adaptToMax)) {
		return -1;
	}
...
static int tedFormatToolGetGapWidth(const RowProperties *rp, int *pValue,
				    APP_WIDGET w)
{
	int value;
	int changed;
	...
	for (col = 1; col < rp->rpCellCount; cp++, col++) {
		...
		cellWidth =
			cp[1].cpRightBoundaryTwips - cp[0].cpRightBoundaryTwips;
...
int tedMarginToolCheckCellMargin(RowMarginsTool *rmt, RowProperties *rp)
{
	int value;

	if (!tedFormatToolGetGapWidth(rp, &value, rmt->rmtCellMarginText)) {
		appLengthToTextWidget(rmt->rmtCellMarginText, value,
				      UNITtyPOINTS);
	}
				 ...
				 void *through, APP_WIDGET pageWidget)
{
	...
	guiToolMakeLabelAndTextRow(&(rmt->rmtLeftIndentRow),
				   ...
				   &(rmt->rmtLeftIndentText), pageWidget,
				   rmtr->rmtrLeftIndent, textColumns, 1);
	...
	guiToolMakeLabelAndTextRow(&(rmt->rmtCellMarginRow),
				   ...
				   &(rmt->rmtCellMarginText), pageWidget,
				   rmtr->rmtrCellMargin, textColumns, 1);
...
int tedRowMarginToolGetMargins(RowMarginsTool *rmt, BufferItem *rowNode,
			       BufferDocument *bd)
{
	int rval = 0;
	BlockFrame bf;
	...
	docBlockFrameTwips(&bf, rowNode, bd, rowNode->biTopPosition.lpPage,
	...
	case DOClevSECT: {
		...
		rmt->rmtParentWidth =
			dg->dgPageWideTwips + dg->dgRightMarginTwips;
	} break;
