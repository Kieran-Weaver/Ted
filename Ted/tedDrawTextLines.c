...
int tedDrawRunUnderline(const DrawTextLine *dtl, int part, int upto,
			int textAttrNr, const TextAttribute *ta, int x0Twips,
			int x1Twips, const LayoutPosition *baseLine)
{
	...
	screenFont =
		utilIndexMappingGet(lc->lcAttributeToScreenFont, textAttrNr);

	x0 = x0FrameShifted + tp[part].tpXContentXPixels;
	drawFontUnderlineGeometry(&h, &y0, lc->lcDrawingSurface, screenFont,
				  baselinePixels);
			    ...
			    const LayoutPosition *baseLine)
{
	...
	x0 = x0FrameShifted + tp[part].tpXContentXPixels;
	drawFontStrikethroughGeometry(&h, &y0, lc->lcDrawingSurface, screenFont,
				      baselinePixels);
...
int tedDrawTab(const DrawTextLine *dtl, int part, int textAttrNr,
	       const TextAttribute *ta, int leader, int x0Twips, int x1Twips,
	       const LayoutPosition *baseLine)
{
	ScreenDrawingData *sdd = (ScreenDrawingData *)dtl->dtlThrough;
	DrawingContext *dc = dtl->dtlDrawingContext;
	...
	case DOCtlDOTS:
		...
		if (ta->taFontIsBold) {
			...
			drawSetLineAttributes(lc->lcDrawingSurface, 2,
					      LineStyleSingleDash, LineCapButt,
					      LineJoinMiter, boldDot,
					      sizeof(boldDot));
		} else {
			static const unsigned char dot[] = { 1, 2 };

			drawSetLineAttributes(lc->lcDrawingSurface, 1,
					      LineStyleSingleDash, LineCapButt,
		...
		drawLine(lc->lcDrawingSurface, drSpan.drX0 - lc->lcOx,
			 baselinePixels - lc->lcOy, drSpan.drX1 - lc->lcOx,
			 baselinePixels - lc->lcOy);
...
static void tedDrawSegments(DrawingContext *dc, int x, int y,
			    ...
			    int segmentCount)
{
	...
	for (seg = 0; seg < segmentCount; seg++) {
		if (segments[2 * seg + 0] > 0) {
			drawString(lc->lcDrawingSurface, x, y, fullScreenFont,
				   s, segments[2 * seg + 0]);
		...
		if (segments[2 * seg + 1] > 0) {
			drawString(lc->lcDrawingSurface, x, y, scapsScreenFont,
				   s, segments[2 * seg + 1]);
...
int tedDrawSpan(const DrawTextLine *dtl, int part, int count,
		...
		const TextAttribute *ta, const char *printString, int nbLen)
{
	int rval = 0;
	...
	fullScreenFont =
		utilIndexMappingGet(lc->lcAttributeToScreenFont, textAttrNr);
	if (fullScreenFont < 0) {
		LLDEB(textAttrNr, fullScreenFont);
		rval = -1;
		goto ready;
	}
	...
	case TEXTvaSUBSCRIPT:
		drawFontGetSubBaseline(&y, baselinePixels, lc->lcDrawingSurface,
				       fullScreenFont);
		break;
	...
	if (ta->taSmallCaps && !ta->taCapitals) {
		...
		tedDrawSegments(dc, spanX0 - lc->lcOx, y - lc->lcOy,
				fullScreenFont, scapsScreenFont, printString,
				segments, segmentCount);
	} else {
		drawString(lc->lcDrawingSurface, spanX0 - lc->lcOx,
			   y - lc->lcOy, fullScreenFont, printString, nbLen);
	}
			    ...
			    const DocumentRectangle *drParaContent)
{
	...
	tedSelectionGeometry(&sgLine, &dsLine,
			     docGetBodySectNode(paraNode, lc->lcDocument),
			     lastLine, lc);
	...
	if (cmp_ShLh <= 0 || (cmp_ShLh == 0 && tableRectangle)) {
		...
		drLine.drX0 = sg->sgHead.pgXPixels;
	}
	...
	if (cmp_StLt > 0 || (cmp_StLt == 0 && tableRectangle)) {
		...
		drLine.drX1 = sg->sgTail.pgXPixels;
	}

	/*  5  */
	inSelection = 0;

	if (docParaStrlen(ds->dsTail.dpNode) == 0 && cmp_StLh == 0) {
		cmp_StLh = 1;
	}
	if (docParaStrlen(ds->dsHead.dpNode) == 0 && cmp_ShLt == 0) {
		cmp_ShLt = -1;
	}
