			     ...
			     int section, int sections)
{
	...
	if (tbr->tbrSection != section) {
		tbr->tbrSection = section;
		changed = 1;
	}
	if (tbr->tbrSections != sections) {
		tbr->tbrSections = sections;
		changed = 1;
	}
			       ...
			       DocumentRectangle *drClip)
{
	...
	drawString(ds, x, y, rd->rdScreenFont, tbr->tbrPageString,
		   tbr->tbrPageStringLength);
...
APP_EVENT_HANDLER_H(tedRedrawBottomRuler, w, voidtbr, event)
{
	...
	if (!rd->rdDrawingSurface) {
		...
		appRulerMakeDrawingSurface(rd, vertical, fontSizeHintPixels,
					   magnification, w);
	}
