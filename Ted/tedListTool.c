...
static int tedListToolSetFormatLevel(ListTool *lt, int number);
...
static void tedListToolSetCurrentLevelPath(int *numberPath, int level,
					   ListTool *lt)
{
	int l;
...
static int tedListToolFormatLevelText(char *listtext, int maxlen, int *offsets,
				      int maxoffsets, ListTool *lt,
				      const ListLevel *ll, int level)
{
	...
	size = docListLevelFormatParagraphNumber(listtext, maxlen, offsets,
						 maxoffsets, level, numberPath,
						 lt->ltStartPath,
						 lt->ltFormatPath, ll);
	if (size < 0) {
		listtext[0] = '\0';
...
static int tedListToolSelectNumberFormatRange(ListTool *lt, const int off0,
					      const int off1)
{
	...
	/*  2  */
	if (off0 == off1 && IS_NUMBER(idx0) && idx0 < 2 * fieldCount + 1 &&
	...
	/*  6  */
	if (lt->ltFormatOffset0 != off0 || lt->ltFormatOffset1 != off1) {
		return 1;
	}
...
static void tedListToolSetLevelFormatText(ListTool *lt, const char *text)
{
	...
	if (sizeof(GtkSignalFunc) != sizeof(gpointer)) {
		LLDEB(sizeof(GtkSignalFunc), sizeof(gpointer));
		return;
	}
					  ...
					  int level)
{
	...
	appTextSelectContents(lt->ltNumberFormatText, lt->ltFormatOffset0,
			      lt->ltFormatOffset1);
...
static int tedListToolModifyConstPiece(ListLevel *ll, int field, int off0,
				       int off1, const char *text, int length)
...
static int tedListToolModifyFormatText(ListTool *lt, const char *text,
				       int length)
{
	...
	if (off1 < lt->ltNumberFormatOffsets[idx0]) {
		LLDEB(off1, lt->ltNumberFormatOffsets[idx0]);
		return -1;
	}
	...
	if (tedListToolModifyConstPiece(ll, idx0 / 2, off0, off1, text,
					length)) {
		LDEB(idx0 / 2);
		return -1;
	}
	...
	while (idx1 > idx0) {
		...
		if (tedListToolModifyConstPiece(ll, idx1 / 2, off0, off1, "",
						0)) {
			LDEB(idx1 / 2);
			return -1;
		}
	...
	while (idx1 >= idx0) {
		docListLevelDeleteNumber(ll, idx1 / 2);
		idx1 -= 2;
	}
...
static int tedListToolSetFormatLevel(ListTool *lt, int number)
{
	...
	if (IS_STRING(idx0)) {
		...
		if (IS_STRING(idx1)) {
			...
			/*  +1: '\0'  */
			off1 = lt->ltFormatOffset1 -
			       lt->ltNumberFormatOffsets[idx1];
			memmove(scratch + split, scratch + split + off1,
				size - off1 + 1);
		} else {
			size = docListLevelGetText(scratch + split,
						   sizeof(scratch) - split - 1,
	...
	if (split >= 0) {
		...
		docListLevelSetText(&changed, ll, scratch + split,
				    idx0 / 2 + 1);
		scratch[split] = '\0';
...
static void tedListToolReplaceText(ListTool *lt, const char *text, int length)
{
	DocumentList *dl = &(lt->ltListPropertiesChosen);
	const int level = lt->ltCurrentLevel;
	ListLevel *ll = &(dl->dlLevels[level]);
...
static void tedListToolRefreshIndents(ListTool *lt, const ListLevel *ll)
{
	appLengthToTextWidget(lt->ltFirstIndentText,
			      ll->llLeftIndentTwips + ll->llFirstIndentTwips,
			      UNITtyPOINTS);

	appLengthToTextWidget(lt->ltLeftIndentText, ll->llLeftIndentTwips,
			      UNITtyPOINTS);
}
...
static void tedListToolSelectLevel(ListTool *lt, int level)
{
	const DocumentList *dl = &(lt->ltListPropertiesChosen);
...
static void tedFormatToolRefreshListPage(ListTool *lt)
{
	...
	if (dl->dlLevelCount > 0) {
		...
		docListGetFormatPath(lt->ltStartPath, lt->ltFormatPath, &ll,
				     dl->dlLevelCount - 1, dl, lo);

		ll = dl->dlLevels;
		for (level = 0; level < dl->dlLevelCount; ll++, level++) {
			tedListToolFormatLevelText(listtext,
						   sizeof(listtext) - 1,
						   (int *)0, 0, lt, ll, level);

			sprintf(listScratch, "%d: %s", level + 1, listtext);

			appGuiAddValueToListWidget(lt->ltListLevelList, level,
						   listScratch);
		}
...
static void tedListEnableListLevel(ListTool *lt, int enabled)
{
...
static void tedFormatToolNoList(ListTool *lt, InspectorSubject *is)
{
...
static int tedFormatToolSetList(ListTool *lt, InspectorSubject *is, int paraNr,
				int ls, int level, ListOverride *lo,
				DocumentList *dl, ListNumberTreeNode *root)
{
	...
	if (paraNr != lt->ltCurrentParagraphNumber || lt->ltCurrentLevel < 0 ||
			...
			const unsigned char *cmdEnabled)
{
	...
	if (ds->dsSelectionScope.ssTreeType != DOCinBODY || !sd->sdHasLists ||
	    sd->sdListOverride < 1 ||
	    docGetListOfParagraph(&lo, &dl, sd->sdListOverride, bd)) {
...
static APP_BUTTON_CALLBACK_H(tedFormatListSetLevelPushed, w, voidlt)
{
	...
	if (lt->ltCurrentLevel < 0 || lt->ltCurrentLevel >= dl->dlLevelCount) {
		LLDEB(lt->ltCurrentLevel, dl->dlLevelCount);
		return;
	}
...
static int tedListToolGetLevelAttributes(ListTool *lt, ListLevel *ll)
{
	int li;
	int lipfi;
	int changed;
	...
	if (appGetLengthFromTextWidget(lt->ltFirstIndentText, &lipfi, &changed,
				       UNITtyPOINTS, INT_MIN, 0, maxValue,
				       adaptToMax)) {
		return -1;
	}

	li = ll->llLeftIndentTwips;
	if (appGetLengthFromTextWidget(lt->ltLeftIndentText, &li, &changed,
				       UNITtyPOINTS, 0, 0, maxValue,
				       adaptToMax)) {
		return -1;
	}
	...
	/**/
	if (appGetIntegerFromTextWidget(lt->ltStartAtText, &(ll->llStartAt), 1,
					0, INT_MAX, 0)) {
		return -1;
	}
...
static APP_LIST_CALLBACK_H(tedListToolLevelChosen, w, voidlt, voidlcs)
{
	...
	if (lt->ltCurrentLevel >= 0 && lt->ltCurrentLevel < dl->dlLevelCount) {
		tedListToolGetLevelAttributes(
			lt, &(dl->dlLevels[lt->ltCurrentLevel]));
	}
...
static void tedListNumberStyleChosen(int style, void *voidlt)
{
	ListTool *lt = (ListTool *)voidlt;
	DocumentList *dl = &(lt->ltListPropertiesChosen);
	int level = lt->ltCurrentLevel;
	...
	if (style < 0 || style >= LISTTOOLcountSTYLES) {
		LLDEB(style, LISTTOOLcountSTYLES);
		return;
	}
...
static void tedListJustifyChosen(int justify, void *voidlt)
{
	ListTool *lt = (ListTool *)voidlt;
	DocumentList *dl = &(lt->ltListPropertiesChosen);
	int level = lt->ltCurrentLevel;
	...
	if (ll->llJustification == DOCllaCENTER ||
		...
		if (ll->llLeftIndentTwips < 20) {
			...
			PROPmaskADD(&(ll->llParaPropertyMask),
				    PPpropLEFT_INDENT);
			refresh = 1;
		}
...
static void tedListFollowChosen(int follow, void *voidlt)
{
	ListTool *lt = (ListTool *)voidlt;
	DocumentList *dl = &(lt->ltListPropertiesChosen);
	int level = lt->ltCurrentLevel;
...
static APP_TXACTIVATE_CALLBACK_H(tedLevelFirstIndentChanged, w, voidlt)
{
	...
	if (tedFormatValidateDimension(&lipfi, &changed, lt->ltFirstIndentText,
				       lipfi)) {
		return;
	}
...
static APP_TXACTIVATE_CALLBACK_H(tedLevelLeftIndentChanged, w, voidlt)
{
	...
	if (tedFormatValidateDimension(&li, &changed, lt->ltLeftIndentText,
				       li)) {
		return;
	}
...
static void tedListToolFormatMouseMove(APP_WIDGET w, APP_EVENT *event,
				       void *voidlt)
{
	ListTool *lt = (ListTool *)voidlt;
			    ...
			    const InspectorSubjectResources *isr)
{
	...
	/**/
	appGuiMakeListInColumn(&(lt->ltListLevelList), pageWidget, visibleItems,
			       tedListToolLevelChosen, (APP_BUTTON_CALLBACK_T)0,
			       (void *)lt);

	/**/
	appMakeColumnFrameInColumn(&(lt->ltSelectionFrame),
				   &(lt->ltSelectionPaned), pageWidget,
				   lpr->lprSelection);

	guiToolMake2BottonRow(&row, lt->ltSelectionPaned,
			      &(lt->ltSetListButton),
			      &(lt->ltRemoveFromListButton), lpr->lprSetList,
			      lpr->lprRemoveFromList,
	...
	appMakeColumnFrameInColumn(&(lt->ltListLevelFrame),
				   &(lt->ltListLevelPaned), pageWidget,
				   lpr->lprListLevel);
	...
	gtk_signal_connect(GTK_OBJECT(lt->ltNumberFormatText), "delete_text",
			   (GtkSignalFunc)tedListToolFormatDeleteText,
			   (void *)lt);

	gtk_signal_connect_after(GTK_OBJECT(lt->ltNumberFormatText),
				 "key_press_event",
				 (GtkSignalFunc)tedListToolNumberKeyPress,
				 (void *)lt);

	gtk_signal_connect_after(GTK_OBJECT(lt->ltNumberFormatText),
				 "button_release_event",
				 (GtkSignalFunc)tedListToolFormatMouseUp,
				 (void *)lt);

	gtk_signal_connect_after(GTK_OBJECT(lt->ltNumberFormatText),
				 "motion_notify_event",
				 (GtkSignalFunc)tedListToolFormatMouseMove,
				 (void *)lt);
	...
	appGuiSetGotValueCallbackForText(
		lt->ltFirstIndentText, tedLevelFirstIndentChanged, (void *)lt);
	/**/
	...
	guiToolMakeLabelAndTextRow(&row, &startLabel, &(lt->ltStartAtText),
				   lt->ltListLevelPaned, lpr->lprStartAt,
				   textColumns, 1);
...
void tedFormatFillListChoosers(ListTool *lt)
{
	...
	appFillInspectorMenu(LISTTOOLcountSTYLES, 0, lt->ltNumberStyleItems,
			     lpr->lprStyleOptionTexts,
			     &(lt->ltNumberStyleOptionmenu));

	appFillInspectorMenu(DOClla_COUNT, DOCllaLEFT, lt->ltJustifyItems,
			     lpr->lprJustifyOptionTexts,
			     &(lt->ltJustifyOptionmenu));

	appFillInspectorMenu(DOCllf_COUNT, DOCllfTAB, lt->ltFollowItems,
			     lpr->lprFollowOptionTexts,
...
static AppConfigurableResource TED_TedListToolResourceTable[] = {
	...
	APP_RESOURCE("listToolSelection",
		     offsetof(ListsPageResources, lprSelection), "Selection"),
	APP_RESOURCE("listToolSetList",
		     offsetof(ListsPageResources, lprSetList), "Set List"),
	APP_RESOURCE("listToolRemoveFromList",
		     offsetof(ListsPageResources, lprRemoveFromList),
		     "Remove from List"),
	APP_RESOURCE("listToolSetLevel",
		     offsetof(ListsPageResources, lprSetLevel), "Set Level"),
	APP_RESOURCE("listToolNewList",
		     offsetof(ListsPageResources, lprNewList), "New List"),

	/**/
	APP_RESOURCE("listToolListLevel",
		     offsetof(ListsPageResources, lprListLevel), "Level %d"),
	...
	APP_RESOURCE("listToolListLevelStartAt",
		     offsetof(ListsPageResources, lprStartAt), "First Number"),
	APP_RESOURCE("listToolListLevelNoRestart",
		     offsetof(ListsPageResources, lprNoRestart), "Continuous"),

	/**/
	APP_RESOURCE("listToolLevelNumberStyle",
		     offsetof(ListsPageResources, lprNumberStyle), "Style"),
	APP_RESOURCE("listToolLevelNumberFormat",
		     offsetof(ListsPageResources, lprNumberFormat), "Format"),
	...
	APP_RESOURCE("listToolLevelJustify",
		     offsetof(ListsPageResources, lprJustify), "Justify"),
	/**/
	APP_RESOURCE("listToolListLevelJustifyLeft",
		     offsetof(ListsPageResources,
			      lprJustifyOptionTexts[DOCllaLEFT]),
		     "Left"),
	APP_RESOURCE("listToolListLevelJustifyCentered",
		     offsetof(ListsPageResources,
			      lprJustifyOptionTexts[DOCllaCENTER]),
		     "Centered"),
	APP_RESOURCE("listToolListLevelJustifyRight",
		     offsetof(ListsPageResources,
			      lprJustifyOptionTexts[DOCllaRIGHT]),
		     "Right"),

	/**/
	APP_RESOURCE("listToolLevelFollowedBy",
		     offsetof(ListsPageResources, lprFollowedBy), "Separator"),
	/**/
	APP_RESOURCE("listToolListLevelFollowTab",
		     offsetof(ListsPageResources,
			      lprFollowOptionTexts[DOCllfTAB]),
		     "Tab"),
	APP_RESOURCE("listToolListLevelFollowSpace",
		     offsetof(ListsPageResources,
			      lprFollowOptionTexts[DOCllfSPACE]),
		     "Space"),
	APP_RESOURCE("listToolListLevelFollowNone",
		     offsetof(ListsPageResources,
			      lprFollowOptionTexts[DOCllfNONE]),
		     "None"),
	...
	APP_RESOURCE("listToolEditLevelText",
		     offsetof(ListsPageResources, lprEditLevelText), "Edit"),
};
