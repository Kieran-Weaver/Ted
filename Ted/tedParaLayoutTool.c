...
static void tedFormatToolRefreshSpaceAround(APP_WIDGET row, APP_WIDGET toggle,
					    APP_WIDGET text, int space,
					    int canChange)
{
	if (space == 0) {
...
static void tedFormatToolRefreshParagraphPage(ParagraphLayoutTool *plt)
{
	...
	if (pp->ppListOverride > 0) {
		appIntegerToTextWidget(plt->pltListLevelText,
				       pp->ppListLevel + 1);
	...
	appLengthToTextWidget(plt->ptFirstIndentText,
			      pp->ppLeftIndentTwips + pp->ppFirstIndentTwips,
			      UNITtyPOINTS);

	appLengthToTextWidget(plt->ptLeftIndentText, pp->ppLeftIndentTwips,
			      UNITtyPOINTS);

	guiEnableWidget(plt->ptRightIndentRow, plt->pltCanChange);
	guiEnableText(plt->ptRightIndentText, plt->pltCanChange);
	appLengthToTextWidget(plt->ptRightIndentText, pp->ppRightIndentTwips,
			      UNITtyPOINTS);
...
static APP_TXACTIVATE_CALLBACK_H(tedParaFirstIndentChanged, w, voidplt)
{
	...
	tedFormatValidateDimension(&lipfi, &changed, plt->ptFirstIndentText,
				   lipfi);
	return;
}
...
static APP_TXACTIVATE_CALLBACK_H(tedParaLeftIndentChanged, w, voidplt)
{
	...
	tedFormatValidateDimension(&value, &changed, plt->ptLeftIndentText,
...
static APP_TXACTIVATE_CALLBACK_H(tedParaRightIndentChanged, w, voidplt)
{
	...
	tedFormatValidateDimension(&value, &changed, plt->ptRightIndentText,
...
static APP_TXACTIVATE_CALLBACK_H(tedParaSpaceAboveChanged, w, voidplt)
{
	...
	tedFormatValidateDimension(&value, &changed, plt->pltSpaceAboveText,
				   pp->ppSpaceBeforeTwips);
...
static APP_TXACTIVATE_CALLBACK_H(tedParaSpaceBelowChanged, w, voidplt)
{
	...
	tedFormatValidateDimension(&value, &changed, plt->pltSpaceBelowText,
				   pp->ppSpaceAfterTwips);
...
static int tedFormatToolGetSpaceAround(int *pValue, int *pChanged,
				       APP_WIDGET text, APP_WIDGET toggle,
				       int value)
{
	int state;
	int changed = 0;
	...
	state = value;
	if (appGetLengthFromTextWidget(text, &state, &changed, UNITtyPOINTS,
...
static int tedParaLayoutGetChosen(ParagraphLayoutTool *plt)
{
	...
	if (appGetLengthFromTextWidget(plt->ptFirstIndentText, &lipfi, &changed,
	...
	if (appGetLengthFromTextWidget(plt->ptLeftIndentText, &li, &changed,
				       UNITtyPOINTS, minValue, 1, maxValue,
				       adaptToMax)) {
		return -1;
	}
	...
	if (appGetLengthFromTextWidget(plt->ptRightIndentText, &value, &changed,
				       UNITtyPOINTS, minValue, 1, maxValue,
				       adaptToMax)) {
		return -1;
	}
...
static APP_TOGGLE_CALLBACK_H(tedFormatSpaceAboveToggled, w, voidplt, voidtbcs)
{
	...
	if (set) {
		if (plt->ptPropertiesSet.ppSpaceBeforeTwips != 0) {
			...
			EditDocument *ed =
				plt->ptApplication->eaCurrentDocument;
			int lineHeight;

			if (!ed || tedGetParaLineHeight(&lineHeight, ed)) {
				XLDEB(ed, set);
			} else {
				pp->ppSpaceBeforeTwips = lineHeight;
			}
		}
	} else {
	...
	if (set) {
		if (plt->ptPropertiesSet.ppSpaceAfterTwips != 0) {
			...
			if (!ed || tedGetParaLineHeight(&lineHeight, ed)) {
				...
				pp->ppSpaceAfterTwips = lineHeight;
			}
		}
	} else {
...
static void tedParaAlignmentChosen(int alignment, void *voidplt)
{
	ParagraphLayoutTool *plt = (ParagraphLayoutTool *)voidplt;
	ParagraphProperties *pp = &(plt->ptPropertiesChosen);

	if (alignment < 0 || alignment >= DOCtha_COUNT) {
		LLDEB(alignment, DOCtha_COUNT);
		return;
	}
...
static void tedParaLineDistChosen(int how, void *voidplt)
{
	ParagraphLayoutTool *plt = (ParagraphLayoutTool *)voidplt;
				      ...
				      const InspectorSubjectResources *isr)
{
	...
	/**/
	guiToolMakeLabelAndTextRow(&(plt->pltListLevelRow), (APP_WIDGET *)0,
				   &(plt->pltListLevelText), pageWidget,
				   plpr->plprParaListLevel, textColumns, 0);

	/**/
	guiToolMakeLabelAndTextRow(&(plt->ptFirstIndentRow),
				   &(plt->ptFirstIndentLabel),
				   &(plt->ptFirstIndentText), pageWidget,
				   plpr->plprParaFirstIndent, textColumns, 1);

	appGuiSetGotValueCallbackForText(
		plt->ptFirstIndentText, tedParaFirstIndentChanged, (void *)plt);
	/**/
	guiToolMakeLabelAndTextRow(&(plt->ptLeftIndentRow),
				   &(plt->ptLeftIndentLabel),
				   &(plt->ptLeftIndentText), pageWidget,
				   plpr->plprParaLeftIndent, textColumns, 1);
	...
	guiToolMakeLabelAndTextRow(&(plt->ptRightIndentRow),
				   &(plt->ptRightIndentLabel),
				   &(plt->ptRightIndentText), pageWidget,
				   plpr->plprParaRightIndent, textColumns, 1);

	appGuiSetGotValueCallbackForText(
		plt->ptRightIndentText, tedParaRightIndentChanged, (void *)plt);
	/**/
	guiToolMakeLabelAndMenuRow(&(plt->pltAlignmentRow),
				   &(plt->pltAlignOptionmenu),
				   &(plt->pltAlignmentLabel), pageWidget,
				   plpr->pprParaAlignment,
	...
	guiToolMakeToggleAndTextRow(
		&(plt->pltSpaceAboveRow), &(plt->pltSpaceAboveToggle),
		&(plt->pltSpaceAboveText), pageWidget, plpr->pprParaSpaceAbove,
		tedFormatSpaceAboveToggled, (void *)plt, textColumns, 1);

	appGuiSetGotValueCallbackForText(plt->pltSpaceAboveText,
					 tedParaSpaceAboveChanged, (void *)plt);
	/**/
	guiToolMakeToggleAndTextRow(
		&(plt->pltSpaceBelowRow), &(plt->pltSpaceBelowToggle),
		&(plt->pltSpaceBelowText), pageWidget, plpr->pprParaSpaceBelow,
		tedFormatSpaceBelowToggled, (void *)plt, textColumns, 1);
	...
	guiToolMake2ToggleRow(&(plt->pltPageRow), pageWidget,
			      &(plt->ptOnNewPageToggle),
			      &(plt->ptKeepOnPageToggle), plpr->pprOnNewPage,
			      plpr->pprOnOnePage, tedParaOnNewPageToggled,
			      tedParaKeepOnPageToggled, (void *)plt,
			      (void *)plt);

	/**/
	guiToolMake2ToggleRow(&(plt->pltKeepRow), pageWidget,
			      &(plt->ptWidowControlToggle),
			      &(plt->ptKeepWithNextToggle), plpr->pprWidctrl,
			      plpr->pprKeepWithNext, tedParaWidowControlToggled,
...

	{
		...
		guiToolMake2BottonRow(&row, pageWidget, &(is->isSelectButton),
				      ...
				      isr->isrDeleteButtonText,
				      tedFormatSelectPara, tedLayoutDeletePara,
				      plt);

		guiToolMake2BottonRow(&row, pageWidget, &(is->isInsertButton),
				      &(is->isAppendButton),
				      isr->isrInsertButtonText,
				      isr->isrAppendButtonText,
				      tedLayoutInsertPara, tedLayoutAppendPara,
				      plt);
	}
	...
	guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
			      ...
			      tedFormatParaRevertPushed, tedLayoutChangePara,
			      plt);
				   ...
				   const ParagraphLayoutPageResources *plpr)
{
	...
	appFillInspectorMenu(DOCtha_COUNT, DOCthaLEFT, plt->pltAlignItems,
			     plpr->pprParaAlignMenuTexts,
			     &(plt->pltAlignOptionmenu));
...
static AppConfigurableResource TED_TedParagraphToolResourceTable[] = {
	...
	APP_RESOURCE("formatToolLineDistExactly",
		     offsetof(ParagraphLayoutPageResources,
			      pprParaLineDistExactly),
		     "Spacing Exactly"),
	APP_RESOURCE("formatToolLineDistAtLeast",
		     offsetof(ParagraphLayoutPageResources,
			      pprParaLineDistAtLeast),
		     "Spacing at Least"),
	/**/
...
void tedFormatToolGetParaLayoutResourceTable(EditApplication *ea,
