				...
				const LayoutContext *lc)
{
	...
	/*  5  */
	drawChildSurface(lc->lcDrawingSurface, io->ioDrawingSurface, destX0,
			 destY0, &drSrc);
}
		  ...
		  int x0Twips, int x1Twips, const LayoutPosition *baseLine)
{
	...
	x0FrameShifted = docLayoutXPixels(
		lc, dtl->dtlParagraphFrame->pfParaContentRect.drX0 +
			    dtl->dtlXShift);
	...
	if (io->ioKind == DOCokEPS_FILE && io->ioDrawingSurface) {
		tedDrawObjectPixmap(io, x0, y0, drClip, lc);
		return 0;
	}
	...
	drawSetLineAttributes(lc->lcDrawingSurface, 1, LineStyleSolid,
...
static int docOpenBitmapObject(struct DrawingSurface **pSp, RasterImage *ri,
			       ...
			       const MemoryBuffer *mb)
{
	...
	if (!ri->riBytes) {
		...
		if ((*readBitmap)(&(ri->riDescription), &(ri->riBytes),
				  sisBitmap)) {
			LDEB(1);
			rval = -1;
			goto ready;
...
static int tedOpenPixmap(struct DrawingSurface **pSp, RasterImage *ri,
...
static int tedOpenEpsfObject(const LayoutContext *lc, InsertedObject *io,
			     const MemoryBuffer *mb)
{
#if DRAW_EPSF
	...
	io->ioDrawingSurface =
		appMakePixmap(add, io->ioPixelsWide, io->ioPixelsHigh);
	if (!io->ioDrawingSurface) {
		XDEB(io->ioDrawingSurface);
		return -1;
	}
	...
	XChangeProperty(add->addDisplay, add->addDrawable, XA_GHOSTVIEW,
	...
	for (;;) {
		...
		got = sioInReadBytes(sisEps, (unsigned char *)command,
				     sizeof(command));
		if (got < 1) {
			break;
		}
		if (sioOutWriteBytes(sosGs, (unsigned char *)command, got) !=
		    got) {
			LDEB(got);
			break;
		}
			       ...
			       const LayoutContext *lc)
{
	...
	default:
		SDEB(docObjectKindStr(pip->pipType));
		rval = -1;
		break;
	}
...
int tedOpenObject(InsertedObject *io, const LayoutContext *lc)
{
	...
	if (io->ioKind != DOCokDRAWING_SHAPE) {
		if (io->ioTwipsWide < 1 || io->ioTwipsHigh < 1) {
			LLLDEB(io->ioKind, io->ioTwipsWide, io->ioTwipsHigh);
			return -1;
		}
	}

	if (io->ioPixelsWide == 0 || io->ioPixelsHigh == 0) {
		LLDEB(io->ioPixelsWide, io->ioPixelsHigh);
		return 0;
	}
	...
	case DOCokDRAWING_SHAPE: {
		...
		if (tedOpenShapePixmaps(ds, &(ds->dsShapeProperties.spRect),
					lc)) {
			LDEB(1);
			return 0;
		}
...
static int tedOpenParaObjects(BufferItem *bi, const LayoutContext *lc)
{
	int part;
	TextParticule *tp;
...
{
	if (ei->dtRoot && tedOpenNodeObjects(ei->dtRoot, lc)) {
		LDEB(1);
		return -1;
	}
...
int tedOpenNodeObjects(BufferItem *bi, const LayoutContext *lc)
{
	int i;
	...
	case DOClevSECT: {
		...
		if (shf) {
			if (tedOpenTreeObjects(&(shf->shfFirstPageHeader),
					       lc)) {
				LDEB(1);
				return -1;
			}
			...
			if (tedOpenTreeObjects(&(shf->shfRightPageHeader),
					       lc)) {
				LDEB(1);
				return -1;
			}

			if (tedOpenTreeObjects(&(shf->shfFirstPageFooter),
					       lc)) {
				LDEB(1);
				return -1;
			}
			...
			if (tedOpenTreeObjects(&(shf->shfRightPageFooter),
					       lc)) {
				LDEB(1);
				return -1;
			}
...
int tedReopenObject(TextParticule *tp, const LayoutContext *lc)
{
	InsertedObject *io;
...
int tedSaveObjectPicture(RasterImage *abiTo, const InsertedObject *io)
{
	if (!io->ioRasterImage.riBytes) {
		LXDEB(io->ioKind, io->ioRasterImage.riBytes);
		return -1;
	}
...
void docScreenCloseObject(const BufferDocument *bd, const TextParticule *tp)
{
	InsertedObject *io;
