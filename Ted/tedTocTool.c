...
static void tedTocToolRefreshFrom(TocTool *tt)
{
	...
	appGuiSetToggleState(tt->ttTocFromOutlineToggle,
...
static APP_TOGGLE_CALLBACK_H(tedTocIncludePageNumbersToggled, w, voidtt,
			     voidtbcs)
{
	TocTool *tt = (TocTool *)voidtt;
	TocField *tf = &(tt->ttPropertiesChosen);
...
static void tedTocLevel1Chosen(int level, void *voidtt)
{
	TocTool *tt = (TocTool *)voidtt;
	TocField *tf = &(tt->ttPropertiesChosen);

	if (level < 0 || level >= PPoutline_COUNT) {
		LLDEB(level, PPoutline_COUNT);
		return;
	}
...
static void tedTocSequenceChosen(int seq, void *voidtt)
{
	TocTool *tt = (TocTool *)voidtt;
	TocField *tf = &(tt->ttPropertiesChosen);
	const char *found;
...
static void tedTocUsingChosen(int type, void *voidtt)
{
	TocTool *tt = (TocTool *)voidtt;
	TocField *tf = &(tt->ttPropertiesChosen);

	if (type < 0 || type >= TOCtype_COUNT) {
		LLDEB(type, TOCtype_COUNT);
		return;
	}
...
static void tedOutlineLevelChosen(int level, void *voidtt)
{
	TocTool *tt = (TocTool *)voidtt;
...
static int tedCountSequence(const char *key, void *val, void *voidtt)
{
	TocTool *tt = (TocTool *)voidtt;
...
static int tedAddSequenceToMenu(const char *key, void *val, void *voidtt)
{
	TocTool *tt = (TocTool *)voidtt;
			  ...
			  const InspectorSubjectResources *isr)
{
	...
	appMakeColumnFrameInColumn(&(tt->ttTocFrame), &(tt->ttTocPaned),
				   pageWidget, isr->isrSubjectName);

	/**************/
	guiToolMakeLabelAndMenuRow(&row, &(tt->ttTocTypeMenu), &rowLabel,
				   tt->ttTocPaned, tpr->tprType,
				   tedTocUsingChosen, (void *)tt);

	/**************/

	guiToolMakeLabelAndToggleRow(&row, tt->ttTocPaned, &rowLabel,
				     &(tt->ttTocFromStylesToggle), tpr->tprFrom,
				     tpr->tprFromStyles,
				     tedTocFromStylesToggled, (void *)tt);

	guiToolMakeLabelAndToggleRow(&row, tt->ttTocPaned, &rowLabel,
				     &(tt->ttTocFromOutlineToggle), "",
				     tpr->tprFromOutline,
				     tedTocFromOutlinesToggled, (void *)tt);

	guiToolMakeLabelAndToggleRow(&row, tt->ttTocPaned, &rowLabel,
				     &(tt->ttTocFromTcToggle), "",
				     tpr->tprFromTcEntries, tedTocFromTcToggled,
				     (void *)tt);

	/**************/
	guiToolMakeLabelAndMenuRow(&row, &(tt->ttTocLevelsMenu), &rowLabel,
				   tt->ttTocPaned, tpr->tprLevels,
				   tedTocLevel1Chosen, (void *)tt);

	guiToolMakeLabelAndMenuRow(&row, &(tt->ttTocSeqOptionMenu), &rowLabel,
				   tt->ttTocPaned, tpr->tprSequence,
				   tedTocSequenceChosen, (void *)tt);

	/**************/
	guiToolMake2ToggleRow(
		&row, tt->ttTocPaned, &(tt->ttTocHyperlinksToggle),
		&(tt->ttTocPageNumbersToggle), tpr->tprMakeHyperlinks,
		tpr->tprIncludePageNumbers, tedTocMakeHyperlinksToggled,
		tedTocIncludePageNumbersToggled, (void *)tt, (void *)tt);

	/**************/
	guiToolMake2BottonRow(&row, tt->ttTocPaned, &(is->isInsertButton),
			      &(is->isDeleteButton), isr->isrInsertButtonText,
			      isr->isrDeleteButtonText, tedTocAddToc,
			      tedTocDeleteToc, (void *)tt);

	guiToolMake2BottonRow(&(is->isApplyRow), tt->ttTocPaned,
			      &(is->isRevertButton), &(is->isApplyButton),
			      isr->isrRevert, isr->isrApplyToSubject,
			      tedFormatRevertTocPushed,
			      tedFormatChangeTocPushed, (void *)tt);
	...
	appMakeColumnFrameInColumn(&(tt->ttOutlineFrame), &(tt->ttOutlinePaned),
				   pageWidget, tpr->tprOutlineLevel);

	guiToolMakeLabelAndMenuRow(&row, &(tt->ttOulineLevelMenu), &rowLabel,
				   tt->ttOutlinePaned, tpr->tprOutlineLevel,
				   tedOutlineLevelChosen, (void *)tt);

	guiToolMake2BottonRow(&row, tt->ttOutlinePaned,
			      &(tt->ttRevertParaButton),
			      &(tt->ttApplyParaButton), tpr->tprRevertParaLevel,
			      tpr->tprApplyParaLevel, tedTocRevertParaPushed,
			      tedTocChangeParaPushed, (void *)tt);
...
void tedTocToolFillChoosers(TocTool *tt, const TocPageResources *tpr)
{
	static const char *const levels[] = { "1", "2", "3", "4", "5",
					      "6", "7", "8", "9" };

	appFillInspectorMenu(TOCtype_COUNT, TOCtypeTOC, tt->ttTocTypeItems,
			     tpr->tprTypeItems, &(tt->ttTocTypeMenu));

	appFillInspectorMenu(PPoutlineDEEPEST + 1, 2, tt->ttTocLevelsItems,
			     levels, &(tt->ttTocLevelsMenu));

	appFillInspectorMenu(PPoutline_COUNT + 1, PPoutlineBODYTEXT,
			     tt->ttOulineLevelItems, tpr->tprOutlineLevels,
...
static AppConfigurableResource TED_TedTocToolResourceTable[] = {
	/**/
	APP_RESOURCE("tocToolType", offsetof(TocPageResources, tprType),
		     "Table of"),
	...
	APP_RESOURCE("tocToolFrom", offsetof(TocPageResources, tprFrom),
		     "From"),
	APP_RESOURCE("tocToolFromStyles",
		     offsetof(TocPageResources, tprFromStyles), "Styles"),
	...
	APP_RESOURCE("tocToolLevels", offsetof(TocPageResources, tprLevels),
		     "Style Levels"),

	APP_RESOURCE("tocToolLevels", offsetof(TocPageResources, tprSequence),
		     "Sequence"),
	...
	APP_RESOURCE("tocToolOutlineLevel1",
		     offsetof(TocPageResources, tprOutlineLevels[0]), "1"),
	APP_RESOURCE("tocToolOutlineLevel2",
		     offsetof(TocPageResources, tprOutlineLevels[1]), "2"),
	APP_RESOURCE("tocToolOutlineLevel3",
		     offsetof(TocPageResources, tprOutlineLevels[2]), "3"),
	APP_RESOURCE("tocToolOutlineLevel4",
		     offsetof(TocPageResources, tprOutlineLevels[3]), "4"),
	APP_RESOURCE("tocToolOutlineLevel5",
		     offsetof(TocPageResources, tprOutlineLevels[4]), "5"),
	APP_RESOURCE("tocToolOutlineLevel6",
		     offsetof(TocPageResources, tprOutlineLevels[5]), "6"),
	APP_RESOURCE("tocToolOutlineLevel7",
		     offsetof(TocPageResources, tprOutlineLevels[6]), "7"),
	APP_RESOURCE("tocToolOutlineLevel8",
		     offsetof(TocPageResources, tprOutlineLevels[7]), "8"),
	APP_RESOURCE("tocToolOutlineLevel9",
		     offsetof(TocPageResources, tprOutlineLevels[8]), "9"),
	...
	APP_RESOURCE("tocToolRevertParaLevel",
		     offsetof(TocPageResources, tprRevertParaLevel), "Revert"),
	APP_RESOURCE("tocToolApplyParaLevel",
		     offsetof(TocPageResources, tprApplyParaLevel),
		     "Apply to Paragraph"),
};

void tedTocToolGetResourceTable(EditApplication *ea, TocPageResources *tpr,
