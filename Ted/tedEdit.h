...
extern int tedAdjustParagraphLayout(TedEditOperation *teo, DocumentTree *dt);

extern int tedStartEditOperation(TedEditOperation *teo, SelectionGeometry *sg,
				 SelectionDescription *sd,
				 struct EditDocument *ed, int fullWidth,
				 int traced);
...
extern int tedEditFinishSelection(TedEditOperation *teo,
				  const DocumentSelection *dsNew);

extern int tedEditFinishRange(TedEditOperation *teo, int col0, int col1,
			      const EditRange *erNew);

extern int tedEditFinishPosition(TedEditOperation *teo,
				 const EditPosition *epNew);

extern int tedEditFinishOldSelection(TedEditOperation *teo);

extern int tedEditReplaceSelection(TedEditOperation *teo, const char *addedText,
...
extern int tedChangeParticuleAttributes(PropertyMask *pTaAllMask,
					...
					struct BufferItem *bi, int partFrom,
					int partUpto,
...
extern int tedDocReplaceSelectionWithField(
	...
	int *pTailPart, DocumentSelection *dsInside,
	DocumentSelection *dsAround, const FieldInstructions *fi, int fieldKind,
...
extern int tedDocFlattenTypedField(struct EditDocument *ed, int fieldType,
				   const PropertyMask *taSetMask,
				   const TextAttribute *taSet, int traced);

extern int tedReplaceSelectionWithRasterImage(struct EditDocument *ed,
					      RasterImage *ri, int traced);

extern int tedIncludeDocument(TedEditOperation *teo, DocumentCopyJob *dcj);

extern int tedEditIncludeDocument(TedEditOperation *teo, DocumentCopyJob *dcj,
				  int command, int posWhere);

extern int tedEditInsertSection(DocumentPosition *dpBeforeSplit,
				DocumentPosition *dpAfterSplit,
				TedEditOperation *teo, int split, int after);
...
extern int tedFlattenFieldImpl(TedEditOperation *teo,
			       ...
			       int headPart, int tailPart, DocumentField *df);
...
extern int tedObjectSetImagePropertiesImpl(TedEditOperation *teo,
					   InsertedObject *io,
...
extern int tedChangeNoteImpl(EditOperation *eo, DocumentField *dfNote,
			     struct DocumentNote *dn, int selInNote,
			     const PropertyMask *npSetMask,
			     const struct NoteProperties *npSet);

extern int tedDocRollRowsInTableImpl(TedEditOperation *teo,
				     const TableRectangle *tr, int move,
				     int rowsdown);

extern int tedDocUpdField(TedEditOperation *teo, DocumentField *df,
			  const FieldInstructions *fi);

extern int tedDocSetField(TedEditOperation *teo, const DocumentSelection *ds,
			  int command, int fieldKind,
			  const FieldInstructions *fi,
			  const PropertyMask *taSetMask,
			  const TextAttribute *taSet);

extern int tedMergeParagraphsInSelection(struct EditDocument *ed, int traced);

extern int tedEditShiftRowsInTable(struct EditDocument *ed, int direction,
				   int traced);

extern int tedRefreshTocField(DocumentSelection *dsAroundToc,
			      TedEditOperation *teo, DocumentField *dfToc);

extern int tedDeleteTableSliceSelection(struct EditDocument *ed, int traced);

extern int tedDocReplaceSelection(struct EditDocument *ed, int command,
				  const char *word, int len, int traced);

int tedDocReplaceSelectionTyping(struct EditDocument *ed, const char *str,
				 int length);

extern int tedDocDeleteSelection(struct EditDocument *ed, int command,
				 int traced);

extern int
tedObjectSetImageProperties(struct EditDocument *ed, InsertedObject *io,
			    const DocumentPosition *dpObject, int partObject,
			    const PropertyMask *pipSetMask,
			    const PictureProperties *pipFrom, int traced);

extern int tedInsertTable(struct EditDocument *ed, int rows, int columns,
			  int traced);

extern int tedEditStartReplace(DocumentSelection *dsTraced,
			       TedEditOperation *teo, int command, int level,
			       unsigned int flags);

extern int tedEditStartStep(TedEditOperation *teo, int command);
extern int tedEditStartTypedStep(TedEditOperation *teo, int command,
				 int fieldKind);

extern int tedFinishEditOperation(TedEditOperation *teo);

extern void tedDocInsertStringWithAttribute(struct EditDocument *ed,
					    const char *bytes, int size,
					    const TextAttribute *taSet,
					    const PropertyMask *taSetMask,
					    int traced);

extern void tedEditInsertSpecialParticule(struct EditDocument *ed, int kind,
					  int command, int redoLayout,
					  int traced);

extern int tedInsertPageNumber(struct EditDocument *ed, int traced);

extern int tedIncludePlainDocument(struct EditDocument *ed,
				   BufferDocument *bdFrom, int traced);
