...
static void tedFormatToolRefreshCellPage(CellTool *ct)
{
	...
	guiEnableWidget(ct->ctMergedToggle, ct->ctCanChange && ct->ctCanMerge);

	appRectangleToTextWidget(ct->ctRowColumnText, row0 - tr->trRow00 + 1,
				 row1 - tr->trRow00 + 1, col0 + 1, col1 + 1);
	...
	tedSetShadingToolByNumber(&(ct->ctShadingTool), bd,
				  cp->cpShadingNumber);
}
			...
			const unsigned char *cmdEnabled)
{
	...
	docSetCellRectangleProperties(&(ct->ctPropertiesSet), rowNode, tr,
				      dam0);
...
static int tedCellToolGetChosen(CellTool *ct, BufferDocument *bd)
{
...
static APP_BUTTON_CALLBACK_H(tedTableChangeCellPushed, w, voidct)
{
	...
	tedDocSetTableProperties(ed, wholeRow, wholeColumn, &cpDifMask,
				 cpChosen, (const PropertyMask *)0,
				 (const RowProperties *)0, traced);
...
static void tedCellValignChosen(int valign, void *voidct)
{
	CellTool *ct = (CellTool *)voidct;
	CellProperties *cp = &(ct->ctPropertiesChosen);

	if (valign < 0 || valign >= DOCtva_COUNT) {
		LLDEB(valign, DOCtva_COUNT);
		return;
	}
...
static void tedCellToolGotColor(void *voidct, int which, const RGB8Color *rgb8)
{
	CellTool *ct = (CellTool *)voidct;
	...
	case CLpropBOTTOM_BORDER:
		tedBorderSetExplicitColorChoice(&(ct->ctBottomBorderTool),
						rgb8);
		break;
	...
	case CLprop_SHADE_FORE_COLOR:
		tedShadeSetExplicitColorChoice(&(ct->ctShadingTool), which,
					       rgb8);
		break;
...
static void tedCellToolMakeBorderFrame(CellTool *ct,
			   ...
			   const InspectorSubjectResources *isr)
{
	...
	guiToolMakeLabelAndTextRow(&(ct->ctRowColumnRow),
				   &(ct->ctRowColumnLabel),
				   &(ct->ctRowColumnText), pageWidget,
				   cpr->cprRowColumn, textColumns, 0);

	guiToolMakeToggleAndLabelRow(&(ct->ctMergedRow), pageWidget,
				     &(ct->ctMergedToggle),
				     &(ct->ctMergedLabel), cpr->cprMerged, "",
	...
	tedFormatMakeShadingTool(&(ct->ctShadingTool), ai, pageWidget,
				 ...
				 &(cpr->cprShadingResources), subjectPage,
				 CLprop_SHADE_FORE_COLOR,
				 CLprop_SHADE_BACK_COLOR,
	...
	/**/
	guiToolMake2BottonRow(&row, pageWidget, &(ct->ctPrevRowButton),
			      &(ct->ctNextRowButton), cpr->cprPrevRow,
			      cpr->cprNextRow, tedCellPreviousRow,
			      tedCellNextRow, ct);
    ...
    */
	...
	guiToolMake2BottonRow(&(is->isApplyRow), pageWidget,
			      ...
			      tedFormatRevertCellPushed,
			      tedTableChangeCellPushed, (void *)ct);
...
{
	appFillInspectorMenu(DOCtva_COUNT, DOCtvaTOP, ct->ctValignItems,
			     cpr->cprValignItemTexts, &(ct->ctValignMenu));
	return;
}
...
static AppConfigurableResource TED_TedCellToolResourceTable[] = {
	APP_RESOURCE("tableToolCellRowColumn",
		     offsetof(CellPageResources, cprRowColumn), "Row, Column"),

	/**/
	APP_RESOURCE("tableToolCellBorders",
		     offsetof(CellPageResources, cprBorders), "Borders"),
	APP_RESOURCE("tableToolCellBorderTop",
		     offsetof(CellPageResources, cprTopBorder), "Top"),
	APP_RESOURCE("tableToolCellBorderBottom",
		     offsetof(CellPageResources, cprBottomBorder), "Bottom"),
	APP_RESOURCE("tableToolCellBorderLeft",
		     offsetof(CellPageResources, cprLeftBorder), "Left"),
	APP_RESOURCE("tableToolCellBorderRight",
		     offsetof(CellPageResources, cprRightBorder), "Right"),

	APP_RESOURCE("tableToolCellMerged",
		     offsetof(CellPageResources, cprMerged), "Merged"),

	/**/
	APP_RESOURCE("formatToolCellBorderWidth",
		     offsetof(CellPageResources,
			      cprBorderToolResources.btrWidth),
		     "Width"),
	APP_RESOURCE("formatToolCellBorderStyle",
		     offsetof(CellPageResources,
			      cprBorderToolResources.btrStyle),
		     "Style"),
	APP_RESOURCE("formatToolCellBorderColor",
		     offsetof(CellPageResources,
			      cprBorderToolResources.btrColor),
		     "Color"),
	APP_RESOURCE("formatToolCellBorderColorChooserAutomatic",
		     offsetof(CellPageResources,
			      cprBorderToolResources.btrColorChooserResources
				      .ccrAutomaticColor),
		     "Automatic"),
	...
	APP_RESOURCE("formatToolCellShading",
		     offsetof(CellPageResources, cprShading), "Shading"),

	APP_RESOURCE("formatToolCellShadingPattern",
		     offsetof(CellPageResources,
			      cprShadingResources.strPattern),
		     "Pattern"),
	...
	APP_RESOURCE("formatToolCellShadingForeColor",
		     offsetof(CellPageResources,
			      cprShadingResources.strForeColor),
		     "Foreground"),
	APP_RESOURCE("formatToolCellShadingBackColor",
		     offsetof(CellPageResources,
			      cprShadingResources.strBackColor),
		     "Background"),

	/**/
	APP_RESOURCE("formatToolCellShadingBackColorChooserNoBackground",
		     offsetof(CellPageResources,
			      cprShadingResources.strBackColorChooserResources
				      .ccrAutomaticColor),
		     "No Background"),
	...
	APP_RESOURCE("formatToolCellShadingForeColorChooserAutomatic",
		     offsetof(CellPageResources,
			      cprShadingResources.strForeColorChooserResources
				      .ccrAutomaticColor),
		     "Automatic"),
	...
	APP_RESOURCE("tableToolCellValignCenter",
		     offsetof(CellPageResources,
			      cprValignItemTexts[DOCtvaCENTERED]),
		     "Center"),
	APP_RESOURCE("tableToolCellValignBottom",
		     offsetof(CellPageResources,
			      cprValignItemTexts[DOCtvaBOTTOM]),
		     "Bottom"),

	APP_RESOURCE("cellToolNextRow", offsetof(CellPageResources, cprNextRow),
		     "Next Row"),
	APP_RESOURCE("cellToolPreviousRow",
		     offsetof(CellPageResources, cprPrevRow), "Previous Row"),
};
